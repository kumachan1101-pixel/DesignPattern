...
- **記事**：`note/output/articles/article-002.md`
- **タイトル**：【ソフトウェア設計】`Manager` クラスが太り続けるのは「抽象で逃げていた」からだった
- **タグ**：ソフトウェア設計 / クラス設計 / プログラミング / エンジニア
...
【条件】
- サイズ：横長（16:9）
- テーマ：ソフトウェア設計・クラス設計・責任の分離
- スタイル：ダークトーン、プロフェッショナル、テクノロジー感のある抽象的なデザイン
- テキストは入れない（タイトル文字は不要）
- 太りすぎたクラス・責任が集まりすぎたイメージ → 整理・分割されるビジュアル
- 色調：ネイビー・ダークブルー系をベースにオレンジ・アンバーのアクセント
```
...
5. ユーザー承認後、以下の方法A→B→C の順で画像をダウンロードする：
   ...
   **方法A：JavaScript で base64 変換して保存**
   ...
   with open('note/output/covers/cover-002.png', 'wb') as f:
   ...
   **方法B：Gemini のダウンロードボタンを使う**
   ...
   → computer ツールで note/output/covers/cover-002.png に移動
   ```
   ...
   **方法C：画像 URL を取得して Python でダウンロード**
   ...
   urllib.request.urlretrieve('IMAGE_URL', 'note/output/covers/cover-002.png')
   print('保存完了')
   "
   ```
...
python3 note/api/post_article.py \
  --title "【ソフトウェア設計】\`Manager\` クラスが太り続けるのは「抽象で逃げていた」からだった" \
  --body-file "note/output/articles/article-002.md" \
  --tags "ソフトウェア設計,クラス設計,プログラミング,エンジニア"
```
...
3. `note/output/covers/cover-002.png` をアップロード（wait 5秒）
...
完了後、このファイルの先頭行を `# [done] 🚀 実行タスク：記事002をNoteに投稿する` に変更して保存すること。
