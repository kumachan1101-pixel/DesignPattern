...
利用者（Unit C）はポインタを持つことはできますが、その先の実装（Unit B）を直接覗くことはコンパイルエラーになります。これを物理的な **アクセス不可** の状態と呼びます。

UnitA（ヘッダ）を中心に、UnitC（利用者）とUnitB（実装）が依存していますが、利用側から実装側への直接アクセスは遮断されています。
...
前述のような物理的な依存の遮断（カプセル化）は、単に再コンパイルを防ぐだけではなく、実行環境全体に影響を及ぼす「安定した境界線」を作り出します。ここで重要になるのが、不完全型がもたらす **ABI互換性** という概念です。
...
構造体の実体が公開されていると、外部モジュールはコンパイル時にそのサイズ分の「メモリの枠（スタック領域）」を確保します。しかし、共有ライブラリ側のアップデートで構造体のメンバが増え、サイズが大きくなった場合、外部モジュールが用意した「古い小さな枠」に対して、ライブラリが「新しい大きなデータ」を書き込もうとします。これが境界を越えた **メモリ破壊（スタック壊し）** を招き、予期せぬクラッシュやセキュリティ上の弱点の原因となります。
...
*   **バイナリレベルの絶縁**　ポインタのサイズは環境（32bit/64bit）によって決まっており、構造体の中身が増減しても変わることはありません。
*   **互換性の維持**　内部構造をいくら変えても「窓口」であるポインタの扱いは変わらないため、外部モジュールを再コンパイルすることなく、実行バイナリの差し替えのみで修正を適用できます。
...
最初は **PIMPLパターン（Pointer to IMPLementation）** です。構造体の内部メンバをヘッダから完全に隠蔽することで、実装の変更がクライアント側の再コンパイルを引き起こさない設計を実現します。

#### システム概要

モジュールのヘッダファイルにはインターフェース（API）のみを公開し、データ構造の具体的な定義（メンバ変数など）を実装ファイル（.c）内に完全に隠蔽するケースです。

#### 設計課題

C言語において、構造体の定義（`struct { int a; float b; };`）をヘッダファイルに記述して公開してしまうと、モジュールを利用するあらゆるクライアントコードが、その内部の変数に直接アクセスできてしまいます。さらに深刻な問題として、構造体にメンバ変数を1つ追加しただけで構造体のサイズ（バイナリレイアウト）が変わるため、そのヘッダをインクルードしている何百ものファイルすべてを「再コンパイル」しなければならなくなり、開発効率が著しく低下します。

#### 改善の指針

この不要な依存と再コンパイルの連鎖を断ち切るための強力な技法が、 **PIMPL (Pointer to Implementation) パターン** です。ヘッダには「構造体の名前だけ（不完全型）」と「そのポインタ（不透明ポインタ）」を残し、中身の完全な定義は実装ファイル（`context.c`）に移動させます。これにより、変更の波及を実装ファイル内に物理的に隔離し、クライアント側には一切影響を与えない（ABI互換性の維持）という強固なカプセル化を実現します。

#### PIMPLパターンの構造

公開ヘッダにはAPIとポインタ型のみを配置し、構造体の定義（完全型）を実装ファイルに閉じ込める、PIMPLパターンの典型的な構成例です。

クライアントはAPI経由でのみメンバにアクセス可能です。直接 `ctx->value` のように書くことはできず、 **メンバへのアクセス** は厳格に制限されます。

クライアントからAPIへの依存は実線（アクセス可能）で示され、完全な型定義への依存は点線（アクセス不可）で示されています。
...
int context_get_value(const Context_t* context);
void context_destroy(Context_t* context);
```

> [!INFO] 読者の疑問：不完全型を使うと依存関係が軽減される？
> 「不完全型を使うと、実態を知る必要はないため、実態が定義されたファイルの `include` は不要になり、依存関係が軽減されるということですか？」
>
> **はい、その通りです。**
>
> ヘッダファイル（`.h`）に構造体の実態（メンバ変数）をあえて書かず、不完全型として宣言だけしておくことで、そのヘッダを `include` する利用側のファイルは「構造体の中身がどうなっているか」に依存しなくなります。
> もし構造体のメンバを変更（追加や削除）したとしても、ヘッダファイル自体は変わらないため、利用側の `.c` ファイルを再コンパイルする必要がなくなります。これが「依存関係が軽減される」という最大のメリットです。
...
Context_t* context_create(void)
{
    ...
    Context_t* context = (Context_t*)calloc(1, sizeof(struct Context));

    if (context == NULL) return NULL;
    // 明示的な初期値が必要なものだけ設定
    context->internal_state = 42;
...
>
> **はい、スタックに置けなくなります。これは意図的なトレードオフです。**
>
> 不完全型を採用するとコンパイラはサイズを計算できないため、利用側はスタック上にその構造体を直接割り当てる（値として持つ）ことが不可能になります。常にポインタ越しに扱い、モジュールの生成関数内部で `malloc` 等による動的確保を行う（あるいは専用のアロケータを使う）という強烈な制約・トレードオフが生じます。しかし、中・大規模なシステム開発においては、この「スタック確保の利便性を捨てる」対価を払ってでも得るべき、圧倒的な「依存関係の遮断（モジュール間結合度の低下と、再コンパイル連鎖の防止）」というメリットが存在するのです。


### 2.2. 活用パターン 2: Setter関数によるデータの整合性チェック（防御的アクセス）

外部からの直接アクセスを禁止し **Setter関数を通じてのみ値を変更させる** パターンです。設定値の範囲チェックや整合性検証をSetter内に集約することで、不正な状態への遷移をAPIの境界で確実に食い止めます。

#### システム概要

カプセル化された内部状態に対して、直接の代入を禁止し、専用のSetter関数を通してのみ値の更新を許可するシステムケースです。入力値の妥当性を関数内でチェックすることで、不正な状態を未然に防ぎます。

#### 設計課題

PIMPLパターン等でデータ構造を隠蔽しても、「値を更新する手段」を提供しなければシステムは機能しません。しかし、更新処理において何のチェックも行わず、単純に値を代入するだけの状態にしてしまうと、モジュールが前提としている「マイナス値は入らない」「最大100まで」といった不変のルール（不変条件）が容易に破壊され、予期せぬクリティカルバグに直結します。

#### 改善の指針

このセキュリティ上の弱点を塞ぐため、不透明ポインタの「構造体の中身が直接見えない」という制約を最大限に活かし、 **Setter関数** を唯一の書き込み経路として強制します。モジュールが管理する **データの整合性（不変条件）** を確実に保護するためです。Setter関数内でバリデーション（妥当性検証）を行うことで、不正な値の混入を入り口で弾く「防御的アクセス」を実現する仕組みです。

Setter関数がバリデーション（Judge）を行い、不適切な値（Failure）を弾くことで、内部状態の整合性を保つための **検問** として機能します。
...
// Setter関数の宣言
int context_set_value(Context_t* context, int new_value);
...
int context_set_value(Context_t* context, int new_value)
{
    ...
    // 3. 検証成功時のみ状態を更新
    context->internal_state = new_value;
...
void use_context_module_with_setter(void)
{
  ...
  // 1. 成功する書き込み (有効な値)
  int test_val_1 = 50;
  int result_ok = context_set_value(my_ctx, test_val_1);
  printf("[client.c] 設定結果 (%d): %d, "
         "現在値: %d\n",
         test_val_1,
         result_ok,
  ...
  // 2. 失敗する書き込み (無効な値: 範囲外)
  int test_val_2 = 200;
  int result_fail = context_set_value(my_ctx, test_val_2);
...
### 2.3. 活用パターン 3: 不透明データ構造へのVTable（振る舞い）の内包

不透明ポインタで隠蔽した構造体の中に関数ポインタ（VTable）を組み込むパターンです。第2章で学んだVTableパターンを応用し、データと振る舞いの両方をカプセル化したより完成度の高い抽象インターフェースを構築します。

#### システム概要

データ構造（状態）の完全な定義を隠蔽しつつ、その内部にVTable（関数ポインタの集合＝振る舞い）を持たせることで、状態と振る舞いを一つの不透明ポインタにカプセル化する設計ケースです。

#### 設計課題

第2章で「VTableによる動的結合」を学びましたが、VTable単体では「独自の内部状態（データ）」を持つことが困難でした。一方、前節（パターン1）のPIMPLを使えば状態は隠蔽できますが、今度は「振る舞いの動的切り替え」ができません。実務において、状態を安全に隠蔽しつつ、振る舞いを外部から動的に注入したい（ポリモーフィズムを実現したい）場合、これら別々のテクニックをどのように統合するかが課題となります。

#### 改善の指針

この課題を解決するため、データ構造自体を不透明ポインタで隠蔽し、その内部メンバとしてVTable（振る舞い）へのポインタを内包させます。データ（状態）と振る舞いの両方を一つのポインタで完全にカプセル化することで、 **依存性逆転原則 (DIP)** を物理的に実現します。
...
ClientPtrからは中身が見えませんが、内部的にはVTable（振る舞いへのポインタ）を経由してoperation_a（具体的な関数）にアクセスできる仕組みが整っており、これによって **カプセル化の徹底** が図られています。
...
#### object.c (デリゲートを利用した生成関数の実装（VTableの注入）)
```c
Object_t* object_create(const VTable_t* vtable_impl)
{
...
void object_do_something(Object_t* self)
{
    ...
    if (self != NULL && self->vtable != NULL
        && self->vtable->operation_a != NULL) {
        // 関数ポインタを経由した間接呼び出し（動的ディスパッチ）
        // selfを渡すことで、実装側が状態にアクセスできるようにする
        self->vtable->operation_a(self);
    }
}
```
...
int object_get_state(const Object_t* self)
{
    ...
    return self->internal_data_state;
}
```
...
> [!NOTE] 補足：ストラテジー（Strategy）パターンとは？
> アルゴリズム（振る舞い）をそれぞれ個別のオブジェクトに切り出してカプセル化し、それらを同じインターフェースを通じて呼び出し時に交換可能にする設計パターンのことです。ここでは `CONCRETE_A_VTABLE` という具体的な振る舞いをライブラリ側に注入（セット）することで、ライブラリの中から利用側のロジックを呼び出させる「制御の反転（IoC）」を実現しています。
...
void concrete_a_operation(Object_t* self)
{
    // ポイント: Objectの中身は隠蔽されているが、公開アクセサを通じて
    // 安全に状態を取得し、ロジックに使用することができる。
    int state = object_get_state(self);
...
#### client.c (VTableを注入するコンポジションルートの構築)
```c
...
ライブラリをアップデートした際、利用側のバイナリを再コンパイルせずにそのまま動かしたい場合、構造体のサイズが変わることは許されません。このための対策として、不完全型を使用することが挙げられます。不完全型を使用すれば、外部から見えるサイズは常に「ポインタのサイズ」で固定されるためです。

内部構造をいくら変えても「窓口」は不変なため、バイナリレベルの絶縁が可能になります。

#### メモリ管理責任の集中

実体がヒープに確保されるため、解放を忘れると即座にメモリリークに繋がります。また、スタックに配置できないため、関数の終了とともに自動破棄される利便性が失われます。これを防ぐ対策として、責任の所在を明確にすることが必須です。Entity Objectのライフサイクル管理責任をファクトリ（`create`）とデストラクタ（`destroy`）に集約し、クライアントにそのペアの履行を契約として要求するように設計してください。

## 本章で必ず理解してほしいことのまとめ

#### 1. 「情報隠蔽」の究極的な実現手段である (不完全型と不透明ポインタの設計上の役割)
...
しかし、これらの強力な道具も、 **「どのファイルに、どう配置するか」** という **物理的な設計図** がなければ、大規模なシステムはすぐに崩壊してしまいます。次章では、これまでの技術を総動員し、実際の開発において最も重要となる **第5章 モジュール構成とヘッダ設計 - 最小限の契約公開と依存の最小化** について解説します。
