...
前項の「生成時注入」がモジュールのライフサイクル全体にわたって依存を保持するのに対し、ここでは **関数を呼び出すその瞬間だけ** 、一時的に「今回使う振る舞い」を外から渡す手法を実践します。呼び出しごとに異なる戦略を注入できるため、データ変換やソートなどの動的な切り替えが求められる場面で特に威力を発揮します。
...
#### data_processor.c (変更の理由が異なる機能が同居するアンチパターン)
```c
...
int main(void) {
    ...
    //  **呼び出し側も「数字（特定の決め打ちの数値）」で具象を指定せざるを得ない**
...
#### ✅ 原則適用後：引数注入により動的な動作切り替えを可能にした設計
...
#### itransform.h (変換という振る舞いだけを定義した抽象的な契約 - 抽象契約)
```c
#ifndef ITRANSFORM_H
#define ITRANSFORM_H
// 変換ロジックの型定義
...
#### concrete_transforms.c (契約に従い詳細を隠蔽したAES変換の実装)
```c
...
static char* aes_impl(void* context, const char* data) {
    ...
    int key_len = context ? *(int*)context : 256;
...
#### concrete_transforms.c (契約に透過的に従うZIPおよびNoOp変換の実装)
```c
// --- ZIP実装 ---

static char* zip_impl(void* context, const char* data) {
    // contextから圧縮レベルを取得する例
    int level = context ? *(int*)context : 6;
    char* res = malloc(strlen(data) + 32);
...
#### concrete_transforms.c (契約に透過的に従うNoOp変換の実装)
```c
// --- NoOp実装（変換なし） ---
static char* noop_impl(void* context, const char* data) {
    // contextをロギング用のプレフィックスとして使用する例
    const char* prefix = context ? (const char*)context : "";
...
#### data_processor.c (戦略を受け取り契約に従って処理を委譲するプロセッサ)
```c
#include "itransform.h"
#include <stdio.h>
#include <stdlib.h>
...
#### main.c (関数呼び出しごとに戦略を動的に切り替える利用者)
```c
#include "itransform.h"
#include <stdio.h>
...
ここでは、オブジェクトの **生成後に依存先を動的に切り替える** 「セッター（プロパティ）注入」を実践します。生成時には依存を決めず空のまま初期化し、後からセッター関数を通じて抽象を注入・交換できる設計です。通知機能のON/OFF切り替えや、実行中の設定変更が求められる場面に最適なパターンです。
...
この要件に対し、DIPの概念を持たずに実装すると、設定用の関数を通じて「どの通知手段を使うか」を示す特定の数値（例：1ならEmail、2ならSMS）と、「具象のポインタ」を強引に受け取るという、非常にもろい設計になりがちです。
...
#### ❌ 原則適用前：具象の通知手段をすべて知り尽くしたサービス

まずはセッター注入を適用する前の、DIPに違反した（具象に依存した）最悪のコードから見てみましょう。設定関数が特定の数値を要求し、それをもとに処理が分岐する密結合の典型例です。
`strdup` は、引数に渡した文字列を **「新しくメモリを確保して、そこに丸ごとコピーする」** という関数です。
#### user_service.c (具象の通知手段をすべて知り尽くしたサービス - DIP違反)
```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#### user_service.c (具象のポインタとタイプを強引に渡すセッター)
```c
...
利用側も最悪です。どの手段を使うかを特定の数値（1や2など）で指定し、ポインタを自ら型変換（キャスト）して渡すという、本来隠蔽されるべき内部構造に深く関与する羽目になっています。

#### user_service.c (数値指定とポインタ変換に苦しむ利用者)
```c
int main(void) {
...
通知手段を変更するために、ソースコードの修正（再コンパイル）が必要な状態です。
...
#### ✅ 原則適用後：セッターを用いた事後的な依存注入による柔軟な構成
...
#### inotifier.h (通知という振る舞いを抽象化した契約 - 抽象契約)
```c
#ifndef INOTIFIER_H
#define INOTIFIER_H
// 通知ロジックの型定義
...
#### notifiers.c (契約に従うメールおよびSMS通知の実装)
```c
#include "inotifier.h"
#include <stdio.h>
#include <stdlib.h>
...
#### user_service.c (不透明ポインタで内部状態を完全に隠蔽したサービス)
```c
#include "inotifier.h"
#include <stdio.h>
#include <stdlib.h>
...
#### user_service.c (実行中にいつでも抽象を交換できるセッター)
```c
...
void register_user(struct UserService* service) {
    ...
    if (service->notifier) {
        ...
        service->notifier->notify(service->notifier->context, "Welcome!");
    }
}
...
#### inotifier.h (上位モジュールが使用するAPIだけを定義した公開ヘッダ - 外部公開APIのイメージ)
```c
#ifndef INOTIFIER_H
#define INOTIFIER_H
...
生成したサービスに対し、実行中に通知元の実装を切り替える例です。

最後に、これらをつなぎ合わせる `main` 関数です。まずはサービスを生成し、直後は「通知なし」の状態で動作を確認。その後、メール通知モジュールを生成して「セッター注入」を行います。

#### main.c (安全に古い通知器を破棄し新しいものを注入する利用者)
```c
#include "inotifier.h"
#include <stdio.h>
...
#### main.c (システムを止めずに依存先を切り替えるセッター注入)
```c
...
章の冒頭（実践パターン1の直前）で触れた通り、これら3つのパターンは「いつ・どのように注入するか」が明確に異なります。ここまで実装手法を見てきたことで、それぞれの使いどころがより鮮明になったはずです。

| パターン名                    | 注入するタイミング      | 主な用途・使い分けの基準                                                   |
| ------------------------ | -------------- | -------------------------------------------------------------- |
| **コンストラクタ注入**<br>（生成時注入） | オブジェクト **生成時** | **【必須の依存】**<br>生成時に不整合を防げるため、DB接続やロガーなど基本はこのパターンを採用します。        |
| **メソッド注入**<br>（関数引数注入）   | 関数 **呼び出し時**   | **【戦略の切り替え】**<br>アルゴリズムの動的な変更など、呼び出しごとに異なる振る舞いを適用したい場合に使用します。  |
| **プロパティ注入**<br>（セッター注入）  | 生成後の **任意時点**  | **【動的な変更】**<br>依存が任意（Null可）の場合や、実行中にシステムを止めずに設定を変更したい場合に使用します。 |

> [!INFO] 読者の疑問：DIPとOCPって、結局やっていることは同じでは？
> 「第9章のOCPでも関数ポインタやVTableを使って『既存コードを修正せずに拡張する』という話でしたよね。DIPでもVTableを使って『抽象に依存させる』。やっていることが同じに見えるのですが、何が違うんですか？」
>
> **いいえ、道具（VTable）は同じでも、両者は「何を守りたいか」という着眼点が明確に異なります。**
>
> 確かに、どちらもVTable（関数ポインタ構造体）という同じ **道具** を使います。しかし、両者は **着眼点（何を守りたいか）** が明確に異なります。
> **OCPの関心事は「拡張性」** です。「新しい種類のログ出力先を追加したいとき、既存のコードを修正しなくて済む構造か？」という問いに答えます。つまり、未来の機能追加に対してコードを **開いておく** ことが目的です。
> 一方、 **DIPの関心事は「依存の方向」** です。「上位の重要なビジネスロジックが、下位の変わりやすい実装の都合に振り回されていないか？」という問いに答えます。その結果として得られるのは、 **テスト容易性**（本物の代わりにモックを注入できる）と **移植性**（ハードウェアやOSに依存するコードを差し替えられる）です。
> 実務では、OCPを意識して設計すると自然にDIPも達成されることが多く、その逆もまた然りです。しかし「なぜこの構造にするのか」を問われたとき、 **「拡張のため」なのか「依存の方向を正すため」なのか** を区別して説明できることが、設計者としての力量の証となります。
...
#### **mock_database.c（テスト用モック）** (モックの実装例)
...
#### mock_database.c (呼び出し回数やクエリ内容を記録するテスト用モック)
```c
#include "idatabase.h"
#include <stdio.h>
#include <stdlib.h>
...
#### mock_database.c (契約を遵守してモックを生成するファクトリ)
```c
...
#### test_business_logic.c (モックの中身を覗き見して検証するテスト準備)
```c
#include "idatabase.h"
#include <stdio.h>
#include <stdlib.h>
...
> [!INFO] 補足：`assert` による自動検証
> テストコード内で使用されている `assert(条件式)` は、与えられた条件が「真（期待通り）」であれば何もせず次の処理へ進み、「偽（期待と異なる）」であればエラーメッセージを出力してプログラムを強制終了させるマクロ機能です。
> これにより、目視での確認不要で、意図した通りの処理結果になっているかを確実に自動検証することができます。

ネットワークもデータベースも必要ありません。外部環境に一切依存しないこの「閉じたテスト」は、CI（継続的インテグレーション）環境で何万回実行されても、常に数ミリ秒で安定した結果を返し続けます。

#### test_business_logic.c (外部環境に一切依存しない超高速なテストコード)
```c
...
「全てのテストが成功しました！」と表示されているのは、コード内の `assert` 文がすべて真となり、プログラムが異常終了しなかった（＝検証がすべてパスした）ことを意味します。
