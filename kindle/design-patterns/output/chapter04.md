...
> **手元で動かすには**
> このコードは1つの `.cpp` に貼り付けて、そのままコンパイル・実行できます（例：`g++ -std=c++14 chapter04.cpp -o app && ./app`）。CSVの中身は `SampleFileStore` の登録済み文字列として与え、実ファイルは読み書きしません。「自由にCSVを作る」とは、`SampleFileStore::get()` が返す行一覧を追加・変更することです。たとえば列不足の行（`"9001,欠損"` のようにカンマ項目が2つしかない行）を登録データへ足せば、その行はパース時に読み飛ばされ、保存件数が1件減った実行結果に表れます。実ファイルI/OやDB保存はこの章の論点ではないため扱わず、メモリ上のサンプルCSVで代替します。
