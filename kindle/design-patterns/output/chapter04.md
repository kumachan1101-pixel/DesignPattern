...
詳細な仕様やコードへ入る前に、1-4（実装コード）の`main()`で直営店とFC店の売上CSVを順にインポートする2ケースを確認します。呼び出しと、その呼び出しが生む結果をケースごとに並べます。
...
10件をDBへ保存
ファイルを閉じる
直営店データ インポート成功: 10件保存
```

続けて、同じ`main()`でFC店フォーマットを処理します。

```cpp
    // 2回目：同じ入口へ、タブ区切り・必須3列・5件のFC店形式を通す
...
2つの結果を対応させて読むと、どちらも**「開く → 読み込む → 保存する → 閉じる」**の順で進んでいます。区切り文字の違いは、1回目の「カンマ区切り」と2回目の「タブ区切り」という出力で直接確認できます。10件と5件は今回の入力データ量の違いであり、差し替える処理そのものではありません。不正行判定の結果はスキップ件数として出ていますが、判定処理の位置と内容は後続のコードで確認します。

この入力と出力から、(1)形式ごとのCSVを受け取り、(2)開く→形式別に読み込む→保存する→閉じるの順に進み、(3)保存件数とスキップ件数が表示される、という一連の動きが読み取れます。同じ入力を含む完全なコードと実行結果は1-4に掲載します。
...
|---|---|---|---|
| 直営店 | カンマ区切り | あり（スキップ） | 不正行をスキップし、件数を報告する |
| FC店 | タブ区切り | なし | 不正行をスキップし、件数を報告する |
...
| 区切り文字  | `,`                 | タブ                     |
| 不正行    | スキップして件数を報告する | スキップして件数を報告する（上の例では1件） |
| 取り込み対象 | `1001`、`1002` の2件   | `2001`、`2002` の2件      |

「ヘッダー行のスキップ」は、CSVファイルの1行目に「商品名,数量,金額」のような列名が書かれている場合、それをデータとして読み込まないための処理です。一方FC店では、ヘッダーなしで最初の行から商品データが続きます。「不正行のスキップ」は、項目が3つに分割できない行を保存対象へ入れず、次の行へ進むことです。現状コードは両形式とも不正行を保存せず、スキップ件数を取り込み結果で報告します。
...
| 直営店CSVで3項目に分割できない行がある | 直営店の解析時 | その行をスキップし、スキップ件数を報告 | **実際に発生**：不正行を含むサンプルで再現し、正常行だけ取り込む |
...
正常系・境界ケースを基準にします。直営店とFC店はどちらも3項目に分割できない行を保存せず、スキップ件数を呼び出し元へ返します。

| 入力ファイル     | フォーマット種別       | データの状態         | 期待する出力        |
| ---------- | -------------- | -------------- | ------------- |
| 直営店CSVファイル | カンマ区切り・ヘッダー行あり | 正常データ10件       | インポート成功、10件保存 |
| FC店CSVファイル | タブ区切り・不正行スキップ  | 正常データ5件        | インポート成功、5件保存  |
| 直営店CSVファイル | カンマ区切り・ヘッダー行あり | 空ファイル（ヘッダー行のみ） | 0件インポート、エラーなし |
| FC店CSVファイル | タブ区切り・不正行スキップ | 全行不正データ | 0件インポート、エラー件数を報告 |
| 直営店CSVファイル | カンマ区切り・ヘッダー行あり | 正常1件＋不正1件 | 正常1件だけ保存、スキップ1件を報告 |
---
...
スキーマは1-1（このシステムの仕様）で示した2種類（`store` / `fc`）です。`main()` と実行結果は最後に、行のまとまりごとに並べます。上から順に連結すれば、そのまま1つのC++14プログラムとして動きます。
...
保存メッセージはDB保存の代替です。掲載コードでは、実行中のメモリへ保存します。
...
行1と同じ順序で進み、形式固有の違いは「カンマ区切り／タブ区切り」とヘッダーの有無です。保存処理はどちらも同じ「保存」として扱います。
...
| --- | --- | --- |
| 直営店CSVファイル | 正常データ10件 | インポート成功、10件保存（変わらず） |
| FC店CSVファイル | 正常データ5件 | インポート成功、5件保存（変わらず） |
...
2-3（関係者ヒアリング）ではこの三点と、保存先の変更可能性を確認します。
...
2-4（ヒアリングで判明した将来リスク）のリスクIDを、形式ごとに変えられるようにする部分と、全形式で守る取込手順へ分けます。「はい」は、フェーズ6で**形式差分が増減しても共通手順を修正せずに済む構造か**を判定するための印です。
...
したがって2-5（変わる見込みと今回維持する範囲を確定する）の出力は、「形式固有の解析・検証・計算は変えられるようにし、取込手順と結果契約は守る」という設計条件です。フェーズ3「問題特定」では変更ID1・変更ID2だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
int main() {
    ...
    cout << "インポート成功: " << r.saved << "件保存、スキップ" << r.skipped << "件\n";
...
**動作は正しくなっています。** ただし冒頭の「EC店CSVを開く」と末尾の「DBへ保存」「ファイルを閉じる」は、直営店・FC店にべた書きした手順と全く同じ形です。
...
1つ目は、同じ修正の繰り返しです。具体的には、ファイルを開く・保存する・閉じるという手順が、`StoreDataImporter`・`FCDataImporter`・`ECDataImporter` の3クラスそれぞれの `import()` に `cout` でべた書きされています（1-4と3-1（変更を試みる）のコードで確認できます）。「バージョンチェックを追加してほしい」という変更ID2をここへ当てると、同じ1行を3箇所へ繰り返し足すことになります。共通の手順であるはずのファイル操作やDB保存のコードが形式ごとに複製されているため、関連クラスを検索し、修正漏れがないか広く確認する必要があります。
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 形式ごとの解析・行検証は `StoreDataImporter`・`FCDataImporter`・`ECDataImporter` の3クラスに分かれて現れます。そして2-4（ヒアリングで判明した将来リスク）のリスクID1が「SNS販売データの形式追加」、リスクID2が「データ項目の追加・計算ルール変更」を挙げています。**現に変更要求で、EC店という形式が1つ増えました。** 形式ごとの差分を同じ問い方で呼べるクラス契約まで分けます。
...
**変更前から抜き出す箇所：`ECDataImporter::import()`** ―― 3-1（変更を試みる）の `import()` から、変更要求で追加したEC店のケース（対策前）
...
**5-3（課題IDと接続点を確定する）が挙げた候補は4つです。** 取得行、検証済み行、保存件数、取込結果について、境界を流す形を1つずつ確定します。
...
3クラスを並べると、1-2（動作例テーブル）の形式仕様がそのままクラスの一覧になります。
...
**中身は1-4（実装コード）のままです。** `ImportFileGateway` の内側は1-4の `SampleFileStore` と同じ `map<string, vector<string> >`、`save()` も1-4と同じ件数表示です。**やったのは、3クラスに散っていた同じ処理を型として名前を付けただけ**で、動きは変えていません。
...
ここまでの決定を、課題ID1の骨格側と差分側に分けて一望へまとめます。この表は設計課題だけを追います。変更要求の受入はフェーズ7の要求ID表、変更影響は7-4（変更シナリオ表）の変更ID表で別に確認します。
...
フェーズ2のリスクIDを採用構造へ再適用し、共通手順をどこまで守れ、どの差し替え点に弱点が残るかを評価します。
...
`SchemaRegistry` は1-4（実装コード）の2種類（store / fc）に、変更要求のEC店（ec）が加わり、ecには会員ランク（memberRank）とポイント（point）の列が増えています。**戻り値の型はすべて `void` をやめました。** `import()` は `ImportResult`（種別・スキーマ名・保存件数・スキップ件数・成否）を、`parseData()` は `vector<ParsedRow>`、`validateRows()` は `ValidationResult` を返します。
...
境界と3形式のImporterを組み立て、同じ `import()` 骨格を順に実行します。実行例の範囲は、今回の要求に含まれる3形式の取込入口です。
...
回帰1〜3は、1-2（動作例テーブル）の動作例と変更ID2の受入条件を完成コードで確認するためのシナリオです。回帰1と回帰2は、フェーズ1から続く「空ファイルでも手順を最後まで通す」「不正行はスキップして件数と理由を返す」という既存動作が、骨格を基底クラスへ移した後も変わっていないことを示します。読む先を差し替えただけで骨格もパースも共有しているため、確認のために書き足したのは `filePath()` の1メソッドだけです。
...
言い換えると、この章で「変わってほしくない」と置いたのは**手順の順序**であって、**保存媒体**ではありません。保存媒体は最初からゲートウェイの裏に隔離してある、というのが7-1（解決後のコード）の構造です。
...
フェーズ3で確認した「EC店形式の追加」のシナリオを、3-2（変更影響グラフ）と同じ粒度で再度適用します。
