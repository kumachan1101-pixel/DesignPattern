...
|:---:|:---|:---|
| **具体×直接**（← 現在地） | ライトニング直生え → ゲーム機（直差し） | `StoreDataImporter` / `FCDataImporter` が `openFile()` / `saveToDB()` を内部に直書きしている |
| **具体×間接** | ライトニング直生え → ゲーム機用アダプタを挟む → ゲーム機 | 共通手順を担う中継クラス（`ImportProcessor`等）を置き、具体クラスがそれを呼び出す |
