...
**この形の考え方：**
インポート処理を店舗ごとにクラスとして切り出し、呼び出し側はその具体クラスを名指しで知りながらも、処理そのものはそのオブジェクトに委ねる（間接）形です。自分で直接処理しない——この「委ねる」という一歩が、案1との違いです。責任の境界は引かれますが、新しい形式の追加時には呼び出し側のコード修正が避けられません。
