...
|---|---|---|---|
| F1 | SNS販売データのインポート形式追加 | 継続的に | インポート対象チャネルが増える予定（システム基盤担当より） |
| F2 | 各店のデータ項目の追加・計算ルール変更 | 継続的に | 頻繁にあると業務担当が言及 |

フェーズ2で「今変わること（確定）」と「将来変わるかもしれないこと（リスク）」を分けて整理できました。次のフェーズ3では、現在の構造で変更を試みたときに何が起きるかを確認します。

### 2-5：変わる見込みと当面安定の前提を確定する

2-4のF-IDを、形式ごとに変えられるようにする部分と、全形式で守る取込手順へ分けます。「はい」は、フェーズ6で**形式差分が増減しても共通手順を修正せずに済む構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：SNS販売データのインポート形式追加 | はい | 形式固有の解析、行検証、項目変換 | 開く、形式確認、保存、閉じるという取込順序と結果契約 |
| F2：各店のデータ項目の追加・計算ルール変更 | はい | 形式固有の項目定義と計算 | ファイル取得・保存の境界、正常行だけを保存する流れ |

したがって2-5の出力は、「形式固有の解析・検証・計算は変えられるようにし、取込手順と結果契約は守る」という設計条件です。フェーズ3ではR1・R2だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
1つ目は、同じ修正の繰り返しです。具体的には、`openFile()` という1行が `StoreDataImporter`・`FCDataImporter`・`ECDataImporter` の3クラスそれぞれの `import()` メソッドに重複して存在しています（1-4節のコードで確認できます）。「バージョンチェックを追加してほしい」というR2に対して、同じ修正を3箇所で繰り返しました。共通の手順であるはずのファイル操作やDB保存のコードが形式ごとに複製されているため、関連クラスを検索し、修正漏れがないか広く確認する必要があります。
...
---
> **📌 問題（確定）**
> R1のEC形式を追加すると共通手順をもう一組複製し、R2の形式バージョン確認では`StoreDataImporter`・`FCDataImporter`・`ECDataImporter`の3クラスを同時に修正した。形式固有処理と共通手順が各クラスに重複しているため、一つの確定要求に対する修正・確認範囲が広い。
---
...
接続点の両側を、`StoreDataImporter::import()` の関連範囲で確認します。呼び出し元に相当する取込骨格の途中に、接続先として切り分けたい直営店形式のパースが埋め込まれ、作成した行一覧を後続の保存手順へ渡しています。
...
ここで追うのは `splitLine()` の呼び出し行だけではありません。前段から渡る `rawLines`、形式処理が作る `SalesRow` の一覧、後段が使う件数と `ImportResult` までがP1の接続です。取得境界と保存境界はフェーズ1では出力スタブであり、フェーズ6で初めて実クラスとして分けます。
...
`import()` にはもう一つ、取得元・保存先のアクセス実装（ファイル／DB／オブジェクトストレージ）も同居していますが、これは形式追加とは別の**基盤都合**で変わる軸です。本章が分離を教える主課題はP1（形式別処理）に絞り、取得元・保存先のアクセスは骨格が保存媒体を知らずに済むよう取得境界・保存境界の裏へ隔離する実装詳細として扱います（保存媒体の具体と、その置き場所となるクラスはフェーズ6で決めます）。
...
この章の接続点はP1です。形式の「読み方」と「行検証」は同じ形式変更で一緒に変わり、取得元・保存先は基盤変更で変わります。一方、取込の業務順序は守ります。どのクラスへ置くか、どの操作で接続するかはフェーズ6で決めます。
...
**着目する共通点：** 現状コードはすでに共通点を持っています。どのImporterも `import()` が「開く→パース→保存→閉じる」を進め、同じ `ImportResult` を返すという同じ骨格にそろっています（手順の共通点）。現場では、この骨格すら形式ごとにばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、呼び出し元や骨格の順序を守ったまま、変わる「形式ごとのパース」だけに絞って対策を考えられます。共通の骨格と形式差分をどう分けて接続するかは、フェーズ6でこの共通点と課題を組み合わせて決めます。

## 🔴 フェーズ6：対策検討 ―― システム全体の最終構造を定める

フェーズ5の一表から、P1の変わる側を業務順序の外へ出し、守る側が形式や保存媒体の詳細を知らずに実行できる構造を直接決めます。
...
|---|---|---|---|
| P1：形式差分 | 形式追加を差分と登録へ限定する | 各Importerの `parseData()`／`validateRows()` とScheduler登録 | 新形式クラスと登録へ変更が閉じた |
| P1を適用したシステム全体 | 共通順序と `ImportResult` を維持する | `AbstractImporter::import()` が取得→版確認→解析→検証→保存を固定 | 骨格を複製せず、件数・スキップ・成否を維持した |

**システム全体の実装結果：達成。** P1の形式差分・境界実装と業務順序が分かれ、フェーズ5で目指した状態を実現しました。実際の動作と変更影響はフェーズ7で確認します。
...
|---|---|---|---|
| F1：SNS販売データのインポート形式追加 | 読込から保存までの共通順をAbstractImporterに残し、形式差分を具体Importerへ置く | 新しいImporter、SchemaRegistry、BatchApplicationの登録 | 共通順を守れる。SNS形式は完成コードへ追加しない |
| F2：各店のデータ項目の追加・計算ルール変更 | parse・validate・transformの差し替え点へ店舗固有知識を閉じる | 対象ImporterとSchemaRegistry | 形式固有変更へ局所化できる。未確定項目は完成コードへ追加しない |

F1・F2はTemplate Methodの差し替え点を決める材料です。将来形式を実装せず、どの共通順を変更から守れるかだけを設計時に確認します。
...
|---|---|---|
| R1：ECサイト形式を追加し、会員ランクとポイントを解析・計算する | 取得・解析・検証・保存の全手順を`ECDataImporter`へ複製 | EC固有処理を`ECDataImporter`へ置き、正常なEC行だけを共通手順で保存することを確認 |
| R2：全形式で、ファイルを開いた直後に形式バージョンを確認する | 3つのImporterへ同じ確認処理を追加 | `AbstractImporter::import()`の1か所で確認し、不一致時に全形式で保存しないことを確認 |
