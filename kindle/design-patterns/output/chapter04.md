...
なるほど、店舗のデータとECサイトのデータ。どちらも「開く → 加工 → 保存」という大きな流れは同じはずですが、中身の計算ルールだけが異なるのですね。
...
### 2-1：`StoreDataImporter` / `FCDataImporter`に混在している知識と担当チーム

`StoreDataImporter.import()` と `FCDataImporter.import()` が現在抱えている知識と、それぞれを変更するチームを確認します。

| 知識（コードが直接持っているもの） | 変更を決めるチーム | 適切か |
|---|---|---|
| ファイルの開閉・DBへの保存手順 | システム基盤担当 | ✅ |
| 直営店CSV特有のパース処理 | 直営店の業務担当者 | ❌ 混在 |
| FC店CSV特有のパース処理 | FC店の業務担当者 | ❌ 混在 |
...
さて、ECサイト対応が完了した翌月、今度は別の要件が届きました。「直営店・FC店・EC店のすべてについて、ファイルメタデータに形式バージョンを付与する。ファイルを開いた直後に、対応可能なバージョンか確認してほしい」という内容です。FC店データにはヘッダー行がないため、確認対象はCSVヘッダーではなく、全形式に共通するファイルメタデータとします。
...
> openFileやsaveToDBを別のヘルパークラスに切り出して呼び出す方法（委譲・コンポジション）も、非常に有効な設計です。Template Methodパターンのメリットは、「手順の骨格（順番）」そのものを強制できる点にあります。チーム開発などで「誰もが必ず同じステップ・同じ順序でインポート処理を実装するようにしたい」という場合には、親クラスで全体の流れを固定するこのパターンが力を発揮します。
...
`import()` が処理の順序を一か所に集約しています。全形式で必要な `parseData()` は純粋仮想関数とし、形式によって要否が異なるパース後処理は `afterParse()` という任意フックにしています。途中の処理が例外を送出しても `closeFile()` を呼んでから再送出し、正常時と異常時の両方でリソースを解放します。実務では、ファイルをRAIIオブジェクトとして保持し、デストラクタで自動解放する方法も有力です。
...
| 全形式共通の前処理（ログ等）を追加 | CSVImporter・JSONImporter・ECImporter 全クラスを修正 | `DataImporter` の `process()` に1箇所追加 |
| 特定形式の変換ロジックを変更 | 対象クラスを直接修正（他との差分が不明確） | 対象のサブクラスのみ修正 |

共通の手順を基底クラスに集約したことで、共通ステップの変更先を予測しやすくなりました。代わりに、サブクラスは基底クラスが定めた順序とフックの位置に従う必要があり、基底クラスの変更が全サブクラスへ意味的な影響を与える可能性があります。継承関係、フックの増加、例外時の後始末を管理するコストを引き受けます。
...
ファイル、DBトランザクション、ロックなどを扱う処理では、差し替えステップが例外を送出しても後始末を実行する必要があります。テンプレートメソッド側で `try` / `catch` を使うか、RAIIで資源の寿命を管理し、「正常系の順序を共通化しただけ」で終わらせないことが重要です。
...
| 途中失敗時に資源解放やロールバックが必要 | テンプレート側で異常系も定義するか、RAIIを使う |
