...
|---|---|---|
| 分離方法 | 変わる部分と守る骨格をどの契約で切るか | `import()` の業務順序から、形式別の `parseData()`／`validateRows()` と、取得・保存操作を切る。行一覧、検証結果、保存件数、`ImportResult` を接続点へ残す |
| 配置場所 | 切り出した責任をどこへ置くか | 業務順序を `AbstractImporter`、形式差分を各Importer、取得を `ImportFileGateway`、保存を `SalesImportRepository` へ置く |
| 組み立て方法（生成・所有・登録・注入） | 誰がどう接続するか | `BatchApplication` が境界と形式別Importerを生成・所有し、Gateway／Repositoryを基底へ注入する。`ImportScheduler` にジョブを登録して選択・実行する |
...
**システム全体の実装結果：** P1の形式差分・境界実装と業務順序が分かれました。実際の動作と変更影響はフェーズ7で確認します。
