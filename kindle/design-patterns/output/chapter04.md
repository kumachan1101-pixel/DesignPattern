...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 店舗形式を選び、形式固有データを解析する | 要求ID1、要求ID3 |
| 機能ID2 | ファイル取込の共通手順を実行する | 要求ID2 |
| 機能ID3 | 不正行を除外し、取込結果を返す | 要求ID4 |
...
|---|---|---|---|
| 機能ID1 | 変更<br/>根拠: 変更ID1 | 直営・FC・EC形式を選び、形式固有データを解析する | 要求ID1、要求ID3 |
| 機能ID2 | 変更<br/>根拠: 変更ID2 | 開いた直後の形式バージョン確認を含む共通手順を実行する | 要求ID2 |
| 機能ID3 | 継続<br/>根拠: — | 不正行を除外し、取込結果を返す | 要求ID4 |
...
フェーズ5の一表から、課題ID1の変わる側を業務順序の外へ出し、守る側が形式や保存媒体の詳細を知らずに実行できる構造を直接決めます。
...
|---|---|---|---|
| 機能ID1 | 変更 | 要求ID1、要求ID3 | 既存2形式を回帰し、EC固有解析・計算を受入確認<br/>**判定:** 合格 |
| 機能ID2 | 変更 | 要求ID2 | 全形式で開く直後にバージョン確認する順序を確認<br/>**判定:** 合格 |
| 機能ID3 | 継続 | 要求ID4 | 不正行を保存せず件数・理由を返す既存動作を回帰確認<br/>**判定:** 合格 |
