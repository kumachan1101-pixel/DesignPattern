...
はじめに、CSVインポート処理という現場でよくあるシステムを例に、その現状を事実として観察していきましょう。
...
変更後の仕様を、1-1と同じ入力・判定・加工・出力の流れとして図で確認します。1-1の図との差分は、フォーマット種別に「EC店」が加わること、共通手順の直後に全形式共通の「形式バージョンを確認」という加工が挟まること、EC店だけ解析の後に「ポイント付与・会員ランク割引の計算」という加工が加わることの3点です。
...
図に加わった「形式バージョンの確認」と「EC店だけの計算」が実際にコードのどこへ書かれるかは、フェーズ3で変更を試すコードと、フェーズ7の最終コード・実行結果で追います。
...
フェーズ2では、まだ「責務が混在している」とは判断しません。ここで行うのは、フェーズ1で見た仕様のうち、どのフォーマット・変換・保存手順が変わりそうかを見当づけることです。
...
この表から、今回の検討対象は「店舗種別ごとのCSVパース規則」に絞れます。共通手順とパース差分が同じ場所に書かれて困るかどうかは、フェーズ3で変更を入れてから確認します。
...
| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| メソッド切り出し | 開く・パース・保存・閉じるの各処理 | 各処理が名前で読め、共通構造に気づける | 骨格（呼び出し順序）が各クラスに散らばったまま |
| フラグ引数で分岐 | パース差分を種別で切り替える | 骨格を1つの関数へ集約できる | 形式が増えるたび中央の分岐を書き足す |
| 骨格固定・差分委譲 | 骨格と、形式ごとのパース処理 | 骨格1か所、差分はサブクラスへ | 継承階層と組み立て登録の初期コスト |

この3案のうち「フラグ引数で分岐」は、共通の骨格を1つの関数にまとめ、変わるパース部分だけを種別で切り替えればよい、という最初に思いつきやすい案です。この案は比較表の中だけで済ませず、ステップ2の中で案Aとして実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。メソッド切り出しはステップ1で、骨格固定・差分委譲はステップ2の案Bでコードにします。

なお、メソッド切り出しと骨格固定は1つ目の「共通化・抽象化」の実行です。フラグ引数と差分委譲は「変わるパース処理を骨格の外から与え、骨格は決まった流れを回すだけにする」という2つ目の実行にあたります。
...
ステップ1の問題は「骨格（呼び出し順序）が各クラスに散らばっている」ことだった。骨格を1か所へ集める方法として、まず比較表の「フラグ引数で分岐」案から確かめる。
...
**案Aに残る課題：**
フェーズ2で「次はSNS経由の販売データを取り込む予定」と予告されていました。形式が増えるたびに、この `import()` の中央にある `if (type == ...)` の分岐を書き足すことになります。骨格を固定した1つの関数の中に、形式ごとに変わるパース処理が `if` として混ざったままなので、パース差分を触るときも骨格を持つ関数を開くことになります。「骨格は固定、変わるパース処理は骨格の外から与える」という形にして、形式追加で中央の分岐を触らずに済むようにしたい――これが次の案の主題です。

**案B：骨格を基底クラスに固定し、変わる部分だけをサブクラスに委ねる**

そこで、変わるパース処理を関数内の `if` から外へ出します。骨格そのものを1つの基底クラスに閉じ込め、変わる部分（`parseData()`）だけを純粋仮想関数としてサブクラスに委ねれば、骨格は1か所だけで管理でき、形式追加は新しいサブクラスの追加だけで済みます。
...
*   **ステップ2・案A（フラグ引数で分岐）で止めるケース：** 骨格を1か所へ集めたいが、形式が2〜3種類で当面増えず、中央の分岐が肥大化する心配が薄い場合。継承階層を作らずに骨格の集約だけを得られます。
*   **ステップ2・案B（抽象基底クラスへの集約）まで進むケース：** 「今後も新しいインポート形式が追加される」と確定している場合。今すぐ初期投資コストを払ってでも、将来の変更箇所を限定するのが適切です。
...
フェーズ2で予告された共通の「バージョンチェック」は基底クラスへ1回追加し、EC店だけの計算処理は `afterParse()` へ置きました。変更理由の異なる2つの処理が、それぞれ対応する場所へ分かれています。未登録タイプ（"online"）は `SchemaRegistry.exists()` で検出し、インポーター呼び出し前にエラーを出力して処理を中断します。
...
| 🔴 フェーズ6：対策検討 | フラグ引数で分岐する案（案A）を含む案を比較し、形式追加で中央の分岐を書き足す残課題から、共通手順を基底クラスにテンプレート化して分離するステップ2を採用しました |
