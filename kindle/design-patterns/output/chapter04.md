...
ここまでの事実から、「システムの中で変わりそうな部分」と「変わらなそうな部分」の仮説を立てます。

| **変動しそう** | **変わりそうな部分** | **なぜそう読み取れるか** |
|---|---|---|
| 🟢 変動しそう | CSVのパース処理（データ項目の追加など） | 各店舗の業務担当者が決定するため、店舗ごとの要望で変わりやすい |
| 🟢 変動しそう | ポイント付与・ランクなどの固有計算ルール | 新しい出店形態（ECなど）が増えると追加されるため |
| 🔴 不変そう | ファイルを開く・パースを呼ぶ・DBへ保存して閉じる全体の流れ | どのフォーマットでも共通の手順であり、システム基盤が担当するため |
...
| **変動項目** | **時期** | **根拠** |
|---|---|---|
| EC向けデータ項目の読み込み追加 | 今回確定 | 変更要求より |
| EC向けポイント・ランク計算の追加 | 今回確定 | 変更要求より |
| SNS販売データのインポート追加 | 将来リスク | ヒアリングで判明（システム基盤担当より） |
| 各店舗のデータ項目の追加やルール変更 | 将来リスク | ヒアリングで判明（業務担当より） |

> **設計の方針：** 🔴 不変な部分（ファイルを開く〜閉じるの処理の流れ）を「骨格」として固定し、🟢 変動する部分（パース処理や固有の計算）をその内側に押し込む。
...
フェーズ3で確認した「変更の辛さ」は、コードのどこから来ているのでしょうか。コードを注意深く観察すると、痛みを引き起こしている構造的な原因が浮かび上がってきます。

### 4-1：痛みの根源を探る（観察と原因）

| **観察した症状（痛み）** | **構造的な原因（痛みの根源）** |
|---|---|
| 新しいインポート形式を追加するたびに、ファイルを開く・閉じるなどの「一連の処理の骨格」を全クラスで書き直す必要がある | 共通の手順（処理の骨格）と、店舗ごとのデータパース（詳細）が、同じメソッドの中に混在しているから |
| 共通であるはずのファイル操作手順に修正が入ったとき、全インポートクラスを修正することになる | 共通の手順という「変わらないもの」を、店舗ごとの詳細という「変わるもの」が引きずり回しているから |

### 4-2：変わるもの / 変わってほしくないもの

コードの中に「変わるもの」と「変わってほしくないもの」が物理的にどう混在しているかを確認します。
...
ビジネス側が「パースルール」を変えるたびに、システム基盤側が管理する「ファイル操作手順（骨格）」まで影響を受けてしまっていることが、設計上の問題です。

### 4-3：接続形態を診断する

現在、骨格となる手順とパースロジックは抽象化がなく、直接依存しています。これは **「具体×直接」** の接続形態と言えます。
...
現状は、iPhone に専用のケーブルを直接ハンダ付けしたような状態です。新しい店舗のCSV形式が増えるたびに、クラス本体に新しい配線（`parseXXX()` メソッド）を直接組み込むことになります。
...
`import()` の中で分けるべき境界は1か所。「パースロジックの生産者」が骨格に渡しているデータを見ます。

現在の結合状況：`StoreDataImporter` と `FCDataImporter` はそれぞれの `import()` の中で骨格手順とパースロジックを直接混在させています。骨格が呼び出す `parseData()` の中身が各クラスで異なる実装になっています。

| 接続点 | 接続するデータ | 変わるもの |
|---|---|---|
| `parseData()` の実装 → `import()` の骨格 | 骨格の呼び出し順序（openFile→parseData→saveToDB→closeFile）→ 処理完了（void） | パースロジックの実装（店舗形式ごとに異なる） |

### 何が変わり、何が変わらないか

- **変わるもの**：`parseData()` の実装（直営店形式・FC形式・EC形式で読み方が異なる）。新しいインポート形式が増えるたびに生産者が増えます。
- **変わらないもの**：骨格の呼び出し順序（open→parse→save→close）。その順序が意味する「インポートという業務の流れ」です。

呼び出し元が期待するのは「ファイルを受け取って適切にDBに保存してくれること」です。課題は「どのようにパースするか」という**生産者の実装**が各クラスに重複して存在していることではないでしょうか。

**具体×直接のままでよい場面**：インポート形式が今後増えない確証があれば、現状の重複実装（具体×直接）で十分だと考えます。接続形態の選択は「**生産者が変わるかどうか**」で決まります。今回は形式追加リスクがヒアリングで確認済みなので、次のフェーズで生産者を差し替えられる設計を検討します。

---
> **📌 課題（確定）**
> 骨格の呼び出し順序（open→parse→save→close）という変わらないものと、`parseData()` の実装という変わり続ける生産者を切り離す必要がある。骨格を1か所に集約し、各インポートクラスはパースルールの実装だけを担う構造にすることで、新しいインポート形式が増えても骨格に触れずに済む。
---

## 🟡 フェーズ6：対策検討 ―― 案を比べ、採用案を決める

いきなりパターンを適用するのではなく、どのような構造の変化を経ることで最適な形に到達するのか、段階的に検討します。

### 案1：現状のまま（具体×直接）
現状のクラスごとの `import()` メソッドです。
...
```
- **メリット**：処理が上から下まで一筆書きで読める
- **デメリット**：`import` の「骨格」が全クラスにコピペされる。将来、新しい手順が追加されたとき修正漏れが起きる。

### 案2：共通メソッド化（関数に切り出す）
骨格となる手順の重複を減らすため、共通の関数として切り出す方法です。
...
class StoreDataImporter : public BaseImporter {
public:
    void import() {
        openFile();
        parseStoreCSV();
...
```
- **メリット**：ファイル操作のロジック自体の重複は排除できる。
- **デメリット**：`import()` という「呼び出しの順序（骨格）」そのものは依然として各子クラスに残っている。新しい子クラスが `saveToDB()` を呼び忘れるといったミスを防げない。

### 案3：条件による個別分岐（具体×間接）
「呼び出しの順序（骨格）」を共通クラスに押し込み、その中で条件分岐を使って呼び分けるアプローチです。

```cpp
class Importer {
public:
    void import(int type) {
        openFile();
...
```
- **メリット**：`import()` の呼び出し順序（骨格）を1か所にまとめられた。
- **デメリット**：条件分岐（`if/else`）の中に全種類のパース知識が集中してしまう。フォーマットが増えるたびにこの巨大な分岐を修正する必要がある。

### 案4：抽象×間接 への進化（パターンの発見）
「手順の骨格」は基底クラスに置きつつ、中身の「条件分岐」をポリモーフィズムで解決します。これこそが今回のゴールとなる構造です。

```cpp
// 骨格を持つ基底クラス
class AbstractImporter {
public:
    // 骨格となる手順を固定する（Template Method）
    void import() {
        openFile();
        parseData(); // 中身は子クラスに委ねる
        saveToDB();
        closeFile();
    }
protected:
...
案3にあった巨大な `if` 文はどこへ行ったのでしょうか。実は、呼び出し側（Main関数やFactory）が「どのクラスをインスタンス化するか」という形で、条件分岐が移動しただけです。これによって、骨格と詳細が見事に分離されます。

> **【Template Method】パターン** ―― 「処理の骨格（手順）」を基底クラスで固定し、「具体的な内容（詳細）」をサブクラスに委ねる構造。

### 採用案の決断

案4（抽象×間接）を採用します。この構造なら、今後「SNS販売データ」などの新しいフォーマットが追加されても、既存のインポートロジックや手順の骨格に一切触れることなく対応できます。
...
> **「EC向け計算処理」が `parseData()` の中にある理由：** 1-5節の仕様表では「EC向け計算処理」を独立したステップとして記載しましたが、これはEC店固有の処理であり `AbstractImporter` の骨格（全インポーターに共通の手順）には含めません。Template Methodパターンでは、骨格は共通手順のみを定義し、EC固有の計算処理は `ECDataImporter.parseData()` の中に自然に収まります。仕様上は独立ステップに見えても、実装上は「パース＋EC計算」がセットになるのが自然な設計ではないでしょうか。
...
フェーズ2で予告された「バージョンチェックの追加」や「EC店の計算処理」が来た場合でも、既存のコードをほとんど触らずに対応できることが、この実行結果からも分かります。
...
| **課題** | 骨格の呼び出し順序（変わらない）と `parseData()` の実装（変わり続ける生産者）を切り離し、骨格を1か所に集約する必要がある |
| **解決策** | Template Method パターン：`AbstractImporter` に骨格を固定し、`parseData()` だけをサブクラスが実装することで、新しいインポート形式の追加が骨格に影響しない構造を実現する |
...
| 🟡 フェーズ5：課題定義 | 接続点で流れるのは骨格の呼び出し順序（安定）、変わるのは parseData() の実装（生産者）であることを特定した |
...
| 店舗固有のパースロジックの実装 | `StoreDataImporter` / `FCDataImporter`（骨格と混在） | `StoreDataImporter` / `FCDataImporter`（`parseData()` のみに集約） |
