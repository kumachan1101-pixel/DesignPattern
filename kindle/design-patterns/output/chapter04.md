...
|---|---|---|
| CSVファイル、フォーマット種別、データ行 | 取り込み対象と形式ごとの入力 | 動作例テーブル、変更要求 |
| 開く、パースする、保存する、閉じる | インポート処理の手順と形式別の差分 | 現状仕様の確認、フェーズ2の仮説、原因分析 |
| インポート結果、処理件数、エラー報告 | 取り込み後に利用者へ返る結果 | 動作例テーブル、実行結果、変更シナリオ表 |
