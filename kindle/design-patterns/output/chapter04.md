...
|---|---|---|---|
| P1 | 新形式の影響から共通順序の複製・再テストを外す | import() の固定順序と形式別 parse/validate の境界を切る。ImportResultと保存順序を守る | 新形式は差分ステップとSchemaRegistry登録だけを追加し、共通順序を複製しない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 共通手順と形式差分へメソッドを分ける | 同じ行と違う行を見分けられる | 共通順序を1か所に置き、形式差分だけを後から定義する |

ここからP1の共通順序と形式差分に該当するコードだけをおさらいします。
