...
**変わらない手順の骨格と、場合によって異なる詳細な処理が同じ場所に混在しているとき、詳細が変わるたびに骨格まで修正が必要になる。こういう問題は、「変わる理由が違うもの」が同じ場所に混在しているシステムで起きている。**
...
インポート処理は以下の4ステップで構成されており、どの店舗形態でもこの大きな流れは変わりません。この「開く→変換する→保存する→閉じる」という順序は、ファイルを扱うバッチ処理では業種を問わず広く採用されている一般的な流れです。順序が固定されているのは、開く前に保存しようとしたり、閉じた後に読もうとしたりするバグを防ぐためです。料理で言えば「材料を取り出す→切る→調理する→片付ける」という手順を守るのと同じで、手順の前後を入れ替えると結果が破綻します。
...
「ヘッダー行のスキップ」は、CSVファイルの1行目に「商品名,数量,金額」のような列名が書かれている場合、それをデータとして読み込まないための処理です。表計算ソフトで言えば「1行目を見出し行として扱う」設定と同じです。一方FC店では、POSレジがヘッダーなしで最初の行から数値データを出力するため、スキップする処理自体が不要です。「不正行のスキップ」は、データが欠けているなど正常に変換できない行を無視して処理を続けるルールで、夜間バッチで大量データを扱う際には取り込み漏れを防ぐよりも処理を止めない方を優先する、実務でよく見られる判断です。
...
ヒアリングで「SNS販売データ形式の追加」と「各店データ項目・計算ルールの変更」が予告されました。この変化が来たとき、仕様がどう変わるかを整理しておきます。

| 変更内容 | 現在 | 将来（時期の目安） |
|---|---|---|
| インポート対象チャネル | 店舗 / FC の2種類 | SNS販売データなど新チャネルが継続的に追加 |
| 各チャネルのデータ項目 | 現状の固定項目 | 業務変更・キャンペーンごとに追加・変更が発生 |
| 各チャネルの集計ルール | 現状の固定計算式 | 計算方法が業務担当判断で随時変わる |
