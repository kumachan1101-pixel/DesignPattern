...
**理由：** 共通手順を基底クラスにテンプレートとして定義し、各店固有のパースロジックをサブクラスへ追い出すことで、変更時の影響を最小化できる案3を採用します。

案3で「十分」と判断した根拠は以下の3点です。第一に、今回の課題は「呼び出し元が具体クラスを知っている」という問題ではなく「手順の骨格が重複している」という問題であり、案3の基底クラスによる骨格の集約で直接解決できます。第二に、ヒアリングで確認した変更（新しいインポート形式の追加・共通ログの追加）はいずれも案3の範囲内で対応可能です。第三に、呼び出し元が `main()` の組み立て部分だけである現在の規模では、仲介クラスを経由させる間接化のコストに見合うメリットが生まれません。

案4は変更容易性・テスト容易性ともに案3と同等のスコアですが、仲介クラス（`ImporterManager`）とその上位インターフェース（`IImporterManager`）の2層が加わることで、理解コストと実装コストが増大します。「呼び出し元が仲介クラスすら知らなくてよい」という恩恵が必要になるのは、呼び出し元が多数あるか、仲介クラス自体を差し替える予定がある場合に限られます。現時点ではその条件を満たさないため、案4は過剰と判断します。
...
> **実は、この案3の構造には名前があります。** 処理の骨格（手順の順序）を基底クラスが定義し、その中の一部のステップだけをサブクラスが差し替える——この構造を、設計の世界では **Template Method（テンプレートメソッド）パターン** と呼びます。フェーズ1から6の思考プロセスを回した結果、この名前の付いた構造にたどり着いたわけです。パターン名は「先にあった答え」ではなく、「思考の積み重ねの末に気づく名前」です。
