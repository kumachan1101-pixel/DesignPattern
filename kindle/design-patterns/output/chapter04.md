...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: 共通手順と形式差分をメソッド分割"] --> S2["P1-変更2: 共通順序を骨格へ集約"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["新しい取込形式を追加"] --> I1["P1: 形式別の解析・検証と登録だけを追加"]
        I1 --> I2["追加形式と共通契約だけをテスト"]
        IS["変更しない: open→version→parse→validate→save→close"]
    end
    C1 -. "P1で複製経路を切る" .-> S1
    S2 -. "形式差分だけ残す" .-> I1
```
...
|---|---|---|---|
| P1 | `StoreDataImporter::import()`、`AbstractImporter::import()`、`parseData()`、`validateRows()`、Registry | **微修正：** メソッド分割→固定順序を骨格へ集約→差分を仮想操作と登録へ残す<br>**結果：** 形式固有の解析・検証だけを追加できる | **守った契約：** open→version→parse→validate→save→close→result<br>**判定：** 分割だけでは未達、骨格集約で達成 |
