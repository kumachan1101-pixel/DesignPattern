...
EC店の正常8件と不正2件を、上のコードで通します（`splitLine`・`SalesRow`・`ImportResult` は1-4のままです）。見るのは、変更要求を現状の構造へ当てはめたとき、修正箇所と痛みがどこに出るかです。
...
**変更ID2（全形式で、ファイルを開いた直後に形式バージョンを確認する）を当てはめる**

さらに、同時に要求されていた形式バージョン確認も当てはめます。「直営店・FC店・EC店のすべてについて、ファイルメタデータに形式バージョンを付与し、ファイルを開いた直後に対応可能なバージョンか確認する」という仕様です。

バージョン確認はフォーマット種別に関わらず全店舗で共通の手順変更です。しかし手順が各クラスの `import()` にべた書きされている構造では、共通の置き場所がありません。`StoreDataImporter`・`FCDataImporter`・`ECDataImporter` の3クラスそれぞれへ、同じコードを書き足すことになります。
...
3クラスへ確認を入れた状態で、バージョンが一致する場合と旧バージョンの場合を実行します。
...
対応バージョンの2形式と、旧バージョン1件を通します。見るのは、全形式で開いた直後に確認できるかと、そのために同じコードを何か所へ書くことになったかです。
...
3形式とも開いた直後に確認でき、旧バージョンのファイルは1件も読まずに止まりました。要求は満たせています。

ただし、そのために同じ確認コードを3クラスへ書き、読み始め位置の修正を3クラスで別々に書き直しました。1つの確定要求に対して修正が6か所です。今後バージョン番号が3へ上がったとき、あるいは確認方法が変わったときも、また3クラスを開いて同じ修正を入れることになります。1クラスだけ直し忘れても、そのクラス以外は正しく動くので、テストで気づける保証がありません。
