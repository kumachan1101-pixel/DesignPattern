...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。運用者が直営店の売上CSV（必須3列・10件）をインポートすると、内部はファイルを開く→カンマ区切りで解析→検証しながらDBへ追加→ファイルを閉じるの順に進みます（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
1つ目は、同じ修正の繰り返しです。具体的には、`openFile()` という1行が `StoreDataImporter`・`FCDataImporter`・`ECDataImporter` の3クラスそれぞれの `import()` メソッドに重複して存在しています（1-4節のコードで確認できます）。「バージョンチェックを追加してほしい」という変更ID2に対して、同じ修正を3箇所で繰り返しました。共通の手順であるはずのファイル操作やDB保存のコードが形式ごとに複製されているため、関連クラスを検索し、修正漏れがないか広く確認する必要があります。
...
|---|---|---|---|
| 課題ID1：形式差分 | 形式追加を差分と登録へ限定する | 各Importerの `parseData()`／`validateRows()` とScheduler登録 | 新形式クラスと登録へ変更が閉じた |
...
| 🔴 フェーズ6：対策検討 | 課題ID1を満たす最終構造は骨格固定構造に一意に定まると確認し、採用クラス図を3ステップでコードへ反映した |
