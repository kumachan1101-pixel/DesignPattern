...
| ファイルの読み書き                   | **スタブ**：実ファイルは作らず、メモリ上のサンプルCSVを保持（`ImportFileGateway`） | 本物のファイルAPI／DB／オブジェクトストレージ |
| DB保存                        | **スタブ**：件数を数えるだけ（`SalesImportRepository`）              | 本物のDBドライバ・トランザクション        |
| 形式バージョン確認                   | **スタブ**：メッセージ表示のみ                                      | メタデータの実検証                 |
| 同期/非同期の実行                   | **スタブ**：スケジューラは逐次実行で代替（決定的な出力のため）                      | ワーカースレッド等で並行実行            |

論点は「開く→変換する→保存する→閉じる」という手順の骨格と、形式ごとに変わるパース処理をどこで分けるかです。ファイルの読み取り失敗、DBトランザクション、文字コード変換、実スレッドは実運用では必要ですが、本章では設計論点から外れるためスタブにしています。とくにファイルの読み書きとDB保存は `ImportFileGateway`／`SalesImportRepository` の内側に閉じており、**保存媒体が変わっても骨格は影響を受けません**（この点はフェーズ7で改めて確認します）。
...
| `function` / `queue` | 実行する処理を値として待ち行列へ積む | 先頭から順に実行 | 非同期ワーカーを逐次実行で代替 |
| Gateway / Repository | ファイル取得とDB保存を受け持つ | 行一覧・保存件数 | メモリスタブ。I/O時間は省略する |

`std::function`は「後で呼ぶ処理」、`std::queue`はその処理の待ち順を表します。実スレッドは使いませんが、投入順と実行順、失敗時に後続を続けるかという契約は明示します。
