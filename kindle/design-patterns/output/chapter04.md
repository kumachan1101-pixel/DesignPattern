...
フェーズ3「問題特定」で「手順の重複が辛い」という事実が確認できました。次のフェーズ4「原因分析」では、なぜこの辛さが構造的に発生するのかを分析します。
...
フェーズ3「問題特定」で確認した「変更の辛さ」は、コードのどこから来ているのでしょうか。コードを注意深く観察すると、痛みを引き起こしている3つの事実が浮かび上がってきます。
...
- **契約と具体による分離**：取込処理に残す共通順と、形式ごとの差分を表す契約・具体をセットで決める。
- **実体の組み立て**：具体を誰が生成・所有し、どの派生を選んで共通順を実行するかを一本で決める。

### 全体のデータと実体の流れを先に決める

最初に、派生Importerを生成してから取込結果へ到達するまでの全体経路を決めます。直営店CSVの取込1件を、派生型の生成→共通順→差し替え点→具体処理の順で固定します。
...
**箱が1つしかありません。** 他章ではここで「骨格」と「契約」の2つの箱と、そのあいだの矢印が描けます。**この章は同じ箱の中に両方あるので、矢印が要りません。** 公開操作 `import()` が上、差し替え点が下（`#` は `protected`）で、上が下を呼びます。
...
**分離の検算で書いたときから、1文字も変わっていません。** 他の章では、ここで穴を埋める行が書き換わります。**この章では書き換わりません。** 骨格は最初から完成していました。
...
**これは第9章の課題ID2と同じで、公開入口が変わらない例です。** 変わったのは呼び方ではなく、この1行の先で誰が順序を持ち、誰が形式を読むかです。全体経路で先に決めた接続が、この実コードで成立することを確認します。
