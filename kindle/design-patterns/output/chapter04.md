...
★以下呼び出し元、やることが多すぎないか。３章のようにstaticで事前にテーブルでデータを保持した状態から開始した方が分かりやすいのでは？こんなにごちゃごちゃしたコード、出力結果、読者は見ようと思うのでしょうか。工夫してほしいです。システム側もプリントだけで処理内容が省略されているので、複雑なうえに処理内容も隠蔽されていて、システムとしてよくわからない。実際にファイル作った方が良いのでは？それか、別のシステム例に切り替えるか。
#### 呼び出し元と実行確認
...
* **業務担当：** 「いいえ、ファイルを開いて閉じるという手順は固定です。ただ、中身のデータ項目が少しずつ増えたり計算ルールが変わったりすることは頻繁にあります。」★本当にそんなことあるのか？保存場所が変わったら、そもそもデータベースへのアクセス方法が変わることは十分にあり得るのでは？このシステム、そもそも前提がおかしいと思います。
