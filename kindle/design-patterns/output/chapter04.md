...
---
> **📌 問題（確定）**
> インポート形式が1つ増えるたびに、`openFile()`・`saveToDB()`・`closeFile()` という共通手順を新しいクラスにも重複して書かなければならない。ヒアリングで「SNS販売データなど今後もインポート対象が増える」と確定しており、この頻度では共通手順の重複管理コストが合わない。
---
...
- **変わるもの**：`parseData()` の実装（直営店形式・FC形式・EC形式で読み方が異なる）。新しいインポート形式が増えるたびに生産者が増える。
- **変わらないもの**：骨格の呼び出し順序（open→parse→save→close）。その順序が意味する「インポートという業務の流れ」。

呼び出し元が期待するのは「ファイルを受け取って適切にDBに保存してくれること」です。問題は「どのようにパースするか」という**生産者の実装**が各クラスに重複して存在していること。

**具体×直接のままでよい場面**：インポート形式が今後増えない確証があれば、現状の重複実装（具体×直接）で十分です。接続形態の選択は「**生産者が変わるかどうか**」で決まります。今回は形式追加リスクがヒアリングで確認済みなので、次のフェーズで生産者を差し替えられる設計を検討します。
...
**今回の決断：**
フェーズ2のヒアリングで、システム基盤担当から「次はSNS経由の販売データを取り込む予定」と明言されています。インポート形式は今後も増え続けると確定しているため、今回は迷わず**ステップ2（抽象基底クラスへの集約・抽象×直接）まで進化させる**決断を下します。
...
> **「EC向け計算処理」が `parseData()` の中にある理由：** 1-5節の仕様表では「EC向け計算処理」を独立したステップとして記載しましたが、これはEC店固有の処理であり `AbstractImporter` の骨格（全インポーターに共通の手順）には含めません。Template Methodパターンでは、骨格は共通手順のみを定義し、EC固有の計算処理は `ECDataImporter.parseData()` の中に自然に収まります。仕様上は独立ステップに見えても、実装上は「パース＋EC計算」がセットになるのが正しい設計です。
