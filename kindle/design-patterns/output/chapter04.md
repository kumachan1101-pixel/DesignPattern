...
仕様表（1-2節）では「ファイルオープン・データパース・DB保存・ファイルクローズ」という4つの処理ルールを整理しました。このシステムは、インフラ寄りの処理を管理する「システム基盤担当」と、フォーマット固有のルールを管理する「各店舗の業務担当者」という2つの立場で保守されています。それぞれのルールを誰が管理しているかを対応させると、以下のようになります。
...
要するに、誰が変更するかが異なる知識が同じメソッド内に混在しています。「システム基盤担当が変えるもの」と「業務担当者が変えるもの」が一つのメソッドに同居している——これが後に見えてくる問題の原点です。

すべてのインポートクラスが同じ手順の骨格を抱え込んでいるという観察から、共通の手順と店舗ごとの詳細な実装が混在しているという構造の問題が見えてくる。
...
ステップ1では「今回の変更で確実に変わること／変わらないこと」を仕様表から読み取りました。ここで分かったのは「データパースが変わる」という事実だけです。しかし、これが今後も繰り返されるのかどうかは、仕様表だけでは判断できません。そこでステップ2では、「今回だけの変化か、それとも継続的に繰り返される変化か」を予測し、ヒアリングで確認すべき仮説として準備します。
...
ヒアリングの結果、以下のように仮説と発言の対応が取れました。

| **仮説** | **それを裏付けた発言** |
| --- | --- |
| 仮説1：インポート対象のシステムは今後も増える | 「次はSNS経由の販売データを取り込む予定です」（システム基盤担当） |
| 仮説2：ファイルの開閉やDB保存の手順は変わらない | 「ファイルを開いて閉じるという手順は固定です」（業務担当） |

仮説1が確認されたことで、パースルールの追加が繰り返されるという設計上のリスクが明確になりました。ファイル操作や保存の手順は将来にわたって安定しており、インポートのデータ構造や加工ロジックは今後も追加され続けるリスクがあると分かりました。
...
ヒアリングで判明した「今後変わるかもしれない」リスクを、今回の確定変更とは別に整理します。これらは現時点では確定していませんが、設計判断に影響する見込みのある変化です。フェーズ6の対策を選ぶ際に、この将来リスクを念頭に置きます。
...
フェーズ5で「何を解くか」が確定しました。次のフェーズ6では、この課題に対してどのような「接続の形」を採用する必要があるか、Step 1〜Step 4を並べてコスト比較を行います。

## 🔴 フェーズ6：段階的進化 ―― どこまで設計を進めるべきか

フェーズ5で課題定義した「共通手順とパースルールの混在」を解消するための対策ステップを検討します。
...
|:---:|:---|:---|
| **具体×直接**（← 現在地） | ライトニング直生え → iPhone（直差し） | `StoreDataImporter` / `FCDataImporter` が `openFile()` / `saveToDB()` を内部に直書きしている |
| **具体×間接** | ライトニング直生え → ゲーム機専用アダプタを挟む → ゲーム機 | 共通手順を担う中継クラス（`ImportProcessor`等）を置き、具体クラスがそれを呼び出す |
| **抽象×直接** | Type-C直生え → 各種機器（直差し） | `AbstractImporter` という抽象基底クラスを定義し、各店舗が `parseData()` だけを実装 |
| **抽象×間接** | ライトニング直生え → Type-C変換アダプタを挟む → 各種機器 | `AbstractImporter` + `IImporterManager` の2層抽象で呼び出し元が具体クラスを一切知らない |

**現在のシステムの位置：**

> **現在のシステムはここ → 具体×直接**
> 各インポートクラス（`StoreDataImporter`・`FCDataImporter`・`ECDataImporter`）がそれぞれ独自に `import()` メソッドを持ち、その中で `openFile()` や `parseCSV()` などを直接呼び出しています。共通の抽象クラスを持たず、骨格を直接所有しているこの構造こそが「具体×直接」の証拠です。Lightningコネクタが機器ごとに個別に用意されているようなイメージです。
...
**この形の考え方：**
フェーズ3で示したコードを、接続の形は変えずにプライベートメソッドで整理した形です。各処理の意味がメソッド名で明確になります。クラスの分割やインターフェースの導入は行わず、if/elseの各分岐を意味のある名前のプライベートメソッドへ抽出します。変更頻度が低く、将来的な変化が見込めない場合に許容される最小コストの選択です。

**手段の比較：**

| 手段 | 方法 | 特徴 |
|---|---|---|
| 手段A：if文追加のまま | 既存メソッド内にif分岐を直書きしたまま | 最小の変更量だが意図が読み取りにくい |
...
以下に各ステップの全コードを掲載します。前のページに戻らなくても、この節だけで各ステップの全体像を把握できます。
...
public:
    ...
    void import() {
        openFile();
        // ← 直接：フォーマット判定をこのクラスが自分で持つ
        if (format == "EC")      importEC();
        else if (format == "FC") importFC();
        else                     importStore();
        saveToDB();
        closeFile();
    }
private:
    ...
    void importStore() {
        skipHeader();    // ヘッダー行をスキップ
        splitByComma();  // カンマで各フィールドに分割
    }
    void importFC() {
        splitByTab();       // タブで各フィールドに分割
        skipInvalidRows();  // 不正な行を読み飛ばす
    }
    void importEC() {
        splitByComma();    // カンマで各フィールドに分割
        readMemberRank();  // 会員ランク列を読み込む
        readPointColumn(); // ポイント列を読み込む
        calcPointBonus();  // ポイント付与量を計算する
    }
};

```

このコードを見ると、フォーマット種別の知識が `DataImporter` の中に直接埋め込まれたままですが、各分岐の処理内容がメソッド名から読み取れるようになっています。EC以外の新形式が増えるたびにここの条件分岐が膨らんでいく点は変わりません。
...
public:
    void run(std::vector<std::string> files) {
        for (int i = 0; i < (int)files.size(); i++) {
            ...
            importer.format = detectFormat(files[i]);
            importer.import();
        }
    }
};
...
> **ケーブルで言うと：** Lightningケーブルはそのままだが、機器ごとに専用ケーブルを整理してラベルを貼る。ケーブル自体の規格は変わらず専用のまま。機器が増えれば専用ケーブルも増え続けるが、Step 1よりは整理されている。

**この形の考え方：**
インポート処理を店舗ごとにクラスとして切り出し、呼び出し側はその具体クラスを名指しで知りながらも、処理そのものはそのオブジェクトに委ねる（間接）形です。自分で直接処理しない——この「委ねる」という一歩が、Step 1との違いです。責任の境界は引かれますが、新しい形式の追加時には呼び出し側のコード修正が避けられません。
...
**手段A**（このステップの「具体×間接」という接続形態の本質を示すため）のコードを以下に示します。
...
classDiagram
    ...
    BatchImportJob --> StoreDataImporter : 具体×間接
    BatchImportJob --> ECDataImporter : 具体×間接
    ManualImportController --> StoreDataImporter : 具体×間接（重複）
    ManualImportController --> ECDataImporter : 具体×間接（重複）
```
...
```cpp
// Step 2（具体×間接）の呼び出し側
// 夜間バッチ：具体クラスを名指しで生成するが、処理はそのオブジェクトに委ねる
class BatchImportJob {
public:
    void run() {
        StoreDataImporter store; // ← 具体：クラス名を直接書く
        store.import();          // ← 間接：処理はstoreオブジェクトに委ねる
        ECDataImporter ec;       // ← 具体：クラス名を直接書く
        ec.import();             // ← 間接：処理はecオブジェクトに委ねる
        // ← ECフォーマット追加時：ここにも新クラスの生成とimport()呼び出しを追加する必要あり
    }
};
...
具体的に重複している箇所を示します。`BatchImportJob::run()` は `ECDataImporter` と `StoreDataImporter` を名指しで生成しており、`ManualImportController::importFile()` も `filename.find("ec_")` という判定で同じクラスを選んでいます。この「どのクラスを呼ぶか」という判断ロジックが2か所に分散しているため、FC店対応を追加するだけで両方のクラスを修正しなければなりません。
...
**手段A**（手順の骨格を完全に基底クラスに閉じ込めることが今回の目的に合致しているため。コンポジション（手段B）は骨格の順序をサブクラスが自由に変えられてしまうリスクがある）のコードを以下に示します。
...
// ← ECフォーマット追加時：このような新サブクラスを1つ追加するだけ。BatchImportJobやManualImportControllerの修正は不要
...
なぜ呼び出し元が変わらないのか。理由は、`BatchImportJob::run()` が受け取るのは `AbstractImporter*` という型だからです。この型は「`import()` を呼べる何か」という契約だけを表しており、実際にどの具体クラスが渡されるかは呼び出し元から見えません。新しい `ECDataImporter` を追加しても、`BatchImportJob` は相変わらず `AbstractImporter*` を受け取って `import()` を呼ぶだけです。「どのクラスを渡すか」を知っているのは `main()` の組み立て部分だけであり、そこだけが変わります。

**ECフォーマット追加時のコード変化（Step 3）：**

```cpp
// 新しいECDataImporterを追加する場合
class ECDataImporter : public AbstractImporter { // ← 新規追加のみ
protected:
    void parseData() override { /* EC特有のパース処理 */ }
};

// BatchImportJob と ManualImportController は一切変更しない
// main()の組み立て部分だけを変える
int main() {
    ECDataImporter ec;   // ← ここだけ追加
    BatchImportJob batch;
    batch.run(&ec);      // ← 受け取る側（BatchImportJob）は変更不要
    return 0;
}
```

呼び出し元は `AbstractImporter*` という型のまま変わらないため、新しいフォーマットが増えるたびに「サブクラスを1つ追加して、`main()` の組み立て部分に渡す」だけで対応が完結します。
...
**手段A**（呼び出し元が仲介役の実装にも依存しない完全な分離を実現するため）のコードを以下に示します。
...
int main() {
    ...
    ECDataImporter ec;            // ← ECフォーマット追加時：ここで新クラスを生成するだけ
    StoreDataImporter store;
...
それぞれのステップには一長一短があります。ステップ3の「抽象×直接（インターフェース＋継承）」は強力ですが、クラス設計が複雑になるという「初期投資コスト」もかかります。どこで止めるかは、**「今後の変更頻度（ビジネス要求）」**で決断します。

*   **Step 1（具体×直接）で止めるケース：** これ以上新しいインポート形式が増える予定が絶対にない場合。コピペしたコードをそれぞれメンテナンスする手間で事足ります。
*   **Step 2（具体×間接）で止めるケース：** 新しい形式は増えないが、将来「読み込み時の共通チェック」などを全ての形式に適用したい場合。
*   **Step 3（抽象×直接）で止めるケース：** 今後も「SNS売上」や「別システムのデータ」など新しい形式が追加されると確定している場合。今すぐ初期投資コストを払ってでも、既存のコードを壊さずに拡張できる仕組みを作るのが適切です。
*   **Step 4（抽象×間接）まで進むケース：** 呼び出し元が非常に多く、インポート機能全体を別のエンジンに差し替えるといった大規模な要件がある場合。

**今回の決断：**
フェーズ2のヒアリングで確認した変更（新しいインポート形式の追加・共通ログの追加）はいずれもStep 3の範囲内で対応可能です。仲介クラス（Manager）を追加するStep 4は、現在の規模では実装コストに見合う恩恵がないため過剰と判断し、今回は**ステップ3（抽象×直接）で止める**決断を下します。

### 6-5：耐久テスト

フェーズ2のヒアリングで挙がった「将来のリスク」が実際に発生した場面をシミュレートし、Step 3の変更耐性を検証します。
...
> **実は、このStep 3の構造には名前があります。** 処理の骨格（手順の順序）を基底クラスが定義し、その中の一部のステップだけをサブクラスが差し替える——この構造を、設計の世界では **Template Method（テンプレートメソッド）パターン** と呼びます。フェーズ1から6の思考プロセスを回した結果、この名前の付いた構造にたどり着いたわけです。パターン名は「先にあった答え」ではなく、「思考の積み重ねの末に気づく名前」です。

## 🟢 フェーズ7：対策実施 ―― 変化に強いコードを完成させる

採用したStep 3の設計を、実際のコードに実装します。これまでは個別のクラスで重複していたファイル操作やDB保存の手順を、基底クラスにテンプレートとして集約します。
...
採用したStep 3の設計では、接続形態が **抽象×直接（USB-C直差し）** へと変化しています。
...
| 🔴 フェーズ6：対策検討 | Step 1〜Step 4を比較し、共通手順を基底クラスにテンプレート化して分離するStep 3を採用しました。 |
