...
> [!INFO] レゴブロックで考える：Template Methodパターン
> この章のパターンは、レゴブロックの**「分ける」**操作に対応しています。
> 「骨格は同じ、一部だけ違う」処理を、共通部分と可変部分に分けます。レゴの基台は同じで、上に乗せるパーツだけ変えるイメージです。
> コードでも同じように、処理の骨格を親クラスに固定し、変わる一部のステップだけ子クラスで上書きする構造を作ります。
...
| 観察した問題 | 分類 | 手札候補 |
|---|---|---|
| 共通処理の重複が複数クラスにある | 2. まとめる | ユーティリティ関数化（共通処理を1箇所に切り出す） |
| 処理の骨格と変わる中身が同じ場所にある | 1. 分ける | 骨格・ステップ分離（骨格を固定し、変わる部分を委ねる） |
...
「共通処理が重複しているなら、ユーティリティとして切り出せばいい」という方向性で試みます。
...
「呼び出し順序の保証」が残ったままです。各バッチクラスが `startLog() → collectData() → ... → endLog()` の順を守る責任を持ち続けます。骨格の誤用（順序違い・呼び忘れ）は実行時まで発見できません。

**手段②：骨格・ステップ分離**

ユーティリティクラスを作るのではなく、「骨格そのものを親クラスに固定する」方法をとります。変わる部分だけを抽象メソッドとし、子クラス（サブクラス）に実装させます。これなら、子クラスを作る人は「流れ」を意識する必要すらなくなります。
...
処理のステップが1つしかない場合や、流れに「守るべきルール」が存在しない場合にまでTemplate Methodを適用すると、単にクラスの数が増えるだけのオーバーヘッドになります。そのような場合は、単純なインターフェースの実装（Strategyなど）にとどめる方がシンプルです。
