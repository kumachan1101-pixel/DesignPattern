...
フェーズ1の仕様表を振り返ります。CSVインポートには「ファイルオープン→データパース→バリデーション→DB保存→クローズ」という一連のステップがあります。このうち変化が予想される仕様があります。

- **CSVのパース仕様**（列の並び・区切り文字・日付形式など）：店舗形態やフォーマット変更のたびに対応が必要です。直営店・FC店・ECと、過去にもフォーマットが増えてきた実績があります
- **計算ルール**（割引率・ポイント付与量など）：店舗の施策変更に合わせて見直されることがあります

一方、「ファイルを開いて読んで保存して閉じる」という処理の骨格（手順の順序）は、インポートという業務の本質として安定している部分です。

**仮説：パース仕様や計算ルールは、今後も変更が続く可能性がある。**
...
ヒアリングで「SNS販売データ」のインポート形式追加が予告されました。この変更が来たとき、仕様がどう変わるかを整理しておきます。

| フォーマット | 現在 | 将来（数ヶ月後） |
|---|---|---|
| 直営店（カンマ区切り・ヘッダーあり） | 対応済み | 変更なし |
| FC店（タブ区切り・不正行スキップ） | 対応済み | 変更なし |
| EC店（ポイント・会員ランク） | 今回追加 | 変更なし |
| SNS販売データ | —（なし） | 追加予定（骨格は既存と共通） |

SNS 形式が追加されると `openFile()`・`saveToDB()`・`closeFile()` という骨格の同じコードをまた新クラスにコピーすることになります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された SNS 販売データの形式も追加になると、また `openFile()`・`checkVersion()`・`saveToDB()`・`closeFile()` を新クラスにコピーし、`checkVersion()` の仕様が変わったとき 4 クラス全員が修正対象になることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける段階ではありませんが、形式が増えるたびに同じ骨格が複製されていく構造は変わりません。
...
分離の境界は決まりました。次は「形式ごとのパース処理をひとつのクラスにまとめるか、形式ごとに別クラスにするか」という粒度の問題です。

ヒアリングで把握した各フォーマットと、変わる理由を確認します。

| フォーマット | 変わる理由 |
|---|---|
| 直営店（カンマ区切り） | 直営店の仕様変更（業務担当者） |
| FC店（タブ区切り） | FC店の仕様変更（同じ業務担当者） |
| EC店（ポイント・会員ランク） | EC向け施策の変更（同じ業務担当者） |
| SNS販売データ（将来追加） | SNS連携仕様（同じ業務担当者） |

すべて同じ業務担当者が管轄していますが、変わり方は独立しています。EC店のポイント計算が変わっても FC 店のタブパースには影響しません。SNS 連携仕様が直営店のパースに波及することもありません。各フォーマットのパース処理は**独立して変わる**ため、フォーマットごとに別サブクラスへ分離します。一方、骨格（openFile → parse → saveToDB → closeFile）はシステム基盤担当が管理する共通部分であり、1 つの基底クラスに集約します。全サブクラスが `parseData()` という同じ接続点で骨格と接続します。

→ **骨格を基底クラスに集約し、各フォーマットのパース実装を個別のサブクラスへ分離する**
...
protected:
    ...
    void checkFormatVersion() {
        cout << "[全共通] ファイルメタデータの形式バージョンを確認しました。" << endl;
    }
...
| 🟣 フェーズ2：仮説立案 | フェーズ1の仕様表からインポートの「手順」は不変だが「パースルール」は変動するという仮説を立て、ヒアリングで裏付けました |
...
| **得られること** | **この章のどこで示したか** |
| --- | --- |
| 1. 変動箇所の識別 | フェーズ2の仮説立案とヒアリングで骨格（不変）とパース（変動）を区別しました |
