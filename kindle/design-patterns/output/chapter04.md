...
本書の各パターン章では、4-3を「接続点へ漏れている章固有の知識を確認する」共通の位置として使います。本章で確認する知識は、取込手順の骨格と形式ごとの差分です。
...
フェーズ4で、各Importerの`import()`が「取込の業務順序（守りたい骨格）」と「形式ごとに変わるパース・行検証」を同じ場所へ抱えていることを確認しました。対策は、業務順序を一度だけ固定し、形式ごとに変わる処理だけを差し替え点として外へ出すことです。この段階では、問題から導いた「骨格固定構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。
...
向き、掲載クラス、関係線はフェーズ1「登場クラスとクラス構成図（1-3）」から変えていません。同じ図に注記と色だけを加えました。変更前は各Importerが共通骨格と形式差分を一本の `import()` に抱え、形式追加のたびに骨格が複製されます。**この図が出発点で、以降の各段でここへ1つずつ足していきます。** `main()` はクラスではないため図に置きません。入力取得とImporter生成はクラス間の依存ではないため、フェーズ1の実行シーケンスと組み立てコードで確認します。
...
**どう解決するか（方針）：** 共通順序を骨格として1か所へ固定し、変わる部分だけを差し替え点の向こうへ出す「骨格固定構造」とします。
...
**まず、境界をクラス契約にする必要があるかを確かめます。** フェーズ5「課題の確定（5-3）」が確定したのは「この接続点を解く」ことであって、「クラスを増やす」ことではありません。次の2つがどちらも成り立つなら、クラス契約を作らず、共通部分を関数へ1つ切り出す形で止めます。
