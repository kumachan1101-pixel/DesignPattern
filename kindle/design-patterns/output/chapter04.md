...
- **生成・注入・実行による組み立て**：具体を誰が生成・所有し、どの派生を選んで共通順を実行するかを一本で決める。
...
**ここから決めること：** 共通順序と形式ごとの差分の間にどの接続点を置き、差分をどこへ配置し、形式ごとの実体をどう利用開始するかを順に決めます。

**この章の課題は1つですが、二つの判断は他章と同じです。** 契約と具体をセットで分け、その実体を生成・注入・実行の一続きで組み立てます。軸が一つなので、それぞれを一度ずつ追えば全体がつながります。

**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、共通骨格と形式差分を、別々の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
**メンバーがありません。** 骨格が使うのは、契約の確認で決めた差し替え点と、ファイル取得・保存の境界だけです。**境界をどう手に入れるかは、まだ決めていません。**
...
**穴が2つ空きました。** `gateway` と `repo` です。3-1では各Importerが `cout` で開閉と保存を代替していましたが、骨格を1本にすると、**開くのも保存するのも1か所からになります。** それを誰が用意するかは、まだ決めていません。**それを決めるのが生成の検討です。**

**この穴は、契約の確認で分けた差し替え点とは別のものです。** 形式が増えても `gateway` と `repo` は増えません。**課題ID1の分離とは関係のない依存が、骨格を1本にした副作用として表に出てきた**というのが正確なところです。生成の検討で、なぜこれを外へ出すのかを書きます。
...
**ここが終わった時点で、実体はすべて存在しています。** 骨格、3つの派生、2つの境界、それらを持つ組み立て側。**まだ説明していないのは1つだけです。骨格の `parseData()` という呼び出しに、どの形式の実装が入るのかです。次に、注入として確認します。**
...
**では、どこで実装が入るのか。** 生成の検討で書いた1行です。

**掲載箇所：`BatchApplication::runStoreImport()`** ―― 生成の検討から、実体を作る行と呼ぶ行
...
| ファイル取得・保存の境界 | 組み立て側が何を渡したか | コンストラクタ引数 | 違う。生成の検討で説明した別の依存 |
...
**これは第9章の課題ID2と同じで、分離と組み立てを通っても呼び方が変わらない例です。** 変わったのは呼び方ではなく、この1行の先で誰が順序を持ち、誰が形式を読むかです。**呼び方が変わるかどうかは、分離と組み立ての結果であって前提ではありません。** 先に見せると、まだ導いていない結論を見せることになります。

利用側が `parseData()` を直接呼ぶこともありません。`protected` なので、そもそも呼べません。

**守る範囲との照合：** **利用側の呼び方**を保っていることを、いま確認しました。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
| 生成・注入・実行による組み立て | 各Importerが入出力も抱える → `BatchApplication` がGateway・Repositoryを所有 | 派生型の生成で差し替え先を決め、同じ `import()` から骨格→契約→具体を実行する | 生成した派生が三つの差し替え点すべてで使われる |

公開入口の `import()` は変わりません。生成する派生型によって、同じ共通順の中で呼ばれる具体だけが決まります。

#### 代表ケースの実行接続

生成した派生Importerが実際に使われるまでを確認します。直営店CSVの取込1件を、派生型の生成→共通順→差し替え点→具体処理の順で追います。
...
| 2. 実行開始＋注入 | `BatchApplication::runStoreImport()` | `StoreDataImporter store(gateway, repo);` ―― **この型が3つの差し替え点の行き先を決める**（＝注入）。続けて `store.import();` | `AbstractImporter::import()` |
...
**注入が2の行に同居しています。** 第9章では注入が独立した行（引き当て関数が参照を返す行）として2回現れましたが、**この章では実体を作る行がそのまま注入の瞬間です。** 3形のうち「継承で決まる」形を選んだ結果が、この表に出ています。
