...
★フォーマット種別って何があるの。具体パターンが何とイメージできない。
...
★現状仕様では扱わないとは、どういう意味？すきっぷするということ？
...
1-3でクラス構成を確認したので、掲載コードで「何を実際に動かし、何をスタブ（仮想）で代替しているか」を先に整理します。この境界は全フェーズで共通です。★以下表の説明、他の章にはないのはなぜ？
...
★全体的にクラス単位でコードブロックを分けて、クラス別に説明を入れてほしい。
