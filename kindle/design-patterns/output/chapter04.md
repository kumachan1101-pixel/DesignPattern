...
このシステムの規模では、抽象化層の導入によるパフォーマンスコストは設計判断を絞り込む制約になりません。ただし、月次の一括インポートで全店舗分を処理する場合、ファイルサイズが大きくなり**ストリーミング設計**が必要になることがあります。

> **ストリーミング設計とは：** CSVファイル全体を一度にメモリに読み込む（一括読み込み）のではなく、1行ずつ読み込みながら逐次処理する設計のことです。大容量ファイルを扱う際にメモリ使用量を抑えるために用います。

処理の骨格と個別の解析ロジックがどう分離されているかが、この将来的な対応しやすさに影響します。案3（抽象×直接）では `AbstractImporter` の `import()` に骨格がまとめられているため、ストリーミング対応が必要になった際は `openFile()` や `import()` を骨格クラスで差し替えるだけで対応できます。案4（抽象×間接）では `IImporterManager` インターフェース越しにストリーミング対応の実装クラスに差し替えることができ、呼び出し元を変更せずに大容量ファイル対応が可能です。
...
**手段A**（現状維持という案の主旨に沿った最小変更として）のコードを以下に示します。
...
public:
    ...
    void import() {
        ...
        else if (format == "FC") parseFCData(); // ← ECフォーマット追加時：ここにelse if分岐を追加
...
**手段A**（この案の「具体×直接」という接続形態の本質を示すため）のコードを以下に示します。
