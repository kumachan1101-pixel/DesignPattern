...
この時点では、共通手順をどこへ置くかは判断しません。まず、どの手順が共通で、どこが店舗形態ごとに変わるかを確認します。
...
|---|---|---|---|
| 入力 | 直営店CSV、FC店CSV | 直営店CSV、FC店CSV、EC店CSV、形式バージョン | EC店データとバージョン情報が増える |
| 判定 | 店舗形態ごとの形式判定 | 店舗形態判定に加え、全形式でバージョン確認 | 共通判定が処理手順へ追加される |
| 加工 | 形式別にパースしDB保存する | バージョン確認後に形式別パースし、EC店はポイント・会員ランク計算を追加 | 共通手順と形式差分の両方が変わる |
| 出力 | 保存成功またはパースエラー | 保存成功、バージョンエラー、EC計算済みデータ | エラー条件と保存データが増える |
...
| 変わるのは `parseData()` など形式別処理なのに、骨格も一緒にコピーされる | 形式追加時に共通手順を再実装しなくて済む接続点を作る | 骨格を親側へ置き、形式別処理だけを差し替える案を見る |
...
今回の課題は、ファイルを開く、形式チェックをする、保存するという共通手順を守りながら、店舗種別ごとのパース差分だけを差し替えることです。そこで、次の案を比べます。
