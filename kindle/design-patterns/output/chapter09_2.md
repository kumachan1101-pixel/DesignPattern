...
**変わる理由が異なる2つの軸が一つの場所に混在していると、一方を変えるたびに他方まで修正範囲が広がっていく。それぞれの変化が独立していないとき、どちらを触っても全体が揺れるという問題が起きている。**
...
```mermaid
flowchart LR
    I["入力<br/>チケット情報、優先度判定条件、状態操作"]
    P["加工<br/>優先度を判定する、状態遷移を判定する、結果を更新する"]
    O["出力<br/>優先度、チケット状態、操作結果"]
...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| チケット情報、優先度判定条件、状態操作 | チケット状態、ユーザー種別、操作名 | 動作例テーブル、`main()`、変更要求 |
| 優先度判定、状態遷移判定、結果更新 | 優先度ルールと状態遷移の条件分岐 | 現状コードの観察、複合変更のシミュレーション |
| 優先度、チケット状態、操作結果 | 実行結果の優先度・状態・メッセージ | 動作例テーブル、実行結果、変更シナリオ表 |
