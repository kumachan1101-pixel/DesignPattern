...
現在の構成では、`TicketManager` クラスがチケットの状態遷移と優先度判定ルールをすべて管理しています。
...
このシステムの規模では、状態遷移やルール判断のパフォーマンスコストは設計判断を絞り込む制約になりません。唯一注意すべき点は、複数のヘルプデスク担当者が同一チケットを同時操作するケースです。**状態遷移の整合性を保証する競合制御**（楽観的ロックまたはシリアライズ）が、設計段階で検討が必要な制約となります。この点は案3・案4の選定に影響するため、各案のトレードオフで触れます。
...
* ※ 同時操作の競合制御：状態遷移のロジックが `StateController` に集約されているため、楽観的ロックによる整合性保証をここ一箇所に実装できる。
...
* ※ 同時操作の競合制御：`IStateController` と `ITicketState` が分離されているため、状態遷移と優先度判定それぞれに独立して楽観的ロックを実装できる。インターフェース越しに注入される構造のため、スレッドセーフな実装への差し替えも容易。
