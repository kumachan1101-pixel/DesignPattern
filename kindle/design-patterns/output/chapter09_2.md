...
この時点では、状態と優先度ルールをどう分けるかは判断しません。まず、どの入力で状態と優先度がどう変わるかを確認します。
...
flowchart LR
    ...
    I --> J
    B -->|No| K([異常出力<br>チケットIDエラー]):::error
...
|---|---|---|---|
| 入力 | チケット、ユーザー種別、操作、現在状態 | チケット、ユーザー種別、操作、現在状態、SLA基準、新状態 | 優先度ルールと状態種類が増える |
| 判定 | 状態ごとの操作可否、固定優先度判定 | 新状態を含む操作可否、SLA基準の優先度判定 | 状態判定と優先度判定が別々に変わる |
| 加工 | 状態更新と優先度計算 | 新状態への遷移とSLA優先度計算 | 二つの加工軸を分けて追う |
...
| 優先度判定の条件が顧客区分やSLAに応じて変わる | 優先度計算ルールを状態遷移とは別に差し替えられるようにする | 優先度計算を独立したアルゴリズムとして扱う案を見る |
| 状態遷移と優先度計算は別の理由で変わる | 二つの接続点を一つの構造で無理に解かず、必要な分離を分けて考える | 状態側と優先度側で別々の候補案を比較し、組み合わせるか判断する |
...
| 優先度ルールだけ分ける | SLAや顧客区分の変更を局所化できる | 状態追加の分岐は残る | 状態が固定なら有効 |
