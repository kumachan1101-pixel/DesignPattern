...
本章の追跡は**要求IDと変更ID**で行います。変更で各要求IDの内容がどう変わるか——継続・変更・追加——は、フェーズ1「変更後要求ベースライン（1-5）」の「変更種別・根拠となる変更ID」列で追えます。既存動作が落ちていないかは、フェーズ7の要求ID別回帰で確認します。
...
フェーズ1で、`TicketManager` がチケットの状態遷移と優先度計算ロジックを直接保持している現状を把握しました。届いた変更要求を踏まえ、この設計における変わる見込みと今回維持する範囲を整理します。
...
フェーズ4で、一つの進行処理が「状態ごとの操作可否・遷移・副作用」と「顧客区分・SLA基準による優先度の判定ルール」という**別々の理由で変わる2つの判断**を、同じ場所へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つのチケット進行経路へ再結合することです。この段階では、問題から導いた「状態分離構造」と「規則差し替え構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。

```mermaid
flowchart TB
    A[現在<br/>状態の振る舞いと優先度ルールが<br/>進行処理に混在] --> B[分離判断<br/>二つの変化軸を別責任へ分け<br/>一つの進行経路へ再結合]
    B --> C[課題ID1<br/>各状態が遷移と副作用を持つ<br/>状態分離構造]
    B --> D[課題ID2<br/>優先度規則を差し替える<br/>規則差し替え構造]
...
| | 内容 | どこで決まったか |
|---|---|---|
| **決まっている（目指す形）** | 状態ごとの振る舞いを共通契約の裏へ揃えて現在状態へ委譲する／優先度判定を差し替え可能なルール契約の裏へ揃える | フェーズ5（独立した2つの変化軸） |
...
**どう解決するか（方針）：** 状態ごとの振る舞いを共通契約の裏へ揃えて現在状態へ委譲する「状態分離構造」とし、優先度判定は差し替え可能なルール契約の裏へ揃えて区分で選んだルールへ委ねる「規則差し替え構造」とします。
...
**課題の順序も、ここで確かめておきます。** 別々の理由で変わるだけでなく、**一方の完了条件が他方の出力の形を指定していないか**を見ます。課題ID1の完了条件は「状態追加が新しい状態動作と遷移登録に閉じる」、課題ID2は「区分追加が新しい優先度規則と登録に閉じる」。どちらも相手に何かの形を要求していません。**だから課題ID1から先に扱っても、課題ID2から先に扱っても同じ結論になります。** ここでは5-3の並び順に従って課題ID1から扱います。片方が他方へ形を要求している場合は、要求する側の契約を先に決めないと、後から作り直すことになります。

| 段 | 何を決めるか | 軸ごとか |
|---|---|---|
| 1【契約】 | 境界を関数・表・クラス契約のどれで表すかと、何が渡るかを決める | 軸ごと |
...
書き換える前のコードはフェーズ3「変更を試みる（3-1）」にあります。**ここで全部を再掲することはしません。** 各段で、その段が触る数行だけを変更試行（3-1）から抜き出し、変更後と並べます。

---

**1. 境界の表し方と、何が渡るかを決める 【契約】**

**まず、境界をクラス契約にする必要があるかを確かめます。** フェーズ5「課題の確定（5-3）」が確定したのは「この接続点を解く」ことであって、「クラスを増やす」ことではありません。次の2つがどちらも成り立つなら、クラス契約を作らず、関数を1つ切り出す形で止めます。

1. 変わる側の判断が、コード上1か所にしか現れない
2. その種類が増える見込みが、2-4のリスクIDにも5-2の評価にも挙がっていない

**第9章はどちらも成り立ちません。** 状態ごとの可否は `updateStatus()` の `switch` に5つの `case` として現れ、優先度の判定は `calculate()` の `if` 連鎖に3か所現れます。そして2-4のリスクID1（区分の細分化）とリスクID4（新状態の追加）が、どちらも「種類が増える」と言っています。**現に変更要求で、状態が1つと区分が1つ増えました。** だから関数の切り出しでは止めず、契約まで進みます。
...
**それでも、書いてみるまで確定はしません。** 2で骨格を実際に書き、この2つで足りることを確かめます。足りなければここへ戻って取り直します。**戻る先が1段だけで済むのは、変更前のコードという確かめようのある根拠から決めているからです。**
...
**この段の守る範囲の照合：** 守る範囲の4つ（公開操作・保存・既存状態の遷移規則・出力）のどれにも触っていません。契約を宣言しただけで、既存のコードは1行も書き換えていません。
...
呼べるのは、具体が増えても変わらない側だけです。出て行った側が出て行った側を呼べば、具体が増えるたびに両方を触ることになり、分けた意味が消えます。
...
**第9章は上の型です。** 1つの `TicketService` が、受付中のチケットも解決済みのチケットも扱います。**同じインスタンスが、呼び出しごとに違う状態の実体を使い分けなければなりません。** だから契約は外の型（`ITicketPhase`）になります。

もし状態ごとに `TicketService` を丸ごと別インスタンスにするなら、契約を基底クラスの内側へ置けます。しかしそれは、チケットを1件読むたびに骨格を作り直すということです。保存済みチケットの状態は実行時にしか分からないので、成り立ちません。**この判断は、1で「現在状態を引数で渡さず、オブジェクトが体現する」と決めたことの延長です。** 体現者が実行中に入れ替わるなら、契約は外に置くしかありません。

残った側は、公開操作を持つクラス——つまり `TicketManager` です。ただし**中身が変わったので、名前が合わなくなります。**

1-3のクラス一覧で、`TicketManager` の役割を「**状態遷移、操作可否、優先度計算の呼び出し**」と書きました。1で決めた形では、この3つのうち**状態遷移と操作可否が契約の裏へ出て行きます。** 優先度計算も課題ID2で同じく出て行きます。**"Manager" が指していた仕事が、ほぼ残りません。**
...
**`PriorityCalculator` が消えました。** 優先度の判定を1で外へ出したからです。残った2つは1-4のままで、持ち方も値メンバのまま——**この章で変えるのは持ち方ではありません。** 3つ目が要るかどうかは、契約の実体をどこから得るかが決まってから分かります。
...
**この段の守る範囲の照合：** 4つのうち3つに触りました。**公開操作**は `updateStatus(id, op, assigneeId)` から `assign(id, assigneeId)` などへ分かれます。守る範囲で挙げたのは「`create()` で起票し、操作を受けて状態を進める」という入口の形であって、メソッド名の粒度ではないので、これは範囲内です。**保存**は `repo.get()`／`repo.save()` のまま変えていません。**出力**の1行も位置と内容を変えていません（コードでは省略表記にしていますが、7-1で実物を確認できます）。**既存状態の遷移規則**は、この段では1つも書いていません。
...
**この段の守る範囲の照合：** 既存状態の遷移規則に触りました。ただし触ったのは**書き方**だけです。受付中→対応中、対応中→解決済み、エスカレーション、差し戻し、再受付——1-2bの遷移表と1本ずつ突き合わせて、増えても減ってもいません。増えたのは変更要求で追加した保留の2本（受付中→保留中、対応中→保留中）と再受付（保留中→受付中）だけで、これは要求ID2そのものです。公開操作・保存・出力には触っていません。
...
class TicketService {
    TicketRepository repo;      // 1-4から継続：チケットの保存・取得
    UserDatabase     users;     // 1-4から継続：依頼者の区分照会
    TicketPolicySet  policies;  // この段で決めた箱。実装を引き当てる相手
public:
    void create(const std::string& ticketId, const std::string& userId);
    void assign(const std::string& ticketId, const std::string& assigneeId);
    // resolve / escalate / reopen / hold / sendBack も同じ形
...
**この段の守る範囲の照合：** 公開操作・保存・既存状態の遷移規則・出力のどれにも触っていません。増えたのは `TicketService` のメンバー1つだけです。

**この段が終わった時点で、実体はすべて存在しています。** 具体状態5つ、具体ルール3つ、それを持つ箱、箱を持つ骨格。**まだ起きていないのは1つだけです。骨格の `ITicketPhase` と書かれた変数へ、具体が入ることです。それが5です。**

---

**5. 実装が骨格へ入る 【注入】**

2で空けた穴は `phaseFor(t.status)` でした。4で、この関数を `TicketPolicySet` が持ち、その箱を `TicketService` が値メンバで持つと決めました。**残っているのは、この関数が何を返し、それを受けた骨格の変数に何が入るかです。**
...
|---|---|---|---|
| **呼び出しごとに引き当てる** | 保存された状態値、入力の属性 | 引き当て関数が契約への参照を返す行 | **第9章（この章）**、第3章、第12章 |
...
**骨格が `policies` へ届く方法は、注入とは別の話です。** 4で値メンバと決めましたが、外から参照で受け取っても `phaseFor()` が実装を返すことは変わりません。**注入はどちらの持ち方でも起きます。**

**骨格の3段そのものは、2で書いたときから1行も変わっていません。** 変わったのは `phaseFor()` の前に `policies.` が付いたことだけです。変わっていたら、骨格の切り方を間違えていたことになります。

**この段の守る範囲の照合：** 公開操作のシグネチャも、保存の呼び方も、出力の1行も変わっていません。変わったのは関数の内側の1行です。
...
**組み立ての行は4で確認したとおり増えていません。** 利用側が `ITicketPhase::assign()` や `IPriorityRule::getPriority()` を直接呼ぶこともありません。

**この段の守る範囲の照合：** 「`create()` で起票し、操作を受けて状態を進める」という入口の形は保たれています。呼ぶ相手の名前とメソッド名が変わりましたが、**呼び出し側が状態も区分も優先度も書いていない**点は変更前と同じです。6段すべてを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、各段で照合済みです。**
...
|---|---|---|---|
| 【契約】（軸ごと） | `updateStatus()` の巨大 `switch` → `ITicketPhase` の6操作／`calculate()` の `if` 連鎖 → `IPriorityRule::getPriority()` | 分けるかを検算し、5-3の候補（状態軸4つ・優先度軸2つ）の形を1つずつ決める。境界を実際に流れるのは2つと1つ | この契約を誰が呼ぶか＝【安定骨格】 |
...
| 【生成】（共通） | `TicketManager` が全分岐を内包 → `TicketPolicySet` が状態とルールを生成・所有し、`TicketService` が値メンバとして持つ | 箱を1つにするか2つにするかを決め、具体の生成と所有を1か所へ集める。箱自身を誰が持つかまで決め切る | 実装がどこで骨格へ入るか＝【注入】 |
| 【注入】（共通） | 利用側が区分を見て呼び分け → `phaseFor(status)`／`priorityRule(type)` が契約への参照を返す | 骨格が持つ契約の変数へ、決め手に応じた実装を呼び出しごとに入れる | すべて決まった結果としての呼び方＝【利用開始】 |
...
| 3. 【安定骨格】優先度軸 | `TicketService::create(const string&, const string&)` | `users.get(userId)` で区分を引く（決め手をそろえる） | `TicketPolicySet::priorityRule()` |
...
| 6. 【安定骨格】状態軸 | `TicketService::assign(const string&, const string&)` | `repo.get(ticketId)` で読み、`t.status` を決め手にする | `TicketPolicySet::phaseFor()` |
...
**【注入】が2回現れることに注目してください。** 組み立てのときに1回ではなく、**軸ごとに、呼び出しのたびに**実装が入ります。決め手が違えば入るものが変わります。これは3つある注入の形のうち「呼び出しごとに引き当てる」形だからで、第6章や第8章のように組み立て時に1回入る章では、この表の【注入】は1回だけ現れます。
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を同時に満たすルール差し替え構造×状態分離構造を、契約→安定骨格→具体→生成→注入→呼び方の6段で導いた。前半3段は軸ごとに、後半3段は両軸まとめて決めた |
...
ここまで問題から導いた「規則差し替え構造」と「状態分離構造」は、一般に **Strategyパターン**と **Stateパターン**と呼ばれます。ここからは題材を離れ、二つの骨格と組み合わせ方を確認します。
...
7つのフェーズを通じて、読者は1つのクラスに混在する2つの変化軸という観察から始まり、フェーズ4〜5で「優先度ルール」と「状態遷移」の独立した接続点をすべて確定しました。フェーズ6では一方ずつ試すのではなく、6段を1周しながら二軸を並べて解きました。前半3段（契約・安定骨格・具体）は軸ごとに、後半3段（生成・注入・呼び方）は共通の判断としてまとめています。StrategyとStateは、この完成構造の各責任に付いた名前です。
