## 第9章 変わるルールと状態の連鎖 ―― ビジネスルールと状態遷移が絡み合う場所をどう解くか
...
| ルール名 | 発動条件 | 結果 | 具体例 |
| --- | --- | --- | --- |
| 緊急優先度 | プレミアムユーザーからの問い合わせ | 優先度を `"High"` として扱う | プレミアム契約ユーザーのチケット |
| 通常優先度 | 一般ユーザーからの問い合わせ | 優先度を `"Normal"` として扱う | 一般ユーザーのチケット |
...
この6つの動作パターンが、このシステムが満たすべき動作の基準です。後で案を比較するときも、「どの案もこれと同じ動作を実現する」という前提で読んでください。
...
フェーズ1での観察（1-9の責任チェック表）を材料に、何が変動し、何が変わらないのかを整理します。

| **分類** | **仮説** | **根拠（フェーズ1の観察から）** |
| --- | --- | --- |
| 🔴 **変動しそう** | チケットのステータスごとの振る舞い（状態遷移） | 1-9で、状態ごとのアクションが if-else で混在していると観察したため。 |
| 🔴 **変動しそう** | 優先度を判定するビジネスルール（SLA判定等） | 1-9で、優先度計算ロジックが管理者に依存して変わると観察したため。 |
...
public:
    ...
    void execute() {
        ...
        cout << "優先度: " << priority << " で ";
        state->handle(this); // ← 直接：中間クラスを挟まずに直接呼び出す
    }
};
