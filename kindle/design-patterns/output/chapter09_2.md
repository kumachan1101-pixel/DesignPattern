...
本章からは本書の第二部です。第一部では、章ごとに主となる一つの変更課題へ焦点を当て、対応する構造を一つずつ学びました。第二部では、変更の決定者や頻度が異なる複数の課題が同じシステムに存在する場合を扱います。同じ7つのフェーズで問題を分析しますが、一つの構造だけでは変更影響を十分に分けられないとき、複数の構造を組み合わせます。「名前を先に置いて設計する」のではなく、「変更理由の種類を分析して必要な境界を選ぶ」という順序は変わりません。
...
この章でいう「SLAを厳格に運用する」とは、今回実装する「法人ユーザーをHighとする優先度ルール」と、次期候補の「一次回答期限超過を自動検出してHighへ引き上げる監視」を区別して管理することです。今回の変更要求は、前者の優先度ルール追加と「保留中」の状態追加という、二つの大きな柱です。期限監視は将来リスクとしてフェーズ2「仮説立案」で記録しますが、掲載コードには入れません。
...
2つの柱が実際のコードでどこに現れるかは、フェーズ3「問題特定」で変更を試すコードと、フェーズ7の最終コード・実行結果で追います。
...
フェーズ1「現状把握」で、`TicketManager` がチケットの状態遷移と優先度計算ロジックを直接保持している現状を把握しました。届いた変更要求を踏まえ、この設計における変わる見込みと今回維持する範囲を整理します。

### 2-1：変わりそうな仕様の見当をつける

フェーズ2「仮説立案」では、フェーズ1「現状把握」で見た仕様のうち、どの状態遷移・優先度ルール・操作条件が変わりそうかを見当づけます。責務の配置は、変更要求を当てた後の痛みと合わせて確認します。
...
この表から、今回の検討対象は「優先度ルール」と「状態ごとの振る舞い」に絞れます。2つの変化候補が同じ条件に重なると困るかどうかは、フェーズ3「問題特定」で変更を入れてから確認します。
...
「状態遷移」という変更軸と「優先度ルール」という変更軸を、今の混沌とした `TicketManager` から切り離す必要がありそうです。フェーズ2「仮説立案」で「何を変え、何を守るか」が確定しました。次のフェーズ3「問題特定」では、この変更要求を実際に今のコードで試みて、具体的にどのような問題が起きるかを明らかにします。
...
したがって2-5の出力は、「状態遷移・優先度判定・競合制御は独立して変えられるようにし、チケット操作と保存の契約は守る」という設計条件です。フェーズ3「問題特定」では変更ID1・変更ID2だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
フェーズ2「仮説立案」で確定した「状態遷移の増加」と「優先度判定ルールの変更」を、今のコードにそのまま実装してみることにしました。
...
フェーズ3「問題特定」で確認したように、チケットの「状態」が増えるたびに、チケット管理クラスのコードが肥大化し、修正のたびに予期せぬ副作用への恐怖を感じる状態にあります。ここでは、この問題の原因を構造的な観点から紐解いていきます。

### 4-1：痛みの根源を探る（観察と原因）

フェーズ3「問題特定」で観察した事実を出発点にします。**左から右へ読んでください。** 観察された事実に対して「なぜそれが起きるのか（変わる理由）」を当て、そこから「コードの構造として何が問題なのか」を言い切ります。症状ではなく構造上の欠陥として言語化することが、このステップの目的です。

| **観察（フェーズ3の事実）** | **変わる理由** | **構造で言語化した原因** |
| --- | --- | --- |
| 新しいチケット状態を追加するたびに、管理クラスが修正される | 状態の種類の追加（保留中・ベンダー確認中など） | **原因A：状態遷移ロジックの混在** |
| 優先度計算ルールが変わると、チケットの状態遷移ロジックまで再テストが必要になる | ビジネスルールの変更（SLA改定・顧客区分の細分化） | **原因B：優先度ルールの混在** |

これら2つの原因は**互いに独立した変化軸**です。優先度ルールが変わっても状態遷移は変わりません。状態の種類が増えても優先度ルールは変わりません。したがってフェーズ5「課題定義」では、一つの原因へまとめず別々の課題候補として追跡します。
...
| **今回変える責任** | **ほかの変更から守る責任** |
| --- | --- |
| チケットの状態ごとの判断（遷移先、操作可否） | 現在状態を保持し、同じ公開操作を受け付ける流れ |
| 優先度判定の条件（SLA基準、顧客要件） | 判定結果を保存して後続処理へ渡す流れ |
...
原因A・原因Bのそれぞれについて、`TicketManager` が「守りたい手順」と「今回変わった差分」のどちらを知ってしまっているかを分けます。原因Aでは状態名と遷移先の対応を、原因Bでは区分ごとの判定条件を見ます。

| 原因 | 変えたくない手順（骨格） | 骨格が知ってしまっている差分 | 接続点に残す最小の約束 |
|---|---|---|---|
| 原因A：状態遷移の混在 | 操作を受けて可否を判定し、成功時だけ保存して結果を出す | `Open` `InProgress` `Escalated` `Resolved` という状態名と、状態ごとの許可操作・遷移先の対応 | 「この操作は許されるか、許されるなら次はどの状態か」を返す操作 |
| 原因B：優先度ルールの混在 | 登録時と再受付時に優先度を決めて保存する | `Premium` ならHigh、それ以外はNormal という区分ごとの判定条件 | 「この依頼者の優先度は何か」を返す操作 |
...
> 1. **状態遷移ロジックの混在**：状態の種類が増えるたびに、管理クラスの条件分岐が直接修正される。
> 2. **優先度ルールの混在**：ビジネスルールの変更による優先度計算の変化が管理クラスに波及する。
>
...
|---|---|---|
| 原因ID1 | 状態遷移ロジックの混在：状態の種類が増えるたびに `TicketManager` の条件分岐が直接修正される | 問題ID1・問題ID2・問題ID3・問題ID4 |
| 原因ID2 | 優先度ルールの混在：ビジネスルール改定による優先度計算の変化が管理クラスへ波及する | 問題ID1・問題ID2・問題ID3 |

問題ID1〜問題ID3は、状態と優先度の両軸が同じ分岐へ混在した複合症状です。問題ID4は状態軸だけから生じています。この対応を保ったまま、次のフェーズで2軸を別々の課題候補へ変換します。

フェーズ4「原因分析」で根本原因が言語化できました。次のフェーズ5「課題定義」では、この整理を元に、解決する課題を具体的に定義していきます。
...
課題IDを定義できたので、ここまでの追跡を一列で見渡します。問題ID1〜問題ID3は二つの原因が重なった症状なので両方の行に現れ、問題ID4は状態遷移の重複だけを示すため原因ID1の行にだけ現れます。

| 問題ID（フェーズ3の痛み） | 原因ID（フェーズ4の構造原因） | 課題ID（達成目標） |
|---|---|---|
| 問題ID1・問題ID2・問題ID3・問題ID4：状態追加で状態分岐・既存操作・表示変換へ変更が広がる | 原因ID1：状態遷移ロジックの混在 | 課題ID1：状態固有の振る舞いをチケット進行から分離 |
| 問題ID1・問題ID2・問題ID3：状態分岐へ優先度判断が絡み、変更と再テストが連動する | 原因ID2：優先度ルールの混在 | 課題ID2：優先度判定をチケット進行から分離 |

## 🔴 フェーズ6：対策検討 ―― 全体のデータと実体の流れを決める

#### まず全体像 ―― どんな構造へ変えるか（抽象）

フェーズ4「原因分析」で、一つの進行処理が「状態ごとの操作可否・遷移・副作用」と「顧客区分・SLA基準による優先度の判定ルール」という**別々の理由で変わる2つの判断**を、同じ場所へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つのチケット進行経路へ再結合することです。この段階では、問題から導いた「状態分離構造」と「ルール差し替え構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。

```mermaid
flowchart TB
    A[現在<br/>状態の振る舞いと優先度ルールが<br/>進行処理に混在] --> B[分離判断<br/>二つの変化軸を別責任へ分け<br/>一つの進行経路へ再結合]
    B --> C[課題ID1<br/>各状態が遷移と副作用を持つ<br/>状態分離構造]
    B --> D[課題ID2<br/>優先度規則を差し替える<br/>ルール差し替え構造]
    C --> E[守る範囲<br/>公開操作・保存・既存状態の遷移規則]
    D --> E
```

**この図は上から下へ、現状・分離の判断・2つの課題の着地点・守る範囲の順で読みます。** 検討はこの図の並び順では進めません。**先に確定するのは、一番下の「守る範囲」です。** 守る範囲が決まらないと、どこで線を引いてよいかが決められないからです。それを確定してから、課題ID1・課題ID2を並べて扱います。
...
向きと掲載クラスは1-3から変えていません。同じ図に注記と色だけを加え、`TicketManager` のどの責任を残し、どの責任を移すかに着目します。**この図を基準に、分離と組み立てで必要な関係を足していきます。**
...
| 3. 骨格優先度軸 | `TicketService::create(const string&, const string&)` | `users.get(userId)` で区分を引く（決め手をそろえる） | `TicketPolicySet::priorityRule()` |
| 4. 優先度規則を選択 | `TicketPolicySet::priorityRule(UserType)` | USR003は一般区分なので`NormalPriority`への参照を返す | 骨格の`IPriorityRule&`が受け取る |
| 5. 契約→具体優先度軸 | `IPriorityRule::getPriority()` → `NormalPriority::getPriority()` | `Priority::Normal` を返す | 戻り値を骨格が保存 |
| 6. 骨格状態軸 | `TicketService::assign(const string&, const string&)` | `repo.get(ticketId)` で読み、`t.status` を決め手にする | `TicketPolicySet::phaseFor()` |
| 7. 状態実体を選択 | `TicketPolicySet::phaseFor(TicketStatus)` | 受付中なので`OpenPhase`への参照を返す | 骨格の`const ITicketPhase&`が受け取る |
| 8. 契約→具体状態軸 | `ITicketPhase::assign()` → `OpenPhase::assign()` | 許可操作なので `{true, TicketStatus::InProgress}` を返す | 戻り値を骨格が保存 |
...
この章では、状態も優先度規則も起動時に生成済みです。呼び出しごとに行うのは新しい実体の生成ではなく、入力に合う既存実体への参照の選択です。

### 全体の流れを実現するコードを決める

**【課題の原因】** 課題ID1は、問題ID1〜問題ID4のうち状態追加で状態分岐・既存操作・表示変換へ広がる痛み＝原因ID1（状態遷移ロジックの混在）。課題ID2は、問題ID1〜問題ID3のうち状態分岐へ優先度判断が絡み、変更と再テストが連動する痛み＝原因ID2（優先度ルールの混在）。問題ID1〜問題ID3は両原因が重なった症状として扱い、この2つを分離対象にします。
...
**ここで全体経路と対応づけるコード：** 状態ごとの判断と優先度条件を別々の接続点へどう分け、どこへ配置し、公開操作の中で二つの結果をどう接続するかをコードで示します。

**2つの課題を、別々に解いて後で合流させる形にはしません。** 5-2で確認したとおり2つは独立した変化軸なので、分離では軸ごとに契約と具体を並べて決めます。一方、組み立てでは両軸の実体を同じ生成役が作り、注入し、実行するところまでを一つの経路として追います。片方だけ先に組み立てると、もう片方を足したときに所有と受け渡しを決め直すためです。

**課題の順序も、ここで確かめておきます。** 5-2で、状態追加と優先度ルール改定は別々の理由で起きると整理しました。課題ID1の完了条件は状態追加の局所化、課題ID2は区分追加の局所化であり、どちらも相手の具体的な型を前提にしません。そこで5-3の並びに従って課題ID1から説明しますが、これは説明順であって依存順ではありません。

**全体経路との対応。** フェーズ5「課題の確定（5-3）」で決まったのは、状態ごとの振る舞いと区分ごとの判定条件を、別々の変更へ閉じる境界を作ることです。全体経路で決めた接続を、ここから**境界を関数・表・クラス契約のどれで表し、どのコードで繋ぐか**へ落とします。
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 状態ごとの可否は `updateStatus()` の `switch` に5つの `case` として現れ、優先度の判定は `calculate()` の `if` 連鎖に3か所現れます。そして2-4のリスクID1（区分の細分化）とリスクID4（新状態の追加）が、どちらも「種類が増える」と言っています。**現に変更要求で、状態が1つと区分が1つ増えました。** 状態と優先度を、それぞれ同じ問い方で呼べるクラス契約まで分けます。

分けると決めたので、コードに線を引きます。線を引いただけでは分けられません。**隙間を何が行き来するかを決めて、はじめて切り離せます。**

**5-3の接続点定義表が挙げているのは、境界を跨ぐ「候補」です。実際に何がいくつ流れるかは、ここで形を決めた結果として決まります。**
...
**5-3が挙げた候補は4つです。** 現在状態、操作、次状態、操作結果。**このうち何がいくつ境界を流れるかは、まだ決まっていません。** 1つずつ形を決めます。
...
**候補は4つでしたが、境界を実際に流れるのは2つです。** 現在状態と操作は「クラスそのものが体現する」「メソッド名になる」と決めたので、引数にも戻り値にもなりません。**消えたのではなく、値として渡す代わりに構造で表すことにしたのです。** 残る次状態と操作結果の2つが、戻り値として境界を流れます。呼ぶ側から見ると、渡すものが無くなり、返るものが2つになりました。

**候補の数と実数が一致しないのが普通です。** 5-3は境界を跨ぐものを漏れなく挙げる表で、ここはそれを形へ変える段です。形を決めれば、値として流れなくなるものが出ます。

**「返す情報の使い道を、まだ決めていないのに形を決めてよいのか」** ——ここは踏み外しやすいので、根拠を書きます。**使い道は、すでに分かっています。** 上の変更前コードで、残る側が実際に使っていたのは次の2つだけでした。

- `t.status = TicketStatus::InProgress;` ―― 遷移先の**状態値**を書き込む
- `changed = true;` ―― 成否を**真偽**で持ち、後で分岐に使う

**つまり「状態値」と「真偽」を返せば、残る側は今までどおり動きます。** 将来こう使うだろうという見込みで決めているのではなく、**変更前のコードが実際に消費していたものを、そのまま契約の戻り値にしています。** 分けるのは処理の置き場所であって、やり取りする情報ではありません。

**この根拠が使えるのは、既存の振る舞いを保ったまま構造を変える章だからです。** 変更前に無かった情報を返す必要があるとき——たとえば「なぜ操作できないのか」を理由付きで返す、遷移先の候補を複数返す——変更前のコードは根拠になりません。そのときは、1-5の変更後要求ベースラインのどの要求がその情報を求めているかを示す必要があります。この章の要求ID1〜要求ID5に、そういう要求はありません。

**それでも、書いてみるまで確定はしません。** 分離の検算で骨格を実際に書き、この2つで足りることを確かめます。足りなければここへ戻って取り直します。**戻る先が1段だけで済むのは、変更前のコードという確かめようのある根拠から決めているからです。**
...
変更前の `case` にあった `t.status = ...` と `changed = true` が、ここにはありません。**では、状態を書き換えるのは誰の仕事になるのか。それは分離の検算で決めます。**
...
**5-3が挙げた候補は2つです。** 顧客区分と、High／Normalの結果。こちらも形を決めます。
...
**候補2つのうち、境界を流れるのは1つです。** 区分はクラスが体現することにしたので流れず、結果だけが戻ります。**こちらも使い道は変更前コードで分かっています。** `t.priority = calc.calculate(db.get(t.userId).userType);` ――残る側が消費していたのは `Priority` 1つだけでした。だから `Priority` 1つを返せば足ります。
...
区分と具体規則の対応は、この代表具体だけでは見えません。全体経路で決めた `TicketPolicySet` の選択コードで一覧にします。

**2つを並べると分かること。** 候補が4つの課題ID1では、前半2つを構造へ変えた結果、6つの操作それぞれが `Transition` を返す形になりました。候補が2つの課題ID2では、1つを構造へ変えて1メソッドで足ります。**候補の数と、そのうち何を構造へ変えたかが、契約の大きさを決めます。** 契約が大きすぎると感じたら、5-3の候補へ戻ります。

ここでは部分クラス図を出しません。このコード片は契約の宣言だけで、**この地点だけを抜き出しても関係が1本もない**からです。全体経路で決めた依存関係は、利用側のコードを示す地点で図にします。
...
呼べるのは、具体が増えても今回維持する側だけです。出て行った側が出て行った側を呼べば、具体が増えるたびに両方を触ることになり、分けた意味が消えます。

**この章では、契約を骨格の外へ置きます。** 1つの `TicketService` が、受付中のチケットも解決済みのチケットも扱います。**同じインスタンスが、呼び出しごとに違う状態の実体を使い分けなければなりません。** だから進行処理とは別の型として `ITicketPhase` を置きます。

もし状態ごとに `TicketService` を丸ごと別インスタンスにするなら、契約を基底クラスの内側へ置けます。しかしそれは、チケットを1件読むたびに骨格を作り直すということです。保存済みチケットの状態は実行時にしか分からないので、成り立ちません。**この判断は、契約の確認で「現在状態を引数で渡さず、オブジェクトが体現する」と決めたことの延長です。** 体現者が実行中に入れ替わるなら、契約は外に置くしかありません。

残った側は、公開操作を持つクラス——つまり `TicketManager` です。ただし**中身が変わったので、名前が合わなくなります。**

1-3のクラス一覧で、`TicketManager` の役割を「**状態遷移、操作可否、優先度計算の呼び出し**」と書きました。契約の確認で決めた形では、この3つのうち**状態遷移と操作可否が契約の裏へ出て行きます。** 優先度計算も課題ID2で同じく出て行きます。**"Manager" が指していた仕事が、ほぼ残りません。**

残るのは、公開操作を受けて「読む・尋ねる・保存する」を順に行うことです。名前を `TicketService` へ変えます。

**判断がゼロになるわけではありません。** ユーザーIDの存在確認は残りますし、エスカレーションのときに優先度を無条件で `High` へ引き上げる判断も残ります（要求ID1。詳しくは後述）。**出て行くのは「具体が増えると変わる判断」だけ**で、操作そのものの意味に属する判断は残ります。それでも `Manager` から `Service` へ変えるのは、**残った判断が「増えない」ものだけになったから**です。

**名前を変えなくても動きます。** これは設計の必要ではなく、読み手への合図です。1-3で挙げた3つの役割のうち2つがここから消える以上、役割の説明も書き換わります。名前だけ据え置くと、クラス一覧と実物が食い違ったままになります。**逆に、責任が変わっていないのに名前だけ変えるのは、差分を増やすだけなのでしません。**

**メンバーも変わります。ただし、ここで分かるのは変更前から持ち越すものだけです。**

**掲載箇所：`TicketService`（クラス宣言・ここで分かるところまで）**
...
**`PriorityCalculator` が消えました。** 優先度の判定を契約の確認で外へ出したからです。残った2つは1-4のままで、持ち方も値メンバのまま——**この章で変えるのは持ち方ではありません。** 全体経路で決めた3つ目の `TicketPolicySet` は、この部分コードではまだ使わないため、後の組み立てコードで追加します。
...
1と3は変更前と同じで、`repo` を使うだけです。**書けないのは分離の検算です。**
...
**`t.status` は `TicketStatus` という値であって、`ITicketPhase` ではありません。** 契約の確認で「現在状態は引数で渡さず、オブジェクトそのものが体現する」と決めた結果、**値から実体を選ぶ仕事が必要です。** 全体経路で決めたとおり、`TicketPolicySet::phaseFor()` が担います。

`TicketService` 自身へ具体状態の対応表を置くと、`OpenPhase` などの名前を知り、契約の確認で分けた意味が消えます。そのため、具体群と対応表は全体経路で決めた `TicketPolicySet` に置きます。

変更前の `switch` と比べると、**状態名で分岐する `case` が1つも無くなりました。** 消えたわけではなく、`phaseFor()` の中へ移る予定です。移した先で1か所に収まるかどうかも、具体の確認で確かめます。
...
**2つの軸で、同じ形の穴が空きました。** 状態軸は「状態名 → 状態の実体」、優先度軸は「顧客区分 → ルールの実体」。**どちらも「値から実体を引く」という同じ仕事です。** ここが具体の確認で1つにまとまる伏線になります。
...
**ここで図に描けるのは、`TicketService` から2つの契約への依存だけです。** 実体を得る経路は全体像で決めていますが、この部分図は契約との依存だけに着目するため、その線は後の組み立て図で示します。
...
**ここで検算します。具体を1つ足したとき、この2つの手順は変わるか。** 1段目と3段目は変わりません。2段目も、`phaseFor()`／`priorityRule()` の**中身**が増えるだけで、この関数の呼び方は変わりません。**穴が空いたままでも、この検算はできます。** 変わってしまうなら割り残しがあるので、契約の確認へ戻って線を引き直します。この章では「保留中」を足しても法人区分を足しても3段のままでした。
...
契約と、それを呼ぶ手順が決まりました。契約の確認では契約の裏を1つずつ覗いて、渡すもの・受け取るものを確かめました。**ここで残り全部を埋めます。**

**課題ID1（状態軸）。** 契約の確認で見た `OpenPhase` の疑問が、ここで解けます。**`t.status = ...` を書かないのは、状態を書き換えるのが分離で確定した骨格の仕事だからです。** 契約の裏に置くのは「許すか」「次はどこか」の2つだけで、それ以外は何も書きません。
...
**課題ID2（優先度軸）。** こちらは契約の確認で見た `CorporatePriority` がそのまま完成形です。**判定を返す以外に書くことがないので、増える段がありません。** 残りの2クラスも同じ形で、変更前の `if` 連鎖の各分岐が1クラスずつに対応します。
...
**もう一度検算します。契約のメソッド以外に書きたくなるものがあるか。** あるなら、契約と具体の切り分けか数え方を間違えています。どちらの軸でも出てきませんでした。

**守る範囲との照合：** 既存状態の遷移規則に触りました。ただし触ったのは**書き方**だけです。受付中→対応中、対応中→解決済み、エスカレーション、差し戻し、再受付——1-2bの遷移表と1本ずつ突き合わせて、増えても減ってもいません。増えたのは変更要求で追加した保留の2本（受付中→保留中、対応中→保留中）と再受付（保留中→受付中）だけで、これは要求ID2そのものです。公開操作・保存・出力には触っていません。
...
**分離の検算で穴が2つ空きました。** `phaseFor()`（状態名 → 状態の実体）と `priorityRule()`（顧客区分 → ルールの実体）です。ここで決めるのは、**この2つを誰が持つか**です。

持つ者に必要な条件は、分離の検算で分かっています。

- **具体クラスを全部知っていること。** `phaseFor()` が `OpenPhase` を返す以上、返す側は `OpenPhase` の名前を知っていなければなりません
- **`TicketService` ではないこと。** 分離の検算で確認したとおり、知った時点で分離が壊れます

**この2つを満たす者は、具体を所有する者と同じです。** 引くには持っていなければならず、持っている以上は名前を知っています。**「引く関数を置く場所」と「具体を生成・所有する場所」が、ここで1つに重なります。** 生成の置き場所は、独立した判断ではなく分離の検算で残した穴から決まりました。

**もう1つ決めることがあります。箱は1つか、2つか。**

- **2つに分ける**（`TicketPhaseSet` と `PriorityRuleSet`）。軸が独立していることが型にも出ます。ただし `TicketService` は2つとも持つことになり、**軸を足すたびに `TicketService` のメンバーが増えます**
- **1つにまとめる**（`TicketPolicySet`）。`TicketService` が持つのは1つのまま。**軸を足しても `TicketService` は変わりません**

**1つにまとめる形を採ります。** どちらも「具体を1か所へ集める」という目的は満たしますが、**軸を足したときに触る場所が違います。** 2つの形では箱を1つ増やし、`TicketService` にもメンバーを1つ足します。1つの形では箱の中だけで済みます。**守る側を変えない、という完了条件に照らすと後者です。**

**この判断は、片方の軸だけを見ていては決められません。** 軸が1つしかなければ、箱が1つなのは当たり前で、選択肢が見えません。課題ID1・課題ID2をまとめて解いてきたのは、この1点のためです。

**箱に名前を付けます。** 状態の実体とルールの実体、つまり2種類の方針をまとめて持つので `TicketPolicySet` とします。**分離の検算で `phaseFor()` と書いた関数は、このクラスのメンバー関数になります。**
...
- **`phaseFor()`：** 分離の検算で穴になっていた関数がここに来ました。状態名から状態オブジェクトを引く `switch` です。**この章で `TicketStatus` の `switch` が残るのはここ1か所だけです。** 変更前は公開操作の中にあった同じ `switch` が、部品を引く1か所へ移りました。分離の検算で「移した先で1か所に収まるかどうかは具体の確認で確かめる」と書いたのが、これです
- **`priorityRule()`：** もう1つの穴です。顧客区分からルールを引く `if` 連鎖で、こちらも1か所だけです

**コンストラクタがありません。** 契約の確認で「次はどの状態か」を名前で返す形にしたので、状態どうしを配線する処理そのものが不要になりました。**契約の判断が、ここへ効いています。**

**状態を1つ足すときに触るのは、`phaseFor()` の `switch` へ1行と、メンバーへ1行だけです。** 区分を1つ足すときも `priorityRule()` へ1行とメンバーへ1行。具体の確認で確認した完了条件の「登録に閉じる」が、この2か所を指しています。
...
- したがって、生存期間を気にする箇所がありません。部品を借りているところが1つも無いからです

**この箱自身は、誰が作り、誰が持つのか。** ここまで答えないと「生成」に答えたことになりません。選択肢は2つです。`TicketService` が自分で持つか、外（`main()`）で作って参照で渡すか。

**この章では `TicketService` が値メンバとして持ちます。** 1-4の `TicketManager` は `repo`・`db`・`calc` を値メンバで持っていました。**この章の課題は「状態と優先度の判定を外へ出す」ことであって、部品の持ち方ではありません。** 課題に無関係なところを変えると、読者は「これも対策の一部か」と読みます。だから持ち方は1-4のままにします。
...
class TicketService {
    TicketRepository repo;      // 1-4から継続：チケットの保存・取得
    UserDatabase     users;     // 1-4から継続：依頼者の区分照会
    TicketPolicySet  policies;  // ここで決めた箱。実装を引き当てる相手
public:
    void create(const std::string& ticketId, const std::string& userId);
    void assign(const std::string& ticketId, const std::string& assigneeId);
    // resolve / escalate / reopen / hold / sendBack も同じ形
...
**コンストラクタがありません。** 3つとも引数なしで作れるからです。**1-4から入れ替わったのは1行だけです。** `PriorityCalculator calc;` が `TicketPolicySet policies;` になりました。`repo` と `users` は1-4のままです。**値メンバなので、この宣言がそのまま生成です。** 外に組み立ての行はありません。
...
**ここが終わった時点で、実体はすべて存在しています。** 具体状態5つ、具体ルール3つ、それを持つ箱、箱を持つ骨格。**まだ起きていないのは1つだけです。骨格の `ITicketPhase` と書かれた変数へ、具体が入ることです。続けて、全体経路で決めた受け渡しをコードで確認します。**

---

##### 全体経路の選択：生成済みの状態と規則を選ぶ

分離の検算で空けた穴は `phaseFor(t.status)` でした。全体経路の準備コードで、この関数を `TicketPolicySet` が持ち、その箱を `TicketService` が値メンバで持つと決めました。**残っているのは、この関数が何を返し、それを受けた骨格の変数に何が入るかです。**
...
**掲載箇所：`TicketService::assign(const string&, const string&)`** ―― 分離で確定した骨格の、穴が埋まった形

```cpp
    // 分離の検算で書いた行：  const ITicketPhase& phase = phaseFor(t.status);
    const ITicketPhase& phase = policies.phaseFor(t.status);  // ←ここで実装が入る
    Transition tr = phase.assign();
```

**この1行は、生成済み状態の選択です。** `phaseFor()`が保存済み状態IDに合う実体への参照を返し、骨格は `ITicketPhase` として受け取ります。新しい状態を生成したり、所有先へ登録したりはしていません。

**そして、どれが入るかは呼ぶたびに変わります。** 決め手は `t.status`——保存済みチケットが持っている状態です。同じ `assign()` の呼び出しでも、受付中のチケットなら `OpenPhase` が、解決済みなら `ResolvedPhase` が入ります。**組み立てのときに1回決まるのではありません。**

課題ID2も同じ形です。`priorityRule(category)` が `IPriorityRule&` を返し、`create()` の中で受けます。違うのは決め手だけです。
...
**2つの構造の違いは、この表の1列に出ます。** 状態分離は**保存された状態**が決め手、規則差し替えは**入力の属性**が決め手です。**入れる仕組みそのものは同じ**で、決め手の出どころが違います。

この章で呼び出しごとに起きるのは、保存済みデータによる選択です。どのような接続方法であっても、共通するのは、利用側が具体型を判定せず契約だけを呼ぶことです。

`TicketService` が `policies` へ届く方法は値メンバです。外から参照で受け取る構成でも `phaseFor()` による選択はできますが、この章では生成・所有と生存期間を一か所に置くため値メンバにしています。

**骨格の3段そのものは、分離の検算で書いたときから1行も変わっていません。** 変わったのは `phaseFor()` の前に `policies.` が付いたことだけです。変わっていたら、骨格の切り方を間違えていたことになります。
...
**組み立ての行は全体経路の準備コードで確認したとおり増えていません。** 利用側が `ITicketPhase::assign()` や `IPriorityRule::getPriority()` を直接呼ぶこともありません。

**守る範囲との照合：** 「`create()` で起票し、操作を受けて状態を進める」という入口の形は保たれています。呼ぶ相手の名前とメソッド名が変わりましたが、**呼び出し側が状態も区分も優先度も書いていない**点は変更前と同じです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、全体経路と詳細コードで照合済みです。**
...
classDiagram
    ...
    note for TicketPolicySet "【新設】具体状態・具体ルールを生成・所有し、決め手で引き当てる"
...
|---|---|---|---|
| 契約と具体による分離 | 状態・優先度の分岐 → `ITicketPhase`・`IPriorityRule` と各具体 | 状態別可否・遷移と優先度値を別の契約・具体へ閉じる | `TicketService` は委譲と保存だけを行う |
| 実体の組み立て | `TicketManager` が全分岐を内包 → `TicketPolicySet` が具体を所有 | 状態ID・利用者区分で契約参照を引き、起票とアサインの公開操作から実行する | 二つの軸は各引当箇所以外で交わらない |

状態操作は目的別の公開操作へ分かれ、起票の呼び方は変わりません。どちらも内部では、保存値を鍵に契約の具体を引いて実行します。

### 6-2：システム全体の契約とデータ配置を確定する

採用システムの契約、生成場所、所有関係を一表で確定します。
`TicketPolicySet` は全Phaseと全ルールを値メンバとして所有し、
`TicketService` はその `TicketPolicySet` と、保存先・依頼者台帳を、
いずれも値メンバとして持ちます。

**掲載箇所：`TicketService::create(const std::string&, const std::string&)`** ―― 6-1で確定した完成形の全文です。

```cpp
// TicketService 内：選択判断を持たず、所有する部品へ問い合わせる
...
| どう実装を渡すか（組み立て方法） | `phaseFor()`／`priorityRule()` が契約への参照を返し、骨格の変数へ呼び出しごとに入る | どの具体が入ったか |
| 安定側はどう実行するか | 利用側は `create()` や `assign()` などの操作だけを呼ぶ | 現在どの状態か、どの優先度ルールか |

状態とルールは `TicketPolicySet` が値メンバとして所有し、その
`TicketPolicySet` を `TicketService` が値メンバとして持ちます。
`main()` が組み立てる部品はありません。保存されるチケットが持つのは
...
操作名の文字列だけは、実行結果の表示を一貫させるための語彙です。

#### 完成後のクラス一覧

完成コードで定義する型を先に一覧化します。責任を持つ15クラスの依存方向と実現関係は、直後のクラス図で確認します。
...
このほかに、値だけを運ぶ型が2つあります。`Transition`（操作結果と次状態）と `UserInfo`（氏名と契約区分）です。振る舞いを持たないので、クラス図には描きません。
...
具体部品を所有し、`TicketService` はそれを値メンバとして持ったうえで
抽象契約だけを使います。`Ticket` 実体は現在状態を値で保持します。
この図はフェーズ6で確定した採用図と同じ定義です。
...
sequenceDiagram
    ...
    Svc-->>Main: void（状態・優先度は標準出力へ表示済み）
```
...
- **副作用：** `TicketRepository` への保存と、実行結果への1行表示
...
申請者を台帳で確認し、契約区分から初期優先度を決めて保存します。未登録のユーザーIDでは、保存にも表示にも進みません。**どの区分がどの優先度かは、この関数からは見えません。**
...
- 状態を保存してから実行結果を表示します。この順序は、表示だけが出て状態が変わっていない状態を作らないためです
...
```
--- 行3 ---
  アサイン: 状態 Open → InProgress 担当=AGT01
--- 行4 ---
...
```
--- 行6 ---
  アサイン: 状態 Open → InProgress 担当=AGT02
--- 行7 ---
...
|---|---|---|---|
| 要求ID1 | 登録時と再受付時は一般Normal・プレミアムと法人Highで計算し、エスカレーションでは全区分をHighへ引き上げる | 各`IPriorityRule`、`TicketPolicySet`、`TicketService::escalate()` | 変更要求1・2のTCK003で、作成・再受付とも法人High。TCK001はNormalで作成し、エスカレーションでHighへ<br/>**判定:** 合格 |
...
|---|---|---|---|
| 課題ID1 | 状態動作を各`ITicketPhase`へ分離 | Pending追加が新Phaseと `phaseFor()` の1行に閉じた | 新Phaseと `TicketPolicySet` の2行 |
| 課題ID2 | 優先度条件を各`IPriorityRule`へ分離 | 法人追加が新Ruleと `priorityRule()` の1行に閉じた | 新Ruleと `TicketPolicySet` の2行 |
#### 変更前→変更後の不変条件照合
...
フェーズ3の変更影響グラフと同じ要求・同じ粒度で比べると、課題ID1の状態追加は `ITicketPhase` の実装と `TicketPolicySet::phaseFor()` の1行へ、課題ID2のSLAルール変更は `IPriorityRule` の実装と `TicketPolicySet::priorityRule()` の1行へ限定されました。`TicketService` は `TicketPolicySet` が返した契約へ操作ごとに委譲するため、片方の変更判断がもう片方へ入り込みません。
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を同時に満たすルール差し替え構造×状態分離構造を、契約と具体による分離、実体の組み立ての二つで導いた |
...
| 状態とルールの同時変化 | 同時に動くため1軸へまとめたくなる | 同時でも軸ごとに振り分ける | 公開操作が両軸の契約を順に呼ぶ |
...
ここまで問題から導いた「ルール差し替え構造」と「状態分離構造」は、一般に **Strategyパターン**と **Stateパターン**と呼ばれます。ここからは題材を離れ、二つの骨格と組み合わせ方を確認します。
...
状態が「Open」「Closed」の2つだけで、ルールも「ハイか否か」1種類だけのシンプルなシステムで、状態と規則をそれぞれクラス群へ分けると、クラス数だけが増えます。

```cpp
// 【過剰コード】状態2種類・ルール1種類のみのシンプルなシステムに
// 状態と規則をそれぞれクラスへ分けた場合の例
...
「状態が2つ以下・ルールが1種類」という条件では、状態と規則を別々のクラス群へ分けても、クラス数が増えるだけで変更耐性の恩恵がありません。変化の見込みがないなら、シンプルな実装が一つの考え方です。
...
7つのフェーズを通じて、読者は1つのクラスに混在する2つの変化軸という観察から始まり、フェーズ4〜5で「優先度ルール」と「状態遷移」の独立した接続点をすべて確定しました。フェーズ6では、生成済みの優先度規則と状態を入力に応じて選び、契約から実行する全体経路を先に決め、その後で二軸の契約・具体・所有・選択をコードへ落としました。StrategyとStateは、この完成構造の各責任に付いた名前です。
