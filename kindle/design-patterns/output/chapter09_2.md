...
フェーズ3「問題特定」で「変更が辛い」という事実が確認できました。次のフェーズ4「原因分析」では、なぜ辛いのかを構造的に言語化します。
...
クラス図の変更として書くと、次の3操作になります。**どんなクラスを新設し、何という名前にするかは、この後の二つの判断で決めます。** ここで確定しているのは操作の種類だけです。
...
- **生成・注入・実行による組み立て**：具体を誰が生成・所有し、公開操作を持つクラスへ契約として渡してどう実行するかを一本で決める。

### 課題ID1・課題ID2を二つの判断で解く
...
**ここから決めること：** 状態ごとの判断と優先度条件を別々の接続点へどう分け、どこへ配置し、公開操作の中で二つの結果をどう接続するかを順に決めます。
...
**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、状態ごとの振る舞いと区分ごとの判定条件を、別々の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
**この根拠が使えるのは、既存の振る舞いを保ったまま構造を変える章だからです。** 変更前に無かった情報を返す必要があるとき——たとえば「なぜ操作できないのか」を理由付きで返す、遷移先の候補を複数返す——変更前のコードは根拠になりません。そのときは、1-5の変更後要求ベースラインのどの要求がその情報を求めているかを示す必要があります。第9章の要求ID1〜要求ID5に、そういう要求はありません。
...
どの区分でどのクラスを選ぶかは、まだ決まっていません。**それは生成の検討で決めます。**
...
ここではまだクラス図を出しません。契約を置いただけで、**関係が1本もできていない**からです。図にできるものが無い段では、図を出しません。
...
**`PriorityCalculator` が消えました。** 優先度の判定を契約の確認で外へ出したからです。残った2つは1-4のままで、持ち方も値メンバのまま——**この章で変えるのは持ち方ではありません。** 3つ目が要るかどうかは、契約の実体をどこから得るかが決まってから分かります。
...
**`t.status` は `TicketStatus` という値であって、`ITicketPhase` ではありません。** 契約の確認で「現在状態は引数で渡さず、オブジェクトそのものが体現する」と決めた結果、**値から実体を引き当てる仕事が新しく生まれました。** それを `phaseFor()` と書きましたが、**この関数が誰のものかは、まだ決めていません。**

`TicketService` 自身に持たせることはできません。持たせれば `OpenPhase` をはじめとする具体状態の名前をここに書くことになり、契約の確認で分けた意味が消えます。**外の誰かが持つことになります。それを決めるのが具体の確認です。**
...
**ここで図に描けるのは、`TicketService` から2つの契約への依存だけです。** 実体をどこから得るかがまだ決まっていないので、その線は引けません。
...
**ここで検算します。具体を1つ足したとき、この2つの手順は変わるか。** 1段目と3段目は変わりません。2段目も、`phaseFor()`／`priorityRule()` の**中身**が増えるだけで、この関数の呼び方は変わりません。**穴が空いたままでも、この検算はできます。** 変わってしまうなら割り残しがあるので、契約の確認へ戻って線を引き直します。第9章では「保留中」を足しても法人区分を足しても3段のままでした。
...
**第9章は `TicketService` が値メンバとして持ちます。** 1-4の `TicketManager` は `repo`・`db`・`calc` を値メンバで持っていました。**この章の課題は「状態と優先度の判定を外へ出す」ことであって、部品の持ち方ではありません。** 課題に無関係なところを変えると、読者は「これも対策の一部か」と読みます。だから持ち方は1-4のままにします。
...
**ここが終わった時点で、実体はすべて存在しています。** 具体状態5つ、具体ルール3つ、それを持つ箱、箱を持つ骨格。**まだ起きていないのは1つだけです。骨格の `ITicketPhase` と書かれた変数へ、具体が入ることです。次に、注入として確認します。**

---

##### 注入：実体を契約として骨格へ渡す

分離の検算で空けた穴は `phaseFor(t.status)` でした。生成の検討で、この関数を `TicketPolicySet` が持ち、その箱を `TicketService` が値メンバで持つと決めました。**残っているのは、この関数が何を返し、それを受けた骨格の変数に何が入るかです。**
...
**「注入とはコンストラクタ引数のことだ」と覚えないでください。** 第9章はコンストラクタを使っていませんし、第4章にはそもそも入る行がありません。**共通しているのは、骨格が具体の名前を1文字も書いていないことだけです。**

**骨格が `policies` へ届く方法は、注入とは別の話です。** 生成の検討で値メンバと決めましたが、外から参照で受け取っても `phaseFor()` が実装を返すことは変わりません。**注入はどちらの持ち方でも起きます。**
...
**分離と組み立てを通っても、呼び方が変わるかどうかは課題によって違います。** そして**どちらも、分離と組み立ての結果であって前提ではありません。** 先に見せると、まだ導いていない結論を見せることになります。

**組み立ての行は生成の検討で確認したとおり増えていません。** 利用側が `ITicketPhase::assign()` や `IPriorityRule::getPriority()` を直接呼ぶこともありません。

**守る範囲との照合：** 「`create()` で起票し、操作を受けて状態を進める」という入口の形は保たれています。呼ぶ相手の名前とメソッド名が変わりましたが、**呼び出し側が状態も区分も優先度も書いていない**点は変更前と同じです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
二つの判断で確定した部分がすべて入っているかを、次の図で照合します。
...
| 生成・注入・実行による組み立て | `TicketManager` が全分岐を内包 → `TicketPolicySet` が具体を所有 | 状態ID・利用者区分で契約参照を引き、起票とアサインの公開操作から実行する | 二つの軸は各引当箇所以外で交わらない |

状態操作は目的別の公開操作へ分かれ、起票の呼び方は変わりません。どちらも内部では、保存値を鍵に契約の具体を引いて実行します。

#### 代表ケースの実行接続

生成・所有した状態と優先度規則が実際に使われるまでを確認します。TCK001の起票とアサインを一本の経路で追います。
...
| 4. 注入優先度軸 | `TicketPolicySet::priorityRule(UserType)` | USR003は一般区分なので `NormalPriority` への参照を返す | 骨格の `IPriorityRule&` へ入る |
...
| 7. 注入状態軸 | `TicketPolicySet::phaseFor(TicketStatus)` | 受付中なので `OpenPhase` への参照を返す | 骨格の `const ITicketPhase&` へ入る |
...
**注入が2回現れることに注目してください。** 組み立てのときに1回ではなく、**軸ごとに、呼び出しのたびに**実装が入ります。決め手が違えば入るものが変わります。これは3つある注入の形のうち「呼び出しごとに引き当てる」形だからで、第6章や第8章のように組み立て時に1回入る章では、この表の注入は1回だけ現れます。
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を同時に満たすルール差し替え構造×状態分離構造を、契約と具体による分離、生成・注入・実行による組み立ての二つで導いた |
...
7つのフェーズを通じて、読者は1つのクラスに混在する2つの変化軸という観察から始まり、フェーズ4〜5で「優先度ルール」と「状態遷移」の独立した接続点をすべて確定しました。フェーズ6では、分離で二軸それぞれの契約と具体を並べ、組み立てで両軸の生成・注入・実行を一つの経路へ集めました。StrategyとStateは、この完成構造の各責任に付いた名前です。
