...
**出発点。** フェーズ5を終えた時点で手元にあるのは3つです。変わる側は「状態ごとの操作可否・遷移・副作用」、変えない側は「公開操作・保存・既存状態の遷移規則」、完了条件は「状態追加が新しい状態動作と遷移登録に閉じる」でした（5-3の接続点定義表）。コードはまだフェーズ3の姿のままで、呼び出し側はこう書いています。
...
変わる側を、コードの形ではなく問いの形へ書き直します。「状態ごとの振る舞い」のままでは漠然としていて、何を切り出すか決められません。状態が変われば答えが変わる問いは、実際には何か。
...
この2つに答えられれば、状態が何種類あっても呼ぶ側は困りません。裏を返すと、これ以外は変わる側ではないということです。表示と監査ログへの記録は状態が増えても同じ処理なので、変えない側に残ります。

問いの数と粒度が、そのまま契約の形になります。操作ごとに答えが違うので、操作ごとに関数を置きます。
...
許可されない操作は既定の `reject()` が引き受けます。だから呼ぶ側は状態を判定しません。

**2. 答える側が知らなくていいものを削る 【契約】**

「次はどの状態か」を答えるとき、次の状態の**オブジェクト**を返す形にもできます。ただしそれだと状態どうしが互いを持つことになり、生成した後に配線する処理が要ります。状態を1つ足すたびに、その状態を指す既存の状態も触ることになる。完了条件に反します。

そこで**状態名**を返す形にしました。上の `Transition` が `TicketStatus next;` を持っているのはこのためです。状態どうしは互いを知りません。

決めているのは戻り値の型ですが、実質決めているのは「増やすとき何か所触るか」です。この判断をクラス図にすると、次の3つだけの関係になります。
...
状態から状態への矢印がありません。それが2で決めたことです。ここに描いていないクラス（`TicketRepository`、`AuditLog` など）は課題ID1では変わりません。

**3. この契約を、誰が呼ぶか 【安定骨格】**

問いを外へ出した以上、誰かが呼ばないと処理が進みません。呼べるのは変えない側だけです。変わる側が変わる側を呼べば、増えるたびに両方を触ることになり、分離した意味が消えます。
...
読む、現在状態へ尋ねる、返った遷移先を保存する。3段だけです。引数の `ticketId` で保存済みチケットを引き、その `status` から現在状態を得ています。状態名から状態オブジェクトを引くのは `policies.phaseFor()` で、`TicketService` は具体状態の名前を1つも知りません。

ここで一度、検算します。**状態を1つ足したとき、この3段は変わるか。** 変わらないなら、1で切った問いは足りています。変わるなら足りていないので、1へ戻って問いを取り直します。第9章では「保留中」を足しても3段のままでした。

**4. 呼ぶには、実体が要る 【注入】**

「現在状態へ尋ねる」と書いた時点で、尋ねる相手の実体が必要になります。上のコードでいえば `policies` です。手に入れ方は3つしかありません。

- 自分で作る。具体状態の名前を知ることになり、分離が壊れます
...
外から受け取れたということは、渡す前に誰かが作っていたということです。生成の責任を安定骨格から外した以上、それを引き受ける場所が要ります。
...
`TicketPolicySet` が全状態を持ち、`main()` がそれを作って渡します。ここで決まるのは生成の場所と所有と寿命です。`TicketPolicySet` にコンストラクタが無いのは、2で状態どうしが互いを参照しない形にしたので、配線する処理そのものが無いためです。2の判断がここへ効いています。
...
ここでもう一度、検算します。**契約のメソッド以外に書きたくなるものがあるなら、1から3のどこかで切り方を間違えています。** 第9章では出てきませんでした。
...
**出発点。** 変わる側は「顧客区分・SLA基準の判定条件」、変えない側は「状態処理・公開操作・保存」、完了条件は「区分追加が新しい優先度規則と登録に閉じる」でした（5-3の接続点定義表）。呼び出し側が今書いているのは、フェーズ3で見たこの1行です。
...
この1行の先で、`PriorityCalculator::calculate` が顧客区分とSLA基準を見て High か Normal かを決めています。

**1. 変わる側を「問い」に言い換える 【契約】**

区分が変われば答えが変わる問いは、いくつあるか。数えると1つでした。

- この区分の優先度は、結局どれか

課題ID1が2つだったのに対して、こちらは1つです。だから契約もメソッド1つで済みます。**問いの数がそのまま契約の大きさを決める**というのが、2つの課題を並べると見えます。
...
`getPriority()` は引数を取りません。区分ごとに別のクラスを用意するので、自分がどの区分かを聞き直す必要が無いためです。チケットの内容も状態も渡していません。渡せば、優先度ルールが状態処理を知ることになり、完了条件の「状態処理を変えない」が崩れます。

呼ぶ側が受け取るのは結果の `Priority`（1-4から存在）だけです。この判断をクラス図にすると、次の関係になります。
...
ルールから状態やチケットへの矢印がありません。それが2で決めたことです。ここに描いていないクラスは課題ID2では変わりません。

**3. この契約を、誰が呼ぶか 【安定骨格】**

課題ID1と同じ理屈で、呼べるのは変えない側だけです。`TicketService::create()` が呼びます。
...
検算します。**区分を1つ足したとき、この手順は変わるか。** 台帳を引く、ルールを選ぶ、結果を保存する——法人区分を足しても3段のままでした。
...
課題ID1・課題ID2で通った7段を、責任の移動として一覧にします。2つの課題は同じ順で同じ判断をしたので、1枚の表にまとまります。**並び順は、考えた順です。**

| ポイント | 変更前の所属 → 変更後の所属 | 設計操作・生成／注入／所有 | このポイントが決まると次に決まること |
|---|---|---|---|
| 【契約】 | `updateStatus()` の巨大 `switch` → `ITicketPhase` の各操作と `IPriorityRule::getPriority()` | 変わる側を問いへ言い換え、状態遷移と優先度判定を別々の契約へ切り出す | この契約を誰が呼ぶか＝【安定骨格】 |
| 【安定骨格】 | 状態と優先度が混ざる `updateStatus()` → `TicketService::assign()`／`create()` が委譲と保存だけを行う | 具体が増えても段数が変わらない委譲・保存の順を固定する | 契約の実体をどう手に入れるか＝【注入】 |
