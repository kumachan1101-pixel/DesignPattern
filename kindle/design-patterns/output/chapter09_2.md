...
課題ID1・課題ID2の6つのポイントが、どのクラス・関数から、どのクラス・関数へ責任を移すかを先に一覧します。断片コードを読む前に、この表で全貌をつかんでください。**並び順は、設計を導いた順です。** フェーズ5で「変わる側」は確定しているので、まずそれを【契約】へ取り出し、取り出した契約を誰が呼ぶかで【安定骨格】が決まり、呼ぶために何が要るかで【注入】と【生成】が決まる、という順に読めます。実行時に通る順はこれとは違うので、各課題ID節の末尾に置く実行接続表で確認します。各ポイントの詳しいコードも、この後の課題ID節に同じラベルで置きます。

| ポイント | 変更前の所属 → 変更後の所属 | 設計操作・生成／注入／所有 | このポイントが決まると次に決まること |
|---|---|---|---|
| 【契約】 | `updateStatus()` の巨大 `switch` → `ITicketPhase` の各操作と `IPriorityRule::getPriority()` | 状態遷移と優先度判定を別々の契約へ切り出す | この契約を誰が呼ぶか＝【安定骨格】 |
| 【安定骨格】 | 状態と優先度が混ざる `updateStatus()` → `TicketService::assign()` ほかが委譲と保存だけを行う | 状態が増えても変えない委譲・保存の順を固定する | 契約の実体をどう手に入れるか＝【注入】 |
| 【注入】 | 利用側が区分を見て呼び分け → `TicketService(repo, users, staff, log, policies)` | 組み立て済みの契約群を注入する（所有は【生成】のまま） | 渡す前に誰が作るか＝【生成】 |
| 【生成】 | `TicketManager` が全分岐を内包 → `TicketPolicySet` が状態とルールを生成・所有 | 具体状態・具体ルールの生成を1か所へ集める | 組み立てが終わり、残るのは契約の裏＝【具体】 |
| 【具体】 | 分岐に埋もれた状態別可否とルール → `OpenPhase::assign()` ほか、`PremiumPriority::getPriority()` ほか | 状態ごとの許可操作と、区分ごとの判定を実装へ閉じる | すべて決まった結果としての呼び方＝【利用開始】 |
| 【利用開始】 | 呼び出し側が操作名の文字列を渡す → `svc.assign("TCK001", "AGT01");` | 組み立てた同じ実体を使い、公開操作を1回呼ぶ | ここから実行が始まり、【安定骨格】へ入る |

この表の上から順に、変更前はどこに判断が集まっていたか、何をどこへ移すか、誰が生成・注入・所有するかを追えます。**【利用開始】が最後にあるのは、対策後の呼び方が設計の結果だからです。** `updateStatus("TCK001", "assign", "AGT01")` が `svc.assign("TCK001", "AGT01")` へ変わるのは、上の5つを決め終えてから分かることで、先に置くと根拠のない結論を先に見せることになります。課題ID節でも同じ順で説明します。
...
**どう解決するか（方針）：** 状態ごとの振る舞いを共通契約の裏へ揃え、現在状態へ操作を委譲します（状態分離構造＝State）。以下は**設計を導いた順**に並べます。前のブロックの結論が、次のブロックが要る理由になります。
...
**出発点：フェーズ5で分けた2つと、今の呼び方。** 変わる側は「状態ごとの操作可否・遷移・副作用」、変えない側は「公開操作・保存・既存状態の遷移規則」でした（5-3の接続点定義表）。呼び出し側が今書いているのは、フェーズ3で見たこの1行です。
...
この1行の先で、受付中ならアサインできて解決済みならできない、という状態ごとの判断が起きています。**その判断を、呼び出し側にも公開操作の中にも置かない。** ではどこへ置くか——変わる側を取り出すところから始めます。

**【契約】変わる側を、契約の裏へ入れる。** 状態ごとに変わるのは「その操作が許されるか」「許されるなら次はどの状態か」の2つだけです。この2つだけを答える契約 `ITicketPhase` を決めます。許可されない操作は既定の `reject()` が引き受けるので、呼ぶ側は状態を判定しません。
...
これで変わる側は契約の裏へ入りました。**では、この契約を誰が呼ぶのか。** 呼べるのは、状態が増えても変えない側しかありません。

**【安定骨格】契約を呼ぶのは、変わらない側。** 保存済みチケットを読み、現在状態へ尋ね、返った遷移先を保存する。この3段だけを `TicketService::assign()` が担います。状態が何種類あっても、この3段は変わりません。
...
ここで足りないものが1つあります。`policies` です。**契約を呼ぶ以上、変わらない側は契約の実体を持っていなければいけません。** それはどこから来るのか。

**【注入】実体は、外から渡す。** `TicketService` が自分で作れば、また具体状態の名前を知ることになり、分離した意味がなくなります。だから外から受け取ります。
...
渡せたということは、**渡す前に誰かが `policies` を作っていた**はずです。それが次に決まる部品です。

**【生成】渡す前に、作る人がいる。** 具体状態を作って持つ役目を1か所へ集めます。`TicketPolicySet` がそれです。
...
`TicketPolicySet` にはコンストラクタがありません。状態が互いを参照しないので、配線する処理そのものが無いためです。

これで組み立ては終わりです。**残っているのは、契約の裏に何を置くかだけになりました。**

**【具体】契約の裏を埋める（進行順は書かない）。** 状態クラスは、許可する操作と遷移先だけを実装します。`OpenPhase` はアサインで対応中へ、保留で保留中へ進みます。`InProgressPhase`（解決・エスカレーション・保留）、`EscalatedPhase`（解決・差し戻し）、`ResolvedPhase`／`PendingPhase`（再受付）も同じ形で、許可する操作だけを上書きします。
...
**メンバー変数が1つもありません。** 次の状態を名前で答えるだけだからです。状態を1つ足すときに書くのは、このようなクラス1つと、`TicketPolicySet::phaseFor()` の1行だけになります。

**【利用開始】その結果、呼び方はこう変わる。** ここまでの5つが決まって初めて、呼び出し側の1行が決まります。出発点で見た `manager.updateStatus("TCK001", "assign", "AGT01");` は、こうなります。
...
第2引数だった操作名の文字列 `"assign"` が、関数名になりました。**文字列で操作を選ぶ場所が、もうどこにも無いからです。** 残る引数はチケットIDと担当者IDだけで、どの状態から始まるかを呼び出し側は書きません。利用側が `ITicketPhase::assign()` や具体状態を直接呼ぶこともありません。

#### 代表ケースの実行接続

ここまでは設計を導いた順でした。**同じ6つを、ここからは実行時に通る順で貫きます。** TCK001のアサイン1件で確認します。
...
**どう解決するか（方針）：** 優先度判定を差し替え可能なルール契約の裏へ揃え、区分に応じて選んだルールへ委ねます（規則差し替え構造＝Strategy）。並べる順は課題ID1と同じ導出順で、【契約】→【安定骨格】→【注入】→【生成】→【具体】→【利用開始】です。
...
**出発点：フェーズ5で分けた2つと、今の呼び方。** 変わる側は「顧客区分・SLA基準の判定条件」、変えない側は「状態処理・公開操作・保存」でした（5-3の接続点定義表）。呼び出し側が今書いているのは、フェーズ3で見たこの1行です。
...
この1行の先で、`PriorityCalculator::calculate` が顧客区分とSLA基準を見て High か Normal かを決めています。取り出すのは、この判定条件だけです。導き方は課題ID1と同じ順をたどります。

**【契約】変わる側を、契約の裏へ入れる。** 区分ごとに変わるのは「結局どの優先度になるか」の1点だけです。それだけを答える契約 `IPriorityRule` を決めます。
...
呼び出し側が受け取るのは結果の `Priority`（1-4から存在）だけで、SLA基準・顧客区分の判定条件は契約の裏に入りました。**では、この契約を誰が呼ぶのか。**

**【安定骨格】契約を呼ぶのは、変わらない側。** `TicketService::create()` が、区分でルールを選び、返った優先度を保存します。どのルールが選ばれたかは知りません。`reopen()` も同じ形です。
...
判定に使う契約区分は、**引数の `userId` で台帳を引いて得ています。** 利用側が区分を渡すのではありません。`reopen()` も、保存済みチケットの `userId` から引き直します。

ここでも `policies` が要ります。**課題ID1と同じ理由で、変わらない側は契約の実体を持っていなければいけません。**

**【注入】実体は、外から渡す。** 渡す行は課題ID1と同じ1行です。状態とルールを別々に渡すのではなく、両方を持つ1つの箱を渡します。
...
**【生成】渡す前に、作る人がいる。** 具体ルールの所有と区分による選択も `TicketPolicySet` へ閉じます。課題ID1と同じ1行で、同じ箱が状態と優先度の両方を持ちます。
...
区分からルールを引くのは `policies.priorityRule()` で、【安定骨格】が呼んでいたのがこれです。**これで組み立ては終わりです。**

**【具体】契約の裏を埋める。** ルールクラスは判定だけを実装します。`CorporatePriority`（法人向けSLA）は High を返します。`PremiumPriority`（High）・`NormalPriority`（Normal）も同じ契約を実装し、SLA改定はルール1クラスの差し替えに閉じます。
...
**【利用開始】その結果、呼び方はこう変わる。** 課題ID2では、**呼び出し側の書き方はほとんど変わりません。**
...
変わったのは呼ぶ相手が `manager` から `svc` になったことだけで、引数はフェーズ3までと同じチケットIDと依頼者IDです。**区分も優先度も、もともと呼び出し側は書いていませんでした。** 課題ID2で変わったのは呼び方ではなく、この1行の先で誰が判定するかです。優先度は【安定骨格】を通って自動で決まり、利用側が `getPriority()` を呼ぶことはありません。
