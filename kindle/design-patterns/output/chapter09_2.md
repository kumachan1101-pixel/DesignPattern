...
### ステップ1：各処理を独立した関数として切り出す（すでに見えている共通構造に名前を付ける）
...
ただし、状態ごとの分岐は「状態を受け取り、遷移と優先度計算を行う」という**同じ形ですでに並んでいます**。共通シグネチャは分割前から見えているので、ここでの関数化は発見ではなく名前付けです。共通形が明確なら、この段階を飛ばして直接ステップ（状態インターフェースの抽出）へ進んでもかまいません。
