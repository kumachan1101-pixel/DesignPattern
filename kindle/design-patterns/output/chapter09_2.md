...
フェーズ1で確認した仕様を振り返ります。このサポートチケット管理には、チケットの状態管理と優先度計算という2つの側面があります。それぞれの仕様について、変化の予想を立てます。

- **状態遷移のルール**（どの状態でどのアクションが許可されるか）：業務プロセスの変更に合わせて、新しい状態の追加や遷移条件の変更が起きることがあります
- **優先度判定のルール**（SLA基準、ユーザー種別ごとの優先度など）：SLA契約の更新や顧客分類の見直しに合わせて改定されることがあります

この2つの変化は、それぞれ異なるビジネス要因で発生する可能性があります。SLAの見直しは契約更新で決まり、状態の追加は業務プロセスの設計変更として発生します。

**仮説：「優先度ルール」と「状態遷移ロジック」は、それぞれ異なるビジネス要因で変更される可能性がある。**
...
このシステムは、社内のITヘルプデスク部門が運用するサポートチケット管理を担っています。サービスが拡大するにつれて、対応フローの複雑さが増し、特に重要顧客向けのSLA（サービスレベル合意）の厳格化が求められるようになっています。変更の主な関係者は、ビジネスルールを管理するSLA管理チームと、業務プロセスを設計する運用プロセスチームです。SLAの基準は契約更新ごとに改定され、業務プロセスの見直しに合わせて状態の追加も起きます。これら2つの仕様が今後どう変わるかを、ヒアリングで確認します。
...
ヒアリングで「プレミアムユーザーの区分細分化」と「ベンダー確認中」などの新状態追加が予告されました。この変更が来たとき、仕様がどう変わるかを整理しておきます。

| 変更軸 | 現在 | 将来（数ヶ月後） |
|---|---|---|
| 状態：Open / InProgress / Resolved | 対応済み | 変更なし |
| 状態：Pending（保留中） | 今回追加 | 変更なし |
| 状態：ベンダー確認中 | —（なし） | 追加予定 |
| 優先度：premium → High | 現在のSLA | 変更なし |
| 優先度：corporate → High | 今回追加 | 変更なし |
| 優先度：プレミアム区分の細分化 | —（なし） | 追加予定（次の契約改定時） |

両軸が同時に増えると、`TicketManager` の 1 つのメソッドで「状態追加」と「SLAルール変更」の両影響を受けることになります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された「ベンダー確認中」状態やプレミアムユーザー区分の細分化も追加されると、`updateStatus()` と `PriorityCalculator.calculate()` の両方にさらなる修正が必要になることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、2 つの変更軸が同じクラスに混在する構造は変わりません。
...
分離の境界が2つあります。それぞれ「まとめて分離するか、個別に分けるか」を確認します。

| 変更軸 | 変わる理由 |
|---|---|
| 状態ごとの振る舞い（Open/InProgress/Resolved/Pending） | 業務プロセス見直し（スタッフ・マネージャーの判断） |
| 優先度判定ルール（SLA基準・顧客区分） | SLA基準の改定（経営判断・四半期ごと一括） |

**接続点A（状態ごとの振る舞い）**：各状態（Open・InProgress・Resolved・Pending）は、業務プロセスの見直しごとに独立して変わります。Pending の振る舞いが変わっても InProgress の処理には影響しません。状態ごとに独立して変わるため、状態ごとに別クラスへ分離します。

**接続点B（優先度判定ルール）**：SLA の基準は四半期ごとに「一括」で見直されます。premium と corporate の基準は同じ改定タイミングで変わります。一つのルールセットとして連動して変わるため、1 つの `IPriorityStrategy` として差し替える形にします。

→ **状態ごとに別クラス、優先度ルールセットは 1 クラスへ分離し、それぞれ独立して差し替えられる構造にする**
...
| 🟣 フェーズ2：仮説立案 | フェーズ1の仕様表から「優先度ルールと状態遷移はそれぞれ異なる要因で変更される」という仮説を立て、運用担当者へのヒアリングで確認した |
...
| **得られること** | **この章のどこで示したか** |
| --- | --- |
| 1. 変動箇所の識別力 | フェーズ2の仮説立案とヒアリングで優先度ルールと状態遷移を変動要因として特定した |
