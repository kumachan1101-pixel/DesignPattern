...
詳細な仕様やコードへ入る前に、1-4（実装コード）の`main()`から1件のチケットを追いかけます。このシステムは1回呼んで終わりではなく、**同じチケットへ操作を重ねるたびに状態と優先度が入れ替わる**ため、準備から複数回の操作までをまとめて見ます。
...
「Open → InProgress → Resolved」という一方向の流れが基本ですが、対応中に緊急対応が必要になれば `Escalated`（緊急対応中）へ進み、そこから解決または対応中への差し戻しに分かれます。「解決済み → 再受付」という逆流もあります。この図の対象は、現行コードにある4状態と6遷移です。
...
契約区分・優先度・状態を名前付きの値として表し、表示用の文字列へ変える関数を添えます。1-2（動作例テーブル）と1-2b（状態と優先度の遷移表）の表に出てきた値が、そのまま列挙子になっています。
...
- **処理：** コンストラクタで1-1（このシステムの仕様）の3名を登録し、以後は問い合わせに答えるだけ
...
この質問を2-3（関係者ヒアリング）で確認し、二つの変化軸を一つにまとめてよいかを判断します。
...
2-4（ヒアリングで判明した将来リスク）のリスクIDを、状態・優先度・同時更新で変えられるようにする部分と、チケット処理の安定側へ分けます。「はい」は、フェーズ6で**独立した変化軸を別々に変更しても、受付・保存・操作の入口へ影響を広げない構造か**を判定するための印です。
...
したがって2-5（変わる見込みと今回維持する範囲を確定する）の出力は、「状態遷移・優先度判定・競合制御は独立して変えられるようにし、チケット操作と保存の契約は守る」という設計条件です。フェーズ3「問題特定」では変更ID1・変更ID2だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
変更した定義は4つです。1-4（実装コード）と同じ並び順で、上から見ていきます。
...
**このフェーズで決めること：** 状態ごとの判断と優先度条件を別々の接続点へどう分け、どこへ配置し、公開操作の中で二つの結果をどう接続するかをコードで示します。

まず、契約、具体、生成・所有、受け渡し、実行を一枚の構想へまとめます。

| 接続点を変える観点 | システム全体での設計判断 | 変えたくない側が知らなくなる詳細 |
|---|---|---|
| 契約と具体 | 課題ID1の状態振る舞いを`ITicketPhase`の各具体へ、課題ID2のSLA判定を`IPriorityRule`の各具体へ置く | 状態の種類・遷移、SLA基準・利用者区分 |
| 生成・所有・受け渡し | `TicketService`が`TicketPolicySet`を値で所有し、集合が全Phase・全ルールを値で所有する。`phaseFor()`／`priorityRule()`が呼び出しごとに契約参照を返す | 具体状態・具体ルールの生成・所有・選択 |
| 公開入口からの実行 | 利用側は`create()`や`assign()`だけを呼び、保存値・利用者区分から選ばれた契約が実行される | 現在状態と優先度ルールの具体名 |

**構想上のコード経路：** `main()`（サービスを生成）→ `TicketService::create()`・`assign()`（公開入口）→ `TicketPolicySet`（状態・規則を選択）→ `IPriorityRule`・`ITicketPhase`（契約）→ 選ばれた具体処理
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 状態ごとの可否は `updateStatus()` の `switch` に5つの `case` として現れ、優先度の判定は `calculate()` の `if` 連鎖に3か所現れます。そして2-4（ヒアリングで判明した将来リスク）のリスクID1（区分の細分化）とリスクID4（新状態の追加）が、どちらも「種類が増える」と言っています。**現に変更要求で、状態が1つと区分が1つ増えました。** 状態と優先度を、それぞれ同じ問い方で呼べるクラス契約まで分けます。
...
**変更前から抜き出す箇所：`TicketManager::updateStatus(const string&, const string&, const string&)`** ―― 3-1（変更を試みる）の `switch` から受付中のケースだけを抜き出したもの（対策前）
...
**5-3が挙げた候補は4つです。** 現在状態、操作、次状態、操作結果について、境界を流す形を1つずつ確定します。
...
現在状態はクラス、操作はメソッド名で表すため、値として境界を流れません。戻すのは、変更前コードが実際に消費していた遷移先の状態値と成否だけです。この2つを `Transition` にまとめ、続く公開入口でそのまま保存処理へ渡せるかを確かめます。

**ここで確認するコード：`Transition`（構造体全体）** ―― 状態操作の可否と次状態を返す値
...
**ここで確認するコード：`ITicketPhase`（クラス全体）** ―― 状態別操作の契約
...
**ここで確認するコード：`OpenPhase`（クラス全体）** ―― 受付中の状態
...
変更前の `case` にあった `t.status = ...` と `changed = true` が、ここにはありません。構想で決めたとおり、契約が返す `Transition` を受けた `TicketService` が状態を書き換えます。
...
**変更前から抜き出す箇所：`PriorityCalculator::calculate(UserType)`** ―― 3-1の `if` 連鎖（対策前）
...
**ここで確認するコード：`IPriorityRule`（クラス全体）** ―― 利用者区分ごとの優先度を返す契約
...
**ここで確認するコード：`CorporatePriority`（クラス全体）** ―― 法人向けSLA
...
区分と具体規則の対応は、この代表具体だけでは見えません。ここまでに決めた `TicketPolicySet` の選択コードで一覧にします。
...
このコード片は契約の宣言だけです。実際の接続は、後の生成・選択・受け渡しと公開入口のコードを続けて読むことで確認します。
...
**課題ID1（状態軸）。** 直前の契約コードで見た `OpenPhase` の疑問が、ここで解けます。**`t.status = ...` を書かないのは、状態を書き換えるのが分離で確定した骨格の仕事だからです。** 契約の裏に置くのは「許すか」「次はどこか」の2つだけで、それ以外は何も書きません。

残りの4クラスも同じ形です。ここでは `OpenPhase` と対照的なものを1つ見ます。

**ここで確認するコード：`EscalatedPhase`（クラス全体）** ―― 緊急対応中の状態
...
5クラスを並べると、1-2b（状態と優先度の遷移表）の状態遷移表がそのままクラスの一覧になります。
...
**課題ID2（優先度軸）。** こちらは直前の契約コードで見た `CorporatePriority` がそのまま完成形です。**判定を返す以外に書くことがないので、増える段がありません。** 残りの2クラスも同じ形で、変更前の `if` 連鎖の各分岐が1クラスずつに対応します。
...
構想で定めた選択点は2つです。`phaseFor()`（状態名 → 状態の実体）と `priorityRule()`（顧客区分 → ルールの実体）を、同じ `TicketPolicySet` が持つことをコードで確かめます。

持つ者に必要な条件は、構想表から分かります。

- **具体クラスを全部知っていること。** `phaseFor()` が `OpenPhase` を返す以上、返す側は `OpenPhase` の名前を知っていなければなりません
- **`TicketService` ではないこと。** 構想表で確認したとおり、知った時点で分離が壊れます

**この2つを満たす者は、具体を所有する者と同じです。** 引くには持っていなければならず、持っている以上は名前を知っています。構想で同じ `TicketPolicySet` に重ねた「引く関数」と「具体の生成・所有」が、ここで一つのコードになります。

状態用と優先度用に箱を分けると、軸を足すたびに `TicketService` のメンバーも増えます。構想どおり両方を `TicketPolicySet` にまとめ、Serviceが持つ選択窓口を1つにします。状態と優先度の判断自体は、箱の中で独立したメンバーと選択関数のまま保ちます。

**ここで確認するコード：`TicketPolicySet`（クラス全体）**
...
- **`phaseFor()`：** 構想で定めた、状態名から状態オブジェクトを引く `switch` です。**この章で `TicketStatus` の `switch` が残るのはここ1か所だけです。** 変更前は公開操作の中にあった同じ `switch` が、部品を引く1か所へ移りました
- **`priorityRule()`：** 顧客区分からルールを引く `if` 連鎖で、こちらも1か所だけです

**コンストラクタがありません。** 契約コードで「次はどの状態か」を名前で返す形にしたので、状態どうしを配線する処理そのものが不要になりました。**契約の判断が、ここへ効いています。**

**状態を1つ足すときに触るのは、`phaseFor()` の `switch` へ1行と、メンバーへ1行だけです。** 区分を1つ足すときも `priorityRule()` へ1行とメンバーへ1行。直前の具体コードで確認した完了条件の「登録に閉じる」が、この2か所を指しています。

具体状態と具体ルールは `TicketPolicySet` の値メンバ、`TicketPolicySet` は `TicketService` の値メンバにします。`Ticket` が保存するのは `TicketStatus` の値だけなので、保存データと状態実体の生存期間は絡みません。外から借用を増やさず、1-4（実装コード）の値メンバ所有も維持します。

**ここで確認するコード：`TicketService`（クラス宣言・完成）**
...
ここまでで、具体状態5つと具体ルール3つの生成・所有を確認できました。次に `phaseFor()` と `priorityRule()` の戻り値を見て、同じ実体が契約への参照として選ばれることを確かめます。

---

##### 選択：生成済みの状態と規則を選ぶ

構想で定めた選択点は `phaseFor(t.status)` です。生成・所有のコードで `TicketPolicySet` が具体を持つことを確認したので、ここでは戻り値と受取側の型がともに契約への参照であることを見ます。

直前の `TicketPolicySet` 全体コードで、`phaseFor()` と `priorityRule()` がどちらも具体型ではなく**契約への参照**を返すことを確認済みです。ここでは、その戻り値を受けてすぐ呼ぶ3行だけを追います。

**ここで確認するコード：`TicketService::assign(const string&, const string&)`** ―― 選択した状態を契約から呼ぶ3行
...
公開入口は「保存値を読む→契約を選ぶ→契約を呼ぶ」の3段です。具体名と状態別の分岐は入りません。

#### 実行骨格：組み立てた実体を契約から呼ぶ

前節で、`TicketPolicySet`が全Phaseと全優先度Ruleを所有し、`TicketService`が同じ集合を使う形を確定しました。Serviceは保存済み状態を`phaseFor()`へ、利用者区分を`priorityRule()`へ渡し、返った契約だけを呼びます。

状態や優先度区分を追加しても、公開入口の三段——値を読む、契約を選ぶ、契約を実行する——は変わりません。具体の所有・選択・状態保存が、同じPolicySetを通る一つの骨格として成立しています。
...
**変更前から抜き出す箇所：`main()`** ―― 3-1の起票とアサイン（対策前）
...
**契約・具体・組み立てで決めた呼び方を、ここで実コードとしてつなぎます。** 課題によって公開入口が変わる場合と変わらない場合がありますが、重要なのは生成した同じ実体が公開入口から具体の処理まで届くことです。

**組み立ての行は生成・所有のコードで確認したとおり増えていません。** 利用側が `ITicketPhase::assign()` や `IPriorityRule::getPriority()` を直接呼ぶこともありません。

**守る範囲との照合：** 「`create()` で起票し、操作を受けて状態を進める」という入口の形は保たれています。呼ぶ相手の名前とメソッド名が変わりましたが、**呼び出し側が状態も区分も優先度も書いていない**点は変更前と同じです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、分離から実行までのコードで照合済みです。**
...
フェーズ2のリスクIDを採用構造へ再適用し、状態・優先度の入口をどこまで守れ、競合や時刻起点に何が残るかを評価します。
...
1-5（変更要求）で確定した変更後の状態遷移仕様と、完成コードのPhase実装を照合します。
...
フェーズ3で確認した「変更要求：状態追加」と「SLAルール変更」のシナリオを、3-2（変更影響グラフ）と同じ粒度で再度適用します。
...
| 🔴 フェーズ6：対策検討 | 状態契約・優先度契約と各具体を決め、`TicketPolicySet`が所有する同じ実体を保存値から引き当て、起票・アサインの公開操作から実行する構造を確定した |
...
7つのフェーズを通じて、読者は1つのクラスに混在する2つの変化軸という観察から始まり、フェーズ4〜5で「優先度ルール」と「状態遷移」の独立した接続点をすべて確定しました。フェーズ6では、二軸の契約と具体を決め、生成済みの優先度規則と状態を入力に応じて選択・受け渡しし、公開入口から実行するところまでをコードでつなぎました。StrategyとStateは、この完成構造の各責任に付いた名前です。
