...
課題ID1・課題ID2の6つのポイントが、どのクラス・関数から、どのクラス・関数へ責任を移すかを先に一覧します。断片コードを読む前に、この表で全貌をつかんでください。**並び順は実行時の呼び出し順です。** 上から下へ読めば、代表入力が流れる経路をそのままたどれます。各ポイントの詳しいコードは、この後の課題ID節に同じラベルで置きます。

| ポイント | 変更前の所属 → 変更後の所属 | 設計操作・生成／注入／所有 | 次の接続先 |
|---|---|---|---|
| 【生成】 | `TicketManager` が全分岐を内包 → `TicketPolicySet` が状態とルールを生成・所有 | 具体状態・具体ルールの生成を1か所へ集める | 【注入】のコンストラクタ引数 |
...
| 【安定骨格】 | 状態と優先度が混ざる `updateStatus()` → `TicketService::assign()` ほかが委譲と保存だけを行う | 状態が増えても変えない委譲・保存の順を固定する | 【契約】の各操作 |
...
この表の上から順に、変更前はどこに判断が集まっていたか、何をどこへ移すか、誰が生成・注入・所有するかを追えます。課題ID節でも同じ順で説明し、節の末尾に代表入力の実行接続表を置きます。
...
**どう解決するか（方針）：** 状態ごとの振る舞いを共通契約の裏へ揃え、現在状態へ操作を委譲します（状態分離構造＝State）。以下は**実行時に通る順**に並べます。【生成】【注入】で部品を組み立て、【利用開始】で1回呼び、【安定骨格】が委譲し、【契約】を経て【具体】が答える、という流れです。
...
**【生成】【注入】部品を1か所で作り、進行処理へ渡す。** 具体状態の所有は `TicketPolicySet` が持ち、それを `TicketService` へ渡します。この2つは `main()` の中で続けて起こるので、まとめて見ます。
...
**【利用開始】組み立てた実体で、公開操作を1回呼ぶ。** 担当者の操作を受けた入口が `TicketService::assign()` を呼びます。利用側が `ITicketPhase::assign()` や具体状態を直接呼ぶことはありません。
...
引数はチケットIDと担当者IDだけです。**どの状態から始まるかを呼び出し側は書きません。**
...
**【契約】共通契約 `ITicketPhase` を定義する。** 上の `.assign()` が届く先です。各操作は「次はどの状態か」を `Transition` で返します。許可されない操作は既定の `reject()` が担うので、公開操作は状態を判定しません。
...
**【具体】状態が許可操作と遷移先だけを実装する（進行順は書かない）。** 実行時に `assign()` が最終的に届くのがここです。`OpenPhase` はアサインで対応中へ、保留で保留中へ進みます。`InProgressPhase`（解決・エスカレーション・保留）、`EscalatedPhase`（解決・差し戻し）、`ResolvedPhase`／`PendingPhase`（再受付）も同じ形で、許可する操作だけを上書きします。
...
上の6ブロックを、TCK001のアサイン1件で貫いて確認します。並び順は上の説明と同じです。
...
**どう解決するか（方針）：** 優先度判定を差し替え可能なルール契約の裏へ揃え、区分に応じて選んだルールへ委ねます（規則差し替え構造＝Strategy）。並べる順は課題ID1と同じで、【生成】【注入】→【利用開始】→【安定骨格】→【契約】→【具体】です。
...
課題ID1と同じく、**実行時に通る順**で見ていきます。

**【生成】【注入】ルールも同じ箱が持つ。** 具体ルールの所有と区分による選択も `TicketPolicySet` へ閉じます。課題ID1とまったく同じ2行で、同じ箱が状態と優先度の両方を持ちます。

**掲載箇所：`main()`** ―― 組み立ての最後2行（課題ID1と同じ）
...
**【利用開始】判定を利用側が呼ぶことはない。** 課題ID1と同じ `svc.create(...)` や `svc.reopen(...)` が起点で、優先度は【安定骨格】を通って自動で決まります。
...
渡すのはチケットIDと依頼者IDだけで、**区分も優先度も呼び出し側は書きません。**
...
**【契約】 共通契約 `IPriorityRule` を定義する。** 上の `.getPriority()` が届く先です。呼び出し側は結果の `Priority`（1-4から存在）だけを受け取り、SLA基準・顧客区分の判定を知りません。
