...
```text
現在：チケット進行処理 が
      状態ごとの遷移・副作用 ＋ 優先度の判定ルール を同じ場所に持つ
                      ↓
変更後：2つの変わる理由を別責任へ分け、一つの進行経路へ再結合する
   状態ごとの操作可否・遷移・副作用 → 各状態が自分の振る舞いを持つ（状態分離＝State）  → 課題ID1
   顧客区分・SLA基準の優先度判定    → 判定規則を差し替え可能にする（規則差し替え＝Strategy）→ 課題ID2
守る範囲：公開操作、保存、既存状態の遷移規則
```

まだクラスの中身は見ません。この段階でつかんでほしいのは「状態ごとの振る舞いを状態オブジェクトへ分け（課題ID1）、優先度の判定規則を差し替え可能にして（課題ID2）、一つの進行経路へつなぐ」という筋だけです。「どのクラスが生成し、どの契約で実行するか」という具体の結論は、この後の課題ID1・課題ID2で決めていきます。決めた結論をまとめて振り返る表は、フェーズ6の末尾（6-4 設計トレース）に置きます。ここでは先に結論表を出しません。
...
|---|---|---|---|
| 課題ID1（状態処理） | 状態分離。`TicketService`が現在Phaseへ委譲し、各状態が次状態を返す | `ITicketPhase`、各状態Phase | 状態追加が新状態と遷移登録に閉じる |
| 課題ID2（優先度判定） | 規則差し替え。`TicketPolicySet`が全ルールを配線し、注入された`IPriorityRule`へ委ねる | `IPriorityRule`、各優先度ルール、`TicketPolicySet` | 区分追加が新ルールと注入に閉じる |
