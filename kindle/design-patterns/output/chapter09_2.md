...
**`PriorityCalculator` が消えました。** 優先度の判定を1で外へ出したからです。持ち方は1-4のままの値メンバで、**この章で変えるのは持ち方ではありません。** メンバーがこれで全部かどうかは、契約の実体をどこから得るかが決まってから分かります。
...
`TicketService` はその `TicketPolicySet` と、依頼者・担当者・保存先・
監査ログを、いずれも値メンバとして持ちます。
