...
まずコードに線を引きます。線を引いただけでは分けられません。隙間を何が跨ぐかを決めて、はじめて切り離せます。**跨ぐものを、両方向とも数えます。**
...
跨ぐものを数えます。**渡すもの：ありません。** 「今どの状態か」は、渡す代わりに相手そのものが体現します。受付中の振る舞いは受付中のオブジェクトが持つ、という形にすれば、状態を引数で渡す必要が消えます。**受け取るもの：2つです。** その操作が許されるか。許されるなら、次はどの状態か。出て行ったものがちょうどこの2つなので、過不足がありません。
...
**契約だけでは、数えたものが本当にそれだけなのか確かめられません。** 実装を1つ見ます。
...
- **渡すもの0の実態。** `assign()` に引数がありません。それでも「受付中からのアサイン」と分かるのは、**このクラスであること自体が『受付中』を意味している**からです。**メンバー変数が1つもない**のはそのためで、状態を引数で渡す必要が消えたことがここで見えます
- **受け取るもの2の実態。** `{true, TicketStatus::InProgress}` の `true` が「許されるか」、`TicketStatus::InProgress` が「次はどの状態か」です。返しているのは**状態名**であって、次の状態のオブジェクトではありません
...
**渡すもの：ありません。** 区分ごとに別のクラスを用意するので、自分がどの区分かを聞き直す必要が無いためです。チケットの内容も状態も渡しません。渡せば優先度ルールが状態処理を知ることになり、完了条件の「状態処理を変えない」が崩れます。**受け取るもの：1つです。** 結局どの優先度になるか。`Priority` は1-4から存在する型なので、新しく決めることもありません。
...
引数が無いのは、渡すものが無いからです。こちらも実装を1つ見て確かめます。
...
- **渡すもの0の実態。** 引数がありません。変更前にあった `if (userType == UserType::Corporate)` の**条件**が消え、**結果**だけが残りました。条件が要らないのは、**このクラスであること自体が『法人』を意味している**からです
- **受け取るもの1の実態。** `Priority::High` の1つだけです。チケットも状態も見ていません

どの区分でどのクラスを選ぶかは、まだ決まっていません。**それは4で決めます。**

**2つを並べると分かること。** 状態軸は跨ぐものが2つで、契約は6操作×`Transition`。優先度軸は1つで、契約は1メソッド。**隙間を跨ぐ数が、そのまま契約の大きさになります。** 契約が大きいと感じたら、跨ぐものを数え直します。
...
残った側は、公開操作を持つクラスです。ここで名前を `TicketManager` から `TicketService` へ変えます。状態判断と優先度判定を手放して、契約を使う側になるためです。**メンバーも変わるので、宣言から見ます。**
