...
状態と優先度は独立して変わるためP1、P2に分けます。フェーズ6の理想グラフでも、この2枝を混ぜずに扱います。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた2つの課題——**状態ごとの振る舞いを切り離すこと**と、**優先度判定ルールを単独で差し替えられるようにすること**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを分解して探します。第二部の要は「いくつの独立した変化軸があるかを数える」ことです。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる

フェーズ4では、状態追加とSLA変更が同じ TicketContext 周辺へ波及していましたが、変わる理由は別です。理想グラフをP1（状態）とP2（優先度）へ分けます。
...
|---|---|---|---|
| P1 | 状態追加の影響を文字列分岐からPhase実装と遷移元へ縮める | updateStatus()の状態選択を切る。公開操作、状態保存、イベントログを守る | 新状態追加で既存の状態選択分岐を増やさない |
| P2 | SLA変更の影響から状態処理を外し、優先度ルールへ閉じる | calculatePriority()のユーザー種別・期限判定を切る。優先度の入力・出力と状態からの独立を守る | SLA変更でPhase実装を触らず、PriorityRuleだけを交換する |

P1とP2は理想グラフの独立した枝なので、IDは重複させず1行ずつ定義します。

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 状態処理を状態ごとのクラスへ移す | 状態別の振る舞いを分離できる | 現在Phaseへ公開操作を一律委譲する |
| P2 | 優先度計算を関数へ出す | SLA判定を状態処理から読める形で分けられる | IPriorityRule契約へ上げ、組み立てから注入する |

ここからP1の状態分岐とP2の優先度判定だけをおさらいします。
