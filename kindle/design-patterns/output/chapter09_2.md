...
**着目する共通点：** 現状コードはすでに共通点を持っています。状態を `Ticket`（`TicketRepository`）に、優先度を `Priority` にそろえて保存・追跡し、公開操作はどれも `updateStatus` を通るという形にそろっています（データと流れの共通点）。現場では、状態や優先度の持ち方がばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、公開操作や保存の形を変えず、変わる「状態ごとの振る舞い（P1）」と「優先度ルール（P2）」を別々の契約へ外に出せば済みます。フェーズ6では、この共通点と2つの課題を組み合わせて、一つの完成システムを設計します。
