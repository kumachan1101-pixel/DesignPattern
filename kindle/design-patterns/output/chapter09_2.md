...
**変更前から抜き出す箇所：`TicketManager::updateStatus(const string&, const string&, const string&)`** ―― 3-1の `switch` から受付中のケースだけを抜き出したもの（対策前）
...
具体状態と具体ルールは `TicketPolicySet` の値メンバ、`TicketPolicySet` は `TicketService` の値メンバにします。`Ticket` が保存するのは `TicketStatus` の値だけなので、保存データと状態実体の生存期間は絡みません。外から借用を増やさず、1-4の値メンバ所有も維持します。
