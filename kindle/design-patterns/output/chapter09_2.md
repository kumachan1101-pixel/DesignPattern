...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。担当者が問い合わせチケットを新規作成すると、内部は申請内容を検証→チケットを作成して状態Open・優先度を付与→保存の順に進みます（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 利用者種別からチケット優先度を決める | 要求ID1 |
| 機能ID2 | チケット状態と操作を管理する | 要求ID2、要求ID4 |
| 機能ID3 | チケットの状態と優先度を保存・取得する | 要求ID3 |
| 機能ID4 | 未登録入力と許可されない操作を拒否する | 要求ID5 |
...
|---|---|---|---|
| 機能ID1 | 変更<br/>根拠: 変更ID1 | 一般・プレミアム・法人から優先度を決める | 要求ID1 |
| 機能ID2 | 変更<br/>根拠: 変更ID2 | Pendingを含む状態と操作を管理する | 要求ID2、要求ID4 |
| 機能ID3 | 変更<br/>根拠: 変更ID2 | Pendingを含む状態と優先度を保存・取得する | 要求ID3 |
| 機能ID4 | 継続<br/>根拠: — | 未登録入力と許可されない操作を拒否する | 要求ID5 |
...
|---|---|---|---|
| チケット進行処理が全状態の操作可否・遷移を分岐する | Pending追加で既存状態と全操作を再確認する | 状態固有の振る舞いをチケット進行から分離する | 状態は独立して追加・変更される |
| 同じ進行処理が顧客区分ごとの優先度条件も分岐する | 法人追加で状態処理まで同時修正する | 優先度判定を状態進行から分離する | 状態と優先度は別の理由で変わる |
...
課題ID1と課題ID2を、次の三つの観点で一つの完成構造へ変換します。
...
|---|---|---|---|
| 機能ID1 | 変更 | 要求ID1 | 一般・プレミアムを回帰し、法人Highを受入確認<br/>**判定:** 合格 |
| 機能ID2 | 変更 | 要求ID2、要求ID4 | 既存4状態・操作を回帰し、Pending遷移と保留を受入確認<br/>**判定:** 合格 |
| 機能ID3 | 変更 | 要求ID3 | 既存状態・優先度を回帰し、Pending保存・取得を確認<br/>**判定:** 合格 |
| 機能ID4 | 継続 | 要求ID5 | 未登録・不正操作の拒否を回帰確認<br/>**判定:** 合格 |
...
|---|---|---|
| `updateStatus()` の `status` 文字列分岐（課題ID1） | **修正しない** | 振る舞いを各状態クラスへ移した |
