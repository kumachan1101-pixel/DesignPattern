...
区分が引数から消え、結果だけが戻ります。**こちらも使い道は変更前コードで分かっています。** `t.priority = calc.calculate(db.get(t.userId).userType);` ――残る側が消費していたのは `Priority` 1つだけでした。だから `Priority` 1つを返せば足ります。
...
- したがって、生存期間を気にする箇所がありません。部品を借りているところが1つも無いからです。**この形をどう持つか（自分で持つか、外から借りるか）は5で決めます。**
...
**コンストラクタがありません。** 3つとも引数なしで作れるからです。**1-4から入れ替わったのは1行だけです。** `PriorityCalculator calc;` が `TicketPolicySet policies;` になりました。`repo` と `users` は1-4のままです。
