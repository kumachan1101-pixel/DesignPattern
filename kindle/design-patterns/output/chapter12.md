...
* **得られること3：** 複数のパターンを組み合わせることで、複雑なワークフローを疎結合（変更の影響が特定のクラスだけに閉じる状態）に保ちつつ、新しい承認フローにも対応できる設計手法を説明できるようになる。
...
**得られること1**（3つの変わる理由の識別）：フェーズ2の確定テーブルで、「承認状態の変化」（State）・「関係者への通知」（Observer）・「承認可否のルール判定」（Strategy）という3つの変化軸が、それぞれ独立した「変わる理由」として識別できることを確認しました。「このコードが変わるとき、それは誰の判断によるものか」という問いが、絡み合った責務を解きほぐす起点になります。
...
**得られること3**（疎結合なワークフロー設計の説明）：フェーズ7の変更シナリオ表で、承認状態が変わるときは `IApprovalState` の実装クラスのみ、通知先が変わるときは `IObserver` の登録のみ、判定ルールが変わるときは `IApprovalRule` の実装クラスのみを変えればよい設計を確認しました。「3つの変化がそれぞれの場所に閉じている理由」を構造から説明できる状態が、設計議論を具体的にします。
...
承認ワークフローシステムという题材を通じて、複数のパターンが交差する複雑な設計を段階的に整理する思考を体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。
