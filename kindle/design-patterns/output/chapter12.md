...
動作例テーブルの1行目（通常申請→審査待ち状態→管理者に通知）と比べると、状態名が「審査待ち」ではなく「承認待ち」、通知先が「管理者」ではなく「申請者」になっており、表現がまだ一致していません。現状コードはシステムの「骨格だけを動かしている」段階であり、詳細な状態名や通知先が固まっていない実装です。次のフェーズで変更が来たときに何が起きるかを確認します。
