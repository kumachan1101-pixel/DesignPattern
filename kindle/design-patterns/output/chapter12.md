...
public:
    ...
    void handle(WorkflowManager* wm) override {
        ...
        // ← まだ通知先がハードコードされている
        cout << "申請者に通知" << endl;
        // wm は次ステップの通知分離構造通知のために受け取っているが、
        // このステップではまだ使わない
        (void)wm;
    }
};
...
class WorkflowManager {
    ...
    vector<INotificationListener*> listeners;  // ← 通知分離構造 リスト
public:
    void setState(IWorkflowPhase* s) { state = s; }
...
`WorkflowManager` は現在の `IWorkflowPhase` を保持し、イベントを現在状態へ渡します。各状態オブジェクトが次状態を選び、Contextの `phase` を更新するため、「状態によって振る舞いが変わる」という状態分離構造の構造をコード上でも確認できます。
...
```cpp
class WorkflowManager {
    // 状態分離構造グラフとListenerはBatchApplicationが所有し、
...
|---|---|---|
| 新しい承認段階（二次承認等）を追加 | `WorkflowManager` の状態管理・通知・ルール判定を全て修正 | 新しい 状態分離構造 クラスを追加するだけ |
