...
| 🟡 フェーズ3：問題特定 | 承認状態が増えるたびに `if-else` 分岐が爆発する「痛み」をシミュレーションした。
 ...
 |
| 🔴 原因分析 | ワークフローのフロー（骨格）と、個別のビジネスルールが混在している構造的問題を特定した。

 |
| 🟣 課題定義 | 状態遷移・通知・ルール判定という3つの接続点を課題として定義した。

 |
| 🟢 対策案検討 | State、Observer、Strategy パターンを複合適用する案4を採用した。

 |
| 🔵 対策実施 | 責務をインターフェースで疎結合化し、変更影響をクラス単位に局所化した。
...
```mermaid
classDiagram
    %% Context → WorkflowManager, State → IWorkflowState, Strategy → IApprovalStrategy
...
```cpp
// 単純なステータス変更のみのフローに、StateとStrategyを導入するのは過剰です。
// この場合は単純な if 文で書くべきです。
