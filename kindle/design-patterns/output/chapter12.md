...
|---|---|---|---|
| F1 | 新しい承認ルートの追加が継続的に発生する | 継続的に | 運用担当者から海外出張ルート・プロジェクト限定フローの必要性を直接確認 |
| F2 | 通知先リストの拡張・変更が繰り返される | 継続的に | 関連部署への通知追加ニーズが言及された |
| F3 | 通知の送信失敗を記録し再送する運用が求められる | 継続的に | 実運用でキュー化した場合、送信先の停止で届かない事例への対応が必要 |
...
2-4のF-IDを、承認ルートと通知で変えられるようにする部分と、申請処理の安定側へ分けます。「はい」は、フェーズ6で**ルートや通知運用が変わっても、申請の受付・状態保存・結果へ影響を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：新しい承認ルートの追加が継続的に発生する | はい | 状態ごとの承認操作と遷移先 | 申請IDを使う操作、状態保存、処理結果 |
| F2：通知先リストの拡張・変更が繰り返される | はい | 通知先の取得と通知手段の登録 | 状態保存後に該当通知だけを送る順序 |
| F3：通知の送信失敗を記録し再送する運用が求められる | はい | 失敗記録、キュー、再送方針 | 送信先ごとの受付結果、申請状態の確定 |

したがって2-5の出力は、「承認ルート・通知先・再送方針は変えられるようにし、申請操作・状態保存・結果契約は守る」という設計条件です。フェーズ3ではR1〜R3だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
**1つ目の痛み：状態管理とアクションの密結合。** R1の通常・緊急ルートを追加するために、`WorkflowManager`内の状態、申請種別、承認者役職の`if-else`を同時に修正しました。「ある状態で誰が何をできるか」というルールが一つのメソッドに混在しています。

**2つ目の痛み：通知と判定の責務過多。** R2の通知先判定とR3の部署別上限を、ワークフローの状態遷移と同じ場所へ追加しました。通知だけ、または承認判定だけを確認できず、今回の三要求をまとめて読み解く必要があります。

**3つ目の痛み：通知失敗が状態処理へ染み出したこと。** R2の送信先別結果を返すため、`process()`の状態遷移コードへ送信可否と失敗継続の判断を追加しました。R3の部署別上限も同じメソッドを修正するため、通知結果と承認判定が状態遷移コード上で衝突しています。

フェーズ3で「変更が辛い」ことが確認できました。次のフェーズ4では、なぜ辛いのかを構造的に言語化します。

---
> **📌 問題（確定）**
> R1の状態遷移、R2の通知先と送信結果、R3の部署別承認判定を`WorkflowManager::process()`へ同時に追加した。緊急ルートでは通知の二重送信も発生し、三つの確定要求が同じメソッドで衝突した。
---
...
|---|---|---|---|
| P1：状態処理 → ワークフロー進行 | 現在状態、操作、次状態、状態保存結果 | 状態固有の操作可否・遷移先 | 公開入口、状態保存、既存フェーズ |
| P2：通知 → 状態確定後の処理 | 状態変更イベント、通知受付結果 | 通知手段・宛先 | 状態確定後に通知する順序、他通知、処理本体 |
| P3：承認判定 → 承認操作 | 金額、承認可否（承認者IDは入口で存在確認） | 承認上限・組織別ルール | 承認操作、状態遷移、既存ルール |

システム全体の課題は、状態遷移・通知・承認判定を `WorkflowManager` から別々に外し、ワークフロー進行の中で「判定→状態確定→通知」の順序を守って接続することです。

---
> **📌 課題（確定）**
> P1は状態固有の遷移判断、P2は通知手段、P3は承認可否の判定を、それぞれワークフロー進行から切り離す。ここでは必要な接続データと守る順序までを課題とし、具体的なクラス名・生成・登録・注入方法はフェーズ6で決める。
---

ターゲットが3つに絞られました。次のフェーズ6では、P1〜P3を入力に一つの完成システムを設計します。

**着目する共通点：** 現状コードはすでに共通点を持っています。状態遷移・通知・承認判定はどれも同じ `process()` に載り、状態は `WorkflowCaseRepository` に、承認判定は `canApprove` にそろえて扱っています（流れとデータの共通点）。現場では、状態や判定の持ち方がばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、公開入口や保存の形を守ったまま、変わる3軸（状態遷移P1・通知P2・承認判定P3）だけに絞って対策を考えられます。それぞれをどう分けてどこへ置き、どう接続するかは、フェーズ6でこの共通点と3つの課題を組み合わせて設計します。
...
| フェーズ5の接続点 | 進行入口は残し、状態・通知・承認を各契約へ委ねればよい | P1を `IWorkflowPhase`、P2を `INotificationListener`、P3を `IApprovalRule` へ置く |
...
1. P1：状態が満たす契約 `IWorkflowPhase`（`handle`）を新設し、遷移判断を各Phaseへ移す（状態分離構造）。
2. P2：通知先が満たす契約 `INotificationListener`（`onStatusChanged`）を新設し、登録制にする（通知分離構造）。
3. P3：承認判定を担う契約 `IApprovalRule`（`canApprove`）を新設し、差し替え可能にする（ルール差し替え構造）。
...
|---|---|---|---|
| P1 | 状態遷移の契約 `IWorkflowPhase` を新設する | 各Phaseが `handle()` で遷移判断を持つ | ステップ1 |
| P2 | 通知の契約 `INotificationListener` を新設する | 各Notifierが `onStatusChanged()` を実装し登録制にする | ステップ2 |
| P3 | 承認判定の契約 `IApprovalRule` を新設する | 各Ruleが `canApprove()` を実装し差し替える | ステップ3 |

このクラス図が、P1〜P3を反映したシステム全体の設計結論です。課題IDは図の差分を追うために使い、以降はこの構造に必要なコードだけを示します。

#### 課題箇所のおさらい（フェーズ3の関連コード）

統合表で特定した箇所だけを振り返ります。P1は状態遷移の条件分岐、P2は通知先の直呼び、P3は金額・承認者の判定です。課題に関係しないコードは省略し、フェーズ3で明記した維持条件をそのまま引き継ぎます。
...
void WorkflowManager::process(const std::string& requestId,
    ...
    if (current == "作成中" && operation == "提出") {   // P1: 状態遷移
        cases.saveState(requestId, "審査待ち");
        notify(requestId);                              // P2: 通知の直呼び
    }
    if (!approvers.canApprove(approverId, amount))      // P3: 承認判定
...
#### 実装ステップ1（P1）：状態遷移の契約 `IWorkflowPhase` を定める
...
**P1との対応：** `IWorkflowPhase <|.. DraftPhase` の実装関係と `WorkflowManager o--> IWorkflowPhase` の委譲を実装しました（状態分離構造）。状態追加は新Phaseと遷移元だけで済みます。

#### 実装ステップ2（P2）：通知の契約 `INotificationListener` を定め、登録制にする
...
**P2との対応：** `INotificationListener <|.. EmailNotifier` の実装関係を実装しました（通知分離構造）。`ChatNotifier` も同じ形で、`WorkflowManager` は具体通知先を知りません。

#### 実装ステップ3（P3）：承認判定の契約 `IApprovalRule` を差し替え可能にする
...
**P3との対応：** `IApprovalRule <|.. ManagerApprovalRule` と `PendingPhase --> IApprovalRule` を実装しました（ルール差し替え構造）。ここで状態分離・通知分離・ルール差し替えの3構造が独立したまま接続されました。
...
```cpp
class WorkflowManager {
    IWorkflowPhase* phase;                         // P1: 現在状態
    std::vector<INotificationListener*> listeners; // P2: 登録リスナー
    WorkflowCaseRepository& cases;
public:
    ...
    void transitionTo(const std::string& s, const std::string& id) {
        ...
        for (auto* l : listeners) l->onStatusChanged(id, s);  // P2: 一律通知
    }
};
```
...
|---|---|---|
| 何を分離するか | P1を `IWorkflowPhase`、P2を `INotificationListener`、P3を `IApprovalRule` へ置く | 状態遷移・通知先・承認ルール |
...
|---|---|---|---|
| P1：状態処理 | 状態・遷移追加で公開入口と既存Phaseを変えない | `IWorkflowPhase` と各Phase | 新Phaseと遷移元へ変更が閉じた |
| P2：通知 | 通知追加で処理本体を変えない | `INotificationListener` の登録リスト | 新Listenerと登録へ変更が閉じた |
| P3：承認判定 | ルール変更で状態遷移を変えない | `IApprovalRule` の注入 | 新Ruleと注入へ変更が閉じた |
| P1〜P3を接続したシステム全体 | 公開入口と判定→状態保存→通知の順を維持する | `process()` が三契約を順に利用する | 三軸を独立させたまま一つの進行経路で動く |
...
|---|---|---|---|
| F1：新しい承認ルートの追加が継続的に発生する | IWorkflowPhase実装とResolverへルート差分を置き、WorkflowManagerの実行入口から分ける | 新Phase、WorkflowPhaseResolver、遷移元 | 通知・承認Ruleを守れる。将来ルートは完成コードへ追加しない |
| F2：通知先リストの拡張・変更が繰り返される | INotificationListener実装とTargetResolverへ通知先差分を置く | 新Listener、NotificationTargetResolver、TargetRepository | 状態遷移を守れる。将来通知先は完成コードへ追加しない |
| F3：通知の送信失敗を記録し再送する運用が求められる | DeliveryResultとNotificationDeliveryLogで受付結果を残すが、再送キューは別の運用責任になる | Listener、DeliveryLog、将来のキュー・再送入口 | 失敗記録までは局所化。キュー化・自動再送は完成コードへ追加しない |

F1〜F3を、State・Observer・Strategyを組み合わせる判断へ接続します。将来ルートや再送運用を実現済みとは扱いません。
...
graph LR
    T1["変更要求：状態・遷移の追加"]
        -->|新規追加| F1["PriorityPendingPhase<br>（IWorkflowPhase実装1クラス）"]
    T2["変更要求：新通知先追加"]
        -->|登録を追加| F2["新Listener + addListener<br>（INotificationListener実装）"]
    T3["変更要求：部署別承認上限"]
        -->|差し替え| F3["DeptApprovalRule<br>（IApprovalRule実装1クラス）"]
    T1 -. "影響なし" .-> A["WorkflowManager本体 ✅"]
    T2 -. "影響なし" .-> A
    T3 -. "影響なし" .-> A
```

フェーズ3の変更影響グラフと同じ要求・同じ粒度で比べると、P1の状態追加は `IWorkflowPhase` の実装1クラスへ、P2の通知先追加は `INotificationListener` の実装と登録へ、P3の承認上限変更は `IApprovalRule` の実装1クラスへ限定されました。`WorkflowManager` は3つの契約を別々に保持するため、どの軸の変更も他軸へ飛び火しません。

| 3-2で影響した場所 | 修正後 | 構造変更との対応 |
|---|---|---|
| `process()` の状態遷移分岐（P1） | **修正しない** | 遷移判断を状態分離構造へ移した |
| `process()` の通知直呼び（P2） | Listener追加と `addListener` 1行 | 通知を通知分離構造へ移した |
| `process()` の金額・承認者判定（P3） | Ruleを1クラス差し替える | 判定をルール差し替え構造へ移した |

### 7-4：変更シナリオ表

今回の確定要求R1〜R3について、フェーズ1の構造で必要だった修正と完成構造の結果を対比します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：通常申請を課長→部長の2段階、緊急申請を部長へ直接進む経路にする | `WorkflowManager`へ申請種別・役職・状態遷移の分岐を追加 | Phaseへ遷移判断を分け、通常は2回、緊急は部長の1回で完了し、不正役職では状態を変えないことを確認 |
| R2：部長承認後は決済部門へ、却下時は申請者へ通知する | 完了・却下分岐へ通知先取得と送信処理を追加 | 状態保存後にRepositoryから対象を取得し、決済部門または申請者だけへ通知した結果を返す |
| R3：承認上限を部署ごとに差し替える | `WorkflowManager`の金額判定へ部署分岐を追加 | 部署別`IApprovalRule`を注入し、同額でも部署設定に応じて判定結果が変わることを確認 |
