...
**RuleChecker クラス・PendingState クラス**
...
```cpp
class PendingState : public IWorkflowState {
public:
...
class EscalationEngine {
    ...
    // ← 抽象：外部から注入されたインターフェースのみ知っている
public:
    EscalationEngine(IApprovalRule* r, IWorkflowState* st)
        : rule(r), state(st) {}
    void escalateOverdue() {
...
classDiagram
    ...
    IWorkflowState --> IApprovalRule : 抽象×直接
```
...
class IStateFactory {
public:
    virtual IWorkflowState* create(string status) = 0;
};
...
class ApprovedState : public IWorkflowState {
public:
    void handle(WorkflowManager* wm) override {
        cout << "承認済み状態へ移行。" << endl;
    }
...
class RejectedState : public IWorkflowState {
public:
    void handle(WorkflowManager* wm) override {
        cout << "却下状態へ移行。" << endl;
    }
...
```cpp
class ConcreteStateFactory : public IStateFactory {
public:
    IWorkflowState* create(string status) override {
        if (status == "PENDING")   return new PendingState();
...
public:
    ...
    void process(string status) {
        IWorkflowState* state = stateFactory->create(status);
...
実はこの構造には名前があります。「状態ごとの振る舞いをオブジェクトとして切り出す」手法は **State パターン**、「判定ルールを外部から差し替え可能にする」手法は **Strategy パターン**、「状態変化を登録されたリスナーへ伝搬させる」手法は **Observer パターン** と呼ばれています。フェーズ6で案4として選んだ構造は、この3つのパターンを組み合わせた複合設計です。

### 7-1：解決後のコード（全体）

判定ルールを `IApprovalRule` として、状態遷移を `IWorkflowState` として、通知を `INotificationListener` として定義しました。 これらを `WorkflowManager` が保持・管理することで、複雑なワークフローを整理します。
...
class WorkflowManager {
    ...
    vector<INotificationListener*> listeners;
public:
    void setState(IWorkflowState* s) { state = s; }
...
// WorkflowManager → IWorkflowState → IApprovalRule という委譲チェーン
```

- `IWorkflowState*` と `vector<INotificationListener*>` はインターフェース型 → **「抽象」** の証拠
...
* **具体化された場所：** `IWorkflowState`, `IApprovalRule`, `INotificationListener`
...
classDiagram
    ...
    IWorkflowState ..> IApprovalRule : 判定委譲
...
**得られること1**（3つの変わる理由の識別）：フェーズ2の確定テーブルで、「承認状態の変化」・「関係者への通知」・「承認可否のルール判定」という3つの変化軸が、それぞれ独立した「変わる理由」として識別できることを確認しました。「このコードが変わるとき、それは誰の判断によるものか」という問いが、絡み合った責務を解きほぐす起点になります。

**得られること2**（密結合な接続点の特定）：フェーズ5で、`WorkflowManager` が状態遷移・通知・承認判定の3つを直接知っている状態を接続点として特定しました。状態が変われば通知も変わり、ルールも変わる——というように責務が連鎖して変わる構造の問題を、接続の形から読み取れるようになります。

**得られること3**（疎結合なワークフロー設計の説明）：フェーズ7の変更シナリオ表で、承認状態が変わるときは状態クラスの実装のみ、通知先が変わるときはリスナークラスの登録のみ、判定ルールが変わるときは判定ルールクラスの実装のみを変えればよい設計を確認しました。「3つの変化がそれぞれの場所に閉じている理由」を構造から説明できる状態が、設計議論を具体的にします。

**得られること4**（状態管理・通知・判定ルールが絡む現場への視点）：フェーズ7の最終コードで、`WorkflowManager` が純粋に「承認フローの進行」だけを知っている状態を確認しました。「状態管理」「通知」「判定ルール」という3つが絡み合う現場のコードに対して、どの変化軸を分離すれば変更の影響が局所化できるかを判断する視点が身についたと思います。

承認ワークフローシステムという题材を通じて、複数の構造が交差する複雑な設計を段階的に整理する思考を体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。
