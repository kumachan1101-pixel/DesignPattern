...
ただし正直に言うと、状態ごとの分岐は「現在状態を読み、次状態を保存し、通知する」という**同じ形ですでに並んでいます**。つまり共通シグネチャは分割前から見えており、ここでの関数化は発見ではなく名前付けにとどまります。共通形がこれだけ明確なら、この段階は飛ばして、直接ステップ4（状態インターフェースの抽出）へ進んでもかまいません。以下は「もし段階を踏むなら」という比較用の中間形です。
