...
* **得られること3：** 複数の構造を組み合わせることで、複数パターンが絡む処理を疎結合（変更の影響が特定のクラスだけに閉じる状態）に保ちつつ、新しい承認フローにも対応できる設計手法を説明できるようになる。
...
担当者が「承認ルールを一つ変えるだけで、ワークフロー全体のステータス管理への影響確認が必要になる」と感じる構造です。状態遷移のロジック、通知先の一覧、判定ルールが一つの大きなクラスで絡み合い、変更の入口を見つけにくくなっています。一見すると承認という一連の業務フローは安定していますが、内側では小さな修正が広い範囲へ波及しやすい状態です。
...
フェーズ3で見えた2つの痛みは、フェーズ4で3つの根本原因に分解されます。「状態管理とアクションの密結合」は根本原因①（承認ルートの変更）と根本原因②（通知先の変更）に対応し、「通知と判定の責務過多」は根本原因③（判定ルールの変更）に対応します。
...
`WorkflowManager`が、状態遷移、通知先、承認判定の詳細をすべて知っています。iPhoneに専用のLightningケーブルを直差ししている状態と同じで、新しい承認ルートが増えるたびに本体側を開いて専用の配線（`else if` 文）を直接追加する必要があるでしょう。
...
---
> **📌 課題（確定）**
> `WorkflowManager` から切り離す塊は3つある。1つ目は「どの状態でどの処理をするか」という状態遷移の知識で、これを `IWorkflowPhase` の実装クラスとして独立させること。2つ目は「誰に通知するか」という通知先の知識で、これを `INotificationListener` のリスナーとして外部から登録できる形に分離すること。3つ目は「承認可否をどう判定するか」という判定ロジックで、これを `IApprovalRule` として差し替え可能な部品として切り出すこと。この3つを分離して初めて、各変化軸への変更が互いに影響しない構造が実現できる。
---
...
### ステップ4：第3章で学んだState パターンを適用する ――状態遷移を分離する
...
### ステップ5：第7章で学んだObserver パターンを追加する ――通知を分離する
...
### ステップ6：第1章で学んだStrategy パターンを追加して完成する
...
フェーズ6のコードは構造の骨格を最小限で示したものです。フェーズ7では1-2bの状態遷移図（審査待ち/承認済み/却下/完了）の各状態に1クラスを対応させた完全実装に移行します。フェーズ6の `SubmittedPhase` は、この完全実装では `PendingPhase`（審査待ち）と `ApprovedPhase`（承認済み）に分割されます。申請の入口となる状態は、フェーズ7では新たに `DraftPhase`（作成中）として切り出されます。
...
承認ワークフローというドメインと State × Observer × Strategy の組み合わせの関係を一言で言うなら、状態・通知・判定という3つの変化軸が1クラスに同居する限り、修正のたびに3方向への影響確認が発生する、ということです。分離して初めて「1つの変更が1か所に収まる」という構造が実現しました。そしてこの章で最も重要なのは、3つのパターンを最初から選んで適用したのではなく、問題を分析した結果として自然に現れたという、その順序です。
