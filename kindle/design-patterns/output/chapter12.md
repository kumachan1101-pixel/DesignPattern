...
| メール・チャットを実際に送信する | `EmailNotifier` / `ChatNotifier` のスタブが `std::cout` で送信内容を表示する | 送信境界だけ扱う。SMTP、チャットAPI、認証、配信結果取得は論点外 |
| DB更新失敗、通知失敗、タイムアウト、リトライ、非同期ジョブ化 | 1-1ではエラー条件表で状態保存・通知の有無を示す。詳細コードは省略する | 本章の中心は状態・通知・判定ルールの責任分離なので、外部基盤の信頼性設計は扱わない |

つまり、`std::cout` は業務ロジックそのものではなく、外部通知境界の先にあるスタブ実装です。通知失敗や処理時間の問題は実システムでは重要ですが、この章では状態遷移・通知先データ・承認判定をどの責任へ分けるかに集中します。
...
|---|---|---|---|
| 入力 | 申請ID、申請内容、承認者ID、金額、操作 | 申請ID、申請内容、承認者ID、金額、操作、申請種別、追加通知先データ | 緊急ルートと通知先データが増える |
| 判定 | 保存済み状態で操作可能か、承認者の上限額内か | 緊急申請か、優先審査待ちで部長承認できるか、追加通知先があるか | 状態判定・承認判定・通知先取得が増える |
| 加工 | 申請IDから状態を取得し、状態を更新し、通知先へ送る | 緊急ルートで状態更新し、申請IDから追加通知先を取得して送る | 状態遷移と通知先データの両方が変わる |
| 出力 | 更新後状態、承認結果、通知結果 | 緊急ルート後の状態、追加通知、却下通知 | 状態結果と通知結果を分けて追う |
...
flowchart TD
    ...
    H --> I[メール送信スタブを呼ぶ<br>申請者・決済部門]
    H --> J[チャット通知スタブを呼ぶ<br>部長]
    G --> K([正常出力<br>更新後状態: 完了])
    I --> L([正常出力<br>通知結果: メール送信])
    J --> M([正常出力<br>通知結果: チャット通知])
...
| 金額が承認上限を超える | 承認者情報の上限額チェック | 承認不可 | 行わない |
| 現在状態で操作できない | 保存済み状態と操作の組み合わせ | 操作不可エラー | 行わない |
| 通知先データがない | `NotificationTargetRepository` に通知先がない | 状態更新のみ。通知結果は空 | 状態保存は行う。通知は行わない |
...
|---|---|---|
| 状態ごとの操作可否と遷移条件が `WorkflowManager` に集まっている | 状態ごとの振る舞いを、申請操作の公開形を変えずに切り離す | 状態処理を関数化した後、状態ごとの部品へ移す案を見る |
| 承認判定の条件が金額・申請種別・承認者で変わる | 承認ルールを状態遷移とは別に差し替えられるようにする | 承認判定を独立したアルゴリズムとして扱う案を見る |
| 通知や履歴は、承認状態や判定ルールとは別の理由で変わる | 申請処理の本体から副作用を切り離し、必要な時点で実行できる形にする | 通知・履歴を別部品として組み合わせるか判断する |
