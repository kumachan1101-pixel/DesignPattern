...
|---|---|---|---|
| 入力 | 申請、状態、承認操作 | 申請、状態、承認操作、緊急申請、追加通知先 | 緊急ルートと通知先が増える |
| 判定 | 現在状態で操作可能か、承認条件を満たすか | 緊急申請か、課長をスキップできるか、通知が必要か | 状態判定・承認判定・通知判定が増える |
| 加工 | 状態を更新し、必要な通知を送る | 緊急ルートで状態更新し、決済部門/申請者へ追加通知する | 状態遷移と通知が別軸で増える |
| 出力 | 更新後状態と通知 | 緊急ルート後の状態、追加通知、却下通知 | 状態結果と通知結果を分けて追う |
