...
ここからの検討は、次の2つのよくある考え方を軸に進めます。1つ目は、**共通部分に着目して共通化し、抽象化する**ことです。2つ目は、**変わる部分を分離して外から注入し、組み立て側は中身を意識せず決まった流れを回すだけにする**ことです。関数切り出し・別クラス化は1つ目の実行、状態・通知先・判定ルールの差し替えは「変わる部分を外から与え、本体は決まった流れを回すだけにする」という2つ目の実行にあたります。

この章には「状態遷移」「通知先」「判定ルール」という、変わる理由の違う3つの軸があります。それぞれについて、読者が最初に思いつきやすい「データの表で持つ」案から確かめます。まず状態遷移です。

**読者が思いつく案：状態遷移をテーブル（マップ）で持つ**
...
class WorkflowManager {
    ...
    map<pair<Phase, Action>, Phase> transitions = {
        {{Phase::Submitted, Action::Approve},  Phase::Approved},
        {{Phase::Approved,  Action::Approve},  Phase::Completed},
        {{Phase::Submitted, Action::Escalate}, Phase::Emergency},
    };
public:
    void handle(Action a) {
        auto it = transitions.find({phase, a});
        if (it == transitions.end()) { cout << "その操作はできません\n"; return; }
        phase = it->second;
        cout << "状態を更新しました\n";
    }
};
...
**この案の評価と残る課題：**
状態から次状態への遷移だけなら、この表で表せます。通知先も「状態→通知先リスト」の表、判定ルールも「部署→上限額」の表で持てそうです。**変わるのが結線や数値だけなら、これらの表で止める判断はあり得ます。** しかしフェーズ2で確認したのは、状態ごとに**実行する処理**が異なること（緊急ルートは部長へ直接通知するなど）、通知の**届け方**が増えること、判定の**決め方**自体が変わること（部署ごとの承認上限制度への移行）でした。処理・届け方・決め方は1つの値では表せず、表のマスの中身が `if` や関数に戻ります。さらにこの3つは変わる理由が独立しているため、1つの表や1つのクラスにまとめると、片方の変更でもう片方を読み解く必要が生じます。そこで以下では、状態ごとの振る舞い・通知・判定ルールを、それぞれ外から差し替えられる形へ分けていきます。
...
| 🔴 フェーズ6：対策検討 | 読者が思いつく状態遷移テーブル案もコードで試し、処理・届け方・決め方は値で表せず3軸が独立する残課題から、3構造統合（状態分離構造 + 通知分離構造 + ルール差し替え構造）まで進化させる決断を下した |
