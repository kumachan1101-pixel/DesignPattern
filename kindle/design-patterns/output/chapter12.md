...
実DBと通知APIは使いません。現状コードは状態保存が成功した後だけ `notify()` を呼びますが、通知失敗や再送はまだ扱いません。
...
4つ目の症状（通知失敗の染み出し）は、2つ目の症状と同じ根＝「通知の知識が状態遷移に混在している」から生じます。そのため独立した原因としては数えず、**通知の軸（後のP2）にまとめて扱います**。独立した根本原因は、次の3つです。
