...
詳細な仕様やコードへ入る前に、1-4（実装コード）の`main()`で提出、承認、承認上限超過を順に試す3ケースを確認します。
...
3回並べると、何が判断を分けているかが見えます。1回目と2回目は**現在の状態**（作成中／審査待ち）に応じた遷移と通知が出ています。3回目は**金額と承認者の上限**によるエラーだけが出て、状態遷移や通知の行は出ていません。実際に更新処理へ進まないことは、後続のコードで確認します。

この入力と出力から、(1)申請ID・操作・金額・承認者を受け取り、(2)承認可否と現在状態を検証してから状態更新と通知へ進み、(3)成功時は更新後状態と通知先、失敗時は上限超過の理由が表示される、という一連の動きが読み取れます。エラー時に状態を更新しない契約は、後続の動作例とコードで確認します。同じ入力を含む完全なコードと実行結果は1-4に掲載します。
...
これらは別々の業務上の理由で決まる知識です。現行コードがこの3つをどう扱っているかは、1-3（登場クラスとクラス構成図）の責任一覧と1-4のコードで確認します。
...
フェーズ1「現状把握」の対象は、状態遷移・通知・承認判定が `WorkflowManager` の `process()` に置かれている現行構造です。
...
承認者マスターは、1-1（このシステムの仕様）で定義した3名（APR001〜APR003）です。氏名・役職（`role`）・承認上限をそのまま保持します。`role` はコード上で `"manager"` / `"director"` / `"executive"`、代表の上限なしは番兵値 `99999999` として表します。
...
受入条件を状態遷移として整理します。「優先審査待ち」は今回の変更要求で追加する、緊急申請ルートの新しい到達状態です。フェーズ1の現行状態一覧との差分として追跡します。
...
2-3（関係者ヒアリング）ではこの三点を確認し、同じ処理内にあっても別々に変わる仕様かを確かめます。
...
2-4（ヒアリングで判明した将来リスク）のリスクIDを、承認ルートと通知で変えられるようにする部分と、申請処理の安定側へ分けます。「はい」は、フェーズ6で**ルートや通知運用が変わっても、申請の受付・状態保存・結果へ影響を広げない構造か**を判定するための印です。
...
したがって2-5（変わる見込みと今回維持する範囲を確定する）の出力は、「承認ルート・通知先・再送方針は変えられるようにし、申請操作・状態保存・結果契約は守る」という設計条件です。フェーズ3「問題特定」では変更ID1〜変更ID3だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
**変更前から抜き出す箇所：`WorkflowManager::process(const string&, const string&, int, const string&)`** ―― 3-1（変更を試みる）の連鎖（対策前・抜粋）
...
**課題ID1（状態軸）。5-3（課題IDと接続点を確定する）が挙げた候補は4つです。** 現在状態、操作、次状態、保存結果。1つずつ形を決めます。
...
5クラスを並べると、1-2（動作例テーブル）の状態遷移表がそのままクラスの一覧になります。
...
**この章の完成構造は一つに定まります。** 3つの軸のうち2つを同じクラスへまとめる形も置けますが、まとめた時点でその2つを別々に変えられなくなり、5-2（課題候補の重複と依存を整理する）で「3つとも独立変化軸」と判断したことに反します。責任配置が異なる完成構造が複数残ったわけではないので、当て馬を並べた比較は行いません。

完成クラス図は、構想を実装した結果としてフェーズ7「対策実施」だけに示します。ここでは、課題との対応と将来リスクへの備えを表で確定します。

#### 課題から採用構想までを照合する

ここまでの決定を、課題ID1→課題ID2→課題ID3の順に一望へまとめます。この表は設計課題だけを追います。変更要求の受入はフェーズ7の要求ID表、変更影響は7-4（変更シナリオ表）の変更ID表で別に確認します。
...
フェーズ2のリスクIDを採用構造へ再適用し、承認入口をどこまで守れ、再送運用に何が残るかを評価します。
...
フェーズ3で確認した変更ID1（緊急ルート追加）・変更ID2（承認後通知追加）・変更ID3（部署別承認上限）を、3-2（変更影響グラフ）と同じ粒度で再度適用します。
