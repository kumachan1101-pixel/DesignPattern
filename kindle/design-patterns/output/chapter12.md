...
フェーズ6の出発点は、フェーズ3で変更要求（緊急ルート・通知先・承認ルールの追加）を当てて痛んだ「変更途中コード」です。変更要求を容れる前の現状コードには戻しません。`WorkflowManager` に状態遷移・通知・承認判定が混じって増えたコードから、同じ形で扱える共通点（状態遷移の骨格と、通知先・承認ルールを差し替える契約）を抜き出し、変わる差分を接続点の外へ出す形へ整理していきます。読者が「痛み → 共通点の発見 → 抽象化」の順で追えるよう、最初の小さな案も、この変更途中コードを整理する形から始めます。
...
ここで注意したいのは、各分岐が「現在状態を読み、次状態を保存し、通知する」という**同じ形ですでに並んでいる**点です。共通シグネチャは分割前から見えているため、いったん独立した関数へ切り出して共通形を"発見"する段階（第0章の「関数へ切り出す」）は踏まず、最初から変化軸ごとに別のクラスへ分けるところから始めます。ステップ2以降は、直前ステップから状態遷移、通知先、承認判定の知識がどこへ移ったかを比べます。

### ステップ1：関心ごとに別クラスへ分離する

> **著者の思考プロセス**
> 「状態遷移も通知も承認判定も、ぜんぶ WorkflowManager 一つが抱えている。このままでは数千行の神クラスになってしまう。共通シグネチャはもう見えているのだから、途中の関数化は飛ばして、まず"種類の違う処理"をクラス単位で分けてみればどうだろうか？」

痛みコードでは判定・通知・状態遷移が `WorkflowManager` に同居しています。まず手を動かして、判定ルールを `RuleChecker` クラスに、通知処理を `NotificationService` クラスに切り出してみます。
...
### ステップ2：単一クラスアプローチの限界 ――3つの関心事が今もひとつのクラスに

ステップ1で判定と通知を別クラスへ切り出しても、気づいていただけたでしょうか。

どれだけ処理を別クラスに委ねても、`WorkflowManager` は依然として「どの状態でどの処理をするか」という状態遷移の知識を保持し続けています。新しい承認ルートが追加されるたびに、このクラスを開いて `if` 文を書き足す必要があります。
...
ステップ1で維持した緊急提出ルートと決済部門通知は、以降も削除しません。ステップ3以降の短いコードは、各変化軸をどこへ移すかを示す差分抜粋として読みます。

### ステップ3：第3章で学んだ 状態分離構造を適用する ―― 状態遷移を分離する

**ステップ2との差：** 単一クラスへ状態名の分岐が残る限界を受け、状態ごとの処理を `IWorkflowPhase` 実装へ移します。緊急提出ルートと決済部門通知は各 Phase の分岐として維持します。
...
### ステップ4：第7章で学んだ 通知分離構造を追加する ―― 通知を分離する

**ステップ3との差：** 状態処理の分離は保ち、Phase内に残っていた通知先の知識を通知先データと通知契約へ移します。
...
### ステップ5：第1章で学んだ ルール差し替え構造を追加して完成する

ステップ3で状態分離構造により状態遷移を分離し、ステップ4で通知分離構造によって通知先を切り離しました。しかし、まだ「判定ルール」の問題が残っています。この限界から、3つ目のパターンとしてルール差し替え構造を追加し、承認判定ロジックをインターフェースで切り出して外部から差し替え可能にします。

このステップは承認判定軸の差分です。ステップ4の `PendingPhase`、通知内容、緊急申請の通知先データ（決済部門を含む）、緊急提出に対応する組み立てはそのまま維持します。
...
それぞれのステップには一長一短があります。ステップ5の3構造統合は強力ですが、クラス数が激増し構造が複雑になる「初期投資コスト」もかかります。どこで止めるかは、**「今後の変更頻度（ビジネス要求）」**で決断します。
...
*   **ステップ1（具体クラス分離）で止めるケース：** 判定ルールや通知処理は切り出したいが、状態の種類は固定されている場合。
*   **ステップ3（状態分離構造のみ）で止めるケース：** 承認ルートの追加が頻繁だが、通知先と判定ルールはほとんど変わらない場合。
*   **ステップ4（状態分離構造 + 通知分離構造）で止めるケース：** 承認ルートと通知先は頻繁に変わるが、判定ルールは単純で固定の場合。
*   **ステップ5（状態分離構造 + 通知分離構造 + ルール差し替え構造の統合）まで進むケース：** 「新しい承認ルートの追加」「通知先の拡張」「部署ごとの承認上限の変更」という3つの独立した要件が、それぞれ頻繁に変化する場合。

**今回の決断：**
フェーズ2のヒアリングで「海外出張ルートや限定フローが今後も追加される（承認ルートの変化）」「関連部署への通知追加ニーズが継続的にある（通知の変化）」「来期から部署ごとの承認上限制度に変わる（判定ルールの変化）」と明言されています。3つの独立した変化軸が確認できたため、初期投資コストと将来の変更範囲を比較し、今回は**ステップ5（3構造の統合）まで進化させる**案を採用します。
...
ステップ5で決断した構造を、実行可能な完全なコードとして組み上げます。各役割ごとにコードを分けて確認します。
...
ステップ5で到達した3構造複合設計の実行時のオブジェクト間のやり取りを可視化します。`BatchApplication` が依存関係を注入し、`WorkflowManager` が具象クラスを知らずに抽象インターフェース経由で処理を委譲する流れが確認できます。
