...
フェーズ6で案4として選んだ構造は、「状態ごとの振る舞いをオブジェクトとして切り出す」手法（第3章で学んだStateパターン）、「判定ルールを外部から差し替え可能にする」手法（第1章で学んだStrategyパターン）、「状態変化を登録されたリスナーへ伝搬させる」手法（第7章で学んだObserverパターン）の3つの仕組みを組み合わせた複合設計です。
