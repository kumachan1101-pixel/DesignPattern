...
ここまでで、状態の種類、状態遷移、承認上限、通知先の意味を説明しました。最後に、代表ケースを使って入力がどの処理で使われ、どの出力に変わるかを整理します。この図は仕様の初出ではなく、上で説明した仕様を具体例で俯瞰するためのものです。
...
- 状態更新が成立した後に、保存済み通知先を読み出して送信する。承認不可や操作不可の場合は、状態保存も通知も行わない。
...
変更後の仕様を、1-1と同じように具体例で確認します。ここでは緊急申請の代表ケースとして、`REQ004` が優先審査待ちに保存されており、`APR002` が500,000円の申請を部長承認する流れを追います。1-1との差分は、状態に「優先審査待ち」が加わること、緊急申請では課長承認を経由しないこと、完了時に複数の通知先データを読み出すことです。
