...
* **得られること3：** 複数の構造を組み合わせることで、複数パターンが絡む処理の影響範囲を特定クラス内に局所化しやすくしつつ、新しいフローにも対応できる設計手法を説明できるようになる。
...
フェーズ1の仕様表を振り返ります。この承認ワークフローには「状態遷移」「通知」「承認ルール」という3つの側面があります。このうち変化が予想される仕様があります。

- **承認ルールの詳細**（金額の閾値、承認者の組み合わせなど）：組織の方針変更や規程の改定に合わせて変わることがあります
- **通知先の種類と条件**（誰にいつ通知するか）：承認プロセスの変化に合わせて、通知先が追加・変更されます

一方、「申請→承認→完了（または却下）」というワークフローの基本的な流れは、承認業務の本質として安定している部分です。

**仮説：承認ルールの内容と通知先の種類は、今後も変更が続く可能性がある。**
...
ヒアリングで判明した将来リスクが実際に変更として届いたとき、仕様がどう変わるかを具体的なテーブルで整理します。これは今後の設計判断の材料です。

| 変更軸 | 現在（今回追加後） | 将来（数ヶ月後） |
|---|---|---|
| 状態：審査待ち / 承認済み / 完了 / 却下 | 対応済み | 変更なし |
| 状態：優先審査待ち（緊急申請） | 今回追加 | 変更なし |
| 状態：海外出張ルート | —（なし） | 追加予定（特殊承認ルート） |
| 通知：申請者 / 管理者 / 部長 / 決済部門 | 今回追加 | 変更なし |
| 通知：関連部署への追加通知 | —（なし） | 追加予定（関連部署対応） |
| 判定：金額閾値（10万円固定） | 対応済み | 来期変更予定 |
| 判定：部署ごとの承認上限制度 | —（なし） | 変更予定（来期制度改定） |
...
さらに、ヒアリングで予告された「海外出張ルート」や「部署ごとの承認上限制度」の追加も視野に入れると、`process()` に海外出張専用の `else if` 分岐がまた1つ増え、部署別の閾値チェックも同じメソッドに積み重なることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、変化軸が増えるたびに同じクラスの条件分岐がさらに積み重なる構造は変わりません。
...
### ステップ4：第3章で学んだ State パターンを適用する ―― 状態遷移を分離する
...
### ステップ5：第7章で学んだ Observer パターンを追加する ―― 通知を分離する

次に「通知の混在」という問題を解決します。通知先をリストとして管理し、`WorkflowManager` が状態変化を登録済みのリスナー全員に伝搬させる形にします。

> [!INFO] コラム: 状態クラスに「誰に通知するか」を教えない理由
> もし状態クラスの中で「課長に通知する」と直接書いてしまうと、後から「関連部署にも通知したい」という要望が出たときに、状態遷移のロジックまで修正しなければならなくなります。Observerパターンを使って「状態が変わったよ！」と叫ぶだけの仕組みにすることで、状態ロジックと通知ルールを完全に切り離すことができます。
...
### ステップ6：第1章で学んだ Strategy パターンを追加して完成する
...
**今回の決断：**
フェーズ2のヒアリングで「海外出張ルートや限定フローが今後も追加される（承認ルートの変化）」「関連部署への通知追加ニーズが継続的にある（通知の変化）」「来期から部署ごとの承認上限制度に変わる（判定ルールの変化）」と明言されています。3つの独立した変化軸が確認できたため、初期投資コストと将来の変更範囲を比較し、今回は**ステップ6（3パターンの統合）まで進化させる**案を採用します。
...
| 🟣 フェーズ2：仮説立案 | フェーズ1の仕様表から「承認ルールと通知先は今後も変更が続く」という仮説を立て、ヒアリングで確認した。今回の確定変更と将来リスクを分けて整理した |
...
| **得られること** | **この章のどこで示したか** |
| --- | --- |
| 1. 変動箇所の識別 | フェーズ2の仮説立案とヒアリングで、承認ルールと通知先を変動する仕様として識別した |
