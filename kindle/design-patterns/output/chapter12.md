...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 申請を提出・承認して状態を進める | 要求ID1、要求ID2 |
| 機能ID2 | 金額上限と承認者情報で承認可否を判定する | 要求ID3 |
| 機能ID3 | 申請状態と通知先を保存・取得する | 要求ID4 |
| 機能ID4 | 未登録申請と許可されない操作を拒否する | 要求ID5 |
...
|---|---|---|---|
| 機能ID1 | 変更<br/>根拠: 変更ID1 | 通常・緊急経路で申請を提出・承認して状態を進める | 要求ID1、要求ID2 |
| 機能ID2 | 変更<br/>根拠: 変更ID3 | 部署・役職ごとの上限で承認可否を判定する | 要求ID3 |
| 機能ID3 | 継続<br/>根拠: — | 申請状態と通知先を既存形式で保存・取得する | 要求ID4 |
| 機能ID4 | 継続<br/>根拠: — | 未登録申請と許可されない操作を拒否する | 要求ID5 |
| 機能ID5 | 追加<br/>根拠: 変更ID2 | 保存後状態に応じて決済部門または申請者へ通知する | 要求ID6 |
...
課題ID1〜課題ID3を、次の三つの観点で一つの完成構造へ変換します。
...
|---|---|---|---|
| 機能ID1 | 変更 | 要求ID1、要求ID2 | 現行提出を回帰し、通常2回・緊急1回の承認を受入確認<br/>**判定:** 合格 |
| 機能ID2 | 変更 | 要求ID3 | 部署・役職ごとの上限判定を確認<br/>**判定:** 合格 |
| 機能ID3 | 継続 | 要求ID4 | 状態・通知先の保存表現不変を回帰確認<br/>**判定:** 合格 |
| 機能ID4 | 継続 | 要求ID5 | エラー時の状態・通知件数不変を回帰確認<br/>**判定:** 合格 |
| 機能ID5 | 追加 | 要求ID6 | 保存後の決済部門・申請者通知を受入確認<br/>**判定:** 合格 |
