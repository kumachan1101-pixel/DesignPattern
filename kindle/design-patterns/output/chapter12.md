...
現在の構成では、状態遷移のロジック・関係者への通知・承認ルールの判定がすべて `WorkflowManager` クラスに集約されており、このクラスがワークフロー全体を担っています。
...
このシステムでは承認操作が非同期・低頻度であるため、間接層の導入によるパフォーマンスコストは問題になりません。唯一注意すべき点は、複数の承認者が同一申請を同時操作するケースです。**状態遷移の競合を防ぐ排他制御**の仕組みは、設計段階で検討が必要な制約となります。この点は案3・案4の選定に影響するため、各案のトレードオフで触れます。
...
* ※ 同時操作の排他制御：承認フロー制御が `WorkflowOrchestrator` に集約されているため、楽観的ロックによる競合検出をここ一箇所に実装できる。
...
* ※ 同時操作の排他制御：インターフェース群が分離されているため、状態遷移の競合検出（楽観的ロック）を `WorkflowOrchestrator` に集中実装できる。呼び出し元（`WorkflowManager`）はこれを意識する必要がない。
