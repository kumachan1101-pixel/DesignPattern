...
状態遷移、通知先、承認額にはそれぞれ異なる業務上の決定理由があります。しかし現行システムでは、その3つを1つの管理処理でまとめて扱っています。

**現在の申請状態一覧**

現行コードが保存する状態は、次の3つだけです。まず、この時点で実装されている範囲と、後の変更要求で追加する状態を混ぜずに確認します。
...
これらは別々の業務知識ですが、現行実装では1つの管理処理に集まっています。後のフェーズでは、どの知識がどの理由で変わり、同じコードへ影響しているかを追います。
...
この現状コードには、変更後に登場する `IWorkflowPhase`、`IApprovalRule`、`INotificationListener`、`DeliveryResult` はまだ存在しません。フェーズ1では変更後の設計要素を先取りせず、状態遷移・通知・承認判定が `WorkflowManager` に集まっている事実だけを確認します。
...
**着目する共通点：** 現状コードはすでに共通点を持っています。状態遷移・通知・承認判定はどれも同じ `process()` に載り、状態は `WorkflowCaseRepository` に、承認判定は `canApprove` にそろえて扱っています（流れとデータの共通点）。現場では、状態や判定の持ち方がばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、公開入口や保存の形を変えず、変わる「状態遷移（P1）・通知（P2）・承認判定（P3）」の3軸だけを別々の契約へ外に出せば済みます。フェーズ6では、この共通点と3つの課題を組み合わせて、一つの完成システムを設計します。
