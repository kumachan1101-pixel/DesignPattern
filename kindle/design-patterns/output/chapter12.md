...
```mermaid
flowchart LR
    U[利用者<br>提出・承認・却下を行う] --> A[承認ワークフローシステム<br>申請IDと操作を受け取る]
    A --> B[(申請状態データ<br>申請IDごとの現在状態)]
    A --> C[(承認者マスター<br>承認者ID・役職・上限額)]
    A --> D[(通知先データ<br>申請IDごとの通知先)]
    B --> A
    C --> A
    D --> A
    A --> E[状態更新<br>次状態を保存]
...
flowchart TD
    ...
    C --> D[承認上限と現在状態で<br>操作可否を確認]
    B --> D
    D --> F[次状態を決定<br>作成中 + 通常申請<br>→ 審査待ち]
...
flowchart TD
    ...
    C --> D[承認上限と現在状態で<br>操作可否を確認]
    B --> D
    D --> F[次状態を決定<br>優先審査待ち + 部長承認<br>→ 完了]
