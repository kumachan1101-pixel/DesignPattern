...
- **契約と具体による分離**：進行処理に残す流れと、状態・通知・判定の差を持つ契約・具体をセットで決める。
- **生成・注入・実行による組み立て**：実体を誰が生成・所有し、進行処理へ契約として渡してどう実行するかを一本で決める。
...
**ここから決めること：** 状態ごとの判断、通知配送、承認条件を三つの接続点へどう分け、どこへ配置し、判定から状態保存・通知までの進行経路へどう接続するかを決めます。

**3つの課題を、別々に解いて後で合流させる形にはしません。** 分離では軸ごとに契約と具体を並べ、組み立てでは3軸の実体を生成・注入・実行の一つの経路へ集めます。課題が増えても判断はこの二つで、増えるのは各判断の中で比較する軸だけです。
...
**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、状態ごとの遷移・通知配送・承認判定を、別々の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
**穴が2つ空きました。** `phase`（現在状態の実体をどこから得るか）と `notifyAll()`（誰に配るか）です。**どちらも具体の名前をここに書くわけにいきません。** 書けば分けた意味が消えます。**それを決めるのが生成の検討です。**
...
**ここが終わった時点で、実体はすべて存在しています。** 6つの状態、3つの規則、2つの通知先、引く役、それらを持つ組み立て役。**まだ起きていないのは1つだけです。骨格の契約と書かれた場所へ、具体が入ることです。次に、注入として確認します。**
...
**守る範囲との照合：** **公開入口**を保っていることを、いま確認しました。引数の形は変わりましたが、「申請の進行を1回の呼び出しで進める」という形は同じです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**

**課題が3つになっても、設計判断は二つのままでした。** 分離では3軸ぶんの契約と具体を並べ、組み立てでは3軸を一つの実行経路へ集めています。**課題の数によらず同じ見方で全体を追える**ことが、この最終章で確かめられます。
...
| 生成・注入・実行による組み立て | 進行処理が全分岐と台帳を内包 → 組み立て役が状態・規則・通知先を所有 | 状態IDで引き、規則を状態へ渡し、通知先を登録して公開操作から実行する | 保存後通知の順と三軸の接点を一経路で追える |

公開入口は操作と承認要求を受け取り、保存状態から具体を引きます。状態、判定規則、通知先は同じ実行経路でつながります。

#### 代表ケースの実行接続

生成・登録した状態、判定規則、通知先が実際に使われるまでを確認します。審査待ちの申請を課長が承認し、状態保存後に通知されるまでを一本で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成 | `BatchApplication`（メンバー） | 状態6つ・規則3つ・通知2つを所有し、引く役と通知先データへ登録する | 注入通知・判定へ |
| 2. 注入通知・判定 | `WorkflowManager::addListener()`／`PendingPhase` のコンストラクタ | 通知先を登録し、状態を作るときに規則を渡す（どちらも組み立て時に1回） | 実行開始へ |
| 3. 実行開始 | `BatchApplication` | `wm.process(WorkflowEvent::Approve, {50000, "課長"});` | `WorkflowManager::process()` |
| 4. 注入状態 | `WorkflowPhaseResolver::resolve(const string&)` | 保存された状態ID「審査待ち」から `PendingPhase` を引く | 骨格の `IWorkflowPhase*` へ入る |
...
**注入が2か所に分かれています。** 2は組み立てのときに1回（通知先と規則）、4は操作のたび（状態）です。**同じ章の中に、2つの入り方が並んでいます。**
