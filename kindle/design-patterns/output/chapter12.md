...
状態遷移・通知・承認判定は独立して変わるためP1、P2、P3の連番にします。フェーズ6では、この3枝の理想グラフからカードと候補を導きます。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた3つの課題——**状態遷移を切り離すこと／通知先を切り離すこと／承認判定ルールを差し替えられるようにすること**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを分解して探します。第二部の集大成として、まず「いくつの独立した変化軸があるか」を数えます。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる

フェーズ4では、状態遷移、通知運用、承認判定が WorkflowManager へ集まり、別々の要求が同じ場所へ波及していました。理想グラフをP1〜P3へ分けます。
...
|---|---|---|---|
| P1 | 状態追加の影響をWorkflowManagerの全組み合わせ分岐からPhase実装へ縮める | process()の状態選択を切る。操作→次状態、保存、監査ログを守る | 新状態・遷移追加でWorkflowManagerの分岐を増やさない |
| P2 | 通知追加の影響を状態保存処理からListener登録へ縮める | 保存後の直接通知を切る。保存成功後だけ通知し、1件失敗で他を止めない | 通知先追加を登録だけで行う |
| P3 | 承認条件変更の影響から状態・通知を外し、ApprovalRuleへ閉じる | 固定DB判定を切る。不許可なら状態不変、理由をログへ残す | 部署別上限変更で状態・通知を変更しない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 状態ごとの次状態判断をクラスへ移す | 状態分岐を分離できる | 現在Phaseへ操作を一律委譲する |
| P2 | 通知先を同じonChanged契約へそろえる | 直接通知の分岐が減る | 登録リストと部分失敗記録へ接続する |
| P3 | 承認判定を関数へ出す | 状態処理と判定条件を分けて読める | IApprovalRule契約へ上げ、組み立てから注入する |

ここからP1〜P3に該当する状態分岐・通知・承認判定だけをおさらいします。
