...
→ **採用：手段B**（`createProcessor` メソッドによる生成委譲）。決済手段の追加が `createProcessor` 内の1行追加だけで完結し、`processPayment` の本体を変えずに済む。設定ファイルほどの複雑さは不要で、このシステムの変更頻度に対して十分な柔軟性を持つ。
...
→ **採用：手段A**（抽象生成インターフェースによる生成の委譲）。`PaymentProvider` インターフェースを介することで、テスト時にモック実装を差し込めるようになり、決済プロバイダー障害時のフォールバック実装（フェーズ5で言及）も Provider を差し替えるだけで実現できる。
