...
```text
現在：processPayment（利用フロー）が
      決済結果を使う処理 ＋ どの方式を作るかの生成判断 ＋ 手段固有の入力検証・同期非同期 を持つ
                      ↓
変更後：生成判断を1か所の生成メソッドへ寄せ、利用フローは共通契約だけを呼ぶ
   具体方式・固有入力検証・同期非同期・エラー対処 → 各Processorを共通契約の実装にし生成メソッドで作る（生成分離＝Factory Method）→ 課題ID1
守る範囲：顧客・注文の照合、結果保存、共通の「依頼→結果」フロー
```

まだクラスの中身は見ません。この段階でつかんでほしいのは「どの方式を作るかの生成判断を一つの生成メソッドへ寄せ、利用フローは共通契約だけを呼ぶ」という筋だけです（この章の接続課題は一つ＝課題ID1で、生成判断と利用フローの境界です）。「どのクラスが生成し、どの契約で実行するか」という具体の結論は、この後の課題ID1で決めていきます。決めた結論をまとめて振り返る表は、フェーズ6の末尾（6-4 設計トレース）に置きます。ここでは先に結論表を出しません。
