...
| 役割 | 担当者 | 管轄する知識 |
|---|---|---|
| 決済基盤開発チーム | 自チーム | 決済フロー・共通インターフェース保守 |
| 事業側（ビジネス担当） | 商品・事業企画部門 | どの決済手段を追加・廃止するかの判断 |
| 各決済手段の開発チーム | 担当開発チーム | 各決済プロセッサーの実装 |
...
仕様と動作例が確認できたところで、クラス間の関係を図で整理します。
...
フェーズ1の仕様表を振り返ります。現在のシステムには「クレジットカード決済」「コンビニ決済」の2種類の決済方法があります。このうち変化が予想される仕様があります。

- **利用可能な決済手段の種類**（クレジットカード・コンビニ・QRコードなど）：ビジネス要件やユーザーニーズに応じて新しい決済方法が追加されることがあります。今回のPayPay追加要求もその一例です
- **各決済手段の生成条件**（どの条件でどの決済クラスを使うか）：新しい決済手段が追加されるたびに変わります

一方、「決済手段を選び、金額を渡して実行する」という決済処理フロー自体は、決済サービスの基本として安定している部分です。

**仮説：利用可能な決済手段の種類と生成条件は、今後も追加・変更が続く可能性がある。**
...
ヒアリングで「銀行系決済・後払いサービス」の追加が予告されました。この変更が来たとき、仕様がどう変わるかを整理しておきます。

| 決済手段 | 現在 | 将来（数ヶ月後） |
|---|---|---|
| CreditCardProcessor（クレジット） | 対応済み | 変更なし |
| ConvenienceStoreProcessor（コンビニ） | 対応済み | 変更なし |
| PayPayProcessor（PayPay） | 今回追加 | 変更なし |
| 銀行系決済 | —（なし） | 追加予定 |
| 後払いサービス | —（なし） | 追加予定 |

銀行系・後払いが追加されると `processPayment()` の条件分岐がさらに増え、各プロセッサークラスも追加が必要になります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された「銀行系決済・後払い」も追加されると、`processPayment()` の `else if` がさらに増え、新クラスも増えることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、決済手段が増えるたびに分岐と生成が連動して膨れる構造は変わりません。
...
分離の境界は決まりました。次は「決済手段ごとのクラスをまとめるか、個別に分けるか」という粒度の問題です。

ヒアリングで把握した決済手段と、変わる理由を確認します。

| 決済手段 | 変わる理由 |
|---|---|
| CreditCardProcessor | クレジットカード会社のAPI仕様（決済担当チーム） |
| ConvenienceStoreProcessor | コンビニ各社のAPI仕様（同じ決済担当チーム） |
| PayPayProcessor | QRコード決済のAPI仕様（同じ決済担当チーム） |
| 銀行系決済（将来） | 銀行APIの仕様（同じ決済担当チーム） |
| 後払いサービス（将来） | 後払い事業者のAPI仕様（同じ決済担当チーム） |

すべて同じ担当チームが管轄していますが、変わり方は独立しています。PayPay の API 仕様が変わっても、クレジットカードの処理には影響しません。コンビニ決済の変更が後払いサービスに波及することもありません。各決済手段は**独立して変わる**ため、決済手段ごとに別クラスへ分離します。どの決済手段も「金額（int）を受け取って決済を実行する」という同じ操作を持ちます。

→ **決済手段ごとに別クラスへ分離し、同じ `IPaymentProcessor` インターフェースで束ねる**
...
> 「生成するだけの別メソッド（関数）を作れば十分では？」と思うかもしれません。しかし、それだけでは「そのメソッドを持つクラス」が全ての具体クラスを知っている状態は変わりません。この生成の抽象化パターンの核心は、「具体的な生成処理をサブクラスに委ねる」ことです。これにより、呼び出し側のコードを一切変えずに、テスト用のモックに差し替えたり、環境ごとに生成するクラスを切り替えたりすることが可能になります。
...
| 🟣 フェーズ2：仮説立案 | 仕様表から「決済手段の種類と生成条件は今後も追加・変更が続く」という仮説を立て、ヒアリングで裏付けた。今回の確定変更と将来リスクを分けて整理した |
...
| **得られること** | **この章のどこで示したか** |
|---|---|
| 1. 変動箇所の識別 | フェーズ2の仮説立案とヒアリングで、決済手段の種類と生成条件を変動要因として特定した |
...
7つのフェーズを通じて、読者は決済処理クラスが具体クラスを知りすぎているという観察から始まり、「誰の判断で決済手段が決まるか」の分析を経て、生成責任を分離するという判断へと進みました。フェーズ2のヒアリングで「決済手段は今後も追加される」と確認した時点で生成の変動軸が確定し、フェーズ4で利用処理が生成知識を持っていることを接続点として特定した時点で、生成と利用を切り分ける方向性が定まりました。`processPayment` の中からif文が消えた瞬間が、この章の到達点です。

あなたのコードの中にも、処理ロジックの中で具体クラスを生成し、そのクラス名や生成条件を呼び出し元が知っている箇所があるはずです。「この生成ロジックは誰の判断で変わるか」を問うことが、Factory Methodを使う理由を見つける入口になります。
