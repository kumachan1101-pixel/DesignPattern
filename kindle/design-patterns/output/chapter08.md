...
| 仕様項目 | この章で扱う値 | 説明 |
|---|---|---|
| 決済種別 | クレジットカード・コンビニなど | どの決済処理を使うかを決める |
| 金額 | 決済対象金額 | 選ばれた決済処理へ渡す |
| 決済設定 | 有効/無効、手数料など | 決済を受け付けてよいかを確認する |
| 出力 | 決済成功またはエラー | 対応する決済処理の結果を返す |

この時点では、生成処理をどこへ移すかは判断しません。まず、決済種別からどの処理へ進むかを確認します。
