...
flowchart LR
    I["入力<br/>決済種別、金額"]
    P["加工<br/>決済種別に対応する処理を選ぶ、決済を実行する"]
    O["出力<br/>決済成功メッセージ、未対応決済のエラー"]
...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 決済種別、金額 | `"credit"`、`"cvs"` などの種別文字列と金額 | 動作例テーブル、`main()`、変更要求 |
| 決済処理の選択、決済実行 | 生成する決済プロセッサーの選択と実行 | 現状コードの観察、決済手段追加時の変更シミュレーション |
| 成功メッセージ、未対応エラー | 実行結果の決済結果表示 | 動作例テーブル、実行結果、変更シナリオ表 |
...
決済システムでは、「どの決済手段を採用するか」という判断と、「その決済手段をどう実装するか」という判断は、属する業務機能が異なることが一般的です。前者は事業上の契約や戦略が絡む業務機能の判断であり、後者は技術的な詳細に属する設計判断です。この分離が設計に直接影響するため、変化の軸を整理しておきます。
...
`PaymentApplication.processPayment()` が現在抱えている知識と、それぞれが属する業務機能を確認します。

| 知識（コードが直接持っているもの） | 業務機能 | 適切か |
|---|---|---|
| 決済処理の実行フロー全体 | 処理の骨格（開発設計判断） | ✅ |
| クレジットカード決済クラスの生成条件と具体型名 | 決済手段・サービス管理 | ❌ 混在 |
| コンビニ決済クラスの生成条件と具体型名 | 決済手段・サービス管理 | ❌ 混在 |
...
> 「生成するだけの別メソッド（関数）を作れば十分では？」と思うかもしれません。しかし、それだけでは「そのメソッドを持つクラス」が全ての具体クラスを知っている状態は変わりません。この生成の抽象化パターンの核心は、「具体的な生成処理をサブクラスに委ねる」ことです。これにより、呼び出し側の変更を抑えながら、テスト用のモックに差し替えたり、環境ごとに生成するクラスを切り替えたりしやすくなります。
...
`ProcessorRegistry` はデータ層として機能します。決済手段の有効・無効や手数料率の変更では、このクラスが主な修正先になります。Factory Methodの生成ロジックとは、変更理由を分けて考えられます。
