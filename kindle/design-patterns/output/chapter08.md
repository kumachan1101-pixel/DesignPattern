...
|---|---|---|
| P1 | 決済手段追加で振り分け、具体Processor生成、手段固有検証、Worker・Webhookの確認へ波及する | `PaymentRequest`→`PaymentResult`、外部Clientとログの境界、非同期処理の入口 |

P1は「決済手段追加の波及」です。フェーズ6では、新しいProcessorと生成登録だけへ変更を閉じる理想を描きます。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた課題——**注文処理の呼び出し元から、決済手段の具体クラス名と生成条件を切り離し、手段固有の知識をProcessorへ閉じる接続点を作る**（`PaymentRequest` → `PaymentResult` の契約は守る）——を受けて始めます。まずフェーズ4で記録した現在の痛みを図の起点に置き、P1でどのコード境界を微修正し、同じ要求を当て直すと影響がどこまで縮むかを一本の経路で描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じP1の行へ落とします。その後、P1に紐づくフェーズ3の関連コードだけをおさらいし、候補を一段ずつ適用します。各段階で「何を変えたか」「どの影響が減ったか」「何が残るか」をコードで回収し、統合後の全体コードはフェーズ7で示します。
#### 痛みの変更影響から、構造をどう変えるか
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: 生成と実行を関数分割"] --> S2["P1-変更2: Processor契約と生成登録へ上げる"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["決済手段を追加"] --> I1["P1: Processor実装と生成登録だけを追加"]
        I1 --> I2["追加手段の要求・結果だけをテスト"]
        IS["変更しない: 共通利用フロー・Worker・Webhook"]
    end
    C1 -. "P1でtype分岐と生成を切る" .-> S1
    S2 -. "決済追加を局所化" .-> I1
```
...
|---|---|---|---|
| P1 | `payByCard()`、Factory、`IPaymentProcessor::pay()`、`PaymentApplication` | **微修正：** 生成/実行を分割→製品契約→生成判断を生成メソッドへ移す<br>**結果：** 共通フロー・Worker・Webhookから固有知識が消えた | **守った契約：** Request→Result、API境界、所有権<br>**判定：** 単一Factoryでは未達、製品契約と生成分離で達成 |
