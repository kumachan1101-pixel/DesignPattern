...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。利用者が注文ORD-1001（1000円）をクレジットカードで決済すると、内部は手段別の入力検証→カード会社APIへ認証→結果を保存の順に進みます（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
この出力から、①注文・金額・支払手段・手段別の入力を渡し、②手段に応じた検証→外部API→結果保存の順に進み、③決済結果が表示される、という一連の動きが読み取れます。これを押さえてから、以降の要求・仕様・コードを読み進めます。
...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | クレジットカードで決済する | 要求ID1、要求ID5 |
| 機能ID2 | 銀行振込を受け付ける | 要求ID2、要求ID5 |
| 機能ID3 | コンビニ払いを受け付ける | 要求ID3、要求ID5、要求ID6 |
| 機能ID4 | 注文ごとの決済結果を保存・更新する | 要求ID4、要求ID6 |
...
|---|---|---|---|
| 機能ID1 | 継続<br/>根拠: — | クレジットカードで決済する | 要求ID1、要求ID5 |
| 機能ID2 | 継続<br/>根拠: — | 銀行振込を受け付ける | 要求ID2、要求ID5 |
| 機能ID3 | 継続<br/>根拠: — | コンビニ払いを受け付け、完了確認する | 要求ID3、要求ID5、要求ID6 |
| 機能ID4 | 変更<br/>根拠: 変更ID2 | 注文ごとの同期・非同期決済結果を保存・更新する | 要求ID4、要求ID6 |
| 機能ID5 | 追加<br/>根拠: 変更ID1、変更ID2 | PayPay決済を受け付け、保留IDで完了確認する | 要求ID5、要求ID6、要求ID7 |
...
変更影響グラフでは、変更ID1・変更ID2の両方から`PaymentApplication`へ矢印が向き、入力構造体、API境界、完了確認まで修正が広がっています。
...
|---|---|---|---|
| 決済利用処理が方式名ごとの具体Processor生成と固有手順を分岐する | PayPay追加で生成分岐・入力検証・完了確認まで同時修正する | 決済方式ごとの生成と処理を利用フローから分離する | 利用フローと方式固有手順は別の理由で変わる |
...
|---|---|---|---|
| リスクID1：決済手段の種類がさらに増加する | IPaymentProcessor実装を生成・登録し、利用フローはpay()だけを呼ぶ | 新Processor、生成関数、ProcessorRegistry | 利用側を守れる。将来手段は完成コードへ追加しない |
| リスクID2：手段固有の入力データと検証ロジック | 手段固有データはPaymentRequest内の専用構造、検証は対象Processorへ置く | 入力構造、PaymentRequest、対象Processor | 変更先は限定できるが共通要求型も増える。未確定入力は完成コードへ追加しない |
| リスクID3：同期/非同期の処理モードの増加 | PaymentResultで成功・失敗・保留を共通化し、具体手順をProcessorへ閉じる | 対象Processor、必要な状態境界 | 共通利用フローを守れる。将来モードは完成コードへ追加しない |
| リスクID4：完了確認の手順の増加 | 保留IDを共通結果で返し、完了確認をPaymentStatusClient境界へ分ける | PaymentStatusClient、対象Processorまたは完了確認窓口 | 一部局所化。未確定の確認手順は完成コードへ追加しない |
...
|---|---|---|---|
| 機能ID1 | 継続 | 要求ID1、要求ID5 | カード入力検証・認証・売上確定を回帰確認<br/>**判定:** 合格 |
| 機能ID2 | 継続 | 要求ID2、要求ID5 | 振込固有入力検証と振込先発行を回帰確認<br/>**判定:** 合格 |
| 機能ID3 | 継続 | 要求ID3、要求ID5、要求ID6 | コンビニ受付と完了確認を回帰確認<br/>**判定:** 合格 |
| 機能ID4 | 変更 | 要求ID4、要求ID6 | 注文ID別の同期・非同期結果保存と状態更新を確認<br/>**判定:** 合格 |
| 機能ID5 | 追加 | 要求ID5、要求ID6、要求ID7 | PayPay固有入力、セッション作成、保留IDの完了確認を受入確認<br/>**判定:** 合格 |
...
| 🔴 フェーズ6 | 課題ID1を満たす最終構造は生成分離構造に一意に定まると確認し、採用クラス図を3ステップでコードへ反映した |
