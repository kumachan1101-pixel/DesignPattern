...
**得られること3**（変更局所化の説明）：フェーズ7で、`IProcessorFactory` インターフェースを介してプロセッサーを受け取る構造に変えたことで、新しい決済手段の追加が新規クラスの作成だけで完結する設計を確認しました。「生成の責任を分離する」というひと手間が、どれほどの変更耐性を生むかを体感できたと思います。
...
決済プロセッサー管理という题材を通じて、「生成の責任をどこに置くか」という設計上の判断を体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。