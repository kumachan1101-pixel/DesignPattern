...
まずは、決済処理システムが何を入力として受け取り、どの処理で加工し、何を出力するのかを整理します。ここでは設計の良し悪しを判断せず、現状を事実としてそろえます。
...
決済システムでは、「どの決済手段を採用するか」という判断と、「その決済手段をどう実装するか」という判断は、属する業務機能が異なることがあります。ここではまだ変わりやすさを判断せず、後のフェーズで確認するために、どの業務機能がどの仕様を決めているかだけを整理します。
...
フェーズ2では、まだ「責務が混在している」とは判断しません。ここで行うのは、フェーズ1で見た仕様のうち、どの決済手段・生成条件・実行手順が変わりそうかを見当づけることです。
...
| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| メソッド切り出し | 手段ごとの生成と実行 | 生成と実行が名前で読める | 生成の知識は同じクラスに残る |
| 生成マップへの登録 | 種別→生成関数の対応 | 手段追加が登録1行で済む | 生成方針そのものを丸ごと差し替えにくい |
| 固定ファクトリへ分離 | 生成の知識全体 | 本体が具体クラス名を知らなくなる | ファクトリ自体が固定で差し替えられない |
| 生成の抽象化・委譲 | 生成方針そのもの | 生成する側を注入で差し替えられる | クラス階層と組み立ての初期コスト |

このうち「生成マップへの登録」は、種別と生成関数（ラムダ）の対応表を持ち、手段の追加は表への登録1行で済ませる、という最初に思いつきやすい案です。この案は比較表の中だけで済ませず、ステップ2の別案として実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。

なお、メソッド切り出し・固定ファクトリは1つ目の「共通化・抽象化」の実行、生成マップへの登録・生成の抽象化は「生成の判断を外から与え、本体は受け取ったプロセッサーを実行するだけにする」という2つ目の実行にあたります。
...
public:
    void registerType(const string& type,
                      function<IPaymentProcessor*()> creator) {
        creators[type] = creator; // 追加は登録1行で済む
    }
    IPaymentProcessor* create(const string& type) {
...
**この案の評価：**
PayPayの追加は `registerType("paypay", ...)` の1行で済み、`PaymentFactory` の中身（`create`）を触る必要がありません。今回の変更要求（決済手段の追加）に対しては、これで十分に応えられます。さらに、登録する内容を環境ごとに変えれば、テスト時にモックのプロセッサーを登録するといった差し替えもできます。**追加する種別の登録さえできればよいなら、この案で止める判断は十分あり得ます。**

**この案に残る課題：**
このマップは「1つの登録済みテーブルを持つ、固定された1つの `PaymentFactory`」です。生成方針そのもの――たとえば「本番用の生成一式」「ステージング用の生成一式」「テスト用の生成一式」を、丸ごと1つの単位として受け渡し・差し替えたい場合、マップの中身を入れ替えるか、複数のファクトリを別々に持って使い分けることになります。生成する側そのものを1つのオブジェクトとして注入で差し替え、決済フローはその生成方針を意識せず使うだけにしたい――これが次のステップの主題です。

もう少し深く考えると、`PaymentFactory`（固定ファクトリ版）は固定された1つのクラスでした。「テスト環境ではモック用のプロセッサーを使いたい」「本番とステージングで生成ロジックを変えたい」といった要求が来たとき、生成方針を1つの単位として差し替える仕組みがありません。

「生成する側そのものを、注入で差し替え可能にできないか」という問いが見えてきます。
...
*   **ステップ2の別案（生成マップへの登録）で止めるケース：** 対応する決済手段を増やすことが主目的で、追加を登録1行で済ませたい場合。生成方針を丸ごと1つの単位として差し替える必要がなければ、この案で足ります。
...
| 🔴 フェーズ6：対策検討 | 読者が思いつく生成マップへの登録案もコードで試し、生成方針を丸ごと差し替えにくい残課題を確認した上で、ステップ3（生成分離構造・抽象化）まで進化させる決断を下した |
