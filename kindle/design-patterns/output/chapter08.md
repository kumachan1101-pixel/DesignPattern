...
- **生成・注入・実行による組み立て**：方式の実体を誰が生成・所有・登録し、共通フローが契約として選んでどう実行するかを一本で決める。
...
**ここから決めること：** 共通の決済要求・結果を守りながら方式ごとの検証・手順・完了方法をどこへ置き、具体方式を誰が選択・生成するかを順に決めます。

**この章の課題は1つですが、二つの判断は他章と同じです。** 契約と具体をセットで分け、その実体を生成・注入・実行の一続きで組み立てます。軸が一つなので、それぞれを一度ずつ追えば全体がつながります。

**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、方式ごとの処理と生成判断を、決済の共通フローとは別の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
変更前の `if` の中にあった「どの具体を作るか」が、ここにはありません。**では、それを決めるのは誰の仕事になるのか。それは生成の検討で決めます。**
...
- **どの段で決まるか。** 生成の検討で決めます
- **`PaymentApplication` 自身に持たせられないのか。** 持たせれば同じことです。**ただし、この章はここが他章と違います。** 詳しくは生成の検討で書きます
...
**変更前と比べると、`if` 連鎖と具体クラス名が1つも無くなりました。** 消えたわけではなく、`createProcessor()` の中へ移る予定です。**移した先で1か所に収まるかどうかも、生成の検討で確かめます。**
...
**ここで図に描けるのは、骨格から契約への依存だけです。** 実体をどこから得るかがまだ決まっていないので、その線は引けません。
...
**契約の確認で見た `CreditCardProcessor` の疑問が、ここで解けます。** 「どの具体を作るか」を書かないのは、それが生成の検討で決まる別の責任だからです。契約の裏に置くのは「この方式でどう検証し、どのAPIを呼び、結果をどう組み立てるか」だけです。
...
**具体クラス名が4つ並んでいます。この章で、それが並ぶのはここだけです。** 変更前は共通フローの中にあった同じ分岐が、生成の1か所へ移りました。分離の検算で「移した先で1か所に収まるかどうかは生成の検討で確かめる」と書いたのが、これです。
...
**ここが終わった時点で、実体はすべて存在するようになりました。** 抽象骨格、それを継いだ派生、4つの方式。**まだ起きていないのは1つだけです。骨格の `IPaymentProcessor*` と書かれた変数へ、具体が入ることです。次に、注入として確認します。**
...
**守る範囲との照合：** **共通の依頼→結果フロー**と**保留から完了確認への2段階**を保っていることを、いま確認しました。保留が返ったら共通の `checkCompletion()` を呼ぶ形も、3-1のままです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
**この章の完成構造は一つに定まります。** 生成役を別のクラスとして切り出し、`PaymentApplication` がそれを保持する案も置けますが、生成の検討で確かめたとおり、この章では共通フローと生成が1対1で結びついているため、保持の行と外から渡す行が増えるだけです。責任配置が異なる完成構造が複数残ったわけではないので、当て馬を並べた比較は行いません。
...
| 生成・注入・実行による組み立て | 統括が具体名を4つ抱える → `DefaultPaymentApplication::createProcessor()` | 方式IDから具体を生成して契約として返し、共通フローが `pay()` を実行して結果を処理する | 生成した方式が同じ呼出中に必ず使われ破棄される |

決済依頼の呼び方は変わりません。共通フローの内側で、方式IDに応じた具体を生成して契約として実行します。

#### 代表ケースの実行接続

生成した決済方式が実際に使われるまでを確認します。クレジットカードでの決済1件を、生成→契約への受け渡し→共通フロー→具体処理の順で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成＋注入外側 | `main()` | `DefaultPaymentApplication app;` ―― **この型が `createProcessor()` の実装を決める**（コード上に入れる行は無い） | 実行開始へ |
...
| 4. 生成＋注入内側 | `DefaultPaymentApplication::createProcessor(const string&)` | 登録と有効を確認し、`new CreditCardProcessor(gatewayClient)` を返す | 骨格の `IPaymentProcessor*` へ入る |
...
**生成の検討で作ったものを、3が捨てます。** 作る場所と捨てる場所が別の関数にあるので、`createProcessor()` の戻りは「借りるもの」ではなく「受け取って所有するもの」だと分離の検算で決めておく必要がありました。**所有を決めずに骨格を書くと、この `delete` を書けません。**
