...
|---|---|---|
| イベントID、操作、現在の予約状態 | 予約対象、利用者操作、予約の進行状態 | 動作例テーブル、状態遷移表 |
| 存在確認、満席判定、状態ごとの操作可否判定、状態更新 | 予約操作を成立させる判定と状態変更 | 現状仕様の確認、変更シミュレーション、原因分析 |
| 成功/エラーメッセージ、更新後の状態 | 利用者に返る結果と次の状態 | 動作例テーブル、実行結果、変更シナリオ表 |
