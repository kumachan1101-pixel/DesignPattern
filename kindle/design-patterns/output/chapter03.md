...
**【1】 イベント在庫（EventInfo / EventDatabase）**
...
**【2】 予約履歴（ReservationRecord / ReservationHistory）**
...
**【3】 キャンセル待ちキュー（ReservationWaitlist）**
...
**【4】 状態インターフェース（IReservationState）**
...
**【5】 予約操作のコンテキスト（TicketReservation）**
...
`AvailableState`は空席の有無で予約または待機登録へ進めます。次は、予約済みと支払済みの責任をまとめて確認します。

**予約済み・支払済み状態（ReservedState / PaidState）**
...
`ReservedState`は席を解放する操作から待機者の自動昇格まで完結させ、`PaidState`は既定の拒否だけを使います。残る待機・保留状態と期限切れ境界を続けます。
...
**【7】 組み立てと実行（BatchApplication / main）**
