...
★この章を代表として、大量に★の指摘を入れています。話の流れがとにかくわかりづらい。まずはこの章を例に指摘内容を理解して、原因を洗い出し、対策を決めて、全章に適用できるような対策案を考えて、全章修正してほしい。
### この章の核心
...
設計の選択肢（案0〜案4）はすべて、この仕様を同じように実現します。「仕様は同じで、構造だけが異なる」ことが比較の前提です。★なんで設計の選択肢の話がここで出てくるのか。意味が分かりません。

**予約状態遷移ルール**
★仕様がまず先でしょ。このシステムはこんな仕様です。という記載が必要です。そのあと、コードを出した方が良いのでは？コードが出せば、状態遷移の話も理解できる。実装コードを
...
★コードをまだ見ていないから以降の話が全然頭に入りません。仕様⇒コード⇒構成⇒仕組み⇒詳細の流れが良いのでは？
...
★main関数がないので、何故以下の実行結果になるのかわかりません。1-6と1-7はセットでよいのでは。
### 1-7：実行結果
...
★この一文は何ですか。急に偉そうになっています。私は常に謙虚に行きたいのです。
### 1-8：責任チェック表

★ここ難しい。どやってこの表を作るの？どんな見方をして、この表を埋めていくのか、その思考の過程を説明してから、表を見せてほしい。
...
★ここはどういう意味？フェーズ1での現状システムに対しての話なので、直前の変更要求の話なのか？普通の話の流れなら、変更要求に対してですよね。そうであれば、フェーズ1の観察にもどつきというより、この変更要求を受けて、変わりそうというのは、どこを変えるのかであり、仮説ではなくないか？ここで言いたいのは、今後はさらにどこが変わりそうかという事だと思う。そうすると、変更要求を受ける。ここが変わる、今後はどうなるか、の方が良いのか。そもそも話の流れが破綻している。これを変更要求の前にもって来た方が良いのでは。それか変更要求を受ける⇒変わるところを特定する⇒この後仮説を立てる⇒追加のヒアリングする⇒仮説を確定、の流れが良いのでは？
...
★これは全章に同じ文言を入れるの？第１部の締めくくりの章を追加して、そこで説明を入れた方が良いのでは？ほかにも説明の都合でこうしているが実際の現場ではこうなる事が多いといった内容も１部のまとめ章に追加したい。
...
★ここを読むときに仕様変更する内容を簡単に整理してから変更シミュレーションに移ってほしい。
...
コードを書きながら、ふと手が止まります。★言い方が少し嫌味になるのでやめてほしい。常に謙虚な姿勢で行きたいのです。
`reserve` メソッドを直したなら、当然 `pay` メソッドや `cancel` メソッドの中にある `if` 文の条件もすべて見直し、新しい状態である `Held`（保留）を考慮しなければなりません。
...
★ステータスと振る舞いは混在するのはよいことでは？ここはどういうロジックで抽出しているの？観察って書いてあるけど、何を観察するの？観察の仕方を示して、何を発見するのか。
...
現在の `TicketReservation` クラスと状態管理の接続は、すべての状態と振る舞いが一枚の大きな回路基板上に溶接されている状態（具体×直接）だと診断できます。★急にこの説明意味不明、話の流れがあるでしょう。0章でTypeCなどの接続方法の説明を入れるからそこの考えをもとにこのシステムを当てはめると、という説明ならわかる。あと、そうするなら、以下表はいらないよね？０章と重複になりませんか。すべての章に入れるのか？というか抽象化されていない構成なら、抽象は該当しないし。ここで判定する意味あるのか。次につなげるには必要と思うが、考え方としては抽象的な関係性はない、間接化しているか、そのあたりを分析して、具体直接を導き出すのが良いのではないか。
...
★半田付け？そんな例にしたいわけではない。具体直接というのは、iPhone専用のライトニングケーブルがiPhone専用の機材で接続されている場合など、他のandroidでは使えないよね？そういう話をしたいのです。半田つけなんて、なんでもあり的な話ではありません。
...
★現在地って何？Apple純正ドックって何？現在地って、今のシステムが具体直接に該当していますって意味なら分かりづらいでしょ？
...
現在の `TicketReservation` クラスがこの接続点のクライアントです。`TicketReservation` は `status` 変数を直接持ち、すべての遷移ロジックを自身で抱えています。この境界を切り離すことは、`TicketReservation` の内部実装を完全に作り変えることと同義であり、既存の状態管理を全く別の仕組みへと移行させることになります。★影響が大きいという話をしているようですが、その分析を次にどのように繋げていくの？
...
★これら機材と、先ほど挙げた状態と振る舞いがどこに配置するのかも、目途づけてこの表に入れた方が良いのでは。その後、各接続形態の詳細説明に進んでいく。
...
★以下の構造図はクラス図にした方が良いのでは？そしてクラス同士の関係性は示してほしい。接続されていないから意味が分かりません。現状維持なのにクラス追加するの？話も矛盾しています。
...
★案０と案1の違いが判らない。案0は構造変えないと言いながら変えていたのだから。
**この形の考え方：**
状態ごとにクラスを分割しますが、呼び出し側の `TicketReservation` がそれらの具体クラスを直接知っている状態です。責任は整理されますが、状態を追加するたびに呼び出し側の依存関係を修正する必要があります。
★このクラスの分離が、どういう責任分担で分けられたのか理解できませんでした。シンプルに状態を分けて、コントロール側は分離する必要はないのでは？考え方が良くわかりません。読者に疑問に持たれませんか？すべてに言える話ですが、読者に疑問に持たれるような説明は絶対にやめてほしい。もし、疑問に持たれそうだなと思ったならば、その疑問を予測して先に回答となる説明を入れておくのです。全体的にそれがないと疑問ばかり溢れて、意味不明な感じで終わります。
...
★案0から４をすべて見ましたが、全然理解できませんでした。仕様もあまり頭に入っていない状態で、いきなりこんな細かい話されても全然ついていけない。シーケンス図なんて見ようと思いません。シーケンス図追加してもらったが意味をなしていないかも。クラス図とあと仕様をどうやってどんな構造で実現しようとしているのか、専用の説明が必要な気がする。仕様と照らしながら説明してくれないと分かりません。コードにコメント沢山いれるのではなく、コードをブロック単位で区切って、構造と紐づけながら説明するのが分かりやすいのでは？少しずつ細かく丁寧に説明してほしい。
...
★まとめが簡略化しすぎる。冒頭に書いた、この本で得られることは本当に得られたのか、振り返ってみた方が良いのではないでしょうか。
