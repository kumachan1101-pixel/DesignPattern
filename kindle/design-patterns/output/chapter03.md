...
この表は、現在の状態ごとに、各操作を受け付けたとき次にどの状態へ進むかを整理したものです。行は「いまの状態」、列は「利用者が行う操作」、セルは「操作後の状態」を表します。まずは、予約可能な状態で予約すると予約済みに進み、予約済みの状態で支払うと支払済みに進む、という基本の流れを押さえます。
...
| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| メソッド切り出し | 各状態の処理の名前 | どの状態の処理か名前で読める | 状態が増えると全操作へ分岐を書き足す |
| 遷移テーブル | 状態と操作の組み合わせ→次状態 | 遷移ルールがデータ1か所に集まる | 状態ごとの振る舞い（処理・メッセージ）は値で表せない |
| 状態ごとのクラス分け | 各状態の振る舞い | 状態ごとにロジックが独立する | 本体が全状態クラス名と選択のif文を知ったまま |
| 状態の契約と委譲 | 状態の振る舞い全体 | 本体から状態判定のif文が消える | 状態クラスと遷移の登録は残る |

この4案のうち「遷移テーブル」は、状態機械を見たときに最初に思いつきやすい案です。状態と操作の組み合わせから次状態を引くだけなら、コードの分岐ではなくデータの表で持てるはずだ、という発想です。この案は比較表の中だけで済ませず、ステップ2の中で案Aとして実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。メソッド切り出しはステップ1で、状態ごとのクラス分けはステップ2の案Bで、状態の契約と委譲はステップ3でコードにします。

なお、メソッド切り出し・クラス分けは1つ目の「共通化・抽象化」の実行、遷移テーブル・契約と委譲は2つ目の「変わる部分を外へ出し、本体は決まった流れを回すだけにする」の実行にあたります。
...
ステップ1の限界は「状態が増えると全操作へ分岐を書き足す」ことでした。状態を増やしても本体を触らずに済ませたい。クラスを増やす前に、比較表の「遷移テーブル」案から確かめます。
...
class TicketReservation {
    ...
    // (現在状態, 操作) → 次状態。遷移ルールがデータ1か所に集まる
    std::map<std::pair<State, Event>, State> transitions = {
        {{State::Available, Event::Reserve}, State::Reserved},
        {{State::Reserved,  Event::Pay},     State::Paid},
        {{State::Reserved,  Event::Cancel},  State::Available},
    };
public:
    void handle(Event ev) {
        auto it = transitions.find({state, ev});
        if (it == transitions.end()) {
            std::cout << "その操作はできません\n"; // 遷移が無い＝エラー
            return;
        }
        state = it->second; // 組み立て側は次状態を引くだけ
        std::cout << "状態を更新しました\n";
    }
};
...
- `handle()` は表を引いて状態を更新するだけです。新しい状態を追加するとき、遷移の追加は表に行を足すだけで済みます。

**案Aの評価：**
状態と操作の組み合わせから「次にどの状態へ進むか」を決めるところは、この表で完全に表せます。遷移の追加が表への1行追加で済む点は大きな利点です。**変わるのが遷移ルールの結線だけなら、この案で止める判断は十分あり得ます。**

**案Aに残る課題：**
この章の変更要求は遷移の追加だけではありません。各状態は、遷移とは別に**状態ごとに異なる振る舞い**を持っています。「予約完了しました」「支払い済みのため再予約できません」といったメッセージや、キャンセル待ちが空きを検知して予約へ昇格する処理は、状態ごとに中身が違います。これらは「次状態」という1つの値では表せず、表の各マスに処理を持たせようとすると、結局そのマスの中身が `if` や関数になって分岐が戻ってきます。遷移の結線はデータで、状態ごとの振る舞いはコードで持ちたい――この振る舞いの置き場が、次の案の主題です。

**案B：状態ごとにクラスを作る**

そこで、「状態ごとの振る舞いを外へ出す」という方向で、置き場を表のマスからクラスへ変えます。`ReservedState`・`AvailableState`・`PaidState` という3つのクラスを作り、`TicketReservation` がそれぞれを直接フィールドとして持つ形を試してみる。
...
* **ステップ2・案A（遷移テーブル）で止めるケース：** 変わるのが状態間の遷移ルール（結線）だけで、状態ごとの振る舞い（処理・メッセージ）は当面変わらないと確認できている場合。遷移の追加が表への1行追加で済みます。
* **ステップ2・案B（状態ごとのクラス分け）で止めるケース：** 状態ごとの処理を別クラスに整理したいが、新しい状態が追加されるかどうかまだ確証がない場合。状態の数が少なく、追加の見込みが薄ければここで止めるのも現実的な選択です。
...
| 🔴 フェーズ6：対策検討 | 遷移テーブル案（案A）を含む案を比較し、状態ごとの振る舞いは値で表せない残課題から、状態の契約を導入して現在の状態へ委譲するステップ3を採用しました。 |
