...
- **生成・注入・実行による組み立て**：実体を誰が生成・所有し、公開操作を持つクラスへ契約として渡してどう実行するかを一本で決める。
...
**ここから決めること：** 状態ごとの判断と待機順の管理を公開フローからどこまで外し、現在状態への操作と席解放後の自動昇格をどう接続するかを順に決めます。
...
**1つ目は、`AvailableState` のような具体の名前をここに書くわけにいきません。** 書けば課題ID1で分けた意味が消えます。**2つ目は、席数・履歴・待ち行列という3つの相手が要ります。** どちらも**生成の検討で決めます。**
...
**ここで図に描けるのは、骨格から2つの契約への依存だけです。** 実体をどこから得るかがまだ決まっていないので、その線は引けません。
...
**ここが終わった時点で、実体はすべて存在しています。** 5つの状態、待ち行列、在庫、履歴、それらを持つ組み立て役。**まだ起きていないのは1つだけです。骨格の `IReservationState*` と書かれた変数へ、具体が入ることです。次に、注入として確認します。**
...
**骨格が状態を借りる方法（ポインタで共有実体を指す）は、生成の検討で決めた話です。** 値で持つと遷移のたびにオブジェクトを作り直すことになりますが、**注入はどちらの持ち方でも起きます。**
...
**守る範囲との照合：** **公開入口**を保っていることを、いま確認しました。`reserve()` の引数が1つ減りましたが、これは呼び出し側が持つべきでない在庫の知識を返しただけで、操作の形は変わっていません。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
| 生成・注入・実行による組み立て | 予約が全分岐・行列を内包 → 組み立て役が共有状態・行列・在庫・履歴を用意 | 生成時に初期状態を渡し、遷移時に次状態を入れ、取消から自動昇格まで実行する | 状態軸と待ち行列軸の接点が一経路で追える |

公開入口の `reserve()` は在庫判定を引数で受けず、自分の依存へ問い合わせます。取消後の昇格も同じ実行経路の中で自動的に起こります。

#### 代表ケースの実行接続

生成した状態と待ち行列が実際に使われるまでを確認します。満席のイベントで待機していた予約が、他の予約の取消によって自動昇格するまでを一本で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成 | `BatchApplication`（`db`・`history`・`waitlist` はメンバー） | 在庫・履歴・待ち行列を1つずつ所有し、状態は `availableState()` などが共有実体を返す | 注入へ |
| 2. 注入 | `BatchApplication` の予約生成行 | `TicketReservation r(availableState(), &db, &history, &waitlist, ...)` で初期状態が入る | 実行開始へ |
...
| 5. 契約→具体状態軸 | `ReservedState::cancel(TicketReservation*)` | `setState(availableState())`（＝注入2回目）→ 席を戻す → 履歴 → `promoteNextWaitlisted()` | `TicketReservation::promoteNextWaitlisted()` |
...
**注入が2回現れることに注目してください。** 分離の検算で初期状態が入り、注入の確認で次の状態が入ります。**組み立てのときに1回ではありません。** 遷移が起きるたびに、次の実装が入ります。
