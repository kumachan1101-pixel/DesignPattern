...
この動作テーブルは、後のフェーズで案を比較するときに「全ステップがこのテーブルと同じ出力を返すことで動作不変を確認する」ための基準として使います。Step 1からStep 4まで、どれを採用しても上記の動作が変わってはいけません。違いはあくまで「変更が来たときにどこを触るか」という構造上の差だけです。
---
...
| 🔴 フェーズ6：対策案検討 | Step 1〜Step 4を比較し、最も変更耐性が高くテストも容易な案3（抽象×直接）を採用しました。 |
