...
「——」は操作を受け付けず、エラーメッセージを出力して終了することを表します。なぜ「——」を設けるのかというと、「現在の状態にとって意味をなさない操作」をシステムが受け入れてしまうと、データの整合性が崩れるからです。たとえば、まだ予約していない座席に対して突然「支払う」を実行できてしまうと、どの予約に対する支払いなのかがシステム上で確定できません。そのため「今の状態では受け付けられない」を明示的にエラーとして返すことが、業務的にも技術的にも正しい設計です。これはチケット予約に限らず、EC注文・銀行振込・申請ワークフローなど、状態を持つシステム全般で広く採用されているルールです。
