...
|---|---|---|
| 分離方法 | 接続点をどう切るか | 状態固有の判断（P1）を公開操作の骨格から、待ち行列の探索・削除・先頭選択（P2）を状態処理から切り出す。境界には状態操作と待機者操作だけを残す |
| 配置場所 | 切り出した責任をどこへ置くか | 状態ごとの振る舞いは状態1つにつき1クラスへ、待ち行列の方針は `ReservationWaitlist` へ置く。守る側には公開入口と座席・履歴の副作用を残す |
| 組み立て方法（生成・所有・登録・注入） | 誰がどう接続するか | `TicketReservation` が現在状態を所有し、遷移元状態が次状態を選ぶ。`BatchApplication` が待ち行列を生成・所有して予約へ注入する。状態登録は不要で、席解放処理が待ち行列の先頭昇格を自動で呼ぶ |
