...
**採用する手札：インターフェース抽出（第0章 手札）をステータスに適用する**

真因に直接対処するには「ステータスをクラスとして定義し、そのクラスの中に cancel・refund・admit 全ての振る舞いをまとめる」しかありません。第0章のインターフェース抽出（第0章 手札）を、アルゴリズムではなく「ステータス」という軸で使います。

「1つのステータスが持つべき振る舞いの契約」をインターフェースとして定義します。

---

### 3.7 インターフェース抽出の適用：IReservationState インターフェースの導入
...
| 状況 | 適切な選択 | 理由 |
|---|---|---|
| 状態が4つ以上・今後も増える見込みがある | 試み②（IReservationState） | ステータス追加の影響が局所化される |
| 複数のメソッドがステータスで振る舞いを変える | 試み②（IReservationState） | 振る舞いを状態軸でまとめられる |
...
| ステップ5 | 真因から手札を選定した。switch文統一は真因を解消しないと判断し、インターフェース抽出（第0章 手札）をステータス軸で適用して IReservationState を導入した |
