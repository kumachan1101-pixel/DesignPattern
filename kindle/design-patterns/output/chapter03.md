...
「集める」という方向性が決まりました。集め方には複数の手段があります。順番に試してみましょう。

---

### 3.6.2 手段①：switch文への統一（直感的な案）

まず思いつく案は、各メソッドに散らばった条件分岐を、最新のC++の機能やswitch文を使って整理することです。

```cpp
// 手段①：全メソッドをswitch文で統一
void cancel() {
    switch (status_) {
        case Reserved: status_ = nullptr; break; // 成功
        default: break; // キャンセル不可
    }
}
// 他のメソッドも同様にswitch化...
```
...
「席変更受付中」という新しいステータスを追加することを想像してください。
- `cancel()` を開いて `case SeatChanging` を追加
- `refund()` を開いて `case SeatChanging` を追加
- `admit()` を開いて `case SeatChanging` を追加

結局、**3つのメソッド全てを開いて修正しなければならない構造**は変わりません。「1つのステータスに関する知識」は依然として3か所に分断されたままです。

---

### 3.6.3 手段②：状態をクラスとして切り出す（発想の転換）

手段①の課題（分断の継続）を受けて、発想を転換します。

第0章の手札選択表を引くと：「オブジェクトの状態とそれに伴う振る舞いが変わる」→ **状態クラス化**（第0章 手札）。この原因に直接対応します。

**「ステータスをクラスとして定義し、そのクラスの中に振る舞いを全てまとめる。」**

このように、変わり続ける状態ごとの振る舞いを個別のクラスに閉じ込め、コンテキストから委譲させる構造を **State（ステート）パターン** と呼びます。
...
classDiagram
    ...
    ReservationManager --> IReservationState
    IReservationState <|.. ReservedState
    note for ReservedState "この1クラスを見れば\n『予約中』の全振る舞いがわかる"
```
...
解決策（手段②：状態クラス化）を導き出しましたが、ここで一度立ち止まります。手段①（switch文）の方がコード量は少なく、直感的だからです。本当に今回の状況で、クラスを量産するコストを払う価値があるかを天秤にかけます。
...
| 評価軸 | なぜこの状況で重要か |
|---|---|
| 変更の局所性 | ステータスの追加やポリシー変更が頻発するため、既存コードへの影響を最小化したい |
| 振る舞いの追跡容易性 | 「このステータスの時はどう動くか」を、一つのファイルを読めば完結するようにしたい |
| 実装のシンプルさ | クラス数が増えることによるファイル管理の煩雑さとのトレードオフ |
...
| 評価軸 | 手段①（switch文への統一） | 手段②（状態クラス化） |
|---|---|---|
| 変更の局所性 | ❌ 新状態追加時に全メソッドの修正が必要 | ✅ 新状態クラスを1つ追加するだけで完了 |
| 振る舞いの追跡容易性 | ❌ 複数メソッドにロジックが分断される | ✅ 1クラスに全ての振る舞いが集約される |
| 実装のシンプルさ | ✅ 非常にシンプルでC言語に近い感覚 | ❌ クラスとインターフェースが増える |

今回の状況では、ステータスの追加が頻繁に予定されており、かつ各ステータスのポリシー（振る舞い）が複雑であることを考慮し、**手段②（Stateパターンによる状態クラス化）を採用します。**

---

### 3.10.3 耐久テスト ―― ヒアリングで挙がった変化が来た
