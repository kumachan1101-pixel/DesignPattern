...
フェーズ6は、フェーズ5で分けた2つの課題——**状態ごとの振る舞いを切り離すP1**と、**自動昇格を維持したまま待ち行列方針を切り離すP2**——を受けて始めます。まずフェーズ4で記録した2本の痛みを図の起点に置き、P1とP2でどのコード境界を微修正し、同じ要求を当て直すと影響がどこまで縮むかを枝ごとに描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じ課題IDの行へ落とします。その後、各IDに紐づくフェーズ3の関連コードだけをおさらいし、候補を一段ずつ適用します。P2ではキャンセルを起点とする自動昇格までをコードと数値ログで確認し、統合後の全体コードはフェーズ7で示します。
#### 痛みの変更影響から、構造をどう変えるか
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S11["P1-変更1: 状態処理をクラスへ寄せる"] --> S12["P1-変更2: 現在状態へ一律委譲する"]
        S21["P2-変更1: 待ち行列を専用クラスへ移す"] --> S22["P2-変更2: cancel直後の自動昇格を接続する"]
    end
...
|---|---|---|---|
| P1 | 状態実装、`IReservationState`、`TicketReservation::reserve/pay/cancel()` | **微修正：** 状態別クラス→`state` 保持→公開操作を一律委譲<br>**結果：** `status` 選択分岐が消えた | **守った契約：** 公開操作、座席、履歴<br>**判定：** クラス化だけでは未達、一律委譲で達成 |
| P2 | `ReservationWaitlist`、`ReservedState::cancel()`、`promoteNextWaitlisted()` | **微修正：** 探索・削除をキューへ移す→`cancel()` 直後に先頭取得と自動昇格を接続<br>**結果：** 手動キュー操作・昇格が不要になった | **守った契約：** 50/50→49/50→50/50、先着順、1件昇格<br>**判定：** キュー分離だけでは未達、自動連鎖で達成 |
