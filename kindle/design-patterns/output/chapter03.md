...
フェーズ4で、`TicketReservation` が「状態ごとの操作可否と遷移」と「待ち行列の探索・削除・先頭選択」という**別々の理由で変わる2つの判断**を、同じ場所へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つの予約進行経路へ再結合することです。ここで使う構造は、第一部で扱う基本構造です。構造名（と対応するパターン名）を語彙として併記しますが、パターン名から設計を選ぶのではなく、上で確認した「別々に変わる2つの判断」から必要な構造を導きます。

```mermaid
flowchart TB
    A[現在<br/>TicketReservationに<br/>状態分岐と待ち行列操作が混在] --> B[分離判断<br/>二つの変化軸を別責任へ分け<br/>一つの予約進行経路へ再結合]
    B --> C[課題ID1<br/>各状態が可否と遷移と副作用を持つ<br/>状態分離＝State]
...
**どう解決するか（方針）：** 状態ごとの振る舞いを共通契約の裏へ揃えて現在状態へ委譲し（状態分離構造＝State）、待機順の探索・削除・先頭選択を専用の係へ閉じます。
...
**出発点。** やることはもう決まっています。`TicketReservation` から状態ごとの振る舞いを、待機順の管理を、それぞれ分けることです。分ける対象も5-3で決まっています。まだ決まっていないのは、**分けた後にどう繋ぐか**だけです。
...
**まず、分けないで済まないかを確かめます。** 5-3が確定したのは「この接続点を解く」ことであって、「クラスを増やす」ことではありません。次の2つがどちらも成り立つなら、契約を作らず、判定を関数へ1つ切り出すだけで止めます。
...
|---|---|---|---|
| リスクID1：状態遷移のルール（アクションの可否）の変更 | 可否と次状態を現在状態のIReservationState実装へ置く | 影響するStateクラスの対象操作 | 他状態と受付入口を守り、変更を遷移元Stateへ閉じられる。複数状態共通の規則は重複しやすい |
| リスクID2：状態の種類（特別優待予約などの追加） | 状態ごとの振る舞いを共通契約で分ける | 新しいStateクラスと、その状態へ入る遷移 | 既存状態と公開操作を守れる。進入元が多い状態では複数Stateの遷移追加が残る |
| リスクID3：状態を動かすイベントの追加 | イベントの起点と状態判断を分け、システム起点の処理もTicketReservationへ接続する | 新イベント入口、IReservationStateの操作、必要なスケジューラ | 状態判断をState側へ保てる。時刻監視や再試行の実行基盤は現在の同期入口の外に必要になる |
