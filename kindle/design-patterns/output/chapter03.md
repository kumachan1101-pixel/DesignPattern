...
| P2 | 待ち行列方針を変えると `cancel()`・`expire()`・静的キューを同時に修正する | 席解放直後の自動昇格、先着順、定員上限 |

状態の振る舞いと待ち行列方針は別の理由で変わるため、P1、P2の連番で分けます。フェーズ6の理想グラフでも同じ2枝を追います。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で分けた2つの課題——**状態ごとの振る舞いを切り離すP1**と、**自動昇格を維持したまま待ち行列方針を切り離すP2**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを変換して探します。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる

フェーズ4では、状態追加が全公開操作の分岐へ波及し、待ち行列の扱いを変えると cancel() / expire() と静的キューを同時に触る必要がありました。変化軸が2つあるため、理想グラフもP1とP2へ分けます。
...
flowchart LR
    ...
    subgraph Ideal["理想の波及"]
        IR1["状態を追加"] --> I1["P1: 状態クラスと遷移元だけを追加"]
        IR2["待ち行列方針を変更"] --> I2["P2: ReservationWaitlist だけを変更"]
        IS["変更しない: 公開操作・自動昇格の起動条件"]
    end
```
...
|---|---|---|---|
| P1 | 状態追加の影響を全操作の文字列分岐から、状態クラスと遷移元へ縮める | status 判定を切り、reserve/pay/cancel/hold/expire の公開入口と在庫・履歴副作用を守る | 状態追加で TicketReservation の状態選択分岐を増やさない |
| P2 | 待ち行列の探索・削除を cancel()/expire() から専用キューへ移す | 席解放→先頭1件の自動昇格を切らず、先着順・定員上限を守る | 利用側は cancel() だけを呼び、キュー操作や昇格操作を呼ばない |

P1とP2は理想グラフの別枝です。したがってIDはP1、P2の連番で、各表に1行ずつだけ置きます。

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 状態ごとの処理をクラスへ寄せる | 状態別の振る舞いを独立して読める | 現在状態へ一律委譲し、中心クラスの状態選択分岐を消す |
| P2 | 待ち行列をイベント別の専用クラスへ移す | キュー方針の変更が状態処理から離れる | 席解放直後の自動昇格接続を状態処理内に維持する |

ここからP1・P2に該当する状態分岐と待ち行列コードだけをおさらいします。
