...
★waitlistのstaticを追加するなら、DataBaseもStaticで持たせた方が、管理場所が統一されるのでは？
★現状は、過去のDataBaseを次の引き継いで使用されることが前提になっており、利用者依存になっています。
★以下コードコンストラクタがなくなっている。クラスを記載するならすべて記載して
★関数の先頭やデータにはコメントを入れてほしい。これは全章共通の指摘です。
...
#### フェーズ6へ渡す課題
★課題の定義は、0章を見ると、解くべき接続点ですよね。以下は課題IDは、解くべき接続点が何か？がIDで定義されるのでは？接続点は出せているからタイトルや説明文を少し変えればよいのかもしれない。
...
★以下コード、おさらいと言っているが、if文のブロックがなくなっており、見た目が変わっている。そろえてください。
