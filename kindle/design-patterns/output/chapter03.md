...
まだクラスの中身は見ません。この段階でつかんでほしいのは「状態ごとの振る舞いを状態オブジェクトへ分け（課題ID1）、待ち行列と自動昇格を専用の係へ寄せて（課題ID2）、公開入口は現在状態へ委譲するだけにする」という筋だけです。「どのクラスが生成し、どの契約で実行するか」という具体の結論は、この後の課題ID1・課題ID2で決めていきます。決めた結論をまとめて振り返る表は、フェーズ6の末尾（6-4 設計トレース）に置きます。ここでは先に結論表を出しません。
