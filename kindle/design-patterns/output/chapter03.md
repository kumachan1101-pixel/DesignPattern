...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。利用者がイベントEVT001（春の音楽祭）を予約し、続けて支払うと、内部は予約枠を確認して状態を予約済みへ進め、支払いで状態を確定させます（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
|---|---|---|---|
| 変更ID1 | 満席時にキャンセル待ちへ登録し、空席発生時に先頭を自動昇格する | イベントID、利用者ID、満席状態、予約済み取消 | 取消で49/50になった直後、先頭待機者が予約済みとなり50/50へ戻る |
...
この章は変化軸が2本あり、それぞれ別の理由で変わるため、最終構造は**2つの分離構造を一つのシステムへ組み合わせて**決まります。各状態クラスに待ち行列操作まで持たせる案は、方針変更のたびに複数の状態クラスを開く途中状態なので比較対象にしません。状態固有の振る舞いは各状態クラスへ、待ち行列方針は `ReservationWaitlist` へ移し、席解放時に前者が後者を呼ぶ1点で接続します。
...
2つの構造は、`ReservedState::cancel()` が席解放後に `ReservationWaitlist` の先頭を自動昇格させる1点だけで接続します。ここを切らないことが、課題ID1と課題ID2を独立に変えても業務契約が壊れない条件です。
