...
|---|---|---|---|
| イベントID | 予約対象のイベント | EVT001 | イベントの存在と空席を確認する |
...
この時点では、状態ごとにクラスを分けるかどうかは判断しません。まず、どの状態でどの操作が許可され、どの結果になるかを確認します。
...
|---|---|---|
| `Booking` が状態名と、各状態で許可される操作をまとめて判定している | 状態ごとの振る舞いを、公開操作の形を変えずに切り離す | まず判定を関数へ分け、状態ごとの差分がどこにあるかを見る |
...
| 状態判定を関数化 | 条件に名前が付く | 状態ごとの振る舞いは同じクラスに残る | 最初の整理として有効 |
| 状態ごとにクラス分離 | 状態ごとの振る舞いを別に読める | 呼び出し側が具体状態を知り続ける可能性がある | 中間策として有効 |
| 現在状態へ委譲する | コンテキストは状態ごとの分岐を知らずに済む | 状態クラスと遷移の組み立てが増える | 状態追加が続くため採用する |
