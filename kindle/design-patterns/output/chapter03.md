...
上の文章と表で仕様を一通り確認したので、まず正常に状態更新できる場合の入力・判定・加工・出力の流れとして整理します。★以下入出力、何故、イベントIDは１つしか記載されていないの？予約の操作方法はすべて記載されているのに。一貫性を持たせてほしい。
...
★書き方を統一して、以下のような１ラインにするのか。改行するのか。このレベルなら、１ラインの方な並列関係で見えるのでとても分かりやすい。類似箇所も同じ考えで統一してほしい。
...
1. **予約する（Available→Reserved）**：利用者が「予約」を押した瞬間に在庫から1席が引かれ、予約済み（未払い）になります。通常はこのまま短時間で支払う想定です。★短時間というのは？このまま何もされなくても、一時保留しなければ、ずっと待機できるの？
...
なお、この章がタイマーで自動失効させるのは「明示的に保留した24時間枠（Held）」です。保留していない Reserved 自体にも「◯分で自動解放」といった短いタイムアウトを併せて置く運用もありますが、その追加も同じ `expire` の考え方（現在状態が期限切れイベントを受けて次状態を決める）で、状態クラスを1つ増やす形で扱えます。
...
この仕様を今の `TicketReservation` クラスに当てはめてみます。追加した箇所が分かるよう、コメントで「追加」と明示します。座席在庫の存在確認・空席判定・座席の増減は1-4の `EventDatabase` がそのまま担うため、この抜粋では在庫まわりを省き、状態遷移に絡む部分へ集中します。新しく登場する待ち行列は、グローバルな `static` で持つと生成順や実行順に依存してしまうため、在庫（`EventDatabase`）と同じく組み立て側が所有する共有ストアとして扱い、コンストラクタで外から受け取ります。こうすると、在庫も待ち行列も「外から渡される共有ストア」で管理場所がそろい、どのインスタンスにも暗黙に引き継がれる状態がなくなります。関数の先頭と主要データにはコメントを付けます。
★待ち行列の使い方は、０章で説明されていないようなら追加して
...
| 状態ごとのイベント処理 → `expire()`/`promoteBySystem()`/決済失敗処理の骨格 | status（string値）と入力イベント → 状態維持/状態遷移 | タイマー、空席発生、決済失敗など、利用者操作以外の入力への反応 |
...
> あわせて、待ち行列の探索・削除・先頭選択を状態分岐から切り離す。ただし、予約済みがキャンセルされて席が空いた直後に、システムが先頭1件を自動昇格させる業務契約は維持する。
---

#### フェーズ6へ渡す課題

課題IDは、フェーズ5で特定した「解くべき接続点」に1つずつ対応します。**P1は「状態ごとの振る舞い ↔ 公開操作の骨格」という接続点**、**P2は「待ち行列 ↔ キャンセル／期限切れ処理」という接続点**です。次の表は、その接続点を今のまま放置したときの現在の変更影響と、切り離しても変えたくない範囲を示します。

| 課題ID | 現在の変更影響 | 変えたくない範囲 |
|---|---|---|
| P1 | 状態を追加すると `reserve()`・`pay()`・`cancel()`・`hold()`・`expire()` の文字列分岐へ横断的に波及する | 公開操作、座席数の更新、履歴などの既存副作用 |
| P2 | 待ち行列方針を変えると `cancel()`・`expire()`・待ち行列操作を同時に修正する | 席解放直後の自動昇格、先着順、定員上限 |
