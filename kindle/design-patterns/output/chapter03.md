...
> [!TIP] 読み飛ばしのヒント
> フェーズ1〜2は仕様の背景とヒアリングの内容が中心です。「コードの問題にすぐ向き合いたい」という方は、フェーズ3（問題特定）から読み始め、「なぜこの変更要求なのか？」と気になったときにこのフェーズに戻る方法も有効です。
...
> [!INFO] 「コンテキスト」とは
> Stateパターンの用語では、**状態オブジェクトを保持し、操作を状態に委譲するクラス**を「コンテキスト（Context）」と呼びます。この章では `TicketReservation` がコンテキストにあたります。「文脈」という一般的な意味ではなく、「状態の保持者・操作の委譲元」という設計上の役割を指す言葉です。
...
> [!INFO] なぜ `static` にして `new` しないのか
> `static` ローカル変数は、その関数が初めて呼ばれたときに1度だけ作られ、プログラムが終わるまで同じオブジェクトが使い回されます。`new AvailableState()` とすると呼び出しのたびにヒープへのメモリ確保が発生し、使い終わったら `delete` で解放する必要も出てきます。各状態クラスはデータ（フィールド）を持たず振る舞いだけを提供するため、インスタンスは1つあれば十分です。`static` により「1回だけ作って、ずっと使い回す」が自然に実現できます。
...
* **原則3「継承よりコンポジションを優先せよ」の現れ**
  ...
  * 解説：状態を継承で表現しようとすると階層が深まり柔軟性を失いますが、コンポジション（あるクラスが別のクラスのインスタンスをフィールドとして内部に保持し、その機能を利用する仕組み。「持つ」関係）として状態を持たせることで、実行時に状態を自由に入れ替えられるようになりました。Java・Python での「継承（`extends`/サブクラス化）」ではなく「委譲（フィールドに持って呼び出す）」に相当します。
