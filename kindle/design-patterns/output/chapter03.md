...
|---|---|---|---|
| F1 | 状態遷移のルール（アクションの可否）の変更 | キャンペーンや運用の見直し時 | 企画担当 鈴木氏との確認 |
| F2 | 状態の種類（特別優待予約などの追加） | 新機能導入時 | 企画担当 鈴木氏との確認 |
| F3 | 状態を動かすイベントの追加 | 外部連携や運用自動化の追加時 | 決済失敗、期限切れ、空席発生など、利用者操作以外の入力が増えるため |

フェーズ2で「今変わること（確定）」と「将来変わるかもしれないこと（リスク）」を分けて整理できました。次のフェーズ3では、現在の構造で変更を試みたときに何が起きるかを確認します。

### 2-5：変わる見込みと当面安定の前提を確定する

2-4のF-IDを、状態を変えられるようにする側と、予約システムとして守る側へ分けます。「はい」は、フェーズ6で**状態や遷移が変わっても、予約の入口・保存・席数管理へ影響を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：状態遷移のルール（アクションの可否）の変更 | はい | 状態ごとの許可操作と遷移先 | 予約操作の入口、予約履歴、席数の保存 |
| F2：状態の種類（特別優待予約などの追加） | はい | 状態の種類と、その状態に固有の振る舞い | イベントID・利用者IDの受け渡し、予約結果 |
| F3：状態を動かすイベントの追加 | はい | イベントと状態遷移の接続 | 状態保存、待ち順、利用者操作の入口 |

したがって2-5の出力は、「状態・遷移・イベント接続は変えられるようにし、予約の受付・保存・席数管理は守る」という設計条件です。フェーズ3ではR1・R2だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
確認したいこと：R1・R2を適用すると、条件分岐と新メソッドが`TicketReservation`へ集中すること
...
コード自体は正しく動いています。しかしR1・R2のために、`reserve()`・`cancel()`・`pay()`・`hold()`・`expire()`へ状態判定を追加した点に注目してください。
...
しかし実装では、Heldの1行を追加するために`pay()`と`cancel()`を開いて`else if`を追加し、`reserve()`でも保留中の予約操作を拒否する必要があるか確認しました。R1のWaitlistedも含めると、二つの確定要求だけで複数の公開操作を横断して状態数×操作数の組み合わせを点検しています。一箇所でも条件を漏らすと、R1・R2の受入条件を満たせません。
...
1つ目は、修正漏れが受入条件の不成立に直結することです。R2では、`pay()`で保留中からの支払いを許可するだけでなく、`reserve()`・`cancel()`・`expire()`でもHeldの扱いを確認しました。一つの確定要求に対してクラス内の複数操作を点検する必要があります。

2つ目は、R1・R2の振る舞いを追うために複数メソッドの`if`と`else if`を行き来する必要があることです。「今この状態なら、この操作は何をするか」を一つの場所で読めず、今回修正した全操作をまとめて確認しなければなりません。

フェーズ3で「変更が辛い」という事実が確認できました。次のフェーズ4では、なぜ辛いのかを構造的に言語化します。

---
> **📌 問題（確定）**
> R1のキャンセル待ちとR2の一時保留を追加しただけで、`reserve()`・`pay()`・`cancel()`・`expire()`など複数のメソッドへ条件分岐を追加する必要がある。状態ごとの許可操作と遷移先が`TicketReservation`全体へ散らばり、一つの確定要求に対する修正・確認範囲が広い。
---
...
接続点の両側をコードで見ると、公開操作の入口が `status` を読み、状態固有の判断をその場で実行しています。さらに席を解放した後は、同じクラス内の待ち行列処理へ接続しています。
...
P1では、呼び出し元に当たる公開操作と、接続先に当たる状態固有の判定が同じメソッドへ重なっています。P2では、席解放処理が待ち行列の先頭取得と自動昇格へ接続しています。したがって、呼び出し行だけでなく、入力された操作、参照する現在状態、返る次状態、続いて起きる座席・待ち行列の副作用までが接続点の範囲です。

| 課題ID・接続点 | 接続するデータ | 変わる側 | 守る側 |
|---|---|---|---|
| P1：状態ごとの振る舞い ↔ 公開操作・イベント入口 | `status` と操作／タイマー／空席／決済失敗イベント → 次状態と操作結果 | 「どの状態で各操作・イベントが何をするか」という状態固有の判断 | 公開操作とイベントの入口、`EventDatabase` の座席増減、履歴などの副作用の位置 |
| P2：待ち行列 ↔ `cancel()`/`expire()` の解放処理 | 待機者（`TicketReservation*`）の追加・先頭取り出し | 待ち行列の方針（先着順・優先度・定員上限）と探索・削除の実装 | 「席が空いた直後に先頭1件を自動昇格させる」という業務契約 |

### 何を変え、何を守るか

- **変わるもの**：状態ごとの振る舞いロジック（どの状態でreserve/pay/cancel/expire/promoteBySystem/paymentFailedが何をするか）。新しい状態やイベントが追加されるたびに分岐が増える。
...
---
> **📌 システム全体の課題（確定）**
> `reserve()`・`pay()`・`cancel()`・`expire()` などの入力入口と状態ごとの振る舞いの間の接続点では、現在状態（`status`）と操作・イベントの種類が流れ、次状態と操作結果が返っている。変わるのは「どの状態のときに何をするか」という状態固有の判断であり、公開操作の入口・座席数の更新・履歴などの副作用は安定させたい。現在はその判断が文字列分岐として各操作メソッドへ埋め込まれており、状態追加のたびに全操作を横断修正している。
> あわせて、待ち行列との接続点では待機者の追加・先頭取り出しが流れている。変わるのは待ち行列の方針と実装であり、「席が空いた直後に先頭1件を自動昇格させる」業務契約は守りたい。
---

**着目する共通点：** 現状コードはすでに共通点を持っています。どの操作（`reserve`／`pay`／`cancel`／`expire`）も、同じ `TicketReservation` の現在状態（`status`）を見て次を決めるという同じ形にそろっています（構造の共通点）。現場では、状態の扱いがばらばらに散らばっていることも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、公開操作の入口を守ったまま、変わる「状態ごとの振る舞い」だけに絞って対策を考えられます。それをどこへどう出して接続するかは、フェーズ6でこの共通点と課題を組み合わせて決めます。
...
P1とP2は別々に採用する対策ではありません。P1で状態固有の判断を分離しても、席解放後の昇格がP2の待ち行列へ自動接続されなければ、変更要求を満たすシステムにはならないからです。

判断は次の順序で導きます。P1の変わる側は「現在状態ごとの判断」なので、公開操作の入口から外し、状態ごとのクラスへ置きます。P2の変わる側は「待機者の探索・削除・先頭選択」なので、状態クラスへ重ねず `ReservationWaitlist` へ置きます。最後に、守りたい「席が空いた直後の自動昇格」を切らないよう、席を解放する状態処理から待ち行列へ接続します。
...
統合表で特定した箇所だけを振り返ります。P1は公開操作ごとの `status` 文字列分岐、P2は状態処理に絡む待ち行列操作と昇格処理です。課題に関係しないコードは省略し、フェーズ3で明記した維持条件（在庫は `EventDatabase`、待ち行列は外から渡す共有ストア）をそのまま引き継ぎます。フェーズ3の変更試行コードから、該当する `if` ブロックをそのままの形で抜き出します。
...
public:
    // 予約する（Available のときだけ予約済みへ）
    void reserve() {
        if (status == "Available") {           // P1: 状態ごとの分岐
            status = "Reserved";
        } else {
            handleReserveError();
        }
    ...
    void cancel() {
        if (status == "Reserved") {
            status = "Available";
            promoteNextWaitlisted();           // P2: 待ち行列操作が状態処理に絡む
        } else {
            handleCancelError();
        }
    }
...
|---|---|---|---|
| F1：状態遷移のルール（アクションの可否）の変更 | 可否と次状態を現在状態のIReservationState実装へ置く | 影響するStateクラスの対象操作 | 他状態と受付入口への波及を抑えられる。将来ルールは完成コードへ追加しない |
| F2：状態の種類（特別優待予約などの追加） | 状態ごとの振る舞いを共通契約で分ける | 新しいStateクラスと、その状態へ入る遷移 | 局所化できる。未確定状態は完成コードへ追加しない |
| F3：状態を動かすイベントの追加 | イベントの起点と状態判断を分け、システム起点の処理もTicketReservationへ接続する | 新イベント入口、IReservationStateの操作、必要なスケジューラ | 接続点は予測できる。未確定イベントは完成コードへ追加しない |

F1〜F3により、State化が現在の要求だけの分岐整理ではなく、状態とイベントの将来変更をどこへ閉じるかという判断になっていることを確認できます。
...
| 状態処理に絡む待ち行列操作と昇格処理（P2） | `ReservationWaitlist` の中だけ修正 | キュー方針を専用クラスへ閉じた |
| 3-2には状態部品の契約がなかった | `NewState` を1クラス追加する | 状態の変更先を新しく作った |

### 7-4：変更シナリオ表

今回の確定要求R1・R2について、フェーズ1の構造で必要だった修正と完成構造の結果を整理します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：満席時にキャンセル待ちへ登録し、空席発生時に先頭を自動昇格する | `TicketReservation`の予約・取消へ待ち行列と昇格条件を追加 | `WaitlistedState`と`ReservationWaitlist`へ責任を分け、取消直後に49/50から先頭を自動昇格して50/50へ戻ることを確認 |
| R2：予約済みを24時間の一時保留へ移し、期限切れで席を解放する | `TicketReservation`の複数操作へ一時保留の条件分岐を追加 | `HeldState`へ保留中の振る舞いを置き、期限切れで空席へ戻した後の自動昇格まで確認 |

R1・R2の判断を状態クラスへ分けた代わりに、状態ごとのクラスと遷移の組み立てを管理するコストを引き受けます。
