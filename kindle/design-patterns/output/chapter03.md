...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 空席を確認して予約・取消する | 要求ID1、要求ID2、要求ID4 |
| 機能ID2 | 予約を支払済みにする | 要求ID3 |
| 機能ID3 | 許可されない操作で状態と予約数を守る | 要求ID5 |
...
|---|---|---|---|
| 機能ID1 | 変更<br/>根拠: 変更ID1 | 空席を確認して予約・取消し、空席発生時は待機者を自動昇格する | 要求ID1、要求ID2、要求ID4 |
| 機能ID2 | 変更<br/>根拠: 変更ID2 | 予約済みまたは一時保留を支払済みにする | 要求ID3 |
| 機能ID3 | 継続<br/>根拠: — | 許可されない操作で状態と予約数を守る | 要求ID5 |
| 機能ID4 | 追加<br/>根拠: 変更ID1 | 満席時にキャンセル待ちへ登録する | 要求ID6 |
| 機能ID5 | 追加<br/>根拠: 変更ID2 | 予約を一時保留し、期限切れで席を解放する | 要求ID7 |
...
フェーズ5の一表から、状態ごとの振る舞いと待ち行列方針を独立して変更でき、キャンセルまたは期限切れで席を解放した直後の自動昇格が切れないシステム構造を直接決めます。
...
|---|---|---|---|
| 機能ID1 | 変更 | 要求ID1、要求ID2、要求ID4 | 既存予約・取消を回帰し、取消直後の49/50→50/50自動昇格を確認<br/>**判定:** 合格 |
| 機能ID2 | 変更 | 要求ID3 | 予約済み・一時保留の支払成功時だけPaid<br/>**判定:** 合格 |
| 機能ID3 | 継続 | 要求ID5 | エラー前後の状態・予約数不変を回帰確認<br/>**判定:** 合格 |
| 機能ID4 | 追加 | 要求ID6 | 待機順の保存と先頭昇格を受入確認<br/>**判定:** 合格 |
| 機能ID5 | 追加 | 要求ID7 | 保留中の席確保と期限切れ後の解放・昇格を受入確認<br/>**判定:** 合格 |
