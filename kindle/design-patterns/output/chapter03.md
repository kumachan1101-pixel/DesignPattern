...
`TicketReservation` の中に `if` 文が一切なくなった。新しい状態 `WaitlistedState` が追加されたとき、`IReservationState` を実装した新クラスを作るだけで組み込める。`TicketReservation`の条件分岐を変更する必要はない（組み立て箇所への登録は必要）。
