...
この表は「クラスがどの知識を持っているか」を可視化するものです。実装コードを読みながら、現在状態の保持、操作の受付、状態ごとの許可・拒否、遷移先の決定を分けて書き出します。同じ企画担当が決めるルールでも、状態ごとにまとまって変わる知識が操作メソッドへ分散しているなら、別の境界を検討する手掛かりになります。
...
責任チェック表でクラスの責任が整理できました。次に、コードの各行が「誰の判断で、何をきっかけに変わる知識か」を確認し、混在している責任をさらに細かく特定します。予約フロー管理の開発チームとは別の担当者が独立した時期に変更する知識は、責任を分ける有力な手掛かりです。ただし、担当者が違うだけで自動的に責任外とは決めず、変更頻度・共変更範囲・クラスの役割も合わせて判断します。

`TicketReservation` の各メソッドを見ると：

| **コードの行** | **持っている知識** | **誰の判断で変わるか** | **変更時に一緒に確認する単位** |
|---|---|---|---|
| `if (status == "Available")` | Availableで予約を許可するルール | 企画担当 | Availableの全操作 |
| `status = "Reserved";` | Availableからの遷移先 | 企画担当 | Availableの全操作 |
| `std::cout << "予約完了しました\n";` | 予約成功時の表示 | 企画・UX担当 | 予約成功時の振る舞い |
| `if (status == "Reserved")` | Reservedで支払いを許可するルール | 企画担当 | Reservedの全操作 |
| `status = "Paid";` | Reservedで支払った後の遷移先 | 企画担当 | Reservedの全操作 |
| `if (status == "Reserved")` | Reservedでキャンセルを許可するルール | 企画担当 | Reservedの全操作 |
| `status = "Available";` | Reservedでキャンセルした後の遷移先 | 企画担当 | Reservedの全操作 |
