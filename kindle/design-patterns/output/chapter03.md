...
**着目する共通点：** 現状コードはすでに共通点を持っています。どの操作（`reserve`／`pay`／`cancel`／`expire`）も、同じ `TicketReservation` の現在状態（`status`）を見て次を決めるという同じ形にそろっています（構造の共通点）。現場では、状態の扱いがばらばらに散らばっていることも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、公開操作の入口を変えず、変わる「状態ごとの振る舞い」だけを状態オブジェクトへ外に出せば済みます。フェーズ6では、この共通点と課題を組み合わせて完成構造を決めます。
