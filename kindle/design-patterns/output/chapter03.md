...
|---|---|---|---|
| 状態ごとの振る舞い ↔ `reserve()`/`pay()`/`cancel()` の骨格 | `status`（string値）と操作の種類 → 次状態と操作結果 | 「どの状態でどの操作が何をするか」という状態固有の判断（新しい状態が追加されると増える） | 公開操作の入口、`EventDatabase` の座席増減、履歴などの副作用の位置 |
| 状態ごとのイベント処理 ↔ `expire()`/`promoteBySystem()`/決済失敗処理の骨格 | `status`（string値）と入力イベント → 状態維持/状態遷移 | タイマー、空席発生、決済失敗など、利用者操作以外の入力への反応 | イベントを受け取る入口と、イベント発生の業務契約（席解放直後の自動昇格など） |
| 待ち行列 ↔ `cancel()`/`expire()` の解放処理 | 待機者（`TicketReservation*`）の追加・先頭取り出し | 待ち行列の方針（先着順・優先度・定員上限）と探索・削除の実装 | 「席が空いた直後に先頭1件を自動昇格させる」という業務契約 |
...
課題IDは、フェーズ5で特定した「解くべき接続点」に1つずつ対応します。**P1は「状態ごとの振る舞い ↔ 公開操作の骨格」という接続点**、**P2は「待ち行列 ↔ キャンセル／期限切れ処理」という接続点**です。受け入れ条件は「解けたと言える影響範囲」を役割の言葉で定めたもので、どんな構造で実現するかはフェーズ6で決めます。

| 課題ID | 現在の変更影響 | 変えたくない範囲 | 受け入れ条件 |
|---|---|---|---|
| P1 | 状態を追加すると `reserve()`・`pay()`・`cancel()`・`hold()`・`expire()` の文字列分岐へ横断的に波及する | 公開操作、座席数の更新、履歴などの既存副作用 | 状態を追加したとき、修正が「その状態の振る舞いの記述と、そこへ遷移する元の記述」に収まり、公開操作の入口と既存状態が修正・再テスト対象に入らない |
| P2 | 待ち行列方針を変えると `cancel()`・`expire()`・待ち行列操作を同時に修正する | 席解放直後の自動昇格、先着順、定員上限 | 待ち行列の方針を変えたとき、修正が「待ち行列を担う場所」に収まり、公開操作と状態の振る舞いが修正・再テスト対象に入らない |

状態の振る舞いと待ち行列方針は別の理由で変わるため、接続点をP1、P2の連番で分けます。フェーズ6でも、痛みのグラフにある同じ2枝を起点として追います。
...
| P2 | 待ち行列方針を変えると `cancel()`・`expire()`・待ち行列操作を同時に修正する | 待機者の探索・削除・先頭選択を `ReservationWaitlist` へ移す | 席解放直後の自動昇格、先着順、定員上限 |

#### 受け入れ条件（フェーズ5から引き継ぎ）

フェーズ5で定めた受け入れ条件を、そのまま候補の評価軸として使います。フェーズ6で条件を作り直すことはしません。

- P1：状態を追加したとき、修正が「その状態の振る舞いの記述と、そこへ遷移する元の記述」に収まり、公開操作の入口と既存状態が修正・再テスト対象に入らない。
- P2：待ち行列の方針を変えたとき、修正が「待ち行列を担う場所」に収まり、公開操作と状態の振る舞いが修正・再テスト対象に入らない。
- 共通の制約：席が空いた直後に先頭待機者を1件だけ自動昇格させる業務契約と、座席数更新・履歴などの副作用の位置を維持する。
...
| 決定 | 問い | この章での答え |
|---|---|---|
| 分離 | 接続点をどう切るか | 状態固有の判断（P1）を公開操作の骨格から、待ち行列の探索・削除・先頭選択（P2）を状態処理から切り出す。境界の契約は「現在状態へ操作・イベントを委譲すると次状態が決まる」こと |
| 配置 | 切り出した知識をどこへ置くか | 状態ごとの振る舞いは状態1つにつき1クラスへ、待ち行列の方針は専用クラスへ置く。守る側には公開入口と座席・履歴の副作用が残る |
| 組み立て | どう繋ぎ直すか | 遷移元の状態が次の状態を選び、待ち行列は組み立て側が所有して注入する。席解放時の自動昇格は状態処理から待ち行列を1点で呼んで維持する |

#### システム全体の最終構造を決める

この章は変化軸が2本あり、それぞれ別の理由で変わるため、最終構造は**2つの構造を組み合わせて**決まります。読者が思いつきそうな「1つの構造で両方を解く」案——各状態クラスに待ち行列の操作まで持たせる形——では、待ち行列の方針変更のたびに複数の状態クラスを開くことになり、P2の受け入れ条件を満たしません。状態ごとの振る舞いは状態分離構造で各状態クラスへ、待ち行列方針は待ち行列分離構造で `ReservationWaitlist` へ移し、キャンセル時に前者が後者を呼ぶ1点で接続します。
