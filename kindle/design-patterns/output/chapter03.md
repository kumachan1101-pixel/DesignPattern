...
現在の構成では、`TicketReservation` クラスが `status` 変数で状態を保持し、各操作メソッドの中で状態に応じた処理を `if` 分岐で管理しています。
...
* 実装コスト：高（設計の複雑度が高い）
* ※ 同時予約の競合制御：状態遷移の判定が `IStateHandler` 経由の実装クラスに封じ込められているため、楽観的ロックやバージョン管理による整合性保証を状態クラス単位で独立して実装・差し替えできる。
