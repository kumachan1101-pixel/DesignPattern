...
**分類が1つ増えるたびに、関係するすべての処理で条件分岐を書き直す必要が生じる。こういう問題は、「変わる理由が違うもの」が同じ場所に混在しているシステムで起きている。**
...
「——」は操作を受け付けず、エラーメッセージを出力して終了することを表します。なぜ「——」を設けるのかというと、「現在の状態にとって意味をなさない操作」をシステムが受け入れてしまうと、データの整合性が崩れるからです。たとえば、まだ予約していない座席に対して突然「支払う」を実行できてしまうと、どの予約に対する支払いなのかがシステム上で確定できません。そのため「今の状態では受け付けられない」を明示的にエラーとして返すことが、業務上のルールとして正しい設計です。これはチケット予約に限らず、EC注文・銀行振込・申請ワークフローなど、状態を持つシステム全般で広く採用されているルールです。
...
- **Available（空席）で「支払う」「キャンセルする」が——の理由**：予約が存在しない座席に対して支払いを受け付けると「誰の予約か」が確定しません。キャンセルについても、そもそも予約がなければキャンセルする対象がなく、操作自体が無意味です。「空席のままキャンセルを求める客」は現実には存在しないため、このルールは自然です。
- **Reserved（予約済み）で「予約する」が——の理由**：同じ座席を二重予約することは、座席が物理的に1つしかない以上、業務上あり得ません。映画館に限らず、ホテルや航空券など席を一意に確保するシステムでは共通の制約です。
- **Paid（支払済み）でのすべての操作が——の理由**：支払いが完了した座席は「確定済み」扱いになります。この状態からのキャンセルを認めてしまうと、返金処理・座席の再販売・会計記録の修正など、複数の業務プロセスが連動して動く必要が出てきます。「一度支払ったらキャンセル不可」という仕様は、映画館に限らず即時課金型のサービス（ライブチケット・格安航空券など）で一般的です。キャンセルポリシーを設ける場合は、「キャンセル済み（Cancelled）」という別の状態を追加して対応するのが通常の設計です。
...
| クラス名 | 役割 | 担当する仕様 |
|---|---|---|
| EventDatabase | イベント情報の保持と検索 | イベントIDの存在確認・定員空き確認 |
...
責任チェック表でクラスの責任が整理できました。次に、コードの各行が「どの業務機能に属する知識か」を確認することで、混在している責任をさらに細かく特定します。判断基準は、「このクラスが担う業務機能（ここでは処理の骨格）とは別の業務機能が変更を決定するかどうか」です。別の業務機能が決定するなら、それは「責任外（❌）」と判断します。

`TicketReservation` の各メソッドを見ると：

| **コードの行** | **持っている知識** | **どの業務機能か** | **責任内か** |
|---|---|---|---|
...
ヒアリングで「状態遷移ルールの変更」と「状態の種類の追加」が予告されました。この変化が来たとき、仕様がどう変わるかを整理しておきます。★０章と構成を合わせて。フェーズ２のアウトプットは、変動不変テーブルの確定ですよね？
...
★変えた箇所、色を変えるとかできませんか。もしくは、どこが変わったのかわかるようにして
