...
| Reserved（予約済み） | 予約する | エラー（既に予約済み） | 予約済み座席への二重予約は不可 |
| Available（空席） | キャンセルする | エラー（キャンセル不要） | 空席状態ではキャンセル操作は無効 |
...
フェーズ1の仕様表を振り返ります。現在の予約システムには「Available（空席）」「Reserved（予約済み）」「Paid（支払い済み）」という3つの状態があります。この状態の定義に関して、変化が予想される仕様があります。

- **状態の種類**：映画館のビジネスニーズが変わると、新しい状態（キャンセル待ち、優先予約など）が追加される可能性があります
- **状態遷移のルール**（どの状態からどの状態へ移れるか）：業務方針によって遷移条件が変わることがあります

一方、予約・支払い・キャンセルという操作フロー（どの操作があるか）は、映画館の基本サービスとして安定している部分です。

**仮説：状態の種類と状態遷移のルールは、今後も追加・変更が続く可能性がある。**
...
ヒアリングで「特別優待予約」などの状態追加が予告されました。この変更が来たとき、状態遷移がどう変わるかを整理しておきます。
...
| Reserved（予約済み） | あり | 変更なし |
| Paid（支払済み） | あり | 変更なし |
| Waitlisted（キャンセル待ち） | 今回追加 | 変更なし |
| Held（一時保留） | 今回追加 | 変更なし |
| VIP（特別優待予約） | —（なし） | 追加予定（Reserved → VIP → Paid） |

VIP 状態が追加されると `pay()` に VIP 対応の分岐、`cancel()` に VIP からのキャンセル対応が必要になります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された「特別優待予約（VIP）」状態の追加も視野に入れると、`pay()` にもう一つ `else if (status == "VIP")` の分岐、`cancel()` にも VIP からのキャンセル対応が必要になることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、状態が増えるたびに全メソッドの条件分岐がさらに積み重なる構造は変わりません。
...
分離の境界は決まりました。次は「状態ごとのクラスをまとめるか、個別に分けるか」という粒度の問題です。

ヒアリングで把握した状態と、それぞれが変わる理由を確認します。

| 状態 | 変わる理由 |
|---|---|
| Available / Reserved / Paid | 映画館の基本予約フロー（企画担当） |
| Waitlisted（キャンセル待ち） | キャンセル待ちの管理ルール（同じ企画担当） |
| Held（一時保留） | 決済保留のルール（同じ企画担当） |
| VIP（特別優待予約） | アンケートキャンペーン企画（同じ企画担当） |

すべての状態は同じ担当（企画担当）が管轄していますが、変わり方は独立しています。VIP のルールが変わっても Held の保留ルールには影響しません。Waitlisted の昇格条件が変わっても Reserved の支払い処理は変わりません。各状態が**独立して変わる**ため、状態ごとに別クラスへ分離します。どの状態も `reserve()`・`pay()`・`cancel()` という同じ操作を受け取るため、同じ `IReservationState` インターフェースで統一します。

→ **状態ごとに別クラスへ分離し、同じ `IReservationState` インターフェースで束ねる**
...
| 🟣 フェーズ2：仮説立案 | フェーズ1の仕様表から「状態の種類と状態遷移ルールは今後も変更が続く可能性がある」という仮説を立て、企画担当者へのヒアリングで裏付けました。 |
...
| **得られること** | **この章のどこで示したか** |
|---|---|
| 1. 変動箇所の識別 | フェーズ1の仕様表から「状態の種類と遷移ルールは今後も変更が続く可能性がある」という仮説を立て、フェーズ2のヒアリングで「状態の種類」と「遷移ルール」が頻繁に変わることを確認したこと。 |
...
チケット予約というドメインと Stateパターンの関係を一言で言うなら、状態と振る舞いを同じクラスに置く限り、状態が増えるたびにすべての条件分岐を開かなければならない、ということです。「仮予約」「確定」「キャンセル」という状態ごとに異なる振る舞いが `TicketReservation` の中に同居していた。その構造が条件分岐の爆発を生んでいた——そこまで分析できれば、「状態の追加を新しいクラスの作成だけで完結させる」という方向性は自然に見えてきます。
