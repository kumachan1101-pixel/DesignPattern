...
ここで見るのは所在だけです。責任の多さや構造の善し悪しは、変更を当てて痛みを観測してから判断します。上の表は、変わりそうな仕様が `TicketReservation` のどのメソッドのどの行にあるかという所在の見立てです。状態遷移の仕様を1つのクラスにまとめている今の配置が、変更要求を当てたときに**実際に痛みになるか**はフェーズ3で確かめ、なぜ辛いのかはフェーズ4で原因として言語化します。2-1は、その検証対象に当たりをつける段階です。
...
フェーズ4で、`TicketReservation` が「状態ごとの操作可否と遷移」と「待ち行列の探索・削除・先頭選択」という**別々の理由で変わる2つの判断**を、同じ場所へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つの予約進行経路へ再結合することです。この段階では、問題から導いた「状態分離構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。
...
向きと掲載クラスは1-3から変えていません。同じ図に注記と色だけを加え、`TicketReservation` のどの責任を残し、どの責任を移すかに着目します。**この図が出発点で、以降の各段でここへ1つずつ足していきます。**
...
| | 内容 | どこで決まったか |
|---|---|---|
| **決まっている（目指す形）** | 状態ごとの振る舞いを共通契約の裏へ揃えて現在状態へ委譲する／待機順の管理を専用の係へ閉じる | フェーズ4・5（独立した2つの変化軸） |
...
- **分離方法**：予約進行に何を残し、何を契約の裏へ外すか。課題ID1と課題ID2で別々の境界になります。
- **配置場所**：外した判断を、どんな単位のクラスへ置くか。状態ごとか、まとめて1つか。
- **組み立て方法**：実体を誰が生成・所有し、公開操作を持つクラスへどう渡すか。
...
**どう解決するか（方針）：** 状態ごとの振る舞いを共通契約の裏へ揃えて現在状態へ委譲する「状態分離構造」とし、待機順の探索・削除・先頭選択を専用の係へ閉じます。

**課題どうしの関係を、2つの基準で確かめます。**

| 基準 | 判定 |
|---|---|
| **A：変化軸** | 別々の理由で変わる。可否の規則が変われば課題ID1側だけ、待機順の方針（先着順から抽選へ、など）が変われば課題ID2側だけ |
| **B：条件付け** | **該当しない。** 課題ID2の完了条件「取消・期限切れから自動昇格まで一つのユースケースで完了する」は、席を解放した処理が待ち行列を呼ぶことを求めているが、**呼ぶ側がどんな形かは指定していない** |

**基準Bに該当しないので、どちらの契約を先に決めても同じ結論になります。** ここでは5-3の並び順に従って課題ID1から扱います。**ただし2つはつながります。** 席を解放する状態処理が待ち行列の先頭を昇格させる——この1本の経路だけが2つの軸の接点です。**接点が1本だけであることを、6段を通して確かめます。**
...
**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、状態ごとの振る舞いと待機順の管理を、予約の公開入口とは別の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
**まず、境界をクラス契約にする必要があるかを確かめます。** フェーズ5「課題の確定（5-3）」が確定したのは「この接続点を解く」ことであって、「クラスを増やす」ことではありません。次の2つがどちらも成り立つなら、クラス契約を作らず、判定を関数へ1つ切り出す形で止めます。

1. 変わる側の判断が、コード上1か所にしか現れない
2. その種類が増える見込みが、2-4のリスクIDにも5-2の評価にも挙がっていない

**課題ID1はどちらも成り立ちません。** 状態ごとの可否は `reserve`・`hold`・`pay`・`cancel`・`expire` の5つの関数それぞれに `if (status == ...)` として現れます。そして2-4のリスクID1（可否の変更）、リスクID2（状態の種類の追加）、リスクID3（入力イベントの追加）が、3方向とも「増える」と言っています。**現に変更要求で、保留中とキャンセル待ちの2状態が増えました。**
...
**この段の守る範囲の照合：** 守る範囲の4つのどれにも触っていません。契約と待ち行列クラスを置いただけで、既存のコードは1行も書き換えていません。
...
呼べるのは、状態や待機順が増えても変わらない側だけです。出て行った側が出て行った側を呼べば、具体が増えるたびに両方を触ることになり、分けた意味が消えます。
...
**この章は上の型です。** 1つの予約が、受付可能から予約済みへ、さらに支払済みへと進みます。**同じインスタンスが、操作のたびに違う状態の実体を使い分けなければなりません。** だから契約は外の型（`IReservationState`）になります。
...
残った側は、公開操作を持つクラス——`TicketReservation` です。**名前は変えません。** 1-3で書いた役割は「予約の進行と座席・履歴の更新」で、この段の後もそれは変わりません。出て行くのは状態ごとの判断であって、予約であること自体ではありません。

**掲載箇所：`TicketReservation`（クラス宣言・この段で分かるところまで）**
...
**1つ目は、`AvailableState` のような具体の名前をここに書くわけにいきません。** 書けば課題ID1で分けた意味が消えます。**2つ目は、席数・履歴・待ち行列という3つの相手が要ります。** どちらも**4で決めます。**
...
**この段で図に描けるのは、骨格から2つの契約への依存だけです。** 実体をどこから得るかがまだ決まっていないので、その線は引けません。
...
**`IReservationState` と `ReservationWaitlist` の間に矢印がありません。** 状態と待機順は互いを知らない、というのが5-2で確認した独立性で、図にもそのまま出ています。**代わりに、契約から骨格へ戻る点線があります。** 1で「操作対象を引数で渡す」と決めた結果です。

**この段の守る範囲の照合：** 4つのうち1つに触りました。**公開入口**です。`reserve()`・`pay()`・`cancel()` などの名前も引数も変えていません。中身が1行になっただけで、利用側から見た形は3-1のままです。座席数更新・履歴・昇格の契約は、まだ穴のままなので触っていません。
...
契約と、それを呼ぶ骨格が決まりました。1では契約の裏を1つずつ覗いて、渡すもの・書き戻すものを確かめました。**ここで残り全部を埋めます。**

**課題ID1（状態軸）。** 1で見た `ReservedState` の疑問が、ここで解けます。**`if (status == ...)` を書かないのは、クラスであること自体が状態を表しているからです。** 契約の裏に置くのは「この状態で何が許されるか」「許されたとき次はどこで、何が起きるか」の2つだけです。
...
**課題ID2（待ち行列軸）。こちらは1で見た `ReservationWaitlist` がそのまま完成形です。** 待機順を扱う以外に書くことがないので、増える段がありません。**実装も1つだけです。** 待機順の方針は今1つで、抽選や優先度つきに替える予定は2-4に挙がっていません。**替えられる形にしておく必要が無いので、契約を作らず、クラス1つで止めます。**
...
**この段の守る範囲の照合：** **座席数更新**と**履歴**に触りました。どちらも呼ぶ場所が公開操作の中から状態クラスの中へ移っただけで、予約で1つ減らし取消と期限切れで1つ戻すことも、状態が変わった操作を記録することも変わっていません。**席解放直後に1件を昇格する契約**にも触りましたが、`ReservedState::cancel()` と `HeldState::cancel()`・`HeldState::expire()` の3か所すべてで `promoteNextWaitlisted()` を呼んでいます。3-1で昇格していた遷移と同じ3つです。
...
**2で穴が2つ空きました。** 骨格の `state` に最初に何が入るのか、そして状態が呼び返す先（席数・履歴・待ち行列）を骨格がどう持つのかです。

**まず状態の実体から決めます。** 5つの状態クラスは**メンバー変数を1つも持ちません**。3で確認したとおり、どれも「渡された予約を動かす」だけです。**同じ状態の実体を、何個の予約が共有しても壊れません。**
...
**宣言を5つ先に並べるのは、状態どうしが互いの遷移先を呼ぶからです。** 1で「状態どうしが互いの名前を知る」と書いたコストが、ここに形として出ます。
...
**次に、状態が呼び返す先を決めます。** 3で `reservation->cancelSeat()`・`record()`・`promoteNextWaitlisted()` と書きました。これらを実行するには、骨格が在庫・履歴・待ち行列を持っている必要があります。
...
**実線が所有、点線が生成、それ以外が借用です。** 5つの状態クラスはこの図に描いていませんが、3から変わっていません。

**この段の守る範囲の照合：** 4つのどれにも触っていません。誰が何を持つかを決めただけで、確認も更新も記録も昇格も動きは変えていません。

**この段が終わった時点で、実体はすべて存在しています。** 5つの状態、待ち行列、在庫、履歴、それらを持つ組み立て役。**まだ起きていないのは1つだけです。骨格の `IReservationState*` と書かれた変数へ、具体が入ることです。それが5です。**

---

**5. 実装が骨格へ入る 【注入】**

**2で書いた骨格には、`state->cancel(this)` としか書いてありません。** `ReservedState` とも `HeldState` とも書いていません。それでも予約済みの予約を取り消せば予約済み用の処理が動きます。**この段で見るのは、いつ・何によって、その実装が入るのかです。**
...
**掲載箇所：`ReservedState::cancel(TicketReservation*)`** ―― 3で書いた1行目

```cpp
        reservation->setState(availableState());   // ←ここで次の実装が入る
```

**この1行が2回目の注入です。** 骨格は `IReservationState*` としか書いていないのに、実行時には `AvailableState` が入ります。**入れているのは遷移元の状態であって、骨格ではありません。骨格は、何が入ったかを最後まで知りません。**

**実装が骨格へ入る形は、この章の形だけではありません。** 骨格が契約としか書いていない場所へ具体が入る、という点はどの構造でも同じですが、入る瞬間は3つに分かれます。

| 形 | 実装が決まる決め手 | 入る瞬間 | この本での例 |
|---|---|---|---|
| **呼び出しごとに引き当てる** | **保存された状態値、入力の属性** | **引き当て関数が契約への参照を返す行** | **第3章（この章）**、第9章、第12章 |
| 生成した実体を渡して保持する | 誰を渡したか（組み立て時に1回） | コンストラクタ引数、`attach()` などの登録 | 第2章、第6章、第7章、第8章 |
...
**課題ID2には決め手がありません。** 3で確認したとおり実装が1つしかないので、選ぶ判断そのものが存在しません。**「決め手が要る」構造と「要らない」構造が、同じ章に並んでいます。**

**骨格が状態を借りる方法（ポインタで共有実体を指す）は、4で決めた話です。** 値で持つと遷移のたびにオブジェクトを作り直すことになりますが、**注入はどちらの持ち方でも起きます。**
...
**この段の守る範囲の照合：** **公開入口**を保っていることを、いま確認しました。`reserve()` の引数が1つ減りましたが、これは呼び出し側が持つべきでない在庫の知識を返しただけで、操作の形は変わっていません。6段すべてを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、各段で照合済みです。**


#### システム全体の最終構造を決める

6段で足した図を重ねると、システム全体の最終構造になります。`TicketReservation` が公開入口と副作用の実行を持ち、5つの状態クラスが可否と遷移を持ち、`ReservationWaitlist` が待機順を持ちます。
...
|---|---|---|---|
| 課題ID1 | 共通契約 `IReservationState` を新設し、公開操作を現在状態へ委譲する | 5つの状態クラスが可否・遷移先・副作用を実装し、`TicketReservation` は状態を判定しない | 6段の【契約】〜【利用開始】 |
| 課題ID2 | 待機順の管理を `ReservationWaitlist` へ移す | 探索・削除・先頭選択をイベントID別の行列へ閉じ、骨格は1件昇格だけを行う | 6段の【契約】〜【利用開始】 |
...
|---|---|---|---|
| 【契約】（軸ごと） | 5つの関数に散った `if (status == ...)` → `IReservationState` の7操作／`promoteNextWaitlisted()` の探索3行 → `ReservationWaitlist` の2操作 | 分けるかを検算し、5-3の候補（状態軸4つ・待ち行列軸3つ）の形を決める。状態軸は操作対象が下るだけ、待ち行列軸は2つが流れる | この契約を誰が呼ぶか＝【安定骨格】 |
| 【安定骨格】（軸ごと） | 状態分岐を持つ公開操作 → 現在状態へ1行で委譲する形／4行の待ち行列操作 → 1件昇格の2行 | 残った側の手順を、具体が増えても変わらない形で固定する。2軸の接点が2行だけであることを確認する | 契約の裏に何を置くか＝【具体】 |
| 【具体】（軸ごと） | 分岐に埋もれた状態別の可否と副作用 → `AvailableState` ほか5クラス／`ReservationWaitlist` 1クラス | 契約のメソッドだけを埋める。待ち行列は実装が1つなので契約を作らない | 実体を誰が作るか＝【生成】 |
| 【生成】（共通） | 予約が全分岐と行列を内包 → 状態は関数ローカルの共有実体、行列・在庫・履歴は組み立て役の値メンバ | メンバーを持たない状態を全予約で共有し、行列は共有が必要なので借用にする | 実装がどこで骨格へ入るか＝【注入】 |
| 【注入】（共通） | 利用側が状態文字列を見て呼び分け → 生成時に初期状態、遷移時に `setState()` | 実装が2回に分けて入る。2回目は遷移元の状態が次を名指しする | すべて決まった結果としての呼び方＝【利用開始】 |
| 【利用開始】（共通） | `r.reserve(true);` → `r.reserve();` | 組み立てた実体の公開操作を呼ぶ。昇格を呼ぶ行は無い | ここから実行が始まり、【安定骨格】へ入る |

**【利用開始】が最後にあるのは、対策後の呼び方が設計の結果だからです。** `reserve()` の引数が1つ減るのは、3で「空席の判定は在庫へ聞く」と決めた結果でした。昇格を呼ぶ行が要らないことも、3まで決めてから分かります。

#### 代表ケースの実行接続

6段は考えた順でした。**ここからは実行時に通る順で貫きます。** 満席のイベントで待機していた予約が、他の予約の取消によって自動昇格するまでを、1本の経路として追います。**2つの軸を1回の操作でまたぐ経路です。**

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 【生成】 | `BatchApplication`（`db`・`history`・`waitlist` はメンバー） | 在庫・履歴・待ち行列を1つずつ所有し、状態は `availableState()` などが共有実体を返す | 【注入】へ |
| 2. 【注入】 | `BatchApplication` の予約生成行 | `TicketReservation r(availableState(), &db, &history, &waitlist, ...)` で初期状態が入る | 【利用開始】へ |
| 3. 【利用開始】 | `BatchApplication` | 予約済みの予約に対して `r.cancel();` を呼ぶ | `TicketReservation::cancel()` |
| 4. 【安定骨格】状態軸 | `TicketReservation::cancel()` | `state->cancel(this)` の1行。状態を判定しない | `IReservationState::cancel()` |
| 5. 【契約】【具体】状態軸 | `ReservedState::cancel(TicketReservation*)` | `setState(availableState())`（＝【注入】2回目）→ 席を戻す → 履歴 → `promoteNextWaitlisted()` | `TicketReservation::promoteNextWaitlisted()` |
| 6. 【安定骨格】待ち行列軸 | `TicketReservation::promoteNextWaitlisted()` | `waitlist->popNext(eventId)` で1件取り出す | `ReservationWaitlist::popNext()` |
| 7. 【契約】【具体】待ち行列軸 | `ReservationWaitlist::popNext(const string&)` | イベントIDの行列から先頭を1件返す | 戻り値を【安定骨格】が昇格へ回す |
| 8. 【契約】【具体】状態軸 | `WaitlistedState::promoteBySystem(TicketReservation*)` | `setState(reservedState())` → 席を1つ減らす → 履歴 | 完了 |

**5と6のあいだが、2つの軸の接点です。** 状態軸の処理が待ち行列軸を呼び、返ってきた予約に対してもう一度状態軸が動きます。**この1本の経路以外に、2つの軸が触れる場所はありません。** 5-2で確認した独立性が、実行経路でも確認できます。

**【注入】が2回現れることに注目してください。** 2で初期状態が入り、5で次の状態が入ります。**組み立てのときに1回ではありません。** 遷移が起きるたびに、次の実装が入ります。
