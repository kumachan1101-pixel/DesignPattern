...
- **出力：** 操作結果、変更後状態、予約数の前後値を返し、不正操作では状態も数値も変えない。
- **掲載コードでの代替：** 実システムの予約DBとイベントDBはメモリ上の登録表、時刻到達はスケジューラから届くイベントのスタブで表す。状態遷移、予約数の増減、キャンセル待ちの自動昇格は実際に行う。
...
**どう解決するか（方針）：** 状態を共通契約の裏へ出し、公開操作は現在状態へ委譲します（状態分離構造＝State）。①契約 →③具体 →④生成 →⑤保持・遷移 →⑥委譲 の順で組み立てます（②骨格は無し）。
