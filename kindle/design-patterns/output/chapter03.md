...
| Paid | reserve | — | 何もしない（操作無効） |

定義されていない操作（例：Available状態でのキャンセル）は、エラーとはならず静かに無視されます。
...
**手段A＋手段Bの併用**（初期状態をコンストラクタで受け取り、遷移時は `setState` で切り替えるのが最も自然なため）のコードを以下に示します。
...
**手段A**（案4の趣旨である「抽象×間接」を実現するには2層が必要なため）のコードを以下に示します。
