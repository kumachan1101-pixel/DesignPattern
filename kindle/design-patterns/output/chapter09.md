...
原因が2つあるので、第0章の「構造的対策（手札）の網羅的リスト」から2つの手札を選び、順に適用します。

- **特定した原因A（割引ルールという任意の振る舞いの内容が変わる）：** インターフェースによる抽象化（境界を引く）
- **特定した原因C（オブジェクトの状態とそれに伴う振る舞いが増減する）：** 状態のオブジェクト化

### 試み①：割引ルールを独立させる（インターフェースによる抽象化）
