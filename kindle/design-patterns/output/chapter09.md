...
Strategyパターンは第1章で解説しています。この章では、もう一方のパターンであるStateパターンを説明します。

### GoFの定義

> オブジェクトの内部状態が変わったとき、そのオブジェクトの振る舞いが変わるようにする。
> オブジェクトはクラスが変わったように見える。
...
| State（インターフェース） | `IOrderState` |
| ConcreteState | `CartState`、`ConfirmedState`、`PaidState`、`ShippedState` |

`OrderService` は `confirm()`・`pay()`・`ship()` を呼ばれると、現在の `IOrderState` にそのまま委譲します。「今どの状態か」を知っているのは各 `ConcreteState` 自身であり、`OrderService` は状態の種類を意識しません。

### StrategyパターンとStateパターンの違い

2つのパターンは構造がよく似ています。違いは「部品が切り替わる理由」にあります。

| | Strategyパターン | Stateパターン |
|---|---|---|
| 切り替えの主体 | 外部（呼び出し元が渡す） | 内部（状態オブジェクト自身が次の状態に遷移する） |
| 切り替えのタイミング | 任意（注入時や実行時に変更） | イベント発生時（`confirm`・`pay`・`ship` が呼ばれたとき） |
| 典型的な使い方 | アルゴリズムの差し替え | 状態遷移機械（ステートマシン） |

### 使う基準

- オブジェクトが複数の状態を持ち、状態ごとに振る舞いが変わる
- 状態が増えたとき、既存のメソッドに `if` を追加したくない
- 状態遷移の責任を状態オブジェクト自身に持たせたい

### 使わない基準

- 状態が2〜3種類で今後も増えない（シンプルな `if` で十分）
- 状態遷移が外部から明示的に指定される（Strategyパターンの方が適切）
