...
現状コードでは通知先の追加・変更のたびに `InventoryManager` を直接修正する必要がありました。改善後は `InventoryManager` に触れず、通知先のクラスだけを変えれば済みます——それがこの設計で手に入れたものです。諦めたものは、通知のたびにインターフェースを経由するというわずかな間接性と、通知分離構造の登録・解除を管理するクラス数の増加です。
