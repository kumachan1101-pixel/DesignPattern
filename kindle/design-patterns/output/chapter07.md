...
現状コードの通知メソッドは `void` で、3つの通知先を順番に呼び、受信箱への蓄積と表示だけを行います。同期・非同期の違い、部分失敗、`DeliveryResult` はまだ存在せず、SMS追加という1-5の変更要求を受けてから検討する要素です。
...
**着目する共通点：** 現状コードはすでに共通点を持っています。通知元はどの通知先へも同じ在庫警告（`StockAlert` 相当）を渡し、順に呼ぶという形にそろっています（データと流れの共通点）。現場では、通知先ごとに渡すデータがばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、通知元の骨格を変えず、変わる「通知先の種類と送り方」を共通契約の登録制へ外に出せば済みます。フェーズ6では、この共通点と課題を組み合わせて完成構造を決めます。
