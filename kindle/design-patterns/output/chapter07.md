...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 在庫を更新し、数値変化を記録する | 要求ID1、要求ID5 |
| 機能ID2 | 閾値以下の在庫を登録通知先へ通知する | 要求ID2、要求ID3 |
| 機能ID3 | 不正な在庫操作を拒否する | 要求ID4 |
...
|---|---|---|---|
| 機能ID1 | 継続<br/>根拠: — | 在庫を更新し、数値変化を記録する | 要求ID1、要求ID5 |
| 機能ID2 | 変更<br/>根拠: 変更ID1 | 既存通知先と非同期SMSへ在庫イベントを送り、個別結果を扱う | 要求ID2、要求ID3 |
| 機能ID3 | 変更<br/>根拠: 変更ID1 | 不正在庫操作と個別通知失敗を局所化する | 要求ID4 |
...
|---|---|---|---|
| 機能ID1 | 継続 | 要求ID1、要求ID5 | 入出庫と在庫数ログを回帰確認<br/>**判定:** 合格 |
| 機能ID2 | 変更 | 要求ID2、要求ID3 | 既存通知を回帰し、SMS受付成功・失敗時も他通知継続を確認<br/>**判定:** 合格 |
| 機能ID3 | 変更 | 要求ID4 | 不正在庫操作の拒否と通知失敗の局所化を確認<br/>**判定:** 合格 |
