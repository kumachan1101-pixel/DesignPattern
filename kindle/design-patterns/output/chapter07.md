...
フェーズ3で確認した「変更箇所が多岐にわたる」「既存のクラスを何度も開いて修正しなければならない」という痛みはなぜ発生するのでしょうか。コードを注意深く観察すると根源が見えてきます。

第一に、新しい通知先を追加するとき、なぜ毎回 `StockManager` を開かなければならないのでしょうか。それは、このクラス自身が「在庫が変わったら、在庫管理画面と本部システムに通知する」という具体的な通知先をすべて直接知ってしまっている（抱え込んでいる）からです。

第二に、なぜ変更の影響範囲が読めず、想定外のバグが起きるのでしょうか。それは、「在庫数を更新する」という変わらない骨格（主要な責任）と、「誰にどうやって伝えるか」という変わりやすい通知先の知識が、同じクラスの同じメソッドの中で物理的に混ざり合っているからです。

この「症状（痛み）」と「根本原因」を整理すると、以下のようになります。

| **観察した症状** | **構造的な原因** |
|---|---|
| 通知先を追加するために `StockManager` を修正する必要が生じます | `StockManager` が各通知先の具体的なクラス名やメソッドを直接知っているから |
| 新しい画面やシステムが増えるたびに `StockManager` が肥大化します | 「在庫の管理」と「通知先の管理」という変わる理由が異なるものが同じ場所に混在しているから |

### 7.16 変わるものと変わらないものの対比

原因の方向性が見えたところで、「変わり続けるもの」と「変わってほしくないもの」を明確に切り分けてみましょう。

| **変わるもの（変動）** | **変わらないもの（骨格）** |
| --- | --- |
| 通知先の種類、それぞれのクラス名、呼び出すべきメソッド名 | 在庫数を更新し、関係する場所に状態の変化を知らせるという処理の骨格 |

コード上でこれらがどう混在しているかを確認します。

**🔴 変わる部分（変わり続ける通知先の知識）**
```cpp
    // ↓ 開発チームや別システムの都合でどんどん増える
    display.updateView(newStock);
    headquarters.sendReport(newStock);
```
...
この状態は、現実の機器に例えるなら「センサー（在庫管理）の基板に、複数の表示ディスプレイ（通知先）のケーブルが直接はんだ付けされている」ようなものです。

新しいディスプレイを追加するたびに、センサーのカバーを開け、直接ケーブルをはんだ付けし直さなければなりません。これではセンサー自体を壊してしまうリスクが常に伴います。

### 7.18 接続点の特定

問題の根源は、通知先という「変わるもの」が、在庫管理という「変わらないもの」の中に直接書き込まれている（はんだ付けされている）接続の形にあります。

この接続の形を、基板を傷つけずに済む「USBハブ（インターフェースのリスト）」のような形に変える必要があります。
...
フェーズ5で「変わるのは通知先の種類（生産者）であり、渡すデータ（在庫数）は安定している」ことが分かりました。ここでは、その通知先をどのように差し替え・追加可能にするかを段階的に検討します。いきなりパターンを適用するのではなく、手続型のアプローチの限界を順に確認していきます。

### 7.22 ステップ1：丸ごと関数に切り出す（とりあえず分ける）

まずは手続型アプローチの第一歩として、通知ロジックを丸ごと別の関数（プライベートメソッド）に切り出してみます。

```cpp
    void notifyAllSystems() {
        display.updateView(stockAmount);
        headquarters.sendReport(stockAmount);
        alertSystem.triggerAlert(stockAmount);
    }
...
**この段階の評価：**
`updateStock()` の見通しは改善されましたが、`notifyAllSystems` の中身は依然としてスパゲティ状態のままです。新しい通知先が来るたびに、この関数に処理を書き足す痛みは何も変わっていません。

### 7.23 ステップ2：処理を個別の関数に分ける

次に、各システムへの通知を個別の関数に分けてみます。

```cpp
    void notifyDisplay() { display.updateView(stockAmount); }
    void notifyHeadquarters() { headquarters.sendReport(stockAmount); }
    void notifyAlert() { alertSystem.triggerAlert(stockAmount); }
...
**この段階の評価：**
それぞれの通知処理が関数として独立しました。しかし、結局 `notifyAllSystems` の中に通知処理が並び続ける構造は解消されていません。

### 7.24 ステップ3：条件も個別の関数に分ける（動的切り替えの試み）

通知先を動的に切り替えられるように、`if` 文やフラグを導入してみます。

```cpp
    bool hasDisplay = true;
    bool hasHeadquarters = true;

    void notifyAllSystems() {
        if (hasDisplay) notifyDisplay();
        if (hasHeadquarters) notifyHeadquarters();
        if (hasAlert) notifyAlert();
    }
```

**この段階の評価：**
一貫した構造が生まれ、「誰に通知するか」をフラグで管理できるようになりました。しかし、結局「どのフラグをチェックするか」という `if` 文の知識は、依然として `StockManager` の中に残ったままです。いくら関数化しても、クラス内に具体的な通知先の知識が残る痛みに直面します。

### 7.25 ステップ4：抽象化（インターフェース化して具体的な呼び出しごと追い出す）

ここで、共通の「通知を受け取る」という形を「抽象化（インターフェース化）」します。通知先を共通の型（`IObserver`）として扱います。
...
public:
    ...
    virtual void onStockUpdated(int stock) = 0;
};
```

`StockManager` は、具体的なクラス（`Display` など）を直接知る代わりに、`IObserver` のリストだけを持ちます。これが**Observer（オブザーバー）パターン**の概念です。

```cpp
class StockManager {
private:
    vector<IObserver*> observers; // ← 「誰か」は知らない。IObserverだということだけ知っている
public:
    ...
    void updateStock(int newStock) {
        ...
        for (auto obs : observers) {
            obs->onStockUpdated(newStock);
        }
    }
};
```

この抽象化を行うと、かつて `StockManager` 内にあった「Displayを更新する」「Headquartersに送る」という具体的な呼び出しはどこへ行くのでしょうか？それは**呼び出し側（登録側）**に移動します。
...
// かつて本体内にあった具体的な通知のひも付けが、ここ（呼び出し側）に移動しただけ
manager.addObserver(&display);
manager.addObserver(&hq);
```

**この段階の評価：**
ついに具体的な呼び出し処理が本体から消え去り、登録（呼び出し側）に移動しました。本体（`StockManager`）は通知先の種類を一切知らなくて済むようになります。条件やひも付けが呼び出し側に移動しただけで、実行結果は1ミリも変わらないリファクタリングが成功しました。

### 7.27 どこまで設計を進めるべきか（採用案の決断）

フェーズ2のヒアリングで、「今後もモバイルアプリや別システムなど、通知先が増えることは確実」と明言されています。したがって、今回は迷わず**ステップ4（抽象化：Observerパターン）まで進化させる**決断を下します。
