...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 商品ID、在庫数の変化、通知閾値 | `productId`、在庫数、しきい値 | 動作例テーブル、`main()`、変更要求 |
| 在庫更新、閾値判定、通知先への連絡 | 在庫管理処理と通知呼び出し | 現状コードの観察、通知先追加時の変更シミュレーション |
| 在庫状態、通知メッセージ | 実行結果の在庫表示と通知文 | 動作例テーブル、実行結果、変更シナリオ表 |
