...
フェーズ6では、まずフェーズ4で確認した「通知先追加が `InventoryManager` と在庫更新の再テストへ波及する経路」から、理想の変更影響グラフを組み立てます。その差からP1の課題カードを導き、カードの完了条件に効く候補を出します。その後でフェーズ3の通知関連コードだけをおさらいして段階変更へ進みます。変更要求を容れる前の現状コードへは戻りません。
...
|---|---|---|---|
| P1 | 通知追加の影響から在庫更新と通知先別ループを外す | notifyAll() の具体型・呼び方を切る。在庫更新を先に確定し、部分失敗で他通知を止めない | 通知先追加でInventoryManagerのメンバ・分岐・ループを増やさない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 通知処理へ共通の名前を付ける | 在庫更新と通知の境界が見える | DeliveryResultを返す同一契約へそろえ、登録された通知先へ配る |
...
ここからP1の通知先別メンバ・呼び出し・結果集計だけをおさらいします。
