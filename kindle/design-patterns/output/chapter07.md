...
通知契約を先に書かず、通知元が渡せる共通データ、通知先の登録時点、後日完了の別経路を決めます。

| 前工程で確定した事実 | ここで決めること | 判断 | 全体経路への反映 |
|---|---|---|---|
| 原因ID1：通知先ごとに関数名・引数・戻り値が違う | 通知元と各手段の境界を何にそろえるか | 在庫警告を入力、受付状態・手段・受付IDを結果とする | 通知元→共通通知操作→手段別処理と結ぶ |
| 通知先の種類は起動時に決まり、在庫イベントごとには変わらない | いつ生成・登録するか | 起動時に通知先を生成し、通知元へ登録する | 起動時経路と在庫更新時経路を分ける |
| 1件の通知失敗で他通知や在庫更新を戻さない | 配布の骨格をどこへ残すか | 通知元が登録順に全件へ配り、結果を1件ずつ記録する | 在庫更新→反復配布→結果記録とする |
| SMSの最終結果は後から受付IDで届く | 同期経路へ待機を入れるか | 後日完了は別の公開入口から状態ログだけを更新する | 通知受付経路と完了コールバック経路を分ける |

### 全体のデータと実体の流れを先に決める

上の判断を順につなぎ、通知先を生成・登録してから在庫イベントを配り、後日のSMS完了を記録するまでの全体経路を決めます。行2のPRD002の出庫1件を起点に一本で固定します。

この節で決めるのは、**どの実体が、いつ用意され、どのデータを受け渡し、どの順序で結果へ到達するか**までです。クラスとメソッドの名前は経路上の役割を識別するために置きますが、メンバー構成やメソッド本体はまだ書きません。次の「全体の流れを実現するコードを決める」で、この経路の各地点を宣言と処理本体へ落とします。

| 実行順・ポイント | 担う場所 | 経路で受け渡すもの・起きること | 次の呼出先 |
|---|---|---|---|
| 1. 生成 | `main()` | 商品マスタ・2つのログ・後日の入口・通知先4つ・通知元をローカル変数として生成し所有する | 通知先の登録へ |
| 2. 通知先を登録 | `main()` | `manager.attach(&email);` ほか4回。`observers`に生成済み実体への借用ポインタが並ぶ | 実行開始へ |
...
**起動時に作った `email` と、`attach()` で登録した実体と、通知時に反復する実体は同じものです。** 4行目から6行目までの間に、通知元が手段を見分ける行は1つもありません。

通知先は起動時に一度登録し、その後は同じ実体のリストを反復します。通知のたびに通知先を生成したり、種類を選び直したりはしません。
...
**ここで全体経路と対応づけるコード：** 在庫警告と受付結果をどの形で接続し、手段ごとの呼び方・文面・結果解釈をどこへ置き、通知先を誰が生成・登録するかをコードで示します。

**この章の課題は1つですが、全体経路を先に決める順序は他章と同じです。** その後、契約と具体をセットで分け、実体が生成から実行まで届くコードを確かめます。軸が一つなので、経路は一度追えば全体がつながります。

**全体経路との対応。** フェーズ5「課題の確定（5-3）」で決まったのは、手段ごとの呼び分けと結果の解釈を、在庫変化の検知とは別の変更へ閉じる境界を作ることです。全体経路で決めた接続を、ここから**境界を関数・表・クラス契約のどれで表し、どのコードで繋ぐか**へ落とします。
...
**3-1で足した3つのメンバが消えています。** `SMSNotifier sms;`、受付IDの状態表 `smsStatuses`、直近の受付ID `lastRequestId` です。具体の通知先を名指しで持たないので、`sms` は置けません。残り2つは、全体経路で決めた配送状態ログと後日の入口へ移します。`db` は1-4のまま値メンバとして書きます。
...
- **`observers` の中身。** `InventoryManager` 自身が具体通知先を作ると、`EmailNotifier` などの名前を知ってしまいます。全体経路で決めたとおり、`main()` が生成した通知先を `attach()` で登録します。
- **`record(r)` の持ち主。** SMSが返す保留の受付IDは、後日の最終配信結果と突き合わせるまで持っておく必要があります（要求ID3）。3-1では `InventoryManager` が `smsStatuses` として自分で持ち、その結果、在庫管理クラスがSMS基盤の受付IDと状態遷移まで知ることになりました。**同じ場所へ戻すわけにはいきません。持ち主は全体経路の準備コードで決めます。**
...
**3-1にあった `receiveSMSCompletion()` が、この宣言にありません。** 後日届く最終配信結果を受ける入口は、在庫操作の公開操作ではないからです。全体経路で決めた `SMSCompletionEndpoint` が受けるコードを、後の準備箇所で示します。
...
**もう一度検算します。契約のメソッド以外に書きたくなるものがあるか。** SMSの最終配信結果を保つ場所がありません。**しかしそれは `send()` の裏に書くものではありません。** 受付IDは通知手段をまたいで追跡する情報で、`SMSNotifier` の中に閉じ込めると、通知先が破棄されたときに履歴も消えます。**置き場所は全体経路の準備コードで決めます。**
...
**ここが終わった時点で、実体はすべて存在しています。** 通知先4つ、状態ログ、後日の入口、在庫変動ログ、商品マスタ、通知元。**まだ起きていないのは1つだけです。骨格の `observers` に、通知先が入ることです。続けて、全体経路で決めた受け渡しをコードで確認します。**

---

##### 全体経路の登録：生成済み通知先を通知元へ登録する
...
**掲載箇所：`main()`** ―― 全体経路の準備コードで作った実体を、契約として通知元へ渡す
...
**この章の通知元は、誰に配るかを選びません。** 登録されている全員へ、同じ `StockAlert` を、同じ `send()` で渡します。
...
- **何が入るか。** 全体経路の準備コードで `main()` のローカル変数として作った4つの実体です。骨格が持つのはその借用ポインタで、実体は `main()` にあります
...
| 商品マスタ・在庫変動ログ・配送状態ログ | 組み立て側が何を渡したか | `InventoryManager manager(...)` の引数 | 違う。全体経路の準備コードで説明した別の依存 |

**通知元が通知先へ届く方法は、`attach()`で借用ポインタを登録することです。** この章が借用にしたのは、`main()` が同じ実体を `detach()`／`attach()` で差し替えるからです。
...
後日の完了は、呼ぶ相手が変わりました。3-1では通知元の公開操作 `receiveSMSCompletion()` を、直近の受付IDを取り出してから呼んでいました。**全体経路の準備コードで後日の入口を別のクラスへ置いたので、在庫操作の入口とは別の起点になりました。** 受付IDは行2の実行結果に出た `SMS-1` を使います。
...
**守る範囲との照合：** **公開操作**を保っていることを、いま確認しました。「`reduceStock()` で出庫し、`replenishStock()` で補充する」という入口の形は1-4のままです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、全体経路と詳細コードで照合済みです。**
...
| 実体の組み立て | 通知元が具体を値保持 → `main()` が通知先・ログ・後日入口を所有 | `attach()` で契約として登録し、在庫更新から全通知先を実行、後日結果は別入口へつなぐ | 生成した4通知先とログが実行経路で使われる |
...
| 🔴 フェーズ6：対策検討 | 契約と具体による分離、実体の組み立ての二つから完成構造を決め、採用クラス図をフェーズ7の3段階でコードへ反映した。 |
