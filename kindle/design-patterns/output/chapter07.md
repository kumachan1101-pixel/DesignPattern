...
誰が何を変える権限を持つかは、設計の判断に直接影響します。たとえば「メールの宛先を変える」のはインフラ管理部門の判断で、「ダッシュボードの表示を変える」のはフロントエンドチームの判断です。一つのクラスがこれら複数の担当者の知識を抱えていると、ある担当者の変更が別の担当者の領域まで触らなければならない状況が生まれます。
