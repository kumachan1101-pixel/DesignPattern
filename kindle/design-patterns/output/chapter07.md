...
|---|---|---|---|
| F1 | 通知先となるクラスの種類とその実装（音声通知システムなど） | 業務要件の変更があるたび | 田中部長との合意 |
| F2 | 通知先の増減（動的な登録・解除） | 随時 | 田中部長との合意 |
| F3 | 通知先ごとの同期・非同期や受付失敗の扱い | 送信基盤や外部連携が増えるたび | 田中部長との合意（在庫は止めない） |
...
2-4のF-IDを、通知先ごとに変えられるようにする部分と、在庫更新の安定側へ分けます。「はい」は、フェーズ6で**通知先や受付方法が変わっても、在庫更新と通知条件へ影響を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：通知先となるクラスの種類とその実装（音声通知システムなど） | はい | 通知手段ごとの送信処理 | 在庫更新、閾値判定、通知データ |
| F2：通知先の増減（動的な登録・解除） | はい | 通知先の構成と登録 | 出庫処理の入口、登録済み通知先を順に呼ぶ流れ |
| F3：通知先ごとの同期・非同期や受付失敗の扱い | はい | 通知先ごとの受付結果 | 通知失敗で在庫更新と他通知を止めない規則 |

したがって2-5の出力は、「通知手段・登録・受付結果は変えられるようにし、在庫更新と閾値判定は守る」という設計条件です。フェーズ3ではR1だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
> R1のSMS追加は数行に見えますが、そのために在庫管理の本体である`InventoryManager`を開き、メンバ・コンストラクタ・通知処理を修正しました。通知手段の変更が在庫更新コードを同時に確認対象へ巻き込んでいます。

R1を現在の構造へ適用すると、SMSのために`InventoryManager`のメンバ変数、コンストラクタ、`notifyAll()`を同時に書き換える必要がありました。
...
1つ目は、`InventoryManager`がR1のために「SMS通知先の存在」「送信方法」「受付結果」を新たに抱えたことです。在庫更新と閾値判定を担うクラスへ、通知手段の具体知識が追加されました。

2つ目は、今回のSMS追加のために通知元の`InventoryManager`まで修正したことです。「在庫管理」と「通知手段の選択」という異なる変更理由のコードが一つのクラスにあります。

3つ目は、R1の同期・非同期・部分失敗の違いまで通知元へ入ったことです。変更途中コードの`notifyAll()`は、SMSの受付結果を確認し、失敗を記録して他通知を続ける判断を持ちました。

フェーズ3では、R1によって通知元クラスを書き換え、通知先ごとの成否・待ち方まで通知元へ入った痛みを確認しました。次のフェーズ4では、通知元と通知先の接続点に漏れている知識を確認します。

---
> **📌 問題（確定）**
> R1のSMS追加で、通知元の`InventoryManager`のメンバ変数・コンストラクタ・`notifyAll()`が連動して変わった。通知手段と受付結果が在庫管理ロジックに混在し、一つの確定要求が通知元まで修正対象にした。
---
...
接続点の両側をコードで見ると、通知元が同じ `StockAlert` を手段ごとに異なる操作へ渡し、各接続先が別々の表現へ変換しています。
...
void InventoryManager::notifyAll(const StockAlert& alert) {
    ...
    dashboard.update(alert);
    chat.send(alert);
    sms.requestAsync(alert);
}

void EmailNotifier::send(const StockAlert& alert) {
    cout << "[Email] 件名:在庫不足 / "
         << alert.productName << " 残" << alert.stock << "\n";
}

void SMSNotifier::requestAsync(const StockAlert& alert) {
    cout << "[SMS受付] 在庫警告 "
         << alert.productId << " 残" << alert.stock << "\n";
}
```

したがってP1は三つの呼び出し行だけを指しません。通知元が作る在庫警告、接続先ごとに異なる操作名・文面・送信方法、返される受付結果、結果を集約して一部失敗でも在庫更新を確定する後続処理までを含みます。
...
システム全体の課題は、在庫更新を行う `InventoryManager` から、通知手段の具体型・固有メソッド・文面・同期非同期を外すことです。そのうえで、在庫更新後に作った `StockAlert` を変わる通知手段の集合へ同じ操作で渡し、共通の `DeliveryResult` を集約するところまでを安定側の責任として残します。
...
---
> **📌 課題（確定）**
> P1の接続点を `StockAlert → DeliveryResult` という契約へ揃え、通知手段の追加・文面・同期非同期を通知を担う側へ閉じる。`InventoryManager` は、在庫更新後に警告データを作り、変わる通知手段の集合へ同じ操作で伝えて結果を集約するところまでに徹する。
---

問題・原因・課題の3点が揃いました。次のフェーズ6では、このP1を入力として、システム全体の完成構造を決めます。

**着目する共通点：** 現状コードはすでに共通点を持っています。通知元はどの通知先へも同じ在庫警告（`StockAlert` 相当）を渡し、順に呼ぶという形にそろっています（データと流れの共通点）。現場では、通知先ごとに渡すデータがばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、通知元の骨格を守ったまま、変わる「通知先の種類と送り方」だけに絞って対策を考えられます。それをどう分けて接続するかは、フェーズ6でこの共通点と課題を組み合わせて決めます。
...
| フェーズ5の接続点 | 通知元は種別を知らず、`StockAlert` を渡し `DeliveryResult` を集めればよい | P1の送信手段・受付結果を各通知クラスへ置く |
...
|---|---|---|---|
| P1：通知手段 | 手段追加・文面・同期非同期を在庫処理へ波及させない | `StockAlert`、`INotification::send()`、各通知アダプター、`attach()` | 具象通知クラスと登録だけが変わり、手段別の文面と受付結果は各アダプターへ閉じた |
| P1を適用したシステム全体 | 在庫更新→ログ記録→警告配布の順序を守る | DB・ログのコンストラクタ注入と `InventoryManager::reduceStock()` 内の自動記録・一律配布 | 利用者の手動ログをなくし、一部通知失敗でも在庫更新と他通知を継続できた |

**システム全体の実装結果：達成。** P1が通知分離構造として在庫更新経路へ接続され、フェーズ5で目指した状態を実現しました。実行結果と変更影響は、完成コードを示した後のフェーズ7で確認します。
...
|---|---|---|---|
| F1：通知先となるクラスの種類とその実装（音声通知システムなど） | StockAlertを受けるINotification実装として通知先を追加する | 新Notifierと登録・所有箇所 | 在庫更新を守れる。音声通知は完成コードへ追加しない |
| F2：通知先の増減（動的な登録・解除） | 通知先の所有と列挙をInventoryManagerの購読境界へ集める | 登録解除操作、通知先の所有方法 | 追加は局所化済み。動的解除は仕様確定後に設計し、完成コードへ追加しない |
| F3：通知先ごとの同期・非同期や受付失敗の扱い | DeliveryResultで受付状態を共通化し、通知先固有の送信方式を各Notifierへ置く | 各Notifier、結果集約、将来の再送・待機境界 | 受付結果までは局所化。再送や非同期完了管理は完成コードへ追加しない |

F1〜F3はObserverの通知契約と所有方法を評価する材料です。将来の通知先や運用機能を実現済みとは扱いません。
...
今回の確定要求R1について、フェーズ1の構造で必要だった修正と完成構造の結果を対比します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：在庫警告の通知先へ非同期SMSを追加し、受付失敗でも在庫更新と他通知を止めない | `InventoryManager`へSMSの生成・呼び出し・成否分岐を追加 | `SMSNotifier`を登録し、4手段の受付結果を個別に残す。SMS失敗時も在庫更新と他通知が完了することを確認 |
