...
| 役割 | 担当者 | 管轄する知識 |
|---|---|---|
| 在庫システム開発チーム | 自チーム | 閾値判定ロジック・システム保守 |
| システム管理者 | インフラ管理部門 | メール等の通知手段の採用・廃止 |
| UI担当者 | フロントエンドチーム | ダッシュボードの表示仕様 |
| チーム管理者 | 各部門マネージャー | チャット等の連絡網の変更 |
...
フェーズ1の仕様表を振り返ります。現在の在庫通知には「メール」「ダッシュボード」「チャット」の3種類があります。このうち変化が予想される仕様があります。

- **通知先の種類**（メール・ダッシュボード・チャット・SMSなど）：システムが成長するにつれて、連携先が追加されることが多い部分です。今回のSMS追加要求もその一例です
- **各通知先の詳細設定**（通知先アドレス、メッセージ形式など）：業務の変化に合わせて調整が入ります

一方、「在庫が閾値を下回ったら通知する」という通知のトリガー（判断基準そのもの）は、在庫管理業務の本質として安定している部分です。

**仮説：通知先の種類は、今後も追加・変更が続く可能性がある。**
...
ヒアリングで「音声通知システム連携」の追加が予告されました。この変更が来たとき、仕様がどう変わるかを整理しておきます。

| 通知先 | 現在 | 将来（数ヶ月後） |
|---|---|---|
| EmailNotifier（メール） | 対応済み | 変更なし |
| DashboardUpdater（ダッシュボード） | 対応済み | 変更なし |
| ChatNotifier（チャット） | 対応済み | 変更なし |
| SMSNotifier（SMS通知） | 今回追加 | 変更なし |
| 音声通知システム | —（なし） | 追加予定（バックヤードの音声アラート） |

音声通知が追加されると `InventoryManager` のメンバ変数と `notifyAll()` にさらに追記が必要になります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された「音声通知システム」も追加されると、`InventoryManager` のメンバ変数と `notifyAll()` にさらに一行ずつ追記が必要になることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、通知先が増えるたびに通知元クラスを修正しに行く構造は変わりません。
...
分離の境界は決まりました。次は「通知先ごとのクラスをまとめるか、個別に分けるか」という粒度の問題です。

ヒアリングで把握した通知先と、変わる理由を確認します。

| 通知先 | 変わる理由 |
|---|---|
| EmailNotifier（メール） | SMTPサーバー設定・メール仕様（IT運用チーム） |
| DashboardUpdater（ダッシュボード） | ダッシュボードのAPI仕様（分析チーム） |
| ChatNotifier（チャット） | チャットプラットフォームのAPI（コラボツール担当） |
| SMSNotifier（SMS通知） | SMS gateway のAPI仕様（田中部長チーム） |
| 音声通知（将来） | バックヤード機器の仕様（運用チーム） |

各通知先は異なる外部システムや担当者に依存しており、変わり方は独立しています。SMS gateway の仕様変更がメールの送信ロジックに影響することはありません。音声通知の追加がダッシュボードの更新処理に波及することもありません。各通知先は**独立して変わる**ため、通知先ごとに別クラスへ分離します。どの通知先も「メッセージ（string）を受け取って通知する」という同じ操作を持ちます。

→ **通知先ごとに別クラスへ分離し、同じ `IInventoryObserver` インターフェースで束ねる**
...
あなたのコードの中にも、ある処理が完了したときに複数の場所へ通知・連携を行っている箇所があるはずです。その通知先が「誰の判断で変わるか」を問うことが、Observerパターンを使う理由を見つける入口になります。
