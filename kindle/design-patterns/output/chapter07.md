...
要求ID5の在庫変動記録は、通知手段の混在から生じた設計課題ではありません。フェーズ6でシステム全体の生成・所有を決めるときに実装場所も確定しますが、課題ID1の追跡には混ぜず、要求IDとしてフェーズ7で検証します。
...
フェーズ4「原因分析」で、`InventoryManager` が「在庫が少なくなった事実」を検知する処理の中に、「誰にどう知らせるか」——通知先のクラス名・関数名・引数の形・戻り値の意味・同期非同期の別——まで抱えていることを確認しました。対策は、在庫変化の事実を配るだけにし、通知先を共通の契約の裏へそろえることです。この段階では、問題から導いた「通知分離構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。
...
向きと掲載クラスは1-3から変えていません。同じ図に注記と色だけを加え、`InventoryManager` のどの責任を残し、どの責任を移すかに着目します。**この図を基準に、分離と組み立てで必要な関係を足していきます。**
...
- **契約と具体による分離**：在庫更新側に残す処理と、通知手段ごとの差を持つ契約・具体をセットで決める。
- **生成・注入・実行による組み立て**：通知先を誰が生成・所有・登録し、在庫更新から契約だけを通してどう配布するかを一本で決める。
...
**ここから決めること：** 在庫警告と受付結果をどの形で接続し、手段ごとの呼び方・文面・結果解釈をどこへ置き、通知先を誰が生成・登録するかを順に決めます。
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 手段ごとの知識は `notifyAll()` の中だけでなく、`InventoryManager` のメンバ宣言・コンストラクタ・受付IDの状態表・`receiveSMSCompletion()` にも現れます。3-1でSMSを足したとき、書き換えた場所は6か所でした。そして2-4のリスクID1（通知先の種類と実装）、リスクID2（通知先の増減）、リスクID3（受付と最終配信結果の扱い）が、どれも「増える・変わる」と言っています。**現に変更要求で、SMSという通知先が1つ増えました。** だから各通知手段を同じ問い方で呼べる契約まで進みます。
...
**それでも、書いてみるまで確定はしません。** 分離の検算で骨格を実際に書き、この3項目で足りることを確かめます。足りなければここへ戻って取り直します。**戻る先が1段だけで済むのは、変更前のコードという確かめようのある根拠から決めているからです。**
...
変更前の `if (...) accepted++; else failed++;` が、ここにはありません。**では、結果を集計するのは誰の仕事になるのか。それは分離の検算で決めます。**
...
呼べるのは、通知先が増えても今回維持する側だけです。出て行った側が出て行った側を呼べば、通知先が増えるたびに両方を触ることになり、分けた意味が消えます。

**この章では、契約を骨格の外へ置きます。** 1回の在庫更新で通知が走るとき、通知元は**同じ実行の途中で相手を4回入れ替えます。** メールへ送り、次に画面を更新し、次にチャットへ投稿し、次にSMSを受け付ける——1つの `notifyAll()` の中で、契約の裏にいる実体が次々に替わります。だから通知元とは別の型として `INotification` を置きます。

**共通順序を基底クラスへ置く形にできないことも確かめます。** 「実体を作った時点で相手が決まる」形にするなら、通知手段ごとに通知元を別インスタンスにすることになります。しかし在庫は1回しか減りません。手段の数だけ通知元を作れば、在庫更新も手段の数だけ走ってしまいます。**通知元が1つで、相手が複数**というのがこの章の形です。

残った側は、公開操作を持つクラス——つまり `InventoryManager` です。**名前は変えません。**

1-3のクラス一覧で、`InventoryManager` の役割を「**在庫数を管理し、必要な通知を呼び出す**」、担当する仕様を「**在庫更新、閾値判定、通知実行**」と書きました。契約の確認で決めた形で外へ出るのは、この3つのどれでもありません。出て行くのは「手段ごとの関数名・引数の組み立て・戻り値の解釈」で、これは1-3の役割一覧に**書かれていなかった知識**です。在庫更新も閾値判定も、通知の実行（登録された全員へ配ること）も、そのまま残ります。**責任が変わっていないので、名前だけ変えることはしません。**

**メンバーは変わります。ただし、ここで分かるのは前工程から持ち越すものと、契約を並べる場所だけです。**

```cpp
class InventoryManager {
    // 契約だけを並べる。中身がどこから来るかは、ここでは未定
...
**3-1で足した3つのメンバが消えています。** `SMSNotifier sms;`、受付IDの状態表 `smsStatuses`、直近の受付ID `lastRequestId` です。具体の通知先を名指しで持たないので、`sms` は置けません。残り2つがどこへ行くかは、まだ決めていません。`db` は1-4のまま値メンバとして書きます。持ち方を見直す理由が出るかどうかは生成の検討で確かめます。
...
- **`observers` の中身。** 誰が、いつ、何を入れるのかが決まっていません。`InventoryManager` 自身が入れることはできません。入れれば `EmailNotifier` をはじめとする具体名をここに書くことになり、契約の確認で分けた意味が消えます。**外の誰かが入れることになります。それを決めるのが注入の確認です。**
- **`record(r)` の持ち主。** SMSが返す保留の受付IDは、後日の最終配信結果と突き合わせるまで持っておく必要があります（要求ID3）。3-1では `InventoryManager` が `smsStatuses` として自分で持ち、その結果、在庫管理クラスがSMS基盤の受付IDと状態遷移まで知ることになりました。**同じ場所へ戻すわけにはいきません。持ち主は生成の検討で決めます。**

**穴が空いたままでも、この骨格が成立するかは検算できます。** 通知先を1つ足したとき、上の3段は変わるか。1段目は登録リストを回すだけなので変わりません。2段目は `send()` を呼ぶだけで、相手が誰でも同じです。3段目は3区分を数えるだけです。**通知先が5つになっても6つになっても、この関数は1行も増えません。** 変わってしまうなら割り残しがあるので、契約の確認へ戻って線を引き直します。

公開操作の側は、3-1から形が戻ります。

**掲載箇所：`InventoryManager::reduceStock(string, int)`** ―― 在庫更新から通知までの制御順（ここで分かるところまで）
...
**3-1にあった `receiveSMSCompletion()` が、この宣言にありません。** 後日届く最終配信結果を受ける入口は、在庫操作の公開操作ではないからです。どこが受けるかは生成の検討で決めます。
...
契約と、それを呼ぶ手順が決まりました。契約の確認では `EmailNotifier` を1つ覗いて、渡すもの・受け取るものを確かめました。**ここで残り全部を埋めます。**

**ここが決めるのは個数ではありません。** 個数は契約の確認で「手段ごとに別クラス」と決めた時点で確定しています。ここで決めるのは、**各具体が何を書かないか**です。契約の確認で見た `EmailNotifier` と対照的なものを1つ見ます。
...
3-1で4通りに分かれていた呼び方が、4つのクラスになりました。**変更要求で足したSMSが、`SMSNotifier` という1クラスに収まっています。** 3-1では同じ追加のために `InventoryManager` を6か所書き換えていました。これで課題ID1の完了条件「新通知は登録だけで加わり、1件失敗しても在庫更新と他通知が完了する」の前半——新しい通知手段が1クラスに閉じること——を満たします。後半の「1件失敗しても」は、分離の検算で確認したとおり反復が止まらないことで満たします。

**もう一度検算します。契約のメソッド以外に書きたくなるものがあるか。** SMSの最終配信結果を保つ場所がありません。**しかしそれは `send()` の裏に書くものではありません。** 受付IDは通知手段をまたいで追跡する情報で、`SMSNotifier` の中に閉じ込めると、通知先が破棄されたときに履歴も消えます。**置き場所は生成の検討で決めます。**
...
**分離の検算で穴が2つ空きました。** `observers` の中身と、`record(r)` の持ち主です。ここでは**実体が存在するようになるまで**を決め切ります。

**まず、通知先の具体を誰が作り、誰が持つか。** 持つ者に必要な条件は、分離の検算で分かっています。

- **具体クラスを全部知っていること。** `EmailNotifier` の実体を作る以上、その名前を書きます
- **`InventoryManager` ではないこと。** 分離の検算で確認したとおり、知った時点で分離が壊れます
...
**次に、`record(r)` の持ち主を決めます。** 分離の検算で残した穴です。持つ者に必要な条件は、穴の性質から出ます。
...
**5-3で課題ID1と分けておいた要求ID5の記録先も、ここで配置を確定します。** 1-4の `cout` だけでは実行の最後に全在庫変動を取り出せないため、在庫変動を1件ずつ積む `StockEventLog` を置き、`main()` が最後に `printAll()` で一覧します。これは通知分離の対策ではなく、要求ID5の実装です。
...
**分離の検算で `ProductDatabase db;` と書いた値メンバが、参照になりました。** 在庫の正本と在庫変動ログは、同じ1件の在庫更新を記録する対です。片方を `main()` が持ち、もう片方を通知元が内側に隠すと、対応が読み取れなくなります。**課題ID1の分離とは別の判断なので、そのことを書いておきます。** 1-4のまま値メンバにしても、課題ID1は成立します。

**分離の検算で空けた穴が、ここで埋まります。**

**掲載箇所：`InventoryManager::notifyAll(const StockAlert&)`** ―― 分離の検算で `record(r)` と書いた行

```cpp
        DeliveryResult r = o->send(alert);
        deliveryStatusLog.record(r);   // ←分離の検算で残した穴が埋まった
```
...
**掲載箇所：`InventoryManager::reduceStock(string, int)`** ―― 分離で確定した制御順へ、在庫変動の記録を足す
...
**`InventoryManager` と `SMSDeliveryCallback` の間に矢印がありません。** 即時の配布と後日の完了は、`DeliveryStatusLog` の受付IDだけでつながっています。**在庫更新の呼び出しが、後日の結果を待つことはありません。** `INotification` と4つの実装はこの図に描いていませんが、具体の確認から変わっていません。所有者は `main()` ですが、`main()` はクラスではないので図には描かず、上の生成コードで示しています。


**ここが終わった時点で、実体はすべて存在しています。** 通知先4つ、状態ログ、後日の入口、在庫変動ログ、商品マスタ、通知元。**まだ起きていないのは1つだけです。骨格の `observers` に、通知先が入ることです。次に、注入として確認します。**

---

##### 注入：実体を契約として骨格へ渡す

**分離の検算で書いた骨格には、`INotification*` としか書いてありません。** メールとも、SMSとも書いていません。それでも実行すれば4手段へ届きます。**ここで見るのは、いつ・何によって、どの実装が入るのかです。**
...
**掲載箇所：`main()`** ―― 生成の検討で作った実体を、契約として通知元へ渡す
...
具体の確認で手段ごとの違いを具体側へ移した結果、**通知元が相手を見分ける必要がなくなりました。** 見分けないので、選ぶ決め手も要りません。**もしここで「この警告はメールだけへ」と選び始めれば、通知元は再び手段の性質を知ることになります。** 配布先を絞る判断が通知元に無いこと自体が、この章で得たものです。

**ここで起きていることを、4つに分けて書きます。**

- **何が入るか。** 生成の検討で `main()` のローカル変数として作った4つの実体です。骨格が持つのはその借用ポインタで、実体は `main()` にあります
...
| 商品マスタ・在庫変動ログ・配送状態ログ | 組み立て側が何を渡したか | `InventoryManager manager(...)` の引数 | 違う。生成の検討で説明した別の依存 |

**骨格が通知先へ届く方法は、生成の検討で決めた別の話です。** 借用ポインタで持つと決めましたが、値で持っても契約としてしか書かないなら注入は起きます。この章が借用にしたのは、`main()` が同じ実体を `detach()`／`attach()` で差し替えるからです。
...
出庫の呼び方は、引数が4つから2つへ戻りました。3-1では商品名・更新後在庫・閾値を呼び出し側が用意していましたが、それは通知の材料を通知元が組み立てられなかったからです。**契約の確認で「4項目を1つの値で渡す」と決め、分離の検算でその値を通知元が商品マスタから作ると決めた結果、呼び出し側が用意するものが無くなりました。** 1-4の `reduceStock(productId, quantity)` と同じ形です。

後日の完了は、呼ぶ相手が変わりました。3-1では通知元の公開操作 `receiveSMSCompletion()` を、直近の受付IDを取り出してから呼んでいました。**生成の検討で後日の入口を別のクラスへ置いたので、在庫操作の入口とは別の起点になりました。** 受付IDは行2の実行結果に出た `SMS-1` を使います。
...
**守る範囲との照合：** **公開操作**を保っていることを、いま確認しました。「`reduceStock()` で出庫し、`replenishStock()` で補充する」という入口の形は1-4のままです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**

#### システム全体の最終構造を決める

分離と組み立ての部分図を重ねると、システム全体の最終構造になります。**この章の完成構造は一つに定まります。** 通知手段ごとの専用メンバを残したまま契約だけを足す形も置けますが、それは課題ID1の波及を解消しない途中状態なので比較しません。責任配置や依存が異なる完成構造が複数残ったわけではないので、当て馬を並べた比較は行いません。採用するのは、共通契約を実装した通知クラスを組み立て側が生成・登録し、通知元が登録リストへ一律配布する**通知分離構造**です。
...
|---|---|---|---|
| 契約と具体による分離 | 手段別の呼出・結果解釈 → `INotification` と4つの通知具体 | `StockAlert` と `NotificationResult` を境界にし、文面・引数・戻り値の翻訳を具体へ置く | 在庫管理は登録先を一律に呼び集計するだけ |
| 生成・注入・実行による組み立て | 通知元が具体を値保持 → `main()` が通知先・ログ・後日入口を所有 | `attach()` で契約として登録し、在庫更新から全通知先を実行、後日結果は別入口へつなぐ | 生成した4通知先とログが実行経路で使われる |

公開入口は商品IDと数量だけを受け取ります。通知材料は在庫管理が作り、組み立て時に登録された通知先へ契約として配ります。

#### 代表ケースの実行接続

生成・登録した通知先が実際に使われるまでを確認します。行2のPRD002の出庫1件と、その後日に届いた `SMS-1` の配信完了を一本で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成 | `main()` | 商品マスタ・2つのログ・後日の入口・通知先4つ・通知元をローカル変数として生成し所有する | 注入へ |
| 2. 注入 | `main()` | `manager.attach(&email);` ほか4回。`observers` に借用ポインタが登録順に並ぶ | 実行開始へ |
| 3. 実行開始 | `main()` | `manager.reduceStock("PRD002", 1);` | `InventoryManager::reduceStock()` |
| 4. 骨格 | `InventoryManager::reduceStock(string, int)` | 存在確認・数量検証・在庫更新・記録の後、閾値以下なので `notifyAll(alert)` を呼び、登録順に `o->send(alert)` を反復する | `INotification::send()` |
| 5. 契約 | `INotification::send(const StockAlert&)` | 登録された契約へ動的ディスパッチする。通知元は相手を選ばない | `EmailNotifier::send()` ほか3クラス |
| 6. 具体 | `SMSNotifier::send(const StockAlert&)` | 受付IDを採番し `{PENDING, "SMS", "SMS-1"}` を返す | 戻り値を骨格の `deliveryStatusLog.record()` へ |
| 7. 実行開始（後日経路） | `main()` | `smsCallback.receive("SMS-1", true);` | `DeliveryStatusLog::complete()` が `SMS-1` を配信完了へ更新 |

**生成で作った `email` と、注入で `attach()` した実体と、骨格が反復する実体は同じものです。** 4行目から6行目までの間に、通知元が手段を見分ける行は1つもありません。

**注入が1回しか現れないことに注目してください。** 第9章では引き当て関数が呼び出しのたびに実装を返すので注入が軸の数だけ現れましたが、この章は組み立て時に登録して以降は同じリストを回ります。**3つある注入の形のうち「生成した実体を渡して保持する」形を選んだ結果が、この表に出ています。**
...
| 🔴 フェーズ6：対策検討 | 契約と具体による分離、生成・注入・実行による組み立ての二つから完成構造を決め、採用クラス図をフェーズ7の3段階でコードへ反映した。 |
