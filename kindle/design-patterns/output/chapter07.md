...
| 通知のトリガー条件 | 在庫が閾値以下になった瞬間 | 変わらない（田中部長と合意済み） |

この変化が来たとき、今の `InventoryManager` はその都度書き換えを迫られます。次のフェーズ3では、実際にSMS通知を追加しようとしたときに何が起きるかを試みます。

### 2-5：将来の変更仕様の見通し

ヒアリングで「通知先の種類の増加」「動的な登録・解除」が予告されました。この変化が来たとき、仕様がどう変わるかを整理しておきます。

| 変更内容 | 現在 | 将来（時期の目安） |
|---|---|---|
| 通知先の種類 | メール通知・アプリ通知の固定構成 | 音声通知システムなど業務要件変更のたびに追加 |
| 通知先の登録・解除 | 静的（コード変更が必要） | 動的な登録・解除（随時変更可能）に切り替わる |
| 在庫減少イベントの発生タイミング | 不変（在庫更新後に発火） | 変わらない（骨格として合意済み） |
