...
現在の構成では、`InventoryManager` クラスが在庫の変動検知から各通知先への連絡まで、すべての処理を担っています。
...
このシステムの規模では、通知先クラスの導入によるパフォーマンスコストは設計判断を絞り込む制約になりません。ただし、セール期間中に在庫が短時間で急変動すると、通知先の数に比例して処理時間が増大します。**同期通知のまま通知先を増やすか、非同期通知に切り替えるか**は、接続形態の選択に影響します。この点は案3・案4の選定に影響するため、各案のトレードオフで触れます。
...
* 実装コスト：中（仲介クラスの作成が必要）
* ※ セール時の通知遅延：`ObserverManager` が通知先を一元管理しているため、セール時の通知量増大への対応（処理の優先度付けや非同期化）をここ一箇所で制御できる。
...
* ※ セール時の通知遅延：`IObserver` インターフェース越しに通知先を差し替えられる構造のため、将来的にキューを挟んだ非同期通知実装への切り替えが、`InventoryManager` を変更せずに可能。
