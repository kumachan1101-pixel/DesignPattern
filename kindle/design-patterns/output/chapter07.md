...
| パフォーマンス | ホットパスか（高頻度で呼ばれるか） | 通常時はホットパスではない。ただしセール期間中は短時間で大量の在庫変動が発生し、通知先が数十になると同期処理で遅延が生じる。非同期化で対応可能だが、今回のスコープ外とする |
...
通常の在庫変動イベントではパフォーマンス上の問題はありません。セール期間中に在庫が急変動すると通知先の数に比例して処理時間が増大する点は残課題として正直に明記します。非同期化による対応は今回のスコープ外ですが、通知先が大幅に増える見通しがある場合は別途検討が必要です。
