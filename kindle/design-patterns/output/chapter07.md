...
フェーズ6では、フェーズ4で確認した「通知先追加が `InventoryManager` と在庫更新の再テストへ波及する経路」を図の起点にします。P1でどのコード境界を微修正し、同じ通知先追加を当て直すと影響がどこまで縮むかを一本の経路で描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じP1の行へ落とします。その後でP1に紐づくフェーズ3の通知関連コードだけをおさらいし、候補を一段ずつ適用します。変更要求を容れる前の現状コードへは戻りません。
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: 通知処理へ共通名を付ける"] --> S2["P1-変更2: DeliveryResult契約と登録リストへ上げる"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["通知先を追加"] --> I1["P1: 通知実装と登録だけを追加"]
        I1 --> I2["追加通知の受付結果だけをテスト"]
        IS["変更しない: 在庫更新・既存通知先"]
    end
    C1 -. "P1で通知先別呼び出しを切る" .-> S1
    S2 -. "通知追加を局所化" .-> I1
```
...
P1でおさらいするのは、フェーズ3のうち通知先ごとのメンバ、呼び出し、結果集計が在庫更新へ混ざった部分です。直前の一続き表で指定した「通知処理へ共通名を付ける」をこのコードへ当て、種類別リストで影響が残るなら、同じ `DeliveryResult` 契約の登録リストへ微修正します。以降の各段階でも同じP1を付け、減った影響と残った影響をコードで判定します。
...
|---|---|---|---|
| P1 | 通知関数、`INotification::send()`、`InventoryManager::attach/notifyAll()` | **微修正：** 共通名→種類別リストの重複確認→同一契約の登録へ統合<br>**結果：** 通知先別メンバ・メソッド・ループが消えた | **守った契約：** 在庫更新を先に確定、部分失敗でも継続<br>**判定：** 種類別リストでは未達、一律配布で達成 |
