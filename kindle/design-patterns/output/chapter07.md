...
課題は「通知先を外へ出す」と言っていますが、**どんな形なら外へ出せるか**は、クラス図を眺めても出てきません。ここが対策検討の中心の思考です。痛みコードの `notifyAll` を分解し、4つの呼び出しの「受け取る型」と「返す型」だけに注目して並べ直します。
...
4つとも「`string` を受け取り `void` を返す」——**名前は違うが、同じ形**です。ここで初めて「通知先には共通の形がある」と分かります。`update` の名前を `send` にそろえれば、4つを1つの形として扱えます。この「共通の形」が、課題で言っていた**接続点の正体**です。クラス図から入っていたら、この共通点には気づけませんでした。
