...
**得られること2**（接続点の診断）：フェーズ4で、通知先のクラス名と送信方法が通知元へ漏れていることを確認しました。これが通知先追加のたびに在庫管理クラスを修正する理由です。

**得られること3**（変更局所化の説明）：フェーズ7の変更シナリオ表で、通知先の実装と登録処理へ変更範囲を寄せられる構造を示しました。

**得られること4**（動的な通知先管理）：フェーズ6と7で、共通の通知操作と`attach()`による登録を用意することで、通知先を動的に追加・削除できることを学びました。
