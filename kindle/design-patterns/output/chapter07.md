...
この時点では、通知先をどう増やすかは判断しません。まず、どの在庫変動で通知が発生するかを確認します。
...
この章では、メール送信、チャット投稿、ダッシュボード更新を `EmailNotifier`、`ChatNotifier`、`DashboardNotifier` などの通知境界スタブで表します。スタブの内部だけが `std::cout` を使います。論点は「在庫変化を複数の通知先へどう伝えるか」であり、通信失敗、リトライ、通知テンプレート管理は本章では扱いません。
...
|---|---|---|---|
| 入力 | 商品ID、在庫変動、通知先リスト | 商品ID、在庫変動、通知先リスト、SMS通知先 | 通知先が増える |
| 判定 | 商品存在、在庫十分、閾値以下 | 同じ判定を維持 | 判定条件は変わらない |
| 加工 | 在庫更新後、登録済み通知先へ通知 | 在庫更新後、既存通知先とSMSへ通知 | 通知先の配布対象が増える |
| 出力 | 在庫更新結果と既存通知 | 在庫更新結果とSMSを含む通知結果 | 通知出力が増える |
...
| 通知処理を関数化 | メールや画面更新に名前が付く | 通知先の一覧と呼び出し判断は本体に残る | 最初の整理として有効 |
| 通知先リストを持つ | 同種の通知先を複数登録できる | 通知先の契約が揃っていないと扱いにくい | 固定種類なら有効 |
| 通知先の契約を統一する | 在庫管理本体は通知先の具体型を知らずに済む | 登録・解除と通知順の設計が必要 | 通知先が増えるため採用する |
