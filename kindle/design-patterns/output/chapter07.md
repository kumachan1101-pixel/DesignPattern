...
フェーズ2では、まだ「責務が混在している」とは判断しません。ここで行うのは、フェーズ1で見た仕様のうち、どの通知先・通知方法・しきい値判定が変わりそうかを見当づけることです。
...
| チャット通知 | 1-5の変更要求で追加 | フェーズ1の現状コードにはない | 新しい通知先として見る |

この表から、今回の検討対象は「在庫判定」と「通知先の増減」に絞れます。通知先が同じ場所に書かれて困るかどうかは、フェーズ3で変更を入れてから確認します。
...
| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| メソッド切り出し | 各通知先への呼び出し | 通知処理が名前で読める | メンバ・初期化・呼び出しの3箇所修正が残る |
| 具体型ごとのリスト保持 | 同種の通知先の集まり | 同種なら追加が登録だけで済む | 型ごとにリストとループが要り、新種で本体を直す |
| 共通契約でリスト保持 | 通知先全体を1つの契約へ | 種類が増えても登録するだけ、本体は回すだけ | 契約の導入と命名統一の初期コスト |

「通知先をリストで持ち、登録された相手へ順に通知するだけにする」という案は、通知の配り先を増やすと聞いて最初に思いつきやすい案です。この案は比較表の中だけで済ませず、ステップ2で具体型ごとのリストとして、ステップ3で共通契約のリストとして実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。

「通知処理を関数（コールバック）として登録する」という案も同じ2つ目の思考の実行です。登録先が具体クラスか関数かの違いで、本体が中身を知らず登録された相手を順に呼ぶだけ、という形は共通です。この章では、通知先ごとに解除や複数の通知種別を扱いやすい契約（インターフェース）の形で進めます。

なお、メソッド切り出しは1つ目の「共通化」、リスト保持と共通契約は「通知先を外から登録し、本体は登録された相手を順に回すだけにする」という2つ目の実行にあたります。
...
ステップ1で気になるのは「メンバ変数・初期化・呼び出しの3箇所」という修正の散らばりです。ここで、比較表の「通知先をリストで持つ」案を確かめます。通知先の追加がリストへの追加だけで済めば、少なくとも修正箇所が減るはずだ、という最初に思いつきやすい発想です。同じ送信メソッド名 `send` を持つ具体クラスをリストにまとめてみます。
...
| **解決策** | 通知分離構造：`INotification` インターフェースを介して通知先を登録リストで管理し、`InventoryManager` は `attach()` と `notifyAll()` だけを知る |
...
| 🔴 フェーズ6：対策検討 | 読者が思いつく「通知先をリストで持つ」案（ステップ2）を含めて比較し、具体型ごとにリストが要る残課題から、通知操作の契約を統一して外部から登録できるステップ3を採用した。 |
| 🟢 フェーズ7：対策実施 | 通知元はインターフェースのリストを保持するだけに留め、通知先を動的に登録・解除できる構造を実現した。この構造が 通知分離構造と呼ばれると知った。 |
