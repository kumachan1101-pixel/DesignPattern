...
|---|---|---|
| 契約と具体 | 課題ID1の文面・送信・受付結果を`INotification`の各具体へ、最終結果を`DeliveryStatusLog`と`SMSDeliveryCallback`へ置く | 通知手段の種類・数・文面・同期非同期 |
| 生成・所有・受け渡し | `main()`がDB・ログ・通知先・後日入口を所有し、通知元へ参照を渡して通知先を`attach()`する | 具体通知先・ログ・コールバックの実体と生存期間 |
| 公開入口からの実行 | 在庫更新が登録先へ一律`send(alert)`を呼び、保留IDは状態ログ、後日の最終結果はコールバックへつながる | 各手段の配信処理と完了時期 |
