...
★以下を図にできませんか。Lightning充電器ならアイフォン限定という絵と、TypeC充電器ならTypeC対応のアイフォンに加え色々な機器に差し替え可能なという絵で比較できるようにしてほしい。
> ```text
> 🔌 電源側 ──── 接続点 ──── 📱 利用する機器
>                     ↑
>          ここで何を受け渡すのか
...
> 各章では、業務データを保持するために次の標準コンテナを使います。コンテナ自体の実装を学ぶ必要はなく、「複数件」「キーで検索」「所有権を持つ参照」のどれを表しているかだけ押さえれば、設計上の役割を追えます。★以下表の下の3つは本書で扱っているの？扱っていないなら説明不要では？
>
> | 記法 | この本で表すもの | 読むときの要点 |
> |---|---|---|
> | `std::vector<T>` | 同じ型の値を順番に並べた複数件 | `push_back` は末尾への追加、`for` は全件の走査 ★取り出し方法は？|
...
>
> `std::make_unique<T>(引数...)`は、`T`のオブジェクトを生成し、その唯一の所有者になる`std::unique_ptr<T>`を返すC++14以降の標準ライブラリ関数です。`<T>`は関数テンプレートへ型を指定するC++の記法であり、デザインパターン固有の文法ではありません。
>
> 本書の掲載コードは設計上の依存関係へ焦点を絞るため、スマートポインタと`std::make_unique`を使いません。上の三つは実務コードを読むときの用語として示しています。各章では`std::vector`と`std::map`に何を保存し、誰が読み書きするかを本文で説明します。

> [!INFO] 生ポインタの使用について
> `IFoo* ptr` のように、指し先の破棄を自動管理しない通常のポインタを、C++では「生ポインタ」と呼びます。本書では、この記法を「別の実装へ差し替えられる参照」として使います。★自分で確保したポインタを解放するというコードを組むという意味でよいか？std::unique_ptrは、標準ライブラリの説明で登場するという事は使うという事でよいか。結局何が言いたいのかわからない。生ポインタも標準ライブラリも両方使うが、使い方が限定的という事を言いたいのか。
> 実務では `std::unique_ptr` や `std::shared_ptr` で寿命を管理する場面もありますが、ここでは所有権ではなく、利用側が具象クラスではなくインターフェースを参照する構造に焦点を絞ります。
...
各章のフェーズ6では、まず接続点の分離方法・配置場所・組み立て方法を決めます。その結果、責任配置や依存関係が異なる完成可能なシステムが複数残る場合だけ、目的・変更影響・導入コストを比較します。一つの完成構造しか導けない場合や、途中までしか課題を解かない案しかない場合は、数をそろえるための比較を置きません。
...
> この疑問の答えは「原則1だけでは説明しきれず、次の原則2が必要になる」という、まさにその境界線です。原則2で詳しく解説します。★３つのインターフェースが持っているが、この３つのインターフェース仕様がそれぞれ変わる場合、変更理由は３つあることになるのでは？この点の説明に絞ってほしい。INFOのタイトルも具体性が分かる形にしてほしい。実態クラスをメンバーに３つ持って使うのと変わらないのでは？「原則2：実装ではなくインターフェースに対してプログラムせよ」では、インターフェースが変わりそうな引数は分離する設計にすることでこのリスクを軽減している。その話を参照させてみるのはどうか。
...
- **特定ドメイン（対象とする業務領域やデータ構造）に特化しすぎているもの**：木構造の処理（Composite）や複雑なデータ構造と処理の分離（Visitor）などは、日常的なビジネスロジックの整理というより特定のデータ構造に対する特殊手札です。
