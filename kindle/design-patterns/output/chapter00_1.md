...
理由はシンプルです。
**パターンを「最初から目指す必要がある答え」として扱っているから**です。
...
> [!INFO] コンウェイの法則とは
> 1968年にメルヴィン・コンウェイが提唱した経験則です。「組織が設計するシステムの構造は、その組織のコミュニケーション構造を反映する傾向がある」というものです。営業チームと開発チームが別々に機能を担当していれば、システムも自然とその境界でモジュールが分かれやすい、という観察です。本書では「誰が変更を判断するか」を探る手がかりとして参照しています。ただし、組織構造とコード構造が必ず一致しなければならないわけではありません。
...
「誰が決めるか」が分からないとき——お客さんからの要求だが社内のどの部門が出したかわからない、伝達者は毎回同じ窓口担当者でそこからはたどれない——というケースがあります。そういうときは、人を探すのをやめて、**要求の内容そのもの**に着目します。

> **「この変更は、何の業務上の判断か？」**

「価格・条件に関する変更」なら料金や営業の領域、「文面・タイミングに関する変更」なら通知やマーケの領域、「保存先・フォーマットに関する変更」ならインフラの領域——という具合に、要求が属する業務の種類で分類できます。判断者の名前は見えなくても、**「何の業務判断か」という性質は要求の内容に含まれている**ことが多いです。
...
| 見える情報 | 使う問い |
|---|---|
| 誰が決めるかわかる | 「変更を決定するのは誰か？」 |
| 人は不明・要求内容はある | 「これは何の業務判断か？」 |
| 変更の履歴が見える | 「この変更はいつも一緒に来るか、別々か？」 |

たとえば「ECサイトの注文処理システム」を想像してください。
- 割引キャンペーンの適用条件を決めるのは**営業担当**です。
- 注文完了メールの文面を決めるのは**マーケティング担当**です。
- 注文データの保存先（データベースなど）を決めるのは**インフラ担当**です。

判断者はそこまで多くありませんが、これらの処理が1つの `OrderService` クラスに混在していると、マーケティング担当の要望でメール文面を変える際に、データベース保存処理に影響が出ないかテストする必要が生じます。だからこそ、決裁者ごとに分離するのです。
...
NGコードでは「どちらの変更でも `OrderService` を開くことになる」のに対し、OKコードでは「変更の決定者ごとに主な修正先を分けられる」ことが分かります。
...
> OKコードを見たとき、「各機能の担当都合で実装クラス（PremiumDiscountなど）が差し替わるなら、OrderService 自体も直す必要があるのでは？」と疑問に思うかもしれません。
> しかし、`OrderService` は `IDiscountCalc` や `INotifier` などの**インターフェース（契約）**しか知りません。そのため、営業チームが新しい割引ルールを追加しても、マーケティングチームが新しい通知手段を追加しても、`OrderService` のコード自体は1行も変わりません（main関数などの外側で差し替えるだけです）。
...
もちろん、GoFの23のパターンをすべて知る必要がないというわけではありません。残りのパターンを学ぶことで、特定のドメインや特殊な問題に対する設計の引き出し（ボキャブラリー）は増え、設計の知識も深まりやすくなります。23個のパターンは、多様な問題解決のカタログとして価値のあるものです。しかし、最初からすべてを暗記しようとして混乱するよりも、はじめにこの8つを通じて「型の適用プロセス」を脳にインストールすることが先決です。
...
> ここから先は「Decoratorパターン」の仕組みを先取りして解説しています。少し難易度が上がるため、今の時点で細部まで理解できなくてもまったく問題ありません。「コンポジションを応用すると、既存コードへの変更を抑えながら機能を追加できる場合がある」という雰囲気だけ掴んでいただければ十分です。読み飛ばして次の「第一部の説明」に進んでも構いません。詳細は第6章でじっくり解説します。
