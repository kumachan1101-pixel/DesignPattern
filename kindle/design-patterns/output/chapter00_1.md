...
| **第一部の説明** | 7つのフェーズ（フェーズ1〜7）・ケーブル比喩・段階的進化アプローチ（ステップ1〜5） |
...
機能が4つになれば組み合わせは指数的に増えます。通知先が2種類（Email・SMS）、追加機能が4種類（リトライ・ログ・制限・暗号化）あると、その組み合わせは 2 × 2^4 = 32クラスが必要になります。機能を1つ追加するたびに既存の全クラス分だけクラスを追加しなければならず、管理が不可能になります。
