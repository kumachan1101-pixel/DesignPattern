...
★全体的に上から目線です。こうしてください。とかあまり言い切らないでほしい、私はこういう理由でこう考えている、こういう実体験の元こうだ、というような言い方にしてほしい。正解はないです。こうしなさい、こうした方が良いなんて言う事はできません。全体的に口調は見直してほしい
...
★以下の実体験も入れてください。
★正直、形を暗記しようとしていた。ただ、いつ使えばよいかわからない。
★正直自分の判断でこの場合はこう使おうと使いこなせたことはなかったと思います。

★あなたのために書いたというような偉そうな言い方はやめて
...
★断定するような言い方はやめてほしい。
★私は、どうしたらデザインパターンのようなきれいな設計が出来るか
★そもそもデザインパターンの形にすることが目的ではなく、
★考え方を理解して、構造の本質を見抜いて、自分なりの最適解を
★導けないか思ったのです。そこで、デザインパターンの本質を理解して
★自分で思考を深めようと思い、この本を書き始めました。
★この本では思考プロセスを扱うので、少しでも読んでくださった方の
★参考になればよいと思ってます。
...
★以下なわけないでしょ。もっと私自身の本音で記載したいです。
★本音は私はろくにプログラミング出来ない、設計何てもってのほかの状態だったが、
★少しずつ設計が良いと変更しやすくなり、きれいになる構造が見やすい、など恩恵が多く
★設計の楽しさにはまっていったのです。ぜひ、読んでくださった方にも、その楽しみを知ってもらえると
★どのような思考をすればよいか人それぞれあると思いますので、自分なりのプロセスを発見して
★自分なりのパターンを見つけてもらえたらとてもうれしいです。

> **「このコードに変更が来たとき、どこを触れば済むかが、コードを読んで5分で分かる。」**

それだけです。派手な知識は一つも必要ありません。「変わる理由が混在している」と見抜く目と、「変わる理由の決定者が異なるコードを異なるクラスに分ける」という判断ができれば、デザインパターンは自然についてきます。
★以下の４つの接続のイメージがつきづらい。左から読むので、左を起点にして、左を充電器にしてほしい。
★４つ目も、起点は充電器のみです。起点は全て同じ、接続方法の違いで汎用性が変わることを説明したいのです。USB-CでもUSB-Cが接続できるiPhoneも入れてほしい。だから、ライトニング限定のiPhoneより、タイプCの方がメリットがある事もわかるはずです。変換アダプタ経由して何になるの？ライトニングですよね。
★できれば、リアル画像にしたいくらいです。もしくは、イメージできる画像にしてほしい
★chapter00_2のフェーズにも登場するか、すみわけはどうするのか。以下はフェーズの方に移動した方が良いのは
...
>
> 以下の図のように、接続の形は「具体か抽象か」「直接か間接か」の組み合わせで4つのパターンに分かれます。
>
> ```text
> 【1. 具体 × 直接】 iPhone専用端子に直接つなぐ（結合度が最も高い）
> 📱iPhone ────(Lightning)──── 🔌充電器
> 
> 【2. 抽象 × 直接】 世界標準の規格で直接つなぐ（相手が誰でもつながる）
>
> 💻PC/スマホ ────(USB-C)──── 🔌充電器
> 
> 【3. 具体 × 間接】 専用端子との「間」にアダプタを挟む（Adapterパターンの形）
> 📱iPhone ──[変換アダプタ]──(USB-C)── 🔌充電器
> 
> 【4. 抽象 × 間接】 標準規格の間に「ハブ（中継役）」を挟む（Facade/Mediatorの形）
> 💻MacBook ──(USB-C)──[マルチハブ]──┬── 📺モニター(HDMI)
>                                 ├── ⌨️キーボード(USB-A)
>                                 └── 🔌充電器(PD)
> ```
>
...
>
> パターンの名前を覚える前に、はじめにこの「分ける理由と接続の形の関係」を理解してください。
...
★以下表の内容が安っぽいです。もっとこの表を読んだだけで、どんなことに着目した章なのか、分かるように記載してほしい。
### 第一部：基本パターンの思考体験

| 役割                      | 内容                                                        |
| ----------------------- | --------------------------------------------------------- |
| **はじめに**                | なぜパターンを覚えても使えないのか・3つの原則                                   |
| **第一部の説明**              | 7つのフェーズ（フェーズ1〜7）・ケーブル比喩・段階的進化アプローチ（ステップ1〜5）★ステップなんてあったっけ？ |
| **第1章** Strategy        | 実行する振る舞い（アルゴリズムやルール）                                      |
| **第2章** Facade          | 複雑な外部連携の詳細                                                |
| **第3章** State           | 状態とそれに伴う振る舞い                                              |
| **第4章** Template Method | 処理の各ステップの実装                                               |
| **第5章** Command         | 実行する操作そのもの                                                |
...
★スコアはもう記載していないよね？現状と異なる記載がないかほかにも見直してほしい
**3. スコアリングは「チームの議論を整理する道具」**
フェーズ6の評価表のウェイト（変更容易性×3など）は本書の例です。チームによってプロジェクトの状況は異なります。大切なのは点数の結果ではなく、比較する前にチームで評価軸を合意するプロセスです。
...
具体的なクラス名（`EmailSender`）を呼び出し元が知っていると、Email→SMS→Pushに切り替わるたびに呼び出し元も変わります。なぜなら、呼び出し元のコードに「`EmailSender` を使う」という前提が書き込まれているため、`EmailSender` を `SmsSender` に替えるときに呼び出し元のコードも修正しなければならないからです。
...
こういう状況に直面したとき、取れる選択肢は3つあります。先に全体像を示します。

- **①型を合意・固定する**：シンプルだが型が変われば全シグネチャが変わる
- **②独自型でくるむ**：型の変更を独自型の内部に閉じ込め、シグネチャを守る
- **③void\*で型情報を持たせない**：シグネチャは変わらないが型安全を失う
...
★継承を使うなという事か。コンポジションより継承が良いパターンもあるのでは。考え方はしっかり説明するように。どちらでも実現可能ならコンポジションを優先せよなのか、考え方によってどちらか選択するのか、優先せよだと、継承が適切な場合でコンポジションが優先されないか。それでも良いならその理由もしっかり説明しないといけない。最後の方で説明入れてたね。読者は結局どうすればよいか迷ってしまわないか。もっと具体的に描いた方が良いのでは。こういう場合は継承でこういう場合はコンポジション。優先せよって言いきって大丈夫か。
...
★以下応用編はここで登場させるのが適切なのか。この本の概要の説明となる章ですよね。考え方はどこかに記載したいが、第１章から難易度上げても読みづらいと思います。
...
- **骨格（流れ）を固定して、各ステップの中身だけを変えたい**場合。「処理のステップ順序は変えず、各ステップの実装だけをサブクラスで差し替える」という場面では継承を意図的に使います。**骨格・ステップ分離**がこれに対応します。
  （→ 対策の詳細は、次の「第一部の説明」の「フェーズ6（ステップ6〜7）：対策検討」を参照）
