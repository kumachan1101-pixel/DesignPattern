...
各章のフェーズ6では、まず入力・実体・処理・結果の全体経路を決めます。その後、契約と具体の境界、実体の生成・所有・登録・選択・受け渡し・実行を、章で実際に起きる操作に沿ってコードへ落とします。その結果、責任配置や依存関係が異なる完成可能なシステムが複数残る場合だけ、目的・変更影響・導入コストを比較します。一つの完成構造しか導けない場合や、途中までしか課題を解かない案しかない場合は、数をそろえるための比較を置きません。
...
本書の各章でも一律にどちらかへ寄せません。利用側が契約型をメンバーとして受け取り、具体を差し替える箇所では、**具体側の契約実装と利用側のコンポジションを併用**します。一方、処理順という安定した骨格を基底クラスで固定する箇所では、目的を明示して実装継承を選びます。見るべきなのは継承の本数ではなく、**何を固定し、何を外から組み替えたいのか**です。
...
| 目的 | 推奨される手段 | なぜか |
| --- | --- | --- |
| **複数の具体を同じ入口から差し替えたい** | **具体側は契約を実装し、利用側は契約を持つ** | C++では前者を`public`継承で、後者をメンバーへの受け渡しで表す。2つは競合せず、セットで使えるため。 |
