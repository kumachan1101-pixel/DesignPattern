...
> | 記法 | この本で表すもの | 読むときの要点 |
> |---|---|---|
> | `std::vector<T>` | 同じ型の値を順番に並べた複数件 | `push_back`は末尾へ追加、`at(i)`は位置`i`の取得、`front()`は先頭の参照、`erase(...)`は指定要素の削除　★back()も使っているなら説明して。他にも関数使っているならこのライブラリ以外にも説明追加して |
> | `std::map<K, V>` | IDなどのキーから値を検索する保存領域 | `find` は存在確認、`at` は対応する値の取得 |
> | `std::deque<T>` | 先頭から順に処理する待ち行列 | `front()`は先頭を参照するだけ、`pop_front()`は先頭を取り除く |
>
...
>
> 具象クラスを直接持つ場合との違いは、契約が変わらない範囲にあります。たとえば`PremiumDiscount`を別の割引実装へ替えても、`IDiscountCalc::calculate(int)`が同じなら、`OrderService`は変更せず、組み立て側で差し替えられます。一方、具象クラスを直接持つと、実装の種類や生成方法の変更まで`OrderService`へ届きます。
>
> 契約の引数や戻り値も変わりやすい場合は、その変化を消すのではなく、どの型を境界へ出すかを見直します。次の原則2では、複数の値を安定した要求型・結果型へまとめる、または変化理由が異なる操作を別の契約へ分けることで、契約変更の波及を小さくする考え方を扱います。
