...
| **第9章** Strategy × State | サポートチケット管理：複数パターンの融合 |
| **第10章** Facade × Observer × Factory Method | 外部連携バッチ：依存の過多・変化の混在・生成と利用の混在 |
| **第11章** Template Method × Decorator × Command | レポート生成エンジン：複数の「変わる理由」が絡み合う複合課題 |
| **第12章** State × Observer × Strategy | 承認ワークフロー：状態変化・通知・判定ルールの混在 |
