...
| ファイル | 役割 | 内容 | 状態 |
| --- | --- | --- | --- |
| chapter00_1.md | **はじめに** | なぜパターンを覚えても使えないのか・3つの原則 | ✅ 完成 |
| chapter00_2.md | **第一部の説明** | 7フェーズの思考プロセス（フェーズ1〜7）・ケーブル比喩・案0〜案4 | ✅ 完成 |
| chapter01.md | **第1章** 変わるアルゴリズム ―― Strategy パターン | 増え続ける割引ルールをどう整理するか | ✅ 完成 |
| chapter02.md | **第2章** 変わる処理の組み合わせ ―― Facade パターン | 振り込み処理の裏側を、呼び出す側に見せない | ✅ 完成 |
| chapter03.md | **第3章** 変わる状態ごとの振る舞い ―― State パターン | 状態によって変わる振る舞いをどう整理するか | ✅ 完成 |
| chapter04.md | **第4章** 変わる処理のステップ ―― Template Method パターン | 手順は同じなのに3つのクラスに書き直していないか | ✅ 完成 |
| chapter05.md | **第5章** 変わる操作の種類 ―― Command パターン | 操作を取り消せるようにするにはどうするか | ✅ 完成 |
| chapter06.md | **第6章** 変わる機能の組み合わせ ―― Decorator パターン | 組み合わせが増えるたびにクラスを作っていないか | ✅ 完成 |
| chapter07.md | **第7章** 変わる通知先 ―― Observer パターン | 在庫が変わったとき、複数のシステムをどう連動させるか | ✅ 完成 |
| chapter08.md | **第8章** 変わる生成の種類 ―― Factory Method パターン | 決済手段が増えるたびに利用側も変わっていないか | ✅ 完成 |

### 第二部：応用演習

| ファイル | 役割 | 内容 | 状態 |
| --- | --- | --- | --- |
| chapter09_1.md | **第二部の説明** | 第一部との違い・複合問題の読み方 | ✅ 完成 |
| chapter09_2.md | **第9章** | 複数パターンの融合：複雑な問題への適用 | 🔄 執筆中 |
| chapter10.md | **第10章** | 依存の過多・変化の混在・生成と利用の混在 | 🔄 執筆中 |
| chapter11.md | **第11章** | 複数の「変わる理由」が複雑に絡み合う複合課題 | 🔄 執筆中 |
| chapter12.md | **第12章** | 状態変化・通知・判定ルールの混在 | 🔄 執筆中 |
...
> これこそが、次項で説明する「**原則2：インターフェースに対してプログラムせよ**」の力です。
...
> ここで紹介する「決定者を基準に分離する」という考え方は、原則1の説明に登場しますが、本書のすべての設計判断の根底にある考え方です。各章のフェーズ1〜2でも、この問いに戻ってきます。
...
> [!NOTE] 意図的に外すケース
> 「変わる可能性がほぼゼロ」と確信できる部分は、分離のコストをかける必要はありません。ヒアリングで「この仕様は創業以来変わっていない」と確認できた部分は、シンプルに一体で書くことが正しい判断です。
...
> [!INFO] コンポジションは「原則1」に反するのでは？という疑問
> 「コンポジションで機能を組み合わせたら、それは結局『変わる理由の決定者が複数いる（＝NG）』という状態に戻ってしまうのでは？」と思うかもしれません。
> しかし、ここが原則1の真髄です。原則1が禁止しているのは「1つのクラスの中に混在して書くこと」です。
> `LoggingDecorator` や `RetryDecorator` はそれぞれ別のクラスに分離されているため、ログ担当者がログの仕様を変えても、リトライ担当者のコードには一切影響しません（再コンパイルすら不要です）。
> そして、それらを「どう組み合わせるか」を決めるのは、利用側（`main`関数やDIコンテナ）の責任になります。それぞれの部品は自分の仕事だけに集中できる。これこそが原則1を守り抜いた姿なのです。

#### この原則を「あえて」外すとき

> [!NOTE] 意図的に外すケース
> 「is-a 関係が明確で、組み合わせの爆発が起きない」と判断できる場合は継承が素直な選択です。第0章で示した通り、後から組み合わせが増えた時点でコンポジションへの移行を検討してください。
...
- **骨格（流れ）を固定して、各ステップの中身だけを変えたい**場合。「処理のステップ順序は変えず、各ステップの実装だけをサブクラスで差し替える」という場面では継承を意図的に使います。**骨格・ステップ分離**がこれに対応します。
  （→ 手札の詳細はこの章の「フェーズ5〜6：課題定義・対策案の検討」を参照）
