...
デザインパターンは、繰り返し現れる設計上の問題と、
その文脈で有効だった解決の構造を整理したものです。
本書ではそれを、目の前の変更要求と複数の選択肢を比べて選んだ
**決断の結果**として捉え直します。
...
それだけです。派手な知識は一つも必要ありません。「変わる理由が混在している」と見抜く目と、変更を決める人だけでなく、変更のきっかけ・頻度・同時に変わる範囲を見て責任の境界を引く判断ができれば、問題に合った設計構造へ近づけると私は考えています。その構造が、既知のデザインパターンに対応することもあります。
...
>
> > [!INFO] 生ポインタの使用について
> > このサンプルでは他言語との横断的な比較とコードの簡潔化のため、生ポインタを使用しています。実際のプロダクションコードでは `std::unique_ptr` などのスマートポインタや参照渡しを使用して、所有権を明確にしてください。
>
...
本書で扱う8つのパターンは、次の3つの原則を手掛かりにすると、
それぞれがどの設計判断を具体化したものかを読み解きやすくなります。
...
> [!INFO] ここでの「カプセル化」は、変数をprivateにすることではない
> Java・Python・TypeScript を学んできた方は「カプセル化 ＝ フィールドを private にして getter/setter で保護すること」と覚えているかもしれません。それも正しい使い方ですが、本書での「カプセル化」はより広い意味を持ちます。**「変わる理由が異なるコードを、一つの変更単位（クラスや関数）の中に閉じ込めて外から隠すこと」**です。フィールドを隠すのではなく、**変わる処理ロジックそのもの**をひとかたまりにして隠すという点が違います。

この原則の核心は、**「誰の判断で、何をきっかけに変わるか」を手掛かりに、コードの責任を分離する**ことです。

決定者が異なる仕様は、別々の時期・理由で変わる可能性が高くなります。それらが同じクラスに混在すると、一方への要求でも他方への影響確認が必要です。そこで、決定者に加えて変更のきっかけ・頻度・同時に変わる範囲を確かめ、独立して変わる責任を分けます。分離そのものがゴールではなく、変更要求に対して影響範囲を説明できる境界を作ることが目的です。
...
多くの業務変更には、判断する人や組織がいます。ただし、変更のきっかけはそれだけではありません。法改正、外部APIの変更、障害、性能要件、技術基盤の更新などから生じることもあります。コンウェイの法則が示唆する組織構造は境界を探す有力な手掛かりですが、コード構造と常に一対一で一致させる規則ではありません。

> [!INFO] コンウェイの法則とは
> 1967年にメルヴィン・コンウェイが提唱した経験則です。「組織が設計するシステムの構造は、その組織のコミュニケーション構造を反映する傾向がある」というものです。営業チームと開発チームが別々に機能を担当していれば、システムも自然とその境界でモジュールが分かれやすい、という観察です。本書では「誰が変更を判断するか」を探る手がかりとして参照しています。ただし、組織構造とコード構造が必ず一致しなければならないわけではありません。
...
この原則を使うための最初の問いは、次のとおりです。

> **「このコードを変更するとき、変更を決定するのは誰か？」**

答えが複数なら、異なる変更理由が混在している可能性を疑います。ただし、同じ人が独立した複数の仕様を決めることも、複数チームが一つの契約を共同で決めることもあります。
**「誰が決めるか」に加えて、「何をきっかけに、どの頻度で、どの範囲が一緒に変わるか」を確かめることが、境界線を引く基準になります。**
...
判断者はそこまで多くありませんが、これらの処理が1つの `OrderService` クラスに混在していると、マーケティング担当の要望でメール文面を変える際に、決済処理に影響が出ないかテストする必要が生じます。この例では決定者も変更のきっかけも異なるため、責任を分離する根拠になります。
...
| CSV形式→JSON形式に変更したい | `ReportService` を開いて `format` を修正 | `JsonFormatter` を追加し、組み立て箇所で選択する |
| 計算ルールを変更しながら同時にフォーマットも変えたい | `ReportService` を1つ開いて両方修正——影響範囲が重なって混乱する | `CommissionCalc` と `CsvFormatter` をそれぞれ独立して修正——影響が交わらない |

NGコードでは「どちらの変更でも必ず `ReportService` を開く必要がある」のに対し、OKコードでは、計算規則と出力形式を別々に変更できます。実際の変更範囲には、実装クラスだけでなく、それを選ぶ組み立て箇所が含まれる場合もあります。
...
> `ReportService` は `Calculator` や `ReportFormatter` という**インターフェース（契約）**だけに依存しています。その契約が安定している限り、新しい実装の追加では `ReportService` を変更せず、main関数などの組み立て箇所で差し替えられます。一方、契約そのものが変われば、`ReportService` 側にも変更が及ぶ可能性があります。
...
これは基本形の応用です。「誰が決める？」は境界を探す有力な入口ですが、それだけで結論にはしません。変更のきっかけ・頻度・同時に変わる範囲も合わせて調べます。

> [!NOTE] この考え方は、全章を通じた基礎になります
> ここで紹介する「変更理由を基準に責任を分離する」という考え方は、本書の設計判断の基礎です。各章では、フェーズ1で依存や条件分岐などの事実を観察し、フェーズ2以降で決定者・変更のきっかけ・影響範囲を確かめます。
...
本書では、GoFのパターン名を最初から結論にせず、変動する部分と比較的安定した部分の境界を考えます。その結果が既知のパターンの構造に対応するなら、パターン名を共通語として利用します。

#### 補足：なぜ「すべてのパターン」が登場しないのか

本書はGoFの23個のパターンすべてを網羅していません。代わりに、8つの代表例を通じて、ほかのパターンにも応用できる設計の「思考の型」を扱います。

本書の8章は、業務コードで遭遇しやすい変更——ロジック、状態、外部連携、処理手順、操作、機能の組み合わせ、通知先、生成——から設計を考えるための入口です。GoFの問題領域を8種類で網羅する分類ではありませんが、問題から構造を選ぶ練習は、ほかのパターンを学ぶ際にも役立ちます。

- **似た構造でも目的を区別するもの**：Adapterは互換性のないインターフェースを変換し、Proxyは同じ窓口の手前でアクセス制御や遅延生成などを担います。FacadeやDecoratorと「間に部品を置く」点は似ていますが、解こうとする問題は異なります。
- **言語機能の背後にも概念が残るもの**：foreach構文の背後では、要素の走査方法をコレクションから分離するIteratorの考え方が使われています。
- **特定の構造で力を発揮するもの**：Compositeは木構造を同じ操作で扱う場面、Visitorは安定した要素構造へ処理を追加する場面で有効です。利用頻度だけで価値の高低は決まりません。
- **適用範囲を慎重に考えるもの**：Singletonはインスタンスを一つに制御する仕組みですが、変更可能なグローバル状態として乱用するとテストや依存関係を分かりにくくします。DIは依存を外から渡す手法であり、Singletonと一対一に置き換わるものではありません。

「この問題には、どの境界と接続方法が合うか」を考え、その構造が既知のパターンに対応するかを確かめる。
このプロセスを8つの代表例で繰り返すことで、パターン名を暗記するだけでなく、設計判断の根拠を説明する練習ができます。

もちろん、GoFの23のパターンをすべて知る必要がないというわけではありません。残りのパターンを学ぶことで、設計の引き出しと共通語は増えます。最初から形だけを暗記するのではなく、まず本書の8つを通じて、問題・制約・変更範囲から構造を選ぶプロセスを練習します。
...
一方、「通知する何か（`INotifier`）」という安定した契約だけを知っていれば、実装の切り替えによる変更を組み立て箇所へ寄せられます。ただし、`INotifier` の契約自体が変われば、呼び出し元にも変更が及びます。
...
*OKでは、INotifier の契約が保たれる実装変更なら、Notifierを変更せずに済む。*
...
>
> **C++ では、どちらも `class A : public B` と書きます。** `B` が純粋仮想クラス（メソッドがすべて `= 0`）なら「インターフェースを実装している」、`B` が実装を持つ具体クラスなら「具体クラスを継承している」です。構文が同じなので、`B` の中身を確認しないと意図が読めません。Java の `implements` と `extends` を区別している方は、C++ では両方が `public` 継承になることに注意してください。
...
> ここから先は「Decoratorパターン」の仕組みを先取りして解説しています。少し難易度が上がるため、今の時点で完全に理解できなくても問題ありません。「コンポジションを応用すると、既存の通知クラスへ条件分岐を書き足さずに機能を組み合わせられる」という雰囲気だけ掴んでいただければ十分です。読み飛ばして次の「第一部の説明」に進んでも構いません。詳細は第6章でじっくり解説します。
...
> `LoggingDecorator` と `RetryDecorator` は別々のクラスなので、ログの処理条件とリトライの処理条件を同じクラス内で混在させずに済みます。ただし、共通インターフェースや組み立て方の契約が変われば、両方へ影響する可能性はあります。目標は影響をゼロと断定することではなく、機能ごとの変更を追いやすい境界に置くことです。
...
3つの原則は関連していますが、すべての設計で必ずこの順番に適用するわけではありません。次の図は、責任を分け、接続方法を決め、再利用方法を選ぶときの代表的な確認順序です。
```mermaid
graph LR
