...
各章のフェーズ6では、まず契約と具体をセットで分け、次に具体の生成・注入・実行を一つの経路として組み立てます。その結果、責任配置や依存関係が異なる完成可能なシステムが複数残る場合だけ、目的・変更影響・導入コストを比較します。一つの完成構造しか導けない場合や、途中までしか課題を解かない案しかない場合は、数をそろえるための比較を置きません。
