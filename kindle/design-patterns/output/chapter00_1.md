...
>
> 接続点の形は、充電器と端末をつなぐケーブルの関係で考えるとイメージしやすいかもしれません。
> 起点を「充電器（左）」とし、接続方法の違いで汎用性がどう変わるかを見てみましょう。
>
> ```text
> 【1. 具体 × 直接】 専用ケーブルで直接つなぐ（結合度が最も高い）
> 🔌充電器 ────(Lightning)──── 📱iPhone(Lightning専用)
> 
> 【2. 抽象 × 直接】 世界標準の規格で直接つなぐ（相手が規格に合えば誰でもつながる）
> 🔌充電器 ────(USB-C)──── 💻PC / 📱iPhone(USB-C対応) / スマホ
> 
> 【3. 具体 × 間接】 専用端子との「間」にアダプタを挟む（Adapterパターンの形）
> 🔌充電器 ──(USB-C)──[変換アダプタ]── 📱iPhone(Lightning専用)
> 
> 【4. 抽象 × 間接】 標準規格の間に「ハブ（中継役）」を挟む（Facade/Mediatorの形）
> 🔌充電器(PD) ──┬──[マルチハブ]──(USB-C)── 💻MacBook
>                  ├── 📺モニター(HDMI)
>                  └── ⌨️キーボード(USB-A)
> ```
>
...
>
> パターンの名前を覚える前に、はじめにこの「分ける理由と接続の形の関係」をイメージしてみてください。なお、この比喩の詳細は後ほどの「第一部の説明（フェーズ4）」でもう一度登場します。
...
| **第一部の説明** | 現場の痛みを設計に変換する「7つのフェーズ」と、接続形態の診断 |
