...
**これが案1の限界です。** コードに手を入れようとした瞬間に、「取り消すメソッドがない」「金額やカテゴリをどこに保存していたか分からない」という問題が次々と現れます。現在の構造のままUndoを追加することは、根本的な構造変更なしには実現できないのです。
...
|  | 直接（直差し） | 間接（アダプター経由） |
|:---:|:---|:---|
| **具体**（専用規格） | **← 現在地**　iPhone → [Lightning] → Apple純正ドック | ライトニング直生え → ゲーム機用アダプタを挟む → ゲーム機 |
| **抽象**（汎用規格） | MacBook → [USB-C] → USB-C対応モニター | ライトニング直生え → Type-C変換アダプタを挟む → 各種機器 |
...
案2の問題は「どの具体クラスを使うか」という選択が `BudgetApp` と `ImportService` の両方に重複していた点でした。案3では、呼び出し元が具体クラス名を一切知らなくてよいよう、`IAction` というインターフェース型を間に挟みます。これにより、新しい操作が追加されても、呼び出し元は `IAction*` を受け取るだけで変更不要になります。
...
案3との違いは「履歴を誰が管理するか」という一点です。案3では `BudgetApp` や `ImportService` が `IAction*` を直接受け取って `execute()` を呼ぶため、「実行した操作を記録してUndoする」という責任は呼び出し元に残ります。案4では `ActionHistory` という仲介役を挟むことで、実行・記録・Undo/Redoの全責任をその1クラスに集約します。「履歴管理が不要なら案3、Undo/Redoが必要なら案4」という判断の分かれ目がここです。
...
対策案が揃いましたので、採用する必要がある案を決定するための「ものさし」を定義します。設計判断の根拠を透明にするため、比較表の提示に先立って評価軸を合意します。
...
**理由：** 今回の要件は「Undo/Redoの実装」と「将来的な操作種類の拡張（ヒアリングで口座移動・一括削除が予告されている）」の2点です。案3では履歴管理の責任が呼び出し元に残るため、Undoのたびに `BudgetApp` や `ImportService` を修正する必要があります。案4では `ActionHistory` がその責任を一手に引き受けるため、新しい操作が追加されても呼び出し元は変更ゼロです。「操作が増えるたびにUIコードを触る必要がなくなる」という具体的なメリットが、初期の実装コスト増加を上回ると判断して案4を採用しました。
...
採用した案4では、接続形態が **抽象×間接（Type-C変換アダプタ経由）** へと変化しています。
...
| 🟡 フェーズ5：課題定義 | UIとマネージャクラスの間の直接的なメソッド呼び出しを接続点として特定し、課題まとめ表を確定しました。 | 5-1〜5-4 |
| 🔴 フェーズ6：対策案検討 | 案1〜案4を2×2マトリクスで比較し、コスト天秤で操作をオブジェクトとしてカプセル化する案4を採用しました。 | 6-1〜6-5 |
