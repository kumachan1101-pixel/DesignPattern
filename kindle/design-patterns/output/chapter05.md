...
フェーズ4で「UIとマネージャを切り離すべき場所」が言語化できました。次のフェーズ5では、この「どこを分けるか」が決まった状態を受けて、「何を取り出すべきか」を具体的に特定します。

## 🟡 フェーズ5：課題定義 ―― 分離するターゲットを特定する

フェーズ4の分析により、問題の根本原因は「操作の意図（ボタンを押した事実）」と「実行手段（どのクラスのどのメソッドに何を渡すか）」という、変わる理由が違う2つのものが同じ場所に混在していることだと分かりました。

今回私たちが解くべき課題は、`UIButtons` の各ボタン処理に埋め込まれた **「マネージャクラスの具体的な呼び出し知識（メソッド名・引数の型と値）」を、丸ごと外に分離すること** です。

```cpp
void onAddExpenseClick() {
    // ↓↓↓ 今回分離するターゲット（変わり続ける実行手段の知識） ↓↓↓
    em.addExpense(1000, "Food");
...
`UIButtons` がこれらの呼び出しを直接持っているということは、「何を呼ぶか（メソッド名）」と「何を渡すか（引数の型・値）」の両方をUIクラスが管理しているということです。操作の種類が増えるたびに、この実行手段の知識の断片がUIクラスに積み重なっていきます。これがUndoを追加したとき、マネージャ側まで改修が波及した根本原因です。

最終的な目標は、`UIButtons` から「どのクラスの何をどう呼ぶか」という知識を完全に追い出し、ボタンが「実行依頼を渡すだけ」の存在にすることです。

フェーズ5でターゲットが明確になりました。次のフェーズ6では、この「呼び出し知識の塊」をどのように分離していくか、段階的なリファクタリングを通じて検討します。
