...
- 「いつ実行するか」と「何を実行するか」を分離することで、操作履歴やUndo（元に戻す）を容易に実装できる視点を持てるようになる　★操作履歴って、ちょっと具体的すぎますかね、例がこれしかないなら良いですが。いつ何を実行する？が、何故操作履歴の話になるのかわかりませんでした。もっと考え方を入れてほしい。
...
**現場の混乱：**
1.  **修正漏れの連鎖**：Aさんは `DELETE` のコード（1文字消す）をそのままコピペしてしまいました。本来、CUTは「選択範囲の長さ分」を消すべきですが、テストで1文字しか消えないバグが発生。
...
第0章の手札選択表を引くと：「要求の発生と実行のタイミングのズレ」→ **振る舞いのデータ化**（第0章 手札）。この原因に直接対応します。

### 5.4 却下した案：操作を個別のメソッドに切り出す

「巨大な `if-else` の中身がごちゃごちゃしているのが問題なら、それぞれの操作をコントローラーのメソッドとして切り出せばいいのでは？」

誰もが最初に思いつくアプローチを試してみましょう。
...
### 5.5 手札の適用（振る舞いのデータ化）：操作を「モノ（オブジェクト）」として独立させる
...
| 基準 | 却下した案（メソッド切り出し） | **振る舞いのデータ化**（Commandパターン） |
|---|---|---|
