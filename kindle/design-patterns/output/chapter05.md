...
Step 3との違いは「履歴を誰が管理するか」という一点です。Step 3では `BudgetApp` や `ImportService` が `IAction*` を直接受け取って `execute()` を呼ぶため、「実行した操作を記録してUndoする」という責任は呼び出し元に残ります。Step 4では `ActionHistory` という仲介役を挟むことで、実行・記録・Undo/Redoの全責任をその1クラスに集約します。「履歴管理が不要ならStep 3、Undo/Redoが必要なら案4」という判断の分かれ目がここです。
...
| 🔴 フェーズ6：対策案検討 | Step 1〜Step 4を2×2マトリクスで比較し、コスト天秤で操作をオブジェクトとしてカプセル化するStep 4を採用しました。 | 6-1〜6-5 |
