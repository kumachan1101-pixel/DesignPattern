...
> **中間コードの継続条件：** 各ステップは操作の実行・取消・履歴管理だけを比較します。`CategoryDatabase` のカテゴリ検証と金額検証は全ステップで維持します。フェーズ1の金額・カテゴリIDという公開入力は、操作オブジェクトの生成時に保持され、最終的に `execute()` / `undo()` から実行先へ渡ります。
