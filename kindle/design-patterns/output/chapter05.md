...
---
> **📌 問題（確定）**
> 操作の種類（支出追加・収入追加など）が増えるたびに、`UIButtons` の `undo()` 内の条件分岐が連動して肥大化する。今回のUndo機能追加というヒアリング済みの変更だけでも、呼び出し元UIと複数のマネージャクラスを同時に修正しなければならない。操作の意図（何をしたいか）と実行手段（どのクラスのどのメソッドを呼ぶか）が同じ場所に混在しているため、一つの変更要求が複数のクラスに連鎖するのだ。
---
