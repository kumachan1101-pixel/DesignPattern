...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。利用者が「食費1000円」の支出を1件登録すると、内部は入力を検証→支出として記録→残高を再計算の順に進みます（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
| 要求ID3 | 不正入力を拒否し、記録件数と残高を変えない | `CategoryDatabase`、各Manager | エラー前後の件数・残高一致<br/>**判定:** 合格 |
| 要求ID4 | 保存済み記録から残高を求める | `LedgerRepository` | 操作・取消・再実行後の数値を照合<br/>**判定:** 合格 |
...
| 要求ID7 | バッチ途中失敗時に当該バッチだけを逆順ロールバックする | 一括操作、`ActionHistory` | 開始前の数値へ戻り、過去履歴は維持<br/>**判定:** 合格 |
