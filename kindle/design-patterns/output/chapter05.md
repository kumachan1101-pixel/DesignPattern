...
| 最後を削除 | 最後に追加したデータを削除する | そのデータ分だけ元に戻る | ユーザー / プロダクトマネージャー |
...
コードを読む前に、このシステムがどんな入力に対してどんな出力を返すかを確認します。フェーズ1〜フェーズ6の途中ステップは一部の行（Undo・Redo・CSVインポート）を実現できないことがありますが、フェーズ7の最終コードはすべての行を実現します。

| 操作 | 入力 | 処理内容 | 残高の変化 |
| --- | --- | --- | --- |
| 支出登録 | 1,000円／食費 | 支出をDBに保存し画面を更新する | 残高が1,000円減る |
| 収入登録 | 5,000円／給与 | 収入をDBに保存し画面を更新する | 残高が5,000円増える |
| 最後を削除 | （直前に登録した1件） | 最後に追加したデータをDBから削除し画面を更新する | そのデータ分だけ残高が元に戻る |
...
## 🟣 フェーズ2：仮説立案 — 何が変わるか観察し、ヒアリングで裏付ける
...
フェーズ3で確認した「変更の辛さ」は、コードのどこから来ているのでしょうか。

### 4-1：痛みの根源を探る（観察と原因）

| **観察した症状（痛み）** | **構造的な原因（痛みの根源）** |
|---|---|
| Undoのロジックが各操作マネージャの中に分散・重複してしまう | 「操作の実行」と「操作履歴の管理」という、本来別の理由で変わる知識が結合しているから |
| 新しい操作ボタンが増えるたびに、UI側とUndoロジック側の両方に修正が必要になる | 呼び出し元（UI）が、個別のマネージャクラス（Expense/Income）を直接知ってしまっているから |

### 4-2：変わるもの / 変わってほしくないもの

コードの中で、どう混在しているかを確認します。
...
「どんな操作を実行するか」という知識と、「それをどう履歴に残すか」という知識が同じ場所に直書きされていることが問題です。

### 4-3：接続形態を診断する

現在、UIクラスが個別のマネージャを直接呼び出しています。これは **「具体×直接」** の接続形態です。
新しいボタン（操作）が増えるたびに、UIクラスにメソッドが追加され、配線が直接ハードコードされてしまいます。
...
**具体×直接のままでよい場面**：操作の種類が今後増えない確証があれば、現状の直接呼び出し（具体×直接）で十分だと考えます。接続形態の選択は「**生産者が変わるかどうか**」で決まります。今回は操作の追加リスクがヒアリングで確認済みなので、次のフェーズで生産者を差し替えられる設計を検討します。

---
> **📌 課題（確定）**
> 切り離すべき境界は「操作の意図（ボタンを押した事実）」と「実行手段（クラス名・メソッド名・引数の組み合わせ）」の間にある。UIは `int` や `string` を渡す先の具体的な知識を持たず、「何かを実行せよ」という依頼だけを送れる形にする必要がある。そのためには、実行手段の生産者を差し替え可能なオブジェクトとして切り出し、UIから分離することが課題だ。
---
...
いきなりパターンを適用するのではなく、手続型の限界をステップで確認しながら設計を進めます。

### 案1：単純な関数化（巨大な分岐）
直前の操作を1つだけ記憶し、Undoが呼ばれたらif文で分岐する素朴なアプローチです。

```cpp
class ActionHistory {
    string lastOp = "";
public:
    void record(string op) { lastOp = op; }
    void undo() {
        if (lastOp == "Expense") {
            // 支出の取り消し処理（残高を戻す、DBから削除など）
            expenseManager.removeLast();
        } else if (lastOp == "Income") {
            // 収入の取り消し処理
            incomeManager.removeLast();
        } else if (lastOp == "Delete") {
            // 削除の取り消し処理（データを復元）
            deleteManager.restoreLast();
        }
    }
};
```
- **メリット**：とりあえずUndoは動く。
- **デメリット**：操作が増えるたびにこの `undo()` の巨大条件分岐（if-else）が際限なく肥大化する。操作の詳細知識が履歴管理クラスに漏れ出している。

### 案2：インターフェース化の伏線（処理の切り出し）
それぞれの取り消し処理を関数として切り出し、条件分岐を少し整理します。

```cpp
class ActionHistory {
    string lastOp = "";
    void undoExpense() { expenseManager.removeLast(); }
    void undoIncome() { incomeManager.removeLast(); }
public:
    void undo() {
        if (lastOp == "Expense") undoExpense();
        else if (lastOp == "Income") undoIncome();
    }
};
```
- **メリット**：各処理の中身は整理された。
- **デメリット**：本質的な巨大条件分岐は残ったまま。履歴管理クラスが「すべての操作の取り消し方」を知らなければならない問題は未解決。

### 案3：抽象×間接への進化（パターンの発見）
「操作」そのものを1つのオブジェクト（カプセル）にしてしまえば、履歴管理クラスは中身を知る必要がなくなります。
...
public:
    void execute() override { expenseManager.addExpense(); }
    void undo() override { expenseManager.removeLast(); }
};
...
案1にあった巨大な `if (lastOp == "Expense")` の分岐は消滅しました。これは分岐がなくなったわけではなく、オブジェクト（`AddExpenseCommand`）に知識を移動させ、ポリモーフィズムによって呼び分けているからです。履歴管理クラスは「`undo()` を呼べば元に戻る」という事実だけを知っていればよくなりました。

> **【Command】パターン** ―― 「要求」や「操作」そのものを1つのオブジェクトとしてカプセル化し、呼び出し元と実行内容を分離する構造。

### 採用案の決断

案3を採用します。これによって、UIとマネージャが切り離され、操作履歴の管理が劇的にシンプルになります。
...
| **課題** | 実行手段の生産者（クラス名・メソッド名・引数の組み合わせ）をUIから切り離し、差し替え可能なオブジェクトとして独立させる必要がある |
...
| 🟡 フェーズ5：課題定義 | 接続点で流れるのはメソッド名・引数の組み合わせ、変わるのは具体的な呼び出し先（生産者）であることを特定した | フェーズ5本文 |
...
**得られること2**（呼び出し元の依存過多の判断）：フェーズ4で、ボタンが具体的な実行ロジックを直接知っている状態を「依存の過多」と診断しました。この診断ができると、なぜ操作の追加・変更のたびにUIコードまで修正することになるのかが接続の形から読めるようになります。
