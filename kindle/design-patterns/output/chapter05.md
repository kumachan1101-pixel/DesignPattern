...
**ある操作を実行するボタンやメニューなどの呼び出し元が、実行内容そのものを直接知っていると、操作の追加や取り消しといった機能拡張が難しくなるのではないかと思いませんか。「何をする必要があるか（操作）」と「どう実行するか（処理）」が同じ場所に混在していることが、その理由ではないかと考えます。**
...
「支出登録」と「収入登録」の2種類しかないのは、このシステムが「まず正確に記録する」ことを最優先に設計されているからです。家計簿アプリの初期リリースでは操作の種類を絞ることが一般的で、後から口座移動・定期登録などを追加する段階的な構成が多くのプロダクトで採用されています。また「判断するチーム」の列は、誰の意思決定で操作が変わるかを明示するためのものです。UIに表示する操作の追加・削除はプロダクトマネージャーが判断し、その実現方法はエンジニアが担う——この2つは変わる理由が異なります。
...
現時点では「取り消し（Undo）」や「削除」機能はありません。これは意図的な仕様です。操作の取り消しはデータ整合性の管理が必要になるため、初期リリースでは対象外とするのが現場では珍しくありません。言い換えると、「Undoは要望が出てから追加する機能」として後回しにされやすく、この章で扱う変更要求のシナリオは現実の開発現場でよく起きる状況を反映しています。
