...
P1は「操作の具体手段が利用側と履歴へ漏れる課題」です。フェーズ6では、操作追加でどこまでの変更を許すかを理想グラフにします。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた課題——**実行手段をオブジェクトとして差し替え可能にし、実行・取消・履歴を同じ単位で扱う接続点を作る**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを変換して探します。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる
...
|---|---|---|---|
| P1 | 操作追加の影響からUI分岐と履歴の種別判定を外す | UIが知る実行先・引数・逆操作を切る。成功操作だけ履歴へ積み、Undo/Redo失敗時にスタックを壊さない | 操作追加でUIとActionHistoryへ種別分岐を追加しない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | UIの呼び出し処理へ名前を付ける | 実行手段がUIへ漏れる箇所が見える | 入力値とexecute/undoを同じ操作単位へまとめ、履歴はその契約だけを扱う |

ここからP1のUI呼び出し・履歴・逆操作に該当するコードだけをおさらいします。
