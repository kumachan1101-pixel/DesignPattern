...
- 利用者の操作は、収入か支出かという操作内容と金額をセットで扱う。
- 操作が成立すると、収支データの登録、残高更新、履歴保存が連続して起きる。
- 出力は残高だけではなく、収支一覧や履歴状態も含むため、後から操作を取り消す余地がある。
...
この設計変更により、新しい操作は操作記録構造クラスとして追加し、UIやバッチへ割り当てる組み立て箇所を変更できます。`ActionHistory` へ操作種別ごとの条件分岐を追加せずに、同じUndo/Redoの仕組みを利用できます。
...
各操作クラスは「実行」と「取り消し」の両方を知っています。操作を増やすときは操作記録構造クラスを追加し、画面やバッチへ割り当てる組み立て箇所を変更します。`ActionHistory` の履歴管理ロジックには、操作種別ごとの分岐を増やさずに済みます。
...
public:
    void execute(IAction* cmd) {
        ...
        if ((int)undoStack.size() > MAX_HISTORY) {
            undoStack.pop_front();  // 最古の操作記録構造を削除する
        }
        redoStack.clear();
    }
    void undo() {
        if (undoStack.empty()) return;
        // undo()が失敗した場合は、操作記録構造をundoStackに残す
        IAction* cmd = undoStack.back();
        undoStack.pop_back();
        cmd->undo();
    ...
    void redo() {
        ...
        // execute()が失敗した場合は、操作記録構造をredoStackに残す
...
`ActionHistory` は `IAction*` として操作記録構造を受け取り、スタックで管理します。具体的な操作クラスは知りません。Undo/Redoの実行に失敗した場合は、対象操作記録構造を元のスタックに残す順序にしています。
...
確認したいこと：外部から見える収支結果を保ちながら、操作の実行・取り消し・再実行が操作記録構造として履歴に接続されていること
...
この実装により、UIは操作記録構造のポインタを履歴へ渡すだけでよくなり、支出・収入ごとの実行手順を管理する必要がなくなりました。コマンドの生成と画面への割り当ては組み立て側に残ります。
...
操作の意図をオブジェクトとして独立させたことで、UIは操作の実行方法を知る必要がなくなりました。代わりに、操作ごとのクラス、操作記録構造の所有権、履歴上限、失敗時の整合性を設計する必要があります。
