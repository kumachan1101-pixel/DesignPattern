...
フェーズ1の仕様表を振り返ります。現在のアプリには「支出登録」「収入登録」という2種類の操作があります。今回のUndo機能追加の要件と合わせて、変化が予想される仕様があります。

- **操作の種類**：アプリが成長するにつれ、新しい操作（振り替え、予算設定など）が追加される可能性があります
- **操作の取り消し・やり直しの仕組み**：今回の要件で初めて導入されますが、履歴の管理方法は今後の要件次第で変わりうる部分です

一方、「ボタン押下を検知してアクションを実行する」というUIの基本的な流れは、アプリの操作体験として安定している部分です。

**仮説：操作の種類と履歴管理の仕組みは、今後も変更・拡張が続く可能性がある。**

この仮説が実際の運用に合っているかどうかを、次のヒアリングで確認します。

#### 補足：変動・不変の仮説テーブル

フェーズ1の担当者情報を材料に、Undo機能の実装に向けた仮説を立てます。
...
ヒアリングで Redo 機能や口座移動・一括削除といった操作の追加が予告されました。これらが来たとき、仕様がどう変わるかを整理しておきます。

| 操作 | 現在 | 将来（数ヶ月後） |
|---|---|---|
| 支出追加（Undo対応） | 今回追加 | 変更なし |
| 収入追加（Undo対応） | 今回追加 | 変更なし |
| Redo機能 | —（なし） | 追加予定（取り消しを戻す） |
| 口座間移動 | —（なし） | 追加予定（Undo対象にもなる） |
| 一括削除 | —（なし） | 追加予定（複数データに作用） |

これらが追加されるたびに、`undo()` / `redo()` 内の条件分岐と、各マネージャクラスへの依存が積み上がっていくことが見えてきます。
...
さらに、ヒアリングで予告された Redo 機能や口座移動操作の追加も視野に入れると、`undo()` に新たな操作種別ごとの分岐が増え、`redo()` でも同様の条件分岐が必要になることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、操作が増えるたびに UIButtons が知るべき情報と条件分岐がさらに積み重なる構造は変わりません。
...
フェーズ4で「UIとマネージャを切り離す場所」が言語化できました。「どこを分けるか」は明確です。次のフェーズ5では、その境界で実際に何が流れているかを値・型のレベルで具体化し、「何が変わり、何が変わらないか」を明確にします。
...
分離の境界は決まりました。次は「操作ごとのクラスをまとめるか、個別に分けるか」という粒度の問題です。

ヒアリングで把握した操作の種類と、それぞれが変わる理由を確認します。

| 操作 | 変わる理由 |
|---|---|
| 支出追加・Undo | 家計簿の支出ルール（UIデザイナー・開発担当） |
| 収入追加・Undo | 家計簿の収入ルール（同じUIデザイナー・開発担当） |
| 口座移動（将来） | 口座管理ポリシー（同じ担当） |
| 一括削除（将来） | UI操作フロー（同じUIデザイナー） |

すべて同じ担当が管轄していますが、変わり方は独立しています。支出追加のルールが変わっても収入追加の処理には影響しません。口座移動の仕様が変わっても一括削除には波及しません。各操作は**独立して変わる**ため、操作ごとに別クラスへ分離します。どの操作も `execute()` / `undo()` という同じインターフェースを持ちます。

→ **操作ごとに別クラスへ分離し、同じ `ICommand` インターフェースで束ねる**
