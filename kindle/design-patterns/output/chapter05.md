...
まずは、家計簿アプリが何を入力として受け取り、どの処理で加工し、何を出力するのかを整理します。ここでは設計の良し悪しを判断せず、現状を事実としてそろえます。
...
フェーズ2では、まだ「責務が混在している」とは判断しません。ここで行うのは、フェーズ1で見た仕様のうち、どの操作種類・呼び出し・履歴管理が変わりそうかを見当づけることです。
...
| 案 | 何を持つか | 期待できる効果 | 限界 |
|---|---|---|---|
| 直前操作をフラグで記憶 | 直前1件の種別・値 | 1回のUndoが最小コストで実現できる | 2回以上のUndoに対応できない |
| 操作を構造体にして履歴へ積む | 種別・金額・詳細のレコード列 | 複数回のUndoが実現できる | 取り消しの分岐が履歴側のswitchに集中する |
| 操作をオブジェクトにする | 実行と取り消しを持つ操作 | 履歴は中身を知らず取り消せる | 操作クラスと組み立て登録の初期コスト |

これらのうち「操作を構造体にして履歴へ積み、取り消しは種別で分岐する」案は、履歴を残すと聞いて最初に思いつきやすい案です。この章では、この案を比較表の中だけで済ませず、ステップ2で実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。フラグで記憶する案はステップ1で、操作のオブジェクト化はステップ3でコードにします。

なお、フラグ記憶・構造体履歴は、共通する記録処理をまとめる1つ目の「共通化」の実行です。操作のオブジェクト化は、変わる取り消し処理を操作自身へ持たせて外から注入し、履歴は決まった流れ（`undo()` を呼ぶだけ）を回すという2つ目の実行にあたります。
...
ステップ1の「1回しか取り消せない」問題を解決するには、過去の操作をスタックとして積み上げればよいと考えられます。これは比較表の「操作を構造体にして履歴へ積む」案にあたり、履歴を残すと聞いて多くの人が最初に書く形です。操作の種別と値を構造体にまとめてスタックへ積み、取り消すときはトップから取り出せばよいはずです。
...
| 🔴 フェーズ6：対策検討 | 構造体履歴＋switchという読者が思いつく案（ステップ2）を含めて比較し、取り消し分岐が履歴側に集中する残課題から、`IAction` インターフェース＋`ActionHistory` 仲介役のステップ3を採用しました。 | フェーズ6本文 |
