...
現時点で扱う操作は、支出登録と収入登録です。取り消しや操作履歴の保存は扱いません（正常系にないため、上の正常系図にも描いていません）。
...
現状コードは、DBトランザクション、履歴、一括処理を扱いません。まず、支出・収入の入力が各Managerを通り、`UIButtons` の残高へ直接反映される出発点を確認します。
