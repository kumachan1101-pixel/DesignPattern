...
| 機能ID1 | 収入を記録する | 要求ID1 |
| 機能ID2 | 支出を記録する | 要求ID2 |
| 機能ID3 | 不正な記録入力を拒否する | 要求ID3 |
| 機能ID4 | 保存済み記録から残高を計算する | 要求ID4 |
...
|---|---|---|---|
| 機能ID1 | 継続<br/>根拠: — | 収入を記録する | 要求ID1 |
| 機能ID2 | 継続<br/>根拠: — | 支出を記録する | 要求ID2 |
| 機能ID3 | 継続<br/>根拠: — | 不正な記録入力を拒否する | 要求ID3 |
| 機能ID4 | 継続<br/>根拠: — | 保存済み記録から残高を計算する | 要求ID4 |
| 機能ID5 | 追加<br/>根拠: 変更ID1、変更ID2、変更ID3 | 操作を履歴化し、取消・再実行・バッチ補償を行う | 要求ID5、要求ID6、要求ID7 |
...
フェーズ5の一表から、課題ID1と課題ID2を同時に解く構造を直接決めます。UIは変わりやすい入力端なので安定側には置きません。守るのは、画面・バッチ・APIのどこからでも「操作を依頼する」だけでよいアプリケーション境界と、一つの台帳が残高を正しく持つ契約です。
...
|---|---|---|---|
| 機能ID1 | 継続 | 要求ID1 | 収入追加を回帰確認<br/>**判定:** 合格 |
| 機能ID2 | 継続 | 要求ID2 | 支出追加を回帰確認<br/>**判定:** 合格 |
| 機能ID3 | 継続 | 要求ID3 | 不正入力時の件数・残高不変を回帰確認<br/>**判定:** 合格 |
| 機能ID4 | 継続 | 要求ID4 | 保存済み記録からの残高計算を回帰確認<br/>**判定:** 合格 |
| 機能ID5 | 追加 | 要求ID5、要求ID6、要求ID7 | 取消・再実行・途中失敗時の逆順ロールバックを受入確認<br/>**判定:** 合格 |
