...
- **出力：** 登録結果、記録件数、変更後残高を返し、不正入力では件数も残高も変えない。
- **掲載コードでの代替：** 画面のボタン操作は `main()` からのメソッド呼び出し、カテゴリDBと記録はメモリ上の登録表、表示は標準出力で表す。検証、記録、残高計算は実際に行う。
...
| 要求ID4 | 保存済み収入合計から支出合計を引いて残高を求める | 操作のたびに件数と残高を照合できる |
...
| 要求ID4 | 継続<br/>根拠: — | 保存済み記録から残高を求める | 操作・取消・再実行後の件数と残高を照合できる |
...
要求ID1〜要求ID4は継続（変更前＝変更後）のため対照表には載せません。変更後ベースラインで内容を確認できます。
...
Undo後の残高は`4000円`から、収入登録前の`-1000円`へ戻りました。変更ID1は実現できています。ただし、そのために、各Managerへ逆操作を追加し、`HistoryItem`へ復元データを持たせ、`UIButtons`へ操作種別の分岐と残高更新を同時に追加しました。さらに変更ID2のRedoと変更ID3の一括補償でも、この三つの責任を連動して修正しました。
...
1つ目は、変更ID1・変更ID2のために`UIButtons`内の`undo()`と`redo()`が操作種別を判定し、変更ID3では成功済み操作と逆順補償まで管理することです。三つの確定要求が同じ呼び出し元の条件分岐へ集中しました。
