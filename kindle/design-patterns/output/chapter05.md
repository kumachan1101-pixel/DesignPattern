...
コードを読む前に、このシステムがどんな入力に対してどんな出力を返すかを確認します。次の表は章全体で目指す動作です。フェーズ1〜フェーズ6の途中ステップでは一部だけを確認し、フェーズ7の最終コードで全項目を扱います。
...
| フェーズ7 | 支出・収入・最後を削除・複数Undo・Redo・CSV一括登録とロールバック |

既存の「最後を削除」ボタンは、履歴の末尾を1回取り消す操作として `ActionHistory::undo()` に対応づけます。新しいUndoボタンと内部処理は同じですが、画面上の入口は既存仕様に合わせて残せます。
---
...
現在の `UIButtons` クラスと各マネージャクラスの接続は、すべての操作ロジックがUIクラスの基板上に「直差し」されている状態だと診断できます。今、呼び出し元と実行クラスは抽象化がなく、直接依存しています。この状態を整理するための言葉が「具体×直接」という接続形態の名前です。これは **「具体×直接」** と言えます。
...
---
> **📌 原因（確定）**
> `UIButtons` がボタン押下という「操作の意図」だけでなく、「どのクラスのどのメソッドを何の引数で呼ぶか」という実行手段まで直接知っている状態（具体×直接）になっている。ヒアリングで確認された「口座間の移動」「一括削除」といった操作の追加が想定される今の状況では、操作が増えるたびに UIButtons の改修が必ず伴う構造は、変更コストが見合わない。
---

（原因は「具体×直接」の接続形態にあると明確になりました。次のフェーズ5では、その接続点で実際に何が流れているかを値・型のレベルで見ていきます。）
...
public:
    ...
    // 既存の「最後を削除」は、履歴末尾を1回取り消す
    void onDeleteLastClick() { history->undo(); }
    void onUndoClick() { history->undo(); }
...
int main() {
    ...
    app.onDeleteLastClick();         // 既存仕様：最後の収入を削除
