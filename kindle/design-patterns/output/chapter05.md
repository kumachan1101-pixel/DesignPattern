...
**このフェーズで決めること：** 実行と取消の対応をどの単位で受け渡すか、台帳と履歴に何を持たせるか、それらを誰が生成・所有して入口へ渡すかをコードで示します。

まず、契約、具体、生成・所有、受け渡し、実行を一枚の構想へまとめます。

| 接続点を変える観点 | システム全体での設計判断 | 変えたくない側が知らなくなる詳細 |
|---|---|---|
| 契約と具体 | 課題ID1の実行値・逆操作を`IAction`の各具体へ、課題ID2の収支と残高更新を`LedgerRepository`へ置く | 操作種別・引数・取り消し方・残高計算 |
| 生成・所有・受け渡し | `main()`が台帳・Manager・`ActionHistory`・`BudgetApp`を所有し、利用時に作ったActionへManagerを渡して履歴へ受け渡す | 具体操作と履歴・台帳の実体 |
| 公開入口からの実行 | `BudgetApp`は操作を履歴へ渡し、履歴が同じAction実体の`execute()`／`undo()`を呼ぶ | 操作別の実行・取消と残高更新の詳細 |

**構想上のコード経路：** `main()`（操作を生成）→ `BudgetApp::onAddExpenseClick()`（操作を渡す）→ `ActionHistory::execute()`（履歴の骨格）→ `IAction::execute()`（契約）→ `AddExpenseAction::execute()`（具体処理）
...
**変更前から抜き出す箇所：`UIButtons::undo()`** ―― 3-1の取消処理（対策前）
...
**ここで確認するコード：`AddExpenseAction`（クラス全体）** ―― 支出登録の操作
...
**変更前から抜き出す箇所：`UIButtons::onAddExpenseClick(int, const string&)`** ―― 3-1の登録処理（対策前）
...
**課題ID1（操作軸）。** 直前の契約コードで見た `AddExpenseAction` の対になるものを見ます。

**ここで確認するコード：`AddIncomeAction`（クラス全体）** ―― 収入登録の操作
...
**具体が2つしかありません。** それでも契約を作るのは、2-4のリスクID1が「削除・一括削除などの追加」を挙げているからです。**種類が増える見込みがあるので、契約コードで契約まで進むと決めました。**
...
**ここで確認するコード：`ActionHistory`（メンバー）** ―― 履歴が持つもの
...
class ActionHistory {
    ...
    // …execute / undo / redo / printLog（構想の確認で示した骨格）…
};
```
...
構想で定めた接続点は、入口が持つ `history` と、入口が生成して履歴へ渡す操作です。ここでは両方を一続きのコードで確かめます。

**まず履歴の所有を確認します。** 持つ者に必要な条件は、構想表から分かります。
...
**ここで確認するコード：`main()`** ―― 組み立てと、操作の生成（対策後）
...
ここまでで、台帳、履歴、入口、操作の生成と `history.execute(action)` による受け渡しまで確認できました。次に履歴の受取側を見て、同じ操作が `IAction*` として所有・実行されることを確かめます。

---

##### 受け渡し：操作を入口と履歴へ渡す

**構想で仮置きした骨格には、`cmd->execute()` としか書いてありません。** `AddExpenseAction` とは書いていません。それでも実行すれば支出が登録されます。**ここで見るのは、いつ・何によって、その実装が入るのかです。**

**ここで確認するコード：`main()`** ―― 生成・所有のコードで作った操作を渡す行
...
前節で、`main()`が所有する1つの`ActionHistory`を`BudgetApp`へ渡し、利用時に生成した`IAction*`を同じ履歴へ受け渡しました。履歴は受け取った操作を所有し、`execute()`、`undo()`、`redo()`を契約から呼びます。

操作の種類を追加しても履歴の手順は変わりません。変わるのは新しいActionと、それを生成する利用入口です。生成、所有、履歴への注入、実行と取消が同じAction実体でつながっています。
...
**変更前から抜き出す箇所：`main()`** ―― 3-1の支出登録と取消（対策前）
...
**守る範囲との照合：** **操作を依頼する入口**を保っていることを、いま確認しました。ボタンを押すと1件の操作が実行される、という形は変わっていません。**表示**も、実行のたびに内容と残高の前後を出す形のままです（出す場所が台帳側の描画へ移りました）。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、分離から実行までのコードで照合済みです。**

### 構想を採用する

ここまでの構想表と要点コードを合わせると、責任配置と実行経路が一つにつながります。`IAction` の実装が実行と取消を持ち、`ActionHistory` が種別を見ずに順序を管理し、台帳が残高の正本になります。
