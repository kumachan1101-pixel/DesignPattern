...
フェーズ1で、家計簿アプリがUIボタンから各マネージャクラスを直接呼び出している現状を把握しました。次に、ユーザーから寄せられた「操作を取り消したい」という要望を起点に、この設計における変わる見込みと今回維持する範囲を整理します。
...
フェーズ4で、`UIButtons` が「操作の実行手順」と「取消・再実行の履歴管理」という**別々の理由で変わる2つの判断**を、同じ場所へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つの実行経路へ再結合することです。この段階では、問題から導いた「操作の部品化構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。

```mermaid
flowchart TB
    A[現在<br/>UIButtonsに<br/>操作の実行手順と履歴管理が混在] --> B[分離判断<br/>二つの変化軸を別責任へ分け<br/>一つの実行経路へ再結合]
    B --> C[課題ID1<br/>操作を実行と取消を持つ部品にする<br/>操作の部品化構造]
...
向きと掲載クラスは1-3から変えていません。同じ図に注記と色だけを加え、`UIButtons` のどの責任を残し、どの責任を移すかに着目します。**この図が出発点で、以降の各段でここへ1つずつ足していきます。**
...
| | 内容 | どこで決まったか |
|---|---|---|
| **決まっている（目指す形）** | 操作を実行と取消を持つ部品にする／残高の正本を台帳へ集め、履歴の積替えを分ける | フェーズ4・5（2つの変化軸） |
...
- **分離方法**：入口に何を残し、何を契約の裏へ外すか。課題ID1と課題ID2で別々の境界になります。
- **配置場所**：外した判断を、どんな単位のクラスへ置くか。操作の種別ごとか、まとめて1つか。
- **組み立て方法**：実体を誰が生成・所有し、画面とバッチへどう渡すか。

### 課題ID1・課題ID2を6段で解く

**【課題の原因】** 課題ID1は、問題ID1・問題ID2（実行手段の混在で入口へ分岐が集中）＝原因ID1（操作が意図と実行手順を抱えオブジェクト化されない）。課題ID2は、問題ID3（補償・巻戻し範囲を外側が判断）＝原因ID2（実行・取消・補償がまとまらず台帳・履歴規則が漏れる）。この2つを分離対象にします。
...
**どう解決するか（方針）：** 操作そのものを、実行と取消の両方を持つ部品として扱う「操作の部品化構造」とします。そのうえで、残高の正本を台帳へ集め、履歴の積替えを専用のクラスへ閉じます。

**課題どうしの関係を、2つの基準で確かめます。**

| 基準 | 判定 |
|---|---|
| **A：変化軸** | 別々の理由で変わる。操作の種類が増えれば課題ID1側だけ、履歴の上限や積替えの規則が変われば課題ID2側だけ |
| **B：条件付け** | **該当する。** 課題ID2の完了条件「失敗時は対象範囲だけ逆順に戻せる」は、課題ID1が作る操作の契約に「自分の逆操作を持っていること」を要求している |

**基準Bに該当するので、契約を決める順序が固定されます。** ここでの「先に決める」は、課題ID2をすべて解いてから課題ID1に取りかかる、という意味ではありません。**課題ID2の完了条件を先に読んでから、課題ID1の契約を決める**という意味です。逆順に戻せる形が要る、と分かっていないまま実行だけの契約を決めると、あとで作り直すことになります。
...
**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、種別ごとの実行・取消手順、履歴の積替え、残高の保持を、別々の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
**課題ID1はどちらも成り立ちません。** 種別ごとの分岐は `onAddExpenseClick`・`onAddIncomeClick`・`undo`・`redo` の4か所に現れます。しかも**同じ組み合わせを逆向きに書いています。** 登録側で「支出なら `addExpense`」と書き、取消側で「支出なら `removeExpense`」と書く。種別が1つ増えれば、両方に1本ずつ足します。そして2-4のリスクID1（操作の種類の追加）とリスクID2（Undo対象の複雑化）が、どちらも「増える」と言っています。
...
**候補2つのうち、境界を流れるのは1つです。** 候補の数と実数が一致しないのが普通です。5-3は境界を跨ぐものを漏れなく挙げる表で、この段はそれを形へ変える段です。

**ここで、課題ID2の完了条件を先に読みます。** 5-2の基準Bで確かめたとおり、課題ID2は「失敗時は対象範囲だけ逆順に戻せる」ことを求めています。**逆順に戻すには、積んだものが自分の戻し方を知っていなければなりません。** 履歴の側が「支出だったから `removeExpense`」と判断するなら、いま外へ出そうとしている `if` が履歴へ移動するだけです。
...
**この段の守る範囲の照合：** 守る範囲の4つのどれにも触っていません。契約と値の置き場を決めただけで、既存のコードは1行も書き換えていません。
...
呼べるのは、操作の種類が増えても変わらない側だけです。出て行った側が出て行った側を呼べば、種類が増えるたびに両方を触ることになり、分けた意味が消えます。
...
**この章は上の型です。** 一括インポートで10件の操作を実行するとき、**同じ `ActionHistory` の中で相手が10回入れ替わります。** ロールバックでも、積んだ順の逆に相手が替わります。**1つの履歴が複数の操作を扱う**以上、契約は外の型（`IAction`）になります。
...
**穴が1つ空きました。** `history` です。**この変数がどこから来るのかは、まだ決めていません。** `ActionHistory` を入口が自分で作ることはできます。しかし**バッチからも同じ履歴を使う必要があります。** 画面で登録したものをバッチのロールバックが戻せないと、履歴が2つに分かれます。**誰が持つかは4で決めます。**

**ここで検算します。操作の種類を1つ足したとき、この骨格は変わるか。** 変わりません。`execute()` も `undo()` も `IAction` だけを見ています。**穴が空いたままでも、この検算はできます。**

**この段で図に描けるのは、2つの骨格から契約への依存だけです。**
...
**この段の守る範囲の照合：** 4つのうち1つに触りました。**操作を依頼する入口**です。ボタンを押すと1件の操作が実行される、という形は保っています。引数が変わりましたが、それは6で確認します。台帳の正本・表示・一括の巻戻し範囲には、この段では触っていません。
...
契約と、それを呼ぶ骨格が決まりました。1では契約の裏を1つ覗いて、持つもの・返すものを確かめました。**ここで残りを埋めます。**

**課題ID1（操作軸）。** 1で見た `AddExpenseAction` の対になるものを見ます。
...
**具体が2つしかありません。** それでも契約を作るのは、2-4のリスクID1が「削除・一括削除などの追加」を挙げているからです。**種類が増える見込みがあるので、1で契約まで進むと決めました。**
...
class ActionHistory {
    ...
    // …execute / undo / redo / printLog（2で示した骨格）…
};
```
...
**この段の守る範囲の照合：** **台帳を残高の正本にする契約**に触りました。3-1では入口が `balance` を持っていましたが、残高は台帳から求める形になります。**守る範囲で挙げたのは「残高は台帳から求め、画面や履歴が別の残高を持たない」ことなので、これは範囲を満たす方向の変更です。** 表示・入口・一括の巻戻し範囲には触っていません。
...
**2で穴が1つ空きました。** 入口が持つ `history` です。そしてもう1つ、**操作の実体を誰が作るか**も決めていません。

**まず履歴から決めます。** 持つ者に必要な条件は、2で分かっています。
...
**この段の守る範囲の照合：** **一括の巻戻し範囲**に触りました。3-1では外側が「どこまで戻すか」を判断していましたが、`ImportService` が成功件数を数え、その件数だけ `undo()` を呼ぶ形になります。**戻す範囲そのもの（そのとき成功していた分だけ）は変わっていません。** 入口・台帳の正本・表示には触っていません。

**この段が終わった時点で、実体はすべて存在しています。** 台帳、履歴、2つの入口、操作。**まだ起きていないのは1つだけです。骨格の `IAction*` と書かれた変数へ、具体が入ることです。それが5です。**

---

**5. 実装が骨格へ入る 【注入】**

**2で書いた骨格には、`cmd->execute()` としか書いてありません。** `AddExpenseAction` とは書いていません。それでも実行すれば支出が登録されます。**この段で見るのは、いつ・何によって、その実装が入るのかです。**

**掲載箇所：`main()`** ―― 4で作った操作を渡す行
...
**この1行が注入です。** 骨格は `IAction*` としか書いていないのに、実行時には `AddExpenseAction` が入っています。入れているのは `main()` であって、骨格ではありません。**骨格は、何が入ったかを最後まで知りません。**

**実装が骨格へ入る形は、この章の形だけではありません。** 骨格が契約としか書いていない場所へ具体が入る、という点はどの構造でも同じですが、入る瞬間は3つに分かれます。

| 形 | 実装が決まる決め手 | 入る瞬間 | この本での例 |
|---|---|---|---|
| **生成した実体を渡して保持する** | **誰を渡したか** | **コンストラクタ引数、`attach()` などの登録** | 第2章、**第5章（この章）**、第6章、第7章、第8章 |
...
**骨格が履歴へ届く方法（借用ポインタ）は、4で決めた話です。** 値で持つと画面とバッチで履歴が分かれますが、**注入はどちらの持ち方でも起きます。**
...
**この段の守る範囲の照合：** **操作を依頼する入口**を保っていることを、いま確認しました。ボタンを押すと1件の操作が実行される、という形は変わっていません。**表示**も、実行のたびに内容と残高の前後を出す形のままです（出す場所が台帳側の描画へ移りました）。6段すべてを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、各段で照合済みです。**


#### システム全体の最終構造を決める

6段で足した図を重ねると、システム全体の最終構造になります。`IAction` の実装が実行と取消を持ち、`ActionHistory` が種別を見ずに順序を管理し、台帳が残高の正本になります。
...
|---|---|---|---|
| 課題ID1 | 共通契約 `IAction` を新設し、操作を実行と取消を持つ部品にする | 各操作クラスが金額・カテゴリを内包し、入口と履歴は種別を判定しない | 6段の【契約】〜【利用開始】 |
| 課題ID2 | 残高の正本を `LedgerRepository` へ、履歴の積替えを `ActionHistory` へ移す | 入口が残高を持たず、成功した操作だけを積み、逆順に戻す | 6段の【契約】〜【利用開始】 |
...
|---|---|---|---|
| 【契約】（軸ごと） | 4か所に散った種別の `if` → `IAction` の実行・取消・説明／入口の `balance` → 台帳と履歴 | 分けるかを検算し、5-3の候補の形を決める。操作軸は候補2つのうち流れるのは成否1つ、履歴軸は候補3つのうち2つ | この契約を誰が呼ぶか＝【安定骨格】 |
| 【安定骨格】（軸ごと） | 種別判定つきの `undo()` → 種別を見ない `ActionHistory::execute()`／`undo()`。入口は1行 | 残った側が2つ（履歴と入口）あり、1つの履歴が複数の操作を扱うので契約は外の型 | 契約の裏に何を置くか＝【具体】 |
| 【具体】（軸ごと） | 登録側と取消側に離れて書かれた組み合わせ → `AddExpenseAction`／`AddIncomeAction` の中で隣り合う | 契約の3操作を埋める。履歴軸は実装が1つなので契約を作らない | 実体を誰が作るか＝【生成】 |
| 【生成】（共通） | 入口が種別ごとの係と残高を持つ → 台帳・履歴は `main()` が持ち、操作は使うたびに作る | 操作の値はボタンが押されて初めて決まるので、組み立て役は持てない。履歴は借用で共有する | 実装がどこで骨格へ入るか＝【注入】 |
| 【注入】（共通） | 利用側が種別文字列を渡す → `IAction*` を渡す1行 | 渡す側がそのつど作って渡す。渡されたものは履歴が長く保持する | すべて決まった結果としての呼び方＝【利用開始】 |
| 【利用開始】（共通） | `ui.onAddExpenseClick(1000, "CAT002");` → 操作を作る行と `app.onAddExpenseClick(&cmd1);` | 組み立てた実体の公開操作を呼ぶ。取消は履歴へ | ここから実行が始まり、【安定骨格】へ入る |

**【利用開始】が最後にあるのは、対策後の呼び方が設計の結果だからです。** 登録が2行になることも、取消の呼び先が入口から履歴へ移ることも、5まで決めてから分かります。

#### 代表ケースの実行接続

6段は考えた順でした。**ここからは実行時に通る順で貫きます。** 支出1件の登録と、その取消を、1本の経路として追います。**この章の【注入】は操作するたびに起きるので、登録と取消で2回現れます。**

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 【生成】 | `main()` | 台帳・履歴・2つの係を作り、`AddExpenseAction cmd1(em, 1000, "CAT002");` で操作を1つ作る | 【注入】へ |
| 2. 【注入】 | `BudgetApp::onAddExpenseClick(IAction*)` | `app.onAddExpenseClick(&cmd1);` で契約への参照が骨格へ入る | 【利用開始】へ |
| 3. 【利用開始】 | `BudgetApp::onAddExpenseClick(IAction*)` | `history->execute(cmd)` の1行 | `ActionHistory::execute()` |
| 4. 【安定骨格】 | `ActionHistory::execute(IAction*)` | `cmd->execute()` を呼び、成功したら `undoStack` へ積む | `IAction::execute()` |
| 5. 【契約】【具体】 | `AddExpenseAction::execute()` | 内包する金額とカテゴリで `em.addExpense()` を呼ぶ | 台帳が残高を更新 |
| 6. 【安定骨格】取消 | `ActionHistory::undo()` | `undoStack` の末尾を取り出し、`cmd->undo()` を呼ぶ | `IAction::undo()` |
| 7. 【契約】【具体】取消 | `AddExpenseAction::undo()` | 同じ金額とカテゴリで `em.removeExpense()` を呼ぶ | 台帳が残高を戻す |

**5と7が、同じ実体の2つのメソッドです。** 金額もカテゴリも、5で使ったものを7がそのまま使います。**3-1では、この2つが離れた場所に別々に書かれていました。**
...
ここまで問題から導いた「操作の部品化構造」は、一般に **Commandパターン**と呼ばれます。ここからは題材を離れ、この名前で共有される抽象的な骨格を確認します。
