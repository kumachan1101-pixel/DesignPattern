...
|---|---|---|
| 利用者の操作、金額、現在の収支状態 | 家計簿に対する操作と現在の状態 | 動作例テーブル、変更要求 |
| 登録、残高更新、操作履歴保持 | 収支操作の実行と履歴管理 | 現状仕様の確認、Undo/Redo追加時の変更シミュレーション |
| 更新後の残高、収支一覧、履歴状態 | 操作後に利用者が確認できる結果 | 動作例テーブル、実行結果、変更シナリオ表 |
