...
|---|---|---|---|
| F1 | 操作の種類の追加（削除・一括削除など） | 機能追加・操作内容の変更時 | UIデザイナーとの確認 |
| F2 | Undo対象となる操作の複雑化（口座移動・編集など） | 口座管理機能の追加時 | システム開発担当との確認 |
...
2-4のF-IDを、操作ごとに変えられるようにする部分と、操作を受け付けて履歴を管理する安定側へ分けます。「はい」は、フェーズ6で**操作の種類や補償方法が変わっても、呼び出し元と履歴管理へ条件分岐を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：操作の種類の追加（削除・一括削除など） | はい | 操作ごとの実行方法と取り消し方 | 操作の受付、実行済み・取消済み履歴の管理 |
| F2：Undo対象となる操作の複雑化（口座移動・編集など） | はい | 操作ごとの復元データと補償順序 | 実行・取消・再実行という入口、履歴の移動規則 |

したがって2-5の出力は、「具体操作と補償方法は変えられるようにし、操作の受付と履歴管理は守る」という設計条件です。フェーズ3ではR1〜R3だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。

## 🟣 フェーズ3：問題特定 ―― 変更の痛みを発見する
このフェーズでは、R1〜R3を、`UIButtons`が各マネージャクラスを直接知っている現在の構造へ実装し、Undo・Redo・一括補償のためにどこを同時に変える必要があるかを確認します。
...
Undo後の残高は`4000円`から、収入登録前の`-1000円`へ戻りました。R1は実現できています。ただし、そのために、各Managerへ逆操作を追加し、`HistoryItem`へ復元データを持たせ、`UIButtons`へ操作種別の分岐と残高更新を同時に追加しました。さらにR2のRedoとR3の一括補償でも、この三つの責任を連動して修正しました。

つまりR1〜R3は「関数を1つ足せば動く」変更ではありません。**各マネージャへ逆操作を足し、呼び出し元の`UIButtons`が操作種別・逆操作・復元データ・実行順を知る形になりました。** 局所的なメソッド追加では済まず、今回の要求だけで責任が`UIButtons`へ集中しています。
...
1つ目は、R1・R2のために`UIButtons`内の`undo()`と`redo()`が操作種別を判定し、R3では成功済み操作と逆順補償まで管理することです。三つの確定要求が同じ呼び出し元の条件分岐へ集中しました。
...
---
> **📌 問題（確定）**
> R1〜R3を満たすために、`UIButtons`の実行・Undo・Redo・一括補償の条件分岐と、複数のマネージャクラスを同時に修正した。操作の意図と実行手段が同じ場所に混在しているため、今回の確定要求が複数クラスへ連鎖した。
---
...
接続点は、呼び出し行だけではありません。**呼び出し元が何を知って渡すか、接続先が何を実行して何を返すか、その結果を呼び出し元がどの責任に使うか**までを一続きで見ます。この章には、UIと実行手段の接続（P1）と、実行結果を残高・履歴へ反映する接続（P2）があります。

**P1：UIからManagerへ操作を渡す接続**

```cpp
void onAddExpenseClick() {
    int delta = em.addExpense(1000, "CAT002");
}
...
呼び出し元は `ExpenseManager`、`addExpense()`、金額とカテゴリIDという具体知識を持ち、接続先は検証と残高差分の計算を担っています。Undoを加えると、呼び出し元は対応する `removeExpense()` まで知るため、操作の追加ごとに両端が連動して変わります。

**P2：Managerの実行結果を残高と履歴へ反映する接続**
...
接続先が返した残高差分を、呼び出し元が残高と履歴の両方へ反映しています。したがってP2で守るべき契約は、単なる戻り値ではなく、**台帳の更新と成功した操作の履歴が食い違わないこと**です。保存失敗時の補償やRedoを加えるなら、この接続の両端で「いつ成功とみなすか」「何を履歴へ積むか」まで揃える必要があります。
...
**着目する共通点：** 現状コードはすでに共通点を持っています。どの操作も、対応するManagerが「実行して残高差分（`int`）を返す」という同じ形にそろっています（データの共通点）。現場では、操作ごとに戻り値の形がばらばらなことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、利用側の呼び出しの形を大きく変えず、変わる「操作の中身と取り消し方」を操作オブジェクトへまとめれば済みます。フェーズ6では、この共通点と課題を組み合わせて完成構造を決めます。
...
|---|---|---|---|
| F1：操作の種類の追加（削除・一括削除など） | 一回の操作と取消データをIAction実装へまとめ、履歴は具体操作を判断しない | 新しいAction、BudgetAppの生成・登録 | ActionHistoryを守れる。削除操作は完成コードへ追加しない |
| F2：Undo対象となる操作の複雑化（口座移動・編集など） | Actionが取消に必要な変更前データを保持する責任を持つ | 対象Action、利用するRepository・Manager | 履歴契約へ局所化できるが、各操作の復元データ設計は将来行う。完成コードへ追加しない |

F1・F2は、履歴へ業務種別のifを置かない判断を支えます。将来操作そのものは要求確定後に追加します。
...
この構造は「リクエストをオブジェクトとしてカプセル化し、呼び出し元から具体的な実行手段の知識を外す」という構造に、GoFがつけた名前です。フェーズ6でP1・P2を満たす構造を決めた理由が、そのまま操作記録構造を選ぶ理由に重なります。構造名を先に知ってから「どこで使うか」を探すのではなく、問題を解いた結果としてこの構造にたどり着く——それがこのプロセスの意図です。
...
|---|---|---|
| R1：直前の収入・支出操作を取り消す | `UIButtons`と各Managerへ操作別の取消分岐を追加 | 各`IAction::undo()`へ補償を置き、残高と収支データが操作前へ戻ることを確認 |
| R2：Undoした同じ操作をやり直す | `UIButtons`へ操作データの保持と再実行分岐を追加 | `ActionHistory`が取消済み操作を戻し、同じ金額・カテゴリで再実行することを確認 |
| R3：複数操作を一括実行し、途中失敗時に成功分を巻き戻す | 呼び出し元が各操作の成功状態と逆順補償を管理 | 操作列を順に実行し、失敗時に成功済み`IAction`だけを逆順で戻して開始残高へ復元 |
...
| | 内容 |
|---|---|
| **問題** | R1〜R3を適用すると、`UIButtons`の条件分岐と各マネージャへの依存を連動して修正する必要があった |
...
| 🔴 フェーズ6：対策検討 | P1・P2を満たす完成構造のうち、台帳を正本にする案と残高集約を正本にする案を比較し、台帳正本案を採用した。実装ステップは採用案の反映順とした |
