...
Step 3との違いは「履歴を誰が管理するか」という一点です。案3では `BudgetApp` や `ImportService` が `IAction*` を直接受け取って `execute()` を呼ぶため、「実行した操作を記録してUndoする」という責任は呼び出し元に残ります。案4では `ActionHistory` という仲介役を挟むことで、実行・記録・Undo/Redoの全責任をその1クラスに集約します。「履歴管理が不要なら案3、Undo/Redoが必要なら案4」という判断の分かれ目がここです。
