...
登録されていないIDを指定するとエラーになります。コードを読む前にこの対応を把握しておくと、動作結果が追いやすくなります。★全章に言える話ですが、クラス単位でコードブロックを分けて、そのクラスで何をやっているか、どこから呼ばれてどんなロジックで動いて、どんな結果を返すか、詳細に説明するようにして。クラス図で関数の概要の説明をしているが、こちらでは、詳細にシステムの動きが分かるように、日本語の説明をまずしてから、コードが来るととても分かりやすいと思います。
...
実行結果：★実行結果について、だらだら記載されているが、分かりづらい、例えば、インデント入れるとか。
...
★参照セクションっているの？各フェーズの部分を見ればわかるよね？
...
Commandパターンは、リクエストをオブジェクトとしてカプセル化し、呼び出し元が具体的な実行先や引数の組み立てを直接知らずに依頼できるようにするパターンです。Commandオブジェクト自身は、処理を委譲するReceiverを保持することがあります。★パターン開設用のシーケンスはないの？デザインパターンの開設本ではないが、自然と理解できていた状態を目指したく、もっと詳しく説明できませんか。
