...
詳細な仕様やコードへ入る前に、1-4（実装コード）の`main()`で支出、収入、入力エラーを順に実行する3ケースを確認します。
...
この入力と出力から、(1)収入または支出の操作・カテゴリ・金額を受け取り、(2)カテゴリを検証してから記録と残高計算へ進み、(3)成功時は登録内容と残高、失敗時はエラーと変わらない残高が表示される、という一連の動きが読み取れます。同じ入力を含む完全なコードと実行結果は1-4に掲載します。
...
上の2行が1-1（このシステムの仕様）の図の正常出力に、下の2行が「カテゴリIDは登録済みか」「金額は1円以上か」の2つの判定で止まる異常出力に対応します。
...
この質問を2-3（関係者ヒアリング）で確認し、今回確定する変更と、将来リスクとして残す範囲を分けます。
...
2-4（ヒアリングで判明した将来リスク）のリスクIDを、操作ごとに変えられるようにする部分と、操作を受け付けて履歴を管理する安定側へ分けます。「はい」は、フェーズ6で**操作の種類や補償方法が変わっても、呼び出し元と履歴管理へ条件分岐を広げない構造か**を判定するための印です。
...
したがって2-5（変わる見込みと今回維持する範囲を確定する）の出力は、「具体操作と補償方法は変えられるようにし、操作の受付と履歴管理は守る」という設計条件です。フェーズ3「問題特定」では変更ID1〜変更ID3だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 種別ごとの分岐は `onAddExpenseClick`・`onAddIncomeClick`・`undo`・`redo` の4か所に現れます。しかも**同じ組み合わせを逆向きに書いています。** 登録側で「支出なら `addExpense`」と書き、取消側で「支出なら `removeExpense`」と書く。種別が1つ増えれば、両方に1本ずつ足します。そして2-4（ヒアリングで判明した将来リスク）のリスクID1（操作の種類の追加）とリスクID2（Undo対象の複雑化）が、どちらも「増える」と言っています。登録と取消が同じ操作を呼べるクラス契約まで分けます。

分けると決めたので、コードに線を引きます。

**変更前から抜き出す箇所：`UIButtons::undo()`** ―― 3-1（変更を試みる）の取消処理（対策前）
...
**5-3（課題IDと接続点を確定する）が挙げた候補は2つです。** 金額・カテゴリを含む操作と実行結果について、境界を流す形を確定します。
...
フェーズ2のリスクIDを採用構造へ再適用し、履歴契約をどこまで守れ、操作固有の復元に何が残るかを評価します。
...
`deleteExpense` / `deleteIncome` は保存済みレコードを取り消し、同じ操作で残高も戻します。**1-4（実装コード）では `UIButtons` が残高を持っていましたが、正本がここへ移りました。** 利用側が収入合計と支出合計の差を計算する必要はありません。
...
UI以外の呼び出し元の例として、複数の操作をまとめて実行する `ImportService` を追加します。フェーズ2「関係者ヒアリング（2-3（関係者ヒアリング））」で「一括削除のような複数データへの操作もあり得る」と予告されていた種類の呼び出し元です。UIと同じ `IAction` インターフェースを通じて操作を実行するため、履歴とUndoの仕組みをそのまま共有できます。まず、UIと一括インポートの2つの呼び出し元クラスを定義します。
...
フェーズ3で確認した「新しい操作（口座間移動）の追加」のシナリオを、3-2（変更影響グラフ）と同じ粒度で再度適用します。
