...
```text
現在：UIボタン / BudgetApp が
      操作の依頼 ＋ 実行先クラス・メソッド・引数 ＋ 残高の所有 を直接持つ
                      ↓
変更後：操作をオブジェクトにして依頼側から実行内容を切り離し、残高は台帳を正本にする
   操作の実行・取消（収入・支出ごと） → 各操作を IAction のオブジェクトにする（操作記録＝Command）→ 課題ID1
   残高の更新・補償・履歴             → 台帳を残高の正本にして一体更新する（正本一元化）      → 課題ID2
守る範囲：操作を依頼する入口（画面・バッチ・API共通）、カテゴリ検証、記録件数の照合
```
