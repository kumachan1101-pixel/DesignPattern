...
現在の構成では、`UIButtons` クラスが各ボタン操作に対応するメソッドを持ち、`ExpenseManager` や `IncomeManager` を直接呼び出す形になっています。現在の機能は収支の記録のみですが、「操作の取り消し（Undo）」や「やり直し（Redo）」という要件の追加が検討されています。
...
このシステムの規模では、操作オブジェクトの生成コストやパフォーマンスは設計判断を絞り込む制約になりません。唯一注意すべき点は、ユーザーが長期間アプリを使い続けると履歴スタックがメモリを圧迫することです。**CommandInvokerの設計段階で、履歴の最大保持件数と廃棄ポリシー**を決めておくことが基本方針になります。この点は案3・案4の選定に影響するため、各案のトレードオフで触れます。
...
* ※ 履歴スタックのメモリ上限：`CommandInvoker` が履歴を一元管理しているため、最大保持件数の上限設定と古い操作の自動廃棄ポリシーをここ一箇所に実装できる。呼び出し元（UIButtons等）はこれを意識する必要がない。
...
* ※ 履歴スタックのメモリ上限：`ICommandInvoker` インターフェース越しに差し替えられる構造のため、廃棄ポリシーの異なる実装（例：最大100件保持）への切り替えが容易。
