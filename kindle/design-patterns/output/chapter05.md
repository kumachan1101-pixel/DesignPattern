...
フェーズ6は、フェーズ5で定めた課題——**実行手段をオブジェクトとして差し替え可能にし、実行・取消・履歴を同じ単位で扱う接続点を作る**——を受けて始めます。まずフェーズ4で記録した現在の痛みを図の起点に置き、P1でどのコード境界を微修正し、同じ要求を当て直すと影響がどこまで縮むかを一本の経路で描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じP1の行へ落とします。その後、P1に紐づくフェーズ3の関連コードだけをおさらいし、候補を一段ずつ適用します。各段階で「何を変えたか」「どの影響が減ったか」「何が残るか」をコードで回収し、統合後の全体コードはフェーズ7で示します。
#### 痛みの変更影響から、構造をどう変えるか
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: UIの具体呼び出しへ名前を付ける"] --> S2["P1-変更2: execute・undoを操作単位へまとめる"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["新しい操作を追加"] --> I1["P1: 操作クラスと登録だけを追加"]
        I1 --> I2["追加操作のexecute/undoだけをテスト"]
        IS["変更しない: UIの実行入口・履歴スタック"]
    end
    C1 -. "P1で具体手段の漏出を切る" .-> S1
    S2 -. "操作追加を局所化" .-> I1
```
...
|---|---|---|---|
| P1 | `addExpense/undoExpense`、`IAction::execute/undo()`、`AddExpenseAction`、履歴 | **微修正：** 名前付け→入力と実行先をActionへ保持→履歴は契約だけを扱う<br>**結果：** UIと履歴から種別・引数・逆操作分岐が消えた | **守った契約：** 成功操作だけ記録、失敗時スタック維持<br>**判定：** 関数化では未達、Action接続で達成 |
