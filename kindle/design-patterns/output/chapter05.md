...
変わらないことも確認できます。現状にはUndoや削除の処理はありません。★←の記載はネタバレになっています。このように、何故ネタバレのような、著者向けなのか、無意味な記載が残っているのか。
...
#### 補足：変わる見込みと当面安定の仮説テーブル　★この章にしかない項目？テンプレ違反しないでほしい。それとも、他章に以下項目がないことが問題なのか。
...
★以下のような表にしてる章としていない章が混在している。統一してほしい。なぜ、記載の方法が各章で異なる項目がぽろぽろ出てくるのか。他にもないか総点検してほしい。
...
★以下もコードブロックに詰め込みすぎ。他章も同様に見直して。なるべく分割してほしいと過去指摘したはずです。
...
### 5-1：原因から課題候補を洗い出す
★以下表は原因IDとも紐づけて。全章共通です。

| 原因として確定した事実 | そのままだと残る痛み | 課題候補 | 候補を導いた理由 |
|---|---|---|---|
| UIとバッチが収入・支出の具体呼出しと引数を知る | Undo・Redo・一括実行ごとに入口へ逆操作の分岐が増える | 実行可能な操作を同じ単位として扱う | 操作の種類と実行入口は別の理由で変わる |
| 台帳更新・取消・履歴積替えの規則が複数箇所に漏れる | 途中失敗時にどこまで戻すかを各入口が判断する | 操作効果と補償、履歴管理の責任を分離する | 台帳の正本と操作履歴は異なる責任だから |
...
★以下成功したら、pop_backをすればよいのでは？一度取り出して失敗後元に戻すは不自然。
...
★以下undoは関数定義のみか？これはどういう意図か？定義のみと実体を混ぜる場合の使い分け。
...
★コードブロックが長すぎる。なるべくクラス単位で分割してほしい。他章も見直してください。
```cpp
...
実行ログ（`executionLog`）はシステム起動時は空で、操作が実行・取り消しされるたびに1件追記されます。`undoStack` とは異なり、削除されることなく全操作の記録として保持します。なお `MAX_HISTORY` は `undoStack` が無制限に伸び続けないためのメモリ保護の安全弁で、仕様要件ではなく本章の動作例では上限に到達しません。★vectorライブラリの操作で、参照と取り出しがあるが、取り出した後失敗して元に戻すタメに積む操作をしているが、それなら、まずは山椒の実にして成功したら取り出しの方が自然では？
...
| 要求ID5 | 最後に成功した操作を取り消す | `ActionHistory::undo()`、各`IAction::undo()` | 件数・残高が操作前へ戻る<br/>**判定:** 合格 |
