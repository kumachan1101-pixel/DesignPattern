...
- **生成・注入・実行による組み立て**：実体を誰が生成・所有し、単発と一括の入口へ契約として渡してどう実行するかを一本で決める。
...
**ここから決めること：** 銀行APIの多段手順を呼び出し元からどこまで外し、依頼と結果をどの形で接続し、具体的な処理役を誰が生成・所有するかを順に決めます。
...
**穴が1つ空きました。** `service` です。**この変数が何型で、どこから来るのかは、まだ決めていません。** `BankTransferService` のような具体の名前をここに書くことはできません。書けば課題ID2の完了条件「実装を差し替えても利用側の呼出手順が変わらない」が最初から崩れます。**それを決めるのが生成と注入です。**
...
**ここで図に描けるのは、2つの骨格から契約への依存だけです。** 実体をどこから得るかがまだ決まっていないので、その線は引けません。
...
**課題ID1（窓口軸）。ここは、他章と1つ違います。実装が1つしかありません。**
...
**ここが終わった時点で、実体はすべて存在しています。** 窓口、2つの骨格、それらを持つ組み立て役。**まだ起きていないのは1つだけです。骨格の `IBankTransferService&` と書かれた変数へ、具体が入ることです。次に、注入として確認します。**
...
**この1行が注入です。** 型は `IBankTransferService&` なのに、実行時に入っているのは `BankTransferService` です。入れているのは生成の検討で書いた `TransferProcessor processor(service);` の側で、骨格ではありません。**骨格は、何が入ったかを最後まで知りません。**
...
**骨格が窓口へ届く方法（参照で借りる）は、生成の検討で決めた話です。** 値メンバで持っても注入は起きますが、この章では「単発と一括が同じ実体を使う」という理由で参照を選びました。**注入はどちらの持ち方でも起きます。**
...
**分離と組み立てを通っても、呼び方が変わるかどうかは章によって違います。** そして**どちらも、分離と組み立ての結果であって前提ではありません。** 先に見せると、まだ導いていない結論を見せることになります。

**守る範囲との照合：** **単発と一括の利用フロー**を保っていることを、いま確認しました。1件ずつ振り込む流れも、一括の中断と戻しも、呼ぶ形は変わっていません。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
| 生成・注入・実行による組み立て | 各入口が具体を持つ → `Application::run()` が窓口を1つ所有し両入口へ貸す | コンストラクタで契約参照を渡し、同じ窓口を単発・一括から実行する | 生成した窓口が両経路で共有される |

公開入口は、4引数ではなく一つの `TransferRequest` を受け取ります。組み立てでは、同じ窓口実体を単発と一括の両方へ渡します。

#### 代表ケースの実行接続

生成した窓口が実際に使われるまでを確認します。ACC001からACC002への5000円の振り込みを、注入→入口→契約→具体の順で追います。
...
**一括振込は3から6へ分かれます。** `BatchTransferProcessor` が `performApprovedBatchTransfer()` を件数ぶん呼び、途中で失敗したら `compensate()` を成功済みの件数ぶん逆順に呼びます。**通る窓口は同じ実体です。** 生成の検討で「単発と一括が同じ実体を使う」と決めた結果が、ここに出ています。
