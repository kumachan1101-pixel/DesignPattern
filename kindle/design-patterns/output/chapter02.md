...
フェーズ6は、フェーズ5で定めた課題——**振込フローの呼び出し元から、銀行APIごとの手順と失敗処理を隠す接続点を作る**——を受けて始めます。まずフェーズ4で記録した現在の痛みを図の起点に置き、P1でどのコード境界を微修正し、同じ要求を当て直すと影響がどこまで縮むかを一本の経路で描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じP1の行へ落とします。その後、P1に紐づくフェーズ3の関連コードだけをおさらいし、候補を一段ずつ適用します。各段階で「何を変えたか」「どの影響が減ったか」「何が残るか」をコードで回収し、統合後の全体コードはフェーズ7で示します。
#### 痛みの変更影響から、構造をどう変えるか
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: API手順を関数へ集める"] --> S2["P1-変更2: 要求・結果を持つ窓口契約へ上げる"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["銀行手順・認証方式を変更"] --> I1["P1: 銀行別の振込窓口だけを変更"]
        I1 --> I2["窓口契約と組み立てだけをテスト"]
        IS["変更しない: 通常振込・給与バッチの業務フロー"]
    end
    C1 -. "P1で手順の漏出を切る" .-> S1
    S2 -. "窓口内へ閉じる" .-> I1
```
...
|---|---|---|---|
| P1 | API手順の抽出関数、`IBankTransferService::performTransfer()`、`TransferProcessor::transfer()`、給与バッチ | **微修正：** API順序を関数化→要求/結果の窓口契約→両入口へ同じ窓口を注入<br>**結果：** 呼び出し元からGateway・OTP順序が消えた | **守った契約：** 検証、成功時だけの残高/履歴更新、失敗結果<br>**判定：** 関数化では未達、共通窓口接続で達成 |
