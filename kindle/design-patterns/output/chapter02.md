...
改めて、ここまで導き出してきた「最終的な設計（図やコード）」を、第0章でお話しした「1つの原則と2つの技法」と照らし合わせてみましょう。一通り設計のプロセスを体験した今なら、あの原則が「コードのどの部分に現れているか」が実感として結びつくはずです。
...
この「知らなくていいという契約」こそがインターフェースによるプログラミングの本質です。
