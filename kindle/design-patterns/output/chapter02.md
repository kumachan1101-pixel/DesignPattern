...
この出力から、①振込元・振込先・金額を入力し、②口座確認→残高確認→認証→残高増減の順に進み、③送金結果と残高更新が残る、という一連の動きが読み取れます。これを押さえてから、以降の要求・仕様・コードを読み進めます。
...
|---|---|---|---|
| 振込処理が口座確認・認証・送金APIの具体手順を知る | API変更で業務フローとバッチ入口まで確認が広がる | 銀行APIの一連手順を振込業務フローから分離する | 外部API手順と振込依頼の利用方法は別の理由で変わる |
| 利用側が具体的な振込実装を直接選ぶ | 接続先変更・テスト差替えで複数入口が変わる | 振込実装の選択を利用側から外す | 実行と実装選択は別の責任である |
...
統合表で特定した箇所だけを振り返ります。課題ID1は手順の並び、課題ID2はその手順を持つクラスの具体依存です。口座残高検証や履歴記録など、課題に関係しないコードは省略し、フェーズ3で明記した維持条件をそのまま引き継ぎます。
...
|---|---|---|---|
| リスクID1：認証フローの多段階化（二段階→三段階認証） | 認証と送金の手順をBankTransferServiceの内側に集め、Applicationの窓口から隠す | BankTransferService、SecurityAuthenticator | 窓口への波及を抑えられる。三段階認証は完成コードへ追加しない |
| リスクID2：送金リクエスト形式の変更（JSON→XML移行計画） | 外部形式の知識をBank境界とService側へ置き、TransferRequestを使う呼び出し元から分ける | Bank、BankTransferServiceの変換処理 | 一部局所化できる。XML変換は完成コードへ追加しない |
| リスクID3：生体認証の導入 | 認証境界をSecurityAuthenticatorとして独立させ、送金窓口は認証の具体方式を知らない | SecurityAuthenticator、BankTransferServiceの認証手順 | 局所化できる。生体認証は完成コードへ追加しない |
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を両方満たす完成構造を一つに定め（責任配置の異なる完成構造は複数成立しないため比較は不要）、採用クラス図を課題ID別の①〜⑥でコードへ反映した |
