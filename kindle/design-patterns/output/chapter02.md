...
| 接続点 | 接続するデータ | 変わる側 | 守る側 |
|---|---|---|---|
| 銀行API手順 ↔ 振込業務フロー | 振込依頼（`from`・`to`・`amount`・`otp`）を渡し、成否（`bool`）を受け取る | `Bank::verifyAccount`／`checkBalance`／`executeTransfer` と `SecurityAuthenticator::promptOTP`／`verifyOTP` の呼び出し順・パラメータ | `db.exists` の検証、`history.add` の記録、依頼へ成否を返す業務フロー |
| 実装の選択 ↔ 振込業務フロー | どの実装で振込依頼を処理するかの決定 | 接続先の実装（本物の銀行連携か、テスト用か、別銀行か） | 呼び出し元（`TransferProcessor`・`BatchTransferProcessor`）が依頼と結果だけを知る接続 |
...
フェーズ2で確定した2つの変化軸をP1・P2として追跡します。この時点で決めるのは「どの知識が変わり、どの手順を守るか」までで、それらをどのクラスへ置くかはフェーズ6で決めます。

| 課題ID | 変化軸 | 変わる側（差し替えたい知識） | 守る側（安定させたい処理） |
|---|---|---|---|
| P1 | 銀行APIの呼び出し手順 | 口座確認→残高確認→認証→送金という呼び出しの並びとパラメータ | 振込を依頼し結果を受け取る業務フロー、自社の検証と履歴記録 |
| P2 | 接続先の実装 | どの実装（本物の銀行連携・テスト用・別銀行）で依頼を処理するか | 呼び出し元が依頼と結果だけを知る接続 |

P1とP2は別の課題ですが、フェーズ4の観察から「手順の一式」と「差し替えの単位」が同じ範囲を指していることが分かっています。この観察は、フェーズ6で分離の単位を決めるときの材料になります。給与バッチが持つ重複手順も、同じ変化軸P1として扱います。
...
受け入れ条件は「解けたと言える影響範囲」を役割の言葉で定めたものです。どんな構造で実現するかは、フェーズ6で決めます。

| 課題ID | 現在の変更影響 | 変えたくない範囲 | 受け入れ条件 |
|---|---|---|---|
| P1 | 認証・送金手順の変更で `TransferProcessor` と `BatchTransferProcessor` の呼び出し順を修正し、両入口を再テストする | 振込の公開入口、口座・残高検証、成功時の残高更新と履歴記録 | 銀行手順が変わったとき、修正が「手順を担う場所と必要な入力」に収まり、2つの呼び出し元・自社検証・履歴記録が修正・再テスト対象に入らない |
| P2 | 別銀行対応やテスト差し替えのたびに呼び出し元が具体実装名を知り、業務クラスまで修正する | 呼び出し元が依頼と結果だけに依存する接続、振込依頼・振込結果の型 | 接続先を差し替えたとき、修正が「組み立ての1か所」に収まり、業務クラスが修正・再テスト対象に入らない |

P1とP2は対策案の番号ではなく、2つの変化軸をコードまで追うIDです。採用構造の判定は分けず、システム全体で一度だけ行います。
...
フェーズ5で定めた受け入れ条件を、そのまま候補の評価軸として使います。フェーズ6で条件を作り直すことはしません。

- P1：銀行手順が変わったとき、修正が「手順を担う場所と必要な入力」に収まり、2つの呼び出し元・自社検証・履歴記録が修正・再テスト対象に入らない。
- P2：接続先を差し替えたとき、修正が「組み立ての1か所」に収まり、業務クラスが修正・再テスト対象に入らない。
- 共通の制約：呼び出し元は振込依頼と振込結果だけを知り、手順の並びも具体実装名も知らない。

#### 接続点の分離・配置・組み立てを決める

受け入れ条件を満たすために、フェーズ5の接続点を次の3つの決定で設計へ落とします。

| 決定 | 問い | この章での答え |
|---|---|---|
| 分離 | 接続点をどう切るか | 銀行・認証の呼び出し手順一式（手順1〜5）を業務フローから切り出す。境界の契約は「振込依頼を渡すと振込結果が返る」1往復 |
| 配置 | 切り出した知識をどこへ置くか | 手順の一式と差し替えの単位が同じ範囲（フェーズ5の観察）なので、手順を1つの窓口クラスへまとめて置く。守る側には自社検証・履歴記録・公開入口が残る |
| 組み立て | どう繋ぎ直すか | 具体窓口の生成・選択は組み立て側（Application）が担い、呼び出し元は共通契約への参照だけを受け取って依頼する |

この3つの決定を満たす実装形を次で確認します。

#### システム全体の最終構造を決める

この章の最終構造は、競合する複数案から選ぶのではなく、受け入れ条件から**一つに定まります**。銀行API手順を呼び出し元から隠すと決めた時点で、「手順一式を窓口の後ろへ閉じ、呼び出し元は契約越しに依頼する」**窓口構造**以外に、上の受け入れ条件をすべて満たす形が残らないためです。

読者が思いつきそうな単純な代替案として、「手順を自由関数 `performTransferSteps()` に切り出す」形もあります。手順の重複（P1）はこれで解けますが、関数は差し替えの単位にならないため、テスト用実装への切り替え（P2）のたびに関数の中身か呼び出し側を書き換えることになり、P2の受け入れ条件を満たしません。設定で手順を解釈する汎用アダプタも理屈の上では置けますが、設定言語の実装という別問題を抱え、この規模では過大です。関数案のコードイメージは章末のコラム「採用しなかった構造：手順の関数化」で確認できます。
