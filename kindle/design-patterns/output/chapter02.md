...
```cpp
    bool transfer(const std::string& from, const std::string& to,
                  int amount, const std::string& otp) {
        bank.verifyAccount(to);             // ① 口座確認（守りたい業務フロー）
        bank.checkBalance(from, amount);    // ② 残高確認（守りたい）
        // ③ 認証・送金（銀行の手順＝変わる側）を呼ぶ
        std::cout << "振り込み完了\n";       // ④ 完了（守りたい）
        return true;
    }
```

★bankのインターフェース「verifyAccount」「checkBalance」は変わる部分なのでは？
### 4-3：接続点に漏れている手順を確認する

ここでの「確認すること」は、前節までに見つけた原因から抽出します。まず、原因文から「守りたい骨格」と「変わる差分」を分けます。次に、その差分を動かすために骨格側が知ってしまっている名前・条件・順序・型を拾います。最後に、接続点に残す最小の約束を、値・型・操作・イベントとして書きます。★ここで書きますと言っている内容、書いたのですか？【銀行APIの手順が呼び出し元へ漏れているコード】のコメントのことでしょうか。続く、と記載している箇所、接続点となる箇所は全て略さずに記載するようにして下さい。
...
public:
    bool transfer(...) {
        // ← 直接：銀行の各手順を窓口なしに順に呼び出す
        bank.verifyAccount(to);
        bank.checkBalance(from, amount);
        auth.requestOTP();
        // bank.executeTransfer(from, to, amount, txId); など続く
    }
};
```
...
★以下変わらない骨格がプリントだけはやめて、骨格でも何でもないですよね。何も処理していないのですから。
★std::cout << "振り込み完了\n"; // ← 変わらない骨格
...
★以下説明だけだといまいち理解できない。ロジックなのか、クラス名とか関数名やデータ名を記載した方が良いのでは？
...
#### フェーズ6へ渡す課題
★以下もよくわからない。対策内容を記載しているのか？
★課題定義で、解くべき接続点が明確になり、対策検討でどのようにその接続点を分けて接続しなおすのかの話になっていくのでは？すでにどのように接続しなおすのかの話がこのフェーズで話が進んでいるのは、スコープとして問題ないのか。窓口を置く話、対策が決まっているという事なのか。
...
★最終構造決める前に、完了条件が定義できるのか。そもそも完了条件って何？
...
この章の最終構造は、競合する複数案から選ぶのではなく、完了条件から**一つに定まります**。銀行API手順を呼び出し元から隠すと決めた時点で、「手順一式を窓口の後ろへ閉じ、呼び出し元は契約越しに依頼する」**窓口構造**以外に、上の完了条件をすべて満たす形が残らないためです。

設定で手順を解釈する汎用アダプタや、手順を関数として束ねる方式も理屈の上では置けますが、いずれも設定言語の実装や関数束の管理という別問題を新たに抱え、この規模では過大です。したがって当て馬を並べた比較は行いません（完成形が複数競合する章でのみ、構造差分を比較します）。
