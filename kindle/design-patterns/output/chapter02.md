...
2回目に注目してください。残高確認が **145,000円** から始まっています。1回目の送金で減った残高が保存され、次の呼び出しはそこから始まります。また、残高不足で止まったため**認証コードの確認まで進んでいません**。この出力から直接分かるのは確認順と停止位置です。その順序を採る理由は、後続の処理説明で確認します。
...
次表の「手順(1)〜(4)」は、1-1（このシステムの仕様）「振り込みの基本手順」の読む順を短く参照したものです。要求を管理するIDではなく、複数クラスが一連の処理のどこを担当するかを示す案内番号です。要求の継続・変更・受入は、別の「要求ID」で追跡します。
...
1-2（動作例テーブル）の動作例5行を、行ごとに区切って実行します。**残高は行をまたいで引き継がれます。**
...
全章共通の簡略化範囲は1-3（登場クラスとクラス構成図）の「実システムの複雑さと掲載コードでの簡略化」で説明済みです。ここでは今回の変更要求に固有の、取引ID付き認証と一括振込の補償だけを具体化します。
...
2-1（変わりそうな仕様の見当をつける）で見当を付けた銀行APIの変化について、設計条件にする前に確認すべき不明点を質問へ変換します。
...
この質問を2-3（関係者ヒアリング）へ持ち込み、今回だけの変更か、将来も続く変化かを確認します。
...
2-4（ヒアリングで判明した将来リスク）のリスクIDを、設計で扱う変化軸へ整理します。「はい」は先行実装の指示ではなく、フェーズ6で**変化する外部仕様を振込処理の安定した入口から分離できるか**を判定するための印です。
...
したがって2-5（変わる見込みと今回維持する範囲を確定する）の出力は、「認証手順・認証方式・外部形式は変えられるようにし、振込依頼から結果保存までの業務上の流れは守る」という設計条件です。フェーズ3「問題特定」では変更ID1・変更ID2だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
1-4（実装コード）の `transfer()` を、行ごとに「変わる側」「守る側」へ塗り分けると次のようになります。接続点の判断材料なので、略さず全行を示します。
...
**変更前から抜き出す箇所：`TransferProcessor::transfer(const string&, const string&, int, const string&)`** ―― 3-1（変更を試みる）の振込フロー（対策前）
...
**5-3（課題IDと接続点を確定する）が挙げた候補は2つです。** 振込依頼と処理結果について、境界を流す形を1つずつ確定します。
...
**「失敗理由を返す形にしてよいのか」** ——ここは踏み外しやすいので、根拠を書きます。**変更前のコードは `bool` しか返していないので、「変更前が消費していた値」を根拠にはできません。** 使えるのは1-5（変更要求）の変更後要求ベースラインです。要求ID5が「未登録口座・残高不足・認証失敗をそれぞれ理由付きで拒否する」と定めていて、変更前は理由を `cout` で直接出していました。**手順を境界の向こうへ移すと、その `cout` も一緒に移ります。** 移した先が理由を持っているのに呼び出し側へ返さないと、要求ID5を満たせません。だから戻り値に理由を含めます。
...
**4行で組み立てが終わります。** `db`・`bank`・`history` は1-4（実装コード）から続く境界で、これは変更前と同じです。**新しく増えたのは `service` の1行と、それを `processor` へ渡す部分だけです。**
...
フェーズ2のリスクIDを採用構造へ再適用し、窓口の外側をどこまで守れ、どの境界に弱点が残るかを評価します。
...
リスクID1〜リスクID3は、集約した窓口の境界を評価する材料です。今回の完成コードは、確定要求に含まれる認証方式と外部形式で構成します。
...
- 判断：`TransferProcessor`は`IBankTransferService`を借り、単発振込の流れと外部APIの多段手順を組み合わせる。
- 継承との区別：`BankTransferService`の`: public IBankTransferService`は契約実装である。`TransferProcessor`を銀行別に派生させて手順を増やすのではなく、利用側が窓口実体を保持するため、銀行側の手順変更は窓口の具体へ閉じる。
- 適用範囲：実装が今後も1つで固定されるなら直接保持でもよい。本章では単発と一括が同じ窓口を共有し、外部手順が独立して変わる事実があるため契約を置いた。
