...
フェーズ5の課題定義で、業務フロー（`TransferProcessor`）と外部システム（銀行API群）の間の接続点を切り離した方がよいと確認しました。このフェーズで最も重要なことは、「最初からどうコードを書くか」を決めないことです。課題の形が決まれば、そこをつなぐ「接続の形」の選択肢は自然と導き出されます。
...
フェーズ4で診断した4つの接続パターンを、対策案として検討します。現在の「具体×直接」からどの接続形態へ移行するかを選ぶことが、このフェーズの目的です。

現在の `TransferProcessor` と外部システム群との接続は、銀行のAPIという「特定の仕様」に専用のLightningケーブルで直差しした状態（具体×直接）です。ここから、今後予想される「認証手順の複雑化」や「APIの仕様変更」に耐えうる柔軟な接続形態へ移行するため、以下の4つの案を検討します。
...
**この形の考え方：**
銀行通信用のクラスを切り出して責任を分担させます。呼び出し側の `TransferProcessor` は切り出した具体クラス（`BankTransferService`）を名指しで知っているものの、処理そのものはその具体クラスに「委ねる」形になります。自分で直接計算・実行しなくなる点が案1との違いです。「責任の分担」という第一段階の整理だけを行いたい場合に合う形です。
...
対策案が揃ったところで、どの案を採用する必要があるかを決めるための「ものさし」を宣言します。後から基準を持ち出すと議論が混迷しやすいため、比較表を提示する前に、チーム内で評価軸とその重要度を合意しておくことが大切です。
