...
## ステップ5：対策案の検討 ―― 層ごとに手札を当てる

ステップ4の原因を確認します。

> **原因：MonthlyBatchが3つのサービスの内部仕様（具体クラス・API形式）を直接知っている。
> 外部依存のAPI・呼び出し構造が変わると、MonthlyBatchまで変わる。**

手札の選択表を引くと：「外部依存のAPI・呼び出し構造が変わる」→ **窓口集約**（第0章 手札）。

ただし、この問題は**層を持っています**。MonthlyBatchの依存を断ち切ると、今度はFacade自身が各サービスの具体クラスに依存していることが見えてきます。その層を解決すると、次は組み立ての問題が見えます。

1つの手札を当てるたびに責任チェックで確認し、問題が残っていれば次の層へ手を伸ばします。

**対策案の比較（ステップ6の天秤のための準備）**

この問題には大きく2つの方向性があります。

| 案 | 内容 | 特徴 |
|---|---|---|
| **案A：窓口集約のみ** | PayrollCoordinator で直接依存を集約する | 手早く・簡単。依存の場所が1箇所に減るが、具体クラスへの依存は残る |
| **案B：多段階インターフェース層化** | 窓口集約 + 各層のインターフェース抽出 + Composition Root | 各クラスが自分の責任だけを持ち、テスト独立性が高まる。実装コストは増える |

どちらを選ぶかは、ステップ6（天秤にかける）で評価軸を使って判断します。今回のシナリオ（外部依存が3系統・API変更が最短四半期に確定）では案Bを選びます。
...
| 第2段階 | インターフェース抽出 | MonthlyBatchとFacadeの間に契約を置く |
| 第3段階 | インターフェース抽出＋値オブジェクト化 | Facadeと各サービスの間も契約化する |
| 第4段階 | Composition Root（哲学3） | 組み立ての責任を1箇所に集める |
