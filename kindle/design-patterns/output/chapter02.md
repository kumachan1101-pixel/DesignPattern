...
ステップ4で言語化した原因を、第0章の4分類に当てはめます。

| 原因 | 4分類 | 手札候補 |
|---|---|---|
| 外部サービスのAPI・呼び出し構造が変わる | 2.まとめる | 窓口集約 |
| 呼び出し元が具体クラスに直接依存している | 3.間に挟む | 依存の逆転 |

候補が2つあります。どちらが原因に直接対処できるか、別々に試します（ケース③）。

---

### 2.5.2 手段①：窓口集約のみ（窓口クラスでまとめる）

第0章の手札「窓口集約」を試します。3サービスへの呼び出しを専用クラスに集めるアプローチです。

第0章の手札選択表より：「外部依存のAPI・呼び出し構造が変わる」→ **窓口集約**（第0章 手札）。

```cpp
// 手段①：集約クラス PayrollCoordinator の導入
class PayrollCoordinator {
public:
    PayrollCoordinator(PayrollService* ps, AttendanceService* as, TaxService* ts)
        : payroll_(ps), attendance_(as), tax_(ts) {}

    void processMonthlyPayroll(int employeeId) {
        int base = payroll_->getBaseSalary(employeeId);
        int hours = attendance_->getWorkedHours(employeeId);
        int tax = tax_->calculateTax(base, hours);
        payroll_->executePayment(employeeId, base + hours * 100 - tax);
    }
private:
    PayrollService* payroll_;
...
値オブジェクトでくるむと型変更をその1箇所に閉じ込められます。
...
この組み立ての責任は、専用のクラスに切り出すのがよいでしょう。
...
呼び出し元は内部の複雑さを知らなくていい——それがFacadeの本質です。
