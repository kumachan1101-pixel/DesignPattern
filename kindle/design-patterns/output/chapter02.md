...
手札の選択表を引くと：「外部依存のAPI・呼び出し構造が変わる」→ **手札④（ファサード集約）**。
...
| 段階 | 使う手札 | 解決する層 |
|---|---|---|
| 第1段階 | 手札④（ファサード集約） | MonthlyBatchの直接依存を集約クラスへ移す |
| 第2段階 | 手札①（インターフェース抽出） | MonthlyBatchとFacadeの間に契約を置く |
| 第3段階 | 手札①＋手札②（値オブジェクト化） | Facadeと各サービスの間も契約化する |
