...
2.4のヒアリングで「来年度からExcel出力の要望が上がっています」という言葉がありました。
その変化が実際に来たとします。
...
2.4で決めた「ビジネス責任で命名する」原則が、ここで実証されました。

---

**型変更リスクへの対処：社員IDがstring型に変わった場合**

ここで、2.4のヒアリングで挙がったもう一つのリスクを確認します。
...
> サブシステム内の複数のインターフェースに対して、統一されたインターフェースを提供する。
> Facadeパターンは、サブシステムを使いやすくする上位のインターフェースを定義する。

### この章での対応関係

| GoFの役割 | この章での実装 |
|---|---|
| Facade | `PayrollFacade` |
| Client | `MonthlyBatch` |
| Subsystem | `AttendanceService`、`TaxService`、`InsuranceService`、`PaymentService` |

`MonthlyBatch` は `PayrollFacade` の1つのメソッドを呼ぶだけです。4つのサービスをどの順番でどう組み合わせるかは `PayrollFacade` の内部知識であり、`MonthlyBatch` には一切漏れ出ません。

### 使う基準

- 複数のサービス・クラスを決まった手順で組み合わせて使う処理がある
- 呼び出し元が「複数の依存先の存在」を知らなくてよい
- 手順が変わっても呼び出し元を変えたくない

### 使わない基準

- 依存しているサービスが1つだけ（直接呼べばよい）
- 呼び出し元が処理の途中結果を使って判断を変える必要がある（Facadeに隠すと制御が取れなくなる）