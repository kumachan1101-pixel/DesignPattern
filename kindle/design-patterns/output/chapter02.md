...
まずは、ネット銀行の振り込み処理が何を入力として受け取り、どの処理で加工し、何を出力するのかを整理します。ここでは設計の良し悪しを判断せず、現状を事実としてそろえます。
...
- 口座確認、残高確認、認証は、送金実行の前に満たすべき条件として並んでいる。
- どこかの確認で失敗した場合は、送金や履歴記録へ進まず、その時点のエラーとして終わる。
...
フェーズ2では、まだ「責務が混在している」とは判断しません。ここで行うのは、フェーズ1で見た仕様のうち、どの条件・手順・外部呼び出しが変わりそうかを見当づけることです。
...
ここからの検討は、次の2つのよくある考え方を軸に進めます。1つ目は、**共通部分に着目して共通化し、抽象化する**ことです。2つ目は、**変わる部分を分離して外から注入し、組み立て側は中身を意識せず決まった流れを回すだけにする**ことです。以下の各案は、この2つの考え方をこの章の課題へ当てたものです。メソッド切り出し、共通関数への集約、窓口クラスへの集約は1つ目の実行です。窓口の抽象化は、呼び出し元が窓口の中身を意識せず契約だけを呼び、実装を外から差し替えられるようにするという、2つ目の実行です。

ここで比べる案は、次の4種類です。

| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| メソッド切り出し | API呼び出しの各操作の名前 | 手順の各段が名前で読めるようになる | 手順の知識は同じクラスに残る |
| 共通関数への集約 | 口座確認から送金までの手順 | 手順の重複が消え、変更が関数へ集まる | 手順が使う部品と状態の置き場がない |
| 窓口クラスへの集約 | 手順と、手順が使う部品の保持 | 呼び出し元は窓口を1つ呼ぶだけになる | 呼び出し元が具体的な窓口クラス名を知る |
| 窓口の抽象化 | 窓口の契約と実装 | テスト用・別銀行向けの差し替えができる | 型と組み立ての初期コストが増える |

この4案のうち「共通関数への集約」は、同じ手順を1つの関数へまとめ、各呼び出し元はそれを呼ぶだけにするという、最初に思いつきやすい案です。この案は比較表の中だけで済ませず、ステップ2の中で案Aとして実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。メソッド切り出しはステップ1で、窓口クラスへの集約はステップ2の案Bで、窓口の抽象化はステップ3で順にコードにします。

### ステップ1：各処理を独立したメソッドに切り出す（同じクラスの中で整理する）

「API呼び出しが複雑に絡み合っているなら、1つひとつの操作を独立したメソッドに切り出してみよう」というのが自然な最初の発想です。これは比較表の「メソッド切り出し」案にあたります。クラスを新しく作るのはコストがかかる。まずは `TransferProcessor` の中で、各API呼び出しをそれぞれ独立したメソッドとして切り出してみます。
...
ステップ1では手順の知識が `TransferProcessor` の中に残りました。この知識をクラスの外へ出す形として、比較表の「共通関数への集約」案から確かめます。

**案A：手順を共通関数へまとめ、各呼び出し元はそれを呼ぶだけにする**

同じ手順が通常振込とバッチに重複しているのなら、手順を1つの関数にまとめてしまえばよい、という発想です。

```cpp
// 案A：口座確認から送金までの手順を1つの共通関数に集約する
bool executeBankTransfer(BankGateway& gateway,
                         SecurityAuthenticator& auth,
                         const std::string& account, int amount,
                         const std::string& otp) {
...
// 呼び出し元：部品は自分で持ち、手順だけを関数に任せる
class TransferProcessor {
    BankGateway gateway;         // ← 部品の保持は呼び出し元に残る
    SecurityAuthenticator auth;  // ← 同上
public:
    void transfer(const std::string& toAccount, int amount,
                  const std::string& otp) {
        if (executeBankTransfer(gateway, auth,
                                toAccount, amount, otp))
            std::cout << "振り込み完了\n";
    }
};
```

- `executeBankTransfer()` に、口座確認→残高確認→認証コード発行→検証→送金という手順が1か所へ集まりました。取引IDの受け渡しも関数の中で完結します。
- 呼び出し元の `transfer()` は関数を1回呼ぶだけになりました。給与バッチ用には、OTPを省いた同じ形の関数をもう1つ用意します。

**案Aの評価：**
通常振込とバッチに重複していた手順が関数へ集まり、認証手順の変更はこの関数（とバッチ用の関数）の修正で済むようになります。呼び出し元の見た目は、この後の窓口クラス案とほとんど変わりません。**手順の置き場を1か所にすることだけが目的なら、この案で止める判断は十分あり得ます。**

**案Aに残る課題：**
関数は部品を保持できないため、`BankGateway` と `SecurityAuthenticator` の生成・保持は呼び出し元に残ります。この章には通常振込と給与バッチという2つの呼び出し元があるので、どちらも同じ具体クラス名を知り、同じ部品を別々に抱えるか、引数で引き回すことになります。また、フェーズ2で確認した将来リスク（多段階認証・生体認証）が来ると、手順のバリエーションと途中で受け渡す値が増え、関数の引数と本数が増えていきます。手順と、手順が使う部品を、1か所でまとめて持てる置き場が必要です。

**案B：手順と部品をまとめて窓口クラスに任せる**

そこで、「手順を1か所へ集める」という案Aの方向は保ったまま、置き場を関数からクラスへ変えます。「銀行APIとのやり取りをすべて別のクラスに任せてしまえば、`TransferProcessor` は呼ぶだけでよくなる」という発想です。手順全体を担当するクラスを新しく作り、呼び出し元はそのクラスを1つ呼ぶだけにします。クラスなら、手順が使う部品（`gateway` と `auth`）も一緒に抱えられます。
