...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。利用者がACC001からACC002へ5000円を個別振り込みすると、内部は振込先口座の確認→送金元残高の確認→認証コード（OTP）照合→残高の増減→完了記録の順に進みます（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 送金前に口座と残高を確認する | 要求ID1、要求ID2 |
| 機能ID2 | 取引を認証する | 要求ID3 |
| 機能ID3 | 認証済み取引を送金する | 要求ID4 |
| 機能ID4 | 成功履歴を保存し、失敗時の整合性を守る | 要求ID5 |
...
| 要求ID5 | 成功した振込履歴を保存し、失敗時は残高・履歴を変えない | 成功時1件保存、各失敗時0件のままになる |
...
|---|---|---|---|
| 機能ID1 | 継続<br/>根拠: — | 送金前に口座と残高を確認する | 要求ID1、要求ID2 |
| 機能ID2 | 変更<br/>根拠: 変更ID1 | 認証コードを発行し、取引IDとOTPを照合する | 要求ID3 |
| 機能ID3 | 変更<br/>根拠: 変更ID2 | 検証済み取引IDを必須入力として送金する | 要求ID4 |
| 機能ID4 | 継続<br/>根拠: — | 成功履歴を保存し、失敗時の整合性を守る | 要求ID5 |
...
| 要求ID5 | 継続<br/>根拠: — | 成功した振込履歴を保存し、失敗時は残高・履歴を変えない | 成功時1件保存、各失敗時は件数・残高不変になる |
...
フェーズ5の一表から、課題ID1と課題ID2を同時に解くシステム構造を直接決めます。前工程の内容を別表へ再統合せず、「変わる側」を分離・配置し、「守る側」が接続データだけを扱える組み立てに変換します。
...
| 要求ID5 | 成功した振込履歴を保存し、失敗時は残高・履歴を変えない | `TransferHistory`、残高ログ | 成功時1件、失敗時は件数・残高不変<br/>**判定:** 合格 |

#### 最終機能の回帰・受入結果

| 機能ID | 最終状態 | 確認する要求ID | 回帰・受入結果 |
|---|---|---|---|
| 機能ID1 | 継続 | 要求ID1、要求ID2 | 不正口座・残高不足で後続処理なしを回帰確認<br/>**判定:** 合格 |
| 機能ID2 | 変更 | 要求ID3 | 取引IDとOTPの正しい組だけ認証成功<br/>**判定:** 合格 |
| 機能ID3 | 変更 | 要求ID4 | 検証済みの同じ取引IDによる送金だけ成功<br/>**判定:** 合格 |
| 機能ID4 | 継続 | 要求ID5 | 成功時だけ履歴・残高を更新し、失敗時不変を回帰確認<br/>**判定:** 合格 |
...
ここでは今回の変更ID1・変更ID2だけを完成構造へ再適用します。
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を両方満たす完成構造を一つに定め（責任配置の異なる完成構造は複数成立しないため比較は不要）、採用クラス図を3ステップでコードへ反映した |
