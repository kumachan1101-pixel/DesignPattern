...
フェーズ6は、フェーズ5で定めた課題——**振込フローの呼び出し元から、銀行APIごとの手順と失敗処理を隠す接続点を作る**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを変換して探します。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる
...
|---|---|---|---|
| P1 | 銀行API詳細から通常振込・給与バッチへ伸びる経路を、銀行別窓口で止める | 口座確認→残高確認→認証→送金→履歴の境界を切る。送金要求、成功時だけ残高・履歴更新、失敗時の結果を守る | 呼び出し元が BankGateway や認証手順を呼ばず、窓口契約だけを使う |

P1は、理想グラフで通常振込と給与バッチを変更対象から外した結果として決まります。

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 銀行API手順を1つの関数へ集める | 手順の所在が1か所になる | 要求・結果を持つ共通窓口へ上げ、通常振込とバッチから同じ形で呼ぶ |

ここからP1の境界に該当するコードだけをおさらいします。
