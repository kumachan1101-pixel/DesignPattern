...
**外部の都合で変わるものが変わるたびに、関係ないはずの自分たちの処理まで修正が必要になる。こういう問題は、「変わる理由が違うもの」が同じ場所に混在しているシステムで起きている。**
...
```mermaid
flowchart LR
    I["入力<br/>送金元口座<br/>振込先口座<br/>送金金額<br/>認証情報"]
    P["加工<br/>口座確認<br/>残高確認<br/>OTP認証<br/>送金実行<br/>履歴記録"]
    O["出力<br/>振り込み完了<br/>エラーメッセージ<br/>振り込み履歴"]
...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 送金元口座、振込先口座、送金金額、認証情報 | `fromAccount`、`toAccount`、`amount`、OTP関連の値 | 動作例テーブル、`main()`、変更要求 |
| 口座確認、残高確認、OTP認証、送金実行、履歴記録 | `TransferProcessor` の処理順序と各API呼び出し | 現状コードの観察、変更シミュレーション、原因分析 |
| 振り込み完了、エラー、履歴 | 実行結果の成功/失敗メッセージと履歴出力 | 動作例テーブル、実行結果、変更シナリオ表 |

このシステムは、ネット銀行の**振り込み処理を実行**します。

「振込先口座番号」「送金金額」を入力として受け取り、銀行の外部システムを通じて以下の手順で振り込みを完了させます。送金後の残高表示は、この章の現状コードでは扱いません。ここでは振り込み処理が成功・失敗のどちらで終わるかを確認します。
...
「失敗したら中止」というルールは、途中まで実行した状態で処理が止まることによる不整合（お金は引き落とされたのに振込先に届かない、など）を防ぐためです。料理のレシピと同じで、前の工程が成立していないと次の工程が意味をなさないため、この章の現状コードではどのステップが失敗しても後続の手順を実行しません。
...
確認したいこと：外部から見える振り込み結果を保ちながら、銀行APIの具体手順がFacadeの窓口内に閉じていること
...
| 🟣 フェーズ2：仮説立案 | 責任チェック表と変わる理由の分析で、`TransferProcessor` が外部システムの詳細を多く抱え込んでいる状態を整理した。今回の確定変更とヒアリングで判明した将来リスクを分けて扱った |
