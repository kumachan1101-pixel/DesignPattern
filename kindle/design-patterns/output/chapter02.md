...
この章で扱う現状仕様は、次の範囲です。★何故、履歴を持つ仕様がないの？
...
この章では、外部銀行APIを単純な1回の呼び出しとして扱いません。振り込みは、複数の確認を順番に通し、前のAPI応答を次のAPIへ渡す処理です。さらに実運用では、送金実行後の結果照会、タイムアウト、再試行も起こります。★以下で設計判断への影響とあるが、なぜこのタイミングで設計の話をするの？システムの仕様の説明のみが適切では？クラス構成以降で話を出すのが良いのでは？
...
★以下はエラーより先に説明する基本的な話では？基本⇒詳細⇒エラーの流れが良いのでは？
**この仕様を決める業務機能**
...
仕様を定義したところで、実際にどのような入力に対してどのような結果が返るかを確認します。このテーブルは「このシステムが正しく動いているとはどういう状態か」の基準になります。後で設計の改善（リファクタリング）を段階的に進めるときも、この表に立ち返ります。★振り込んだら、残高がどうなったかは記載しないの？送金元金額や振込金額は記載しているのに。
...
#### このシステムの登場クラス
★実際はデータベースは実現していないなどの説明はここで説明するのが良いの？コードを見ると、プリントしか実装されていない箇所が多数ある。BankGatewayって何のためにあるの？AccountDatabaseのデータのチェックとは異なるのか？
...
**クラス図に出てくる主なメンバーと操作**
★もしくは、ここで出来ることを記載したうえで、今回はプリントのみ出力としている事を記載するのが良いか。
...
★mapやVectorのライブラリが使われているが、基本的な使い方は０章に記載するのが良いか。全章通しては、ライブラリの使い方はどこかで説明したい。
...
#### 振り込み処理クラス
★クラス別にコードブロックを分割して、クラス別に説明するようにしてほしい。
...
★以下の関数の中の詳細を記載した方が良いのでは？すべて同じ場所になっていて、何が分かるのか？
...
★変更要求が反映された状態で、対策権をのステップを実行してますか？説明はそのように記載されているが、コードを見てもそのように見えず。あと、ステップ１の前の状態と退避できるようにした方が良いのでは？変更前という形でおさらいの意味です。ステップ２以降は、その前のステップがあるから、わざわざ比較しなくてもよい。
