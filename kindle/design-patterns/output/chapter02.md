...
|---|---|---|---|
| F1 | 認証フローの多段階化（二段階→三段階認証） | 銀行側のセキュリティ強化時 | 銀行API担当者との確認 |
| F2 | 送金リクエスト形式の変更（JSON→XML移行計画） | 来年以降の基幹システム連携時 | 銀行API担当者との確認 |
| F3 | 生体認証の導入 | 数ヶ月後の予定 | 銀行API担当者との確認 |

フェーズ2で「今変わること（確定）」と「将来変わるかもしれないこと（リスク）」を分けて整理できました。次のフェーズ3では、現在の構造で変更を試みたときに何が起きるかを確認します。

### 2-5：変わる見込みと当面安定の前提を確定する

2-4のF-IDを、設計で扱う変化軸へ整理します。「はい」は先行実装の指示ではなく、フェーズ6で**変化する外部仕様を振込処理の安定した入口から分離できるか**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：認証フローの多段階化（二段階→三段階認証） | はい | 認証手順の段階数と実行順 | 振込依頼の入口、送金結果、成功時だけ保存する流れ |
| F2：送金リクエスト形式の変更（JSON→XML移行計画） | はい | 外部APIへ渡す形式と変換 | 送金元・振込先・金額という業務入力、振込履歴 |
| F3：生体認証の導入 | はい | 認証方式の具体処理 | 振込処理の入口と結果契約、口座確認 |

したがって2-5の出力は、「認証手順・認証方式・外部形式は変えられるようにし、振込依頼から結果保存までの業務上の流れは守る」という設計条件です。フェーズ3ではR1・R2だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
「振り込みを実行する」という業務上の命令を処理している`TransferProcessor`が、R1で追加した「取引IDを保持する」という一時的な状態管理まで背負いました。その結果、認証変更のために振り込み処理全体を修正・再テストしています。
...
> **グラフの読み方：** この矢印は、R1・R2を適用した際に確認が必要になった依存関係を示しています。`TransferProcessor` が `Bank` と `SecurityAuthenticator` を直接知っているため、今回の認証手順変更が `TransferProcessor` を経由して両クラスへ波及したことを可視化しています。

### 3-3：痛みの言語化

**1つ目：今回の外部手順変更が業務ロジックへ波及したこと。** `TransferProcessor`が銀行APIの使い方を直接知っているため、R1の認証手順とR2の送金引数を追加するために、振り込み業務の核心部分を書き換えました。
...
---
> **📌 問題（確定）**
> R1の認証手順とR2の送金パラメータを適用すると、業務フローを管理する`TransferProcessor`を直接書き換える必要があった。外部API手順と振り込み業務が同じ場所にあり、今回の変更が業務ロジック全体へ波及した。
---
...
|---|---|---|---|
| F1：認証フローの多段階化（二段階→三段階認証） | 認証と送金の手順をBankTransferServiceの内側に集め、Applicationの窓口から隠す | BankTransferService、SecurityAuthenticator | 窓口への波及を抑えられる。三段階認証は完成コードへ追加しない |
| F2：送金リクエスト形式の変更（JSON→XML移行計画） | 外部形式の知識をBank境界とService側へ置き、TransferRequestを使う呼び出し元から分ける | Bank、BankTransferServiceの変換処理 | 一部局所化できる。XML変換は完成コードへ追加しない |
| F3：生体認証の導入 | 認証境界をSecurityAuthenticatorとして独立させ、送金窓口は認証の具体方式を知らない | SecurityAuthenticator、BankTransferServiceの認証手順 | 局所化できる。生体認証は完成コードへ追加しない |

F1〜F3はFacadeの窓口を評価する材料です。今回の完成コードには、確定要求にない認証方式や外部形式を先回りして実装しません。
...
ここでは今回の確定要求R1・R2だけを完成構造へ再適用します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：認証を、認証コード発行と取引ID付き照合の2段階へ変える | `TransferProcessor`へ発行・取引ID保持・照合の手順を追加 | `BankTransferService`内で2段階認証を完了し、誤った組み合わせでは送金しないことを確認 |
| R2：検証済み取引IDを送金APIの必須入力にする | `TransferProcessor`から認証結果を送金呼び出しへ渡す修正が必要 | 同じ取引IDを`BankTransferService`から`Bank`へ渡し、成功時だけ残高と履歴を更新することを確認 |
