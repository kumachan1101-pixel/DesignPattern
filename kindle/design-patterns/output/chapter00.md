...
★以下の「変わるもの」と「変わらないもの」を分離するは、以降の説明で登場しますか？変わる理由が複数の決定者かどうか、しか論点がなさそうです。ただ、デザインパターンの各章の説明を見ると、変わるものと、変わらないものが、本質があり、分離しているように見えます。この２点、明確に使い分けした方が良いのではないですか。１章以降の説明に大きく影響します。多分ですが、２番の変わるものの決定者を特定して分離したら、おのずと、１番の変わるものと変わらないものを分離できた状態になるのではないでしょうか。正しければ、そのようにしたいので、２つに分けるのではなく１つにしてください。そうすれば、以降の使い方と辻褄があい、修正量も少なくなりますかね。
...
★継承でクラス膨大した、RetryLoggingEmailNotifierの例は、コンポジションだとどのように改善されるのか、コード例を記載してください。
...
★以下の哲学1がぶれはじめています。結局、変わるものの中で決定者を分離する話は含むのか？含まないのか？
...
★ステップ0から2が、上記で言う現状把握ですよね？グルーピングが分かるようにして。色だけだとわかりづらくないですか？
...
★哲学１では、変わるものと変わらないものを決定者の観点で見極めるという話では？責任範囲外のコードが書かれているか？を具体的にどうやるかを記載して
...
★原因は３つの哲学に紐づく？本当に？対策は、分ける、統合、中間層、もの化ですよね？少なくとも対策が４つあるという事は、この４つの原因に分類できるという事ではないのですか？つまり、最後の対策一覧と原因は密接な関係があると思っていて、ここでも、最後の対策一覧に紐づける必要がないのですか？対策一覧は原因一覧でもあるかのように思えるのです。読者にはこの本の思考ロジックを使えば、原因と対策のプロセスを導き出してくれる、そんな本にしたいのです。ただ、ステップ５に記載しているように、複数の手札で処置できない場合があるので、そのあたりの記載の工夫は必要です。
...
その結果によって、次の3つのケースに分かれます。
★以下のケース①②は同じですよね？要するに、複数の対策候補が出て場合に、天秤をかけるでしょ？①②は、結果的に対策候補が１つしかないという話ですよね？

1. **ケース①：候補が1つの場合**
   その手札を直接適用します。コードで実装し、変更後のクラス図を示します。このプロセスの最後に、辿り着いた結論として初めて「パターン名」が登場します。ステップ6では天秤（比較表）は不要で、耐久テストと「使う/使わない判断」だけを行います。

2. **ケース②：候補が複数・組み合わせで解決できる場合**
   複数の手札を組み合わせて、1つの統合された実装として示します。クラス図でどの手札がどこに適用されているかを明示します。ステップ6では天秤は不要です。

3. **ケース③：候補が複数・別々に試す必要がある場合**
   手札Aを手段①として実装し、「残る課題」を言語化します——これが手段②への橋渡しになります。次に手札Bを手段②として実装し、残る課題を解消します。クラス図とともに示し、パターン名は手段②の後に初登場します。手段①も手段②も、どちらも手札選択表から選んだ正当な候補です。ステップ6では2つの手段を天秤にかけます。
...
★以下の説明分かりませんん。要するに、対策の検討をした結果、複数の候補が出た場合に、天秤にかけるですよね？何で、２つの手段に限定されている？手段の定義も記載してくれないか？私が言っている複数の候補が手段という事ですよね？つまり、対策候補として２通り以上ある場合。３通りかもしれないし④通りかもしれないですよね？何故、手段が２つの前提になっているのかわかりません。

**ケース③（手段①・手段②を別々に試した場合）のみ：**
...
★例として以下を記載しているが、DiscountCalculatorは知らないから、あまりイメージが湧かない。もっとわかりやすく説明して。変更シナリオというのは、ステップ１から６で出てきた変更をまとめたという事？整理する目的は何ですか。次の項目の表で説明されている、「中心の問い」と一致してますか？「何を得て何を諦めたか」とありますが、その内容になっているのでしょうか。この指摘が出てしまうと、他のステップも、「中心の問い」と一致しているのかが気になってきます。見直してほしい。
...
> [!INFO] 「手札」という言葉について
> この本では、設計の構造的な操作のことを**「手札」**と呼びます。
> トランプのように「あらかじめ決まったカードの束」があり、原因の種類に応じて適切な1枚を引いて使うイメージです。
> 「直感」や「センス」で解決するのではなく、原因と手札の対応表を参照して体系的に選ぶ——それがこの本の立場です。

★手札を組み合わせて対策することも、手段と呼ぶ？手札自体も手段だと思うのですが。手段ではなく、対策案の方が良いのではないですか？言葉の定義を明確にして見直してほしい。
