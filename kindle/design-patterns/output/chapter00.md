...
原因が言語化できれば、いきなりパターンを当てはめる必要はありません。まず「どっちの方向に進むべきか」という大まかな指針（方向性）を定め、そこから具体的な解決策へと段階的に近づいていきます。このステップを踏むことで、「なぜそのパターンが必要だったのか」という必然性が読者（そして自分自身）に明確になります。

**このステップでやること**

原因の種類から、まず「解消の方向性」を定めます。「混ざっているものを分ける」「分散しているものを集める」「実装をインターフェースの裏に隠す」など、原因から自然に導かれる方向を選びます。

次に、具体的な解決手段を順番に試していきます。

1. **手段①：直感的な案**
   まず思いつくシンプルな方法（関数の切り出し等）をコードで試します。
2. **残る課題の発見**
   手段①を適用した後に残る課題（「まだ骨格が複製されている」「呼び出し元に判断が残る」など）を言語化します。これが次のステップへの動機になります。
3. **手段②：本質的な案（発想の転換）**
   手段①の課題を解決するために、本章の末尾にある「手札の選択表」を引いて、インターフェースなどを使ったより本質的な手札を適用します。

このプロセスの最後に、辿り着いた結論として初めて「パターン名」が登場します。

**3つの哲学との接続**

手段①から手段②への転換は、**哲学1**（変わるものを完全にカプセル化する）を徹底しようとする過程そのものです。そして手段②でインターフェースを導入する判断は、**哲学2**（インターフェースに対してプログラムする）の具体化となります。
...
解決策（手段②：パターン）を導き出した後に、一度立ち止まる必要があります。どんな設計も万能ではなく、必ず複雑化というコストを伴うからです。手段①（シンプルな案）と手段②（本質的な案）を冷静に天秤にかけ、「本当にこの複雑さのコストを払ってまでパターンを使うべきか？」を判断します。

**このステップでやること**

まず、比較のための**評価軸を先に宣言**します。
- 「変更の局所性（1つの変更が1箇所で済むか）」
- 「テストの独立性（各部品を単独でテストできるか）」
- 「実装のシンプルさ（コードが理解しやすいか）」

次に、手段①と手段②をこの評価軸で比較した表を作成します。基準を先に置くことで、結論ありきの後付けの比較を防ぎます。

比較の結果、今回の状況において手段②が優れていると判断できれば、それを正式に採用します。

最後に、ステップ2のヒアリングで出た「将来の変化」を実際にコードで試す**耐久テスト**を行います。「新しい部品を追加するだけ」でその変化に対応できることを実証し、設計の正しさを確信に変えます。
