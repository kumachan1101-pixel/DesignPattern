...
graph TD
    ...
    C["第2章<br/>Facade"]
    D["第3章<br/>State"]
    E["第4〜8章<br/>Observer / Decorator<br/>Template Method / Iterator 等"]
...
- **骨格（流れ）を固定して、各ステップの中身だけを変えたい**場合。「処理のステップ順序は変えず、各ステップの実装だけをサブクラスで差し替える」というパターンがあります。この本では扱いませんが、そういう場面では継承を意図的に使います。
...
まず「一番シンプルな解決策」を試みます。たとえば割引ルールを引数で渡す、専用の関数に切り出す、など。試みた結果、どこで限界に当たるかを確認します。

```
【試み①】割引率を引数として外から渡す

  例：calculate(double discountRate) という形にする

  → 「大口法人のとき、プレミアム会員かつ100個以上なら追加5%引き」
    のような複合条件を「割引率1つ」では表現できない。
  → 条件の判断ロジックが呼び出し元（BillingCalculatorを使う側）に移っただけで、
    「判断する責任がどこにあるか」という問題は解消されていない。
```

> `BillingCalculator` とは各章に登場する「割引計算の骨格を担うクラス」です。
> 「合計金額を計算する」という処理の流れを担い、具体的な割引ルールは外から受け取ります。

限界の理由を掴んだら、次の試みに進みます。

```
【試み②】「割引ルール」という振る舞いそのものをインターフェースとして切り出す

  → BillingCalculator は「apply() を持つ何か」という契約だけを知ればいい。
  → 新しい割引ルールが追加されても、BillingCalculator は変わらない。
  → 呼び出し元は「どの割引か」を知らなくていい。
```

**3つの哲学との接続**

試み②の発想の転換が、**哲学1**（分離）と**哲学2**（インターフェースで繋ぐ）の適用そのものです。試みが失敗した理由を分析するとき、「どの哲学が実現できていないか」を問うと方向が定まります。解決後の組み立て（Composition Root：「具体クラスを知ってよい唯一の場所」）は**哲学3**（コンポジション）の具体化です。
...
哲学1（変わるものをカプセル化せよ）の実践は、「変わるのが何か」を正確に言語化することから始まります。

### 手札の選択表

| ステップ4で特定した「変わるもの」 | 使う手札 | 対応する章 |
|---|---|---|
| アルゴリズム・ルールの内容 | ①インターフェース抽出 | 第1章（Strategy） |
| IDや識別子の型 | ②値オブジェクト化 | 全章（共通） |
| 機能の組み合わせ | ③コンポジション | 第4章以降 |
| 外部依存のAPI・呼び出し構造 | ④ファサード集約 | 第2章（Facade） |
| オブジェクトの状態 | ⑤状態オブジェクト化 | 第3章（State） |
| 共通骨格の中の個別ステップ | ⑥骨格・ステップ分離 | 第4章以降 |

ステップ5では「ステップ4で特定した変わるものは何か → 上の表の該当行の手札を使う」という順序で進めます。

### なぜこの6つか

「変わるもの」を分類すると、OOP設計で問題を引き起こす変化は6種類に収まります。

アルゴリズムが変わる、型が変わる、機能の組み合わせが変わる、外部依存が変わる、状態が変わる、処理ステップが変わる——この6種類が、本書で扱うパターン（Strategy / Facade / State / Decorator / Template Method）のカバーする変化の種類と対応しています。

この本の範囲内では、「どの手札を使うか」はステップ4の出力から決まります。
