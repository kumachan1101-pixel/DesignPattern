...
★ここでもう一声入れたい。この本を読むと、デザインパターンという結果がどのような思考で生まれたのかが、
デザインパターン自体の本質も理解でき、プロセスの思考を柔軟に身に着けることが出来るため、デザインパターンを適用とするのではなく、独自の状況に合わせて、適切な考え方で対処する、いわば自分なりの型で対応できるようになると思ってます。

★この本はC++で扱っている点、本の前提になる話なので、先頭で説明した方が良いです。
...
★以下の図が、目次と異なっています。また、表の方が見やすくないですか

```mermaid
graph TD
    A["第0章<br/>3つの哲学 + 8ステップの思考プロセス"]
    B["第1章<br/>Strategy<br/>変わる：アルゴリズム・ルール"]
    C["第2章<br/>Facade<br/>変わる：外部依存のAPI・構造"]
    D["第3章<br/>State<br/>変わる：オブジェクトの状態"]
    E["第4章<br/>Decorator<br/>変わる：機能の組み合わせ"]
    F["第5章<br/>Template Method<br/>変わる：骨格内の個別ステップ"]
    G["第6章<br/>Observer<br/>変わる：通知先の種類"]
    H["第7章<br/>Iterator<br/>変わる：コレクションへのアクセス方法"]
    ...
    style F fill:#e8f4e8,stroke:#006600
    style G fill:#e8f4e8,stroke:#006600
    style H fill:#e8f4e8,stroke:#006600
```
...
★何か以下の説明分かりづらい。以下の２つの論点があると思ってます。正しいなら明確に区別して
・変わるものと変わらないものを分離する
・変わるものの中で複数の決裁者がいる場合は分離する

#### なぜこの哲学が生まれたのか

コードが変わる理由は、ビジネスの変化によって生まれます。
割引ルールを決めるのは営業チームです。API仕様を変えるのはインフラ担当です。
出力フォーマットを決めるのは経理担当です。
...
★これはとても大切なので強調したい。シンプルに判断者はそれほど多くない。何かのシステムを例に幾つか例を出すとイメージが湧きます。

下の図で、問題のある構造と解決後の構造を比べてみてください。
★以下のコメントが横長になりすぎていて見づらい
...
★変わるものと、変わらないものを、分けて記載しませんか。分離した結果の変わるもの側だけ書かれても、もともと全体が何だったのかわかりませんので、よくわかりません。

| パターン | 分離した「変わるもの」 |
|---|---|
| Strategy（第1章） | 実行する振る舞い（アルゴリズムやルール） |
| Facade（第2章） | 複雑な外部連携の詳細 |
| State（第3章） | 状態とそれに伴う振る舞い |
| Template Method（第4章） | 処理の各ステップの実装 |
| Command（第5章） | 実行する操作そのもの（要求の発生と実行） |
| Decorator（第6章） | 追加する機能の組み合わせ |
| Observer（第7章） | 通知先の種類や依存方向 |
| Factory Method（第8章） | 作るオブジェクトの種類（生成と利用） |

GoFのパターンは、決して最初から目指すものではありません。「変わるものをカプセル化せよ」という哲学を適用した結果の姿に過ぎないのです。
...
★残りのデザインパターンを見ることで、設計の知識は深まるという事は補足してほしい。知らなくてもよいとなると、では、もともと何のために２３個のデザインパターンが定義されたのか、となってしまいます。

★タイトルから察するに、以下説明の定義を明確にして。上記課題に該当するから、哲学を適用しよう、でも、こんなケースでは適用すると危険、という話だと思ってます。でも内容を見ると、少しニュアンスが違うように見えます。哲学の適用が難しいケースがありますという内容にも見える。ここの項目を変えると、哲学2と3の動向も国も変える必要があるから慎重に検討して。
#### この哲学を意識的に外すとき
...
★哲学１の以下に相当する項目の書き方を変えるならこちらも変えてください。
#### この哲学を意識的に外すとき
...
- **is-a 関係が成立し、振る舞いを「重ねる」必要がない**場合。たとえばシステムに「管理者ユーザー」と「一般ユーザー」という2種類があって、管理者が一般ユーザーの機能をすべて持ちつつ追加権限を持つ——この場合は `AdminUser extends User` という継承が概念を正直に表現します。振る舞いを後から組み合わせるのではなく、分類するだけなら継承は素直な選択です。★誰もが最初はis aと思って実装したが、後から機能追加があった場合に、そのまま継承を続けてしまった結果、壊れていくという話に見えたのですが、違うのでしょうか。
...
>
> 3つの哲学を読んで気づいた通り、どの哲学にも「意識的に外すとき」があります。
