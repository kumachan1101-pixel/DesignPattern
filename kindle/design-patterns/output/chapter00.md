...
- **骨格（流れ）を固定して、各ステップの中身だけを変えたい**場合。「処理のステップ順序は変えず、各ステップの実装だけをサブクラスで差し替える」という場面では継承を意図的に使います。手札の「骨格とステップの分離」がこれに対応します。
...
ステップ4で原因が言語化できれば、使う手札は絞られます。「変わるのは何か」が明確なら、このセクション末尾の「構造的対策（手札）の網羅的リスト」から対応する手札を引けます。手札を闇雲に当てはめるのではなく、原因から逆算して手札を選ぶことが、このステップの要点です。

**このステップでやること**

ステップ4の原因を確認し、「変わるのは何か」を一言で言語化します。その言葉を手札のリストに照らし、対応する手札を特定します。

```
例：ステップ4の結論が「任意の振る舞い（割引ルール）の内容が変わる」なら
  → 手札リストの「任意の振る舞い」の行
  → 手札「インターフェースによる抽象化」を使う
```
...
| 特定した原因（何が変わるのか・どんな痛みか） | 使うべき手札（設計の構造的対策） |
|:---|:---|
| **一部の共通処理**が複数箇所で重複・散在している | **処理の抽出と共通化（関数の切り出し）** |
| **既存の関数呼び出し**の前後に処理を追加したい | **処理のラップ（ラッパー関数の導入）** |
| **内部変数が多すぎて**切り出せない巨大関数 | **関数のオブジェクト化（メソッドオブジェクトの導入）** |
| **巨大なクラス**が複数の異なる役割を抱えている | **クラスの分割（責任の委譲）** |
| **内部のデータ構造**（List等）が外部から直接操作される | **コレクションの隠蔽（カプセル化）** |
| **グローバル変数や暗黙の依存**によりテストができない | **依存の外部注入（パラメータ化・DI）** |
| **任意の振る舞い**（計算ロジック、ルールなど） | **インターフェースによる抽象化（境界を引く）** |
| **機能の組み合わせ**（ログ付き、リトライ付きなど） | **継承から委譲への転換（コンポジション）** |
| **オブジェクトの生成判断**（どのクラスを作るか） | **生成と利用の分離（制御の反転）** |
| **外部システム**のAPI仕様や呼び出し順序 | **窓口の集約（複雑性の隠蔽）** |
| **要求の発生**と**実行**のタイミングのズレ | **振る舞いのデータ化（処理のオブジェクト化）** |
| **プリミティブな型**（ID、フラグ、金額など） | **概念への命名（値と振る舞いの一体化）** |
| 既存部品との**インターフェースの不一致** | **間へのクッション配置（翻訳層の導入）** |
| **オブジェクトの状態**とそれに伴う振る舞い | **状態のオブジェクト化** |
| **共通の骨格**に対する部分的なステップ | **骨格とステップの分離** |
| **通知先**や処理の依存方向 | **依存の逆転（イベント駆動への転換）** |
| オブジェクト間の**多対多の複雑なやり取り** | **通信の調停（仲介者の導入）** |
| オブジェクトへの**アクセス制御**や遅延評価 | **代理人の導入** |
| コレクションの**要素へのアクセス手順** | **走査手順の分離** |
| **全体と部分**（ツリー構造など）の扱い | **階層構造の同一視** |
| オブジェクトの**内部状態の保存と復元** | **スナップショットの作成** |
| 生成コストが高いオブジェクトの**重複** | **オブジェクトの共有化** |
| データ構造に対する**後付けの処理** | **データ構造と処理の分離** |
