...
> 以下の表は、各章を読み進めた後に「あのパターンはここに分類されるのか」と確認するための参照表です。今はパターン名を知らなくて構いません。

| パターン | 分離した「変わるもの」 | 使う手札 |
|---|---|---|
...
## 問題を発見した後の手札 ―― 「変わるのは何か」で手札を選ぶ

ステップ5は「ではどうやって分離するか」を検討する場所です。しかし手札を闇雲に当てはめても意味がありません。**ステップ4で特定した「変わるもの」が、使う手札を決めます。**

### 手札の前に：実装レベルの技法と設計レベルの手札は違う

変更を受けたとき、最初に思いつくのは「引数を変える」「関数を新規追加する」といった実装レベルの対処です。これらは間違いではありませんが、**設計レベルの結合問題を根本的には解消しません。**

| 技法 | レベル | 結合問題を解消するか |
|---|---|---|
| 引数を変える | 実装レベル | ✗ 判断ロジックが呼び出し元に移るだけ |
| 関数を新規追加する | 実装レベル | ✗ 骨格がコピーされ変更箇所が増える |
| 手札①〜⑥ | 設計レベル | ✅ 変わる理由を持つクラスを構造的に分離する |

手札①〜⑥は「変わるものを独立した単位として定義する」という設計の操作です。実装レベルの技法が「今のコードをどう直すか」を考えるのに対し、手札は「誰が何を知るべきか」という責任の境界線を引きます。

### 手札の選択表

| ステップ4で特定した「変わるもの」 | 使う手札 | 対応するパターン |
|---|---|---|
| 任意の振る舞い（アルゴリズム・ルール・通知先・操作など） | ①インターフェース抽出 | Strategy / Observer / Command |
| IDや識別子の型 | ②値オブジェクト化 | 全章（共通） |
| 機能の組み合わせ | ③コンポジション | Decorator |
| 外部依存のAPI・呼び出し構造 | ④ファサード集約 | Facade |
| オブジェクトの状態 | ⑤状態オブジェクト化 | State |
| 共通骨格の中の個別ステップ | ⑥骨格・ステップ分離 | Template Method |

手札①は「変わる振る舞いをインターフェースとして定義する」という汎用的な手段で、最も広く使われます。Strategy・Observer・Command・Iteratorはいずれも手札①の応用です。

### なぜこの6つか

OOP設計で問題を引き起こす変化を「変わるのは何か」という軸で分類すると、**振る舞いが変わる・型が変わる・組み合わせが変わる・外部依存が変わる・状態が変わる・ステップが変わる**の6種類に収まります。

ただしこれは「プログラミングにおける全ての変化」を網羅しているわけではありません。たとえば「生成の方法が変わる（Factory系）」「既存インターフェースを変換する（Adapter）」といった変化は、本書の扱うパターンの範囲外であるため手札に含まれていません。この本の範囲内では、ステップ4の出力から手札が決まります。

---

以下に各手札の詳細を示します。各章を読み進める際に「ステップ5でどの手札を使っているか」と照らし合わせながら参照してください。

### 手札①：インターフェース抽出

**変わるのは：** 任意の振る舞い（アルゴリズム・ルール・通知先・操作など）

**使う状況：** 呼び出し元が「具体的な実装クラス」に直接依存していて、実装が変わるたびに呼び出し元まで変更が波及する。振る舞いの内容（何をするか）が変わるあらゆるケースが対象。
...
**繋がるパターン：** 各章で登場するパターンのほぼすべてがこの手札を使います
...
**繋がるパターン：** 第1章（Strategy）・第3章以降で登場するパターン

---

### 手札④：ファサード集約

**変わるのは：** 外部依存のAPI・呼び出し構造
...
これらは「組み合わせて使う」ものです。たとえば①と②は、インターフェースを定義するとき常に一緒に検討します。各章を読み終えると「この章ではどの手札を使ったか」が見えるようになります。
