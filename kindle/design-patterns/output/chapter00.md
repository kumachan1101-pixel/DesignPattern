...
>
> 設計の判断には必ず「状況」が関係します。
> 変化が来ないなら分離はコストです。実装が1つしかないならインターフェースは不要です。
> is-a 関係が成立するなら継承は正しい選択です。
>
> 哲学は「守らせるもの」ではなく、**「逸脱を意識させるもの」**です。
> 逸脱するとき、その理由を言語化できていれば、それは正しい判断です。
> 言語化できていなければ、それは偶然の設計です。
>
> 各哲学の末尾に「この哲学を意識的に外すとき」を示します。
> 例外を知ることが、哲学を道具として使いこなす第一歩です。
...
- **変わる見通しがない**場合。ステップ2のヒアリングで「変わらない」と合意できたものは分離不要です。分離のコストが価値を超えます。
- **大規模なレガシーコードで、絡み合いが深い**場合。一気の分離はリスクです。段階的な置き換え（新しい機能から新しい構造で書く）の方が現実的です。
- **概念的に不可分なデータと操作**の場合。`Point.x` と `Point.y` を別クラスに分けても意味がありません。「変わる理由が同じもの」は一緒にいて良い。
...
解決後の組み立て（Composition Root：「具体クラスを知ってよい唯一の場所」）は**哲学3**（コンポジション）の具体化です。
