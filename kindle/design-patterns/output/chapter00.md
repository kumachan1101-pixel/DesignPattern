...
ステップ0〜2が「問題の把握」をそれぞれ異なるタイミングで行うことに注目してください。

| ステップ | タイミング | やること |
|---|---|---|
| **ステップ0** | コードを読む**前** | 言葉だけを頼りに「何が変わりそうか」を予測する |
| **ステップ1** | コードを**読みながら** | 各クラスの責任を確認し、責任外の知識を見つける |
| **ステップ2** | コードを読んだ**後**（ヒアリング） | 予測を関係者との対話で根拠づけ、確定する |
...
コードを読む前に問いを立てておかないと、コードを読みながら「動きを追う」だけになります。
設計の読み方は「このコードは正しく動くか」ではありません。
「変わる理由がどこに混在しているか」という問いで読むことです。

問いを持って読むか、持たずに読むかで、同じコードから得られる気づきが大きく変わります。

**このステップでやること**

まず、システムのシナリオ説明を聞きます（言葉だけ。コードはまだ開きません）。
「どんな業務を担当するシステムか」がわかった段階で、仮説テーブルを埋めます。

> 「このシステムの中で、何が変わりやすく、何は変わらないか？」

コードを読まなくても、ビジネスの性質から「変わりやすそうな部分」はある程度想像できます。
外部ベンダーの都合で変わるもの、営業施策のたびに変わるもの、「会社がある限り変わらない」もの。

| 分類 | 仮説 | 根拠（言葉の段階で感じたこと） |
|---|---|---|
| 🔴 変動しそう | （例）APIの仕様 | 外部ベンダーの都合で変わりそうだから |
| 🟢 変わらなそう | （例）業務フローの骨格 | 会社の業務根幹だから |
...
「変わるものをカプセル化する」ためには、まず「何が変わるか」を予測することから始まります。
...
「このクラスは何のためにあるか」「誰の判断で変わるか」を定義しないまま読んでも、
設計の問題は見えてきません。

**このステップでやること**

まず仕様表を見て、各クラスの「責任（1文で言えること）」を言語化します。

| クラス | 責任（1文） | 知るべきこと |
|---|---|---|
| （例）OrderService | 受注処理のフローを完了させる | 処理の順序 |
| （例）IDiscountCalc | 割引額を計算する | 割引のルール |

責任を言語化したうえで、コードを1行ずつ読みます。
...
|---|---|---|---|
| ステップ0 | コードを読む**前** | 何が変わりやすそうか（予測） | 哲学1への準備 |
| ステップ1 | コードを**読みながら** | 各クラスの責任は1つか | 哲学1（責任の単一性） |
