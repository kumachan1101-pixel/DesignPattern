...
原因が言語化できれば、次にやることは1つです。この章の末尾にある**「手札の選択表」を引く**ことです。「直感」や「思いつき」で試行錯誤するのではなく、ステップ4で特定した原因の種類に対応する手札を表から選びます。
...
1. **候補が1つの場合**
   その手札を直接適用します。コードで試し、変更後のクラス図を示します。このプロセスの最後に、辿り着いた結論として初めて「パターン名」が登場します。

2. **候補が2つ以上の場合**
   候補の中でよりシンプルな手札から順に試します。最初の手段（手段①）を適用した後に「残る課題」を言語化します——これが次の手段への橋渡しになります。より本質的な手段（手段②）で残課題を解消し、クラス図とともに示します。パターン名は手段②の後に初登場します。
...
次に、手段①と手段②をこの評価軸で比較した表を作成します。基準を先に置くことで、結論ありきの後付けの比較を防ぎます。比較の結果、今回の状況において手段②が優れていると判断できれば、それを正式に採用します。

**候補数に関わらず必ず行うこと：**

ステップ2のヒアリングで出た「将来の変化」を実際にコードで試す**耐久テスト**を行います。「新しい部品を追加するだけ」でその変化に対応できることを実証し、設計の正しさを確信に変えます。

また、「この手段を使わない方が良い状況」を具体的なコード例で示します。設計は常に採用すべきではなく、変更頻度・チーム規模・将来の見通しによって判断が変わります。
