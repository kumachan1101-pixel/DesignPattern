...
| パターン | 分離した「変わるもの」 |
|---|---|
| Strategy（第1章） | アルゴリズム・ルールの内容 |
| Facade（第2章） | 外部サービスとの連携詳細 |
| State（第3章） | 状態ごとの振る舞い |
| Decorator（第4章） | 追加する機能の組み合わせ |
| Template Method（第5章） | 処理の各ステップ |
| Observer（第6章） | 通知先の種類 |
| Iterator（第7章） | コレクションへのアクセス方法 |
...
- **骨格（流れ）を固定して、各ステップの中身だけを変えたい**場合。「処理のステップ順序は変えず、各ステップの実装だけをサブクラスで差し替える」という場面では継承を意図的に使います。手札⑥（骨格・ステップ分離）がこれに対応します。
...
「Strategyパターンを使う」「Facadeパターンを使う」というのは、手札ではありません。それは特定の状況に対して先人たちが複数の対策を組み合わせた「結果（カタログ）」に過ぎません。現実の泥臭いコードが、都合よくカタログ通りに解決できるほど世の中は甘くありません。

私たちが本当に持つべき手札とは、特定のパターン名ではなく、**あらゆる原因に対して処置できる抽象度の高い「対策の考え方」そのもの**です。

### 実装の小手先と、設計の「手札」の違い

痛みに直面したとき、「とりあえず引数を増やす」「if文を追加する」といった実装レベルの対処は、問題を先送りするだけで根本解決にはなりません。設計の手札とは、コードの骨格（責任の境界線）を引き直す操作のことです。

### あらゆる原因に対応する「7つの抽象的な手札」

ソフトウェアの変更に関する根本的な原因（痛み）を取り除くための、抽象度の高い対策の考え方を「7つの手札」として網羅しました。世の中の優れた設計（GoFのパターンや各種アーキテクチャ）はすべて、この考え方のいずれか、あるいは組み合わせによって成り立っています。

#### 1. 「境界」を引く（抽象への依存）
**直面する原因：** 呼び出し元が「相手がどうやって処理しているか（詳細）」を知りすぎているため、相手が変わると道連れになる。
**対策の考え方：** 具体的な実装を直接触らせず、「何ができるか」という契約（インターフェース）の境界を引く。呼び出し側は契約だけを信じ、裏側の変化から守られる。

#### 2. 「委譲」する（所有への転換）
**直面する原因：** 継承を使って機能を足していった結果、クラスが爆発している。または1つの巨大なクラスがすべてを抱え込んでいる。
**対策の考え方：** 自分が親から受け継ぐ（is-a）のではなく、別の小さな専門家を内部に持ち、その専門家に仕事を任せる（has-a / コンポジション）。専門家を付け替えるだけで振る舞いが変わる柔軟性を得る。

#### 3. 「生成」を外に追い出す（制御の反転）
**直面する原因：** 処理の流れ自体は完璧なのに、コードの中で 
ew（具象クラスの指定）をしているせいで、特定の環境や実装に縛り付けられている。
**対策の考え方：** 自分で必要なものを作るのをやめ、外から「すでに完成した部品」を渡してもらう（依存性の注入など）。使う側のコードから「作ることの責任」を完全に引き剥がす。

#### 4. 「窓口」を絞る（複雑性の隠蔽）
**直面する原因：** 複数の外部システムや複雑なサブシステムを使うため、呼び出す順番や仕様などの前提知識が多すぎる。
**対策の考え方：** 複雑なやり取りを裏側で一手に引き受ける「代表者」を立てる。呼び出し元には最もシンプルな1つの入り口だけを見せ、背後の複雑さを完全に隠蔽する。

#### 5. 「振る舞い」をモノとして扱う（処理のデータ化）
**直面する原因：** 「どのタイミングで何を実行するか」という制御ロジックと、実際の処理ロジックが絡み合っている。
**対策の考え方：** メソッドとして固定するのではなく、処理自体を変数に入れられる「オブジェクト（モノ）」に変換する。これにより、処理を持ち運んだり、順番を並び替えたり、履歴として保存したりできるようになる。

#### 6. 「概念」に名前をつける（型による保護）
**直面する原因：** 単なる int や string がシステム中を飛び交い、あちこちで「それが有効な値か」のチェック（if文）が重複している。
**対策の考え方：** プリミティブな値や「状態」を、ただの変数として扱うのではなく「独立した型（クラス）」として格上げする。その型自身にルールを守らせることで、システム全体の安全性を担保する。

#### 7. 「間」にクッションを挟む（間接化と翻訳）
**直面する原因：** 繋ぎたい2つのシステム（またはクラス）があるが、インターフェースが合わない、あるいは直接対話させると依存関係が複雑になりすぎる。
**対策の考え方：** 両者が直接やり取りするのをやめさせ、間に「翻訳者」や「仲介役」を配置する。ズレの吸収や複雑な調整をクッション層に押し付ける。

---

### 原因から手札を引く（問いの立て方）

現場でコードの痛みに直面したとき、パターンのカタログを開くのではなく、原因からロジカルに手札を引いてください。

| 痛みの根本原因（ステップ4の結論） | 検討すべき手札 |
|---|---|
| 「相手の実装詳細を知りすぎている」 | **1.「境界」を引く** |
| 「機能を組み合わせるたびにクラスが増殖する」 | **2.「委譲」する** |
| 「使う部品が決まってしまう（テストでモックにできない）」 | **3.「生成」を外に追い出す** |
| 「この処理を呼び出すための前提知識が多すぎる」 | **4.「窓口」を絞る** |
| 「実行する処理を後から自由に変更・取り消ししたい」 | **5.「振る舞い」をモノとして扱う** |
| 「この int の値が正しいかどうかの if 文が散らばっている」| **6.「概念」に名前をつける** |
| 「直接繋ぐとお互いが強く結びついてしまう」 | **7.「間」にクッションを挟む** |

### デザインパターンは「手札を使った結果のよくある形」

この抽象的な「7つの手札」を組み合わせて特定の問題を解決した結果、出来上がる構造にはいくつかの**「よくある定番の形」**があります。それに名前を付けたものがGoFのデザインパターンです。

- 「境界」を引き、「委譲」を使ってアルゴリズムを切り替えた結果 ＝ **Strategyパターン**と呼ばれる。
- 「概念」に名前をつけて状態を型にし、「委譲」で切り替えた結果 ＝ **Stateパターン**と呼ばれる。
- 「窓口」を絞って背後の複雑さを隠した結果 ＝ **Facadeパターン**と呼ばれる。
- 「生成」を外に追い出し、工場側に「境界」を引いた結果 ＝ **Factoryパターン**系と呼ばれる。
