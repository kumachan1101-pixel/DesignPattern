...
| --------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------- |
| ・1つのファイルが数千行に膨れ上がっている<br><br>  <br><br>・「〇〇Manager」という名前のクラスがある | 変更の理由が異なる機能が癒着して固まっているため、**物理的に切り離す**。         | クラスの分割<br><br>  <br><br>（単一責任）        |
...
|**具体：コードの不吉な臭い**|**抽象：物理操作の理由**|**具体：構造的対策案**|
|---|---|---|
|・`int` などの生データがそのまま引き回されている|生データを箱で包み込み、直接書き換えられないよう**蓋をして守る**。|値オブジェクト化|
|・外部APIを呼ぶための手順や順番が複雑すぎる|複雑な手順を一つの箱に閉じ込め、外には**「代表のポッチ（窓口）」だけを見せる**。|窓口の集約<br><br>  <br><br>(Facade)|
...
|**具体：コードの不吉な臭い**|**抽象：物理操作の理由**|**具体：構造的対策案**|
|---|---|---|
|・`if (type == A)` のような型による分岐が無限に増える|中身が違う複数の部品の、**上面のポッチの形（呼び出し方）を全く同じ規格に統一する**。|インターフェース抽出<br><br>  <br><br>(Strategy 等)|
