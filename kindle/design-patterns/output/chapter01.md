...
* **得られること2：** 接続点（クラスとクラスのつなぎ目）が「具体×直接」（専用の型を直接知っている状態）になっているクラスを見て、そこが変更の痛みの発生源だと判断できるようになる
* **得られること3：** 接続点の形を変えると変更がどのように局所化（変更の影響が1クラスだけで済む状態）されるかを、構造から説明できるようになる
* **得られること4：** 増え続けるルールに対して、いつ・どのように構造を分けるべきかの判断ができるようになる

この章を読み終えると、「新しい割引ルールの追加」が**既存コードに一切触れずに完結する**構造を、自分の手で作れるようになります。
...
当時の担当者が、厳しい納期や急な仕様変更の中で必死につないできた跡が、このコードには詰まっています。このコードが今日まで現場の売上を守ってきたことは確かです。
...
このコードは正しく動いています。プレミアム会員向けの20%オフが適用され、10000円の商品が8000円になっています。これから検討するのは、同じ機能を保ちながら、変更に強い構造をどう作るかという点です。
...
こういうとき困る、という感覚が伝わっているでしょうか。まずは現状の辛さを言語化することが、良い設計への第一歩になります。
...
このシステムの規模では、仮想関数呼び出しのオーバーヘッドやメモリ消費は設計判断を絞り込む制約になりません。唯一注意すべき点はタイムセール時の同時実行です。複数リクエストが同一のルールオブジェクトを共有する場合、**ステートレス（内部状態を持たない）設計**にしておくことが基本方針になります。この点は案3・案4の選定に影響するため、各案のトレードオフで触れます。
...
* ※ タイムセール時の同時実行：`DiscountManager` がステートレス（内部状態なし）であれば複数リクエストで共有しても安全。状態を持つ場合はリクエストごとに生成する設計が必要になる。
...
* ※ タイムセール時の同時実行：`IDiscountManager*` 経由で共有する `DiscountManager` がステートレスであれば安全。インターフェース越しに注入される構造のため、スレッドセーフな実装に差し替えることも容易。
...
> **なぜ案4（抽象×間接）ではなく案2（抽象×直接）か**
>
> 採点表を見ると、案2（17点）と案4（16点）は僅差です。決め手になったのは**可読性の1点差**と、このシステムの使われ方です。案4では「どの割引クラスを使うかを決める仲介役（Factory）」がひとつ増え、新規メンバーがコードを読むときに「組み立てがどこで起きるか」を追跡するコストが増します。
>
> この支払い計算は1リクエストごとに呼ばれるホットパスです。一方で、割引ルールの切り替えは「月に1回の設定変更」であり、**実行中に動的にルールの種類を変える要件はありません**。その頻度なら、`main()` や組み立てコードで直接クラスを渡すだけで十分で、Factory経由の間接層を挟む理由がありません。
>
> 将来「ルールの種類が実行時の入力によって動的に決まる」「組み立て方法自体も変化する」という変化軸が生まれたとき、初めて案4が候補になります。現時点でその変化軸は存在しないため、案4は**今は不要な複雑さを持ち込む過剰設計**です。
...
フェーズ2で予告されていた「1000円引きクーポン（定額引き）」が実際に来た場合も確認してみましょう。パーセント計算とは全く異なる「引き算」ロジックですが、次のように `CouponDiscount` クラスを1つ追加するだけで完結します。
...
`PaymentCalculator` の中身は一行も変わりません。S2のヒアリングで予告された「計算方法の種類が変わる」というリスクが、このインターフェースの設計によって無害化されていることが確認できました。
...
採用した Strategy パターンでは、接続形態が **抽象×直接（USB-C直差し）** へと変化しています。

**「抽象×直接」の証拠となるコード：**

```cpp
class PaymentCalculator {
    IDiscountRule* discountRule; // ← インターフェース型 = 「抽象」の証拠
public:
    PaymentCalculator(IDiscountRule* rule) : discountRule(rule) {}
    int calculate(int total) {
        if (discountRule != nullptr) {
            total = discountRule->apply(total); // ← 直接呼び出し = 「直接」の証拠
        }
        return total;
    }
};
...
- メンバ変数 `discountRule` の型が `IDiscountRule*`（純粋仮想クラス）→ **「抽象」** の証拠（具体的なルールクラス名を知らない）
- `discountRule->apply(total)` は中間クラスを挟まない直接呼び出し → **「直接」** の証拠

「割引ルールを差し替えたい」という動機から、**抽象×直接** が選ばれました。
...
| 🔴 フェーズ4：原因分析 | 変化の理由が異なる「ルール」と「計算ロジック」が混在していることが痛みの元だと突き止めた。 |
| 🟣 フェーズ5：課題定義 | 「接続点A」を抽象化し、クライアント（`OrderProcessor`）への影響を抑えることを課題とした。 |
| 🟢 フェーズ6：対策案検討 | 案0〜案4を比較し、将来の変更コストを抑えるためにインターフェースを採用した。 |
...
> **この章を終えて、あなたが手に入れたもの**
>
> 「変更が来るたびに既存ロジックを開いて `if` 文を追加する」という作業から、あなたは解放されました。新しいルールは新しいクラスとして**外から追加**するだけ。`PaymentCalculator` は何も知らなくていい。
>
> これが「変わる理由を1箇所に閉じ込める」という設計が現場で生む、最も実感しやすい価値です。次のコードレビューで同じ構造を見かけたとき、「ここ、分けませんか？」と言える根拠が、この章で手に入りました。
...
3. **テストの範囲を測る：** 新しい条件を1つ追加したとき、再確認が必要だったテストは何件でしたか？「新しい条件のテストだけ」で終われましたか？
4. **分けた後を想像する：** もし「変わる計算ロジック」を別クラスに切り出したとすると、次の変更要求が来たとき、触らなくて済むファイルはどこですか？
...
**得られること1**（変動箇所の識別）：フェーズ1の責任チェック表を通じて、`calculate` メソッド内の割引ルール部分が「マーケティングチームが決める知識」として識別できました。「実行する振る舞い」という切り口で、変動箇所を探す視点が養われたはずです。

**得られること2**（接続点の診断）：フェーズ4で、`PaymentCalculator` が割引クラスの具体型を直接知っている状態が「具体×直接」の接続であると診断しました。この診断ができると、なぜ割引ルールが増えるたびに `PaymentCalculator` の修正が必要になるのかが、接続の形から読めるようになります。

**得られること3**（変更局所化の説明）：フェーズ7の変更シナリオ表で、新しい割引ルールの追加が `IDiscountStrategy` を実装した新規クラスの作成だけで完結することを確認しました。「なぜ変更がここだけで済むのか」を接続の形で説明できる状態が、設計の会話で力を発揮します。

**得られること4**（いつ構造を分けるか）：フェーズ6の過剰コード警告と使い分け判断を通じて、状態の種類が2〜3パターン程度で固定されているケースでは、この分離が逆にコストになることも確認しました。「パターンを使う」判断と「使わない」判断の両方が、設計力の中核です。

ECサイトの割引計算という一つの題材を通じて、変動箇所の識別から設計の選択までを体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。