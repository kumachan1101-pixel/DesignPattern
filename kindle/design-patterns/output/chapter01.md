...
```mermaid
flowchart LR
    A[/商品リスト<br>Tシャツ 3,000円<br>靴 6,000円/]:::input --> G[小計を合算<br>9,000円]:::process
...
この章では、割引計算を単純な1回の掛け算として扱いません。現実のECサイトでは、会員割引、期間限定キャンペーン、季節セール、クーポンなどが同時に存在し、どれを先に適用するかで金額が変わります。そこで本章では、次の複雑さを設計判断の材料として残します。

| 複雑さ | この章での扱い | 設計判断への影響 |
|---|---|---|
| 順次適用 | キャンペーン10%引き後にサマーセール5%引きを掛ける | 割引の計算式だけでなく、適用順序も変わる知識として扱う |
| 排他条件 | Premium会員にはキャンペーンやサマーセールを重ねない | どのルールを選ぶかの判定が計算本体へ混ざる原因になる |
