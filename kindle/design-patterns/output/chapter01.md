...
## 🔵 フェーズ1：現状把握 ―― 仕様・動作・構造を把握する

変更要求が来る前のシステムの現状を事実として把握するところから始めます。まず仕様と動作例で「このシステムが何をするか」を確認し、それからコードを読みます。
...
| キャンペーン割引 | `isCampaignActive = true` | 10%引き |

**優先・排他ルール**

| 条件 | 動作 |
|---|---|
| Premium かつ キャンペーン中 | Premium のみ適用（キャンペーンは無効） |

**このルールを使う場所**

同じ割引計算を2か所で使います。この「2か所で使う」という事実が、設計の違いを生む起点になります。
...
コードを読む前に、このシステムが「何をすべきか」をこの表で確認できました。次は「どのように実装されているか」を見ていきます。
...
動作例テーブルの行1（Premium / キャンペーンなし / 10,000円 → 8,000円）と一致しています。このコードは正しく動いています。問題は機能ではなく、構造にあります。
...
| `PaymentCalculator` | 注文内容をもとに最終的な支払金額を計算する | 商品の小計の出し方、適用すべき割引ルール |
| `Order` | 注文された内容と顧客の属性を保持する | 注文商品のリスト、顧客の種別、キャンペーン状態 |
| `Item` | 商品一つの情報を保持する | 商品名、単価 |

### 1-7：責任チェック表

コードの各行が「誰の判断で変わる知識か」を確認します。
...
フェーズ1でシステムの現状が把握できました。次のフェーズ2では、具体的な変更要求を受けて「何が変わり、何が変わらないか」を整理します。

---

## 🟠 フェーズ2：変更要求と仮説立案 ―― 変わるものと変わらないものを整理する
...
> **現実のヒアリングでは——** 「変わるかどうか分からない」「たぶん変わらない」という答えが返ることも多いです。そのときは `git log` や過去の障害記録を「ヒアリングの代わり」として使ってみてください。「過去に何度変わったか」が最も正直な証拠です。
...
## 🟡 フェーズ3：変更シミュレーション ―― 変更を試みて、痛みを発見する
...
「サマーセール中のプレミアム会員は15%オフ」という要件は、既存の「プレミアム会員は常に20%オフ」と競合します。単純に `else if` を追加するだけでは済まず、既存の `if (customerType == "Premium")` ブロック内を書き換える必要があります。さらに、`Order` クラスに `isSummerSale` フラグを追加しなければなりません。
...
決済の合算ロジックと個別の割引ルールは、変わる理由が全く異なります。これらを同じ場所に置くべきではないと分かりました。

フェーズ4で根本原因が言語化できました。次のフェーズ5では、解決すべき課題を具体的に定めます。
...
→ **採用：手段A**（最小コストで今回の要求に対応でき、この案の方針（構造を変えない）と一致するため）
...
→ **採用：手段A**（将来のインターフェース化に向けて境界を明確にするため）
...
**この案の考え方：**
`PaymentCalculator` は具体的な割引クラスを知らなくていい。すべての割引ルールが満たすべきインターフェース（契約）を定義し、呼び出し側はその型だけを知るようにします。後からいくらでも新しい実装を差し替えられるようになります。
...
→ **採用：手段A**（初期化時点で依存が確定し未設定の状態が起きない）
...
**手段Aを選ぶ理由：**
手段BのFactory Methodは「オブジェクトの生成責任の分離」という別の設計問題を混入させています。今回の課題は「ルールの差し替え」であり、生成の方法は別の文脈で考えるべきです。手段Aは今回の課題（接続の形）に集中しています。

**この案のトレードオフ：**
- 変更容易性：高（どの層の実装が変わっても他層は無影響）
- テスト容易性：高（全層でスタブに差し替え可能）
- 実装コスト：高（インターフェース2層＋仲介クラスの設計が必要。コードを追う負担が増す）

---

## 🟢 フェーズ7：採用案の選定 ―― コストで比べ、構造を決める
...
フェーズ7で採用案が決まりました。次のフェーズ8では、その決断を最終的なコードに落とし込みます。

---

## 🔵 フェーズ8：対策実施 ―― 決断し、変化に強い設計を手に入れる
...
| **フェーズ** | **この章でやったこと** |
|---|---|
| 🔵 フェーズ1：現状把握 | 仕様と動作例テーブルを確認した後、コードをクラス単位で読んだ。責任チェック表で変わる理由の異なる知識の混在を確認した |
| 🟠 フェーズ2：変更要求と仮説 | 今回の確定変更とヒアリングで判明した将来リスクを分けて整理した |
| 🟡 フェーズ3：変更シミュレーション | サマーセールの追加を試み、影響が `Order` と `CartPreviewService` にまで波及することを確認した |
| 🔴 フェーズ4：原因分析 | 変わる理由が異なる2つのものが同じ場所にいることが痛みの根本と特定した |
| 🟣 フェーズ5：課題定義 | 接続点Aを定義し、非機能制約（ステートレス設計）とクライアント影響を確認した |
| 🟢 フェーズ6：対策案の検討 | 4案を比較した。各案で複数の実装手段を確認し、根拠を持って1手段を選んだ |
| 🟢 フェーズ7：採用案の選定 | 案3（抽象×直接）を採用。この構造が **Strategy パターン** と呼ばれることをここで初めて確認した |
| 🔵 フェーズ8：対策実施 | 最終コードを実装し、変更影響グラフで変更の局所化を確認した |
...
| **得られること** | **この章のどこで示したか** |
|---|---|
| 1. 変動箇所の識別 | フェーズ1の責任チェック表で、変わる理由の異なる知識の混在を発見した |
| 2. 接続形態の診断 | フェーズ4で「具体×直接」の状態を診断した |
| 3. 変更局所化の説明 | フェーズ8の変更シナリオ表で、変更が新規クラスに閉じる構造を示した |
...
- 解説：フェーズ6の案3・手段Cで確認した通り、継承を使うとルールごとにクラス階層が深くなる。コンストラクタインジェクションによるコンポジションは、複数ルールの組み合わせも将来的に可能にする。

---

## 整理：8フェーズとこの章でやったこと

この章では、割引ルールが `PaymentCalculator` に直接埋め込まれている構造が、変更のたびに既存コードへの手術を強いることを確認しました。8フェーズの思考プロセスを適用して、どう解決したかを振り返ります。

| **フェーズ** | **この章でやったこと** |
|---|---|
| 🔵 フェーズ1：現状把握 | 動作例テーブルで仕様を確認し、`PaymentCalculator` が割引ロジックを直接持っている現状をコードで把握しました |
| 🟠 フェーズ2：仮説立案 | ヒアリングを通じ、割引ルールが今後も増え続けるリスクと、計算処理の呼び出し元は変わらないことを確認しました |
| 🟡 フェーズ3：問題特定 | 新ルール追加のシミュレーションで、`PaymentCalculator` を毎回開いて書き換えなければならない痛みを確認しました |
| 🔴 フェーズ4：原因分析 | 「変わる割引ルール」と「変わらない計算の呼び出し構造」が同一クラスに混在していることが原因だと言語化しました |
| 🟣 フェーズ5：課題定義 | `PaymentCalculator` と各割引ロジックの間に「接続点」を設け、差し替え可能にすることを課題として定義しました |
| 🟢 フェーズ6：対策案検討 | 案1〜4を手段ごとに比較し、コンストラクタインジェクションによる抽象×直接（案3）を採用しました |
| 🟤 フェーズ7：対策選定 | コスト天秤で案3を選定。この構造がStrategyパターンであることを確認しました |
| 🔵 フェーズ8：対策実施 | `IDiscountRule` インターフェースを導入し、具体ルールを `BatchApplication` だけが知る最終構造に移行しました |

### 各クラスの最終的な責任

| **クラス名** | **責任（1文）** | **変わる理由** |
|---|---|---|
| `PaymentCalculator` | 割引後の支払金額を計算する | 計算の呼び出し手順が変わるとき |
| `CartPreviewService` | カートプレビューの概算金額を返す | プレビューのビジネスロジックが変わるとき |
| `IDiscountRule` | 割引額を算出する契約（インターフェース）を定義する | 契約自体が変わるとき（ほぼない） |
| `PremiumDiscount` | プレミアム会員向け20%割引を実装する | プレミアム会員の割引率が変わるとき |
| `SummerSaleDiscount` | サマーセール10%割引を実装する | キャンペーンの割引条件が変わるとき |
| `BatchApplication` | 依存関係を組み立て、処理をキックする | 利用する具体クラスの組み合わせが変わるとき |

> **このプロセスを回した結果にたどり着いた構造こそが Strategy パターンです。**
...
| **得られること** | **この章のどこで示したか** |
|---|---|
| 1. 変動箇所の識別 | フェーズ1の動作例テーブルで複数ルールを確認し、フェーズ2の確定変更テーブルで「割引ロジック」が変動することを特定したこと |
| 2. 接続形態の診断 | フェーズ4で `PaymentCalculator` が具体ルールを直接知っている「Lightning直差し（具体×直接）」状態を診断したこと |
| 3. 変更局所化の説明 | フェーズ8の変更シナリオ表で、新ルール追加時に `BatchApplication` の1行変更だけで済む構造を示したこと |
| 4. 構造変更の判断 | フェーズ6の案1〜4比較で、「差し替えが発生するか否か」という判断軸で接続形態を選ぶ方法を示したこと |
