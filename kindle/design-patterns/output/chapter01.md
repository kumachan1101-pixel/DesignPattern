...
ここから登場する主なクラスとデータの流れを整理します。

- **Order / Item**：注文情報と商品データを持つクラスです。ここから金額の元データが流れます。
- **PaymentCalculator**：今回の主役となるクラスです。商品の合計金額を計算し、割引ルールを適用します。
- **CartPreviewService**：画面表示用のサービスクラスです。計算結果を使ってプレビューを表示します。

**注目ポイント**：
データは Item から Order へ、そして PaymentCalculator で計算され、最終的な金額が出力されます。この章では、この計算フローの中で「割引ルールの変更」がどのように影響を与えるかに注目して読んでみてください。
...
この「責任チェック表」は、「誰の判断でそのコードが変わるのか」を可視化するものです。クラスが持っている知識を並べ、それぞれの知識がどのチームの判断で変わるのかを見てみましょう。
...
ここで注目したいのは、クラスの責任として正しいかどうかではありません。**「誰の判断で変わるのか」が複数のチームにまたがって混ざっている**という事実です。1つのクラスの中に、複数のチームの判断基準が集中してしまっています。
...
**2つ目：grep地獄。** キャンペーンのたびに条件分岐が追加されていくと、半年後には PaymentCalculator が数百行の複雑な分岐の塊になります。「どの条件が今のキャンペーンのものか」「過去のセール条件とどう違うのか」を理解するために、コードの隅々まで解読する作業が発生します。単に機能として動いてはいても、変更作業そのものが辛い解読作業になってしまいます。
...
| grep地獄・複雑化     | 変わる理由が違う2つのもの（「合算ロジック」と「割引条件」）が同じメソッドの中に混在しているから。異なる理由で変わるロジックが分離されず、同じメソッド内に直接書かれているため、割引条件が変わるたびに合算ロジックも含めたメソッド全体を触る作業が発生する。 |
...
今、PaymentCalculatorクラスは割引ルールの具体的な条件（isPremiumやisSummerSale等）を自分の中に直接知っています。インターフェースを介さず、条件分岐として直接コードに書き込まれている状態です。この状態を、コード同士がどうつながっているかという「接続形態」の視点から整理してみます。
...
この状態は **「具体×直接」の接続形態** です。iPhoneに専用のLightningケーブルを直差ししている状態と同じで、新しいキャンペーンが増えるたびに本体側を開いて専用の配線（`else if` 文）を直接追加する作業が発生します。
...
フェーズ5で「変わるのは生産者（計算ロジック）であり、流れるデータの型は安定している」ことが分かりました。ここでは、その生産者をどのように差し替え可能にするかを段階的に検討します。いきなり正解へ飛ぶのではなく、各ステップで「どこまで痛みが解消されるか」を確認しながら、今回の要件において「どのステップで止めるのが良いか」を決断します。
...
仕様変更後（サマーセール割引追加済み）のコードでの実行結果は、変更前と結果自体は変わりませんが、コードの構造は大きく変わっています。既存の計算フローには一切触れず、新しいルールだけを独立して追加できるようになった点に注目してください。
...
フェーズ3の変更影響グラフと比べると、変更要求の中心が新しいルールクラスへ移り、計算本体の条件分岐は変更せずに済むようになりました（組み立て箇所への登録は必要です）。
...
**得られること3**（接続の形の効果を説明する）：フェーズ6と7で、「抽象×直接」へと接続の形を変えることで、既存の計算フローを変更せずに新しい割引ルールを追加できるようになること（変更の局所化）を学びました。

**得られること4**（構造を分ける判断）：フェーズ6の「どこまで設計を進めるのが良いか」で見たように、ルールが一つしかない状態から、今後も増え続けると予想される状態へと変化したタイミングこそが、Strategyパターンへの移行を決断する時です。
