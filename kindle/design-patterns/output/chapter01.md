...
|---|---|---|---|
| 割引ロジック → `calculate()` の骨格 | `int` 型の合計金額を渡し、割引適用後の `int` を受け取る | 割引の計算式（`* 80 / 100` など、誰がどう割引するか） | 小計の合算ループと結果を返す骨格 |
| ルール選択 → 割引ロジック | 会員種別（`memberType`）、キャンペーン状態（`CampaignContext`）、適用順序 | 適用条件（`memberType == "Premium"` など）と選択候補 | 「条件に合うルールを順に評価して選ぶ」という選択の手順 |
...
呼び出し元は「適用するルールを共通の方法で選べること」と「割引後の金額を受け取れること」が必要です。つまり、選択側の境界に残るのは「この施策が今の注文に当てはまるか」を問える操作、計算側の境界に残るのは「金額を渡すと割引後の金額が返る」操作です。この操作にどんな名前を付け、どのクラスに置くかはフェーズ6で決めます。2つの変化軸は別々に追跡しますが、両方を同時に扱える一つのシステム構造として解決します。
...
|---|---|---|---|
| P1 | 割引ルールの種類 | 各施策の適用条件（`memberType == "Premium"` などの判定） | 「条件に合うルールを順に評価して選ぶ」という選択手順 |
| P2 | 割引の計算方法 | 各施策の計算式（`* 80 / 100` など）と逐次適用の順序 | 小計を合算し、割引後の金額を受け取って返す骨格 |

P1とP2は異なる変化軸ですが、ある施策の「適用条件」と「選ばれた後の計算式」は同じ施策の変更で一緒に変わります。この観察は、フェーズ6で分離の単位を決めるときの材料になります。

---
> **📌 システム全体の課題（確定）**
> 計算の骨格・選択の手順と、施策ごとの適用条件・計算式の間の接続点では、合計金額（`int`）と会員種別・キャンペーン状態が流れている。変わるのは施策ごとの条件と式であり、骨格・選択手順・利用側は安定させたい。現在は条件と式が骨格側へ漏れており、施策追加のたびに骨格を開いている。
---

★以下の「フェーズ6へ渡す課題」って必要ですか？上記の課題で、変わる側と守る側が分かれば、次は、それを分離してどういう配置にして、再度接続するかの話にすればよいのでは？それは、対策検討で行っている
#### フェーズ6へ渡す課題
...
|---|---|---|---|
| P1 | 新しい割引種類の追加で `PaymentCalculator` の選択条件が増え、`OrderProcessor` と `CartPreviewService` まで再確認する | 共通の選択手順、注文処理、プレビュー処理、既存ルール | 施策を追加したとき、修正が「その施策の記述と、その施策を使うと宣言する箇所」に収まり、選択手順・注文処理・プレビュー・既存施策が修正・再テスト対象に入らない |
| P2 | 定額引きや逐次適用の追加で `PaymentCalculator` の計算式と適用順を修正し、小計計算まで再確認する | `PaymentCalculator` の小計計算、金額を渡して割引後の金額を受け取る契約、既存ルールの計算式 | 計算方法を追加したとき、修正が「その施策の計算式の記述」に収まり、小計計算と既存施策の式が修正・再テスト対象に入らない |

P1とP2は対策案の番号ではなく、2つの変化軸をコードまで追うIDです。採用構造の判定は分けず、システム全体で一度だけ行います。

## 🔴 フェーズ6：対策検討 ―― システム全体の最終構造を定める

P1とP2を別々に解くのではなく、条件と計算式を施策単位で分離し、選択手順・小計計算・利用側をまとめて安定させるシステムを設計します。
...
| ---- | ------------------------------------------------ | --------------------------- | ------------------------ |
| P1   | 割引種類の追加で `PaymentCalculator` の条件分岐が増え、利用側まで再確認する | 適用条件を施策側へ移し、共通の選択操作だけを残す    | 選択手順、注文処理、プレビュー、既存ルール    |
| P2   | 計算方法の追加で割引式と適用順を修正し、小計計算まで再確認する                  | 計算式と逐次適用順を施策側へ移し、金額の契約だけを残す | 小計計算、`int`→`int`、既存ルールの式 |
...
- P1：施策を追加したとき、修正が「その施策の記述と、その施策を使うと宣言する箇所」に収まり、選択手順・注文処理・プレビュー・既存施策が修正・再テスト対象に入らない。
- P2：計算方法を追加したとき、修正が「その施策の計算式の記述」に収まり、小計計算と既存施策の式が修正・再テスト対象に入らない。
- 共通の制約：金額を渡して割引後の金額を受け取る契約、整数演算の順序、割引の優先順位を維持する。

★課題定義から次に考えたいのは以下。答えっていうのはよくわからないが、各課題に対して、分離、配置、組み立てを考えたい。課題ID別に考えるわけではなく？具体的に説明に加えて、コードレベル記載してほしい。
#### 接続点の分離・配置・組み立てを決める
...
| 決定 | 問い | この章での答え |
|---|---|---|
| 分離 | 接続点をどう切るか | 施策ごとの適用条件と計算式を、選択手順・小計計算から切り出す。境界の契約は「注文に当てはまるかを問う操作」と「金額を渡すと割引後の金額が返る操作」 |
| 配置 | 切り出した知識をどこへ置くか | 適用条件と計算式は同じ施策の変更で一緒に変わる（フェーズ5の観察）ため、施策単位のまとまりへ一緒に置く。守る側には選択手順と小計計算だけが残る |
| 組み立て | どう繋ぎ直すか | 施策の一覧は組み立て側（`main()`）が生成・登録し、選択役と計算骨格は登録済みの施策を契約経由でだけ扱う |

この3つの決定を満たす実装形は一つではありません。次で候補を比較します。

#### システム全体の最終構造を決める

★分離の方向が決まったのなら、最終構造ってもうほぼ決まるのでは？３つあるけど、本当に今のやり方で３つがあるのか。もし出すなら、２つの課題を別のクラスにするとか。分離したクラスをどこで生成してどのように注入するかはポイントになるが、１つ前にすでにやり方決まっているなら、もう方向性は確定しているのではないか。

本章では、上の受け入れ条件を満たし得る、責任の置き場所と依存関係が異なる完成システムが3つ成立します。そのため、この構造差分を比較します。

| 最終システム構造 | 責任配置と依存の差 | 施策追加時に触る場所 | 判断 |
|---|---|---|---|
| 設定を解釈するルールエンジン | 条件・式・優先順位を設定へ置き、共通エンジンが選択・計算する | 設定、入力定義、検証 | コードをリリースせず追加できるが、式言語、検証、エラー表示、運用手段が必要で現在の規模には過大 |
| 条件関数と計算関数の登録システム | 条件関数・計算関数・名前をデータとして束ね、固定処理が呼ぶ | 関数の束、登録、必要な入力 | 小さく実装できるが、施策単位の責任と将来持つ状態が見えにくい |
| 共通契約を実装する具象ルールの登録システム | 条件・式・名前を具象ルールへ置き、選択と計算は契約だけに依存する | 具象ルール、登録、必要な入力 | クラスは増えるが、施策単位でレビュー・テストできる。**今回採用する** |

3案の構造差分は、選択と計算の安定側が「設定を解釈するエンジン」「関数の束」「インターフェース」のどれへ依存するかです。採用しなかった2案が実際にどんなコードになるかは、章末のコラム「採用しなかった2つの構造」で確認できます。

このシステムでは施策をアプリケーションのリリースで追加し、条件・計算・表示名を施策単位でレビュー・テストします。そのため具象ルールの登録システムを採用し、`IDiscountRule`、`RuleSelector`、`PaymentCalculator` の責任として具体化します。
...
**P1・P2との対応：** `RuleSelector o--> IDiscountRule` の登録関係と、選択済みルールの注入を実装しました。ここで2つの変化軸が一つの実行経路として接続されました。
...
|---|---|---|---|
| 新しい施策の追加先を限定する | 具象ルールの `matches()`・`apply()`・`name()` と `main()` の登録 | 新しいルール、登録、必要な入力 | 達成 |
| 選択・計算・利用側を安定させる | 固定の `RuleSelector`、契約だけを呼ぶ `PaymentCalculator`、参照注入 | 既存クラスの修正なし | 達成 |
| 具体条件と式を安定側から隠す | `IDiscountRule` と具象ルールへの責任移動 | 施策詳細は対象ルール内だけ | 達成 |
| 金額と優先順位の契約を守る | 「金額を `int` で受け取り、割引後の金額を `int` で返す」という `apply()` の入出力の型と、登録した順に適用する約束 | 小計・整数演算・優先順位を維持 | 達成 |
...
フェーズ6で確定したシステム全体のクラス構成を、完成コードでも確認します。薄い黄色は、変更前クラス図の責任を見直した結果、操作または依存を変更・新設したクラスです。`PaymentCalculator` と `CartPreviewService` がContext、`IDiscountRule` がStrategy、各割引クラスがConcreteStrategyに対応します。`RuleSelector` は、具体条件を知らずに登録ルールを選ぶ、このシステム固有の役割です。
...
| 🔴 フェーズ6：対策検討 | 受け入れ条件を評価軸に、分離・配置・組み立てを決定。構造差分のある3つの完成システムから採用構造を決め、3ステップでコードへ反映した |
...
6. 受け入れ条件をすべて満たす最終構造は何か。複数あるなら、変更影響と導入コストのどちらが今回の文脈に合うか。
...
フェーズ6の最終構造比較で採用しなかった2案が、実際にどんなコードになるかを抜粋で示します。どちらも受け入れ条件を満たし得る完成形であり、状況が違えば採用があり得ます。

**案1：設定を解釈するルールエンジン。** 条件・式・優先順位をコードではなくデータ（設定）として持ち、共通エンジンが解釈します。

```cpp
// 施策を「データ」として登録する（コードのリリース不要が狙い）
struct RuleConfig {
    std::string name;        // 施策名
    std::string condition;   // 例: "memberType == Premium"
    int percentOff;          // 例: 20（20%引き）
    int priority;            // 適用優先順位
};

class RuleEngine {
public:
    // 設定の条件式を解釈して合致判定し、割引率を適用する
    int applyRules(const std::vector<RuleConfig>& configs,
                   int total, const std::string& memberType);
};
```

施策の追加は `RuleConfig` の追加だけで済みますが、`condition` の文字列を安全に解釈する式パーサー、設定の検証、間違った設定を入れたときのエラー表示、設定を配布する運用手段が必要になります。毎月数件の追加のために式言語を保守するのは、この規模では割に合いません。

**案2：条件関数と計算関数の登録システム。** クラスを作らず、関数のペアを登録します。

```cpp
struct RuleEntry {
    std::string name;                                  // 施策名
    std::function<bool(const RuleInput&)> matches;     // 適用条件
    std::function<int(int)> apply;                     // 割引計算
};

// 登録：クラス定義なしで施策を追加できる
std::vector<RuleEntry> rules;
rules.push_back({"プレミアム割引",
    [](const RuleInput& in) { return in.memberType == "Premium"; },
    [](int total) { return total * 80 / 100; }});
```

実装は最小で済み、施策追加は `push_back` の1箇所です。受け入れ条件の大半を満たしますが、1つの施策の条件・式・名前が登録箇所のラムダに散らばるため、施策単位のレビュー・テスト・将来の状態追加（適用回数の記録など）がしにくくなります。施策を独立した単位として扱いたい本章の文脈では、具象ルールクラスの形を選びました。
...
本章で共通契約とSelectorを採用した根拠は「毎月新しい割引が追加される」というヒアリング結果でした。変更頻度の裏付けがない別の題材なら、まずその題材の受け入れ条件を定め直します。その条件を満たせるなら、関数や単純な分岐だけで完成させる方が適切な場合もあります。
