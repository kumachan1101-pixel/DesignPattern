...
フェーズ6は、フェーズ5で定めた課題——**小計計算の骨格を守りつつ、割引ルールごとの判定・計算式を差し替えられる接続点を作る**——を受けて始めます。まずフェーズ4で記録した現在の痛みを図の起点に置き、P1でどのコード境界を微修正し、同じ要求を当て直すと影響がどこまで縮むかを一本の経路で描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じP1の行へ落とします。その後、P1に紐づくフェーズ3の関連コードだけをおさらいし、候補を一段ずつ適用します。各段階で「何を変えたか」「どの影響が減ったか」「何が残るか」をコードで回収し、統合後の全体コードはフェーズ7で示します。
#### 痛みの変更影響から、構造をどう変えるか
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: 割引式を関数へ出す"] --> S2["P1-変更2: apply契約へそろえ選択を外へ移す"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["新しい割引を追加"] --> I1["P1: 新しい割引ルールと登録だけを追加"]
        I1 --> I2["追加ルールと組み立てだけをテスト"]
        IS["変更しない: 小計計算・注文処理・既存ルール"]
    end
    C1 -. "P1で痛みの経路を切る" .-> S1
    S2 -. "変更先を限定" .-> I1
```

この図は完成構造を先に置くものではありません。フェーズ3・4の痛みの経路を左に残し、どの微修正で経路を切り、同じ要求の到達先をどう縮めるかを中央から右へ追う図です。
...
|---|---|---|---|
| P1 | `applyPremium()`・`applySummerSale()`、`IDiscountRule::apply()`、`PaymentCalculator::calculate()`、`RuleFactory` | **微修正：** ①式を関数化→②`apply(int)` へ統一→③`calculate()` は注入ルールだけを呼ぶ→④選択をFactoryへ移す<br>**結果：** 具体割引名と選択分岐が消え、変更先がルールと組み立てへ縮んだ | **守った契約：** 小計、整数演算順、`Order`→`PaymentResult`<br>**判定：** 関数化だけでは未達、契約と外部選択で達成 |

この判定はP1表の各列をコードへ当てた結果です。次のクラス図は、この微修正を統合した構造だけを示します。
