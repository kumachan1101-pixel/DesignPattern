...
コードを読む前に、このシステムが何を受け取り、何を加工し、何を出力するかを図で押さえます。

```mermaid
flowchart LR
    I["入力<br>商品リスト（名前・単価）<br>会員種別（Premium / Regular）<br>キャンペーンフラグ"]
    P["加工<br>小計を合算し<br>割引ルールを適用する"]
    O["出力<br>支払金額（円）<br>顧客ID・空注文のエラー"]
    I --> P --> O
```

この図で注目するのは、中央の「加工」にある**割引ルールの選択と適用**です。入力された会員種別とキャンペーンフラグから、どの割引をいくら適用するかが決まり、支払金額が出力されます。この章では、この割引ルールの部分が変更要求のたびにどう膨らむかを追いかけます。

この図は割引計算そのものを表します。カートプレビューは図に別枠で描いていませんが、決済計算（`OrderProcessor`）と並ぶ**この同じ加工の利用者**です。どちらもこの計算を呼び出して同じ支払金額を得ます（1-1末尾の「この割引計算を使う場所」）。つまりプレビューは独立した出力ではなく、同じ加工→出力を共有しています。
...
そのため掲載コードでも、顧客情報の取得は `try/catch` で失敗に備えています（1-4の `OrderProcessor`）。本番ではさらに、リトライ、タイムアウト、エラー戻り値、ログ記録などを加えます。
...
この章の中心です。1-1の「加工」——小計の合算と割引ルールの適用——を1つのメソッドで行っています。
...
- **(1) 小計の合算**：`for` で `order.items` を回し、各 `item.price` を `total` に足します。1-1の加工「全商品の小計を算出」にあたり、割引が変わっても変わらない骨格です。
...
注目してほしいのは、**変わらない(1)と、施策のたびに増える(2)が、同じメソッドに同居している**ことです。
...
- 割引ロジックを2か所に書かないので、決済とプレビューで金額が食い違いません。裏を返すと、`PaymentCalculator` が変わるとプレビュー側も影響を受けます。
...
- 最後に、購入した商品と単価、小計、支払金額を表示します。割引でいくら下がったかが「小計→支払金額」で読めます。
...
上記コードを実行すると、各注文について「誰が・何を・いくらで買い、小計から支払金額がいくらになったか」が表示されます。1-2の動作例テーブル（4行）と排他ルール・エラー条件が現状コードで再現されることを確認します。実行結果：
...
`PaymentCalculator.calculate()` が現在抱えている知識（判定条件・計算式）と、それぞれがどの業務機能に属するかを確認します。ここでは断定ではなく、変わりそうな箇所を予測し、後続のヒアリングで裏付けるための仮説として整理します。

**適切／混在の判断基準**：その知識が、`PaymentCalculator` 本来の責任（金額計算の骨格）と**同じ理由で変わるか**、それとも**別の業務機能の都合で変わるか**で見ます。骨格と同じ理由で変わるなら「適切」、別の業務機能が別のきっかけで変えるなら、この計算クラスに置くと「混在」です。

| 知識（判定条件・計算式） | 変える業務機能 | 計算クラスに置くのが適切か |
|---|---|---|
| 商品単価の合算ロジック | インフラ・システム管理 | ✅ 適切（計算の骨格そのもの） |
| プレミアム割引の条件・割引率 | 料金・プロモーション管理 | ❌ 混在（別機能が変える） |
| キャンペーン割引の条件・割引率 | マーケティング・通知管理 | ❌ 混在（別機能が変える） |

合算ロジックは計算の骨格なので、計算クラスにあって当然です。一方、2つの割引条件は料金・プロモーション管理とマーケティング・通知管理という**別々の業務機能が、別々のきっかけで**変えます。それが計算クラスに同居しているため、❌（混在）と判断します。この1つのメソッドを複数の業務機能が異なる時期に変更することになり、これが後の変更の痛みの予兆です。
...
ヒアリング結果をもとに、今回確定した変更・将来繰り返される変更軸・当面安定している前提を、混ぜずに整理します。変わるものだけでなく、**変わらないもの（当面安定）**も明示して、フェーズ3で何を当て、何を守るかを確定します。
...
増えたのは下の3つの分岐です。サマーセールは「Regular会員のみ」「キャンペーンと重なると重ね掛け」という複合条件なので、`else if` を1行足すだけでは済みません。

- `isSummerSale && isCampaignActive` → サマーセール5%引きの後にキャンペーン10%引きを重ねる（`* 95 / 100` のあと `* 90 / 100`）。
- `isSummerSale` のみ → サマーセール5%引き（`* 95 / 100`）。
- `isCampaignActive` のみ → 従来のキャンペーン10%引き。
- この重ね掛けの分岐を入れ忘れると、両方有効な顧客の金額がずれます。1つの施策追加で、既存分岐の順序と組み合わせまで見直すことになります。
...
上から順に、Premium優先（10000円→8000円）、Regularの重ね掛け（10000円に10%引きと5%引きで8550円）、Regularのサマーセール単独（10000円→9500円）、割引なし（10000円のまま）です。小計→支払金額が、1-5の変更後の動作例と対応していることを確認できます。
...
割引には、変わるきっかけの異なる軸が混ざっています。これらを1つずつ独立した割引として分けるか、重ね掛けまで含めて1単位として扱うかを、変わる理由から判断します。

| 変化軸 | 変わる理由 |
|---|---|
| 各割引ルール（プレミアム/キャンペーン/サマーセール） | それぞれの担当が独立した施策として変える |
| 割引どうしの重ね掛け（サマーセール＋キャンペーン） | 併用可否のポリシーとして変わる |

個々の割引は変わる理由が独立しているため、別々のルールとして分離します。一方、重ね掛けは「どの割引を同時に効かせるか」という別の判断なので、本章ではこの組み合わせも1つのルールとして表す方針を採り、フェーズ6で具体化します。
...
- 重ね掛けは `SummerSaleAndCampaignDiscount` という1つのルールとして表します（フェーズ5の方針）。
...
上記コードを実行すると、注文ごとに購入品・小計・支払金額・カートプレビューが表示されます。実行結果：
...
| Regular・あり・あり | イヤホン(10000) | 10000 → 8550（重ね掛け） | 8550 |
...
フェーズ3の変更影響グラフと比べると、割引の計算詳細は新しいルールクラスへ移り、計算本体の条件分岐は変更せずに済みます。一方、**どのルールを選ぶか**という条件は `RuleFactory` に残り、施策を表すフラグが増えるなら `CampaignContext` も変わります。Strategyが分離するのは計算アルゴリズムであり、選択条件や入力モデルまで自動的に不変にするわけではありません。
