...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 商品リスト、会員種別、キャンペーンフラグ | `Item`、`customerType`、`campaignFlag` | 動作例テーブル、`main()`、変更要求 |
| 小計合算、割引ルール適用 | `calcTotal()` 内の合算処理と条件分岐 | 現状コードの観察、フェーズ2の仮説、フェーズ4の原因分析 |
| 支払金額、エラー | 実行結果の金額表示、未登録ID・空注文エラー | 動作例テーブル、実行結果、変更シナリオ表 |
...
1-3で登場クラスと責任を確認したので、掲載コードで何を代替しているかを整理してから現状コードへ進みます。
