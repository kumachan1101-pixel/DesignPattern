...
|---|---|---|
| P1 | 新しい割引の追加で `PaymentCalculator` の条件分岐が増え、`OrderProcessor` と `CartPreviewService` まで再テストする | 小計計算、注文処理、既存の割引ルール |

P1は対策案の番号ではなく、独立して扱う「割引追加の波及」という課題の識別子です。フェーズ6ではこの影響をどこまで縮めるかを先に描きます。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた課題——**小計計算の骨格を守りつつ、割引ルールごとの判定・計算式を差し替えられる接続点を作る**——を受けて始めます。まず問題定義で得た変更影響を左へ引き継ぎ、P1の影響を切るクラス・契約・依存関係を中央に置き、同じ要求を再適用した変更影響を右に描きます。その差、守る契約、完了条件、候補を同じP1の行へ落とします。その後、P1に紐づくフェーズ3の関連コードだけをおさらいし、中央の構造が妥当かを最小単位のコード変更で一段ずつ検証します。各段階で「変更理由」「対象コード」「変更内容」「減った影響」「残った問題」「次の判断」を言語化し、統合後の全体コードはフェーズ7で示します。

#### 問題定義の変更影響を、どの構造で変えるか

フェーズ4の変更影響グラフでは、新しい割引を追加すると、割引条件を持つ PaymentCalculator だけでなく、それを使う注文処理とプレビューまで再確認が必要でした。フェーズ5の課題を満たすなら、同じ要求が再来したときの影響は次の範囲に収まるはずです。

```mermaid
flowchart LR
    subgraph Pain["問題定義：変更前の変更影響"]
        direction TB
        CR["新しい割引を追加"] --> C1["PaymentCalculator の分岐を修正"]
        C1 --> C2["OrderProcessor / CartPreviewService を再テスト"]
    end
    subgraph TargetStructure["影響を切る構造の形"]
        direction TB
        U["OrderProcessor / CartPreviewService"] --> PC["PaymentCalculator"]
        PC --> R["IDiscountRule::apply"]
        RI["割引ルール実装"] -. "契約を実装" .-> R
        F["RuleFactory"] --> RI
    end
    subgraph Result["同じ要求を再適用した変更影響"]
        direction TB
        IR["新しい割引を追加"] --> I1["P1: 割引ルール実装と登録を追加"]
        I1 --> I2["追加ルールと組み立てだけをテスト"]
        IS["変更しない: PaymentCalculator・注文処理・プレビュー"]
    end
    C1 -. "P1: 選択・計算の境界を契約で切る" .-> R
    R -. "新しい変更先" .-> I1
```

中央は変更作業の順番ではなく、採用する依存関係です。`PaymentCalculator` が具体的な割引名ではなく `IDiscountRule` だけへ依存するため、左で利用側まで伸びていた影響が、右では割引ルール実装と登録で止まる、と読みます。

#### 構造と変更後の影響から、課題と候補を一続きで導く

| 課題ID | 変更の到達点 | 最初に試すコード変更 | 残る問題に対する次のコード変更 |
|---|---|---|---|
| P1 | **現在→理想の差：** `PaymentCalculator` の分岐と利用側の再テストを、追加ルールと組み立てへ縮める<br>**切る境界・守る契約：** `calculate()` の割引選択を切り、`Order`→`PaymentResult`、小計、整数演算順を守る<br>**完了条件：** 既存の `PaymentCalculator`・`OrderProcessor`・ルールを修正しない | **候補：** 割引式を関数へ出す<br>**減る影響：** 計算式に名前が付き、分岐との差を読める | 同じ `apply(int)` 契約へそろえ、ルールの登録・組み立てへ選択を移す |

P1の横一行は、痛みの経路、守る契約、候補を別々に列挙したものではありません。左から右へ読み、後続コードでも同じP1を使って各変更の適用先と結果を回収します。
...
|---|---|---|---|
| P1 | `applyPremium()`・`applySummerSale()`、`IDiscountRule::apply()`、`PaymentCalculator::calculate()`、`RuleFactory` | **段階的な変更：** ①割引式だけを関数化し、選択分岐が残ることを確認→②式を `apply(int)` 契約へ統一→③`calculate()` を注入ルールへの委譲だけに変更→④残ったルール選択をFactoryへ移動<br>**結果：** 具体割引名と選択分岐が消え、変更先がルールと組み立てへ縮んだ | **守った契約：** 小計、整数演算順、`Order`→`PaymentResult`<br>**判定：** 関数化だけでは未達、契約と外部選択で達成 |
...
|---|---|---|---|
| P1 | `IDiscountRule` の全実装、`RuleFactory`、`PaymentCalculator`、`CartPreviewService` | 新しい割引の選択・計算はルールとFactoryへ入り、注文処理とプレビューは契約だけを利用した | `PaymentCalculator`・`OrderProcessor`・既存ルールを修正せず、追加ルールと選択だけで拡張できる |
