...
P1は対策案の番号ではなく、独立して扱う「割引追加の波及」という課題の識別子です。フェーズ6では、この影響を生む現在のシステム構造を描き、変わる責任をどこへ分けるかを先に整理します。変更影響が実際に縮んだかは、完成コードの後で確認します。
...
3-2で影響が広がった直接の理由は、`PaymentCalculator` という1つの部品に、変わる理由の異なる3つの責任が集まっていることです。ここでは計算クラスだけを切り取らず、入力、注文処理、計算、プレビュー、表示まで含めたシステムの中で、現在の配置を確認します。

この図の箱は、ここで突然考えたものではありません。前工程の結果を次のように使います。

| 図を作る材料 | 前工程で確認したこと | 構造図での使い方 |
|---|---|---|
| フェーズ1のクラスと処理の流れ | 入力、注文処理、計算、プレビュー、表示を担うクラス | まず現在のシステムを構成する箱と依存をそのまま置く |
| フェーズ2の変化予測 | 割引施策は毎月増える | 施策条件と割引式に `【移す】` の候補印を付ける |
| フェーズ4の原因 | 小計計算と割引知識が `PaymentCalculator` に混在する | 1つの箱の中に、残す責任と移す責任を分けて書く |
| フェーズ5の接続点 | 小計を渡し、割引後の `int` を受け取ればよい | 移した後もつなぐ共通契約の根拠にする |

図中の印と線は、次の意味だけで使います。

- `【残す】`：そのクラスに残す責任。
- `【移す】`：施策ごとに変わるため、現在のクラスから外す責任。
- `【維持】`：外から見た役割を保つ部品。構造移行時に配線コードを一度直すことはあります。
- `【新設】`：移した責任を安定部分へ接続するために追加する部品。
- 実線の矢印：相手のデータまたは機能を利用する依存。
- 破線の矢印：共通契約を実装する関係。

**変更前の図は、次の順に読みます。** ①中央の `PaymentCalculator` を探す、②箱の中の `【残す】` と `【移す】` を分ける、③その箱を利用している `OrderProcessor` と `CartPreviewService` をたどる。この3手で、条件追加が安定した利用側の再確認へ広がる理由を読めます。

**変更前の構造：**

```mermaid
flowchart TB
    subgraph SYS["注文金額計算システム（変更前）"]
        direction TB
        INPUT["入力・参照データ<br>Order / CustomerDatabase / CampaignContext"]
        OP["OrderProcessor<br>【維持】検証・取得・計算・表示"]
        CP["CartPreviewService<br>【維持】プレビュー入口"]
        OUT["CheckoutResultRenderer<br>【維持】結果表示"]
        PC["PaymentCalculator<br>【残す】小計計算<br>【移す】割引条件の判定<br>【移す】各割引式"]
        INPUT --> OP
        INPUT --> CP
        OP --> PC
        CP --> PC
...
`OrderProcessor` と `CartPreviewService` は、どちらも会員種別と施策条件を `PaymentCalculator` へ渡します。その先の1クラスに、小計計算、条件判定、割引式が同居しています。このため、施策固有の条件や式を1つ増やすだけでも、注文処理とプレビューが共通利用する計算部品を開くことになります。

P1の「小計計算、注文処理、プレビューは守り、割引の追加先だけを分ける」をシステム構造へ置き換えると、次の責任配置を目指すことになります。

**変更後の図は、変更前と同じ名前の箱から読みます。** ①`OrderProcessor`、`CartPreviewService`、`PaymentCalculator` が残っていることを確認する、②`PaymentCalculator` から `【移す】` と書いた2責任が消えたことを確認する、③消えた責任の移動先である割引ルール群を探す、④間に追加された `IDiscountRule` と `RuleSelector` が具体条件を持っていないことを確認する。この順なら、箱が増えた理由を前図との差として追えます。

**変更後に目指す構造：**

```mermaid
flowchart TB
    subgraph SYS["注文金額計算システム（変更後の目標）"]
        direction TB
        INPUT["入力・参照データ<br>Order / CustomerDatabase / CampaignContext"]
        ROOT["main / Composition Root<br>【移す先】生成・登録・所有"]
        OP["OrderProcessor<br>【維持】処理手順"]
        CP["CartPreviewService<br>【維持】プレビュー入口"]
        OUT["CheckoutResultRenderer<br>【維持】結果表示"]
        RS["RuleSelector<br>【新設・固定】登録順にmatchesを評価"]
        PC["PaymentCalculator<br>【残した】小計計算"]
        R["IDiscountRule<br>【新設・固定】matches / apply 契約"]
        G["割引ルール群<br>【移した】適用条件と割引式"]
        INPUT --> OP
        ROOT --> OP
        ROOT --> RS
        ROOT --> G
...
システムの入口と出口は変えません。変えるのは中央の責任配置です。`OrderProcessor` は構造移行時に一度だけ組み立て処理を変更しますが、移行後に新しい割引を追加するときは、その処理手順を変更しない状態を目指します。

前後の図で確認する構造変化は、次の3点だけです。

1. `PaymentCalculator` の `【残す】小計計算` は、変更後も同じクラスに残る。
2. `【移す】割引条件の判定` と `【移す】各割引式` は、各割引ルールの `matches()` と `apply()` へ移る。
3. 利用側と移動先を直接結ばず、`IDiscountRule` と、具体条件を持たない `RuleSelector` を間に置く。

新しい箱が増えたこと自体が目的ではありません。変更前の1クラスに混在していた3責任を、1つ残し、2つを同じ変更理由の単位へ移し、安定部分との接続方法を固定した結果です。

構造変化とコード変更を一対一で対応させると、次のようになります。

| 構造上どう変えるか | コードレベルで何をするか | 後続の検討で確認すること |
|---|---|---|
| 小計計算と割引式を同じクラスから分ける | `applyPremium()` などを抽出し、式が小計計算の外へ出せるか試す | 検討1：式を出すだけで変更影響が切れるか |
| 割引式を個別の部品にし、計算側との接続を固定する | `IDiscountRule::apply(int)` を定義し、`PremiumDiscount` などへ式を移す。`PaymentCalculator` は契約だけを呼ぶ | 検討2：具体的な割引名と式を計算本体から消せるか |
| 条件判定も施策ごとに分ける | `IDiscountRule::matches(memberType, context)` を追加し、各ルールが自分の適用条件と計算式を持つ | 検討3：条件の `if` を別の1クラスへ移すだけになっていないか |
| 選択手順を安定させる | `RuleSelector` は登録順に `matches()` を評価するだけにし、`main()` でルールを優先順に登録する | 検討3：新しい割引で選択役のコードを修正せずに済むか |
| 注文計算とプレビューで同じ選択結果を使う | 選ばれた `IDiscountRule` を `PaymentCalculator` と `CartPreviewService` へ参照で注入する | 検討3：利用側へ条件分岐を重複させず、既存処理手順を守れるか |

#### 自分のコードから同じ構造図を作る手順

読者が別のシステムでもこの図を作れるように、作図操作を固定します。

| 手順 | 問うこと | この章での答え | 図へ行う操作 |
|---|---|---|---|
| 1. システム境界を決める | 変更要求が入ってから結果が出るまで、どの部品を通るか | Order、OrderProcessor、PaymentCalculator、CartPreviewService、Renderer | フェーズ1のクラス構成から関係する箱を抜き出し、現在の依存を結ぶ |
| 2. 各箱の中の責任を書く | そのクラスは何を判断・計算・保持しているか | `PaymentCalculator` は小計、条件判定、割引式を持つ | クラス名だけでなく、今回の要求に関係する責任を箱の中へ書く |
| 3. 残す責任と移す責任を印で分ける | 変更理由と頻度が同じか | 小計は残す。施策条件と式は移す | `【残す】` と `【移す】` を付ける。この時点では新しいクラス名を考えない |
| 4. 同じ理由で変わるものをまとめる | どの条件と式が同じ施策として一緒に変わるか | Premium条件＋20%式、Summer条件＋5%式 | 施策単位の移動先を「割引ルール群」として置く |
| 5. 接続点を置く | 残す側は移動先から何を受け取れればよいか | `int` を渡し、割引後の `int` を受け取る | フェーズ5の接続点を `IDiscountRule::apply(int)` として間に置く |
| 6. 選択と所有を置く | 誰が候補を並べ、誰が選び、誰が生存期間を持つか | `main()` が所有・登録し、Selectorが `matches()` で選ぶ | 組み立て箱と選択箱を追加し、具体条件を安定側へ戻さない |
| 7. コード操作へ変換する | 各移動を、どの抽出・契約・注入で実現するか | 関数抽出、契約化、条件移動、登録、参照注入 | 構造変化とコード変更の対応表を作り、後続の検討番号を付ける |

手順3で `【残す】` と `【移す】` を分けられない場合は、原因分析へ戻ります。手順5で接続点の値や操作を言えない場合は、課題定義へ戻ります。新しいクラス名を先に考えず、前工程で確定した事実を図へ移すのがポイントです。

この図は採用済みの完成クラス図ではありません。「変わる部分を安定部分から外す」というP1を、システム内の部品配置へ翻訳した仮説です。次のコード検討では表の上から順に、各構造変化を実現できるかを確かめます。全課題を満たした後の詳細なクラス図は6-3、完成コードへ同じ変更要求を当てた結果は7-3で示します。

#### 構造から対策案が出てくる思考回路

ここで、いきなり `IDiscountRule` や `RuleSelector` を答えとして置かないようにします。現在のコードから、次の案が出てくる理由を順にたどります。
...
冒頭のシステム構造をコードへ落とした結果を、ここではクラスと依存の粒度で確認します。フェーズ7の完成コードに登場するクラスを省略せずに載せます。図は出発点ではなく、検討1〜3の結論です。
...
6-3で見立てた構造が完成コードでも成立しているかを確認します。`PaymentCalculator` と `CartPreviewService` がContext、`IDiscountRule` がStrategy、各割引クラスがConcreteStrategyに対応します。`RuleSelector` は、具体条件を知らずに登録ルールを選ぶ、このシステム固有の役割です。
