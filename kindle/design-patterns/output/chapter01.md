...
★以下はなぜ、フェーズ２が出てくるのか。各フェーズの結果を引き継いで、次のフェーズに行くのでは？一つ前で接続点が整理されているが、以下はさらに何をしようとしているの？仕様変更を踏まえてどうするかという事を言いたいのか。上記接続点は、変更の痛みすべて出しているから、以下はいらないように見えるが。
### フェーズ2・4の整理を接続点へ落とす
...
P1とP2は対策案の番号ではなく、2つの変化軸をコードまで追うIDです。フェーズ6では、上の課題をそのまま受け取り、分離の単位・配置・再接続方法を決めます。

## 🔴 フェーズ6：対策検討 ―― システム全体の最終構造を定める

#### 接続点の分離・配置・組み立てを決める

P1とP2は別々に採用する対策ではありません。一つの施策では適用条件と計算式が一緒に変わるため、システム全体を組み立てる3つの決定へ両方を当てます。

判断は次の順序で導きます。P1の条件とP2の式は同じ施策の変更で一緒に変わるため、同じ単位として切り出します。守る側は施策名や具体条件を知る必要がないため、「適用できるか」と「適用するといくらになるか」だけを共通操作にします。守る側が具象クラスを生成すると再び施策追加の影響を受けるため、具象ルールを知る役割はシステムの組み立て場所へ集め、選択役と計算役には共通契約を注入します。★組み立てるのは、配置したものが新たなクラスなのであれば、生成も必要になると思います。生成の話は必要ありませんか。
...
| 組み立て | `main()` が具象ルールを生成・所有・登録し、安定側へ契約の参照を渡す | `RuleSelector::add()` と固定の `select()` で選ぶ | 選択した `IDiscountRule&` を `PaymentCalculator` へ注入する |
...
★ルールエンジンや関数登録ってそもそも何？ってなりませんか。記載する必要があるのか？ノイズになるなら話に出さないでください。
ルールエンジンや関数登録は「配置」を決める前なら候補になり得ますが、本章では施策単位でレビュー・テストすることと、条件と式を同じ施策オブジェクトへ置くことを上で決めました。ここから異なる完成構造は分岐しないため、比較表は作りません。
...
クラス図の変更とコード変更を一対一で対応させると、次のようになります。★新たに用意したクラスを生成する処理は考慮されているか。
