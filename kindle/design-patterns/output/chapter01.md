...
責任を定義したうえで、実際のコードを行ごとに確認していきます。
...
動いています。しかし、このコードには設計上の問題が潜んでいます。
...
改めて、ここまで導き出してきた「最終的な設計（図やコード）」を、第0章でお話しした「1つの原則と2つの技法」と照らし合わせてみましょう。一通り設計のプロセスを体験した今なら、あの原則が「コードのどの部分に現れているか」がはっきりと見えるはずです。

### 原則「変わるものをカプセル化せよ」の現れ
