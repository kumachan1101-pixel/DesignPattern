...
さらに、ヒアリングで予告された「定額クーポン（1000円引き）」の追加も視野に入れると、`PaymentCalculator` の条件分岐にパーセント引きとは性質の異なる引き算ロジックがまた1つ混入し、クーポンとサマーセールが重なる場合のケースも書き足す必要が生まれることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、ルールが増えるたびに同じクラスの条件分岐が肥大化する構造は変わりません。
...
| 変更軸 | 変わる理由 |
|---|---|
| プレミアム割引ルール | 会員サービス企画チームの判断で変わる |
| キャンペーン割引ルール | マーケティングチームの判断で変わる |
