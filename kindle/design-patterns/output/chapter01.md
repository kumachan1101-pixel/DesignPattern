...
しかし、最大の痛みが残っています。新しいルールが来るたびに、結局はこの `PaymentCalculator` クラスを開いて新しい関数を追加し、`applyDiscount` の中に **新しい `if` 文を書き足さなければならない** のです。このままでは、「クラスが永遠に変わり続ける」という根本問題は解決していません。
