...
- **実体の組み立て**：施策を誰が生成・所有・登録し、価格計算が契約だけを通してどう実行するかを一本で決める。

この二点を別々の工程にはしません。起動時の組み立てから注文時の選択・計算・結果返却までを先に一本で決め、後続のコードで境界と接続を具体化します。結論はフェーズ6「決めた流れとコードの照合（6-1）」で確認します。

### 全体のデータと実体の流れを先に決める

対策検討で最初に決める完成経路は、次の形です。ここではコードの詳細へ入らず、**起動時に何を準備し、注文時にどのデータと実体が移動するか**を固定します。
