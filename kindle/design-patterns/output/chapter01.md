...
- `db.get()` で会員種別を取り出し、`calculate()` へ渡します。`Order` 自身は会員種別を持たないので、ここでIDから補います。
...
> [!NOTE]
> 上から順に、動作例1（Premium・10000円→20%引きで8000円）、動作例2（同じPremiumにキャンペーンを当てても優先は変わらず8000円）、動作例3（Regular・キャンペーン10%引きで9000円＋同じ計算を共有するカートプレビュー）、動作例4（Regular・割引なしで3000円のまま）、最後にエラー条件です。小計→支払金額を見れば、割引でいくら下がったかを追えます。
...
- `OrderProcessor` は、エラー条件の確認 → 会員種別の取得 → `RuleFactory` でルール選択 → `PaymentCalculator`/`CartPreviewService` を組み立てて実行、という流れだけを担います。
...
> [!NOTE]
> 上から順に、Premium（10000円→8000円）、Premium優先でキャンペーン・サマーセールが無効（8000円）、Regularの重ね掛け（10000円→8550円）、Regularのサマーセール単独（10000円→9500円）、割引なし（3000円）です。小計→支払金額を1-5「変更後の動作例」と照らすと、リファクタリング後も同じ金額になることを確認できます。カートプレビューも同じ計算を共有するため支払金額と一致します。
