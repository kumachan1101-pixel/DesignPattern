...
なぜこの痛みが発生しているのか、第0.5章の「3次元」に当てはめて構造的な原因を言語化します。

| 問題の次元     | 観察される痛み                         | 本質的な原因（なぜ痛いか）                              |
| :-------- | :------------------------------ | :----------------------------------------- |
| **要素の定義** | 割引が増えるたびに、同じ関数が肥大化する            | 変わる理由が異なるもの（各ルールと骨格）が、1つの塊に**癒着・混在している**から |
| **関係の構築** | ルールを変えると、呼び出し元も影響を受ける           | 呼び出し元が、個別のルールの「具体的な実装」に**直接依存している**から      |
| **構造の発展** | 新しいルールを追加するたびに、既存コードを開いて直す必要がある | 状況によって**振る舞いを入れ替えたい**のに、固定されたif文で縛られているから  |
...
*   **要素の癒着**に対して → 手札 **① 分割する（切る）**
*   **具体への依存**に対して → 手札 **③ 規格化する（形を揃える）**
*   **振る舞いの入れ替え**に対して → 手札 **⑤ 置換する（入れ替える）**
...
★上記指摘を対応すれば、十分に柔軟な構造になると思っています。手段②とどちらが良いかは天秤で判断したい。関数ポインタにはしないといけないかも。１回の関数コールではやはり限界があるような気がするのか。。

割引ルールを分けても、「どのルールを使うか」という判断が呼び出し元に残ってしまいます。
これは「変わるルールの詳細を、変わらない骨格が知っている」という当初の問題と
構造が変わっていません。

★こちらも手段②のように、関数分割後の構造図を表現してくれませんか。
...
★手段①は適切に関数分割した内容で差し替えてください。天秤にかけたら、手段①の方が良かったりするのでは？もしかしたら、耐久テストを実施しないと判断できないのかもしれません。
