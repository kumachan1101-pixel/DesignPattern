...
フェーズ6は、フェーズ5で定めた課題——**小計計算の骨格を守りつつ、割引ルールごとの判定・計算式を差し替えられる接続点を作る**——を受けて始めます。本章で解く課題はP1の1件です。ここでは迷った順番を実況しません。判断材料を整理し、複数案をP1の完了条件で比較して採用設計を先に確定し、その後で完成形へ至る実装順を示します。

検討は、次の思考順序で進めます。

1. 3-2の変更影響とフェーズ5の課題を、比較の前提として固定する。
2. 何を守れば採用できるかをP1の完了条件として先に定める。
3. 読者が考えそうな案を同じ評価軸で比較し、採用案と不採用理由を確定する。
4. 1-3のクラス図を使い、採用案で操作・責任・依存がどう変わるかを完成形として示す。
5. 採用したクラス図の差分を、契約、具象ルール、選択、注入の実装順へ変換する。
6. 完成コードへ同じ変更要求を当て、3-2と同じ変更影響グラフで結果を確認する。

この順序なら、読者は最初に「結局、何を採用するのか」を把握できます。その後の段階コードは設計を探す実験ではなく、確定した責任移動を一つずつ実現する説明として読めます。
...
| 比較する案 | 関数抽出、設定データ、継承、Factory、共通契約＋施策ルール＋固定Selector |
| 採用案 | `matches()` と `apply()` を持つ施策ルールを共通契約で扱い、固定の `RuleSelector` へ登録する |
| 採用理由 | P1の完了条件をすべて満たし、毎月の割引追加を新しいルール・必要な入力・登録へ限定できる |
| コードへの実装順 | ①`apply()` 契約と式の移動、②`matches()` と施策単位の責任化、③Selector・登録・参照注入 |

カードは上から下へ読みます。3-2との差から境界と完了条件を決め、すべての案をその条件で比較したうえで、採用案と実装順まで一続きにします。後続コードでは結論を再検討せず、各実装ステップがカードのどの責任移動を実現したかを確認します。

#### コードを書く前に採用する形を決める

候補は、コードを書きながら順番に思いついたものとして並べません。フェーズ1〜5の事実をそろえた時点で考えられる案を棚卸しし、同じP1の評価軸で一度に比較します。

| 案 | コードレベルで起きること | P1に対する評価 | 今回の判断 |
|---|---|---|---|
| 何もしない | `PaymentCalculator::calculate()` の `if-else` へ条件と式を追加し続ける | 計算本体と利用側の回帰確認が残る | 毎月追加される前提と合わない |
| 関数へ切り出す | `applyPremium(total)` のように式へ名前を付けるが、呼び分ける `if-else` は残る | 式は読みやすくなるが変更影響は切れない | 整理には使えるが採用設計にはしない |
| 割引率を設定データにする | 条件と率を表から読む | 率の違いには強いが、定額引き、排他、逐次適用を同じ形へ押し込む | 今回の仕様には合わない |
| `PaymentCalculator` を継承する | 施策ごとに `calculate()` を上書きする | 守りたい小計計算まで継承・複製の影響を受ける | 変化の単位が大きすぎる |
| Factoryの `if-else` で選ぶ | 条件分岐を計算本体からFactoryへ移す | 条件が別の1クラスへ集まり、追加時にFactoryを修正する | 問題の移動に留まる |
| 共通契約＋施策ルール＋固定Selector | 条件と式を各ルールへ置き、Selectorは登録順に `matches()` を呼ぶ | 計算本体・選択手順・利用側を変えずに追加できる | **P1を満たすため採用する** |

**採用する形：** `IDiscountRule` に `matches()`・`apply()`・`name()` を定め、施策ごとのクラスが条件と式を持ちます。`RuleSelector` は具体条件を持たず、登録されたルールを同じ操作で評価します。`PaymentCalculator` は選択済みルールを受け取り、小計計算後に `apply()` を呼びます。以降はこの結論を変えず、クラス図とコードへ落とします。
...
**採用した変更後クラス図は、変更前と同じクラスから読みます。** ①`OrderProcessor`、`CartPreviewService`、`PaymentCalculator` を探す、②`PaymentCalculator::calculate()` の引数から会員種別と `CampaignContext` が消えたことを確認する、③新しい依存先 `IDiscountRule` をたどる、④移した責任が具象ルールへ入り、`RuleSelector` には固定の選択操作だけがあることを確認します。
...
|---|---|---|
| `PaymentCalculator` の注記を小計計算だけにする | 割引境界を `apply(int)` として切り、計算本体は契約だけを呼ぶ | ステップ1：契約を追加し、式を具象ルールへ移す |
| `IDiscountRule` と利用関係を追加する | `PaymentCalculator` の依存先を具体条件から共通契約へ置き換える | ステップ1：具体的な割引名と式への依存を消す |
| 具象ルールと実装関係を追加する | `matches()` と `apply()` を施策クラスへまとめる | ステップ2：条件と式を同じ変更理由の単位へ移す |
| `RuleSelector` と登録関係を追加する | 登録順に `matches()` を評価し、`main()` で優先順を与える | ステップ3：具体条件を持たない選択処理を実装する |
| 利用側の関連を契約中心へ変える | 同じ `IDiscountRule` を計算とプレビューへ参照で注入する | ステップ3：利用側へ条件分岐を戻さず責任を維持する |
...
変更後クラス図は、P1と案の比較から確定した採用設計です。次のコードでは、この結論を探し直すのではなく、クラス図で変更した操作と関連を上から順に実装します。6-3では段階コードを統合したクラス図として再確認し、完成コードへ同じ変更要求を当てた結果は7-3で示します。

#### クラス図の差分から対策案が出てくる思考回路

採用したクラス図が、どの事実から導かれたかを整理します。ここで示すのは結論が揺れた順番ではなく、変更前クラス図の操作・依存と、採用した各変更を結ぶ論理です。

1. **変わる式と、変えたくない小計計算が同じメソッドにある。**
   - 変更理由が異なるため、式を小計計算から分けます。
   - 各式は `int` を受けて割引後の `int` を返すので、境界を `apply(int)` と定められます。
2. **適用条件と計算式は、同じ施策の追加・変更で一緒に変わる。**
   - 条件だけをFactoryへ移すと、分岐の集約点が移るだけです。
   - そこで条件と式を同じ施策クラスへ置き、`matches()` と `apply()` の組にします。
3. **各施策が `matches()` を持てば、選択手順から具体条件を消せる。**
   - 選択処理は「登録順に問い合わせ、最初の一致を返す」という固定ループにできます。
   - この固定処理を `RuleSelector` とし、Premium優先や併用優先は登録順で表します。
4. **外から登録する構造では、生成・所有・生存期間を決める必要がある。**
   - `main()` が具体ルールを所有し、`RuleSelector` と計算側は非所有参照を受け取ります。
   - その結果、新しい割引の変更先を、新しいルール、必要な入力、登録へ限定できます。

同じ事実でも前提が違えば別案になります。採用条件を混ぜないため、分岐条件を先に整理します。
...
以上から本章では共通契約、施策ルール、固定Selectorを採用します。以降のステップ1〜3は案を選び直す場面ではなく、確定したクラス図をコードへ実現する順序です。
...
#### 実装ステップ1：`apply(int)` の契約を作り、割引式を移す
...
**クラス図との対応：** `PaymentCalculator --> IDiscountRule` の利用関係と、各具象ルールの `apply()` を実装しました。これで小計計算から割引式を分離できます。採用設計には適用条件の移動も含まれるため、次は `matches()` を同じ施策クラスへ加えます。

#### 実装ステップ2：適用条件と式を施策クラスへまとめる
...
これで、`Premium` という条件と20%引きの式は `PremiumDiscount`、サマーセール条件と5%引きの式は `SummerSaleDiscount` にまとまります。

**クラス図との対応：** `IDiscountRule <|.. PremiumDiscount` などの実装関係をコードにしました。各施策が自分の条件と式を持つため、新しい施策で既存ルールを修正しません。次は、これらのルールを具体条件なしで選択する関連を実装します。

#### 実装ステップ3：固定Selectorへ登録し、選択済みルールを注入する
...
**クラス図との対応：** `RuleSelector o--> IDiscountRule` の登録関係と、`OrderProcessor` からSelector、`PaymentCalculator` から選択済みルールへの関連を実装しました。新しい割引の変更先は、新しいルールクラス、必要な入力、登録行に限定されます。`PaymentCalculator`、`RuleSelector`、`OrderProcessor` の処理手順、`CartPreviewService`、既存ルールは修正しません。

**P1の実装結果：達成。** これはステップ3で設計案が初めて決まったという意味ではありません。コードを書く前に確定した採用設計を、ステップ1〜3ですべて実現できたという確認です。
...
**実装の結論：** ステップ1で `apply(total)` の契約と式の移動、ステップ2で `matches()` を含む施策単位の責任化、ステップ3で固定Selector・登録・参照注入を実装しました。読者は先に示した採用クラス図と照合しながら、完成形がどのコードで成り立つかを順番に確認できます。

### 6-2：採用した契約とデータの置き場所を確定する

ステップ1〜3で実装した形を、割引ルールが満たすべき契約とデータ配置として確定します。
...
0章で共通化した4つの問いへ当てると、この章の設計は次のように整理できます。

| 共通の問い | 第1章での答え | 変えたくない側が知らなくなる詳細 |
|---|---|---|
| 何を分離するか | 施策ごとの適用条件・割引式・表示名を具象ルールへ分離する | 具体的な会員条件、キャンペーン条件、計算式 |
| どこで生成・選択するか | `main()` がルールを生成・所有し、`RuleSelector` が登録順に選択する | 具体クラスの生成方法と選択条件 |
| どう依存を渡すか | Selectorには登録、`PaymentCalculator` とプレビューにはコンストラクタ参照で注入する | 選択済みルールの具体クラス名 |
| 安定側はどう実行するか | `PaymentCalculator` は `IDiscountRule::apply()` だけを呼ぶ | 条件判定、割引式、生成・登録の手順 |
...
|---|---|---|---|
| P1 | `IDiscountRule::matches()`・`apply()`、施策ごとの具象ルール、`RuleSelector::select()`、契約を受け取る `PaymentCalculator` | **ステップ1：** `apply(int)` の契約と式の移動<br>**ステップ2：** 条件・式・名前を施策クラスへ集約<br>**ステップ3：** 固定Selectorへの登録と参照注入<br>**結果：** 変更先が追加ルール・必要な入力・登録へ縮んだ | **守った契約：** 割引境界の `int`→`int`、小計、整数演算順、Premium優先<br>**完了条件：** `PaymentCalculator`・`RuleSelector`・`OrderProcessor`・`CartPreviewService`・既存ルールは無変更<br>**最終判定：達成** |

この判定はP1表の各列をコードへ当てた結果です。次のクラス図は、段階的なコード変更を統合した構造だけを示します。

### 6-3：採用設計を統合したクラス図

冒頭で確定した変更後クラス図と、ステップ1〜3のコードが一致しているかを、同じ粒度で再確認します。薄い黄色は、冒頭と同じく責任を見直したクラスです。フェーズ7の完成コードに登場するクラスを省略せずに載せます。
...
採用案の比較はコードを書く前に終えています。ここでは選び直さず、採用した設計がP1の完了条件をコード上で満たしたかだけを確認します。変更影響が実際に縮んだかは、完成コードへ反映した後、フェーズ7の7-3で3-2と同じ変更要求を再適用して確認します。

| P1の完了条件 | 対応するコード | 確認結果 |
|---|---|---|
| 新しい割引で `PaymentCalculator` を修正しない | `PaymentCalculator` は `IDiscountRule::apply()` だけを呼ぶ | 達成 |
| 共通の選択手順を修正しない | `RuleSelector::select()` は登録ルールへ `matches()` を順に呼ぶだけ | 達成 |
| 注文処理とプレビューへ条件分岐を置かない | 選択済み `IDiscountRule` を参照で注入する | 達成 |
| 既存ルールを修正しない | 新しい条件と式は新しい具象ルールへ閉じる | 達成 |
| 小計・整数演算順・Premium優先を守る | `PaymentCalculator` の小計処理、各 `apply()`、登録順で維持する | 達成 |

コード上の完了条件はすべて達成しました。次のフェーズ7では、採用案を全コードへ統合し、実行結果と変更影響の両方で最終確認します。

### 採用した形と判断根拠を再確認する

採用判断はコードを書く前に示した比較表で確定しています。フェーズ2のヒアリングでは、マーケティング責任者から「今後も毎月ルールが追加される」と明言されています。共通契約、ルールクラス、登録処理が増える初期投資コストはありますが、今後の施策追加では `PaymentCalculator` と `RuleSelector` を開かず、適用条件と式を新しいルールへ閉じられます。

したがって、**`matches()` と `apply()` を持つ共通契約を通してルールを登録・選択する形を採用する**という結論は変わりません。受け入れるコストはルールクラスと登録行が増えること、得る効果は変えたくない処理が具体条件・式・生成方法を知らずに実行できることです。

フェーズ6では、先に確定した採用設計を接続点の契約・データ配置・クラス構造・実装ステップまで具体化しました。次のフェーズ7では、この設計を実行可能な全コードへ統合し、変更要求で効果を確認します。
...
| 🔴 フェーズ6：対策検討 | 案を同じ基準で比較して採用構造を先に確定し、責任分離→施策ルール→Selector・依存注入の実装順でコードへ反映した |
...
本章で共通契約とSelectorを採用した根拠は「毎月新しい割引が追加される」というヒアリング結果でした。変更頻度の裏付けがないままインターフェースとSelectorを用意すると、増えない柔軟性のためにクラス数・組み立てコスト・呼び出しの段数だけが増えます。変更要求の頻度が低いなら、関数抽出や単純な分岐を採用する方が適切です。
