...
1-5の変更後の動作例を、上のコードで通します。見るのは、サマーセールを足したことで `PaymentCalculator` の割引分岐がどれだけ増えたかです。
