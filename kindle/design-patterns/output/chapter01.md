...
★recordsはstaticで保持すればデータ保持されるのでは？
...
★「← 1-4で示した他のelse ifブロックがここに続く」の部分、略さずに変えて、同じ構造である接続点は、
★判断すべきとても大切な情報です。他の章でも接続点はすべて記載する事
...
呼び出し元は「適用するルールを共通の方法で選べること」と「割引後の金額を受け取れること」が必要です。したがって、選択側に必要な安定操作は `matches()`、計算側に必要な安定操作は `apply(int)` です。2つの変化軸は別々に追跡しますが、両方を同時に扱える一つのシステム構造として解決します。
...
フェーズ2で確定した2つの変化軸をP1・P2として追跡します。そのうえで、システム全体として、1つの具象ルールへ一緒に置く責任と、選択役へ置く責任を分けます。
...
P1とP2は異なる変化軸ですが、ある施策の「適用条件」と「選ばれた後の計算式」は同じ施策の変更で一緒に変わります。そのため、コード上では同じ具象ルールクラスへ `matches()` と `apply()` を置きます。サマーセールとキャンペーンの逐次適用は、計算方法と順序を持つ `SummerSaleAndCampaignDiscount` としてP2で追跡します。

---
> **📌 システム全体の課題（確定）**
> P1の適用条件とP2の計算式を施策単位の責任として分離し、共通の選択手順と小計計算から具体的な割引知識を外す。新しい施策を追加しても、注文処理・プレビュー・選択手順・小計計算・既存ルールを変更しないシステムにする。
---
...
- 新しい割引施策の追加を、施策固有の条件・式・表示名、必要な入力、組み立て情報に限定する。
- 共通の選択処理、`PaymentCalculator`、`OrderProcessor`、`CartPreviewService`、既存施策を変更しない。
- 選択手順は具体条件を知らず、小計計算は具体式を知らない。
- 小計、`int`→`int`、整数演算順、割引優先順位を維持する。
...
3案の構造差分は、選択と計算の安定側が「設定を解釈するエンジン」「関数の束」「インターフェース」のどれへ依存するかです。★今回採用しなかった案のイメージが湧かない。コラムでもよいから、章後半で抜粋でもいいからコードイメージや構造を説明してくれないか。他の章は、最終システム構造は１案しかないのか？この章のように複数あるなら、同じようにコラムで章後半で説明入れてほしい
...
| 🔴 フェーズ6：対策検討 | P1・P2をシステム全体の完了条件へ統合し、構造差分のある3つの完成システムから採用構造を決め、3ステップでコードへ反映した |
