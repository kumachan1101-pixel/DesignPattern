...
### 案2：具体×直接 ―― クラスを分けて責任を整理する

**この案の考え方：**
案1では `PaymentCalculator.calculate()` の1メソッドにすべての割引ロジックが詰まっていました。この案では割引ロジックを個別クラスに切り出します。`PaymentCalculator` は依然として各クラスの具体型を名指しで知っていますが、責任の境界が生まれ、コードが格段に読みやすくなります。
...
// PaymentCalculatorは具体クラスを直接使う（具体×直接）
class PaymentCalculator {
...
案1と比べると、各割引ロジックの計算式が独立したクラスに移りました。`PaymentCalculator` は条件の判断だけを担い、具体的な計算式は各クラスが持っています。接続形態（具体×直接）は同じでも、読みやすさと修正箇所の局所化が大きく改善されます。
