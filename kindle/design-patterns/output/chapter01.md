...
|---|---|---|---|
| F1 | 新しい割引ルールの追加が毎月続く | 継続的に | マーケティング責任者から直接確認 |
| F2 | 計算方法が「パーセント引き」から「定額引き」に変わる | 数ヶ月後 | 秋のクーポン企画として言及 |

フェーズ2で「今変わること（確定）」と「将来変わるかもしれないこと（リスク）」を分けて整理できました。次はリスクをもう少し具体的な「仕様の変化」として整理します。

### 2-5：変わる見込みと当面安定の前提を確定する

2-4のF-IDを、設計で扱う変化軸へ整理します。ここで「はい」とした項目は、機能を先に実装するという意味ではありません。フェーズ6で、**変えられるようにする部分を当面守る部分から分離し、その変化が起きても影響を局所化できる構造か**を判断するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：新しい割引ルールの追加が毎月続く | はい | 割引ルールの追加と選択 | 小計の合算、注文処理、支払金額の返却 |
| F2：計算方法が「パーセント引き」から「定額引き」に変わる | はい | 割引条件と金額計算式 | 割引前金額の受け渡し、注文処理、結果表示 |

したがって2-5の出力は、「割引ルールと計算式は差し替え対象にし、小計の合算から結果表示までの流れは守る」という設計条件です。フェーズ3ではR1・R2だけを現在の構造へ適用し、このF-IDはフェーズ6で採用構造を評価するときに使います。
...
public:
    ...
    bool isSummerSale = false;   // ← R1で追加
};
```

- R1のために、計算分岐だけでなく`CampaignContext`（1-4①）にも`isSummerSale`を追加しました。一つの確定要求が入力データと計算本体の両方へ波及しています。

R2の逐次割引を満たすには、既存キャンペーンとサマーセールの適用順を同じ `if` 群の中で調整します。1件の確定要求だけでも、条件の追加だけでなく既存分岐の計算順序まで確認対象になることが分かります。
...
**1つ目：影響範囲が広いこと。** R1のサマーセールを追加するために、`CampaignContext`と`PaymentCalculator`を同時に修正し、既存のPremium・キャンペーン割引も回帰確認しました。

**2つ目：既存分岐の解読が必要なこと。** R2の逐次割引を入れる位置を決めるために、`PaymentCalculator`内の既存キャンペーン条件、Premium条件、計算順序をまとめて読み直す必要がありました。
...
---
> **📌 問題（確定）**
> R1・R2を適用すると、`PaymentCalculator`と`CampaignContext`を修正し、その計算を使う`CartPreviewService`まで回帰確認する必要があった。割引条件・計算式・入力フラグが計算本体に混在し、一つの確定変更が広い影響確認を強いた。
---
...
割引ルールが計算の骨格に返しているのは「割引適用後の合計金額（`int`）」です。ここまでに見つけた二つの接続点を、解くべき課題として **P1・P2**（課題ID）で確定します。この番号は、以降のフェーズ6（対策）・フェーズ7（実装・結果）まで同じIDで追跡できるようにするための目印です。
...
**着目する共通点：** 見落としたくないのは、現状コードがすでに持っている共通点です。どの割引ルールも `PaymentCalculator::calculate(order, memberType, campaign)` が受け取り、`int` の支払金額を返すという、同じ入出力の形にそろっています（データと流れの共通点）。現場では、この共通の形すらない状態から入出力の型をそろえる作業が先に必要になることも多く、その意味で現状コードはすでに良い出発点です。共通点がそろっているからこそ、計算の骨格や利用側（`OrderProcessor`）の呼び出しを組み替えず、変わる「割引ルールの判定」だけを外へ出せば済みます。フェーズ6では、この共通点と課題を組み合わせて完成構造を決めます。
...
|---|---|---|---|
| F1：新しい割引ルールの追加が毎月続く | 割引ごとの差分をIDiscountRule実装として追加し、選択と計算の共通経路から分ける | 新しいRule、RuleSelectorへの登録、組み立て | 局所化できる。将来ルールは完成コードへ追加しない |
| F2：計算方法が「パーセント引き」から「定額引き」に変わる | 条件と計算式を同じ具体Ruleへ置き、金額を返す契約を維持する | 対象Ruleのmatches()とapply() | 局所化できる。未確定の定額ルールは完成コードへ追加しない |

F1・F2は、Strategyを採る根拠の一部です。ただし今回実装するのはR-IDで確定した割引だけであり、F-IDは変更影響の予測に使います。
...
ここでは今回の確定要求R1・R2だけを完成構造へ再適用し、フェーズ1で必要だった修正範囲と、実装後の結果を比較します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：Regular会員へサマーセール5%を追加し、Premium会員は対象外にする | `CampaignContext`と`PaymentCalculator`の分岐を同時に修正 | `SummerSaleDiscount`へ条件と式を置き、Regularだけに5%を適用。既存Premiumルールは変更しない |
| R2：既存キャンペーンと重なる場合は逐次割引する | `PaymentCalculator`の既存分岐と適用順を修正 | `RuleSelector`が選んだルールを優先順に適用し、10,000円から8,550円になることを実行結果で確認 |
