...
|---|---|---|
| P1 | 新しい割引の追加で `PaymentCalculator` の条件分岐と `CampaignContext` の入力項目が増え、`OrderProcessor` と `CartPreviewService` まで再テストする | `PaymentCalculator` の小計計算、注文処理、プレビュー処理、既存の割引ルール |

P1は対策案の番号ではなく、独立して扱う「割引追加の波及」という課題の識別子です。フェーズ6では、1-3のクラス図へ責任の注記を加え、どの操作と依存をどう変えるかを先に整理します。変更影響が実際に縮んだかは、完成コードの後で確認します。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた課題——**小計計算の骨格を守りつつ、割引ルールごとの判定・計算式を差し替えられる接続点を作る**——を受けて始めます。本章で解く課題はP1の1件です。途中の候補を採用案とはせず、P1の完了条件をすべて満たした形だけを最後に採用します。

検討は、次の思考順序で進めます。

1. 3-2で確認した変更影響を、そのまま比較元にする。
2. その影響を生んでいる現在の構造と、責任を分けた目標構造を描く。
3. 課題箇所だけをおさらいし、目標構造へ近づける最小のコード変更を1つ試す。
4. 変更結果をP1の完了条件で判定する。未達なら、残った影響だけを次の候補へ渡す。
5. P1が達成されるまで繰り返し、達成後のコードだけを採用する。
6. 修正後に同じ変更要求を当て、3-2と同じ変更影響グラフで結果を確認する。

この順序なら、後から完成コードを当てはめるのではなく、読者も「何が残ったから次の変更が必要なのか」を同じ判断材料から追えます。
...
|---|---|
| 課題ID | P1 |
| 現在→理想の差 | `PaymentCalculator` の分岐修正と `CartPreviewService` の回帰確認を、追加ルール・入力・登録の変更へ縮める |
| 切る境界・守る契約 | `calculate()` の小計計算と割引選択の間を切る。割引境界の `int`→`int`、小計、整数演算順を守る |
| 完了条件 | 新しい割引を追加しても、既存の `PaymentCalculator`・`OrderProcessor`・`CartPreviewService`・既存ルール・共通の選択手順を修正しない |
| 最初に試す候補 | 割引式を関数へ出す |
| それで減る影響 | 計算式に名前が付き、選択分岐と分けて観察できる |
| 残れば次に試す候補 | 同じ入出力なら共通契約へそろえる。それでも選択分岐が残れば、条件も施策単位へ移し、選択手順を共通化する |

カードは上から下へ読みます。3-2との差から完了条件を決め、その条件を壊さない最小候補を選び、残った影響だけを次の候補へ渡します。候補を試しただけでは採用しません。P1の完了条件をすべて満たしたかを、後続コードで毎回判定します。

#### 1-3のクラス図を、責任分離したクラス図へどう変えるか

責任の見直しに新しい種類の図は使いません。フェーズ1の1-3で作ったクラス図をコピーし、そこへフェーズ2〜5の判断を書き込み、同じクラス図を変更後の形へ更新します。
...
| フェーズ5の接続点 | 小計を渡し、割引後の `int` を受け取ればよい | 移動先との間に共通操作 `apply(int)` を置く |

クラス図の読み方も1-3から変えません。四角はクラス、四角の中はメンバーと操作、実線矢印は利用関係、破線矢印は参照や生成時だけ使う依存、白いひし形は保持関係です。**薄い黄色のクラスが、責任の見直しで着目する場所**です。色だけに依存せず、`【残す】`・`【移す】`・`【新設】` の注記でも同じ意味を示します。

**変更前クラス図は次の順に読みます。** ①1-3と同じクラスがあることを確認する、②`PaymentCalculator::calculate()` の注記で `【残す】` と `【移す】` を分ける、③`OrderProcessor` と `CartPreviewService` から `PaymentCalculator` へ向かう利用関係をたどる。この3点から、施策追加が共有計算クラスと利用側へ波及する構造を読み取ります。
...
`PaymentCalculator::calculate()` は `Order`、会員種別、`CampaignContext` を受け取ります。つまり、小計計算だけでなく、割引を選ぶための入力まで計算クラスが知っています。さらに `OrderProcessor` と `CartPreviewService` の両方がこのクラスを利用するため、施策条件を増やすと共有先の再確認へ影響が伸びます。

P1をクラス図の変更として書くと、次の4操作になります。

1. `PaymentCalculator` から会員種別と `CampaignContext` への依存を外し、小計計算だけを残す。
2. 移した条件判定と割引式の共通点を、`IDiscountRule::matches()` と `apply()` にする。
3. 各施策を具象ルールクラスにし、共通契約の実装関係として結ぶ。
4. 具体条件を持たない `RuleSelector` を追加し、登録済みの `IDiscountRule` だけを利用させる。

**変更後クラス図は、変更前と同じクラスから読みます。** ①`OrderProcessor`、`CartPreviewService`、`PaymentCalculator` を探す、②`PaymentCalculator::calculate()` の引数から会員種別と `CampaignContext` が消えたことを確認する、③新しい依存先 `IDiscountRule` をたどる、④移した責任が具象ルールへ入り、`RuleSelector` には固定の選択操作だけがあることを確認します。
...
classDiagram
    ...
    note for PaymentCalculator "【残した】小計計算\n具体的な条件と式を知らない"
    note for IDiscountRule "【新設】条件判定と割引式の共通契約"
    note for RuleSelector "【新設・固定】登録順にmatchesを評価"
    note for PremiumDiscount "【移した】Premium条件と20%引き"
...
前後のクラス図で確認する変更は、次の5点です。

1. `PaymentCalculator` 自体は残し、`calculate(Order, string, CampaignContext)` を `calculate(Order)` へ変える。
2. `PaymentCalculator` から `CampaignContext` への依存を削除し、代わりに `IDiscountRule` への利用関係を追加する。
3. 条件判定と割引式の移動先として、各具象ルールクラスを `IDiscountRule` の実装関係で追加する。
4. 選択条件を利用側へ戻さないため、`RuleSelector` と `IDiscountRule` の登録関係を追加する。
5. `OrderProcessor` と `CartPreviewService` の責任は維持し、選ばれたルールを組み立てる関係だけを変更する。

クラス数を増やすことが目的ではありません。変更前のクラス図にある操作と依存を見直し、1クラスに混在した責任を移した結果として、新しい契約、実装クラス、選択クラスが必要になりました。

クラス図の変更とコード変更を一対一で対応させると、次のようになります。

| クラス図をどう変えるか | コードレベルで何をするか | 後続の検討で確認すること |
|---|---|---|
| `PaymentCalculator` の注記を小計計算だけにする | `applyPremium()` などを抽出し、式が小計計算の外へ出せるか試す | 検討1：注記した責任を本当に分けられるか |
| `IDiscountRule` と利用関係を追加する | `apply(int)` を定義し、`PaymentCalculator` は契約だけを呼ぶ | 検討2：具体的な割引名と式への依存を消せるか |
| 具象ルールと実装関係を追加する | `PremiumDiscount` などへ式を移し、`matches()` も施策クラスへ置く | 検討3：条件と式を同じ変更理由の単位へ移せるか |
| `RuleSelector` と登録関係を追加する | 登録順に `matches()` を評価し、`main()` で優先順を与える | 検討3：Selectorの操作を割引追加で変更せずに済むか |
| 利用側の関連を契約中心へ変える | 同じ `IDiscountRule` を計算とプレビューへ参照で注入する | 検討3：利用側へ条件分岐を戻さず責任を維持できるか |

#### 自分のコードから同じクラス図を作る手順

読者が別のシステムでも同じ責任見直しを行えるように、クラス図の編集操作を固定します。

| 手順 | 問うこと | この章での答え | クラス図へ行う操作 |
|---|---|---|---|
| 1. 既存図を複製する | 現在のクラス・操作・関連は何か | 1-3のクラス図 | 新しい図を発明せず、既存クラス図を変更前としてコピーする |
| 2. 混在責任へ注記する | 1クラスの中に異なる変更理由があるか | 小計、条件判定、割引式 | 対象クラスを着色し、`【残す】` と `【移す】` の注記を付ける |
| 3. 移す責任の共通操作を探す | 移動後も利用側が必要とする操作は何か | `int` を受けて `int` を返す | インターフェースと `apply(int)` を追加する |
| 4. 移動先クラスを追加する | 何が同じ理由で一緒に変わるか | 施策の条件と式 | 施策ごとのクラスを追加し、実装関係でインターフェースへ結ぶ |
| 5. 元クラスの依存を変える | 元クラスは具体名を知らずに済むか | `PaymentCalculator` は契約だけを使う | 具体入力への依存を削り、インターフェースへの利用関係へ置換する |
| 6. 選択関係を追加する | 誰が実装を選び、優先順位を持つか | Selectorが選び、`main()` が登録順を与える | Selectorクラスと登録関係を追加する。`main()` の所有はコードで補足する |
| 7. 変更前後を差分確認する | 残すクラスの責任が増えていないか | 小計・注文処理・プレビューは維持 | 強調色のクラス、変更した操作、削除・追加した関連を1つずつコード検討へ対応させる |

手順2で `【残す】` と `【移す】` を分けられない場合は原因分析へ戻ります。手順3で共通操作を言えない場合は課題定義へ戻ります。新しいクラス名から考えず、既存クラス図の操作と依存をどの理由で変えるかを先に決めるのがポイントです。

変更後クラス図は、まだ採用済みの完成図ではありません。P1を満たすための仮説です。次のコード検討では、クラス図で変更した操作と関連を上から順に実現できるか確かめます。6-3ではコード検討後のクラス図として再確認し、完成コードへ同じ変更要求を当てた結果は7-3で示します。

#### クラス図の差分から対策案が出てくる思考回路

変更後クラス図を最初から正解として扱わないようにします。変更前クラス図のどの操作・依存に着目し、なぜ次のクラス図変更が必要になるのかを、コードの観察結果と往復しながら順にたどります。

1. **変わる式と、変えたくない小計計算が同じメソッドにある。**
   - 同じ場所にあることが波及の起点なので、まず式だけを外へ出せるかを試します。
   - この時点ではクラスは不要です。最小候補は関数抽出です。
2. **関数へ出した結果、どの式も `int` を受けて `int` を返すと分かる。**
   - 呼び出し側から見た形が同じなら、具体的な関数名を知らず、同じ操作名で呼べます。
   - ここで初めて、共通操作 `apply(int)` という候補が出ます。
3. **`apply(int)` をそろえても、「どれを使うか」の `if-else` が利用側に残る。**
   - 分岐をFactoryへ丸ごと移すだけでは、条件が集まる場所を変えただけです。
   - 各分岐を見ると、適用条件と割引式は同じ施策の理由で一緒に変わります。そこで両方を1つのルールへ移し、`matches()` と `apply()` の組にします。
4. **各ルールが `matches()` を持てば、選択側の処理は「順に聞き、最初の一致を返す」で共通化できる。**
   - 具体条件を知らない固定ループとして `RuleSelector` を作れます。
   - Premium優先や併用優先は、`if-else` の順ではなく登録順として組み立て側に置けます。
5. **ルールを外から登録するなら、生成・所有・生存期間を決める必要がある。**
   - `main()` が具体ルールを所有し、`RuleSelector` と計算側は非所有参照を受け取ります。
   - これで新しい割引の変更先は、新しいルール、必要な入力、登録へ限定できます。

別案へ分かれる条件もあります。

| コードから見えた事実 | 自然に出る別案 | 今回その案を選ばない理由 |
|---|---|---|
| 割引率という数値だけが違う | 割引率の表・設定データ | 定額引き、排他、逐次適用があり、同じデータ式では表しにくい |
| 差し替える操作が `int → int` の1つだけ | `std::function<int(int)>` | 適用条件、計算、表示名を同じ施策単位で扱う必要がある |
| 一致したルールをすべて適用する | ルールを順に通すパイプライン | Premiumは排他であり、今回は最初に一致した1方針を選ぶ仕様である |
| ルールが少なく、今後増えない | `if-else` または関数抽出で止める | 毎月追加されることがヒアリングで確定している |

以降の検討1〜3は、この思考をコードで確かめる部分です。各検討の結果が直前の推論を裏づけなければ、次の案へは進みません。
...
課題カードだけでは、最終的なクラス構成はまだ決まりません。ここからは、P1の課題箇所に対して **観察 → 仮説 → 最小変更 → 結果 → P1判定 → 次の判断** の順で進めます。各候補は完成形の一部を先に見せるためのものではなく、直前まで残っている問題を確かめる実験です。

#### 検討1：計算式を関数へ出せば、P1は解けるか

**観察：** `calculate()` には、「割引額をどう計算するか」と「どの割引を選ぶか」が同じ `if-else` に混ざっています。

**この候補を最初に試す理由：** まず計算式だけを関数へ出すのが、既存の制御構造を変えない最小の変更です。これで影響が十分に減るなら、新しいクラスや契約を増やす必要はありません。
...
int applySummerSaleAndCampaign(int total) {
    return applySummerSale(applyCampaign(total));
}

// calculate() の関連部分
if (memberType == "Premium") {
    total = applyPremium(total);
} else if (context.isSummerSale && context.isCampaignActive) {
    total = applySummerSaleAndCampaign(total);
} else if (context.isSummerSale) {
    total = applySummerSale(total);
} else if (context.isCampaignActive) {
    total = applyCampaign(total);
}
```

**結果：** 計算式には名前が付きましたが、新しい割引を追加すると `calculate()` の分岐を修正する点は変わりません。ただし、すべての式が **`int` を受け取り、割引後の `int` を返す**同じ形だと分かりました。

**P1判定：未達。** `PaymentCalculator` を修正しない、という完了条件を満たしていません。減ったのは式を読む範囲だけで、3-2の変更影響経路は切れていません。

> **「関数へ出したところで止めれば十分では？」**
>
> 割引追加がまれなら、その判断も成立します。しかし本章では毎月追加される前提です。関数を増やすたびに選択分岐を開くため、P1は残ります。この候補は採用案ではなく、次に「同じ入出力を契約として扱える」と判断する根拠にします。

> **「割引率を表や設定データにすれば、クラスは不要では？」**
>
> すべてが「条件に一致したら率を掛ける」だけなら、データ表の方が適しています。しかしヒアリングでは1000円引きクーポンが予告され、併用時には適用順もあります。式の種類と組み合わせ規則が同じデータ形に収まらないため、本章では振る舞いとして分けます。割引率だけが増えるシステムなら、表を採用する方が単純です。

> **「施策ごとに `PaymentCalculator` を継承すればよいのでは？」**
>
> サブクラスごとに `calculate()` を上書きすると、守りたい小計計算まで複製するか、親クラスの内部手順へ強く依存します。変えたいのは計算全体ではなく、小計後に適用する割引だけなので、継承の単位が大きすぎます。

#### 検討2：同じ入出力を共通契約へそろえれば、P1は解けるか

**前の検討から引き継ぐ事実：** 計算式はすべて `int → int` です。そこで、割引を選ぶ側が具体的な計算式を知らずに扱えるよう、同じ操作名へそろえます。
...
`PaymentCalculator` から具体的な割引名と選択分岐は消えました。しかし、実行するには、どこかで次の選択が必要です。

```cpp
const IDiscountRule* rule = nullptr;
if (memberType == "Premium") {
    rule = &premiumDiscount;
} else if (context.isSummerSale) {
    rule = &summerSaleDiscount;
}

PaymentCalculator calculator(*rule);
```

**結果：** 割引計算と小計計算の境界は `apply(int)` として切れました。`PaymentCalculator` は変更不要になりましたが、選択分岐が呼び出し側へ露出しています。このまま複数の利用側がそれぞれ選べば、同じ分岐が `OrderProcessor` やプレビューの組み立てへ重複します。

**P1判定：一部達成。** `PaymentCalculator` を守る条件は満たしましたが、注文処理とプレビュー処理を守る条件はまだ保証できません。残った問題は、割引の生成・選択を置く場所です。

> **「クラスではなく `std::function<int(int)>` でもよいのでは？」**
>
> `apply` だけなら、その方が小さく書けます。ただし次の検討では、適用条件 `matches()`、計算 `apply()`、表示名 `name()` を同じ施策単位で扱います。3つを1つのオブジェクトとして登録するため、本章ではインターフェースを選びます。関数とデータの組でも同じ契約を表せるなら、それを選んでも構造上の目的は達成できます。

#### 検討3：適用条件も各ルールへ分け、選択手順を固定すれば、P1を完了できるか

**前の検討から引き継ぐ問題：** `apply(int)` の契約はできましたが、利用側に選択分岐を置くと、変更影響が注文処理やプレビューへ戻ります。別のFactoryへ同じ `if-else` を移すだけでは、条件が1クラスに集まる構造自体は残ります。そこで、各ルールに「自分が適用対象か」を問い合わせられるようにします。
...
これで、`Premium` という条件と20%引きの式は `PremiumDiscount`、サマーセール条件と5%引きの式は `SummerSaleDiscount` にまとまります。次に、選択役は具体的な条件を持たず、登録された順に `matches()` を呼ぶだけにします。
...
登録順は、最初に一致したルールを採用する優先順位です。Premiumの排他、サマーセールとキャンペーンの併用、単独割引、割引なし、という業務ルールが組み立てコードに見える形になります。

**結果：** 新しい割引の適用条件と計算式は、新しいルールクラスにまとまります。選択役の `RuleSelector` は具体条件を知らないため変更せず、組み立て側で新しいルールを1行登録します。新しい判定材料が必要な場合だけ `CampaignContext` を増やします。`PaymentCalculator`、`RuleSelector`、`OrderProcessor` の処理手順、`CartPreviewService`、既存ルールは修正しません。責任の置き場所は、冒頭でP1から導いた目標構造と一致しました。

**P1判定：達成。** 課題カードの完了条件をすべて満たしたため、この段階を採用候補にします。検討1と検討2の途中状態は、判断根拠として残しますが採用しません。

> **「Factoryに `if-else` を集める方が短いのでは？」**
>
> ルールが少なく増えないなら、その方が単純です。しかし本章では毎月増えます。Factoryの条件分岐は新しい集約点になり、「条件を分ける」という構造変化を達成しません。そのため、条件と式を同じルールへ閉じ、選択役は同じ `matches()` を繰り返すだけにします。

> **「それでも追加時に登録行を直すのでは？」**
>
> その通りです。P1は変更をゼロにする課題ではなく、変更を新しいルールとシステムの組み立てへ限定する課題です。プラグインのように組み立てコードも一切変えず追加したいなら、設定ファイルや自動登録を次の候補にします。本章では月次施策をアプリケーションのリリースで追加するため、優先順位が読める登録行を明示するところで止めます。

> **「一致した割引をすべて順番に適用すれば、併用クラスも不要では？」**
>
> 任意の割引を自由に重ねる仕様なら、ルール列を順に適用するパイプラインが自然です。今回はPremiumを他施策より優先して打ち切り、サマーセールとキャンペーンは指定順で逐次適用するため、「最初に一致した1つの方針」を選びます。将来、任意併用が主要求になった時点では、複数ルールの合成を別課題として検討します。

**検討の結論：** 検討1で共通の入出力を発見し、検討2で `apply(total)` を契約にし、検討3で条件も `matches()` として各ルールへ移し、選択手順を固定しました。最初からStrategyパターンを当てたのではなく、P1の未達部分を1つずつ減らした結果として、適用条件と計算式を施策単位で差し替える形に到達しました。

### 6-2：見つけた形を契約にし、データの置き場所を決める

見つけた形を、割引ルールが満たすべき契約として定義します。

ここで、読者が「なぜ突然 `PaymentResult` が増えるのか」と感じないよう、P1とは別の統合上の判断を明示します。割引境界そのものは `int → int` で十分です。しかし実行結果では、内部の数値変化として小計と支払金額、さらに選ばれたルール名を表示します。最終金額の `int` だけを返すと、利用側が小計を再計算し、適用ルールを推測することになります。それでは、守りたい小計計算が利用側へ重複します。

そこで、割引ルールの契約は小さいまま保ち、`PaymentCalculator` から利用側へ返す結果だけをまとめます。
...
`PaymentResult` はP1を解くための新しい抽象ではなく、既存の計算結果とログに必要な状態を重複計算なしで渡す結果オブジェクトです。もし呼び出し側が最終金額だけを必要とし、内部状態を表示しないなら、`calculate()` は `int` を返すままでも構造分離は成立します。

```cpp
PremiumDiscount premium;
PaymentCalculator calculator(premium);        // 選択は組み立て側
PaymentResult result = calculator.calculate(order); // 利用側は選択条件を知らない
```

この時点でP1の対象コード、依存注入、利用入口までがつながりました。全クラスを統合したコードと実行結果はフェーズ7で確認します。
...
| 各割引を適用する条件 | `PaymentCalculator` の `if` 分岐 | 各 `IDiscountRule::matches()` | 条件と計算式を同じ施策単位へ閉じれば、追加時に既存条件を開かずに済む |
| ルールの優先順位 | `if-else` の記述順 | `main()` で `RuleSelector` へ登録する順序 | Premiumの排他や併用優先を、計算本体の分岐順から組み立て方へ移す |
| 選択手順 | `PaymentCalculator` の `if-else` | `RuleSelector` が登録順に `matches()` を評価する固定処理 | 新しい条件を増やしても選択アルゴリズムを変更しないため |

`PaymentCalculator` は `IDiscountRule` を受け取って `apply` を呼ぶだけです。接続点で受け渡すのは、**割引後の整数金額**です。逐次割引（サマーセール＋キャンペーン）は `SummerSaleAndCampaignDiscount` という1つのルールとして表します。具体ルールは `main()` のスタック変数として所有し、`RuleSelector`、`PaymentCalculator`、`CartPreviewService` は `const IDiscountRule&` を非所有で参照します。所有者が先に生成され、利用者より後に破棄される順序まで、フェーズ7の組み立てコードで確認します。

クラス分離を完成させるには、分離先だけでなく次の順で組み立てを確認します。

| 判断 | 関連コードで確認すること |
|---|---|
| 誰が具体実装を選ぶか | `RuleSelector` は具体名を知らず、登録ルールの `matches()` だけで選ぶ |
| 誰が生成するか | `main()` が具体ルールを生成し、業務上の優先順に登録する |
| 誰が所有するか | `main()` のスタック変数が破棄まで担い、Selectorと利用側は非所有参照にする |
...
|---|---|---|---|
| P1 | `applyPremium()`・`applySummerSale()`、`IDiscountRule::matches()`・`apply()`、`RuleSelector::select()`、`PaymentCalculator::calculate()` | **検討1：** 関数化だけでは分岐が残り未達<br>**検討2：** `apply(int)` へ統一すると計算本体は守れるが、利用側に選択が残り一部達成<br>**検討3：** 条件も各ルールへ移し、固定のSelectorへ登録して達成<br>**結果：** 変更先が追加ルール・必要な入力・登録へ縮んだ | **守った契約：** 割引境界の `int`→`int`、小計、整数演算順、Premium優先<br>**完了条件：** `PaymentCalculator`・`RuleSelector`・`OrderProcessor`・`CartPreviewService`・既存ルールは無変更<br>**最終判定：達成。検討3だけを採用** |

この判定はP1表の各列をコードへ当てた結果です。次のクラス図は、段階的なコード変更を統合した構造だけを示します。

### 6-3：構造の見立て（変換の結果、こうなる）

冒頭の変更後クラス図という仮説をコードへ落とした結果を、同じクラス図の粒度で再確認します。薄い黄色は、冒頭と同じく責任を見直したクラスです。フェーズ7の完成コードに登場するクラスを省略せずに載せます。この図は出発点ではなく、検討1〜3を通して確定した結論です。
...
図から読み取ること：`PaymentCalculator` から割引の条件判定と計算式が消え、契約 `IDiscountRule` への依存だけが残ります。各具体ルールが `matches()` と `apply()` を実装し、`RuleSelector` は登録された契約を評価するだけです。条件を別の1クラスへ移したのではなく、施策ごとのクラスへ分割した構造になりました。

### 6-4：課題カードに対して採用できる案かを判定する

フェーズ6は設計候補をコード断片で検討する段階です。ここでは変更影響グラフの結果を先に描かず、各候補がP1のどこまでを解いたかだけを判定します。完成コードへ反映した後、フェーズ7の7-3で3-2と同じ変更要求を再適用します。

| 候補 | 構造上できたこと | P1に残ること | 採用判定 |
|---|---|---|---|
| 検討1：関数抽出 | 小計計算と割引式をメソッド内で見分けられる | `PaymentCalculator` の条件分岐を修正し続ける | **未達。最終案にはしない** |
| 検討2：`apply(int)` 契約 | 計算本体から具体的な割引式を外せる | ルール選択の条件分岐が利用側に残る | **一部達成。最終案にはしない** |
| 検討3：`matches()` と固定Selector | 条件と式が施策ごとのルールへ分かれ、選択手順も固定される | 新しいルールの登録と、必要なら入力項目の追加だけが残る | **P1達成。採用する** |

採用するのは検討3だけですが、検討1・2を削除しません。前の候補で残った構造上の問題が、次の候補を必要にした証拠だからです。次のフェーズ7では、採用案を全コードへ統合し、実行結果と変更影響の両方で完了条件を確認します。
...
| 割引率をデータ表にする | 同じ計算式で率だけが違うルールを簡潔に追加できる | 定額割引、排他、逐次適用を同じデータ形に押し込む | 率だけが変わる場合なら採用候補。今回は合わない |
| `PaymentCalculator` を継承する | 施策ごとの計算クラスを作れる | 小計計算まで継承・複製の影響を受ける | 変化の単位が大きすぎる |
| 具体クラスへ分ける | 割引ごとの計算を別ファイルに置ける | 呼び出し側が具体クラスを知る | 中間策として有効 |
| Factoryの `if-else` で選ぶ | 条件分岐を計算本体から1か所へ移せる | 条件がFactoryへ集まり、追加のたびに選択本体を修正する | 「移動」に留まるため採用しない |
| 条件と式を各ルールへ分け、固定のSelectorへ登録する | 計算本体と選択手順を変えず、施策単位で追加できる | ルールクラスと登録行が増える | 今回の変更頻度なら採用する |

**今回の決断：** フェーズ2のヒアリングで、マーケティング責任者から「今後も毎月ルールが追加される」と明言されています。共通契約、ルールクラス、登録処理が増える初期投資コストはありますが、今後の施策追加では `PaymentCalculator` と `RuleSelector` を開かず、適用条件と式を新しいルールへ閉じられます。変更頻度と将来の回帰確認コストを比べ、今回は初期投資に見合うため、**`matches()` と `apply()` を持つ共通契約を通してルールを登録・選択する形を採用する**決断を下します。

フェーズ6で採用する設計（接続点の契約・データ配置・構造・影響範囲）が決まりました。次のフェーズ7では、この決断を動く実装（`CustomerDatabase`・全ルールクラス・`RuleSelector`・`PaymentCalculator`・実行結果）に落とし込み、変更要求で効果を確認します。

## 🟢 フェーズ7：対策実施 ―― 変化に強いコードを完成させる
### 7-1：解決後のコード（全体）

フェーズ6の検討3で採用した構造を、実行可能な完全なコードとして組み上げます。各役割ごとにコードを分けて確認します。
...
|---|---|---|---|
| P1 | `IDiscountRule` の全実装、`RuleSelector`、`PaymentCalculator`、`CartPreviewService`、`main()` の登録 | 新しい割引の条件と計算は同じルールへ入り、Selector・注文処理・プレビューは契約だけを利用した | `PaymentCalculator`・`RuleSelector`・`OrderProcessor`・`CartPreviewService`・既存ルールを修正せず、追加ルールと登録だけで拡張できる |

### 7-2：動作シーケンス図

フェーズ6の検討3で到達したルール差し替え構造について、実行時のオブジェクト間のやり取りを可視化します。`main()` がルールを登録し、`RuleSelector` が共通契約で選び、`PaymentCalculator` が具象クラスを知らずに処理を委譲する流れを確認できます。
...
この結果は、検討3を完成コードへ統合した後に得たものです。検討1・2の途中案では `PaymentCalculator` または利用側の分岐が残るため、この変更影響にはなりません。CIで既存テスト一式を実行することと、変更影響として個別に再確認する範囲は区別します。
...
| 🔴 フェーズ6：対策検討 | 関数抽出→共通操作→条件と式を持つルール＋固定Selectorの3段階で、直前に残った問題から次の案を導いた |
...
本章で検討3まで進めた根拠は「毎月新しい割引が追加される」というヒアリング結果でした。変更頻度の裏付けがないままインターフェースとSelectorを先に用意すると、増えない柔軟性のためにクラス数・組み立てコスト・呼び出しの段数だけが増えます。変更要求の頻度が見えないうちは、検討1〜2の範囲で止める判断もあり得ます。
