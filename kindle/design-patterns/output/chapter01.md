...
まだクラスの中身は見ません。この段階でつかんでほしいのは「変わる割引ルールを骨格から切り出し、選択（課題ID1）と計算（課題ID2）を一つの契約へまとめて、骨格は契約だけを呼ぶ」という筋だけです。「どのクラスが生成し、どの契約で実行するか」という具体の結論は、この後の課題ID1・課題ID2で決めていきます。決めた結論をまとめて振り返る表は、フェーズ6の末尾（6-4 設計トレース）に置きます。ここでは先に結論表を出しません。
