...
フェーズ4は「なぜ辛いか」を答えました。フェーズ5が問うのは「分けるべき境界で、実際に何が流れているか」「どちら側が変わり、どちら側を守るのか」「何ができれば解けたと言えるのか」です。クラスの参照関係ではなく、**値・型のレベル**に降りていきます。このフェーズでは対策の形（どんなクラスを作るか、切り出した知識の置き場所）はまだ決めません。
...
割引ルールが計算の骨格に返しているのは「割引適用後の合計金額（`int`）」です。ここまでに見つけた二つの接続点を、そのままP1・P2として確定します。

| 課題ID・接続点 | 接続するデータ | 変わる側 | 守る側 |
|---|---|---|---|
| P1：ルール選択 → 割引ロジック | 会員種別（`memberType`）、キャンペーン状態（`CampaignContext`）、適用順序 | 適用条件（`memberType == "Premium"` など）と選択候補 | 「条件に合うルールを順に評価して選ぶ」という選択の手順 |
| P2：割引ロジック → `calculate()` の骨格 | `int` 型の合計金額を渡し、割引適用後の `int` を受け取る | 割引の計算式（`* 80 / 100` など）と逐次適用の順序 | 小計の合算ループと結果を返す骨格 |

P1とP2は対策案ではなく、接続点で見つけた二つの変化軸をコードまで追うIDです。ある施策の適用条件と計算式は同じ施策変更で一緒に変わりますが、どのクラスへ置き、どう接続し直すかはまだ決めません。この事実を次のフェーズで分離単位を決める材料にします。

---
> **📌 システム全体の課題（確定）**
> 現在は、施策ごとに変わる適用条件・計算式が、守りたい計算の骨格・選択手順へ入り込んでいる。そのため、新しい施策を追加すると、P1の条件とP2の式だけでなく、注文処理・プレビュー・既存施策まで修正・再確認の対象になる。目指すのは、施策を追加しても変更対象がその施策固有の知識に収まり、骨格・選択手順・利用側・既存施策を変更しない状態である。
---

フェーズ6では、この課題をそのまま受け取り、分離の単位・配置・生成場所・再接続方法を決めます。

## 🔴 フェーズ6：対策検討 ―― システム全体の最終構造を定める

#### 接続点の分離・配置・組み立てを決める

P1とP2は別々に採用する対策ではありません。一つの施策では適用条件と計算式が一緒に変わるため、一つのシステムについて分離・配置・組み立てを決めます。ここでいう**組み立て**は、切り出したクラスを生成するだけでなく、所有・登録し、実行時に選択して利用側へ注入するまでの一連の処理です。

判断は次の順序で導きます。P1の条件とP2の式は同じ施策の変更で一緒に変わるため、同じ単位として切り出します。守る側は施策名や具体条件を知る必要がないため、「適用できるか」と「適用するといくらになるか」だけを共通操作にします。守る側が具象クラスを生成すると再び施策追加の影響を受けるため、`main()`が具象ルールとSelectorを生成・所有・登録して`OrderProcessor`へ渡します。`OrderProcessor`は実行時にルールを選び、その参照を使って計算役とプレビュー役を生成します。
...
| 組み立て方法（生成・所有・登録・注入） | `main()`が具象ルールと`RuleSelector`を生成・所有・登録して`OrderProcessor`へ注入し、`OrderProcessor`が選択したルールで計算役とプレビュー役を生成する | `PremiumDiscount`などを生成し、`RuleSelector::add()`で登録してから、Selectorを`OrderProcessor`へ渡す | 選択した`IDiscountRule&`で`PaymentCalculator`と`CartPreviewService`を生成する |

表の左から右へ読むと、課題定義の「変わる側／守る側」が、メソッド、クラス、生成・登録・注入のコードへ変換されます。後続のクラス図と実装ステップ1〜3は、この3行を同じ順序で具体化します。

#### システム全体の最終構造を決める

分離・配置・組み立てを決めた時点で、本章の最終構造は**具象ルールの登録システム**一つに定まります。P1とP2を別クラスへ分けると、同じ施策の条件クラスと計算クラスを対応づける新しい管理が必要になり、一緒に変わる知識を再び別々に修正することになります。そのため、本章の課題には合いません。
...
**薄い黄色が着目クラス**です。変更前では`PaymentCalculator`の`【残す】`と`【移す】`、変更後では移動先の`【新設】`に加え、選択結果から計算役とプレビュー役を生成する`OrderProcessor`を追います。矢印は1-3と同じ利用・参照・保持関係です。
...
このクラス図が、P1・P2を統合したシステム全体の設計結論です。課題IDは図の差分を追うために使い、以降はこの構造に必要なコードだけを示します。フェーズ7では、完成コードがこの設計どおりかを確認するため、同じMermaid定義をそのまま再掲します。

#### 課題箇所のおさらい（フェーズ3の関連コード）

統合表で特定した箇所だけを振り返ります。P1は条件分岐、P2は各分岐内の式です。小計計算など、課題に関係しないコードは省略します。
...
**P2との対応：** `PaymentCalculator --> IDiscountRule` の利用関係と、各具象ルールの `apply()` を実装しました。この時点では計算責任を移しただけで、システム全体の完了判定はまだ行いません。
...
**P1との対応：** `IDiscountRule <|.. PremiumDiscount` などの実装関係をコードにしました。P1の条件とP2の式を、同じシステム構造の中で施策単位にまとめました。
...
// main()内：Selectorを生成し、ルールを優先順に登録する
RuleSelector selector;
...
// OrderProcessor::process()内：選択結果で利用側を生成する
...
**P1・P2との対応：** `main()`での生成・所有・登録、Selectorの注入、`OrderProcessor`での選択と利用側の生成までを実装しました。これでクラス図の関連が実行時のオブジェクトとして成立し、二つの変化軸が一つの実行経路として接続されました。
...
ここではP1・P2を別々の対策として判定しません。一つの完成システムについて、各変化軸がどのコードへ反映され、その結果として守る側がどうなったかを追跡します。
...
**システム全体の実装結果：達成。** P1・P2の責任が同じ採用構造で接続され、フェーズ5の課題定義で目指した状態を実現しました。実際の動作と変更影響はフェーズ7で確認します。
...
| 🔴 フェーズ6：対策検討 | P1・P2を一つのシステムとして分離・配置・組み立てへ変換し、一意に定まった具象ルールの登録構造を3ステップでコードへ反映した |
