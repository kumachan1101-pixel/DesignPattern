...
判断は次の順序で導きます。P1の条件とP2の式は同じ施策の変更で一緒に変わるため、同じ単位として切り出します。守る側は施策名や具体条件を知る必要がないため、「適用できるか」と「適用するといくらになるか」だけを共通操作にします。守る側が具象クラスを生成すると再び施策追加の影響を受けるため、具象ルールを知る役割はシステムの組み立て場所へ集め、選択役と計算役には共通契約を注入します。
