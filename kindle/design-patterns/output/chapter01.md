...
| OrderHistory | 注文履歴の記録 | メモリとファイル（orders_log.txt）への追記 |
| PaymentCalculator | 支払金額の計算 | 合計金額の算出と割引ルールの適用 |
| CartPreviewService | カート画面のプレビュー表示 | 計算結果を使った金額プレビューの生成 |
| OrderProcessor | 注文処理の統合 | バリデーション・計算・履歴記録の一連の流れを担う |

---

### 1-3：クラス構成図

登場クラスの依存関係を図で整理します。`OrderProcessor` が中心となり、`CustomerDatabase`・`PaymentCalculator`・`OrderHistory` の3つを使います。`PaymentCalculator` は `CartPreviewService` とも共有されています。
...
classDiagram
    ...
    OrderProcessor --> OrderHistory : 使う
...
`OrderProcessor` が `CustomerDatabase` で顧客情報を取得し、`PaymentCalculator` で支払金額を計算して、`OrderHistory` に結果を記録しています。`CartPreviewService` は同じ `PaymentCalculator` を利用します。
...
はじめに、注文データ・顧客情報・履歴記録の役割を担うクラス群を見ます。
...
`CustomerDatabase` は `std::map` でIDと顧客情報を対応づけています。`OrderHistory` は同一セッションの注文をメモリに保持しながら、`orders_log.txt` にも追記することで、次回起動時に過去の注文を参照できます。
...
`OrderProcessor` は顧客IDのバリデーション・割引計算・履歴記録をまとめて担います。
...
`CustomerDatabase` からの情報を使ってバリデーションと計算を行い、`OrderHistory` が結果をメモリとファイルの両方に記録しています。次のフェーズで変更が来たときに何が起きるかを確認します。
