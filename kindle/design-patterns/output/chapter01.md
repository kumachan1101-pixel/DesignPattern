...
2回目に注目してください。**キャンペーンを有効にしても支払金額は8,000円のまま**です。Premium会員の20%引きが優先され、キャンペーンの10%引きは併用されません。1回だけ実行していたら、この「効かない」という仕様は見えませんでした。★基本形の動きを見せたいので、レギュラー会員のキャンペーン割引にして。
...
クラス図は「誰が誰を持つか」という静的な関係を示します。次のシーケンス図は、正常系（会員種別あり・キャンペーン中）で、注文1件を処理する間に各クラスがどの順で呼ばれ、どの値を受け渡すかを時系列で示します。割引額を決める具体の分岐は `PaymentCalculator` の注記に書き出し、エラー系はメモとして添えます。1-4の現状コードを読む前の地図として使います。★mainは、CheckoutResultRendererから最後結果をもらうように見えませんか？mainの相手はOrderProcessorのみです。voidの場合は、voidと記載しませんか。全シーケンス見直して。
...
クラスを1つずつ、上から順に読みます。**メンバー変数と、それを使う処理を同じ場所で見られるように**しています。宣言と定義を分けるのは `OrderProcessor` だけです。
> **メンバーを見ないと読めない関数は、メンバーと一緒に置く。**★これは、内部メモでは？他にもあったら消して
...
---
★カートプレビューの機能って必要ですか？この章の本質として何か必要？修正が手間なら今のままでよいが、気になったのは、以下のコード、processを実行するために、プレビューした方が良いのでは？プレビューで問題ないことを確認して実行したいのですよね。
...
| Regular | ✓/✗（どちらでも） | ✗      | 変更なし　★変更ない場合の金額を記載して、他は記載しています。                   |
...
★この項目、全章採用になってます。判定方法は正しいのか、本当にすべて採用なのか。不採用がないなら、この項目を伝えるメリットがないように思います。ただ、対策検討の契約で参照しているのですね。
...
フェーズ4で、割引の処理が「どの施策を選ぶか」と「選んだ施策がいくら引くか」という**別々の理由で変わる2つの判断**を、同じ `if` 連鎖へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つの価格計算経路へ再結合することです。この段階では、問題から導いた「規則差し替え構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。

```mermaid
flowchart TB
    A[現在<br/>一本のif連鎖に<br/>適用条件と計算式が混在] --> B[分離判断<br/>二つの変化軸を別責任へ分け<br/>一つの価格計算経路へ再結合]
    B --> C[課題ID1<br/>施策ごとの対象条件と優先度を<br/>施策の側へ]
    B --> D[課題ID2<br/>施策ごとの計算式を<br/>共通契約の裏へ＝規則差し替え構造]
...
★以下よくわからない。曖昧な表現すぎて、全くイメージできないのですが。他の章も同様。これから決める事だけを記載しておけばよいのでは。決まっている事はここで復習しなくてもよいのでは。
...
**この3つは独立して決められません。** 分離方法が決まらないと配置場所が決まらず、配置場所が決まらないと組み立て方法が決まりません。だから順に決めます。決まった結論をまとめて振り返る表は、フェーズ6「生成・所有・実行順のまとめ（6-1）」に置きます。
...
**どう解決するか（方針）：** 施策ごとの対象条件と計算式を、施策そのものが持つ「規則差し替え構造」へ揃えます。
...
**本文は、課題が2つあっても6段を1周します。課題別に節を切りません。** 後半3段（生成・注入・利用開始）は課題をまたぐ共同の判断なので、課題別に書くと同じ判断を2回書くことになります。課題どうしの比較も、同じ段の中で隣り合っていないと読めません。前半3段は課題ごとに、後半3段はまとめて決めます。★２回書くことになるって、著者都合の話は消して。

**課題どうしの関係を、2つの基準で確かめます。**
★以下の考え方は何ですか。よくわからない。この結果が次の工程にどう接続されるのか。必要なのですか。全章見直して、というか、「この課題（何を解きたいか）」に記載しておけばよいのでは。
...
**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、`if`連鎖に混在する施策ごとの対象条件と計算式を、別々の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
**まず、境界をクラス契約にする必要があるかを確かめます。** ここは踏み外しやすいところです。フェーズ5「課題の確定（5-3）」が確定したのは「この接続点を解く」ことであって、「クラスを増やす」ことではありません。**フェーズ6に入ったら必ずクラスが増える、というわけではありません。**
★以下該当しないケースはあったのでしょうか。ないなら、毎回問わなくてもよいのでは？
...
**4操作のうち、2つが課題ID1、1つが課題ID2、1つが守る範囲のためです。** `matches()` と `priority()` が選択、`apply()` が計算、`name()` が結果返却に使われます。**2つの課題が1つの契約に同居しています。** 分ける単位（施策）が同じだからです。★nameなんて要件あったか？ログ用ならそう記載して。
...
★この対応はかなり微妙。他の具体クラスを意識しないと、新しい具体クラスが足せません。何故、この方法でよいと判断したのか。具体クラス同士が意識しないから拡張できるのです。
...
**もう一度検算します。契約のメソッド以外に書きたくなるものがあるか。** 出てきませんでした。4操作で、条件も優先度も式も名前も表せています。★著者メモですか？著者メモは消して。他にもないか。全章見直して
...
ここまで問題から導いた「規則差し替え構造」は、一般に **Strategyパターン**と呼ばれます。ここからは題材を離れ、この名前で共有される抽象的な骨格を確認します。
