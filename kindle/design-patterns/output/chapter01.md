...
上記コードを実行し、1-2の動作例テーブル（4行）と排他ルール・エラー条件が現状コードで再現されることを確認します。実行結果：

```
田中 一郎 の支払金額は 8000 円です。   // 動作例1：Premium 20%引き
田中 一郎 の支払金額は 8000 円です。   // 動作例2：Premium優先（無効）
佐藤 花子 の支払金額は 9000 円です。   // 動作例3：キャンペーン10%引き
カートプレビュー: 9000 円です。        // 動作例3：同じ計算を共有
鈴木 次郎 の支払金額は 3000 円です。   // 動作例4：割引なし
エラー: 顧客ID UNKNOWN は登録されていません  // エラー条件
```

> [!NOTE]
> 6行の出力は、1-2の動作例テーブルの各行（Premium 20%引き・Premium優先・キャンペーン10%引き・割引なし）と排他ルール・エラー条件に対応しています。
...
> [!NOTE]
> 5件の出力は、1-5「変更後の動作例」の各行（Premium優先・重ね掛け8,550円・サマーセール単独9,500円・割引なし）と一致しています。リファクタリング後も支払金額が変わっていないことを、この対応で確認できます。
