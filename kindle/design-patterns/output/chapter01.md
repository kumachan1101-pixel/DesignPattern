...
| ケーブル比喩 | コードの対応箇所 |
|---|---|
| 「抽象」＝汎用USB-C規格 | `calculator.calculate(order)` という呼び出し口は汎用的で、`OrderProcessor` は内部の割引アルゴリズムを知らない |
| 「直接」＝直差し | `PaymentCalculator calculator;` — `OrderProcessor` が具体クラスをメンバとして直接保持しており、外から差し替える口がない |
