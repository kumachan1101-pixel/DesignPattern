...
| 仕様項目 | この章で扱う値 | 説明 |
|---|---|---|
| 商品リスト | 商品名と単価の一覧 | 小計を計算する材料になる |
| 顧客IDと会員種別 | 登録済み顧客のID、Premium / Regular | 顧客IDから会員種別を確認し、Premiumの場合だけ会員割引を適用する |
| キャンペーンフラグ | 期間中 / 期間外 | 期間中の場合だけキャンペーン割引を適用する |
| 出力 | 最終的な支払金額 | 小計から該当する割引を反映した金額を返す |

この時点では、なぜ割引ルールを分けるべきかはまだ判断しません。まず、どの入力がどの判定や計算に使われ、最終金額へつながるかだけを確認します。
