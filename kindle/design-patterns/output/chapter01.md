...
**代表的な動作（先に動かして全体像をつかむ）：** 詳細に入る前に、このシステムを一度動かした様子を見ます。Premium会員の田中さんがワイヤレスイヤホン（10000円）を1件注文すると、内部は会員種別とキャンペーンの有無から割引を判定し、支払金額を計算して表示します（同じ入力の完全なコードと実行結果は1-4に掲載します）。
...
| 要求ID5 | 継続<br/>根拠: 変更ID1, 変更ID2 | 計算結果を購入結果として表示する | 適用した全割引と最終金額を表示する |
...
| `CheckoutResultRenderer` | 購入結果を表示する | **変更なし** |
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を一つのシステムとして分離・配置・組み立てへ変換し、一意に定まった具象ルールの登録構造を3ステップでコードへ反映した |
