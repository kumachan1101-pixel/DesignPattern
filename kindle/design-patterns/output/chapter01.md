...
**割引ルールが変わるたびに、関係ないはずの場所まで修正が必要になる。これは「変わる理由（個別の割引ルール）」と「変わらない構造（処理の全体的な流れ）」が、同じ場所に混在しているからではないかと思いませんか。**
...
classDiagram
    ...
    class PaymentCalculator {
        +calculate(Order, CampaignContext) int
    }
    ...
    OrderProcessor ..> CampaignContext : 参照する
    Order o-- Item : 持つ
```
