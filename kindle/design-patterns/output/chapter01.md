...
> [!INFO] コラム: 振る舞いを持つべきは誰か？
> オブジェクト指向では「データと振る舞いをまとめる」と教わることが多いですが、今回のような「割引の計算ロジック」をデータクラス（Order）に持たせるのは少し危険です。割引ルールは頻繁に変更されるため、単なるデータの入れ物であるOrderクラスに計算ルールを混ぜてしまうと、データ構造の変更とルールの変更という「2つの異なる理由」でクラスが修正されることになってしまいます。役割を明確に分けることが保守性の鍵となります。
