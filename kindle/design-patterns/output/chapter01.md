...
★粘土の話、ここでも出してください。どのような粘土の操作が、この章の話とつながるのかを、子供でもわかるように説明してほしい。これは、発明的な本になると思ってます。何故なら、各章の先頭部分の粘土の話は、子供でも理解できる内容になっているからです。子供でも理解できるなら、あとは、粘土がプログラミング言語に変わるだけですよね？ここの絵の作成は、トークンを多く使うので、プロンプロトここに埋め込んでもらえたら、別途画像生成AIで作成します。すみません。やっぱりレゴブロックの話にしてほしい。
...
★以下のヒアリングで疑問に思う事があります。消費税率って、国で決まるものですよね？ルールによって変わりますか？ってどんな意図の質問でしょうか？むしろ、ここで気にすることは、国の政治状況から、消費税率が変わるかどうかって話なのではないのでしょうか。私が思うに、数年に一度は消費税率が変わっている印象があります。もし消費税率が良く変わるなら、どうしますか。ただの税率をかけるだけだから、クラスを分けるまででもないと思います、それならそれでも現状でよいならOKです。
...
| 原因 | 4分類 | 手札候補 |
|---|---|---|
| 変わる割引ルールと変わらない計算骨格が同居している | 1.分ける | インターフェース抽出 |
| 変わるルールを変わらない骨格から切り出したい | 1.分ける | （関数として切り出す案も「分ける」方向） |

候補は「分ける」方向で2つあります。どちらが原因に直接対処できるか、別々に試します（ケース③）。

---

### 1.5.2 手段①：関数として切り出す

手札「分ける（クラス分割前の関数切り出し）」を試します。割引計算を別の関数に切り出すアプローチです。
