...
動作例テーブルの4パターンをコードが処理する流れを確認できました。次のフェーズで変更が来たときに何が起きるかを確認します。
...
フェーズ1の仕様表を振り返ります。現在の割引ルールは「プレミアム会員20%オフ」と「キャンペーン時10%オフ」の2種類ですが、これらはビジネス戦略に応じて変更される性質の仕様です。

- プレミアム会員の割引率：会員優遇の施策として改定されることがある
- キャンペーン割引の条件・率：販促施策の内容に合わせて変わる可能性がある
- 割引の計算構造（パーセント引きか定額引きか）：今後多様化する可能性がある

一方、注文金額の計算フロー（小計→割引→合計）は、ECサイトの基本的な決済の流れとして安定している部分です。

**仮説：割引ルールの種類・条件・計算方法は、今後も追加・変更が続く可能性がある。**
...
ヒアリングで判明した将来変更を踏まえ、「秋のクーポン（1000円引き）」が実際に届いた場合、割引ルール仕様がどう変わるかを整理しておきます。

| ルール名 | 現在 | 将来（数ヶ月後） |
|---|---|---|
| プレミアム割引 | Premium会員に20%引き | 変更なし |
| キャンペーン割引 | Regular会員にキャンペーン10%引き | 変更なし |
| サマーセール割引 | Regular会員に5%引き（今回追加） | 変更なし |
| **定額クーポン（将来）** | —（なし） | **一律1000円引き（対象条件は別途決定）** |

計算方法がパーセント引きと定額引きで異なるため、単純に `else if` を追加するだけでは済まない変更になります。この変更が来たとき、現在の構造でどう対応するかをフェーズ3で確認します。
...
ヒアリングで「秋に1000円引きクーポンが来る予定」と確認しました。現在の変更が終わった直後のコードに、そのクーポンも追加しようとするとどうなるか確認してみます。

```cpp
// サマーセール対応後のコードに、ヒアリングで予告されたクーポンも追加しようとすると…
if (order.customerType == "Premium") {
    total = total * 80 / 100;
} else if (context.isSummerSale && context.isCampaignActive) {
    ...
    total = total * 90 / 100;
} else if (context.isCouponActive) {
    total = total - context.couponAmount;   // ← 引き算のロジックが混入
}
// クーポンとサマーセールが同時に来たら？ クーポンとキャンペーンが重なったら？
// さらなる条件分岐の追加が必要になる
```

パーセント引きと定額引きという性質が違う計算が混在し始めました。重複ケースの条件もすべて書き足す必要があります。ヒアリングで「毎月新しいルールが来る」と確認した以上、この構造での対応は限界が見えてきました。
...
現状の `PaymentCalculator` はプレミアム割引・キャンペーン割引のすべてのルールを持っています。しかしこれらのルールは「連動して変わる」ものではありません——会員サービス企画チームとマーケティングチームが**それぞれ独立したタイミングで**変更を判断します。独立して変わるものが同じクラスに混在しているため、どちらかの変更がもう一方のコードを常に巻き込んでしまいます。
...
「割引ルールを分離する」と決めたとき、次に考えるべきことがあります。プレミアム割引とキャンペーン割引を、**1つのクラスにまとめるか、それとも別々のクラスに分けるか**です。

フェーズ4の分析で明らかになったように、プレミアム割引は会員サービス企画チームが、キャンペーン割引はマーケティングチームがそれぞれ独立したタイミングで変更を判断します。この2つは連動して変わるわけではないため、1つのクラスにまとめると「プレミアム割引だけ変えたのに、キャンペーン割引まで確認が必要になる」という問題が残ります。

**変わる理由が異なる → それぞれを別々のクラスに分ける。** これが今回の粒度の判断です。

ただし、どちらも「割引後の合計金額（`int`）を返す計算」という操作の種類は同じです。インターフェースとして抽象化する際は、同じ `IDiscountStrategy` で表現できます。クラスは別々、インターフェースは共通——この構造がフェーズ6での設計方針になります。
...
| 🟣 フェーズ2：仮説立案 | フェーズ1の仕様表から「割引ルールの種類・条件・計算方法は今後も変更が続く」という仮説を立て、ヒアリングで裏付けた |
...
| **得られること** | **この章のどこで示したか** |
|---|---|
| 1. 変動箇所の識別 | フェーズ2の仮説立案とヒアリングで、割引ルールの種類・条件・計算方法が今後も変更され続けることを確認した |
