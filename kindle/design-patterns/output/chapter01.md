...
フェーズ4で根本原因が言語化できました。次のフェーズ5からは、この原因（混在）を解きほぐし、抽象へ向けて段階的に設計を進化させる旅に出ます。

---

## 🟡 フェーズ5：課題定義 ―― 解くべき「塊」を特定する

フェーズ4の分析により、問題の根本原因は「計算の骨格」と「割引の条件分岐」という、変わる理由が違う2つのものが混在していることだと分かりました。

したがって、今回私たちが解くべき課題は、`PaymentCalculator` の中にある **「割引の条件文（if）」と「中の処理（計算式）」を含むブロック全体を、丸ごと外に分離すること** です。
...
最終的な目標は、この `PaymentCalculator` から「どんな条件のときに割引するか（if文）」という知識すらも完全に追い出し、無傷の骨格だけにすることです。

フェーズ5でターゲットが明確になりました。次のフェーズ6では、この「if文の塊」をどのように分離していくか、関数化・クラス化・抽象化と段階的に対策を検討していきます。
...
はじめに、クラスを分けずに、ターゲットの塊をプライベートメソッド（関数）として分離してみます。
...
**この段階の評価：**
`calculate()` の骨格は非常にスッキリしました。しかし、分離したとはいえ `applyDiscount` は同じクラス内にあります。「新しい割引ルール」が来るたびに、結局はこの `PaymentCalculator` クラス自体を開いて `if` 文を書き足す必要があります。**条件文が本体に取り残されている**ため、「変わる理由の混在」という根本問題は解決していません。

### ステップ2：別クラスに切り出す（具体×直接）

次に、このメソッドを完全に別のクラス `DiscountCalculator` に移動し、そちらに処理を委ねる形にしてみます。

```cpp
// 割引計算の専用クラス
class DiscountCalculator {
public:
    int apply(int total, const Order& order) {
        if (order.customerType == "Premium") return static_cast<int>(total * 0.80);
        // ... (サマーセール等のif文が続く)
        return total;
    }
...
class PaymentCalculator {
    DiscountCalculator discountCalc; // 特定の具体クラスを直接知っている
public:
    int calculate(const Order& order) {
        int total = 0;
        for (const auto& item : order.items) total += item.price;
        
        return discountCalc.apply(total, order); // 具体クラスに委ねる
    }
};
```

**この段階の評価：**
割引のロジックが別のファイル（クラス）に分かれました。しかし、`PaymentCalculator` は依然として `DiscountCalculator` という特定の具体クラスに直結（具体×直接）しています。この状態では、もし「全く別の割引計算ルール（例：海外向け計算クラス）」を作って切り替えたい場合、呼び出し元である `PaymentCalculator` のコードを書き換えなければなりません。差し替えができないのです。

### ステップ3：入れ替え可能にする（抽象×直接）

既存の呼び出し元を一切触らずに、新しいルールをポンと差し替え可能にするにはどうすればよいでしょうか？
「特定の具体クラス」ではなく、「割引ルールが守るべきインターフェース（抽象）」に依存させます。
...
**この段階の評価：**
ついに、`PaymentCalculator` の中から「どの割引を適用するか」という `if` 文（条件文）が完全に消え去りました！ 実行時にどんな `IDiscountRule` が渡されるかは呼び出し側（`main` 等）が決定します。接続の形が**「抽象×直接」**へ到達したことで、本体は無傷のまま、いくらでも新しいルールを差し替えられるようになりました。

---

### どこまで設計を進めるべきか（採用案の決断）

それぞれのステップには一長一短があります。ステップ3の抽象化は強力ですが、ファイル数や型が増えるという「初期投資コスト」もかかります。どこで止めるかは、**「今後の変更頻度（ビジネス要求）」**で決断します。

*   **Step 1（関数化）で止めるケース：** 今回のサマーセールが「今回限りの特例」の場合。余計なクラスを作らず、ファイルをシンプルに保つべきです。
*   **Step 2（別クラス化）で止めるケース：** 将来的にルールが増えるかもしれないが確証がない場合。ファイルだけ分けておき、本当に増えた時にStep 3へ進化させる「様子見」の判断です。
*   **Step 3（抽象化）まで進むケース：** 「毎月新しい割引が追加される」と確定している場合。今すぐ初期コストを払ってでも、将来の変更コストをゼロにするべきです。
...
ステップ3で到達したStrategyパターンの実行時のオブジェクト間のやり取りを可視化します。`main()` が依存関係を注入し、`PaymentCalculator` が具象クラスを知らずに抽象インターフェース経由で処理を委譲する流れが確認できます。
...
| 🔴 フェーズ6：対策検討 | 4案を比較した。各案で複数の実装手段を確認し、根拠を持って1手段を選んだ。Step3（抽象×直接）を選定した |
