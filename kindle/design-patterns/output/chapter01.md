...
★決定って何？接続点のどのように変更するかの手段カテゴリを記載している？

| 決定 | システム全体の考え方 | P1のコードへの反映 | P2のコードへの反映 |
|---|---|---|---|
| 分離 | 安定側から施策固有の条件と式を外し、共通操作だけを境界に残す | `matches(memberType, context)` を契約にする | `apply(int)` を契約にする |
| 配置 | 同じ施策で一緒に変わる条件・式・表示名を、同じ施策オブジェクトへ置く | `PremiumDiscount::matches()` などへ条件を置く | 同じ具象ルールの `apply()` へ式を置く |
| 組み立て（生成・所有・登録・注入） | `main()`が具象ルールと`RuleSelector`を生成・所有・登録して`OrderProcessor`へ注入し、`OrderProcessor`が選択したルールで計算役とプレビュー役を生成する | `PremiumDiscount`などを生成し、`RuleSelector::add()`で登録してから、Selectorを`OrderProcessor`へ渡す | 選択した`IDiscountRule&`で`PaymentCalculator`と`CartPreviewService`を生成する |
