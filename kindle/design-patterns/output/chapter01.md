...
> この後のステップ5で選ぶ試み②（インターフェースによる分離）の採用は、
> コストに見合わない可能性が高くなります。
...
> このステップは、第0章で紹介した「手札」を実際の状況に当てはめる場所です。
> 最初の試みは直感的な案から始め、それぞれの限界を確かめた後、
> 最終的に**手札①（インターフェース抽出）**を適用した試み②にたどり着きます。

### 1.6 試み①：現実的な最初の案と、その限界

変更要求を受けたとき、エンジニアが最初に試みるのは大きく3つです。それぞれ確認します。

---

**案A：割引率を引数として渡す**

割引の詳細を呼び出し元から渡せれば、`calculateTotalAmount` は変わらずに済むはずです。
...
// 呼び出し側
int corp = calculateTotalAmount(100000, 10);   // 10%引き
int sale = calculateTotalAmount(10000, 15);    // 15%引き
```
...
「知識の移動先が `calculateTotalAmount` から呼び出し元に変わっただけ」です。

さらに、**秋セール期間中に法人注文が入った場合**を考えてみます。
...
呼び出し元が「2つのルールが重なるとき、どちらを優先するか」「合算してよいか」を先に判断し、1つの数値に圧縮してから渡すことになります。
ルールの判断を `calculateTotalAmount` の外に出したはずが、呼び出し元に同じ判断ロジックが移動しただけです。

---

**案B：引数をもっと増やす**

案Aの限界（複合条件が表現できない）を解消しようとすると、引数を増やす方向に進みます。
...
int calculateTotalAmount(
    ...
    // 全ルールの判断がここに戻ってくる
    int amount = baseAmount;
    if (isCorporate) { amount = amount * 9 / 10; }
    if (isCorporate && isPremium && quantity >= 100) { amount -= 50000; }
    // ...
}
```

引数の数だけルールの条件が増え続けます。呼び出し元はすべての条件を知っていなければならず、問題の構造は変わりません。「変わる理由が異なるルール同士が同じ関数の引数として混在している」状態はそのままです。

---

**案C：新しい関数を追加する（既存コードに触れない）**

「既存の `calculateTotalAmount` には触れず、秋セール用の関数を新規に追加する」方針です。

```cpp
// 案C：既存関数に触れず、新しい関数を追加する
int calculateTotalAmount(int baseAmount, ...) {
    // 既存コードをそのまま残す（変更しない）
}

// 秋セール用に新規追加
int calculateAutumnSaleAmount(int baseAmount) {
    int amount = baseAmount * 85 / 100;  // 15%引き
    amount = static_cast<int>(amount * 1.1); // 消費税 ← 骨格のコピー
    return (amount > 0) ? amount : 0;
}
```

既存コードへの影響はゼロです。一見安全に見えます。しかし「消費税を加算する」という骨格が2箇所にコピーされました。消費税率が変わったとき、2箇所を同時に変更しなければなりません。ルールが増えるたびに骨格のコピーが増えていきます。
...
| 案 | 限界 |
|---|---|
| A：割引率を引数で渡す | 複数ルールが重なると「何%か」を呼び出し元が先に計算して渡すことになる。判断の責任が移動しただけ |
| B：引数を増やす | 呼び出し元が全ルールの条件を知ることになり、問題の構造は変わらない |
| C：新規関数を追加 | 計算の骨格（消費税など）がコピーされ、変更が複数箇所に波及する |

3つの案に共通する根本原因は「割引ルールという概念を1つの単位として独立させられていない」ことです。

**「問題の場所が変わっただけで、問題の構造は変わっていない」**——これが試み①の結論です。
...
| ステップ5 | 割引率を引数で渡す試み①と、ルールをインターフェースとして独立させる試み②を比較し、②を選んだ |
| ステップ6 | テストの独立性・変更の局所性・チームの分担という評価軸で試み①②を比較し、試み②のコストに見合うことを確認した |
