...
まずは、ECサイトの支払金額計算が何を入力として受け取り、どの処理で加工し、何を出力するのかを整理します。ここでは設計の良し悪しを判断せず、現状を事実としてそろえます。
...
- 支払金額は、商品リストだけでは決まらず、会員種別とキャンペーンフラグによって変わる。
- 加工の中心は、小計の合算ではなく、どの割引ルールを選んで適用するかにある。
- 出力は金額だけではなく、未登録顧客や空注文のように計算へ進めない結果も含む。
...
| 事後状態 | 支払金額が表示される。未登録IDや空注文ならエラーとして中断する |
...
| 条件 | 動作 |
|---|---|
| 顧客IDが顧客データに存在しない | エラーメッセージを出力し、処理を中断する |
...
変更後の仕様を、1-1と同じ入力・判定・加工・出力の流れとして図で確認します。1-1の図との差分は、入力に「サマーセールフラグ」が加わることと、割引額を計算したあとに「サマーセール5%を逐次適用する」という加工が挟まることの2点です。
...
フェーズ1の事実をもとに、「どの仕様が変わりやすいか」を見立てます。ここでは現状の責務配置が良いか悪いかは判断しません。それは変更を当てて痛みを見たあと（フェーズ3・4）で考えます。
...
ここからの検討は、次の2つのよくある考え方を軸に進めます。1つ目は、**共通部分に着目して共通化し、抽象化する**ことです。2つ目は、**変わる部分を分離して外から注入し、組み立て側は中身を意識せず決まった流れを回すだけにする**ことです。以下の各案は、この2つの考え方をこの章の課題へ当てたものです。関数切り出しと割引ごとのクラス分けは1つ目の実行です。割引率テーブルは「変わる値を外へ出す」、共通契約と注入は「変わる計算を外から注入し、本体は決まった流れを回すだけにする」という、2つ目の実行です。

ここで比べる案は、次の4種類です。

| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| 関数切り出し | 割引の計算式と選ぶ判定の名前 | どの式がどの割引かが見えやすくなる | 追加のたびに同じクラスを直す点は変わらない |
| 割引率テーブル | 80・90・95という率の数値 | 率の改定がデータ1か所の修正で済む | 適用条件と、率以外の計算方法は値で表せない |
| 割引ごとのクラス分け | 各割引の計算処理 | 計算がクラス単位で独立する | 本体が全クラス名と選択のif文を知ったまま |
| 共通契約と注入 | 割引の計算ロジック全体 | 計算フローから割引選択の分岐が消える | ルール選択と組み立ての変更は残る |

この4案のうち「割引率テーブル」は、割引の違いが率の数値だけに見えることから、「率をデータとして持てばコードを変えずに済むのでは」という、最初に思いつきやすい案です。この案は比較表の中だけで済ませず、ステップ2の中で案Aとして実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。関数切り出しはステップ1で、クラス分けはステップ2の案Bで、共通契約と注入はステップ3〜4で順にコードにします。

### ステップ1：各処理を独立した関数として切り出す（共通構造を発見する）

「if-else が乱立しているなら、まずそれをメソッドに切り出して整理しよう」というのが自然な最初の発想です。これは比較表の「関数切り出し」案にあたります。クラスを新しく作るのはコストがかかる。同じクラスの中で、割引の計算を種類ごとに独立したプライベートメソッドとして分離してみます。
...
ステップ1では関数を種類ごとに独立させ、「共通のシグネチャを持つ関数が並んでいる」という構造が見えてきました。しかし、関数が増えるにつれて1つのクラスの中に処理がどんどん積み重なっていきます。

クラスを増やす前に、比較表の「割引率テーブル」案を確かめます。3つの割引の違いが「80・90・95という率の数値」だけに見えるなら、率をテーブル（マップ）のデータとして持ち、コードではなくデータを直せば済むはずだ、という発想です。

**案A：割引率をテーブル（マップ）のデータとして持つ**

```cpp
// 案A：割引率をデータとして持ち、計算式は1つに共通化する
class PaymentCalculator {
    // 施策名 → 割引後に残る率（%）。率の改定はこの表を直すだけ
    std::map<std::string, int> rateTable = {
        {"Premium",  80},   // プレミアム割引 20%引き
        {"Campaign", 90},   // キャンペーン割引 10%引き
        {"Summer",   95},   // サマーセール割引 5%引き
    };
public:
    int calculate(const Order& order, const std::string& memberType,
                  const CampaignContext& ctx) {
        ...
        // どの率を使うかの判定は、依然としてコード側に残る
        if (memberType == "Premium") {
            total = total * rateTable["Premium"] / 100;
        } else {
            if (ctx.isCampaignActive)
                total = total * rateTable["Campaign"] / 100;
            if (ctx.isSummerSale)
                total = total * rateTable["Summer"] / 100;
        }
        return total;
    }
};
...
- `rateTable` が「施策名 → 割引後に残る率」の対応表です。率の数値がコードの分岐から1か所のデータへ集まりました。
- 計算式は `total * rateTable[...] / 100` の1つだけになり、Regular会員の逐次割引はキャンペーンとサマーセールの率を順に掛けることで表せています。

**案Aの評価：**
割引率の改定（20%引き→25%引きなど）は、この表の数字を1つ直すだけで済みます。逐次割引も率を順に掛けるだけなので、この形のまま表せています。**変わるのが割引率の数値だけだと確認できているなら、この案で止める判断は十分あり得ます。**

**案Aに残る課題：**
フェーズ2のヒアリングで予告されたのは、率の改定だけではありませんでした。1つ目は「一律1000円引きクーポン」です。引き算の割引は「率」の値では表せず、`total * rate / 100` という計算式そのものを変えることになります。2つ目は適用条件です。「Regular会員のみ」「キャンペーンと重なったら逐次適用」といった条件はテーブルの値では表せないため、施策が増えるたびに `calculate()` の `if` を書き足す作業が残ります。この章の割引は、率の数値だけでなく計算のしかたと適用条件まで変わるものが混ざっているため、値の差し替えでは足りず、計算という振る舞いごと差し替えられる形が必要です。

**案B：割引ごとに別のクラスへ分ける**

「割引ロジックが増えてきたなら、計算のしかたごと、それぞれを別のクラスにしよう」という発想が次に来ます。今度は割引の種類ごとにクラスを作ってみます。
...
*   **ステップ2・案A（割引率テーブル）で止めるケース：** 変わるのが割引率の数値だけで、適用条件や計算方法（率か定額か）は当面変わらないと確認できている場合。
*   **ステップ2・案B（具体クラスへの分離）で止めるケース：** ファイルを分けて整理したいが、インターフェース導入のコストをまだかけたくない場合の「中間策」です。
...
| 🟣 フェーズ2：仮説立案 | 割引ルールが変わりやすいと見立て、誰がなぜ変えるかをヒアリングで裏付けた。今回の確定変更と将来リスクを分けて整理した（責務の良し悪しはまだ判断しない） |
...
| 🔴 フェーズ6：対策検討 | 比較表で4案を並べ、割引率テーブル案（案A）を含む段階的進化でそれぞれの限界を確認し、ステップ4（インターフェース化・依存性の注入）まで進化させる決断を下した |
