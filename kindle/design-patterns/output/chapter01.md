...
> アルゴリズムの族を定義し、それぞれをカプセル化して、互いに切り替え可能にする。
> Strategyパターンにより、アルゴリズムを使うクライアントとは独立して、アルゴリズムを変えられるようにする。

### この章での対応関係

| GoFの役割 | この章での実装 |
|---|---|
| Context | `BillingCalculator` |
| Strategy（インターフェース） | `IBillingRule` |
| ConcreteStrategy | `CorporateRule`、`CampaignRule`、`StandardRule` |

`BillingCalculator` はどの `IBillingRule` が渡されるかを知らない。渡された部品を呼ぶだけです。割引ルールの「中身」を知っているのは、各 `ConcreteStrategy` と `Composition Root` だけです。

### 使う基準

- 同じ処理の「やり方」が複数あり、条件に応じて切り替える必要がある
- やり方が今後も増える可能性がある
- 切り替えのロジックを呼び出し元に書きたくない

### 使わない基準

- やり方が1種類で、今後も変わる見込みがない（インターフェースを挟む必要がない）
- 切り替えがなく、ただ処理を関数として分けたいだけ（その場合は普通のメソッド分割で十分）