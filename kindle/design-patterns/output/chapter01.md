...
2回目に注目してください。**Regular会員の同じ注文でキャンペーンを有効にすると、支払金額が10,000円から9,000円へ変わります。** 入力条件が割引判定へ渡り、結果へ反映される基本の動きを最初に確認できます。Premium会員の排他条件は、1-2と1-4の別ケースで確認します。
...
ここで言えるのはそこまでです。責務は設計者が決めるもので、「割引も含めて支払金額を計算する」という今の責務配置は、それ自体が間違いというわけではありません。複数の主体が同じメソッドを変えうることが**実際に痛みになるか**はフェーズ3「問題特定」で変更を当てて確かめ、なぜ辛いのか（＝変わる理由の異なるものが同居している）はフェーズ4「原因分析」で原因として言語化します。2-1は、その検証対象に当たりをつける段階です。
...
したがって2-5の出力は、「割引ルールと計算式は差し替え対象にし、小計の合算から結果表示までの流れは守る」という設計条件です。フェーズ3「問題特定」では変更ID1・変更ID2だけを現在の構造へ適用し、このリスクIDはフェーズ6で採用構造を評価するときに使います。
...
1-3で示した `calculate` メソッドの割引判定ブロックが、キャンペーンのたびに変わる箇所です。フェーズ3「問題特定」でサマーセールを足した後の割引判定を、省略せず全体で示します。
...
フェーズ4「原因分析」で、割引の処理が「どの施策を選ぶか」と「選んだ施策がいくら引くか」という**別々の理由で変わる2つの判断**を、同じ `if` 連鎖へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つの価格計算経路へ再結合することです。この段階では、問題から導いた「ルール差し替え構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。

```mermaid
flowchart TB
    A[現在<br/>一本のif連鎖に<br/>適用条件と計算式が混在] --> B[分離判断<br/>二つの変化軸を別責任へ分け<br/>一つの価格計算経路へ再結合]
    B --> C[課題ID1<br/>施策ごとの対象条件をルールへ<br/>競合時の順序を組み立てへ]
    B --> D[課題ID2<br/>施策ごとの計算式を<br/>共通契約の裏へ＝ルール差し替え構造]
...
この4つが変わっていないことは、フェーズ7の受入・回帰エビデンスで確認します。ただし、**分離と組み立ての判断を終えるたびにも照合します。** 設計の途中で守る範囲を壊していないか、その場で確かめながら進めるという意味です。
...
| フェーズ4の原因 | 原因ID1（選択の流れが条件と競合時の順序を抱える）、原因ID2（計算骨格が式を抱える） | 同じクラスの中で `【残す】` と `【移す】` を分ける |
...
向きと掲載クラスは1-3から変えていません。同じ図に注記と色だけを加え、`PaymentCalculator` のどの責任を残し、どの責任を移すかに着目します。**この図を基準に、分離と組み立てで必要な関係を足していきます。**

クラス図の変更として書くと、次の3操作になります。**どんなクラスを新設し、何という名前にするかは、この後の二つの判断で決めます。** ここで確定しているのは操作の種類だけです。

1. 課題ID1：施策ごとの対象条件をルールへ移し、競合時の順序を組み立て側で明示する。
...
- **契約と具体による分離**：価格計算に何を残し、何を契約の裏の施策へ移すかをセットで決める。
- **生成・注入・実行による組み立て**：施策を誰が生成・所有・登録し、価格計算が契約だけを通してどう実行するかを一本で決める。

先に分離の両側を確定し、その実体が実行されるところまで組み立てます。結論はフェーズ6「分離と組み立てのまとめ（6-1）」で二行に整理します。
...
**ここから決めること：** 施策ごとの対象条件と計算式を共通の選択・計算手順からどう切り離し、何を接続点にして、誰が生成・登録するかを順に決めます。

対象条件と計算式は別々の理由で変わりますが、実行時には「条件で1つ選ぶ → 選んだ規則で計算する」という順で接続します。このため、まず選択側が何を返すかを決め、その結果を計算側の契約へ渡せることを確認します。
...
**この章では、境界をクラス契約で表します。** 割引の `if` 連鎖自体は1か所ですが、変更要求で枝の追加と並べ替えが実際に起き、2-4では新しい割引が毎月増えると確認できています。関数へ切り出すだけでは、その関数の中に同じ条件連鎖と順序依存が残ります。複数の施策へ同じ問いを投げ、条件と式を施策単位で追加できる形が必要なので、共通のクラス契約まで進みます。
...
呼べるのは、施策が増えても今回維持する側だけです。**出て行った側が出て行った側を呼べば、施策が増えるたびに両方を触ることになり、分けた意味が消えます。** たとえば `PremiumDiscount` が `CampaignDiscount` を呼ぶ形にすると、施策を1つ足すたびに既存の施策を触ることになります。

**この章では、契約を骨格の外へ置きます。** 注文を1件ずつ処理していくと、プレミアム会員の注文ではプレミアムの施策が、一般会員のキャンペーン注文では別の施策が使われます。**同じ価格計算が、注文ごとに違う施策を使い分けなければなりません。** だから価格計算とは別の型として `IDiscountRule` を置きます。

**残った側は2つあります。** 施策を選ぶ側と、金額を計算する側です。**課題が2つあるので、骨格も2つに分かれます。**

**課題ID1（選択軸）の骨格。** 契約の確認で「競合方針は登録順で一か所に示す」と決めたので、選ぶ側の手順は2段になります。
...
**Selectorが知るのは「先に登録された一致ルールを返す」という共通手順だけです。** どの施策を先にするかは知りません。新しい施策を足してもSelectorも既存ルールも変えず、組み立て側の一覧へ追加します。

**穴が1つ空きました。** `rules` です。**この並びに何が入り、誰が入れるのかは、まだ決めていません。** ここへ `NoDiscount` や `PremiumDiscount` の名前を書くことはできません。書けば、施策を足すたびにこのクラスを開くことになります。**それを決めるのが生成の検討です。**
...
**2つの軸で、同じ形の穴が空きました。** 選択軸は「施策の並びをどこから得るか」、計算軸は「選ばれた1つをどこから得るか」。**どちらも「実体をどこから得るか」という同じ問いです。** ここが生成の検討で1つにまとまる伏線になります。

**ここで検算します。施策を1つ足したとき、この2つの手順は変わるか。** 変わりません。選ぶ側は並びを1周するだけ、計算する側は渡されたものへ聞くだけです。**穴が空いたままでも、この検算はできます。** 変わってしまうなら割り残しがあるので、契約の確認へ戻って線を引き直します。
...
classDiagram
    ...
    RuleSelector --> IDiscountRule : 条件を聞く
...
契約と、それを呼ぶ2つの骨格が決まりました。契約の確認では契約の裏を1つ覗いて、渡すもの・返すものを確かめました。**ここで残り全部を埋めます。**

**契約の確認で見た `PremiumDiscount` の疑問が、ここで解けます。** 施策が `total` を直接書き換えないのは、書き換えるのが分離で確定した骨格の仕事だからです。契約の裏に置くのは「自分は誰に当てはまるか」「いくら引くか」「何という名前か」の3つだけで、それ以外は書きません。
...
この表の上から順に登録すると、3-1の枝と同じ競合結果になります。順序を隠さず、アプリケーション全体の方針として一か所で読めるようにしました。各具体クラスは他の具体クラスも自分の順位も知らないため、新しいクラスは既存クラスから独立して実装できます。追加時に必要な全体判断は、どの既存条件より前後に置くかを組み立て側で決めることだけです。
...
**分離の検算で穴が2つ空きました。** 選ぶ側の `rules`（施策の並び）と、計算する側の `rule`（選ばれた1つ）です。
...
**まず `rules` から決めます。** 持つ者に必要な条件は、分離の検算で分かっています。

- **具体クラスを全部知っていること。** `PremiumDiscount` を並びへ入れる以上、その名前を知っていなければなりません
- **選ぶ側そのものではないこと。** 分離の検算で確認したとおり、知った時点で分離が壊れます
...
3-1では枝の位置に条件と競合方針が混在していました。変更後は、条件を各ルールへ閉じ、競合方針を `main()` の登録順へ集めます。登録順には意味があるため、組み立て箇所の近くに優先関係を明記します。
...
**なぜ借りるのか。** 施策はメンバー変数を1つも持ちません。具体の確認で確認したとおり、どれも「渡された値を見て答える」だけです。**同じ実体を何回使っても壊れないので、コピーを持つ理由がありません。** そして所有しないので、破棄の責任も生まれません。

**分離の検算で穴だった `rules` が、参照の並びとして埋まりました。** 骨格の手順は1行も変わっていません。
...
**`OrderProcessor` が現れました。** 顧客照合と、選ぶことと計算することをつなぐ入口です。具体の確認で描いた5つの施策クラスはこの図に描いていませんが、変わっていません。


**ここが終わった時点で、実体はすべて存在しています。** 5つの施策、それを借りて持つ選ぶ側、それを使う入口。**まだ起きていないのは1つだけです。骨格の `IDiscountRule` と書かれた場所へ、具体が入ることです。次に、注入として確認します。**

---

##### 注入：実体を契約として骨格へ渡す

**分離の検算で書いた骨格には、`rule.apply(subtotal)` としか書いてありません。** `PremiumDiscount` とは書いていません。それでも実行すればプレミアム会員には20%引きが適用されます。**ここで見るのは、いつ・何によって、その実装が入るのかです。**

**この章では、実装が2回に分けて入ります。**

**1回目は、組み立てのときです。**

**掲載箇所：`main()`** ―― 生成の検討で作った施策を、選ぶ側へ登録する
...
**この章では、登録時と注文ごとの二つの瞬間があります。** 登録は組み立てのとき1回、選択は注文のたびです。
...
**骨格が施策へ届く方法（参照で借りる）は、生成の検討で決めた話です。** 値で持つとコピーが増えますが、**注入はどちらの持ち方でも起きます。**
...
**守る範囲との照合：** **顧客照合**と**結果返却**を保っていることを、いま確認しました。顧客IDの存在確認は3-1のままです。結果返却は、返す項目が金額1つから小計・適用後金額・施策名の3つへ増えましたが、これは要求ID5が求めたもので、**呼び出し側へ計算結果を返すという形は変えていません。**分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**


#### システム全体の最終構造を決める

分離と組み立ての部分図を重ねると、システム全体の最終構造になります。5つの施策が `IDiscountRule` を実装し、`RuleSelector` が登録順に条件を問い、最初に一致した1つを `PaymentCalculator` へ渡します。
...
|---|---|---|---|
| 契約と具体による分離 | `if` 連鎖の条件・式 → `IDiscountRule` と5つの施策 | `matches()`・`apply()`・`name()` の契約と、施策固有の条件・式を対応させる | 選択と計算は具体名を書かず契約だけを呼ぶ |
| 生成・注入・実行による組み立て | 計算クラスが全式を内包 → `main()` が施策を所有し `RuleSelector` へ登録 | 登録時に契約として渡し、注文ごとに選択した1つを計算へ渡す | `process()` から選択→契約→具体の実行までつながる |

公開入口の `process()` は変わりません。組み立てで増えたのは、分離した施策を生成・登録して入口へ渡す部分です。

#### 代表ケースの実行接続

生成した施策が実際に使われるまでを確認します。一般会員がサマーセールとキャンペーンの重なった日に注文したケースを、登録→選択→計算まで一本で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成 | `main()` | 5つの施策を作り、`RuleSelector` を1つ作る | 注入1回目へ |
| 2. 注入登録 | `RuleSelector::add(const IDiscountRule&)` | Premium、複合、単独、割引なしの優先関係が分かる順で参照を登録する | 実行開始へ |
| 3. 実行開始 | `main()` | `processor.process(order, context);` | `OrderProcessor::process()` |
| 4. 骨格選択軸 | `OrderProcessor::process(const Order&, const CampaignContext&)` | 顧客照合で会員種別を引き、`selector.select(...)` を呼ぶ | `RuleSelector::select()` |
| 5. 契約→具体選択軸 | `IDiscountRule::matches()` → 各施策 | 登録順に聞き、最初に条件へ合うものを返す | 骨格の `const IDiscountRule&` へ入る（＝注入2回目） |
| 6. 骨格計算軸 | `PaymentCalculator::calculate(const Order&)` | 小計を求め、`rule.apply(subtotal)` を呼ぶ | `IDiscountRule::apply()` |
| 7. 契約→具体計算軸 | `SummerSaleAndCampaignDiscount::apply(int)` | 10%引き後の金額へさらに5%引きを適用して返す | 戻り値を骨格が結果へ詰める |

**4〜5と6〜7が、別々の軸です。** 選択軸は「どれを使うか」で完結し、計算軸は「渡されたものへ聞く」だけです。**共有しているのは、5が返して6が受け取る1つの参照だけです。**

**注入が2回現れることに注目してください。** 2は組み立てのときに1回、5は注文のたびです。**同じ章の中に「組み立て時に1回」と「呼び出しごと」の両方があります。**


### 6-2：システム全体の契約とデータ配置を確定する

採用システムの契約、生成場所、依存注入を一表で確定します。`PaymentResult` は対策の抽象ではなく、要求ID5の購入結果表示へ小計・支払金額・適用ルール名を渡す結果オブジェクトです。
...
| どこで生成・選択するか | `main()` が生成・所有し、競合方針を登録順で示す。`RuleSelector` は最初の一致を返す | 具体クラスの生成方法と選択条件 |
...
**骨格 計算の安定骨格。** 「商品を順に足して小計を出し、割引後金額を返す」制御順は変わらず、割引式は契約 `apply()` の向こうにあります。
...
**具体 各施策が割引式だけを実装する。**（`apply()` 部分。上の具体で示した各施策が同じ場所に持ちます。）
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を一つのシステムとして、契約と具体による分離、生成・注入・実行による組み立てへ変換した。一意に定まった具象ルールの登録構造は、フェーズ7の実装手順【1】〜【6】でコードへ反映した |
