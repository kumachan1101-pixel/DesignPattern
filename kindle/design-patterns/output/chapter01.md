...
★割引ルールって具体的な内容出さないで。得られることは、割引ルール限定の話ではなく、もっと抽象的な思考プロセスです。例えば、ビジネスロジックとその組み立て部分とかではないのですか？
...
私自身、何度もこの状況で迷いました。★この一文は何を言いたい？架空の話であり、実体験ではないですよね？
...
★いきなり以下の話しないで、まだ思考プロセスを通していないのに、何故先の話をするの？ステップ５でこの話題を取り上げてください。
...
★他にも手札が該当するのでは？単純にクラス分離という案もあるはず。いきなりインタフェースのみに限定にはならないと思います。
...
★手札一覧のどれの話ですか？原因からこの手札が候補にあがっているが、天秤ではじくという話ではないのですか？
### 補足：手札を誤ると何が起きるか
...
**インターフェース抽出** 適用後のテスト
★ライブラリを使ったテストでしょうか？このライブラリの仕様を説明してから、テストの説明に入るようにして下さい。
...
★以下の基準、chapter0ですべて話ありますか？chapter0が不足しているなら修正してください。chapter1以降は、chapter0をベースにした考え方にしてください。chapter1以降で新しい思考は出さないでください。いわば、chapter0が全てのベースであり、chapter1以降は、その実践だけです。
...
★ルールのリストをもって順番に適用していくMultiBillingCalculatorは適用しなかったのですね？前述の説明は何だったのでしょうか。違う方向性にするなら、説明が必要ですよ。
...
★消費税率は分けなくてもよい？結構、消費税って変わっているように思うんですけど。大した計算ではないから、そこまでしていないなら、そのように説明してほしい
...
★GOFの説明をするなら、一番最初に登場したときに説明してください。
## パターン解説：Strategyパターン
...
★最後に章の締めは何かありませんか。しまっていません。要するに、こういう時にはこんな考え方をして解決しましょうという内容で締まると、まとめになってよいかと思います。