...
---
> **📌 問題（確定）**
> 割引ルールが1つ変わるたびに、`PaymentCalculator` の条件分岐・`Order` クラスのフラグ・`CartPreviewService` の重複ロジックの3か所を必ず同時に修正しなければならない。
---
...
---
> **📌 原因（確定）**
> 「変わらない合算ロジック」と「キャンペーンのたびに変わる割引条件」が、同じ `calculate()` メソッドの中に物理的に混在しているから。
---
...
---
> **📌 課題（確定）**
> 割引ロジック（生産者）を計算の骨格から切り離し、接続点に流れる `int` 型の金額だけを安定させ、生産者を外から差し替えられる設計にする。
---
...
| | 内容 |
|---|---|
| **問題** | 割引ルールが1つ変わるたびに、`PaymentCalculator` の条件分岐・`Order` クラスのフラグ・`CartPreviewService` の重複ロジックの3か所を同時に修正しなければならない |
| **原因** | 「変わらない合算ロジック」と「キャンペーンのたびに変わる割引条件」が、同じ `calculate()` メソッドの中に物理的に混在しているから |
| **課題** | 割引ロジック（生産者）を計算の骨格から切り離し、接続点の `int` 型の金額だけを安定させ、生産者を外から差し替えられる設計にする |
| **解決策** | Strategy パターン：`IDiscountRule` インターフェース経由で割引ロジックを外から注入する（ステップ4） |
