...
フェーズ6は、フェーズ5で定めた課題——**小計計算の骨格を守りつつ、割引ルールごとの判定・計算式を差し替えられる接続点を作る**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを変換して探します。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる
...
理想グラフは完成構造を決める図ではありません。「新しい割引」という同じ要求を、どこまでに閉じ込めたいかを先に決めた図です。

#### 理想グラフとの差から課題カードを導く

| 課題ID | 現在→理想の差 | 切る境界・守る契約 | 完了条件 |
|---|---|---|---|
| P1 | PaymentCalculator の条件分岐と利用側の再テストを、追加ルールと組み立ての変更へ縮める | calculate() 内の割引選択を切る。Order→PaymentResult、小計、整数演算の適用順は守る | 新割引で既存の PaymentCalculator・OrderProcessor・既存ルールを修正しない |

P1の着目箇所と完了条件は、理想グラフで消した C1→C2 の経路から導いています。

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 割引式を関数へ出す | 計算式に名前が付き、分岐との差を読める | 同じ apply(int) 契約へそろえ、ルールの登録・組み立てへ選択を移す |

候補はP1を作る根拠ではなく、P1の完了条件を満たすための手段です。ここからP1の関連コードだけをおさらいします。
