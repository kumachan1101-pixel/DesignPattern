...
次に、初期化を行うためのコンストラクタの引数にも、この2つの真偽値（boolean）を追加必要があります。
...
最初の頃、トッピングが「ミルク」と「ホイップ」だけだった時代は、一つのクラスを見ればすべての処理が追えるという大きなメリットがありました。当時の担当者が、素早く機能を提供するためにこの形を選んだのは、非常に合理的だったと思います。しかし、トッピングの種類が増えるにつれて、一つのクラスが「知りすぎている」状態になってしまったのではないでしょうか。私自身、現場で「とりあえずフラグを足しておこう」と安易に判断して後悔したことが何度もあります。
...
現在のシステムがどのような接続形態になっているのかを診断してみましょう。

今の `CustomDrink` クラスと各トッピングの関係を整理すると、クラスの中に `hasMilk`・`hasWhip`・`hasSyrup` というトッピングを名指しするフラグが直接並んでいます。抽象化が何もなく、`CustomDrink` が具体的なトッピング名を直接知っている状態です。この「抽象化のない直接的な知識の抱え込み」を言語化するための言葉が、**「具体×直接」** という接続形態の名前です。

現在の `CustomDrink` クラスは、ミルクやホイップといったトッピングの存在を、真偽値（boolean）のメンバ変数という形で具体的に直接知っています。これをケーブルの比喩で言えば、Lightningケーブルで直差しの状態（具体×直接）だと言えます。新しいトッピング（差し替えたい機器）が来るたびに、本体（`CustomDrink`）側に専用の差込口を追加必要が生じます構造です。
...
---
> **📌 原因（確定）**
> `CustomDrink` が各トッピングの価格・名前という具体的な条件を直接知っている（具体×直接）状態になっている。ヒアリングで確認された「毎月のトッピング入れ替わり」や「ホイップのダブル追加」が想定される今の変更頻度では、基本ドリンクの骨格と変わり続けるトッピングの知識が同じ場所にある接続コストが合わない。
---

（原因は「基本ドリンクとトッピングが同じ場所に混在している」具体×直接の接続形態にあると確認できました。次のフェーズ5では、切り離すべき境界で何が流れているかを値・型のレベルで見ていきます。）
...
| 接続点 | 接続するデータ | 変わるもの |
|---|---|---|
| トッピング → `getPrice()` の骨格 | `int` 型の追加価格 | トッピングの種類・組み合わせ（生産者） |
| トッピング → `getDescription()` の骨格 | `string` 型の表示名 | トッピングの種類・組み合わせ（生産者） |

### 何が変わり、何が変わらないか

- **変わるもの**：トッピングの種類・組み合わせ（生産者）。新しいトッピングが増えるたびに `bool` フラグと `if` 分岐が増える。
- **変わらないもの**：`getPrice()` が `int` を返し、`getDescription()` が `string` を返すという契約。呼び出し元（`MobileApp`）が使うインターフェースは変わらない。

呼び出し元は「価格と説明を取得できれば十分」なので安定しています。問題は「どのトッピングがいくらで何という名前か」という**生産者の組み合わせ**が膨れ続けること。

**具体×直接のままでよい場面**：トッピングが今後増えない確証があれば、現状の `bool` フラグ（具体×直接）で十分です。接続形態の選択は「**生産者が変わるかどうか**」で決まります。今回はトッピング追加リスクがヒアリングで確認済みなので、次のフェーズで生産者を差し替え可能にする設計を検討します。

---
> **📌 課題（確定）**
> 切り離すべき境界は「基本ドリンクの価格を保持する骨格」と「各トッピングが何円で何という名前か」の間にある。接続点を流れるのは `int` 型の金額と `string` 型の名前という安定した型だが、その生産者（どのトッピングがいくらで何という名前か）が毎月変わり続ける。トッピングの生産者を差し替え可能な形で骨格から切り離すことが、この章の課題だ。
---
...
フェーズ5で「変わるのはトッピングの種類・組み合わせ（生産者）であり、`getPrice()` / `getDescription()` の戻り値型は安定している」ことが分かりました。ここでは、その生産者をどのように差し替え可能にするかを段階的に検討します。それぞれの段階（ステップ）でどこまで痛みが解消されるかを確認し、今回の要件において「どのステップで止めるべきか」を決断します。
...
ここで自然と「フラグの増加を止めるには？」という問いが浮かぶ。私自身もこの段階でしばらく悩みました。まずは関数分割を試みましたが、やはり呼び出し側の修正やクラス内部の肥大化は防げませんでした。フラグを増やし続けるのは無理だから、「これはもう、役割ごとにクラスを分けるしかない」という発想に至ったのです。次に思いついたのが、その手始めとして「組み合わせ全体をクラスで表現する」というアプローチでした。
...
| **課題** | トッピングという生産者（価格・名前の知識）を骨格から切り離し、差し替え・追加が骨格に影響しない形で独立させる必要がある |
...
| 🟡 フェーズ5：課題定義 | 接続点で流れるのは int/string（安定）、変わるのはトッピングの種類・組み合わせ（生産者）であることを特定した |
...
| 2. 接続形態の診断 | フェーズ4で「具体×直接」の状態を診断し、ケーブル比喩で接続形態を言語化した |
...
**得られること1**（変動箇所の識別）：フェーズ1とフェーズ2を通じて、「トッピングの種類・価格・表示名」という機能の組み合わせ自体が変化し続けるものであることを確認しました。「変わる機能」と「変わらない機能」を区別する問いが、変動箇所を見抜く目を育てるという視点が得られたはずです。

**得られること2**（変更の痛みの発生源の判断）：フェーズ4で、ミルクやホイップという具体的なトッピング名をフラグとして `CustomDrink` に直書きしている状態を「具体×直接」の接続と診断しました。この接続形態が、トッピングが追加されるたびに既存コードと呼び出し側を壊すリスク（痛みの発生源）になっていると判断できるようになります。

**得られること3**（接続の形の効果を説明する）：フェーズ6と7で、「インターフェースで全層を統一する」形へと接続の形を変えることで、中心となる既存クラスを変更せずに新しいトッピングを追加できるようになること（変更の局所化）を学びました。変更シナリオ表がその証拠として残っています。

**得られること4**（同じインターフェースで重ねる視点）：フェーズ6のステップ5で確認した通り、Decoratorパターンの核心は「トッピングをインターフェース化する」だけでなく「基本ドリンクとトッピングを同じインターフェースで扱う」ことにあります。これにより、トッピング自体が別のドリンクを包む仲介役になれ、「追加するたびに呼び出し側も変える必要が生じます」という痛みから解放されます。
