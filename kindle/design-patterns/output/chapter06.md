...
この変更シミュレーションを通じて、現場のエンジニアが直面する具体的な辛さが3つ見えてきました。
...
---
> **📌 問題（確定）**
> 変更ID1の2種類を追加するために、`CustomDrink`のメンバ変数・コンストラクタ・`getPrice()`・`getDescription()`と呼び出し側を連動して修正した。変更ID2の入力順と変更ID3の販売可否も、同じクラスと生成処理へ足すことになる。フラグが増えるほど修正範囲が広がる形が、変更ID1の試行で確認できた。
---
...
| 問題ID3 | 販売停止・表示順を足すと、価格計算以外の条件も `CustomDrink` へ増える（変更ID1の試行から見込まれる痛み） | 変更ID2・変更ID3 |
