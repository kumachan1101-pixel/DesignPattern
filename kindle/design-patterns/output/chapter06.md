...
**組み合わせの種類が増えるたびに、条件分岐やクラスの数が際限なく膨れ上がる。こういう問題は、「変わらない基本の処理」と「後から付け加わる処理」が同じ場所に混在しているシステムで起きている。**
...
```mermaid
flowchart LR
    I["入力<br/>基本ドリンク、選択されたトッピング"]
    P["加工<br/>注文名を組み立てる、追加料金を合算する"]
    O["出力<br/>表示用の注文名称、合計金額"]
...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 基本ドリンク、選択されたトッピング | `Drink`、`Milk` などのトッピング指定 | 動作例テーブル、`main()`、変更要求 |
| 注文名の組み立て、追加料金の合算 | 名前と価格を積み上げる処理 | 現状コードの観察、組み合わせ追加時の変更シミュレーション |
| 表示用の注文名称、合計金額 | 実行結果の注文名と金額 | 動作例テーブル、実行結果、変更シナリオ表 |
...
コードを読む前に、このシステムがどんな注文に対してどんな表示と金額を返すかを確認します。ここでは、現状コードで扱える Milk・Syrup・Whip の組み合わせだけを基準にします。

| 注文内容 | 注文名称 | 合計金額 |
| --- | --- | --- |
| ホットコーヒーのみ | `ホットコーヒー` | 400円 |
| ホットコーヒー + ミルク | `ホットコーヒー + Milk` | 450円（400 + 50） |
| ホットコーヒー + ミルク + シロップ | `ホットコーヒー + Milk + Syrup` | 480円（400 + 50 + 30） |
| ホットコーヒー + ミルク + ホイップ | `ホットコーヒー + Milk + Whip` | 520円（400 + 50 + 70） |
| ホットコーヒー + シロップ + ホイップ | `ホットコーヒー + Whip + Syrup` | 500円（400 + 70 + 30） |
| 未登録メニューID | エラー表示 | 注文を作成しない |

この表がフェーズ1で把握する現状の動作です。ここではまだ「なぜこの構造が問題なのか」までは判断しません。読者が確認すべきことは、注文内容から注文名称と合計金額を予測できること、そしてその計算をどのクラスが担当しているかを追えることです。
