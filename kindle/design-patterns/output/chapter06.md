...
まだクラスの中身は見ません。この段階でつかんでほしいのは「基本とトッピングを同じ契約にそろえ、トッピングは内側を包んで自分ぶんだけを足す」という筋だけです（この章の接続課題は一つ＝課題ID1で、基本の処理と重ねる追加機能の境界です）。「どのクラスが生成し、どの契約で実行するか」という具体の結論は、この後の課題ID1で決めていきます。決めた結論をまとめて振り返る表は、フェーズ6の末尾（6-4 設計トレース）に置きます。ここでは先に結論表を出しません。
