...
|---|---|---|---|
| F1 | トッピングの種類の増減 | 毎月のキャンペーンごと | 商品企画部 佐藤マネージャーから直接確認 |
| F2 | トッピングの価格改定 | 年に数回（原材料費等による） | 商品企画部 佐藤マネージャーから直接確認 |
| F3 | 同じトッピングの複数回追加（ダブル、トリプル等） | 将来的な機能拡張時 | 商品企画部 佐藤マネージャーからの要望 |
| F4 | サイズや前段価格で変わる追加料 | 次回サイズ展開時 | 大サイズでは追加量と料金が変わり、固定額の足し算だけでは表せない |
| F5 | 販売停止・在庫切れの扱い | 店舗在庫やキャンペーン終了時 | 商品企画部・店舗オペレーション部の運用で変わる |
| F6 | 表示名の順序ルール | 店舗オペレーション変更時 | スタッフの作業順に合わせる必要がある |
...
2-4のF-IDを、トッピングごとに変えられるようにする部分と、注文・価格計算の安定側へ分けます。「はい」は、フェーズ6で**種類・価格・重ね方が変わっても、基本ドリンクと注文の入口へ分岐を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：トッピングの種類の増減 | はい | トッピング固有の名前と追加責任 | 基本ドリンク、注文入力、価格と表示名の返却 |
| F2：トッピングの価格改定 | はい | トッピングIDごとの価格 | 基本価格の取得、合計価格の返却 |
| F3：同じトッピングの複数回追加（ダブル、トリプル等） | はい | 同じ部品を重ねる回数 | 一要素ずつ価格と表示名を加える規則 |
| F4：サイズや前段価格で変わる追加料 | はい | 注文文脈を使う追加料の計算 | 基本ドリンクとトッピングを同じ入口で扱う契約 |
| F5：販売停止・在庫切れの扱い | はい | トッピングごとの販売可否 | 注文生成前に可否を確認し、不可なら生成しない流れ |
| F6：表示名の順序ルール | はい | トッピングを重ねる順序 | 各要素が自分の表示名を一つ加える規則 |

したがって2-5の出力は、「トッピング固有の値・可否・重ね方は変えられるようにし、基本ドリンクと注文結果の契約は守る」という設計条件です。フェーズ3ではR1〜R3だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
2つ目は、R1のMatcha・Choco追加だけで呼び出し側まで修正が広がったことです。`CustomDrink`のメンバ、コンストラクタ、`getPrice()`、`getDescription()`を変更し、生成箇所にも二つの真偽値引数を追加しました。

フェーズ3で変更を試みた際に生じた痛みが確認できました。次のフェーズ4では、なぜこのような痛みが生じるのか、その根本的な原因をコードの構造という観点から言語化していきます。

---
> **📌 問題（確定）**
> R1の2種類を追加するために、`CustomDrink`のメンバ変数・コンストラクタ・`getPrice()`・`getDescription()`と呼び出し側を連動して修正した。R2の入力順とR3の販売可否も同じクラスと生成処理へ追加され、今回の三要求だけで修正範囲が広がった。
---

（R1〜R3を適用すると複数箇所が連動して変わる問題を確認できました。次のフェーズ4では、なぜこの連鎖が起きるのかを構造的に分析します。）
...
`CustomDrink` が抱えるトッピングの知識に注目すると、三つの観察点が現れます。これらは別々の課題ではなく、「トッピングを注文へ加える」という一つの接続点を構成します。

- **観察点A**：コンストラクタ引数 ―― トッピングの有無を `bool` フラグで表現。新しいトッピングが増えるたびに引数が増える
- **観察点B**：`getPrice()` / `getDescription()` の戻り値 ―― `int` 型の金額と `string` 型の説明を返す
- **観察点C**：注文組み立て前の販売可否 ―― 販売停止・在庫切れのトッピングを、ドリンクへ組み立てる前に拒否する

この章で注目したい点があります。トッピングは1つとは限りません。「コーヒーにミルクを追加し、さらにホイップを追加する」といった具合に、機能が**連鎖**していきます。つまりトッピング同士を順に重ねられる接続点が必要です。

接続点の両側をコードで見ると、利用側は価格と説明だけを要求しますが、接続先の `CustomDrink` はトッピングの種類・有無・加算順をすべて知っています。

```cpp
CustomDrink drink(item, true, true, false);
int total = drink.getPrice();
string label = drink.getDescription();
...
public:
    ...
    string getDescription() const {
        ...
        if (hasMilk) text += " + ミルク";
        if (hasWhip) text += " + ホイップ";
        if (hasSyrup) text += " + シロップ";
        return text;
    }
};
```

呼び出し元から渡る基本商品と選択情報、接続先が行う価格・説明の加工、呼び出し元へ返る `int` と `string` を一続きで見ます。販売可否は、この生成より前に注文組み立て側が確認する接続です。

| 課題ID・接続点 | 接続するデータ | 変わる側 | 守る側 |
|---|---|---|---|
| P1：トッピング ↔ 注文の価格・説明 | トッピングID、選択順、販売状態、`int`価格、`string`説明 | 種類・価格・販売可否・価格計算・説明の組み立て方 | 基本ドリンクと、注文が合計価格・説明を返す契約 |
...
この章の接続点はP1です。価格・表示名・販売可否は同じトッピングの変更で一緒に変わります。価格や説明が単なるデータで済むか、内側のドリンクを参照する振る舞いまで変わるかによって完成構造が分岐するため、フェーズ6で二つの完成案を比較します。
...
**着目する共通点：** 現状コードはすでに共通点を持っています。どのトッピング組合せでも、`getPrice()→int` と `getDescription()→string` という同じ契約で価格と名称を返す形にそろっています（契約の共通点）。現場では、この呼び口すらそろっていないことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、利用側の呼び出しを変えず、変わる「トッピングの価格・名称」を同じ契約でそろえた部品へ分ければ済みます。フェーズ6では、この共通契約と課題を組み合わせて完成構造を決めます。
...
P1をすべて解く完成案は二つあります。ここでは手段ではなく、トッピングが「データだけ」か「内側のドリンクへ作用する振る舞い」かで比較します。

| 完成構造 | 向く目的 | P1への効果 | コスト・判断 |
|---|---|---|---|
...
リスト案をコードにすると、次のようになります。`ToppingSpec` が名前と固定額だけを持つ限り、これでP1を十分に解けます。
...
|---|---|---|---|
| F1：トッピングの種類の増減 | 同じIDrink契約のDecoratorとして種類を追加し、組み立てをOrderAssemblerへ集める | 新Decorator、ToppingCatalog、OrderAssembler | 基本ドリンクと注文実行を守れる。将来種類は完成コードへ追加しない |
| F2：トッピングの価格改定 | 価格の正本をToppingCatalogへ置き、Decoratorへ価格を埋め込まない | ToppingCatalogの登録値 | 局所化できる。将来価格は完成コードへ追加しない |
| F3：同じトッピングの複数回追加（ダブル、トリプル等） | 順序付きトッピング列を同じDecoratorの連結へ変換する | 注文入力の個数規則、OrderAssembler | 連結構造は再利用できる。数量仕様は未確定なので完成コードへ追加しない |
| F4：サイズや前段価格で変わる追加料 | 固定価格のpriceOf()では不足するため、価格計算の入力契約を見直す変更先を限定する | ToppingCatalog、ToppingWrapper、OrderAssembler | 一部局所化。価格ポリシーは確定後に設計し、完成コードへ追加しない |
| F5：販売停止・在庫切れの扱い | 利用可否の正本をカタログ側に置き、組み立て前に検証する | ToppingCatalog、OrderAssembler | 変更先は予測できる。在庫機能は完成コードへ追加しない |
| F6：表示名の順序ルール | 適用順と表示順が異なる場合は表示組み立ての責任を分ける | OrderAssemblerまたは新しい表示境界 | 現構造だけでは完全に局所化しない。表示規則は確定後に設計し、完成コードへ追加しない |

F4・F6のように現在の構造だけでは吸収し切れないリスクも明記します。将来リスクを「対応済み」と見せず、変更先と残る設計課題を予測することが目的です。
...
|---|---|---|
| R1：Matcha（60円）とChoco（40円）を追加する | `CustomDrink`のフラグ・価格分岐・表示分岐を種類ごとに修正 | `Matcha`と`Choco`を組み立てへ登録し、既存と同じ方法で名称と価格へ加算 |
| R2：利用者が指定した順にトッピングを重ねる | `if`文の固定順を入力順へ作り直す | 順序付き入力どおりに部品を包み、表示名と合計価格が同じ順序で得られることを確認 |
| R3：販売停止・在庫切れのトッピングを注文前に拒否する | 呼び出し側または`CustomDrink`へ個別判定を追加 | `ToppingCatalog`で組み立て前に確認し、販売不可なら注文を生成しないことを確認 |
...
| | 内容 |
|---|---|
| **問題** | R1〜R3を適用すると、`CustomDrink`のメンバ変数・コンストラクタ・価格計算・名称生成と呼び出し側を連動して修正する必要があった |
