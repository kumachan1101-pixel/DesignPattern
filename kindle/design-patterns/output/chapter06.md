...
public:
    ...
    string getDescription() const {
        ...
        for (int seq = 1; seq <= 5; ++seq) {
            if (hasMilk   && milkSeq   == seq) desc += " + Milk";
            if (hasWhip   && whipSeq   == seq) desc += " + Whip";
            if (hasSyrup  && syrupSeq  == seq) desc += " + Syrup";
            if (hasMatcha && matchaSeq == seq) desc += " + Matcha";
            if (hasChoco  && chocoSeq  == seq) desc += " + Choco";
        }
        return desc;
    }
};
...
| 触る箇所 | 変更ID1だけのとき | 変更ID1〜変更ID3を当てたとき |
|---|---|---|
| `CustomDrink` のメンバ | `bool hasX` の1つ | `bool hasX` と `int xSeq` の2つ |
| コンストラクタ引数 | `bool x` の1つ | `bool x` と `int xS` の2つ |
| `getPrice()` | `if` を1つ | `if` を1つ |
| `getDescription()` | `if` を1つ | 走査の中の `if` を1つ |
| `ToppingSale` | なし | フィールドを1つ |
| `canOrder()` | なし | `if` を1つ |
| 呼び出し側の実引数 | 1つ | 2つ |

トッピングを1種類足すときに触る箇所は、変更ID1だけなら5か所、3件すべてを当てはめると9か所になります。増えた4か所は価格計算とは無関係で、順番の管理と販売可否という別の理由で増えています。
...
```mermaid
graph LR
    T1["変更ID1：Matcha・Choco追加"] -->|"フラグ・価格if・表示if"| A["CustomDrink"]
    T2["変更ID2：入力順で重ねる"] -->|"順番の変数・表示の走査"| A
    T3["変更ID3：販売可否で拒否"] -->|"置き場所が無い"| C["canOrder / ToppingSale"]
    A -->|"コンストラクタ引数が7→12個"| B["MobileApp"]
    C -->|"呼び出し忘れると素通り"| B
```

3件の変更要求がすべて `CustomDrink` か、その呼び出し側へ集まっています。変更ID1と変更ID2は同じクラスの中を変更させ、コンストラクタ引数の増加を通じてモバイルアプリ側へ飛び火します。変更ID3は `CustomDrink` の中に置き場所が無く、クラスの外の `canOrder()` として呼び出し側の責任になりました。変わる理由が3つとも別なのに、影響先が同じ場所へ重なっています。
...
変更ID1から変更ID3までを順に当てはめた結果、現場のエンジニアが直面する具体的な辛さが4つ見えてきました。
...
3つ目は、変更ID2で「選んだかどうか」と「何番目に選んだか」という二重の情報を持つことになった辛さです。フラグは順番を持てないので、トッピングごとに選択順の番号を足しました。コンストラクタ引数は7個から12個へ増え、`hasMilk` と `milkSeq` の片方だけを間違えても、価格には加算されるのに名称へ出ないという食い違いが作れてしまいます。表示順という変わる理由が、価格計算と同じクラスへ入りました。

4つ目は、変更ID3の販売可否をクラスの中へ置けなかった辛さです。`CustomDrink` は生成時に全トッピングが決まっている前提なので、「注文を作る前に拒否する」判定の置き場所がありません。結果として `ToppingSale` と `canOrder()` をクラスの外へ作り、業務判断が注文を組み立てる呼び出し側へ移りました。`canOrder()` を呼ばない経路があれば、販売停止のトッピングがそのまま注文になります。

3件を当てはめた後は、トッピングを1種類足すたびに触る箇所が5か所から9か所へ増えました。増えた4か所は価格計算とは無関係で、順番の管理と販売可否という別の理由で増えています。

フェーズ3で変更を試みた際に生じた痛みが確認できました。次のフェーズ4では、なぜこのような痛みが生じるのか、その根本的な原因をコードの構造という観点から言語化していきます。

---
> **📌 問題（確定）**
> 変更ID1の2種類を追加するために、`CustomDrink`のメンバ変数・コンストラクタ・`getPrice()`・`getDescription()`と呼び出し側を連動して修正した。変更ID2の入力順は同じクラスへ選択順の変数を足させ、コンストラクタ引数を7個から12個へ増やした。変更ID3の販売可否は`CustomDrink`に置き場所が無く、`ToppingSale`と`canOrder()`としてクラスの外へ出て呼び出し側の責任になった。トッピング1種類あたりに触る箇所は5か所から9か所へ増え、変わる理由が3つとも同じクラスと生成処理へ集まる形が、3件すべての試行で確認できた。
---
...
|---|---|---|
| 問題ID1 | トッピング種類追加で、メンバ・コンストラクタ・`getPrice()`・`getDescription()`と呼び出し側まで連動修正した | 変更ID1 |
| 問題ID2 | 追加のたびにクラス内の複数の `if` 文を探して直し、足し忘れが価格不整合へ直結する | 変更ID1 |
| 問題ID3 | 表示順を足すため選択順の変数を並べ、コンストラクタ引数が7個から12個へ増えた。選択の有無と順番が二重管理になり、片方だけ食い違うと価格と名称がずれる | 変更ID2 |
| 問題ID4 | 販売可否を`CustomDrink`へ置けず、`ToppingSale`と`canOrder()`がクラスの外へ出て、業務判断が呼び出し側の責任になった | 変更ID3 |
...
フェーズ3では、変更箇所が散らばって見落としやすいこと、呼び出し側が壊れること、選択の有無と順番が二重管理になること、販売可否の置き場所がクラスに無いことの4つの痛みを確認しました。この痛みがなぜ発生するのか、コードを注意深く観察することで根源が見えてきます。
...
| 表示順を持たせるとコンストラクタ引数が7個から12個へ増え、選択の有無と順番が二重管理になります | 表示順という別の変わる理由が、価格計算と同じクラスへ入っているから |
| 販売可否を `CustomDrink` へ置けず、`canOrder()` として呼び出し側の責任になりました | 生成時に全部そろっている前提の構造なので、注文を作る前の判定を受け取れないから |
...
|---|---|---|
| 原因ID1 | `CustomDrink` が各トッピングの名前・価格・有無・表示順・組み合わせ方まで持ち、基本ドリンクの骨格とトッピングの知識が同居している | 問題ID1・問題ID2・問題ID3 |
| 原因ID2 | 販売可否の確認と入力順の組み立てを受け取る場所が契約に無く、利用側が種類別の組立条件を知っている | 問題ID4 |
...
|---|---|---|---|
| 原因ID1：一つの注文クラスが全トッピングの有無・価格・説明順を分岐する | 種類・順序・販売状態の変更が同じクラスへ集中する | トッピングごとの価格・説明を独立した部品へ分離する | 各トッピングは独立して追加・停止される |
| 原因ID2：利用側が種類別の組立条件を知る | 順序や販売状態変更で利用側の分岐も変わる | 順序付き入力から部品を組み立てる責任を利用側から外す | 利用と組み立ては別の責任だから |
...
**【課題ID1の原因】** 問題ID1・問題ID2・問題ID3（追加のたび複数箇所と呼び出し側が連動）＝原因ID1（基本骨格とトッピングの知識が `CustomDrink` に同居）と、問題ID4（販売可否が呼び出し側の責任になった）＝原因ID2（販売可否と入力順の組み立てを受け取る場所が契約に無い）。この2つの原因を分離対象にします。
