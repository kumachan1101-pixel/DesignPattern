...
変更ID1〜変更ID3は同じ現状構造へ順に加わります。3件を別々に見ると互いの影響が消えてしまうので、3件を重ねた一つのコードで確認します。フェーズ1の `MenuDatabase` による基本ドリンクIDの存在確認は維持し、ここでは変更が集中する `CustomDrink` と呼び出し側を実行可能な最小構成にしています。抜粋していない周辺は現状のままです。
...
**掲載箇所：`CustomDrink` の全文（`MenuDatabase` は1-4のまま）** ―― フラグ5つをやめ、選択順の`vector`へ置き換えた形です。
...
続いて `main` です。販売可否の確認は、注文を組み立てる側に置くことになります。

**掲載箇所：`main()`** ―― 3件の変更要求を当てた注文と、最後に痛みを1つ確かめる注文です。
...
種類追加、入力順、販売不可の3ケースと、最後に種類名の打ち間違いを通します。見るのは、変更ID1〜3を同じ構造へ重ねたとき、何が解けて、どの痛みが残るかです。
...
新しい注文は正しく動き、期待される出力（ホットコーヒー 400円 + Milk 50円 + Matcha 60円 + Choco 40円 = 550円）が得られます。3件の変更要求を当てた結果を、1件ずつ見ていきます。
...
**この変更は、痛みを減らしています。** フェーズ3で当てた3件のうち、素直な作りの変更で解けるものは解ける、というのは大事な事実です。ここで残るものだけが、構造を変えないと解けない痛みです。

**変更ID1（Matcha・Chocoの2種類を追加する）を当てはめる**

種類が増えると、`getPrice()` の中の種類名で分岐する `if` の連鎖へ、1種類につき1本ずつ足すことになります。価格の知識は依然として `CustomDrink` の中の1か所に集まっていて、種類を足すたびにこのクラスを開きます。
★以下は誰向けのメッセージ？
...
`CustomDrink` は、積まれたトッピングをそのまま受け取るだけの構造です。「注文へ重ねてよいか」を判定する場所がクラスの中にないため、`ToppingSale` を外に作り、`main()` 側で1件ずつ確認してから `addTopping()` を呼ぶ形になりました。Chocoを在庫切れにした実行結果では拒否できます。
...
|---|---|---|---|
| ① 契約 | `CustomDrink` がフラグで全トッピングを持つ → `IDrink::getPrice()` / `getDescription()` | 基本ドリンクとトッピングを同じ操作で呼べるようにする | ③のoverride |
| ② 骨格 | `getPrice()` の `if` 列挙 → `ToppingWrapper::getPrice()` の「内側へ委譲して自分ぶんを足す」形 | トッピングが増えても変えない委譲の形を固定する | ①の `getPrice()` |
| ③ 具体 | フラグ分岐に埋もれた価格・表示名 → `Coffee` と各トッピング部品 | 基本／部品それぞれの差分だけを実装へ閉じる | ①経由で外側の②へ戻る |
| ④ 生成 | 組み合わせごとのクラスや分岐 → `OrderAssembler::wrapOne()` の `new` | 入力順に内側から包んで連鎖を作る（所有は外側の部品） | ⑤の内側参照 |
| ⑤ 注入 | 各部品が価格を直書き → 生成時の `IDrink*` と `ToppingCatalog*` | 内側の契約と価格カタログを外から渡す | ⑥が呼ぶ `assemble()` |
| ⑥ 利用開始 | 呼び出し側がフラグを並べる → `assembler.assemble(request);` | ④⑤で組み立てた同じ連鎖を使い、公開操作を1回呼ぶ | ②の `getPrice()` |
...
**② 安定骨格：`ToppingWrapper`が内側への委譲を固定する。** トッピングは内側の`IDrink`を1つ所有して包み、価格も表示名も「まず内側へ委譲し、自分ぶんを1つ足す」形に固定します。この委譲の形は、トッピングが何種類増えても変わりません。包む順で任意の組み合わせを表せるため、組み合わせ別クラスは要りません。最外の部品を破棄すると、デストラクタが内側を連鎖して破棄します。（③の各トッピングは、この基底を継いで自分の差分だけを実装します。）
...
`DRINK001` に Milk → Matcha を重ねる1件を、④から③まで実コードで追います。設計を説明する順は①から⑥ですが、実行時の呼出順は④→⑤→⑥→②→①→③です。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. ④生成 | `OrderAssembler::wrapOne()` | `return new Milk(inner, &catalog);` で内側を包む部品を作る | ⑤へ |
| 2. ⑤注入 | `OrderAssembler::wrapOne()` | 生成時に内側の `IDrink*` とカタログ参照を渡す | ⑥へ |
...
| 6. ③具体 | `Coffee::getPrice() const` | 基本価格を返して連鎖を折り返す | 外側の②へ戻り、順に加算される |

④で生成した部品の連鎖と、⑤で渡した内側参照と、⑥の呼び出しから②が委譲する相手は同じ連鎖です。カタログは⑤で配置した1か所を全部品が共有します。
