...
コードが実際に「知っていること」を一行ずつ照合し、その知識が**どの業務機能に属するのか**を観察します。これは「クラスが責任を果たしているかどうか」ではなく、「同じクラスの中に、異なる業務機能の知識が混在していないか」を可視化するための確認です。
...
責任チェック表でクラスの責任が整理できました。次に、コードの各行が「どの業務機能に属する知識か」を確認することで、混在している責任をさらに細かく特定します。判断基準は、「このクラスの担当者（ここではドリンク開発チーム）とは別の業務機能が変更を決定するかどうか」です。別の業務機能が決定するなら、それは「責任外（❌）」と判断します。

`CustomDrink.getPrice()` と `CustomDrink.getDescription()` の各行を見ると：

| **コードの行** | **持っている知識** | **業務機能** | **責任内か** |
|---|---|---|---|
