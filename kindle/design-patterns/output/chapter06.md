...
**得られること2**（変更の痛みの発生源の判断）：フェーズ4で、トッピング名をフラグとして`CustomDrink`へ直書きし、基本ドリンク側が種類と組み合わせを知りすぎていることを確認しました。

**得られること3**（接続点をそろえる効果）：フェーズ6と7で、基本ドリンクとトッピングが同じ「価格と説明を返す」契約を持つことで、トッピングを重ねて組み合わせられることを学びました。
