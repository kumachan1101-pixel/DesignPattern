...
★クラス図の書き方として問題ないか調べたい。以下のような内容は、クラス同士のデータのやり取りですが、クラス図に記載してルールとして問題ないか、見やすいから問題ないならこのままにしたい、クラス図は、クラスがどのように設定されているか誰がどのクラスを持つのかを示す配置図のような位置づけと思っていたので、データのやり取りは別の図で表現した方が良いのでは？と思いました。
...
★完了条件って何？やはり意味が分からない。対策決まった後、どういう状態になっていれば良いかという条件でしょうか。対策する形が決まった後に話を出すなら良いが、対策決まっていない状態で話を出すのはおかしい
#### システム全体の完了条件を固定する
...
★１つしか対策案がないという事？たとえば、トッピング用のクラスを用意して、それを利用者にAdd関数で追加させる。そのクラスは金額も持っている。実行が呼ばれるとリストに積まれたクラスを１つずつ取り出して計算するだけ。それがこの章の対策と同じ？違うなら別案として出して。安易に最終構造は１つと決めつけないでほしい。むしろ私の案の方がシンプルでこのくらいの小規模だったら私の案の方が良かったりしないか。
