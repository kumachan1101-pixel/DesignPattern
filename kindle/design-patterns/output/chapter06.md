...
| 「直接」＝直差し | `if (hasMilk) total += 50;` — `getPrice()` 内でラッパーを介さず各トッピングの価格計算をインラインで直接記述している |
...
- **接続点B**：追加されたトッピング（ミルクなど） ←→ さらに重ねて追加されるトッピング（ホイップ、抹茶など）の境界
...
今回のモバイルアプリ側（呼び出し元）は、`new CustomDrink("Coffee", 300, true, true, false)` のように、コンストラクタの引数にフラグを直接渡す方式を使っています。クラスを分離し、接続点の形を変えれば、この引数の渡し方は使えなくなります。つまり、モバイルアプリ側の「インスタンスを組み立てる部分」には確実な影響が波及し、修正が必要になります。呼び出し側のコードに手を入れなければならないという事実をあらかじめ覚悟しておく必要があります。

これらの接続点をどのような形にするかが、今回の設計の鍵を握ります。これらの接続点が「具体×直接」のまま残るのか、それとも形を変えることができるのか。私自身、この課題定義を飛ばしてすぐにコードをいじり始め、後で「あ、クライアント側にこんなに影響が出るなんて」と手戻りになった経験が何度もあります。
...
| 🟡 フェーズ5：課題定義 | 接続点A・Bを定義し、クライアント影響を確認した |
...
| `ToppingWrapper` | 内部のドリンクを包む仲介役の責任を定義する。 | デコレータの連鎖構造に変更が生じる場合 |
