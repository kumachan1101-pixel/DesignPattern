...
* **得られること1：** 「機能の組み合わせ」という観点で、コードの変動箇所を識別できるようになる。★どういう考えをするから識別できるのか。
* **得られること2：** 接続点が「具体×直接」になっているクラスを見て、そこが変更の痛みの発生源だと判断できるようになる。★どういう考えをするから、発生源だと判断できるのかを記載して。
* **得られること3：** 接続点の形を変えると変更がどのように局所化（変更の影響が1クラスだけで済む状態）されるかを、構造から説明できるようになる★それはどんな考えが身につくからなのか
* **得られること4：** 基本機能と追加機能を同じインターフェースで扱うことで、呼び出し側に違いを意識させずに機能を何層でも重ねていく視点★この構造にしようと思えるきっかけは？思いつけますか。

前段でこの章の目的と得られる視点を共有しました。ここからはいよいよ、実際のコードを前にして思考プロセスを回していきます。最初のステップであるフェーズ1では、システムの現状を「事実として」観察することから始めましょう。最初から完璧に読み解けなくても、間違えても大丈夫です★この一文いりますか。上から目線はやめてほしい。一緒に少しずつ解きほぐしていきましょう。
...
システムが立ち上がった当初、メニューは「コーヒー」や「紅茶」といったシンプルな基本ドリンクのみでした。しかし、ビジネスが成長し「自分好みにカスタマイズしたい」というお客様の声が大きくなるにつれて、ミルクの追加、ホイップの増量、シロップの変更など、多種多様なトッピング機能が追加されてきました。店舗のオペレーションと連動するため、注文システムは正確な「合計金額」と、バリスタ★バリスタって何？ドリンク作る人？余計な専門用語使わないで。に伝えるための「注文内容（名前）」を算出する重要な役割を担っています。

★以下のコードを見るととあるが、読者はまだコード見ていないのですよ。コードだしてからこの説明がないと、読む側からしたら何の話となりませんか？1-6が先に来てから、分析結果を整理していく流れが良いでしょうか？それほど複雑なコードではないのだから、コードを先に見せてから、構造の説明に入った方が良いのでは？
コードを眺めてみると、すべてのトッピングの有無を真偽値（booleanフラグ）で持たせ、一つのクラスの中で金額と名前を組み立てる構成になっています。追加要望が来るたびにフラグと `if` 文を足していくこのアプローチは、当時の現場のスピード感に最も合っていたのでしょう。

一見すると、このコードは処理が一つにまとまっており、うまく整理されているように見えます。このシステムが今日まで多くのお客様の複雑なカスタマイズ注文をさばき、お店の売上を支えてきた事実を、まず率直に認めたいと思います★上から目線ではないですか？何とか動いているみたいな馬鹿にした表現に見えるのですが。
...
フェーズ1の責任チェックが完了しました。この観察をもとに、次のフェーズ2では、変更要求を受けて「何が変わり、何が変わらないか」の仮説を立て、関係者に確認します。
...
間違えても大丈夫です、と自分に言い聞かせながら、こうした未知の要件を初期段階で引き出せたことを前向きに捉えたいと思います。
...
フェーズ2で「何が変わり、何が変わらないか」が確定しました。次のフェーズ3では、今のコードのまま変更要求を実際に試みて、何が起きるかを確認します。
...
★観察から原因の方向を導き出した思考ロジックを言語化して挿入してほしい。フォーマット変えたい。
...
### 4-3：ケーブルで考える
★ケーブルで考える、というタイトルが分かりづらい。例えばですが、抽象的に視点を見るためにケーブルにたとえてみる、というようなタイトルが良いのでは？
...
★以下「rror parsing Mermaid diagram!」となています。
```
...
変更頻度が「高」であることから、ここをしっかり抽象化して差し替えやすくする投資は、十分に元が取れそうです。また、今回はホットパス（頻繁に呼び出されるコードパス）でもなくメモリの制約も厳しくないため、間接層を増やすことによるオーバーヘッドを過度に心配する必要はありません。純粋に「変更しやすさ」を優先して接続形態を選ぶことができます。
...
public:
    CustomDrink(string name, int price, Milk* m = nullptr, Whip* w = nullptr)
        : baseName(name), basePrice(price), milk(m), whip(w) {}
        ★これはどんな文法？コンストラクタに設定するだけだと思うが見慣れない。最近の文法は使わず、シンプルな構造にして。余計な思考をさせたくないのです。
...
★呼び出し側は型だけ知ると書いてありながら、呼び出し側のコードがありません。他の例も同様、接続部分のコードは入れてほしい。私の理解が間違っていたら指摘してほしい。
...
このコードを見ると、呼び出し側のコードはシンプルになりますが、すべての負担を仲介役である `OrderManager` が背負い込むことになったことが分かります。★クラス名が変わっただけでは？間接的にしたというのは本当に意味あっているか？
...
| 1 | 変更が4クラス以上に波波する | 実装に依存しテストが困難 | 中間層・インターフェースが複数増え理解コストが高い |
...
フェーズ6で採用案が決まりました。次のフェーズ7では、その決断を実際のコードに落とし込み、変更がどのように局所化されたのかを確認します。
