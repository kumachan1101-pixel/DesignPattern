...
フェーズ1でシステムの現状を観察しました。次のフェーズ2では、現場に届いた変更要求を起点にして「何が変わり、何が変わらないか」の仮説を立て、関係者とのヒアリングを通じてそれを確定させていきます。

### 2-1：変わりそうな仕様の見当をつける

コードを変更する前に、「この仕様は今後どれくらい変わりそうか」という仮説を立てておきます。フェーズ1の仕様表を振り返ると、現在のシステムには「基本ドリンク」と「トッピング」という2つの要素があります。このうち変化が予想される仕様があります。

- **トッピングの種類**（Milk・Syrup・Whipなど）：季節メニューや新商品として追加・廃止されることが多く、変化頻度が高い部分です
- **トッピングの価格**（50円・30円・70円など）：商品の価格改定やキャンペーンに合わせて変わります

一方、「価格を足し合わせる計算の骨格」そのものは、カフェの注文システムとして安定している部分です。

この仮説が正しければ、「トッピングに関する知識」はこれからも繰り返し変わり続ける可能性があります。この仮説をヒアリングで確認します。

---

### 2-2：変わる理由の分析

仮説が立てられました。次に、コードの各行が「誰の判断で変わる知識か」を確認することで、仮説をコードレベルで具体化します。

`CustomDrink.getPrice()` と `CustomDrink.getDescription()` の各行を見ると：

| **コードの行** | **持っている知識** | **誰の判断で変わるか** |
|---|---|---|
| `string baseName; int basePrice;` | 基本ドリンクの名称と基本価格 | ドリンク開発チームが決定 |
| `if (hasMilk) total += 50;` | ミルク追加の価格設定（50円） | 商品企画部が価格改定を決定 |
| `if (hasWhip) total += 70;` | ホイップ追加の価格設定（70円） | 商品企画部が価格改定を決定 |
| `if (hasSyrup) total += 30;` | シロップ追加の価格設定（30円） | 商品企画部が価格改定を決定 |
| `if (hasMilk) desc += " + Milk";` | ミルク追加時の表示名 | 店舗オペレーション設計部が決定 |

1つのクラスの中に、変える理由が異なる複数の知識が並んでいます。今すぐ問題とは言えませんが、変わる理由の決定者が複数いるという観察を、ここで記録しておきます。

要するに、`getPrice()` や `getDescription()` の中にトッピングごとの処理が `if` 文で書き連ねられているという観察から、「後から追加される処理（各種トッピング）」と「基本となる処理（ドリンク本体）」が同じ場所に混在しているという構造が見えてくる。
...
ヒアリングで「同じトッピングを複数回追加（ダブル・トリプル）」機能の追加が予告されました。この変更が来たとき、仕様がどう変わるかを整理しておきます。

| トッピング | 現在 | 将来（数ヶ月後） |
|---|---|---|
| Milk（ミルク） | 対応済み（+50円） | 変更なし |
| Whip（ホイップ） | 対応済み（+70円） | 変更なし |
| Syrup（シロップ） | 対応済み（+30円） | 変更なし |
| Matcha（抹茶パウダー） | 今回追加（+60円） | 変更なし |
| Choco（チョコチップ） | 今回追加（+40円） | 変更なし |
| ダブル/トリプル数量指定 | —（なし） | 追加予定（同一トッピングを複数回追加） |

ダブル/トリプルが追加されると `hasMilk` という真偽値では表現できなくなり、数量フィールドへの変更が必要になります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された「ダブル・トリプル数量指定」も視野に入れると、`hasMilk` という真偽値では表現できなくなり `milkCount` のような数量フィールドへの変更が必要になることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、今の bool フラグの構造がその追加を受け止めにくいことは確認できます。
...
分離の境界は決まりました。次は「トッピングごとのクラスをまとめるか、個別に分けるか」という粒度の問題です。

ヒアリングで把握したトッピングと、変わる理由を確認します。

| トッピング | 変わる理由 |
|---|---|
| Milk（ミルク） | 原材料費・価格改定（商品企画部） |
| Whip（ホイップ） | 原材料費・価格改定（同じ商品企画部） |
| Syrup（シロップ） | 原材料費・価格改定（同じ商品企画部） |
| Matcha（抹茶パウダー） | 季節キャンペーン企画（同じ商品企画部） |
| ダブル/トリプル（将来） | 顧客要望・オペレーション設計（同じ商品企画部） |

すべて同じ商品企画部が管轄していますが、変わり方は独立しています。Milk の価格改定が Whip の価格に影響することはありません。Matcha の廃止が Syrup の処理に波及することもありません。各トッピングは**独立して変わる**ため、トッピングごとに別クラスへ分離します。どのトッピングも `getPrice()`・`getDescription()` という同じ接続点を持ち、連鎖して重ねられます。

→ **トッピングごとに別クラスへ分離し、同じ `IDrinkComponent` インターフェースで束ねる**
...
| 🟣 フェーズ2：仮説立案 | 仕様表からトッピング種類・価格が今後も変わり続けるという仮説を立て、ヒアリングで裏付けた。確定変更と将来リスクを分けて整理した |
...
| **得られること** | **この章のどこで示したか** |
| --- | --- |
| 1. 変動箇所の識別 | フェーズ2の仮説立案とヒアリングで、トッピング種類・価格という変動する仕様を特定した |
