...
一見してシンプルに見えたコードですが、一行ずつ観察していくと、商品企画部門が決めるべき「トッピングの価格」や、店舗設計部門が関わる「表示名」という異なる性質の知識が、一つのクラスの中に並んでいることが見えてきます。
...
---
> **📌 問題（確定）**
> トッピングの種類が増えるたびに、`CustomDrink` のメンバ変数・コンストラクタ・`getPrice()`・`getDescription()` の4箇所を連動して修正しなければならない。ヒアリングで「毎月のキャンペーンごとにトッピングが入れ替わる」と確認された今の変更頻度では、1種類追加するだけで呼び出し側のコードまで壊れるこのコストは合わない。
---
...
| **観察した症状** | **構造的な原因** |
|---|---|
| トッピングを追加するたびにクラス内の複数の `if` 文を探して修正しなければならない | `CustomDrink` が各トッピングの価格・名前という具体的な条件を直接知っているから |
...
**得られること4**（同じインターフェースで重ねる視点）：フェーズ6のステップ5で確認した通り、Decoratorパターンの核心は「トッピングをインターフェース化する」だけでなく「基本ドリンクとトッピングを同じインターフェースで扱う」ことにあります。これにより、トッピング自体が別のドリンクを包む仲介役になれ、「追加するたびに呼び出し側も変えなければならない」という痛みから解放されます。
