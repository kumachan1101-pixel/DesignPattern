...
フェーズ4の分析により、問題の根本原因は「基本ドリンクの骨格」と「各トッピングの知識」という、変わる理由が違う2つのものが混在していることだと分かりました。

現在の `CustomDrink` クラスからトッピングの知識を切り出すと、以下のような接続点が現れます。

- **接続点A**：コンストラクタ引数 `bool addMilk`, `bool addWhip`, `bool addMatcha`（bool型）―― トッピングの有無を個別のフラグで表現しており、新しいトッピングが増えるたびにコンストラクタの引数が増える
- **接続点B**：`if` 分岐内にハードコードされた価格（`100`, `150`, `200` 等 int型）と表示名（`"ミルク"`, `"ホイップ"` 等 string型）―― トッピングを重ねても同じクラスの同じ `if` 群で制御するため、組み合わせや順序を独立して管理できない

ここで少し立ち止まって、考えてみてください。トッピングは1つとは限りません。「コーヒーにミルクを追加し、その上にさらにホイップを追加する」といった具合に、機能が連鎖していく特性を持っています。そのため、基本のドリンクとトッピングを繋ぐだけでなく、トッピング同士を数珠繋ぎにしていく接続点も考慮する必要があります。

これらの接続点が「具体×直接」のまま残るのか、それとも形を変えることができるのか。この選択が今回の設計の鍵を握ります。

フェーズ5で、私たちが解くべき課題と制約が明確になりました。次のフェーズ6では、この課題に対してどのような解決策があるのかを、段階的に検討していきます。いきなり一つの正解を決め打ちするのではなく、はじめに選択肢を段階的に並べてみましょう。
...
sequenceDiagram
    ...
    participant M2 as Matcha（2個目）
    participant M1 as Matcha（1個目）
    participant Mi as Milk
    participant C as Coffee

    OA->>C: new Coffee()
    OA->>Mi: new Milk(coffee)
    OA->>M1: new Matcha(milk)
    OA->>M2: new Matcha(matcha1)
    OA->>M2: getPrice()
    activate M2
    M2->>M1: baseDrink->getPrice()
    activate M1
    M1->>Mi: baseDrink->getPrice()
    activate Mi
