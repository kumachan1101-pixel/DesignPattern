...
この時点では、トッピングをどう重ねる設計にするかは判断しません。まず、基本ドリンクと追加トッピングから何が計算されるかを確認します。
...
この章では、注文画面、在庫確認、レシート発行を省略し、注文名と金額の計算結果だけを実行結果として確認します。実システムなら画面表示やレシート発行は `OrderViewRenderer` や `ReceiptPrinter` のような境界へ渡しますが、本章の論点は「基本ドリンクに追加機能を組み合わせる構造」であり、決済、在庫、印刷処理は扱いません。
...
1-1の現状仕様を退避し、変更要求を当てた後の仕様と同じ粒度で並べます。ここで見るのは、設計の良し悪しではなく、以降の分析対象になる差分です。

| 要素 | 変更前（1-1の現状仕様） | 変更後（今回の要求） | 差分として追うもの |
|---|---|---|---|
| 入力 | 基本ドリンク、Milk/Syrup/Whip | 基本ドリンク、Milk/Syrup/Whip/Matcha/Choco | トッピングの種類が増える |
| 判定 | トッピングが有効か | 追加されたMatcha/Chocoも有効か | 有効な選択肢が増える |
| 加工 | トッピング価格と説明を重ねる | 新トッピングの価格と説明も同じ流れで重ねる | 組み合わせ方は維持し、部品だけ増える |
| 出力 | 注文名と合計金額 | Matcha/Chocoを含む注文名と合計金額 | 出力名と金額の組み合わせが増える |
