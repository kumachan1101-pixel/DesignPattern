...
| 装飾連結構造 | 前段の価格・説明を参照し、順序で振る舞いが変わる部品を重ねる | 各部品が同じ `IDrink` を包み、内側へ作用する計算・説明を独立追加できる | クラス数と動的生成が増える。本章は「サイズ別追加料」「直前の説明へ加工を重ねる」要求まで対象にするため採用 |

リスト案は途中状態ではなく、十分に完成した別案です。価格と名前を足すだけの小規模システムなら、読者の案の方が簡潔です。本章では、フェーズ2で確認した順序依存の表示と、内側の価格を使うサイズ別追加料まで変化対象に含めるため、`IDrink` を連結する構造を採用します。
