...
まずは、ドリンク注文カスタマイズが何を入力として受け取り、どの処理で加工し、何を出力するのかを整理します。ここでは設計の良し悪しを判断せず、現状を事実としてそろえます。
...
- 未対応トッピングがある場合は、名前や金額の組み立てに進めない。
...
この表がフェーズ1で把握する現状の動作です。ここではまだ「なぜこの構造が問題なのか」までは判断しません。読者が確認すべきことは、注文内容から注文名称と合計金額を予測できることです。どのクラスが担当しているかは、次のクラス構成で確認します。
...
この章では、注文画面、在庫確認、レシート発行を省略し、注文名と金額の計算を `std::cout` で確認します。論点は「基本ドリンクに追加機能を組み合わせる構造」であり、決済、在庫、印刷処理は本章では扱いません。
...
変更後の仕様を、1-1と同じ入力・判定・加工・出力の流れとして図で確認します。1-1の図との差分は、入力の「追加トッピング」の選択肢が3種から5種へ増えることだけで、判定・加工・出力の流れ自体は変わりません。
...
- 図の箱は1つも増えておらず、変わるのは「追加トッピング」の選択肢（Matcha・Chocoの2種が加わる）だけである。
- 「トッピングは有効か」の判定と「トッピング価格を重ねる」の加工が、新しい2種を受け付けられるようになる必要がある。
...
フェーズ1でシステムの現状を観察しました。次のフェーズ2では、現場に届いた変更要求を起点にして「何が変わる見込みで、何を当面安定と見るか」の仮説を立て、関係者とのヒアリングで裏付けます。ここではまだ、現状の責任配置が問題かどうかは判断しません。
...
コードが実際に「知っていること」を一行ずつ照合し、その知識が**どの業務機能に属するのか**を観察します。これは「クラスが責任を果たしているかどうか」を判定する作業ではなく、後続フェーズで変更要求を当てるための材料をそろえる確認です。

| **クラス名** | **責任（1文）** | **知るべきこと** |
| --- | --- | --- |
| `CustomDrink` | ドリンクの基本料金とトッピング料金を合算して提供する。 | 基本価格、ミルクの有無と価格、ホイップの有無と価格、シロップの有無と価格。 |

一見してシンプルに見えたコードですが、一行ずつ観察していくと、商品企画部門が決めるべき「トッピングの価格」や、店舗オペレーション部が関わる「表示名」という、**異なる業務機能に属する知識**が一つのクラスの中に並んでいることが見えてきます。変わる理由の業務機能が混在しているという事実を、ここで確認しておきます。

---

#### 変わる理由の分析

業務機能の所在表でクラスの責任が整理できました。次に、コードの各行が「どの業務機能に属する知識か」を確認することで、混在している責任をさらに細かく特定します。判断基準は、「このクラスの担当者（ここではドリンク開発チーム）とは別の業務機能が変更を決定するかどうか」です。別の業務機能が決定するなら、それは「責任外（❌）」と判断します。

`CustomDrink.getPrice()` と `CustomDrink.getDescription()` の各行を見ると：

| **コードの行** | **持っている知識** | **業務機能** | **この章の中心クラスが直接持つ知識か** |
|---|---|---|---|
| `string baseName; int basePrice;` | 基本ドリンクの名称と基本価格 | システム設計・生成管理 | ✅ |
| `if (hasMilk) total += 50;` | ミルク追加の価格設定（50円） | 料金・プロモーション管理 | ❌ 別担当者 |
| `if (hasWhip) total += 70;` | ホイップ追加の価格設定（70円） | 料金・プロモーション管理 | ❌ 別担当者 |
| `if (hasSyrup) total += 30;` | シロップ追加の価格設定（30円） | 料金・プロモーション管理 | ❌ 別担当者 |
| `if (hasMilk) desc += " + Milk";` | ミルク追加時の表示名 | 業務ルール管理 | ❌ 別担当者 |
...
---
> **📌 課題（確定）**
> 切り離す境界は「基本ドリンク」と「各トッピングが何円で何という名前か」の間にある。接続点で受け渡すのは `int` 型の金額と `string` 型の説明だが、トッピングの種類と組み合わせは変わり続ける。各トッピングを同じ契約で包み、基本ドリンク側の条件分岐を増やさず組み合わせられる形にすることが、この章の課題だ。
---
...
どのステップも、フェーズ1の動作例テーブルの動作を実現します（ホイップ×2はステップ4の別案以降で対応可能になります）。違うのは「変更が来たときにどこを触ることになるか」です。

ここからの検討は、次の2つのよくある考え方を軸に進めます。1つ目は、**共通部分に着目して共通化し、抽象化する**ことです。2つ目は、**変わる部分を分離して外から注入し、組み立て側は中身を意識せず決まった流れを回すだけにする**ことです。以下の各案は、この2つの考え方をこの章の課題へ当てたものです。

ここで比べる案は、次の5種類です。

| 案 | 何を分けるか | 期待できる効果 | 限界 |
|---|---|---|---|
| 関数切り出し | 各トッピングの価格・説明の処理 | 処理が名前で読め、共通構造に気づける | フラグとif文の追加は残る |
| 継承で全組み合わせ | 組み合わせごとの型 | 組み合わせを型として表せる | 組み合わせ数だけクラスが爆発する |
| トッピングをクラス化 | 各トッピングの知識 | 価格変更が1クラスで済む | 本体が具体クラス名を直接持つ |
| トッピングのリスト保持 | 本体が持つトッピングの集まり | 追加もダブルも「積むだけ」で済む | 装飾済みがドリンクと同じ型にならない |
| トッピングがドリンクを包む | 基本とトッピングを同じ契約へ | 装飾済みも同じ型として何層でも重ねられる | クラス数と組み立ての初期コスト |

このうち「トッピングのリスト保持」は、本体にトッピングのリスト（配列）を持たせ、価格や説明はループで合成すればよい、という最初に思いつきやすい案です。この案は比較表の中だけで済ませず、ステップ4の別案として実際にコードにし、どこまで痛みが解消され、何が残るかを確かめます。

なお、関数切り出し・トッピングのクラス化は1つ目の「共通化・抽象化」の実行、リスト保持・ドリンクを包む形は「トッピングを外から与え、本体は決まった流れ（合計・連結）を回すだけにする」という2つ目の実行にあたります。
...
class CustomDrink {
    string baseName;
    int basePrice;
    vector<ITopping*> toppings; // ← 固定スロットではなくリスト
public:
    CustomDrink(string name, int price)
        : baseName(name), basePrice(price) {}
    void add(ITopping* t) { toppings.push_back(t); } // 組み立て側は積むだけ
    int getPrice() const {
        int total = basePrice;
        for (auto* t : toppings) total += t->getPrice();
        return total;
    }
    string getDescription() const {
        string desc = baseName;
        for (auto* t : toppings) desc += t->getName();
        return desc;
    }
};
```
...
**この案の評価：**
`Choco`（チョコチップ）を追加しても、`CustomDrink` は一切変わりません。新しいトッピングクラスを作り、組み立て側で `order.add(&choco)` と積むだけです。ステップ3・4で残っていた「トッピングを足すたびに `CustomDrink` へスロットと `if` を書き足す」という痛みは、ここで解消します。さらに、ステップ4まで実現できなかった「ホイップをダブルにする」も、`add(&whip)` を2回呼ぶだけで表せます。**この章の動作例（価格の合計と説明の連結、ホイップ×2）だけを見れば、この案で止める判断は十分あり得ます。**

**この案に残る課題：**
このリスト方式では、トッピングを付けた結果は依然として `CustomDrink` という、基本ドリンク（`IDrink`）とは別の型です。「トッピング済みのドリンクを、また別のドリンクの材料として扱う」「基本ドリンクを受け取る場所へ、トッピング済みのものをそのまま渡す」といった、装飾済みを装飾なしと区別せず同じ型で扱いたい場面では、`CustomDrink` と `IDrink` の橋渡しが別途必要になります。次のステップでは、トッピング自体を `IDrink` にすることで、装飾済みのドリンクも「ただのドリンク」として何層でも重ねられる形を確かめます。この「装飾済みも同じ型」という性質は、第11章で複数の構造を組み合わせるときに効いてきます。
...
- **ステップ4の別案（リスト保持）で止めるケース：** トッピングの追加やダブルには対応したいが、トッピング済みのドリンクを「ただのドリンク」として別の処理へ渡したり、さらに重ねたりする必要がない場合。価格の合計と説明の連結だけならこの案で足ります。
- **ステップ5まで進むケース：** 「毎月新しいトッピングが追加される」「同じトッピングを複数回重ねる要望がある」ことに加え、装飾済みのドリンクを装飾なしと同じ型で扱いたい場合。
...
| **解決策** | 装飾連結構造：基本ドリンクとトッピングを同じ `IDrink` インターフェースで統一し、トッピング自身が別のドリンクを包む `ToppingWrapper` 構造で機能を動的に重ね合わせる |
...
| 🔴 フェーズ6：対策検討 | 段階的進化で各アプローチの限界を確認した。読者が思いつくトッピングのリスト保持案もコードで試し、追加もダブルも解けるが装飾済みがドリンクと同じ型にならない残課題から、ステップ5（トッピング自体がドリンクを包む）まで進化させる決断を下した |
