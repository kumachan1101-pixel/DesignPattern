...
|---|---|---|---|
| 基本ドリンク | 注文種別から基本価格・説明を返す | 金額・商品説明 | 商品DBを`std::map`で代替する |
| トッピング | 内側のドリンク結果へ価格・説明を加える | 合成後の金額・説明 | 生ポインタは包む対象への参照 |
| `std::vector` | 注文したトッピング列を保持する | 指定順に適用する | 組み合わせ入力の一覧 |
| `assert` | 期待金額と実際の金額を比較する | 不一致なら検証失敗 | テスト用で本番処理ではない |

決済や在庫引当は扱わず、価格と説明の合成だけを実物として動かします。生ポインタの寿命は組み立て側が管理し、装飾対象より先に破棄しません。
...
|---|---|---|
| Matcha / Chocoトッピングを追加 | `CustomDrink` にフラグ追加・価格の if 文追加・説明の if 文追加（種類ごとに3箇所） | `Matcha` / `Choco` クラスを作成し、組み立てへ登録 |
| トッピングの価格を変更 | `CustomDrink` の if 文内を修正 | `ToppingCatalog` の対象データのみ更新 |
