...
| 手段 | 方法 | 特徴 |
|---|---|---|
| 手段A：フラグ直書き | `bool` メンバ変数を追加し `if` 文を書き足す | 変更が最小限。コンストラクタ引数が増え続ける |
| 手段B：デフォルト引数の活用 | コンストラクタのデフォルト引数で既存呼び出し元への影響を緩和する | 呼び出し元の即時破壊は防げるが、クラスの肥大化は止まらない |

**手段A**（この案の本質である「構造を変えない」という意図をそのまま示すため）のコードを以下に示します。
...
#### 案2：具体×直接 ―― クラスは分けるが、呼び出し側は具体型を直接知る
...
#### 案3：抽象×直接 ―― インターフェースを挟み、呼び出し側は型だけを知る
...
4つの案がテーブルに並びました。どれが正解かはプロジェクトの状況によって異なります。次のステップでは、評価の「ものさし」を宣言し、コストの天秤にかけていきます。

### 6-2：評価軸

4つの対策案が出揃ったところで、どの案を採用するかを決めるための「ものさし」を先に宣言しておきましょう。比較表を見せてから「この軸で評価しました」と言うのは後付けになってしまうため、はじめに評価の基準を透明にしておきます。

また、スコアリングに入る前に「パフォーマンスによる足切り（VETO）」の確認を行います。フェーズ5の課題定義で確認した通り、今回のドリンクの価格計算処理はマイクロ秒を争うようなホットパスではありませんでした。そのため、間接層が増える案3や案4を採用してもシステム全体への致命的な遅延は発生しないと判断し、今回はすべての案をスコアリングの対象としてテーブルに載せます。
...
- 案1の変更容易性=1：トッピング追加のたびにメンバ変数・コンストラクタ・if文の3か所以上を修正し、呼び出し元まで波及するため最低点。
- 案1のテスト容易性=1：具体クラスに強依存し、スタブへの差し替えが不可能なため最低点。
- 案1の可読性=3：クラス数が増えず、既存の読み方がそのまま使えるため最高点。
- 案2の変更容易性=1：具体型のセッターが増え続けるため、案1と同様に修正の波及が止まらない。
- 案2のテスト容易性=2：クラスは分離されたが具体型への依存が残り、一部のスタブ差し替えは可能。
- 案2の可読性=3：クラスが分かれたが関係が直感的で、読む工数は案1と同等。
- 案3の変更容易性=2：IToppping経由でCustomDrinkの修正は不要だが、「ダブル・トリプル」など基本と同じ型で扱う要件には追加の設計変更が必要。
- 案3のテスト容易性=3：インターフェース経由のためスタブ1つで完全に切り離せる。
- 案3の可読性=2：IToppping＋Customdrinkと1〜2クラス増えるが、関係は比較的追いやすい。
- 案4の変更容易性=3：新トッピングはクラスを1つ追加するだけで完結し、既存コードに触れない。
- 案4のテスト容易性=3：IDrink*型のスタブを差し込むだけでどの層も独立してテストできる。
- 案4の可読性=1：IDrink・ToppingWrapper・各トッピングと中間層が複数増え、初見での構造把握に時間がかかる。

案1や案2は今のコードの延長線上にあり、クラス数も増えないため可読性（理解のしやすさ）は非常に高くなります。しかし、次のトッピング追加が来たときの変更容易性は著しく低く見積もられます。
...
**理由：**
商品企画部へのヒアリングにより、今後もトッピングの種類が高頻度で増減し続けることが確定しています。この「毎月変更が確定している」という事実こそが、案4を選ぶ判断基準です。変更頻度が低ければ、案1（現状のまま）も十分に正当な選択です。フラグを1本足すだけで済む変更が年に1回しか来ないなら、クラス数を増やして複雑さを上げる必要はありません。

しかし今回は、毎月のキャンペーンごとにトッピングが入れ替わり、ダブル・トリプルの複数追加も要求が来ることがヒアリングで確定しています。この変更頻度の高さが、案4に踏み込む根拠です。中長期的な運用を見据えた場合、トッピングの追加や複雑な組み合わせが要求されるたびに既存コードを壊し続ける未来コストは許容できません。クラス数が増えるという現在の実装コスト（とわずかな可読性の低下）を支払ってでも、変更が1つのクラス追加だけで完結する拡張性の高い構造を手に入れるため、案4を採用します。

どちらが正しいかは、チームの状況やビジネスの文脈によって変わります。判断基準は「変更が何度来るか」です。変更頻度が低ければ案1が正当であり、変更頻度が高ければ案4の投資が回収できます。
...
もし案1のフラグ管理（`bool hasWhip;`）のままだった場合、ダブルに対応するためには `int whipCount;` のように型を変え、価格計算のロジックをすべて掛け算に書き直すという大規模な修正が必要になっていたはずです。

しかし、今回採用する案4ではどうなるでしょうか。基本機能（ドリンク）を拡張機能（トッピング）で何層にも包み込む構造になっているため、「ホイップのダブル」は以下のように呼び出し側でクラスを2回重ねるだけで実現できます。
...
フェーズ6で対策案を比較し、未来の変更に備えて「抽象×間接」の形を採用する決断を下しました。最後のフェーズ7では、この決断を実際のコードに落とし込み、変更がどのように局所化されたのかを確認します。
