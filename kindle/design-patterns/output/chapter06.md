...
|---|---|---|
| P1 | トッピング追加で `CustomDrink` のフラグ、価格・説明分岐、組み合わせ確認へ波及する | `getPrice()`・`getDescription()` の利用方法、注文検証、既存の組み合わせ |

P1は「トッピング追加の波及」を表します。フェーズ6では、追加トッピングと組み立てだけへ影響を縮める理想を描きます。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた課題——**基本ドリンクから、トッピングごとの加算処理と説明追加を切り離し、順に重ねても同じ操作で扱える接続点を作る**——を受けて始めます。まずフェーズ4で記録した現在の痛みを図の起点に置き、P1でどのコード境界を微修正し、同じ要求を当て直すと影響がどこまで縮むかを一本の経路で描きます。その経路から、現在→理想の差、守る契約、完了条件、候補を同じP1の行へ落とします。その後、P1に紐づくフェーズ3の関連コードだけをおさらいし、候補を一段ずつ適用します。各段階で「何を変えたか」「どの影響が減ったか」「何が残るか」をコードで回収し、統合後の全体コードはフェーズ7で示します。
#### 痛みの変更影響から、構造をどう変えるか
...
flowchart LR
    ...
    subgraph StructuralChange["フェーズ6：構造を微修正"]
        S1["P1-変更1: 価格・説明の加算を関数へ出す"] --> S2["P1-変更2: 同じ操作で1段ずつ包む"]
    end
    subgraph Result["同じ要求を再度当てた結果"]
        IR["トッピングを追加"] --> I1["P1: トッピング1クラスとカタログ登録を追加"]
        I1 --> I2["追加部品と組み立てだけをテスト"]
...
|---|---|---|---|
| P1 | 加算関数、`IDrink::getPrice/getDescription()`、`Milk`、カタログ | **微修正：** 関数化→組み合わせ爆発を確認→同じ契約で1段ずつ包む<br>**結果：** フラグと組み合わせ分岐が消えた | **守った契約：** 価格、説明、販売可否、選択順<br>**判定：** 関数化・組み合わせクラスでは未達、連結で達成 |
