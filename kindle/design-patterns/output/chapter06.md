...
カフェのドリンクシステムでは、「基本ドリンク」と「トッピング」を分けて管理するのが一般的です。基本ドリンクは注文の土台（ベース価格）として固定されており、トッピングはあとから積み上げて合計価格に加算されます。これは「ベース価格 ＋ オプション料金の加算」という、飲食業界でよく見られる料金体系です。★メニューIDの話は？
...
コードを読む前に、このシステムがどんな入力に対してどんな出力を返すかを確認します。以下の動作を基準とし、フェーズ7では掲載した組み合わせを実行結果で照合します。★以下関数名出さないで。まだクラス構成説明していないよね？
