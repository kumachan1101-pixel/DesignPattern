...
フェーズ6は、フェーズ5で定めた課題——**基本ドリンクから、トッピングごとの加算処理と説明追加を切り離し、順に重ねても同じ操作で扱える接続点を作る**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを変換して探します。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる
...
|---|---|---|---|
| P1 | 追加影響からCustomDrinkの全フラグと全組み合わせを外す | getPrice()/getDescription() の境界で1段の加算を切る。価格、説明、販売可否、重ね順を守る | 新トッピングは1クラスとカタログ登録で追加し、組み合わせクラスや分岐を増やさない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 価格・説明の加算を関数へ出す | トッピングごとの差分が見える | ドリンクと追加部品を同じ操作へそろえ、1段ずつ包んで組み立てる |

ここからP1の価格・説明・組み合わせ分岐だけをおさらいします。
