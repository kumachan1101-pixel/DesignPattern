...
**着目する共通点：** 現状コードはすでに共通点を持っています。どのトッピング組合せでも、`getPrice()→int` と `getDescription()→string` という同じ契約で価格と名称を返す形にそろっています（契約の共通点）。現場では、この呼び口すらそろっていないことも多く、その意味で現状コードはすでに良い出発点です。共通点があるからこそ、利用側の呼び出しを変えず、変わる「トッピングの価格・名称」を同じ契約で包める部品へ分ければ済みます。フェーズ6では、この共通契約と課題を組み合わせて完成構造を決めます。
