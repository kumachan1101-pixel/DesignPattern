...
```mermaid
flowchart LR
    U["利用者<br>基本ドリンクとトッピングを選ぶ"] --> O["ドリンク注文システム"]
    O --> M[("メニュー表<br>表示名・価格・販売状態")]
    M --> O
...
動作例テーブルの「ホットコーヒー + Milk + Whip」と同じ結果です。任意の注文については、基本価格に選んだトッピングの価格を足し、注文名称へ選んだトッピング名を追加する、と読めば出力を予測できます。
...
> **中間コードの継続条件：** 以下は価格と表示名の分岐だけを抜き出します。フェーズ1の `MenuDatabase` による基本ドリンク・トッピングIDの存在確認は維持し、有効な選択だけを `CustomDrink` へ渡します。販売可否や価格を扱う `ToppingCatalog` は、変更要求を実装する過程で追加する境界です。
