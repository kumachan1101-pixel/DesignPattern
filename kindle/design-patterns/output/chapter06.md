...
| ルール名 | 発動条件 | 結果 | 具体例 |
| --- | --- | --- | --- |
| 基本ドリンク料金 | 常に適用 | 基本価格をそのまま合計に加算する | コーヒー → 300円 |
| トッピング加算 | 対象トッピングが注文に含まれる場合 | トッピングごとの追加料金を合計に上乗せする | ミルク → +50円、ホイップ → +70円、シロップ → +30円 |
| 注文内容の表記 | 常に適用 | 基本ドリンク名に、追加されたトッピング名を順に連結する | 「Coffee + Milk + Whip」 |

**このルールを使う場所**

同じ「価格計算と注文内容の組み立て」を2か所で使います。この「2か所で使う」という仕様が、設計の違いを生む起点になります。

| 使用場所 | 用途 |
| --- | --- |
| `MobileOrderService` | お客様の注文確定時に合計金額と注文内容を算出する |
| `StaffPreviewService` | スタッフがラベル印刷前に注文内容と金額を確認する |
...
接続点の形は「なぜ分けたか」だけで決まるわけではありません。パフォーマンスやメモリといったシステム上の制約が、選択できる接続の形を絞り込むことがあります。ここで事前に確認しておきましょう。

| **確認項目** | **内容** | **この章での判断** |
| --- | --- | --- |
| 変更頻度 | この接続点はどのくらいの頻度で変わるか | 高（毎月のキャンペーンごとに種類が増減する） |
| パフォーマンス | ホットパスか（高頻度で呼ばれるか） | いいえ（注文処理時の計算であり、マイクロ秒を争う処理ではない） |
| メモリ | 間接層の追加でオーバーヘッドが問題になるか | いいえ（1注文あたりのトッピングは数個程度であり、問題にならない） |
| 同時注文 | ランチピーク時に注文が集中した場合の処理量はどの程度か | 要確認（モバイルオーダーでは昼のピーク時に短時間で数百件の注文が入る可能性がある。注文オブジェクトの生成コストよりも、注文内容の計算・出力の一貫性を保証する設計が重要） |

変更頻度が「高」であることから、ここをしっかり抽象化して差し替えやすくする投資は、十分に元が取れそうです。また、今回はホットパスではなくメモリの制約も厳しくないため、間接層を増やすことによるオーバーヘッドを過度に心配する必要はありません。ただし、ランチピーク時に数百件の注文が短時間で集中する場合は、注文内容の計算と出力の一貫性を保証する設計が重要になります。この点を接続形態の設計判断に加えておきます。
...
classDiagram
    ...
    class MobileOrderService {
        +placeOrder(bool, bool, bool)
    }
    class StaffPreviewService {
        +getOrderLabel(bool, bool, bool) string
    }
...
**この形の考え方：**
ドリンクの計算とトッピングの組み合わせという複雑な処理を、専用の仲介役（`OrderManager`）に引き受けさせます。呼び出し側は仲介役に「コーヒーにミルクとホイップをお願い」と伝えるだけで済みますが、仲介役自身はすべてのトッピングの具体型を知っています。「呼び出し側からの複雑さを隠したい」場合に合う形です。
...
classDiagram
    ...
    class OrderManager {
        -Coffee base
        +calculateTotal(bool, bool, bool) int
        +buildName(bool, bool, bool) string
    }
    class MobileOrderService {
    ...
    class StaffPreviewService {
        -OrderManager manager
        +preview(bool, bool, bool)
    }
    MobileOrderService --> OrderManager : 具体×間接
    StaffPreviewService --> OrderManager : 具体×間接
    OrderManager --> Coffee : 具体×直接
```

両呼び出し元は `OrderManager` だけを知り、トッピングの組み合わせロジックと金額・名称計算が仲介役の一箇所に集約されている。

```cpp
class Coffee {
public:
    int getPrice() const { return 300; }
    string getName() const { return "Coffee"; }
};
...
public:
    // ベースドリンクに複数トッピングを組み合わせて合計金額を計算する
    int calculateTotal(bool addMilk, bool addWhip,
                       bool addSyrup) const {
        int total = base.getPrice();
        // ピーク時でも計算の一貫性を保証するため、
        // 組み合わせロジックを一箇所に集約する
        if (addMilk)  total += 50;
        if (addWhip)  total += 70;
        if (addSyrup) total += 30;
        return total;
    }

    // 注文名称を組み立てる（レシート・ラベル共通ロジック）
    string buildName(bool addMilk, bool addWhip,
                     bool addSyrup) const {
        string name = base.getName();
        if (addMilk)  name += " + Milk";
        if (addWhip)  name += " + Whip";
        if (addSyrup) name += " + Syrup";
        return name;
    }
};
...
class MobileOrderService {
    OrderManager& manager;
public:
    MobileOrderService(OrderManager& m) : manager(m) {}
    void placeOrder(bool milk, bool whip, bool syrup) {
        int price = manager.calculateTotal(milk, whip, syrup);
        string name = manager.buildName(milk, whip, syrup);
        cout << "注文確定: " << name
             << " / " << price << "円" << endl;
    }
};
...
class StaffPreviewService {
    OrderManager& manager;
public:
    StaffPreviewService(OrderManager& m) : manager(m) {}
    void preview(bool milk, bool whip, bool syrup) {
        string name = manager.buildName(milk, whip, syrup);
        int price = manager.calculateTotal(milk, whip, syrup);
        cout << "[ラベルプレビュー] " << name
             << " (" << price << "円)" << endl;
...
このコードを見ると、`OrderManager` は依然として各具体クラスを知っており、新しいトッピングが増えれば修正が必要です。しかし、複合トッピングの組み合わせと合計金額・注文名称の算出ロジックを一箇所に集め、`MobileOrderService`（顧客注文）と `StaffPreviewService`（スタッフラベル確認）が同じ計算ロジックを重複なく共有できる点に、この形の価値があります。
...
int main() {
    ...
    // 顧客モバイルアプリ：注文確定
    MobileOrderService mobile(manager);
    mobile.placeOrder(true, true, false); // ミルク＋ホイップ

    // スタッフ：ラベル印刷前のプレビュー確認
    StaffPreviewService preview(manager);
    preview.preview(true, true, false);   // 同じ注文内容を確認
    return 0;
}
```
...
sequenceDiagram
    ...
    main->>MOS: placeOrder(milk, whip, syrup)
    MOS->>OM: calculateTotal(milk, whip, syrup)
    Note right of OM: 内部で Coffee を直接知っている<br/>if addMilk → +50
    OM->>C: getPrice()
    C-->>OM: 300円
    OM-->>MOS: 最終金額
    MOS->>OM: buildName(milk, whip, syrup)
    OM->>C: getName()
    C-->>OM: "Coffee"
    OM-->>MOS: 注文名称
    MOS-->>main: 注文確定
    main->>SPS: preview(milk, whip, syrup)
    SPS->>OM: buildName(milk, whip, syrup)
    OM->>C: getName()
    C-->>OM: "Coffee"
    OM-->>SPS: 注文名称
    SPS->>OM: calculateTotal(milk, whip, syrup)
    OM->>C: getPrice()
    C-->>OM: 300円
    OM-->>SPS: 最終金額
    SPS-->>main: ラベルプレビュー
...
一文要約：両方の呼び出し元が同じ `OrderManager` を経由するため注文計算と名称組み立ての呼び出し経路が1本に収束し、選択ロジックの重複が消える。
...
**得られること4**（多層重ね掛けの視点）：フェーズ7で、`BaseDrink` → `MilkDecorator` → `WhipDecorator` と何層でもラップできる構造を確認しました。基本機能と追加機能を同じインターフェースで扱うことで、呼び出し側に違いを意識させずに機能を重ねていく発想が身につきます。

カフェのカスタマイズ注文という题材を通じて、機能の組み合わせをコードで表現する設計の視点を体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。