...
| 問題ID3 | 販売停止・表示順を足すと、価格計算以外の条件も `CustomDrink` へ増える | 変更ID2・変更ID3 |
...
課題IDを定義できたので、ここまでの追跡を一列で見渡します。本章は基本骨格とトッピング差分という一つの軸なので、3つの痛みは一つの原因・一つの課題へ集約します。
...
```text
現在：注文の組み立て が
      基本ドリンクの価格・説明 ＋ トッピングごとの価格・説明・販売可否 ＋ 組み合わせ分岐 を持つ
                      ↓
変更後：基本とトッピングを同じ契約にそろえ、トッピングは内側を包んで自分ぶんを重ねる
   トッピングの種類・価格・説明追加 → 各Wrapperが内側のIDrinkを包み自分ぶんを足す（機能連結＝Decorator）→ 課題ID1（連結）
   価格・販売可否・組み立て         → データと組み立てを専用クラスへ寄せる（データ／組立分離）        → 課題ID1（配置）
守る範囲：基本ドリンクが価格と説明を返す契約、入力順に重ねる規則、不正な組合せの拒否
```
