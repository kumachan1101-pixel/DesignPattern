...
| **根本原因** | **観察** | **原因の方向** |
| --- | --- | --- |
| 根本原因A：複数サブシステムとの複雑な接続が露出（→Facadeが必要） | 複数の連携先（A社・B社・C社）との通信詳細が `BatchExecutor` 内に直接展開されている | `BatchExecutor` が連携先ごとの具体的な接続手順を全て知っており、1クラスで複数サブシステムの複雑さを丸ごと抱え込んでいるから。 |
| 根本原因B：通知先の変動がメインフローに波及（→Observerが必要） | 転送結果の通知仕様を変えると、連携処理のフロー全体まで影響を受ける | 「データ転送」という連携処理と、「処理結果の通知」というイベント通知が、同じクラス内で密結合に混在しているから。 |
| 根本原因C：連携先クライアントの生成知識が利用側に混在（→Factory Methodが必要） | 新しい連携先を追加するたびに、統括クラス（`BatchExecutor`）の修正が必須になる | `BatchExecutor` が、連携先クライアントの「具体的なクラス名」と「インスタンス化の方法」を直接知っているから。 |
...
今の `BatchExecutor` と各クライアント、および通知サービスとの接続は、巨大なハブに対して、各機器の専用ケーブルが直接差し込まれている状態（具体×間接）だと言えます。
...
案3でインターフェース化を導入したことで、連携先クライアントをインターフェース経由で差し替えられるようになりました。しかし、まだ2つの問題が残っています。

1. **生成の問題**：どのクライアントを生成するかの判断が`BatchExecutor`に残ったまま。連携先が増えるたびに`BatchExecutor`を修正しなければなりません。
2. **通知の問題**：通知先が増えるたびに`BatchExecutor`を修正しなければなりません。通知先の変化軸がまだ密結合です。
...
まず「生成の問題」を解決します。どのクライアントを生成するかの判断を`BatchExecutor`から切り離し、生成専門のクラスへ移します。

しかし生成を分離しても「通知の問題」はまだ残ります。そこでさらに、通知先をリストで管理し変動に対応できる仕組みを加えます。

最後に、これらの複数サブシステムへの接続をまとめて管理するために窓口クラスを設けます。
