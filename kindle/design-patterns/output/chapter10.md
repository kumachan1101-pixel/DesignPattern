...
フェーズ6の出発点は、フェーズ3で変更要求（新しい連携先の追加）を当てて痛んだ「変更途中コード」です。変更要求を容れる前の現状コードには戻しません。`BatchExecutor` に通信・通知・生成が混じって増えたコードから、同じ形で扱える共通点（連携先ごとに差し替わる通信・通知・生成の契約と、実行手順の骨格）を抜き出し、変わる差分を接続点の外へ出す形へ整理していきます。読者が「痛み → 共通点の発見 → 抽象化」の順で追えるよう、最初の小さな案も、この変更途中コードを整理する形から始めます。
