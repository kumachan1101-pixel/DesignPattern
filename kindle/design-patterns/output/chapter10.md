...
これまでの分析を、フェーズ6の対策検討に向けた課題として整理します。
...
| 手段C：生成を内部に集約（振り分け関数） | `if-else` で具体型を生成するヘルパーをメンバに持つ | 外部には型を隠せるが、内部では具体型を知っている。次のステップ（間接）に近い発想 |
...
フェーズ6でどこまで進化させるかが決まりました。次のフェーズ7では、このステップを実際にコードとして実装し、採用した構造の名前を確認します。
...
| 🔴 フェーズ6：対策検討 | Step 1〜Step 4を並べ、各ステップで手段を比較しながらコスト天秤をかけてStep 4を採用した。 |
