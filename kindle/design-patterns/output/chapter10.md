...
| ケーブル比喩 | コードの対応箇所 |
|---|---|
| 「具体」＝専用規格ケーブル | `if (targetId == "A") { SystemAClient client; ... }` / `{ SystemBClient client; ... }` — 連携先の具体クラス名を `if-else` 分岐にハードコードしている |
| 「間接」＝アダプター経由 | `BatchExecutor::execute(targetId)` というメソッドが呼び出し元と各クライアントの間に立ってルーティングを行っている（ただしハブ内部には専用回路が直結） |
