...
★仕様の整理でなぜクラス名が登場するのか。クラス名の説明は後続であります。主要システム↔外部連携バッチシステム↔外部システムのデータのやり取りや各システムの保存データを記載した方が全体の仕様のイメージやデータの流れが分かるのでは？システムAとシステムBと接続するのでしょうか。ほかの章も同様の観点で見直してほしい。この図が出来上がれば、仕様整理図は必要なのか？仕様とシステムの図を分けるとか？システム内の処理の動きを整理したいという事であれば、外部連携バッチシステムに何を入力すると、何が行われ、何が出力されるのか、構造的に示して。全章、同じ見方が出来る形にしてほしい。
...
★どことも関係がないクラスはあり得ません。全章見直して
★DeliveryResultがいない。現状コードでも実装されていない。なぜ、こんな状態になっているのか、原因を突き止めて。
