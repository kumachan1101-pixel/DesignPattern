...
この時点では、連携・通知・生成をどう分けるかは判断しません。まず、バッチが何を受け取り、どこへ送り、どの結果を返すかを確認します。
...
|---|---|---|---|
| 入力 | A社/B社、同期対象データ | A社/B社/C社、同期対象データ、Slack通知先 | 連携先と通知先が増える |
| 判定 | 連携先は有効か、データは送信可能か | C社を含めて有効か、通知先は有効か | 連携先判定と通知先判定が増える |
| 加工 | 送信用形式へ変換し外部連携する | C社へも連携し、完了後にSlack通知する | 通信と通知の加工が増える |
| 出力 | 同期結果 | 同期結果とSlack通知結果 | 通知結果も追う |
...
|---|---|---|
| `BatchExecutor` が連携先ごとの通信詳細を直接知っている | バッチの実行順序から、連携先ごとの送信処理を切り離す | まず送信処理を関数化し、通信詳細がどこに残るかを見る |
...
今回の課題は、外部連携先、通知先、連携クライアント生成という3つの変化軸が同じ実行処理へ集まっていることです。1つの構造で全部を解こうとするのではなく、どの案がどの軸に効くのかを分けて考えます。
