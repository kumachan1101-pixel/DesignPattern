...
**変わる理由がそれぞれ異なる複数の責任が一か所に集まっていると、どの変化が来ても同じ場所を修正しなければならなくなる。変化の種類だけ、その場所が揺れ続けるという状況が起きている。**
...
```mermaid
flowchart LR
    I["入力<br/>連携先、同期対象データ、実行要求"]
    P["加工<br/>認証する、形式を変換する、外部へ送信する、関係先へ通知する"]
    O["出力<br/>同期結果、通知結果、エラー情報"]
...
| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 連携先、同期対象データ、実行要求 | 連携先名、送信データ、バッチ実行指定 | 動作例テーブル、`main()`、変更要求 |
| 認証、形式変換、外部送信、通知 | 連携先ごとの認証・変換・送信・通知処理 | 現状コードの観察、連携先追加時の変更シミュレーション |
| 同期結果、通知結果、エラー情報 | 実行結果の同期/通知メッセージ | 動作例テーブル、実行結果、変更シナリオ表 |
...
このフロー自体はどの連携先でも変わらない「守りたい骨格」です。一方、各ステップの中身（どの会社のAPIに接続するか、どのサービスへ通知するか）は連携先ごとに変わります。この「変わらない流れ」と「変わる中身」の分離が、この章の核心になります。
...
> BatchExecutor の中で直接 new SystemCClient() と書いてしまうと、連携先が増えるたびにバッチ実行のコアロジックを修正することになります。Factory Methodを導入することで、BatchExecutor は「誰かが作ってくれたクライアントを使うだけ」になり、新しい連携先が増えたときの主な変更を生成側へ寄せられます。
