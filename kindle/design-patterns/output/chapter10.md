...
| 要求ID | 確定要求 | 入力 | 受入条件 |
|---|---|---|---|
| R1 | C社の外部連携を追加する | C社設定、同期要求、注文データ | C社へ送信し、既存と同じ結果契約で保存する |
| R2 | 各社の成功・失敗をSlackへ自動通知する | 各送信結果 | A社・B社・C社の各結果がSlackへ1件ずつ通知される |
| R3 | A社・B社・C社のジョブを登録順に一つのバッチで実行する | 順序付きジョブ列 | 実行ログの順序が登録順と一致する |
| R4 | 途中の送信失敗を保存・通知し、後続ジョブを続ける | 各ジョブの送信成否 | B社を失敗させてもC社が実行され、3件すべての結果が残る |

`DeliveryResult`と`BatchLog`は変更前から維持する共通基盤、`NotificationResult`と`NotificationLog`はR2の受付成否を呼び出し元へ返す内部契約です。D社・Email・ログ基盤、再試行は将来リスクまたは対象外であり、今回の完成コードへは入れません。
...
行1がR1〜R4を一度に確認する受入ケースです。行2・3は変更対象外の既存入口とエラー契約が維持されたことを確認します。
...
|---|---|---|---|
| F1 | D社・E社など連携先がさらに増える（`BatchExecutor` 内の振り分けと送信ジョブ列が変わる） | 近い将来（すでに控えている） | 運用担当者「D社・E社も控えている」 |
| F2 | Slack以外にメール・ログ基盤への通知が追加される（通知処理全体が変わる） | 検討中 | 運用担当者「ログ収集基盤も検討中」 |
| F3 | 順次実行の途中失敗の扱いが増える（送信成否の判定と失敗通知の分岐が変わる） | 連携先の増加に伴い | 運用担当者「結果の通知という仕組みは変わらない」 |
...
2-4のF-IDを、連携先・通知先・失敗方針で変えられるようにする部分と、バッチ実行の安定側へ分けます。「はい」は、フェーズ6で**三つの変化軸を別々に変更しても、順次実行と結果保存へ影響を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：D社・E社など連携先がさらに増える（`BatchExecutor` 内の振り分けと送信ジョブ列が変わる） | はい | 連携先ごとの通信と生成・登録 | 同期要求、登録順のジョブ実行、送信結果の保存 |
| F2：Slack以外にメール・ログ基盤への通知が追加される（通知処理全体が変わる） | はい | 通知先ごとの送信と登録 | 各ジョブ結果を通知へ渡す流れ、連携処理 |
| F3：順次実行の途中失敗の扱いが増える（送信成否の判定と失敗通知の分岐が変わる） | はい | 失敗時の継続・停止・通知方針 | ジョブ単位の結果保存、登録順の実行 |

したがって2-5の出力は、「連携・通知・失敗方針は独立して変えられるようにし、同期要求・順次実行・結果保存は守る」という設計条件です。フェーズ3ではR1〜R4だけを現在の構造へ適用し、F-IDはフェーズ6の構造評価に使います。
...
R1のC社分岐とR2のSlack呼び出しを同じ`execute()`へ追加したため、別々の確定要求が一つのメソッドを同時に修正しました。

加えて、複数連携先を順番に流す順次実行を同じメソッドへ書き足すと、この肥大化はさらに進みます。「A社→B社→C社を順に送る」という実行順の骨格と、「各社へどう送るか」という通信詳細が同じ場所に並び、途中で送信が失敗したときに後続ジョブと通知をどう続けるかという成否判定まで、すべて `execute()` の中へ入ってきます。順次実行という外部手順の骨格、送信失敗の扱い、そして通知の3つが、生成の分岐と一緒に1つのメソッドへ積み上がっていきます。
...
1つ目は、`BatchExecutor`が抱える責任の多さです。R1〜R4を適用すると、全体フローに加えてC社の通信、Slack通知、3社の実行順、途中失敗時の継続判断、具体クライアントの生成まで同じクラスへ入りました。
...
---
> **📌 問題（確定）**
> R1〜R4を適用すると、C社通信、Slack通知、登録順実行、途中失敗の継続判断をすべて`BatchExecutor`へ追加した。異なる要求が同じクラスへ集中し、関係のないA社・B社通信まで再テスト対象になった。
---
...
**P1との対応：** `IExternalClient` を新設しました（窓口構造）。戻り値には1-4から存在する`DeliveryResult`をそのまま使います。`SystemAClient`〜`SystemCClient`が実装し、通信詳細だけを各Clientへ閉じます。ここで結果契約を新設・変更しているわけではありません。
...
**P2との対応：** `INotifier <|.. SlackNotifier` の実装関係を実装しました（通知分離構造）。`BatchExecutor` は具体通知先を知りません。
...
|---|---|---|---|
| F1：D社・E社など連携先がさらに増える（`BatchExecutor` 内の振り分けと送信ジョブ列が変わる） | IExternalClientとIClientCreatorの実装を登録し、順次実行骨格から会社固有処理を分ける | 新Client、Creator、PartnerDatabase、組み立て | BatchExecutorの共通順を守れる。D社・E社は完成コードへ追加しない |
| F2：Slack以外にメール・ログ基盤への通知が追加される（通知処理全体が変わる） | INotifier実装を追加し、BatchExecutorは共通通知契約だけを呼ぶ | 新Notifierと登録・所有箇所 | 連携Clientを守れる。メール・ログ通知は完成コードへ追加しない |
| F3：順次実行の途中失敗の扱いが増える（送信成否の判定と失敗通知の分岐が変わる） | DeliveryResultとBatchRecordで結果を共通化したが、再試行などの運用規則は実行骨格からさらに分ける余地がある | BatchExecutor、将来の実行ポリシー、BatchLog | 一部局所化。未確定の再試行規則は完成コードへ追加しない |

F1〜F3を当てることで、Facade・Observer・Factory Methodの各境界が別々の将来変更を受け持つことを確認します。
...
`execute()` は1件の共通処理、`executeBatch()` はR3・R4の順次実行を担います。`executeBatch()`はジョブ列を登録順に走査し、失敗結果も保存・通知してから次のジョブへ進みます。したがって、`main()`が失敗後の続行を手動で呼び分けるのではありません。
...
行1の実行順はA社→B社→C社です。B社が失敗しても、その失敗結果とSlack通知を残した後にC社が実行されています。これによりR1〜R4を一つの起点操作で確認できます。行2・3では、仕様変更の対象外である手動入口と無効設定の扱いも維持しています。
...
|---|---|---|---|
| R1：C社の外部連携を追加する／P1 | `SystemCClient`と`SystemCClientCreator`を追加し、`BatchExecutor`の共通実行骨格へ接続 | C社の結果を既存と同じ`DeliveryResult`で保存 | 新Client・Creator・組み立て登録 |
| R2：各社の成功・失敗をSlackへ自動通知する／P2 | `SlackNotifier`を通知契約へ接続し、`NotificationResult`を別ログへ保存 | A社・B社・C社それぞれの送信結果をSlackへ通知 | 新Notifierと登録 |
| R3：A社・B社・C社のジョブを登録順に一つのバッチで実行する／P1 | `BatchJob`と`BatchExecutor::executeBatch()`に順序付きジョブ列を渡す | 一つの起点操作でA社→B社→C社の順に実行 | ジョブ登録を担う組み立て |
| R4：途中の送信失敗を保存・通知し、後続ジョブを続ける／P1・P2 | `executeBatch()`が各`DeliveryResult`を受け取ってもループを中断しない | B社失敗を保存・通知した後にC社を実行 | 再試行は今回の対象外 |
...
graph LR
    T1["変更要求：連携先の追加"]
        -->|新規追加| F1["新しいClient + Creator<br>（IExternalClient / IClientCreator実装）"]
    T2["変更要求：通知先の追加"]
...
今回の確定要求R1〜R4について、フェーズ1の構造で必要だった修正と完成構造の結果を対比します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：C社の外部連携を追加する | `BatchExecutor`へC社の生成・通信分岐を追加 | `SystemCClient`とCreatorを登録し、C社へ送信した結果を既存契約で保存 |
| R2：各社の成功・失敗をSlackへ自動通知する | 各社の成功・失敗分岐へSlack呼び出しを追加 | `SlackNotifier`を登録し、A社・B社・C社の結果を各1件通知 |
| R3：A社・B社・C社のジョブを登録順に一つのバッチで実行する | `BatchExecutor`が3社の順序を条件分岐で保持 | 登録順のジョブ列を実行し、実行ログがA社→B社→C社になることを確認 |
| R4：途中の送信失敗を保存・通知し、後続ジョブを続ける | 送信分岐ごとに失敗保存・通知・継続判断を追加 | B社失敗を保存・通知した後もC社を実行し、3件すべての結果を保持 |
