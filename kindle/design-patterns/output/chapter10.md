...
classDiagram
    class BatchExecutor {
        ...
        +executeNightlyBatch(targets)
    }
    class ManualTriggerController {
        -facade ExternalFacade
        +triggerManual(targetId, data)
    }
    ...
    BatchExecutor --> ExternalFacade : 具体×間接
    ManualTriggerController --> ExternalFacade : 具体×間接
    ExternalFacade --> SystemAClient : 具体×直接
    ExternalFacade --> SystemBClient : 具体×直接
    ExternalFacade --> SystemCClient : 具体×直接
```
...
**得られること1**（各パターンの対応箇所の識別）：フェーズ2のヒアリングと分類表で、「窓口の統合」（Facade）・「通知先の増減」（Observer）・「連携先クラスの生成」（Factory Method）という3つの変化がそれぞれ独立して変わりうることを識別しました。「このパターンはシステムのどの変化に対応しているのか」を問う視点が、複合的な設計を読み解く入口になります。
...
**得られること3**（疎結合な連携アーキテクチャの説明）：フェーズ7の変更シナリオ表で、連携先が変わるときは `IExternalClient` の実装クラスのみ、通知先が変わるときは `IObserver` の登録のみ、生成方法が変わるときは `IClientFactory` の実装のみを変えればよい設計を確認しました。「変更が1か所に閉じる理由」を構造から説明できる状態が、チームの設計判断を支えます。
...
外部連携バッチシステムという题材を通じて、複数のパターンが交差する設計を段階的に解きほぐす思考を体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。