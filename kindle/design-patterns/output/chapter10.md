...
| C社（配送管理） | 出荷指示の同期 | 月次バッチ | REST API（独自認証） |
...
責任チェック表でクラスの責任が整理できました。次に、コードの各行が「誰の判断で、何をきっかけに変わる知識か」を確認し、混在している責任をさらに細かく特定します。バッチ処理開発チームとは別の担当者が独立した時期に変更する知識は、責任を分ける有力な手掛かりです。ただし、担当者が違うだけで自動的に責任外とは決めず、変更頻度・共変更範囲・クラスの役割も合わせて判断します。
