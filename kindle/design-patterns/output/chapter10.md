...
ここからの検討は、次の2つのよくある考え方を軸に進めます。1つ目は、**共通部分に着目して共通化し、抽象化する**ことです。2つ目は、**変わる部分を分離して外から注入し、組み立て側は中身を意識せず決まった流れを回すだけにする**ことです。関数切り出し・別クラス化は1つ目、窓口・通知登録・生成の分離は2つ目の実行にあたります。

この章には「連携先の生成」「通知」「通信の詳細」という、変わる理由の違う3つの軸があります。このうち、読者が最初に思いつきやすいのは「連携先の追加を、生成関数の登録だけで済ませる」案でしょう。まずこれをコードで確かめます。
...
public:
    void registerClient(const string& id, function<IClient*()> c) {
        creators[id] = c; // 追加は登録1行
    }
    IClient* create(const string& id) {
...
**この案の評価と残る課題：**
連携先の追加（D社・E社）は、`registerClient()` の登録1行で済み、`BatchExecutor` の分岐を触りません。**生成先の追加だけが目的なら、この案で止める判断はあり得ます。** ただし、この案が解くのは3つの軸のうち「生成」だけです。通知の届け方が増える軸（ログ基盤の追加など）と、各連携先の通信手順そのものの複雑さ（窓口でまとめたい軸）は、このマップでは扱えません。また、`BatchExecutor` と `ManualTriggerController` の2つの呼び出し元が同じ登録済みレジストリを共有するには、レジストリを誰が持ち、どう渡すかという組み立ての設計が別途要ります。以下では、この「生成」の分離に加えて、通信の窓口化と通知の登録制を、それぞれ独立した接続点として分けていきます。
...
| 🔴 フェーズ6：対策検討 | 読者が思いつく生成関数の登録マップ案もコードで試し、3軸のうち生成しか解けない残課題を確認した上で、窓口・通知分離・生成分離を分けたステップ6を採用した。 |
