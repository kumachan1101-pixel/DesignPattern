...
案3でインターフェース化を導入したことで、連携先クライアントをインターフェース経由で差し替えられるようになりました。しかし、**案3では通知の変化軸が残っています**。具体的には以下の2つの問題が残ります。
...
連携先クライアントのインターフェース化は達成できましたが、「誰に通知するか」という軸の変動に対しては、まだ`BatchExecutor`が直接知っている状態です。通知先が1件追加されるだけで`BatchExecutor`を触らなければならないため、通知の変化軸が独立していないことが案3の限界です。
...
案3の限界で確認した通り、「通知の変化軸」がまだ`BatchExecutor`に残っています。案4ではこの残課題を段階的に解決します。
