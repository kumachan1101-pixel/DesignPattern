...
まだクラスの中身は見ません。この段階でつかんでほしいのは「3つの変わる理由を2つの接続課題へ分け、最後に一本へつなぐ」という筋だけです（通信手順と生成は同じ連携先の追加で一緒に動くため、課題ID1として一つの接続点にまとめます）。「どのクラスが生成し、どの契約で実行するか」という具体の結論は、この後の課題ID1・課題ID2で一つずつ決めていきます。決めた結論をまとめて振り返る表は、フェーズ6の末尾（6-4 設計トレース）に置きます。ここでは先に結論表を出しません。
