...
| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 有効な外部連携先を選ぶ | 要求ID1、要求ID5 |
| 機能ID2 | 社内データを外部連携先へ送る | 要求ID2 |
| 機能ID3 | 送信結果を保存する | 要求ID3 |
| 機能ID4 | 送信完了を社内へ通知する | 要求ID4 |
...
|---|---|---|---|
| 機能ID1 | 変更<br/>根拠: 変更ID1 | A社・B社・C社から有効な連携先を選ぶ | 要求ID1、要求ID5 |
| 機能ID2 | 継続<br/>根拠: — | 社内データを指定した外部連携先へ送る | 要求ID2 |
| 機能ID3 | 変更<br/>根拠: 変更ID4 | 成功・失敗を含む各送信結果を保存する | 要求ID3 |
| 機能ID4 | 変更<br/>根拠: 変更ID2 | 各社の送信結果をSlackへ通知する | 要求ID4 |
| 機能ID5 | 追加<br/>根拠: 変更ID3、変更ID4 | 複数ジョブを登録順に実行し、途中失敗後も継続する | 要求ID6、要求ID7 |
...
| リスクID3 | 順次実行の途中失敗の扱いが増える（送信成否の判定と失敗通知の分岐が変わる） | 連携先の増加に伴い | 運用担当者「結果の通知という仕組みは変わらない」 |
...
2-4のリスクIDを、連携先・通知先・失敗方針で変えられるようにする部分と、バッチ実行の安定側へ分けます。「はい」は、フェーズ6で**三つの変化軸を別々に変更しても、順次実行と結果保存へ影響を広げない構造か**を判定するための印です。
...
1つ目は、`BatchExecutor`が抱える責任の多さです。変更ID1〜変更ID4を適用すると、全体フローに加えてC社の通信、Slack通知、3社の実行順、途中失敗時の継続判断、具体クライアントの生成まで同じクラスへ入りました。
...
---
> **📌 問題（確定）**
> 変更ID1〜変更ID4を適用すると、C社通信、Slack通知、登録順実行、途中失敗の継続判断をすべて`BatchExecutor`へ追加した。異なる要求が同じクラスへ集中し、関係のないA社・B社通信まで再テスト対象になった。
---
...
|---|---|---|---|
| バッチ骨格が連携先ごとのClient生成・API手順を知る | C社追加で実行順・結果保存まで修正する | 連携先固有の生成・通信をバッチ骨格から分離する | バッチ順序と外部APIは別の理由で変わる |
| バッチ骨格が具体通知先と失敗処理を直接知る | Slack追加・通知失敗で送信継続処理まで変わる | 通知先の種類と配送をバッチ骨格から分離する | 外部送信と社内通知は別の理由で変わる |
...
課題ID1・課題ID2を、次の三つの観点で一つの完成構造へ変換します。
...
| 課題ID1：生成・所有 | 両入口から具体Client生成を外す | `IClientCreator`と共有Application | Client選択がCreatorへ集まり、生成物をRAIIで破棄する |
...
| 要求ID5 | 未登録・無効な連携先を外部送信せず記録する | `PartnerDatabase`、`BatchExecutor` | スキップ結果だけ保存<br/>**判定:** 合格 |
...
|---|---|---|---|
| 機能ID1 | 変更 | 要求ID1、要求ID5 | A・B社を回帰し、C社と無効先除外を確認<br/>**判定:** 合格 |
| 機能ID2 | 継続 | 要求ID2 | 注文・在庫データの送信を回帰確認<br/>**判定:** 合格 |
| 機能ID3 | 変更 | 要求ID3 | A・B・Cの成功・失敗3件を保存<br/>**判定:** 合格 |
| 機能ID4 | 変更 | 要求ID4 | 各社結果のSlack個別通知を確認<br/>**判定:** 合格 |
| 機能ID5 | 追加 | 要求ID6、要求ID7 | 登録順実行とB社失敗後のC社継続を受入確認<br/>**判定:** 合格 |
...
| 🔴 フェーズ6：対策検討 | 課題ID1・課題ID2を同時に満たす窓口×通知分離×生成分離の完成構造を確定し、3ステップは採用構造の実装順とした。 |
