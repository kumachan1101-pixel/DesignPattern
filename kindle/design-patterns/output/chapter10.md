...
これら2つの根本原因は**それぞれ独立した変化軸**です。

- 連携先が増えても通知先は変わりません
- 通知先が増えても連携先クライアントの生成方法は変わりません

なお、「生成の混在」と「複雑さの露出」はどちらも `BatchExecutor` が連携先の具体クラスを直接知っていることから生じており、同一の変化軸（接続点A）として扱います。

2つが独立しているからこそ、1つのパターンだけでは解決しきれません。
...
---
> **📌 課題（確定）**
> 解くべき課題は2つある。接続点Aでは、連携先クライアントの生成と通信詳細（`SystemAClient` 等の具体クラス）を `BatchExecutor` から切り離し、連携先が増えても `BatchExecutor` を変更しなくて済む構造にすること。接続点Bでは、通知先（`NotificationService` 等）を `BatchExecutor` から切り離し、通知先が増えても `BatchExecutor` を変更しなくて済む仕組みを持つこと。
---
...
| **課題** | 連携先の生成と通信詳細（接続点A）と通知先の仕組み（接続点B）を、それぞれ独立して差し替えられる構造に切り離すこと |
