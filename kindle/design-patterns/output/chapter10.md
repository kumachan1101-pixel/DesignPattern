...
フェーズ6は、フェーズ5で定めた3つの課題——**連携先ごとの通信を切り離すP1／通知先を切り離すP2／クライアント生成を切り離すP3**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを分解して探します。第二部の要は「いくつの独立した変化軸があるかを数える」ことです。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる

フェーズ4で見えた波及は、通信仕様、通知運用、具体Client生成という3つの独立した枝でした。理想グラフでもP1〜P3へ分けます。
...
flowchart LR
    ...
    subgraph Ideal["理想の波及"]
        IR1["連携先を追加"] --> I1["P1: Client実装だけを追加"]
        IR2["通知先を追加"] --> I2["P2: Notifier実装と登録だけを追加"]
        IR3["生成条件を変更"] --> I3["P3: Creator登録だけを変更"]
        IS["変更しない: BatchExecutor・ManualTriggerController"]
    end
```
...
|---|---|---|---|
| P1 | 連携先固有通信をBatchExecutorのif分岐からClient実装へ閉じる | if(partnerId)内の通信を切る。認証→取得→送信とDeliveryResultを守る | 連携先固有通信をBatchExecutorへ戻さない |
| P2 | 通知追加の影響をバッチ骨格からNotifier登録へ縮める | メール・Slack直接呼び出しを切る。送信確定後だけ通知し、部分失敗を記録する | 通知先追加でバッチ骨格を変更しない |
| P3 | 生成条件の影響をバッチ・手動入口からCreator登録へ縮める | 具体Client生成を切る。Creatorの所有権返却と両入口の同一Application利用を守る | 手動入口へ別の生成・送信フローを実装しない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 連携先別送信を同じ通信契約へ出す | BatchExecutorから通信詳細が減る | Client実装だけを差し替える |
| P2 | 通知を同じ結果契約のリストへ変える | 通知先別分岐が減る | 登録・部分失敗集計を共通化する |
| P3 | 具体生成をCreatorへ移す | バッチ本体の生成分岐が減る | Creator登録をComposition Rootへ集める |

ここからP1〜P3に該当する通信・通知・生成の分岐だけをおさらいします。
