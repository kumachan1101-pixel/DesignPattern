...
連携先が2社だと管理しやすく見えますが、ビジネスの拡大に伴って「C社も追加」「D社とも連携」という要求は現実によく起きます。最初に設計の境界を引かないまま追加していくと、コードが複雑になっていく典型的な例です。
...
担当者が複数いるのは、「誰がどの知識を持つか」が分かれているからです。インフラ担当はAPIのプロトコルを知っており、通知先担当チームは通知文面のルールを知っています。この知識の分散は業務上自然なことですが、それぞれの担当者の判断が変わるたびに同じクラスを修正することになると、設計上の問題になります。
...
担当者が3者に分かれているということは、変更の理由も3つの異なる方向から来るということです。「A社のAPI仕様が変わった（インフラ担当の領域）」「Slackを通知先に追加したい（通知担当の領域）」「新しいC社を連携先に追加する（設計者の領域）」——これらは互いに無関係な変更です。この事実が、後のフェーズで「3つの変化軸」として問題の核心になります。
...
> BatchExecutor の中で直接 new SystemCClient() と書いてしまうと、連携先が増えるたびにバッチ実行のコアロジックを修正することになります。Factory Methodを導入することで、BatchExecutor は「誰かが作ってくれたクライアントを使うだけ」になり、新しい連携先が増えてもコアロジックを一切書き換える必要がなくなります。
