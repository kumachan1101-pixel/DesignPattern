...
フェーズ1で、`BatchExecutor` が連携先クライアントの生成・通信・通知処理をすべて直接保持している現状を把握しました。届いた変更要求を踏まえ、この設計における変わる見込みと今回維持する範囲を整理します。
...
フェーズ4で、バッチの本体が「連携先ごとのAPI・認証・通信手順とクライアント生成」と「通知先の種類と配送」という**別々の理由で変わる2つの知識**を、同じ場所へ抱えていることを確認しました。対策は、この2つを別々の責任へ分け、一つのバッチ経路へ再結合することです。この段階では、問題から導いた「窓口集約構造」「生成分離構造」「通知分離構造」という名前だけを使います。一般に共有されているデザインパターン名は、実装と効果を確認した後で結び付けます。
...
**どう解決するか（方針）：** 連携先ごとの通信を共通契約の裏へ揃える「窓口集約構造」とし、どれを作るかの判断は「生成分離構造」として外へ出します。通知は「通知分離構造」とし、送信が確定した後で登録済みの相手へ配ります。
...
ここまで問題から導いた「窓口集約構造」「生成分離構造」「通知分離構造」は、一般に **Facadeパターン**、**Factory Methodパターン**、**Observerパターン**と呼ばれます。ここからは題材を離れ、三つの骨格と接続関係を確認します。
