...
フェーズ3「問題特定」で「今の構造では変更が辛い」という事実が確認できました。次のフェーズ4「原因分析」では、この痛みの原因を構造的に分析します。
...
クラス図の変更として書くと、次の4操作になります。**どんなクラスを新設し、何という名前にするかは、この後の二つの判断で決めます。** ここで確定しているのは操作の種類だけです。
...
- **契約と具体による分離**：バッチ本体に残す順序と、連携先・通知手段の差を持つ契約・具体をセットで決める。
- **生成・注入・実行による組み立て**：実体を誰が生成・所有・登録し、バッチ本体へ契約として渡してどう実行するかを一本で決める。
...
**ここから決めること：** 連携先ごとの通信と通知配送を別々の接続点へどう分け、具体クライアントと通知先を誰が生成・所有し、順次実行の骨格へどう渡すかを決めます。

連携先の追加と通知先の追加は別々の理由で変わりますが、通知は連携処理が返す送信結果を使います。そのため、先に通知本文に必要な連携先名・成否・理由を洗い出し、連携契約の戻り値へ含めます。その結果を通知境界へ渡す一本の経路で二つを接続します。

**出発点。** フェーズ5「課題の確定（5-3）」で決まったのは、連携先ごとの通信・生成と通知先ごとの配送を、バッチの共通フローとは別の変更へ閉じる境界を作ることです。まだ決まっていないのは、**境界を関数・表・クラス契約のどれで表し、どう繋ぐか**です。
...
変更前の `if` の中にあった「どのクライアントを作るか」が、ここにはありません。**では、それを決めるのは誰の仕事になるのか。それは生成の検討で決めます。**
...
**穴が2つ空きました。** `creator`（誰が作る役を渡すのか）と `notifiers`（誰に配るのか）です。**どちらも具体の名前をここに書くわけにいきません。** 書けば分けた意味が消えます。**それを決めるのが生成の検討です。**
...
**ここが終わった時点で、実体はすべて存在しています。** 3つの生成役、通知先、ジョブの並び、それらを持つ組み立て役。**まだ起きていないのは1つだけです。骨格の契約と書かれた場所へ、具体が入ることです。次に、注入として確認します。**
...
**分離と組み立てを通っても、呼び方が変わるかどうかは章によって違います。** そして**どれも、分離と組み立ての結果であって前提ではありません。** 先に見せると、まだ導いていない結論を見せることになります。

**守る範囲との照合：** **登録順実行**と**後続継続**を保っていることを、いま確認しました。並びの順に実行し、1件失敗しても次へ進みます。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
| 生成・注入・実行による組み立て | 本体が具体名を抱える → 組み立て役が生成役と通知先を所有 | ジョブへ生成役を渡し、実行時にクライアントを生成、送信・保存・通知まで実行する | クライアントの生成と破棄、通知失敗後の継続を一経路で追える |

公開入口はジョブの並びを受け取ります。各ジョブに入れた生成役が実行時にクライアントを作り、送信・保存・通知までつなぎます。

#### 代表ケースの実行接続

生成役と通知先が実際に使われるまでを確認します。A社への注文データ連携1件を、ジョブ登録→クライアント生成→送信→通知まで一本で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成＋注入組み立て | `BatchApplication` | 生成役3つと通知先を所有し、`addNotifier()` で登録、`BatchJob` へ生成役を詰める | 実行開始へ |
...
| 4. 注入実行 | `IClientCreator::createClient()` → `SystemAClientCreator` | ジョブに入っていた生成役が `SystemAClient` を作って返す | 骨格の `IExternalClient*` へ入る |
...
**生成の検討で作ったものを、5の後に骨格が捨てます。** 作る場所と捨てる場所が別なので、`createClient()` の戻りは「受け取って所有するもの」だと分離の検算で決めておく必要がありました。
