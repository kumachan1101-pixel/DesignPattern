...
このシステムではバッチ処理であり、即時性への制約は低いため、間接層の導入によるパフォーマンスコストは問題になりません。ただし、外部システムとの連携を含む夜間バッチには完了期限（締め時刻）があります。**タイムアウトと再試行ポリシー**は、外部クライアントクラスの設計方針に直接影響します。この点は案3・案4の選定に影響するため、各案のトレードオフで触れます。
...
* ※ バッチ完了期限とリトライ：外部連携処理が Facade クラスに集約されているため、タイムアウトと再試行ポリシーをここ一箇所に実装できる。
...
* ※ バッチ完了期限とリトライ：外部連携クラスが `IExternalClient` インターフェース越しに差し替えられる構造のため、タイムアウトポリシーや再試行ロジックを実装した具体クラスへの切り替えが、`BatchExecutor` を変更せずに可能。
