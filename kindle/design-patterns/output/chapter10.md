...
| 問題ID3 | 順次実行の途中で送信が失敗したとき、後続ジョブや通知の扱いを実行本体で書き分ける | 変更ID4 |
...
```text
現在：BatchExecutor.execute() が
      通信手順 ＋ 通知先 ＋ 連携クライアント生成 を全部持つ
                      ↓
変更後：3つの変わる理由を別責任へ分け、一直線の経路へ再結合する
   連携先ごとの通信手順 → 窓口契約の裏へ通信を隠す（窓口固定＝Facade）      → 課題ID1
   連携クライアント生成 → 生成を専用の作成者へ委ねる（生成分離＝Factory Method）→ 課題ID1
   結果を届ける通知先   → 一つの結果を登録通知先へ配る（通知連結＝Observer）  → 課題ID2
守る範囲：連携先確認→社内データ取得→外部API送信→結果保存→社内通知の実行順、DeliveryResult・BatchLog、社内データ取得・外部API・通知サービスの境界
```
