...
4. **対策検討：** 全課題IDを満たす入力・実体・処理・結果の全体経路を先に決め、その各地点を契約と具体、実体の組み立てとしてコードへ落とす。完成構造が複数成立するときだけ、構造差と目的を比較する。
...
そして、どの題材でも最後にやっていたことは、同じ四拍子でした。**共通点を見つけて分離し、それを部品として生成し、依存として注入し、実行部は具体を意識しないまま動かす。** 割引ルールも、予約の状態も、通知先も、決済手段も、承認フローも、この順で「変わる部分」を安定した骨格の外へ追い出しただけです。State、Strategy、Observer、Command、Template Method、Factory、Decorator——12章で名前は違っても、やっていた動作は一つでした。デザインパターンの名前は後からついてくる語彙で、暗記する対象ではありません。目の前のコードでこの四拍子を回せること、それが本書を通していちばん持ち帰ってほしい設計の醍醐味です。
