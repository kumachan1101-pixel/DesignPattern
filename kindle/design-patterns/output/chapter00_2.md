...
フェーズ6では、確定課題を二つの判断へ変換します。

1. **契約と具体による分離**――何を境界に残し、変わる判断をどの具体へ置くかを、契約と代表具体を並べて決めます。
2. **生成・注入・実行による組み立て**――具体を誰が作って持ち、どの瞬間に契約として骨格へ渡し、公開入口からどう実行するかを一本の経路で決めます。
...
**組み立てでは、生成を単独で説明しません。** 作った実体がどこへ渡り、公開入口からどの骨格・契約を通って使われるかまで一続きで示します。骨格とは、具体が増減しても維持する選択・委譲・反復・配布・履歴移動などの制御順です。

注入とは、骨格が契約としか書いていない場所へ具体が入ることです。入る瞬間は、呼び出しごとの引き当て、生成物の受け渡し・登録、派生型の生成による決定など章ごとに異なります。この一般的な違いは第0章でだけ説明し、各章では実際に採用した経路だけを追います。

各章のフェーズ6「分離と組み立てのまとめ（6-1）」では、二つの判断を二行で振り返り、代表ケース1件を`生成→注入→実行開始→骨格→契約→具体`の順で追います。考え方を二つにまとめたまま、生成した実体が使われるまでの実行順も確認できます。
...
flowchart LR
    ...
    B["二つの設計判断<br/>契約と具体で分離<br/>生成・注入・実行で組み立て"]
    C["設計判断ごとの<br/>部分クラス図"]
    D["統合した<br/>採用全体クラス図"]
    E["一本の実行経路<br/>生成→注入→実行開始<br/>→骨格→契約→具体"]
    F["フェーズ7<br/>完成コードと検証"]
...
部分クラス図は未採用案ではなく、採用全体図を二つの判断単位に分けたものです。契約と具体は分離の両側として、生成・注入・実行は組み立ての一経路として読みます。
### 掲載コードを手元で動かす
...
**3. 断片を読む前に、二つの判断と実行経路を見てください。**

フェーズ6「分離と組み立てのまとめ（6-1）」には、**契約と具体による分離**と、**生成・注入・実行による組み立て**を二行で示す表があります。その直後の代表ケースでは、実際のコードを`生成→注入→実行開始→骨格→契約→具体`の順で追います。生成コードだけを読まず、その実体が使われる行まで続けて確認してください。
...
| 6：対策検討 | 契約と具体で分離し、生成・注入・実行で組み立てて最終構造を確定する |
...
graph TD
    ...
    PH6["フェーズ6<br>対策検討<br>（分離と組み立てを決める）"]
...
| 🔴 **フェーズ6：対策検討** | 契約と具体で分離し、生成・注入・実行で組み立てる |
...
| 6：対策検討 | 契約と具体による分離 | 接続点へ残す操作・値と、変わる判断を持つ具体をセットで示す。守る処理が具体名を知らず契約だけを呼べるか検算する |
| 6：対策検討 | 生成・注入・実行による組み立て | 誰が具体を生成・所有し、どこで契約として渡し、公開入口から骨格・契約・具体へどう到達するかを一本で示す |
| 6：対策検討 | 6-1：分離と組み立てのまとめ | 二つの判断を二行で振り返り、代表ケースを生成から具体の実行まで追う |
...
契約と具体で分離し、生成・注入・実行で組み立てる → フェーズ6
...
**目的：フェーズ5の全課題を解くシステム構造を、「契約と具体による分離」「生成・注入・実行による組み立て」の二つで確定すること**

> **入力：** フェーズ5の課題ID付き接続点表、完了条件、システム全体の完了状態
> **成果物：** 契約と具体の分離、生成から実行までの接続、部分クラス図、採用全体図

### 二つの判断：分離と組み立て

フェーズ5の接続点定義表とシステム全体の課題から、次の二つを決めます。

| 判断 | システム全体の問い | 決めること |
|---|---|---|
| **契約と具体による分離** | 変わる部分と守る部分を、どこで切るか | 境界へ残す値・型・操作・イベントと、その裏で変わる判断を持つ具体 |
| **生成・注入・実行による組み立て** | 分離した部品を、どう動くシステムへ戻すか | 誰が生成・所有し、どこで契約として渡し、公開入口からどう実行するか |
...
1. フェーズ5の接続点表を読み、契約と具体の両側をセットで決める。
2. 設計判断ごとに、変更対象だけを抜き出した部分クラス図で責任・依存の変化を示す。
3. 部分図を統合し、採用する完成構造を変更前と同じ粒度のクラス図へ反映する。
4. 具体の生成・所有、契約として入る瞬間、公開入口からの実行を一続きのコードと経路図で示す。
5. フェーズ7で全体コードを完成させ、同じ変更依頼を当てて変更影響が縮んだか確認する。

二つを決めた結果、完成構造が一つに定まるなら比較は行いません。全課題を満たし、責任配置や依存関係が異なる完成構造が複数残る場合だけ、システム全体の効果・変更影響・導入コストで比較します。

したがって、対策検討は失敗履歴ではありません。採用設計を、分離の両側と、組み立ての実行経路という二つのまとまりで説明します。
...
本書の全12章は、題材（割引・状態・通知・決済…）こそ違え、毎回この二つをたどります。デザインパターン名は、この流れを**どう実現したか**を後から共有するための語彙にすぎません。
...
| 二つの判断 | 確認すること | 章ごとに変わる代表例 |
|---|---|---|
| 契約と具体による分離 | 条件、計算式、処理手順、状態別動作、生成判断、通知処理のどれを契約の向こうへ出し、どの具体が持つか | 施策ルール、状態クラス、コマンド、通知先 |
| 生成・注入・実行による組み立て | 誰が生成・所有し、何を鍵に契約へ渡し、公開入口からどう呼ぶか | 組み立て役、生成役、実行役、登録処理、状態遷移、継承による差し替え |
...
flowchart TB
    ...
    E["組み立ての部分クラス図"]
    F["採用全体クラス図"]
    G1["代表経路<br/>生成→注入→実行開始<br/>→骨格→契約→具体"]
    G2["6-1<br/>分離と組み立てのまとめ"]
...
ここで最も重要なのは、パターン名を先に当てはめないことです。フェーズ1〜5の事実から分離と組み立てを導き、システム全体の最終構造を決めます。
...
1. フェーズ5の確定課題から、契約と具体の境界を決める。
2. 契約と代表具体を続けて示し、分離がコードとして成立するか確認する。
3. 具体の生成・所有、契約への注入、公開入口からの実行を一つの経路で示す。
4. 二つの判断を統合した完成構造を、変更前と同じ粒度のクラス図で示す。
5. フェーズ2のリスクIDを採用構造へ再適用し、将来の変更先、守れる範囲、残る設計課題を確認する。
6. フェーズ7で全体コードと変更影響を確認する。
...
この章で持ち帰っていただきたいのは、パターン名の一覧ではありません。変更が届いたとき、現状の仕様とコードへ実際に当て、痛みを観測し、原因から課題を確定して、契約と具体で分離し、生成・注入・実行で組み立てるという順序です。
