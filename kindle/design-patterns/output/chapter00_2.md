...
src/app/
  main.cpp                 # 生成・登録・注入を行う組み立て場所
```

| 配置先 | 書くもの | 書かないもの |
|---|---|---|
| `.h` | 公開するクラス宣言、契約、引数・戻り値に必要な型 | 長い判定手順、具体的な組み立て |
| `.cpp` | `.h`で宣言したメソッドの処理本体、外へ見せない補助処理 | 他の利用側が知る必要のない実装詳細を公開しない |
| `main.cpp` | 実体の生成・所有・登録・注入、公開入口の呼び出し | 個別ルールの判定式や業務処理本体 |

インターフェースのように処理本体を持たない型は、宣言と仮想デストラクタなどの短い定義をヘッダーへ置き、対応する`.cpp`を作らない構成もあります。反対に、テンプレートなど定義を利用側から見せる必要があるC++固有の例外もありますが、本書の掲載コードでは扱いません。また、`OrderRequest`と`OrderResult`のような小さな値の型は、同じ機能境界で一緒に変更されるなら`OrderTypes.h`へまとめても構いません。**1掲載ブロック＝1ファイルではなく、掲載は読解、ファイルは依存と変更単位の判断**です。
...
| 箱の中の記号 | 意味 | C++で確認する場所 |
|---|---|---|
| `<<abstract>>` | 直接生成できず、共通実装や状態も持てる抽象基底クラス | 純粋仮想関数を1つ以上含み、業務上の共通実装またはデータメンバーも持つ |
| `<<interface>>` | 利用側と実装側を結ぶ、契約だけの抽象クラス | データメンバーや業務上の共通実装を持たず、公開する業務操作が純粋仮想関数になっている。仮想デストラクタの既定実装は持てる |
| ステレオタイプなし | 通常のクラス。別クラスの基底になる場合もある | 純粋仮想関数がなければ直接生成できる。継承の有無だけで`abstract`や`interface`にはならない |
...
|---|---|---|---|
| 1 | 実線＋白抜き三角 | 継承。派生クラスが、共通実装を持つ抽象基底または通常の基底クラスを`public`継承する | 共通実装を受け継ぎ、一部を上書きするとき |
...
このブロックでは `MemberDiscountRule` の定義だけを確認します。
...
このブロックでは `DiscountCatalog` の定義だけを確認します。
...
このブロックでは `CheckoutService` の定義だけを確認します。
...
ただし、この問いから完成経路やコードを一足飛びに作ることはできません。各章では、フェーズ4・5で確定した事実を、次の判断表へ一度変換します。

| 前工程で確定した事実 | ここで決めること | 判断 | 全体経路への反映 |
|---|---|---|---|
| 原因ID・課題ID・今回守る責任 | 変わる判断と守る順序をどこで分けるか | 境界へ残す入力・戻り値・イベントと、具体が持つ判断を決める | 公開入口から契約・具体へ進む地点を置く |
| 変更が起きる時点と、同じ実体を使う範囲 | 起動時か要求時か、都度生成か共有か | 生成時点、所有者、生存期間を決める | 生成・登録・選択・受け渡しを実際の順に置く |
| 具体を決める業務上の値 | 誰が、何を鍵に選ぶか | 選択役、鍵の取得元、選択結果の渡し先を決める | 入力から選択・実行・結果までを結ぶ |

左から右へ答えたあとで、右端を実行順に並べたものが「全体のデータと実体の流れ」です。この段階ではクラス名・メソッド名を役割の識別に使っても、C++のメソッド本体は書きません。全体経路が後続コードの設計条件となり、次の節で初めて宣言と処理本体へ落とします。

### 全体経路から採用設計を組み立てる

フェーズ5で確定した複数の課題IDは、独立した接続点（変化軸）を追跡するためのものです。フェーズ6では課題ごとに別の設計を採用せず、すべてを同時に解く一つのシステムとして考えます。

1. フェーズ5の接続点表を読み、事実を「ここで決めること」へ変換する。
2. 生成時点・所有者・選択条件・境界を通るものを判断表で決める。
3. 判断表の右端を並べ、入力から結果までの全体経路にする。
4. 経路上の各操作を、生成・登録・選択・受け渡し・実行など、実際に行う名前で区別する。
5. 契約と具体をセットで示し、経路の接続点がコードとして成立するか確認する。
6. 生成・所有と、実体を利用側へ渡す箇所を示し、同じ実体が実行まで届くか確認する。
7. 部分図を統合し、採用する完成構造を変更前と同じ粒度のクラス図へ反映する。
8. フェーズ7で全体コードを完成させ、同じ変更依頼を当てて変更影響が縮んだか確認する。

全体経路とその実現方法を決めた結果、完成構造が一つに定まるなら比較は行いません。全課題を満たし、責任配置や依存関係が異なる完成構造が複数残る場合だけ、システム全体の効果・変更影響・導入コストで比較します。

したがって、対策検討は部品を一つずつ発見する失敗履歴ではありません。前工程の事実から判断を導き、その判断をつないだ全体経路を示してから、経路を成立させる構造とコードを説明します。
