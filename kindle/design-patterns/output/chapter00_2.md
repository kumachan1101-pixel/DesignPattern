...
```mermaid
graph TD
    S0["ステップ0<br>システム把握<br>（クラス構成・責任一覧）"]
    S1["ステップ1<br>実装コードを読む<br>（責任チェック）"]
    S2["ステップ2<br>仮説を立て・確定する<br>（観察→ヒアリング）"]
...
| 🟣 **仮説立案** | ステップ2 | S0・S1の観察から仮説を立て、ヒアリングで確定する |
| 🟣 **問題特定** | ステップ3 | 変更を試みたとき、何が痛いかを発見する |
| 🟠 **原因** | ステップ4 | 痛みの根本にある設計の問題を言語化する |
| 🟡 **課題** | ステップ5 | 解決する必要がある対象を具体的に定義する |
| 🔴 **対策案検討** | ステップ6〜7 | 分け方と接続の形を決め、コストで選ぶ |
| 🟢 **対策実施** | ステップ8 | 実装し、設計の成果を確認する |

色分けは局面を示しています（青＝現状把握、紫＝仮説立案・問題特定、橙＝原因、黄＝課題、赤＝対策案検討、緑＝対策実施）。以下では各局面の役割とステップを順に見ていきます。

> **【用語の対応について】**
> この図では「ステップ0〜8」という名称を使っています。各章の本文では同じ内容を **「フェーズ1〜7」** と呼んでいます。これは「7つの局面」と「9つのステップ」という2つの粒度で同じプロセスを表しているためで、指しているものは同じです。
>
> | 本文の呼び方 | 対応するステップ |
> |---|---|
> | 🔵 フェーズ1：現状把握 | ステップ0〜1 |
...
> | 🟢 フェーズ7：対策実施 | ステップ8 |
>
> 章を読み進めるうちに自然と対応関係が身につきますので、最初は気にせず読み進めてください。

ステップ0〜2は、仕様変更が来る前に完了している「準備の局面」です。それぞれ異なる対象に対してシステムを理解し、変更を安全に扱える状態を整えます。

| ステップ | 読む対象 | やること |
|---|---|---|
| **ステップ0** | クラス構成の概要（仕様表・責任一覧） | クラスの責任を把握する（事実のみ。この段階では仮説を立てない） |
| **ステップ1** | 実装コード（`if` 文の中身・各行） | 各行が責任範囲内かを確認し、責任外の知識を見つける |
| **ステップ2** | S0・S1の観察 → 関係者（ヒアリング） | 観察から仮説を立て、「なぜ変わるのか・誰が決めるのか」を確認して確定する |

> **【ステップ2のコツ：「誰が決めるか」を必ず確認する】**
> 変動/不変の仮説を立てるとき、「この仕様は変わりそうか？」と問うだけでは不十分です。「**誰の判断で変わるか**」まで確認することで、初めて分離の根拠が固まります。
>
> ヒアリングで確認する必要がある問いは次の3つです：
...
「どのクラスが何を担当するか」という事実を把握することが、このステップの唯一の目的です。

この段階では仮説を立てません。観察した事実をステップ2に持ち込み、そこで初めて「変わりそうか・変わらないか」を考えます。
...
システムの全コードを読むわけではありません。ステップ0で把握したクラス構成に基づき、「今回の仕様変更の影響を受けそうなクラス」や「変更を加える予定の箇所」に当たりをつけ、その対象クラスの実装コードを1行ずつ読んでいきます。
...
### ステップ2：仮説を立て、確定する ―― S0・S1の観察から仮説を作り、ヒアリングで裏付ける

> **入力：** ステップ0のクラス構成・責任一覧 ＋ ステップ1の責任チェック結果
> **産物：** 確定した変動/不変テーブル（根拠付き）。「誰の判断で変わるか」が明記されたもの。

**このステップでやること（2段構え）**

S0・S1で観察した事実を踏まえ、ここで初めて「何が変わりそうか」の仮説を立てます。その後、関係者ヒアリングで仮説を確定します。

**第1段：仮説を立てる**

> 「このシステムの中で、何が変わりやすく、何は変わらないか？ そしてそれは『誰の決定（都合）』で変わるのか？」

S0で把握したクラスの責任一覧を見れば、「このクラスは営業部長の施策変更で変わりそうだ」「このフローは会社の根幹だから変わらない」というように、変更の決定権を持つ人ベースでの仮説を立てることができます。
...
ヒアリングの結果がS0・S1の観察から立てた仮説と食い違うことがあります。「変わると思っていたが、変わらない」「変わらないと思っていたが、実は頻繁に変わる」——この逆転は設計判断を変えます。
...
**具体×直接**は最もシンプルな形です。Aは「Bが何をするか」を直接知っています。「責任の整理」が目的なら、はじめにここから始めれば十分です。
...
| 原則 | 使いどころ |
|---|---|
| **原則1：変わるものをカプセル化せよ** | 「分ける（案1〜N）」か「分けない（案0）」かの判断軸。変わる理由が異なるものが混在しているなら分ける |
...
3つの原則は「どこまで変えるか」の価値基準です。最終的にどこで止めるかは、ステップ7のコスト天秤で判断します。
...
この5案が「対策案の基本セット」です。どの案を選ぶかはステップ7のコスト天秤で決めます。
