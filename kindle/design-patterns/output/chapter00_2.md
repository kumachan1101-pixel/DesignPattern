...
| フェーズ6内：痛みから構造変更を描く | フェーズ3・4の変更影響グラフを左側に引き継ぎ、どのコード変更で痛みの経路を切り、同じ要求の到達先を縮めるかを描く |
| フェーズ6内：課題と候補を一続きにする | 現在→理想の差、境界・契約、完了条件、最初と次の候補を同じ課題IDの横一行へまとめる |
| フェーズ6内：ステップ1〜n | 各案が何を解き、何を残すかを順に試す |
| フェーズ6内：採用する形を決める | 変更頻度・将来リスク・初期投資コストを比べ、今回採用する形を選ぶ |
...
| 複雑さの種類 | フェーズ1で見ること | フェーズ4・5で課題にすること | フェーズ6で比べる対策 |
|---|---|---|---|
...
大切なのは、複雑さを見つけた瞬間にパターン名へ飛ばないことです。まずフェーズ1で「何が入力され、どの順序で加工され、どの状態や出力になるか」をそろえます。次にフェーズ2で「どの仕様が変わりそうか」を見ます。フェーズ3で変更を当てたときの痛みを確認し、フェーズ4・5で原因と課題を言葉にしてから、フェーズ6で対策を比べます。
...
フェーズ5で作る課題文は、フェーズ6の対策案を生むための設計仕様です。ここで「何をどこから切り離すか」「接続点には何を残すか」「何を変えずに守るか」が曖昧なままだと、フェーズ6はパターン名や好みで案を選ぶ場になってしまいます。

課題文は、次の形まで落とします。

```text
【変えたいもの】を【変えたくないもの】から切り離し、
接続点には【境界を越えて渡す値・型・操作・イベント】だけを残す。
ただし【守る制約】は維持する。
```

たとえば「割引ルールを価格計算の骨格から切り離し、接続点には注文情報を受け取って割引額を返す操作だけを残す」のように書ければ、フェーズ6で比較すべき案が見えてきます。関数で足りるのか、クラスへ移すのか、契約を定めるのか、組み立て箇所を分けるのかを、この課題文から逆算できます。
...
フェーズ5の出口は、フェーズ6の入口です。この時点では対策の形を決めず、課題ごとに次の3点を確定します。

| フェーズ6へ渡すもの | 内容 |
|---|---|
| 課題ID | 独立して扱う変化軸をP1、P2のように識別する |
| 現在の変更影響 | フェーズ3・4の変更影響グラフで、要求がどのクラス・メソッド・テストへ波及したか |
| 変えたくない範囲 | 同じ要求が再来したとき、修正・再テスト対象から外したい業務フローや既存機能 |
...
フェーズ5で課題が明確になりました。フェーズ6では、その課題をコード上の変更手順へ変換します。ここで大切なのは、採用候補の全体コードを最初に見せて説明を後付けしないことです。読者が「この行が課題だから、最初の変更はここになる」「この変更だけでは、この問題が残るから次へ進む」と追える順序にします。

設計判断の出発点は、フェーズ3・4で確認した現在の変更影響です。フェーズ6ではそのグラフを捨てて理想図へ飛びません。左に**問題定義で得た変更前の変更影響**、中央に**影響を切るクラス・契約・依存関係の形**、右に**同じ要求を再適用した変更影響**を置きます。図で示すのは作業手順ではなく、構造を変えると影響経路がどう変わるかです。

次に、現在→理想の差、切る境界・守る契約、完了条件、最初に試す候補、その候補で減る影響、残れば次に試す候補を、同じ課題IDの横一行へまとめます。課題カードと候補を別表に分けません。関連コードを段階変更した後も同じIDを使い、適用したクラス・メソッド、変更結果、守った契約、完了条件の判定を回収します。

### 問題定義の変更影響から、構造と変更後の影響を追う

構造変更グラフは、変更1→変更2という作業の流れを描く図ではありません。左の変更影響を生んでいる依存関係に対し、中央でどのクラス・契約・委譲・登録構造を境界として置くかを示し、右で同じ要求の修正・再テスト範囲がどこまで縮んだかを示す図です。

中央は最終クラス図の全クラスを先出しする場所でもありません。問題となった影響経路を切るために必要な**関連構造だけ**を示します。中央を読むと「どの依存関係を切るのか」、右を読むと「その構造によって変更先がどこへ移ったのか」が分からなければなりません。全クラスを統合した最終クラス図は、コードで構造を検証した後の6-3に置きます。

中央に置いた構造は、この時点では検証対象です。後続の段階的なコード変更で、最小変更から順にコードへ当て、契約を守れるか、影響が実際に減るか、別の問題が残るかを確認します。コードの結果によって中央の構造を修正することもあります。

ここでいう**段階的なコード変更**は、完成コードを便宜的に小分けして実装することではありません。課題の原因に対し、判断・処理・生成責任のどれか一つだけを動かす、結果を判定できる単位の変更です。変更量が小さいかではなく、次の6点を一段ごとに説明できるかで区切ります。

1. **変更が必要な理由**：前段階に残ったどの影響経路を切るのか。
2. **変更対象**：どのクラス、関数、メソッド、分岐、生成、登録を触るのか。
3. **コード上の変更**：判断・処理・生成責任を、どこからどこへ移すのか。
4. **変更後の結果**：どの修正・再テスト範囲が減ったのか。
5. **残った問題と守った契約**：何がまだ残り、入力・出力・順序・副作用の何を維持したのか。
6. **次の判断**：完了条件を満たしたので止めるのか、残った問題を根拠に次の変更へ進むのか。

したがって、関数化から契約導入へ進む場合も、「完成形がインターフェースだから」では進みません。関数化後も呼び出し元の選択分岐が残った、という結果が確認できたときだけ、その判断を境界の外へ移す次の変更を検討します。

| 課題から取り出すもの | コード上で確認すること |
|---|---|
| 現在の不要な波及 | 痛みのグラフでは要求が届くが、構造変更後には届かなくなるノード・経路 |
| 理想で許す変更 | 構造変更後も変更対象として残すクラス・登録・組み立て・テスト |
| 切る境界 | 不要な波及が始まるメソッド・分岐・生成・登録 |
| 守る契約 | 境界を切っても維持する入力、出力、操作名、実行順序、副作用 |
| 完了条件 | 同じ要求を再度当て、構造変更後に許した範囲だけを修正・再テストすれば済むこと |

この読み取りを、次の一表へまとめます。

| 課題ID | 変更の到達点 | 最初に試すコード変更 | 残る問題に対する次のコード変更 |
|---|---|---|---|
| P1 | **現在→理想の差：**【現在から消す波及】<br>**切る境界・守る契約：**【切るコード境界と維持する入力・出力・順序・副作用】<br>**完了条件：**【同じ要求で触らなくする場所】 | **候補：**【原因へ直接効く最小単位のコード変更】<br>**減る影響：**【その変更で消える経路】 | 【残った経路を根拠に行う次のコード変更。残らなければ「なし」】 |

課題IDは表の行番号ではなく、**独立した課題の識別子**です。表では一課題一行とし、P1から欠番なく振ります。構造変更グラフでは、P1を「変更前の影響→中央の構造上の境界→変更後の影響」の一経路に付けます。同じP1を引き渡し表、一続き表、関連コード、コード適用結果、完成コード結果でも使い、一つの課題を最後まで追跡します。段階的なコード変更の順番は本文で説明し、中央の構造図を作業フローにはしません。

候補は課題を作る根拠ではなく、完了条件を満たす手段です。各候補をコードへ当てた後は、「候補を適用したコード」「段階的なコード変更」「その結果」「守った契約」「完了条件の判定」を同じP1で必ず記録します。フェーズ7ではさらに、完成コードの適用先と最終結果をP1で回収します。

### 一つの課題を段階的に解く手順

各課題では、次の流れを繰り返します。

1. フェーズ3・4の痛みの変更影響グラフを左側へ引き継ぐ。
2. 影響を切るクラス・契約・依存関係の形を中央へ置く。
3. 同じ要求をその構造へ再適用し、変更後の変更影響グラフを右側へ描く。
4. 現在→理想の差、境界・契約、完了条件、候補を同じ課題IDの横一行にする。
5. 着目コードに該当するフェーズ3の関連部分だけをおさらいする。
6. 原因へ直接効く最小変更を示し、どの影響が減り、何が残ったか確認する。
7. 残る影響経路が次の変更を必要とする場合だけ進み、適用コードと結果を同じIDで記録する。
8. フェーズ7の完成コードで、完了条件を満たしたか同じIDで最終確認する。

```text
課題A
  変更前の変更影響 → 影響を切る構造の形 → 変更後の変更影響
      ↓ 同じP1で影響経路と構造上の境界を追跡
  差・境界/契約・完了条件・候補
      ↓ 適用先を特定
  着目コード
...
課題Bがある場合は、同じ流れを別に行う
      ↓
最後に課題A・Bを統合し、構造と影響範囲を確認する
```

この順序なら、関数化、クラス分離、契約、登録、生成責任の移動は、あらかじめ決めた儀式ではなくなります。たとえば関数化だけで完了条件を満たすなら、クラスやインターフェースを増やす理由はありません。反対に、関数化しても呼び出し元の条件分岐が残るなら、その残った問題が次の責任移動を考える根拠になります。

ここで最も重要なのは、**「デザイン構造名を最初から思い浮かべない」「クラス図から入らない」**ことです。クラス図（構造）は分解の**結論**であって出発点ではありません。変更要求を当てたときに困っている境界を分解し、共通の形を見つけた結果として、構造が出てきます。

対策検討は、フェーズ5の課題文を次のように読み替えるところから始めます。

| 課題文に出てくる言葉 | 対策候補を考える問い |
|---|---|
...
この表は、パターン名の対応表ではありません。課題文から候補案を出すための思考の補助線です。同じ課題でも、変更頻度が低ければ関数化で止める判断があります。逆に、変更頻度が高く、交換や組み合わせが続くなら、契約や登録、組み立て責任の移動まで進む判断があります。
...
### 対策検討の進め方 ―― 課題ごとにコードを変える（6-1〜6-4）

いきなり採用後の全体コードへ飛ばず、前工程の変更影響、影響を切る構造、変更後の影響を対応させてから、段階的なコード変更でその構造を検証します。各章のフェーズ6は次の順で進みます。

- **変更前の影響→構造→変更後の影響のグラフ**。問題定義の変更影響を左へ引き継ぎ、中央に影響を切る関連構造、右に同じ要求を再適用した変更影響を描きます。中央を変更作業のフローにはしません。
- **課題と候補の一続き表**。現在→理想の差、境界・契約、完了条件、最初と次の候補を同じ課題IDの横一行へ置きます。
- **課題箇所のおさらい**。カードの着目コードに該当するフェーズ3の関連部分だけを示します。
- **6-1 課題と着目コードを対応づける**。課題ごとに、どのクラスのどの部分を見るのか、なぜそこが最初の変更箇所になるのかを示します。
- **6-2 課題ごとに最小変更を積み重ねる**。関連部分の変更前・変更後コードを並べ、「この変更で減ること」「まだ残ること」「次へ進む理由」を毎段階で確認します。複数課題は一度に解かず、課題ごとに分けます。
- **コード適用結果**。課題IDごとに、適用コード、段階的なコード変更、結果、守った契約、完了条件の判定をまとめます。
- **6-3 構造の見立て**。各課題の検討を統合した**結果**として、全クラスを含む対策後クラス図を描きます。図は出発点ではなく結論です。
- **6-4 影響範囲**。各課題の完了条件を使い、変更要求をもう一度当てたときの修正・再テスト範囲を確認します。
- **採用**。途中案を含め、変更頻度・影響範囲・導入コストでどこまで採用するかを決めます。
- **フェーズ7の完成コード結果**。同じ課題IDで、完成コードの適用先、実装後の動作、完了条件の最終確認を示します。

6-2で試す変換（関数へ切り出す、別クラスへ移す、契約を定めるなど）は、一本道の作業手順ではありません。各変換で残った問題が、次の一手を必要とする根拠です。残った問題がないのに「次はインターフェース」と進む説明は、思考ではなく完成形の逆算になってしまいます。

中間コードは課題に関係する部分へ絞ります。コードブロックの外で、対象クラス・対象メソッド・省略している不変部分を明記します。コードブロック内に `...` や「以下省略」を置かず、変更対象のメソッド、必要な契約、利用側、生成・注入箇所は、各段階の判断に必要な範囲を一組として示します。クラスを分離する段階では、分離先だけでなく、誰が生成・所有・注入するかも関連コードとして示します。

クラスを分離するときは、①誰が具体実装を選ぶか、②誰が生成するか、③誰が破棄まで所有するか、④コンストラクタ・登録・生成契約のどれで注入するか、⑤利用側に具体クラス名が漏れていないか、⑥新しい実装を追加すると組み立てのどこが変わるか、の順で確認します。ただし、これらは完成コードを先に見せるための一覧ではありません。各段階で新しく生じた接続上の問いへ答えるために、必要になった時点でコードを追加します。

章ごとにやり方を変えるときも、守るものは同じです。

- フェーズ5で定義した課題から始める。
- まず読者が自然に思い浮かべる案を棚卸しする。
- 各案で、何を移し、接続点に何を残すかを示す。
- 各案が解ける課題と、まだ残る課題を分ける。
- 省略した案や分岐した案がある場合は、論点外・効果不足・導入コスト過多・接続点が別である、などの理由を書く。
- 採用案を、変更頻度・影響範囲・導入コストから決める。

各章のフェーズ6では、最低限、次のような思考を本文に出します。

| 案 | 解けること | 残ること | 今回の判断 |
|---|---|---|---|
| 何もしない | 追加コストはない | 変更要求を当てた痛みが残る | 変更頻度が低い場合だけ候補 |
| 関数化 | 判断や処理に名前が付く | 呼び出し元が条件や順序を知ることは残る | 最初の整理として有効 |
| クラス分離 | 変わる処理を別責任へ移せる | 呼び出し元が具体クラスを知る場合がある | 中間策として有効 |
| 契約・登録・組み立て移動 | 呼び出し元が具体差分を知らずに済む | クラス数、設定、理解コストが増える | 変更頻度と将来リスクが高い場合に採用 |
...
> [!NOTE] 「関数へ切り出す」を飛ばしてよいとき
> 1（関数へ切り出す）が有効なのは、**共通シグネチャがまだ見えていないとき**だけです。分岐が入り組んでいたりメソッド名がばらばらだったりして、切り出して初めて「どれも同じ引数・戻り値の形だ」と気づける場合は、関数化がその共通形を見つける手段になります。
> 逆に、変更途中コードの分岐を見た時点で**共通の形がすでに見えているなら、関数化は名前を付けるだけの空回り**です。その場合はこの段階を飛ばし、直接3（契約を定める＝インターフェースの抽出）へ進んでかまいません。関数化は「共通シグネチャを発見する手段」であって、必ず踏む儀式ではありません。
...
|:---|:---|:---|
| 低い | 低い | 採用しやすい候補 |
| 高い | 低い | 長期重視なら採用。納期優先なら現状維持の暫定対応＋後日作り直す計画を立てる |
...
この考え方を練習すると、未知の問題でも、変更要求と原因から候補となる構造を比較しやすくなります。既知の構造は、その候補を検討し、意図を共有するための重要な語彙です。以降の章では、この考え方を具体的なコードに適用し、どの判断がどの構造へつながるのかを追体験します。
