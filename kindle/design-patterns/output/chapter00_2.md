...
> [!TIP] この章の読み方
> この章は全7フェーズの詳細な説明書です。最初から通読してもよいですが、**各フェーズの冒頭にある「目的」と「産物」だけ読んで先の章へ進み、疑問が出たときに戻ってくる**という使い方が馴染みやすいかもしれません。「このフェーズで何をやっているんだろう？」と感じたら、この章の該当セクションが手がかりになります。
...
| 1-3：クラス構成図 | 図を見る前に登場クラスの責任を把握し、依存関係を追えるようにする |
...
> **産物：** 確定した変動/不変テーブル（根拠付き）。今回確実に変わることと、将来リスクが分けて整理されたもの。
...
フェーズ1で観察した事実を踏まえ、「何が変わりそうか」の仮説を立て、今回確実に変わることを切り分け、ヒアリングで裏付け、最終的に変動/不変テーブルとして確定します。第1章以降では、2-1で仮説を立て、2-2で今回の変更を固定し、2-3で関係者に確認し、2-4で将来リスクを整理し、2-5で次のフェーズに渡す見通しをそろえます。
...
> 変動/不変の仮説を立てるとき、「仕様は変わりそうか？」という漠然とした問いより、「この仕様は、どんな事業上・技術上の理由で変わりそうか？」と問う方が精度の高い仮説になります。
>
> ヒアリングで確認する問いは次の3つです：
...
**フェーズ2の末尾：将来の変更仕様の見通し**

ヒアリングで判明した将来リスクが実際に変更として届いたとき、仕様がどう変わるかをテーブルで整理します。これはフェーズ3以降の設計判断の材料です。「現在（今回追加後）」と「将来（数ヶ月後）」を対比し、今回の変更と将来の変更を同じ軸で見えるようにします。将来の仕様がまだ固まっていない場合でも、「どの変更軸が将来も繰り返されうるか」という視点で整理します。

> **第1章での体験箇所：** 「2-1：変わりそうな仕様の見当をつける」〜「将来の変更仕様の見通し」で、割引ルールが今後も変わり続けることの仮説を立て、マーケティング担当者とのヒアリングで事実として裏付け、将来のクーポン変更がどう届くかをテーブルで整理します。
