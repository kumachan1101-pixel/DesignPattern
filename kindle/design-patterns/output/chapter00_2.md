...
| 5：課題定義 | 接続点を特定する | 変更が波及する境界を見つけ、課題ID・流れるデータ・変わる側・守る側を一表で確定する |
| 5：課題定義 | システム全体の課題を確定する | 現在の波及と、対策後に守りたい範囲を文章化する。クラスや接続方法はまだ書かない |
| 6：対策検討 | 接続点を変える三つの観点 | 分離方法で境界、配置場所で責任の置き場、組み立て方法で生成・所有・登録・選択・注入を決める |
| 6：対策検討 | 最終構造を決める | 三つの観点から完成構造が一つに定まるなら比較せず、実現可能な完成構造が複数残る場合だけ目的と構造差を比較する |
| 6：対策検討 | 責任を見直す | フェーズ1のクラス図を同じ粒度で更新し、着目クラスを色で示す |
| 6：対策検討 | 課題箇所をおさらいする | フェーズ3のコードから、P1〜Pnに直接関係する部分だけを再掲する |
| 6：対策検討 | ステップ1〜n | 確定した構造の責任分離、生成、依存の受け渡し、安定側からの実行を順にコードへ反映する |
| 6：対策検討 | コード適用結果 | P1〜Pnごとに目指した状態・適用コード・結果を追い、システム全体で一度だけ達成を判定する |
...
また、変更前後を比べる前に、変更要求を根拠として「今回変える要素」と「変えない共通基盤」を分けます。保存方法、既存の結果契約、入力検証、履歴、外部境界が変更要求に含まれないなら、変更後だけへ追加したり、変更前から削除したりしません。1-5では誤って作り替えやすい共通基盤を「変更なし」と明記し、1-4と7-1の差分は変更要求または課題IDで説明できるものだけにします。
