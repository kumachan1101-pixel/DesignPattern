...
フェーズ5で課題が明確になりました。フェーズ6で決めるのは**実装ではなく設計**です——接続点の契約、データの置き場所、構造（クラス図）、影響範囲を決め、採用案を選びます。動く実装一式（`main`・実行結果）は次のフェーズ7で書きます。フェーズ名のとおり、6は「対策を検討して決める」、7は「対策を実施する」場です。

出発点は、フェーズ3で仕様変更を当てた後の「痛みコード」です。フェーズ1の変更前コードへ戻すのではありません。この痛みコードを**分解して、接続点の"形"（共通のシグネチャ＝契約）を見つける**ところから始めます。追加済みの要求はすべて保ったまま分解し、設計を簡単に見せるために仕様を落としていないかを確認します。
...
- **6-1 痛みコードを分解して、接続点の「形」を探す**（思考の中心）。痛みコードを実際に変換し、詰まる場所から共通の形（同じ引数・戻り値のシグネチャ）を見つけます。ここで書くのは動く実装ではなく、「こう変えるとどこで詰まるか」を確かめるスケッチです。
- **6-2 見つけた形を契約にし、データの置き場所を決める**。共通シグネチャを契約（インターフェース）として定義し、どのデータをどのクラス・リポジトリが持つか（データ配置）を決めます。契約は定義と骨だけを書き、実装本体はフェーズ7へ回します。
