...
設計判断の出発点は、フェーズ3で仕様変更を当てた後の「痛み」です。元の構造を読者の記憶に委ねないため、フェーズ6の冒頭には、**フェーズ1で確認した現行コード全体**を振り返りとして一度だけ掲載します。その直後に、フェーズ3で変更要求を試した関連部分を「痛みの差分」として並べます。現行コード全体と痛みの差分を一組にすることで、全体構造を見失わず、対策の起点もフェーズ3から動かしません。

その後は全体コードを繰り返しません。各課題に関係するクラス・メソッド・組み立て箇所だけを抜き出し、変更前と変更後を段階ごとに示します。採用後の全体コードはフェーズ7で掲載し、動作例と実行結果で確認します。
...
- **現行コード全体と痛みの差分**。フェーズ1の全体コードを一度だけ掲載し、続けてフェーズ3で変更した関連部分を示します。この一組が全段階の比較基準です。
