...
graph TD
    ...
    F6["フェーズ6<br>対策検討<br>（案を比べ、採用案を決める）"]
...
フェーズ4で「分けるべき場所がある」という判断ができました。しかしそのままフェーズ6の対策検討に進むのは早すぎます。「分ける」という判断だけでは、まだ何を解くべきかが曖昧なままです。
...
> **この本のコード例について：** 各章で扱うコードは学習用の小規模なサンプルです。仮想関数のオーバーヘッドやスレッド安全性を気にする必要はありません。しかし実際の大規模システムでは、接続の形を選ぶ前にこれらの制約を確認することが重要です。「抽象×間接」が理想でも、ホットパスにある接続点なら「具体×直接」を意図的に選ぶ判断もあります。
...
設計の良し悪しを決める「依存の形」を、身近な充電器に例えて整理してみましょう。

前提として、起点は「コンセントに挿さった充電器と、そこから直に生えている1本のケーブル」で完全に固定されているとします。私たちは、この充電器本体を分解したり作り直したりすることはできません。この変わらない起点から、「その先をどう設計するか」という工夫だけで、システムの柔軟性がどう変わるかを4つの段階で一緒に見ていきましょう。

| 接続形態 | ケーブルの先端（端子） | 接続の工夫 |
| --- | --- | --- |
| **① 具体 × 直接** | ライトニング（専用規格） | そのまま挿す |
| **② 抽象 × 直接** | Type-C（共通規格） | そのまま挿す |
| **③ 具体 × 間接** | ライトニング（専用規格） | ゲーム機用アダプタを挟む |
| **④ 抽象 × 間接** | ライトニング（専用規格） | Type-C変換アダプタを挟む |

#### 【1. 具体 × 直接】 ―― ライトニング直生え × iPhoneへ直接

充電器から「iPhone専用のライトニングケーブル」が直に生えており、それをiPhoneに直接挿している状態です。iPhoneしか充電できず、iPhoneを手放せばこの充電器も用済みとなります。特定のクラスを名指しで呼び出している、最も変更に弱く硬直した「密結合」の設計です。

#### 【2. 抽象 × 直接】 ―― Type-C直生え × 各種デバイスへ直接（★Strategyパターンの基本）

ケーブルの先端を、最初から世界標準の「Type-C（抽象）」として設計した状態です。Type-Cの規格さえ守っていれば、Androidでも最新のゲーム機でも、どんな相手でもそのまま挿すだけで柔軟に切り替えることができます。起点はそのままに、実行する相手を自在に差し替えられる。これこそが、Strategyパターンの本質です。

#### 【3. 具体 × 間接】 ―― ライトニング直生え ＋ ゲーム機変換アダプタ（★Adapterパターン）

iPhone専用のライトニングケーブルしかない充電器で、どうしても専用端子を持つゲーム機を充電したい。そんな時、先端に「ゲーム機専用の変換アダプタ」を被せて繋ぎますよね。これは、既存の古い専用クラスを捨てずに、中間層（アダプタ）を挟むことで新しいルールに適合させる、まさにAdapterパターンの構造です。

#### 【4. 抽象 × 間接】 ―― ライトニング直生え ＋ Type-C変換アダプタ（★レガシーの救済）

充電器はiPhone専用（ライトニング直生え）の古いものですが、その先端に「Type-C変換アダプタ」を被せた状態です。これだけで、ガチガチの専用充電器が、どんなType-C機器でも充電できる汎用的な充電器（抽象）として生まれ変わります。修正できない古い密結合のコードを、アダプタ一つで最新の世界に繋ぐ、実務で最も救いとなる設計手法です。
...
この4案が「対策案の基本セット」です。どの案を選ぶかはコスト天秤で決めます。

### 「接続の形を変える」とはどういうことか

案1から案4に向かうほど、接続点の抽象度と間接度が上がります。
...
どこまで変えるかは、コスト天秤で判断します。

### コスト天秤にかける ―― 「現在」と「未来」で対策案を比べる

**なぜコスト天秤が必要か**

対策案が複数ある場合、どれを選ぶかを決める必要があります。どんな設計も万能ではなく、必ず何らかのコストを伴うからです。「このコストを払ってまで、この設計を選ぶ価値があるか」を判断します。
...
|:---|:---|:---|
| 低い | 低い | 迷わず採用 |
| 高い | 低い | 長期重視なら採用。納期優先なら案1を暫定対応＋リファクタ計画を立てる |
| 低い | 高い | 案1の暫定対応として採用。リファクタ計画を必ず明記する |
| 高い | 高い | 対策案を再検討する |
