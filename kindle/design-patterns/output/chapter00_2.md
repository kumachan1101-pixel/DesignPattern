...
フェーズ1では、現行仕様と今回確定した変更要求へR1、R2、R3の連番を付け、入力と受入条件をコードより先に固定します。フェーズ2では、将来リスクへF1、F2、F3の連番を付け、確定要求、実装上の内部契約、今回の対象外と分けます。フェーズ3以降で実装してよいのは確定要求だけです。ただしF-IDは捨てず、フェーズ6で採用構造へ再適用して、将来の変更先と守れる範囲を評価します。
...
| 2：仮説立案 | 2-5：変わる見込み/当面安定の前提の確定 | 次のフェーズで当てる変更対象と、守る前提を確定する |
...
| 7：対策実施 | 7-4：変更シナリオ表 | 将来の変更ごとに、主な修正先と確認観点を整理する |
...
フェーズ7では、今回確定したR-IDを実際のコードへ当てる受入確認を行います。将来リスクはフェーズ6でF-IDごとの設計評価を済ませ、7-4では「もし確定した場合に触ると予測される場所」が完成構造と矛盾しないかを確認します。未確定の機能や結果を完成コードへ先回りして実装しません。
...
最初に、完成コードへ登場するクラス一覧、フェーズ6と同じMermaid定義の完成後クラス図、完成コードに対応する実行シーケンスを示します。読者が全体の責任と呼び出し順を把握してから、値・境界・契約・具体クラス・組み立て・`main()`の順に、クラス単位でコードを示します。最後にR-IDごとの実行結果と、変更シナリオごとの「変わるクラス・変わらないクラス」を整理します。
