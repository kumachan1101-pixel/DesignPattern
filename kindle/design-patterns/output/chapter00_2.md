...
フェーズ1では、現行仕様と今回確定した変更要求へR1、R2、R3の連番を付け、入力と受入条件をコードより先に固定します。フェーズ2では、確定要求、将来リスク、実装上の内部契約、今回の対象外を分けます。フェーズ3以降で実装してよいのは確定要求だけです。
...
フェーズ1で観察した事実を踏まえ、「何が変わりそうか」の仮説を立て、今回確実に変わることを切り分け、ヒアリングで裏付け、最終的に変わる見込み/当面安定の前提として確定します。各章では、2-1で仮説を立て、2-2で今回の変更をR-IDとして固定し、2-3で関係者に確認し、2-4で将来リスクを整理し、2-5で次のフェーズに渡す見通しをそろえます。ここで、今回の要求を実現するための戻り値や結果型などは「内部契約」、採用しない動作は「対象外」として別に置きます。将来リスク・内部契約・対象外を、確定要求の機能としてフェーズ3や7へ実装しません。
...
フェーズ7では、今回確定したR-IDを実際のコードへ当てる受入確認を行います。将来リスクは7-4で「もし確定した場合に触ると予測される場所」を示すだけで、未確定の機能や結果を完成コードへ先回りして実装しません。
