...
フェーズ5では、解くべき接続点を洗い出し、「解けた」と言える受け入れ条件を構造に依存しない形で定めます。フェーズ6では、その接続点をどう切るか（分離）、切り出した知識をどこへ置くか（配置）、誰が生成・所有・注入して繋ぎ直すか（組み立て）を検討し、受け入れ条件で候補を評価して最終構造をコードを書く前に確定します。その後の「ステップ1」「ステップ2」は、案が揺れた履歴ではなく、確定した構造を理解できる単位で実装する順序です。
...
| 5：課題定義 | 解くべき接続点を洗い出し、流れるデータと変わる側・守る側、受け入れ条件を定める |
| 6：対策検討 | 接続点の分離・配置・組み立てを検討し、受け入れ条件で候補を評価して最終構造と実装順を確定する |
...
graph TD
    ...
    F5["フェーズ5<br>課題定義<br>（接続点を洗い出し受け入れ条件を定める）"]
    F6["フェーズ6<br>対策検討<br>（分離・配置・組み立てを決める）"]
...
flowchart TB
    ...
    O5["成果物<br>接続点定義表<br>流れるデータ・変わる側/守る側<br>受け入れ条件"]

    P6["6 対策検討<br>分離・配置・組み立てを決める"]
    O6["成果物<br>採用構造と採用理由<br>採用クラス図<br>実装結果"]
...
フェーズ5で接続点と受け入れ条件を定めないままフェーズ6へ進むと、システム全体の構造設計がパターン名選びになってしまいます。
...
| 🟡 **フェーズ5：課題定義** | 解くべき接続点を洗い出し、受け入れ条件を定める |
...
| フェーズ5内：接続点を特定する | 変わるものと安定させたいものの境界を洗い出す |
| フェーズ5内：接続点で流れるデータを整理する | 境界を越えて渡す値・状態・判断材料と、変わる側・守る側を確認する |
| フェーズ5内：受け入れ条件を定める | 変更要求を再適用したとき、修正がどの範囲に収まれば「解けた」と言えるかを構造に依存しない形で定める |
| フェーズ6内：課題を統合する | 各課題ID（＝接続点）の変化軸・現在の波及・守る範囲を一表にまとめる |
| フェーズ6内：分離・配置・組み立てを検討する | 接続点をどう切り、切り出した知識をどこへ置き、誰が生成・所有・注入して繋ぎ直すかを候補として挙げ、受け入れ条件で評価する |
| フェーズ6内：責任を見直す | フェーズ1のクラス図を引き継ぎ、採用した分離・配置を変更前後のクラス図で示す |
| フェーズ6内：採用する形を決める | 受け入れ条件を満たす構造を採用する。単純な代替案も含めて評価し、採用しなかった案は理由とコードイメージを示す |
| フェーズ6内：ステップ1〜n | 採用構造の責任分離、生成、依存の受け渡し、安定側からの実行を順にコードへ反映する |
...
変更要求を当てて痛みを見る    → フェーズ3
痛みの原因を言語化する        → フェーズ4
接続点を洗い出し受け入れ条件を定める → フェーズ5
分離・配置・組み立てを決める  → フェーズ6
...
> - **「課題」**（フェーズ5で定義）：原因を踏まえて「次に解くべき接続点」を具体的に絞り込んだもの。問題（症状）でも原因（診断）でもなく、「どの境界で、何が流れ、どちら側が変わるのか」を特定し、「解けたと言える受け入れ条件」を添えたものです。この時点では、対策の形（どんなクラスを作り、どこへ移すか）はまだ決めません。
> - **「受け入れ条件」**（フェーズ5で定める）：変更要求を再適用したとき、修正・再テストがどの範囲に収まれば課題が解けたと言えるかの条件。「新しいクラス1つで済む」のような構造の形ではなく、「追加される施策の記述と、その利用宣言だけを修正し、計算の流れと既存施策は触らない」のように役割の言葉で書きます。
> - **「対策」**（フェーズ6で確定）：接続点をどう切るか（分離）、切り出した知識をどこへ置くか（配置）、誰が生成・所有・注入して繋ぎ直すか（組み立て）を決めた結果としての最終構造のこと。受け入れ条件を満たし得る候補（単純な案も含む）を評価して採用し、関数化やクラス分離は採用構造を実現するコード変更として扱います。
...
**目的：原因を踏まえて「解くべき接続点」を洗い出して定義し、解けたと言える受け入れ条件を定めること**

### 対策に入る前に「何を解くか」を確定する

> **入力：** フェーズ4で特定した「分けるべき場所」と「接続点の存在」
> **成果物：** 接続点定義表（接続点・流れるデータ・変わる側/守る側）と受け入れ条件

フェーズ4は「なぜ分けるか」に答えました。フェーズ5が答えるのは「その境界で何のデータが流れているか」「どちら側が変わり、どちら側を守るのか」「何ができれば解けたと言えるのか」です。

このフェーズでは、**対策の形をまだ決めません**。どんなクラスを作るか、切り出した知識をどこへ置くか、どう繋ぎ直すかは、すべてフェーズ6の仕事です。フェーズ5でやることは3つだけです。

1. **接続点を洗い出す**：変更要求のたびに知識が漏れて修正が波及する境界を、全部リストにする。
2. **接続点を定義する**：それぞれの境界で、何（値・型・操作・イベント）が流れ、どちら側が変わり、どちら側を守りたいのかを書く。
3. **受け入れ条件を定める**：変更要求を再適用したとき、修正・再テストがどの範囲に収まれば「解けた」と言えるかを、構造に依存しない形で書く。
...
```text
【変えたいもの】と【変えたくないもの】の間の接続点では、
【境界を越えて渡る値・型・操作・イベント】が流れている。
変わるのは【変わる側】であり、【守る側】は安定させたい。
変更要求【X】を再適用したとき、修正が【許す範囲】に収まり、
【守る範囲】が修正・再テスト対象に入らなければ、解けたと言える。
```

たとえば「割引ルールと価格計算の骨格の間では、注文情報と割引額が流れている。変わるのは割引ルールで、計算の流れは守りたい。新しい施策を追加したとき、修正が施策の記述とその利用宣言に収まり、計算の流れと既存施策を触らなければ解けたと言える」のように書きます。この文には、クラス名も、契約の形も、置き場所も出てきません。それらを決めるのがフェーズ6です。

**受け入れ条件の書き方の注意**：「新しいクラス1つと登録だけで済む」のような**構造の形をした条件はここでは書きません**。それは対策の先取りです。「追加される施策の詳細」「既存の計算骨格」のような**役割の言葉**で、修正してよい範囲と守る範囲を書きます。
...
| 接続点 | 接続するデータ（型・値） | 変わる側 | 守る側 |
|---|---|---|---|
| 個別ルールの呼び出し | 引数 `(type: string, optionFlag: bool)` と戻り値 `(int)` | ルールの判定条件と計算式 | 呼び出しの流れと利用側 |
...
このフェーズの出口は、「どの境界を解くべきか」「境界を越えて何が流れているか」「どちら側が変わり、どちら側を守るか」「何ができれば解けたと言えるか」を説明できるところです。

フェーズ5の出口は、フェーズ6の入口です。この時点では対策の形を決めず、接続点（＝課題ID）ごとに次の4点を確定します。

| フェーズ6へ渡すもの | 内容 |
|---|---|
| 課題ID | 独立して扱う接続点（変化軸）をP1、P2のように識別する |
...
**目的：洗い出した接続点をどう切り、切り出した知識をどこへ置き、どう繋ぎ直すかを決め、受け入れ条件で検証できる構造に落とすこと**

### 3つの決定：分離・配置・組み立て

フェーズ5の接続点定義表と受け入れ条件を受けて、フェーズ6では次の3つを順に決めます。この3つが決まって初めて「対策が決まった」と言えます。

| 決定 | 問い | 決めること |
|---|---|---|
| **分離** | 接続点をどう切るか | どの判断・処理・生成・データを、どの境界で切り出すか。境界を越える契約（値・型・操作）は何か |
| **配置** | 切り出した知識をどこへ置くか | 切り出したものを、どの既存クラス・新設クラスへ置くか。守る側には何が残るか |
| **組み立て** | どう繋ぎ直すか | 誰が具体クラスを生成・所有し、どうやって（注入・登録・継承・遷移）安定側へ渡すか。安定側はどの契約だけを呼ぶか |

### 判断を整理してから、採用設計を段階的に実装する

フェーズ5で確定した複数の課題IDは、独立した接続点（変化軸）を追跡するためのものです。フェーズ6では課題ごとに別の設計を採用せず、すべてを同時に解く一つのシステムとして考えます。

1. 各課題IDの変化軸、現在の波及、守る範囲を一表にまとめる。
2. 受け入れ条件を満たし得る構造候補を挙げる。関数化やリスト保持のような単純な案も、読者が思いつくなら候補に含める。
3. 各候補の分離・配置・組み立てを受け入れ条件と変化軸で評価し、採用を決める。採用しなかった案は、理由と（必要なら章末コラムで）コードイメージを示す。
4. 採用構造を変更後のクラス図として確定し、操作・責任・依存の差分を示す。
5. クラス図の差分をコードへ反映し、受け入れ条件に対する実装結果を一度だけ判定する。
6. フェーズ7の完成コードへ同じ要求を当て、変更影響が縮んだか確認する。

したがって、段階コードは「試したら駄目だったので次を考えた」という失敗履歴ではありません。採用設計を一度に完成コードとして見せると理解しにくいため、責任分離、生成・選択、依存の受け渡し、安定側からの実行という意味のある単位に分けて説明します。

### 全章に共通する4つの問い

多くの章に共通する核は、**変わる詳細を分離し、その詳細を生成・選択する責任を安定側から外し、安定した接続点を通して渡し、変えたくない処理は詳細を知らずに実行する**ことです。各章では、次の4つの問いに必ず答えます。

| 共通の問い | 確認すること | 章ごとに変わる代表例 |
|---|---|---|
| 1. 何を分離するか | 条件、計算式、処理手順、状態別動作、生成判断、通知処理のうち、どれが独立して変わるか | 施策ルール、状態クラス、処理ステップ、生成クラス、通知先 |
| 2. どこで生成・選択するか | 具体クラスを誰が生成・所有し、いつどの実装を選ぶか | `main()`、Factory、Context、登録処理、状態遷移処理 |
| 3. どう依存を渡すか | コンストラクタ、メソッド引数、登録、生成メソッド、継承フックのどれで接続するか | 単一依存の注入、複数実装の登録、実行時切替、サブクラス実装 |
| 4. 安定側はどう実行するか | 変えたくない処理が、具体名・条件・生成方法を知らず、どの契約だけを呼ぶか | `apply()`、`execute()`、`handle()`、`notify()`、テンプレート内のフック |
...
変更後クラス図はシステム全体の設計結論です。後続のコードでは、クラス図のどの差分を実装しているかを毎段階で対応づけます。改善後の変更影響グラフは、完成コードへ同じ変更要求を当てられるフェーズ7で示します。

### 課題を統合し、分離・配置を決める

課題IDは、独立した接続点（変化軸）をコードまで追跡する識別子です。課題ごとに採用案や完了判定を持たせず、まず次の表でシステム構造に必要な材料をそろえます。「構造で移す責任」は、フェーズ6で初めて決める配置の決定です（フェーズ5の接続点定義表には書きません）。

| 課題ID | 変化軸と現在の変更影響 | 構造で移す責任 | 変えたくない範囲 |
|---|---|---|---|
| P1 | 【独立して変わる仕様と現在波及する場所】 | 【分離先へ移す判断・処理＝配置の決定】 | 【安定させる処理・利用側】 |
| P2 | 【別の変化軸と現在波及する場所】 | 【分離先へ移す判断・処理＝配置の決定】 | 【安定させる処理・利用側】 |

構造候補の比較も採用も、フェーズ5で定めた**受け入れ条件**を基準にします。「どの変更が来ても、どの範囲だけを修正し、どの範囲を修正しないシステムにするか」は受け入れ条件が既に答えているため、フェーズ6で条件を作り直すことはしません。フェーズ6の仕事は、その条件を**満たす構造を決めて示す**ことです。
...
6. **システム全体への効果**：受け入れ条件のどの部分を実現したか。

次の流れは共通の骨格を3段階で示した例です。分離の単位、生成・選択の場所、注入・登録・遷移の方法が異なるため、実際のステップ数と実装順は章ごとに変えます。ただし、上の4つの問いに答えず、変えたくない側へ具体的な条件や生成判断を残したままにはしません。
...
課題P1・P2…（接続点）と受け入れ条件
  ...
  分離・配置・組み立ての候補を挙げる
      ↓ 受け入れ条件と変化軸で候補を評価
      ↓ 採用理由を確定（採用しない案は理由とコードイメージ）
  採用する変更後クラス図
      ↓ クラス図の差分を実装順へ変換
  ステップ1：変わる詳細と共通契約を分離
  ステップ2：具体クラスの生成・選択場所を実装（配置）
  ステップ3：依存を渡し、安定側を契約呼び出しへ変更（組み立て）
      ↓ フェーズ7
  完成コードへ同じ要求を当て、受け入れ条件を確認
```

ここで最も重要なのは、パターン名を先に当てはめないことです。フェーズ1〜5の事実から4つの問いへ答え、受け入れ条件を満たす最終構造を決めます。本文では結論を先に示し、その構造を実現するコード変更だけを段階的に説明します。
...
この表はパターン名の対応表ではなく、課題の受け入れ条件から必要な責任・契約・依存を導く補助線です。受け入れ条件が単なる関数分離で満たせるならそこで止め、交換・追加・組み合わせまで求めるなら、契約や登録、組み立て責任まで含む完成形を作ります。
...
原則はチェックボックスではありません。インターフェースが不要な章もあれば、仲介役だけで十分な章もあります。受け入れ条件を満たす最小の最終構造を選びます。

### 対策検討の進め方 ―― システム全体の構造をコードへ落とす

フェーズ6で必要なのは、次の5点です。

1. 課題IDごとの変化軸、移す責任（配置の決定）、守る範囲を統合表にまとめる。
2. 受け入れ条件を満たし得る構造候補（単純な案を含む）を挙げ、評価して最終構造を決める。
3. フェーズ1のクラス図を変更前・変更後で示し、移す責任と残す責任を色と注記で示す。
4. 課題箇所のコードだけを再掲し、クラス図の差分を必要な実装ステップへ変換する。
5. 課題IDと対応コードを追跡しつつ、受け入れ条件に対する実装結果を一度だけ判定する。フェーズ7で変更影響を再測定する。

節番号を埋めるための確認節は作りません。「クラス図とコードが一致する」「受け入れ条件を満たす」と書くだけなら、クラス図と実装結果の説明に統合します。実際の動作と変更影響はフェーズ7で確認します。

中間コードは課題に関係するクラス・メソッド・生成箇所だけを示します。部分的な操作を比較案にはせず、採用した最終構造のどの差分を実現するかで位置づけます。

| 最終システム構造 | 受け入れ条件をどう満たすか | 残る変更・受け入れるコスト | 判断 |
|---|---|---|---|
| 【最終構造A】 | 【全課題IDへの答え】 | 【登録・設定・クラス数・運用・性能】 | 【採用／不採用と理由】 |
| 【最終構造B】 | 【全課題IDへの答え】 | 【登録・設定・クラス数・運用・性能】 | 【採用／不採用と理由】 |

この表は、責任配置や依存関係が異なる完成システムが複数成立する場合に使います。読者が思いつきそうな単純な構造（リスト保持や関数の束など）が受け入れ条件をどこまで満たすかも、この評価で正直に扱います。採用しなかった案は、どの受け入れ条件・変化軸で採用案に劣るのかを述べ、必要なら章末コラムでコードイメージを示します。「何もしない」のような、受け入れ条件を最初から満たさない途中状態だけは比較の対象にしません。
...
比較が必要な場合に比べるのは、実装途中の段階ではなく、受け入れ条件を満たし得る最終構造同士です。各構造について、責任配置・依存関係・変更箇所に加え、現在の導入コストと将来コストを見積もります。
...
性能が重要な場所では、受け入れ条件を満たせるなら仮想呼び出しや中間層を増やさず、関数やテンプレートで完成させる判断があります。業務ルールの変更頻度が高い場所では、実行時コストより変更範囲の小ささを優先する場合があります。分類名から決めず、計測結果と変更要求から決めます。
...
| 低い | 高い | 今回の受け入れ条件を満たしても継続変更に弱い。採用期間と見直し条件を明記する |
| 高い | 高い | 対策を再検討する |

「今すぐ納期を間に合わせたいか、将来の変更コストを下げたいか」——このトレードオフも判断材料の一つです。

このフェーズの出口は、「全課題を統合すると、なぜこのシステム構造になるのか」「接続点をどう切り、どこへ置き、どう繋ぎ直したか」「クラス図のどの差分を、どのコードで実現したか」「受け入れ条件を満たしたか」を説明できることです。
