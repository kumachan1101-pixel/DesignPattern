...
| フェーズ6内：痛みから構造変更を描く | フェーズ3・4の変更影響グラフを左側に引き継ぎ、どの微修正で痛みの経路を切り、同じ要求の到達先を縮めるかを描く |
...
設計判断の出発点は、フェーズ3・4で確認した現在の変更影響です。フェーズ6ではそのグラフを捨てて理想図へ飛びません。左に**現在の痛み**、中央に**コード構造の微修正**、右に**同じ要求を再度当てた結果**を置き、前工程のどの経路を、どの変更で切るのかを一本の図で追います。
...
構造変更グラフは完成クラス図ではありません。「前工程のどの痛みに、どの微修正を当て、同じ要求の影響先がどう変わったか」を表す経過図です。現在と変更後を同じ要求で結び、次のように読み取ります。
...
|---|---|---|---|
| P1 | **現在→理想の差：**【現在から消す波及】<br>**切る境界・守る契約：**【切るコード境界と維持する入力・出力・順序・副作用】<br>**完了条件：**【同じ要求で触らなくする場所】 | **候補：**【最小の微修正】<br>**減る影響：**【その修正で消える経路】 | 【残った経路へ効く次の修正】 |

課題IDは表の行番号ではなく、**独立した課題の識別子**です。表では一課題一行とし、P1から欠番なく振ります。図では同じ課題の経過を `P1-変更1`、`P1-変更2` のような枝番で示せますが、別課題としてP1を重複定義するものではありません。同じP1を引き渡し表、構造変更グラフ、一続き表、関連コード、コード適用結果、完成コード結果で使うのは、一つの課題を最後まで追跡するためです。

候補は課題を作る根拠ではなく、完了条件を満たす手段です。各候補をコードへ当てた後は、「候補を適用したコード」「コード上の微修正」「その結果」「守った契約」「完了条件の判定」を同じP1で必ず記録します。フェーズ7ではさらに、完成コードの適用先と最終結果をP1で回収します。
...
2. 痛みの経路を切るコード構造の微修正を中央へ置き、同じ要求を再適用した結果を右側へ描く。
3. 現在→理想の差、境界・契約、完了条件、候補を同じ課題IDの横一行にする。
4. 着目コードに該当するフェーズ3の関連部分だけをおさらいする。
5. 原因へ直接効く最小変更を示し、どの影響が減り、何が残ったか確認する。
6. 残る影響経路が次の変更を必要とする場合だけ進み、適用コードと結果を同じIDで記録する。
7. フェーズ7の完成コードで、完了条件を満たしたか同じIDで最終確認する。

```text
課題A
  痛みの変更影響 → 構造の微修正 → 同じ要求の再適用結果
      ↓ 同じIDの横一行
  差・境界/契約・完了条件・候補
      ↓ 適用先を特定
  着目コード
...
いきなり完成形（クラス図や採用後の全体コード）へ飛ばず、前工程の痛みから構造の微修正と結果を連続して追います。各章のフェーズ6は次の順で進みます。

- **痛み→構造変更→結果のグラフ**。フェーズ3・4の変更影響を左へ引き継ぎ、中央に微修正、右に同じ要求を再適用した結果を描きます。
...
- **コード適用結果**。課題IDごとに、適用コード、微修正、結果、守った契約、完了条件の判定をまとめます。
