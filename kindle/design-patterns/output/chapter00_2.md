...
| フェーズ | 各章で扱うこと |
|---|---|
| 1：現状把握 | 仕様・動作例・クラス構成・現状コード・変更要求を事実としてそろえる |
...
このフェーズの末尾では、読者が「この章のシステムは何をし、どの入力からどの出力を返し、どのクラスが何を担い、今回どんな変更要求を受けたのか」を説明できる状態にします。
...
フェーズ1で観察した事実を踏まえ、「何が変わりそうか」の仮説を立て、今回確実に変わることを切り分け、ヒアリングで裏付け、最終的に変わるもの/変わらないものとして確定します。各章では、2-1で仮説を立て、2-2で今回の変更を固定し、2-3で関係者に確認し、2-4で将来リスクを整理し、2-5で次のフェーズに渡す見通しをそろえます。
...
このフェーズの末尾では、読者が「今回確実に変わるもの」「将来変わりそうなもの」「当面安定として扱うもの」を根拠付きで区別できる状態にします。
...
このフェーズの末尾では、読者が「変更要求を当てると、どの修正・確認・再テストが増えるのか」を、単なる感想ではなく観察結果として説明できる状態にします。
...
このフェーズの末尾では、読者が「痛みの原因はどの混在・依存・漏れている前提なのか」を、フェーズ1〜3の事実に戻って説明できる状態にします。
...
このフェーズの末尾では、読者が「どの境界を解くべきか」「境界を越えて何を渡すか」「何を安定した約束として残すか」を説明できる状態にします。
...
このフェーズの末尾では、読者が「どの案を採用し、どの案を採用しなかったか」「その理由は変更頻度・影響範囲・導入コストのどれか」を説明できる状態にします。
...
このフェーズの末尾では、読者が「改善後、どこを触ればよくなり、どこを触らずに済むのか」「代わりにどんなコストを受け入れたのか」を説明できる状態にします。
