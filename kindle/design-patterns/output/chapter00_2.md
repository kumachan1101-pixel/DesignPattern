...
各章のフェーズ6の末尾（6-1）には、代表ケース1件を**実行時に通る順**で追う実行接続表を置きます。本文の6段が設計を決めた順であるのに対し、この表だけが`【生成】→【注入】→【利用開始】→【安定骨格】→【契約】→【具体】`という実行順を担います。全課題を解き終えてから1本の経路として追うので、変化軸が2つあれば【注入】は2回現れます。
...
フェーズ6の末尾（6-1）には「構造ポイントの全貌 ―― どの責任がどこへ移るか」という表があります。【契約】から【利用開始】までが、**変更前はどのクラス・関数にあり、変更後はどこへ移るか**を1枚で示す表です。本文の6段が、この表の1行ずつを順に埋めていく作業だったと思って読み返してください。
...
本書で使う関係線は次のとおりです。矢印の見た目だけでなく、右列のコード上の事実と対応させて読みます。

| Mermaid記法 | 本書での意味 | コードで確認する事実 |
|---|---|---|
| `Base <|-- Derived` | 継承 | `class Derived : public Base`で、実装を持つ基底クラスから派生する |
| `Contract <|.. Concrete` | 契約の実装（実現） | `class Concrete : public Contract`で、純粋仮想関数を具体実装する |
| `Owner *-- Part` | 合成・強い所有 | OwnerがPartの生存期間を管理し、デストラクタなどで破棄する |
| `Holder o-- Part`／`Holder o--> Part` | 集約・借用保持 | HolderがPartへのポインタや参照を保持するが、破棄責任は別の所有者にある |
| `User --> Target` | 関連・継続的な利用 | メンバー参照などを通じてTargetの操作を呼ぶ。所有を表す場合は`*--`／`o--`を使い分ける |
| `User ..> Value` | 一時的な依存 | 引数・戻り値・ローカル生成など、その操作中だけValueを使う |

三角形の先は基底クラスまたは契約、ひし形の側は保持する側、通常の矢印・点線矢印の先は利用される側です。同じ二つのクラス間に「契約を実装する」と「内側の部品として保持する」の両方がある場合は、意味の異なる2本を描きます。図の線種と所有・破棄・引数・戻り値がコードで一致しない場合は、図かコードのどちらかが誤りです。
...
フェーズ1のクラス構成・責任一覧と実装コードの観察結果を使い、変わる見込みと当面安定の前提を根拠付きで整理します。
...
最初に、完成コードへ登場するクラス一覧、フェーズ6と同じMermaid定義の完成後クラス図、完成コードに対応する実行シーケンスを示します。読者が全体の責任と呼び出し順を把握してから、値・境界・契約・具体クラス・組み立て・`main()`の順に、クラス単位でコードを示します。最後に全有効要求IDの受入・回帰エビデンス、課題IDごとの構造改善結果、変更IDごとの変更影響を、それぞれ別の追跡表で整理します。
