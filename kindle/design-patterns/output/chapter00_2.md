...
| フェーズ6内：理想の変更影響を描く | フェーズ3・4の現在グラフと同じ要求を使い、変更を許す範囲と波及させない範囲を先に決める |
| フェーズ6内：課題カードと候補を導く | 現在→理想で消した経路を課題カードにし、その完了条件に効く候補だけを出す |
...
設計判断の出発点は、フェーズ3・4で確認した現在の変更影響です。フェーズ6では、いきなり候補や課題カードを作りません。まず同じ変更要求が再来したときの**理想の変更影響グラフ**を描き、「変更してよい範囲」と「波及させたくない範囲」を先に決めます。

次に、現在のグラフと理想グラフの差から、切るべき影響経路、着目コード、守る契約、完了条件を課題カードへ落とします。そのカードから初めて対策候補を出し、課題に該当する関連コードだけをおさらいして段階変更へ進みます。

### 理想の変更影響グラフから課題カードを作る

理想グラフは完成クラス図ではありません。「変更要求がどこまで届けばよいか」だけを表す目標です。現在のグラフと理想グラフを同じ要求で並べ、差を次のように読み取ります。

| 課題から取り出すもの | コード上で確認すること |
|---|---|
| 現在の不要な波及 | 現在グラフでは要求が届くが、理想グラフでは届かないノード・経路 |
| 理想で許す変更 | 理想グラフで変更対象として残すクラス・登録・組み立て・テスト |
| 切る境界 | 不要な波及が始まるメソッド・分岐・生成・登録 |
| 守る契約 | 境界を切っても維持する入力、出力、操作名、実行順序、副作用 |
| 完了条件 | 同じ要求を再度当て、理想グラフで許した範囲だけを修正・再テストすれば済むこと |

この差分が課題カードです。カードの着目コードと完了条件は、候補から思いつきで書くのではなく、理想グラフで消した影響経路から導きます。課題が複数ある章では、理想グラフの独立した枝をP1、P2、P3の連番にし、カードも同じIDで分けます。

課題IDは表の行番号ではなく、**独立した課題の識別子**です。一つのグラフ・課題カード・候補表の中で、同じIDを複数行へ付けません。P1から始めて欠番なく振り、同じIDをグラフ、課題カード、候補、関連コードで再び使うのは、同じ課題を追跡するときだけです。たとえば一つの課題に原因が三つあってもP1を三行にはせず、P1の現在→理想の差として一行にまとめます。

カードができてから、切る境界に対して関数化、クラス分離、契約、登録、生成責任の移動などの候補を出します。候補はカードを作る根拠ではなく、カードの完了条件を満たす手段です。

### 一つの課題を段階的に解く手順

各課題では、次の流れを繰り返します。

1. 現在の変更影響グラフから、同じ要求に対する理想グラフを描く。
2. 消したい影響経路から、課題カードの着目コード・守る契約・完了条件を導く。
3. カードの完了条件に効く候補を出す。
4. 着目コードに該当するフェーズ3の関連部分だけをおさらいする。
5. 原因へ直接効く最小変更を示し、理想グラフへ近づいたか確認する。
6. 残る影響経路が次の変更を必要とする場合だけ、その理由を書いて進む。
...
いきなり完成形（クラス図や採用後の全体コード）へ飛ばず、理想の変更影響から課題カードと候補を導きます。各章のフェーズ6は次の順で進みます。

- **理想の変更影響グラフ**。フェーズ3・4の現在グラフと同じ変更要求を使い、変更を許す範囲と波及させない範囲を先に描きます。
- **グラフ差分から課題カードを導く**。現在にはあり理想では消した経路を、着目コード・切る境界・守る契約・完了条件へ変換します。
- **課題カードから候補を出す**。完了条件を満たす手段だけを候補にし、候補からカードを逆算しません。
