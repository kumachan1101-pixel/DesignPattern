...
システムの全コードを読むわけではありません。前半で把握したクラス構成に基づき、今回の変更要求に関係するクラスへ範囲を絞ります。
...
責任チェックでは、次の3つを問いかけます。

1. **誰の判断で変わるか**
2. **異なる決定者の知識が同じクラスに混在していないか**
3. **一方を変えたとき、どこまで影響確認が必要になるか**
...
| 🟢 当面は安定 | （今回の検討範囲で変わりにくい部分） | （見直しの契機） | （誰と合意したか） |
...
> **第1章での体験箇所：** 「4-1：痛みの根源を探る」〜「4-3：接続形態の診断」で、計算の骨格と割引条件という「変わる理由が異なる2つ」が同じメソッドに混在していることを構造的な原因として言語化します。
