...
1. **共通点を見つけて分離する** ―― バラバラに見える処理から、変わらない骨格・共通の契約を見抜き、変わる部分をそこから切り出す。
...
次の流れは共通の骨格を示した例です。分離の単位、配置場所、生成・所有・登録・選択・注入の方法が異なるため、使う番号と組み立て順は章と課題ごとに変えます。ただし、三つの設計決定に答えず、変えたくない側へ具体的な条件や生成判断を残したままにはしません。
