...
フェーズ6では、フェーズ5の課題を統合し、システム全体として満たす完了条件と最終構造をコードを書く前に確定します。完了条件を満たす最終システム構造が複数ある場合だけ構造差分を比較します。その後の「ステップ1」「ステップ2」は、案が揺れた履歴ではなく、確定した構造を理解できる単位で実装する順序です。
...
| 6：対策検討 | 全課題を解くシステム全体の最終構造を確定し、その実装順を示す。完成構造が複数ある場合だけ比較する |
...
graph TD
    ...
    F6["フェーズ6<br>対策検討<br>（システム全体の最終構造を決める）"]
...
flowchart TB
    ...
    P6["6 対策検討<br>課題を統合して全体構造を決める"]
    O6["成果物<br>システム全体の完了条件<br>採用クラス図<br>実装結果"]
...
フェーズ5で接続点と制約を定めないままフェーズ6へ進むと、システム全体の構造設計がパターン名選びになってしまいます。
...
| フェーズ6内：課題をシステム条件へ統合する | 各課題IDの変化軸・移す責任・守る範囲をまとめ、システム全体の完了条件を定める |
| フェーズ6内：採用する形を決める | 完了条件を満たす最終構造を確定する。複数の完成構造が成立する場合だけ構造差分とコストを比べる |
...
> - **「対策」**（フェーズ6で確定）：全課題を統合したシステム全体の完了条件を満たす最終構造のこと。構造差分のある完成形が複数成立する場合だけ比較し、関数化やクラス分離は採用構造を実現するコード変更として扱います。
...
フェーズ5で作る課題文は、フェーズ6でシステム全体の構造を決めるための設計仕様です。ここで「何をどこから切り離すか」「接続点には何を残すか」「何を変えずに守るか」が曖昧なままだと、フェーズ6はパターン名や好みで形を選ぶ場になってしまいます。
...
たとえば「割引ルールを価格計算の骨格から切り離し、接続点には注文情報を受け取って割引額を返す操作だけを残す」のように書ければ、最終システムに必要な責任と契約が見えてきます。関数、クラス、契約、組み立て場所は個別の対策案ではなく、その最終構造を実現する部品として課題文から逆算します。
...
フェーズ5の出口は、フェーズ6の入口です。この時点では対策の形を決めず、変化軸ごとに次の3点を確定します。フェーズ6ではこれらを別々に解くのではなく、一つのシステム全体の完了条件へ統合します。
...
## 🔴 フェーズ6：対策検討 ―― システム全体の最終構造を決める

**目的：すべての課題を同時に解くシステム全体の構造を決め、責任と依存の差分をコードへ落とすこと**

### 判断を整理してから、採用設計を段階的に実装する

フェーズ5で確定した複数の課題IDは、独立した変化軸を追跡するためのものです。フェーズ6では課題ごとに別の設計を採用せず、すべてを同時に解く一つのシステムとして考えます。

1. 各課題IDの変化軸、現在の波及、移す責任、守る範囲を一表にまとめる。
2. それらを統合し、システム全体の完了条件を一度だけ定める。
3. 完了条件を満たす最終構造を決める。成立する完成構造が複数ある場合だけ、システム全体の構造差分を比較する。
4. 採用構造を変更後のクラス図として確定し、操作・責任・依存の差分を示す。
5. クラス図の差分をコードへ反映し、システム全体の実装結果を一度だけ判定する。
...
課題IDは、独立した変化軸をコードまで追跡する識別子です。課題ごとに採用案や完了判定を持たせず、次の表でシステム構造に必要な材料をそろえます。

| 課題ID | 変化軸と現在の変更影響 | 構造で移す責任 | 変えたくない範囲 |
|---|---|---|---|
| P1 | 【独立して変わる仕様と現在波及する場所】 | 【分離先へ移す判断・処理】 | 【安定させる処理・利用側】 |
| P2 | 【別の変化軸と現在波及する場所】 | 【分離先へ移す判断・処理】 | 【安定させる処理・利用側】 |

この表を統合して、「どの変更が来ても、どの範囲だけを変更し、どの範囲を変更しないシステムにするか」を完了条件として一度だけ定義します。比較も採用も、このシステム全体の条件を基準にします。

| システム全体の完了条件 | 内容 |
|---|---|
| 想定する変更 | 【P1・P2など、同時に扱う変更軸】 |
| 変更してよい範囲 | 【新しい実装、登録、設定、必要な入力など】 |
| 変更しない範囲 | 【安定させる処理、利用側、既存実装】 |
| 維持する契約 | 【入力・出力・順序・状態遷移・副作用】 |
...
6. **システム全体への効果**：統合した完了条件のどの部分を実現したか。
...
ここで最も重要なのは、パターン名を先に当てはめないことです。フェーズ1〜5の事実から4つの問いへ答え、完了条件を満たす最終構造を決めます。本文では結論を先に示し、その構造を実現するコード変更だけを段階的に説明します。
...
この表はパターン名の対応表ではなく、課題の完了条件から必要な責任・契約・依存を導く補助線です。完了条件が単なる関数分離で満たせるならそこで止め、交換・追加・組み合わせまで求めるなら、契約や登録、組み立て責任まで含む完成形を作ります。
...
原則はチェックボックスではありません。インターフェースが不要な章もあれば、仲介役だけで十分な章もあります。システム全体の完了条件を満たす最小の最終構造を選びます。

### 対策検討の進め方 ―― システム全体の構造をコードへ落とす

フェーズ6で必要なのは、次の5点です。

1. 課題IDごとの変化軸、移す責任、守る範囲を統合し、システム全体の完了条件を固定する。
2. 完了条件を満たす最終システム構造を決める。複数ある場合だけ構造差分を比較する。
3. フェーズ1のクラス図を変更前・変更後で示し、移す責任と残す責任を色と注記で示す。
4. 課題箇所のコードだけを再掲し、クラス図の差分を必要な実装ステップへ変換する。
5. 課題IDと対応コードを追跡しつつ、システム全体の実装結果を一度だけ判定する。フェーズ7で変更影響を再測定する。

節番号を埋めるための確認節は作りません。「クラス図とコードが一致する」「完了条件を満たす」と書くだけなら、クラス図と実装結果の説明に統合します。実際の動作と変更影響はフェーズ7で確認します。

中間コードは課題に関係するクラス・メソッド・生成箇所だけを示します。部分的な操作を比較案にはせず、採用した最終構造のどの差分を実現するかで位置づけます。

| 最終システム構造 | 全体の完了条件をどう満たすか | 残る変更・受け入れるコスト | 判断 |
|---|---|---|---|
| 【最終構造A】 | 【全課題IDへの答え】 | 【登録・設定・クラス数・運用・性能】 | 【採用／不採用と理由】 |
| 【最終構造B】 | 【全課題IDへの答え】 | 【登録・設定・クラス数・運用・性能】 | 【採用／不採用と理由】 |

この表は、責任配置や依存関係が異なる完成システムが複数成立する場合だけ使います。最終構造が一つなら比較表を作らず、採用構造と導出理由を示します。「何もしない」「関数だけ切り出す」「分岐を別クラスへ移す」など、完了条件を満たさない途中状態は比較しません。
...
比較が必要な場合に比べるのは、実装途中の段階ではなく、システム全体の完了条件を満たす最終構造同士です。各構造について、責任配置・依存関係・変更箇所に加え、現在の導入コストと将来コストを見積もります。
...
性能が重要な場所では、完了条件を満たせるなら仮想呼び出しや中間層を増やさず、関数やテンプレートで完成させる判断があります。業務ルールの変更頻度が高い場所では、実行時コストより変更範囲の小ささを優先する場合があります。分類名から決めず、計測結果と変更要求から決めます。
...
| 低い | 高い | 今回の完了条件を満たしても継続変更に弱い。採用期間と見直し条件を明記する |
| 高い | 高い | 対策を再検討する |

「今すぐ納期を間に合わせたいか、将来の変更コストを下げたいか」——このトレードオフも判断材料の一つです。

このフェーズの出口は、「全課題を統合すると、なぜこのシステム構造になるのか」「クラス図のどの差分を、どのコードで実現したか」「システム全体の完了条件を満たしたか」を説明できることです。
