...
2×2の接続マトリクスは、設計の出発点ではありません。フェーズ3〜5で痛み・原因・課題を特定した後に、対策候補を比較するための道具です。

### 接続の2軸

今、あなたのコードでAクラスとBクラスがどうつながっているかを想像してみてください。おそらく、インターフェースなどの抽象化がなく、クラス名（具体）を直接呼び出している状態になっていることが多いのではないでしょうか。この状態を整理するための言葉が「具体×直接」という接続形態の名前です。この状態から、どのように接続を変化させていくかを考えるための軸が、次の2つです。
...
分けた理由は、接続の形の候補を絞る手がかりになります。最終的な形は、変更頻度、実装コスト、性能要件、チームの習熟度なども含めて決めます。
...
*   **ステップ3：条件ごとに関数化する**（条件も分け、一貫した構造に気づく。if文は残る）
    *   *【思考の遷移】ここまで関数に分けて整理を試みたものの、今度は一つのクラスの中に関数が膨大に増えてしまい、結局「このクラスが何者か」が分からなくなってきました。ここで初めて「関数を増やすのではなく、クラス自体を分けるしかない」という発想に至ります。*
...
> [!NOTE] 各章のステップ数について
> 各章が扱う問題の性質によって、必要なステップが異なります。例えば、「条件ごとのルール差し替え」が中心のStrategyパターン（第1章）ではステップ2・3（関数化）を省略して4ステップになることがあります。逆に「処理の骨格固定と個別機能の動的追加」が複雑に絡むTemplate Method×Decorator複合（第11章）では6ステップを踏む場合があります。ここで示す5段階はあくまで「思考の参照軸」です。
...
ステップ5+A：[A] → [C] → [B]                  ← 具体×間接（中間層を挟む）
ステップ5+B：[A] → «interface» → [C] → [B]   ← 抽象×間接（型＋中間層）
```

「ステップ5+A / 5+B」の2段階（具体×間接・抽象×間接）は、ステップ1〜5の先にある追加の進化です。Facade・Adapter・Proxy・Mediator等のパターンが到達する接続の形に相当します。各章では、その章の問題に対して何ステップ踏んでどの形に辿り着くかが異なります。
