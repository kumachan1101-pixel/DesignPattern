...
フェーズ6では、確定課題から接続点を変える三つの観点――分離方法、配置場所、組み立て方法――を順に考えます。責任、継承、実装、保持、依存を変える判断ごとに部分クラス図を示し、最後にそれらを統合した最終構造をコードを書く前に確定します。組み立て方法には、生成・所有・登録・選択・注入・実行までを含めます。その後は、確定した構造を`【契約】 →【安定骨格】 →【具体】 →【生成】 →【注入】 →【利用開始】`の6段で組み立てます。**この並びは、設計を決めていく順です。** まず境界を関数・表・クラス契約のどれで表すかと、そこに何が渡るかを決め、今回維持する側の手順を固定し、契約の裏を埋め、実体を作って持つ人を決め、実装が骨格へ入る場所を決め、最後に呼び方が結果として決まります。案が揺れた履歴ではありません。

**1段目は「分けるかどうか」から始まります。** フェーズ5が確定したのは「この接続点を解く」ことであって、「クラスを増やす」ことではありません。変わる側の判断がコード上1か所にしか現れず、種類が増える見込みも挙がっていないなら、契約を作らずに関数を1つ切り出すだけで止めます。**6段は必ずクラスを増やす手順ではありません。**

**各段の終わりに、守る範囲を1行で照合します。** フェーズ6の冒頭で「この章で変えないもの」を挙げるので、各段がそれに触ったかどうかをその場で判定します。フェーズ7の受入エビデンスまで持ち越しません。

**本文は、課題がいくつあっても6段を1周します。課題別に節を切りません。** 後半3段（【生成】【注入】【利用開始】）は課題をまたぐ共同の判断なので、課題別に書くと同じ判断を2回書くことになります。課題どうしの比較も、同じ段の中で隣り合っていないと読めません。前半3段は変化軸ごと、後半3段は共通として書き分けます。

**2段目の「安定骨格」は、章によって正体が違います。** 分岐を外した後に**残った手順**である章（この場合、契約は骨格とは別のクラスになります）と、変更前から存在していた**共通手順を取り出した**章（この場合、契約は基底クラスの中に置けます）があります。見分ける基準は「同じ実行の途中で相手が入れ替わるか」です。入れ替わるなら契約は外へ、実体を作った時点で決まるなら基底の中へ置けます。

**5段目の「注入」は、コンストラクタ引数のことではありません。** 骨格が契約としか書いていない場所へ具体が入ること、という意味です。入る瞬間は3つに分かれます。**呼び出しごとに引き当てる**（引き当て関数が契約への参照を返す行）、**生成した実体を渡して保持する**（コンストラクタ引数や登録）、**継承で決まる**（コード上に行が無く、実体を作った時点で決まる）。どれであっても、骨格が具体の名前を1文字も書いていない点は同じです。
【契約】〜【利用開始】は、その課題の採用設計をコードへ反映する全章共通の番号です。【安定骨格】の「安定骨格」はTemplate Methodの基底アルゴリズムだけを指しません。具体実装が増減しても維持する、選択・委譲・反復・配布・履歴移動・生成後の呼び出しなど、公開入口の内側にある制御の順序を表します。どの章でも【安定骨格】を省略せず、何を安定させる骨格かを章固有の名前で示します。

**【安定骨格】と【利用開始】は別の番号です。** 【安定骨格】は呼ばれる側で、公開入口の内側にある制御順です。【利用開始】は呼ぶ側で、利用者や外部イベントが公開入口を起動する行です。「骨格が契約を呼ぶこと」は【安定骨格】の内部にある契約呼び出しであり、【利用開始】ではありません。同じメソッドに見えても、呼ぶ側と呼ばれる側で責任が違うので、各章は【安定骨格】と【利用開始】を別々の見出しとコードで示します。

各章のフェーズ6「生成・所有・実行順のまとめ（6-1）」には、代表ケース1件を**実行時に通る順**で追う実行接続表を置きます。本文の6段が設計を決めた順であるのに対し、この表だけが`【生成】→【注入】→【利用開始】→【安定骨格】→【契約】→【具体】`という実行順を担います。全課題を解き終えてから1本の経路として追うので、変化軸が2つあれば【注入】は2回現れます。

フェーズ6のポイントは番号ではなくラベル（【生成】【注入】【利用開始】【安定骨格】【契約】【具体】）で示します。番号を振ると、番号順と実行順のどちらを指しているのか読者が迷うためです。フェーズ1の現状コードと仕様の読解順は `(1)`、フェーズ3で変更を試すコードの分割は `[試行1]` を使います。
...
flowchart LR
    ...
    B["三つの設計決定<br/>分離・配置・組み立て"]
    C["設計判断ごとの<br/>部分クラス図"]
    D["統合した<br/>採用全体クラス図"]
    E["6段を1周する実装<br/>【契約】→【安定骨格】→【具体】<br/>→【生成】→【注入】→【利用開始】"]
    F["フェーズ7<br/>完成コードと検証"]
...
部分クラス図は未採用案ではなく、採用全体図を判断単位に分けたものです。【契約】〜【利用開始】は、その確定構造をコードへ組み立てる読書順です。
### 掲載コードを手元で動かす
...
**3. 断片を読む前に、全貌表で地図を見てください。**

フェーズ6「生成・所有・実行順のまとめ（6-1）」には「構造ポイントの全貌 ―― どの責任がどこへ移るか」という表があります。【契約】から【利用開始】までが、**変更前はどのクラス・関数にあり、変更後はどこへ移るか**を1枚で示す表です。本文の6段が、この表の1行ずつを順に埋めていく作業だったと思って読み返してください。

全貌表のすぐ下にもう1つ「代表ケースの実行接続」という表があります。こちらは**実行時に何がどの順で呼ばれるか**を、実際のコード行で追う表です。全貌表が「どこからどこへ移るか」を考えた順に示すのに対し、この表は「どの行がどの行を呼ぶか」を`【生成】→【注入】→【利用開始】→【安定骨格】→【契約】→【具体】`の実行順で示します。並びが違うのは、設計を決める順と、プログラムが動く順が別だからです。
...
| 6：対策検討 | 部分クラス図と採用全体クラス図 | 分離方法・配置場所・組み立て方法の各判断を部分クラス図で説明し、その和としてフェーズ1と同じ粒度の採用全体図を作る。変更前コードはまとめて再掲せず、各判断で触る数行だけを変更試行から抜き出す |
| 6：対策検討 | 6段を1周する節（【契約】〜【利用開始】） | 確定した構造の責任分離・生成・所有・選択・依存の受け渡し・安定側からの実行を、`【契約】 →【安定骨格】 →【具体】 →【生成】 →【注入】 →【利用開始】`の順で1周だけ反映する。前半3段は変化軸ごと、後半3段は共通 |
| 6：対策検討 | 6-1：生成・所有・実行順のまとめ | 誰が何を生成・所有し、どの順で安定側から呼ぶかを、課題IDをまたいで一度に確認する。構造ポイントの全貌表と、全課題を解き終えた代表ケースの実行接続表を置く |
...
本書のクラス図は、クラスの配置と依存の向き（誰が誰を知っているか）を示す静的な構成図です。そのうえで、矢印に「商品情報を取得」のような**やり取りの説明ラベル**を付けることがあります。厳密なUMLではデータの流れはシーケンス図の領分ですが、本書では「この依存は何のためにあるか」を図だけで掴めるよう、依存の説明としてラベルを許容しています。呼び出しの時間順を追いたいときは、各章のフェーズ7「解決後のコード（7-1）」で完成コードの前に示す実行シーケンスを見てください。フェーズ7「動作シーケンス図（7-2）」では、そのシーケンスが完成コードと実行結果に一致することを検証します。
...
classDiagram
    ...
    Order *-- OrderLine : 3 所有
    CheckoutService o-- IDiscountRule : 4 借用保持
    Order --> Customer : 5 継続的な参照
    CheckoutService ..> PaymentResult : 6 戻り値として使う
```

この図では、`CheckoutService`が割引規則を借りて会計し、`Order`が明細を所有しています。速達用の送料規則は共通処理を持つ基底クラスを継承し、会員割引は割引契約だけを具体化しています。線は、こうした**コード上の事実を短く表したもの**です。
...
| 3 | 実線＋黒ひし形 | 合成・強い所有。ひし形側が部品を値や所有ポインタで持ち、部品の寿命を管理する | 所有者が消えたら部品も消える関係を示すとき |
| 4 | 実線＋白ひし形 | 集約・借用保持。ひし形側がポインタや参照を保持するが、破棄しない | 外で作られた部品を借り、差し替えて使うとき |
| 5 | 実線＋矢印 | 関連・継続的な利用。一方が他方をメンバーなどから継続して参照する | 所有よりも「知っていて利用する」ことを示したいとき |
...
class Order {
    ...
    Customer* customer;      // 5：顧客を参照するが所有しない
};

class CheckoutService {
    IDiscountRule* discountRule;  // 4：外から受け取り、借りて保持する
public:
    PaymentResult checkout(const Order& order);  // 6：結果型を戻り値に使う
};
```

迷ったときは、まず寿命を確認します。相手を破棄するなら黒ひし形、保持するが破棄しないなら白ひし形、保持せず操作中だけ使うなら点線矢印です。そのうえで、基底クラスの実装を受け継ぐなら実線の白三角、契約だけを実装するなら点線の白三角を使います。図の線種と、所有・破棄・引数・戻り値が一致しない場合は、図かコードのどちらかが誤りです。
...
> **成果物：** 分離・配置・組み立ての決定、部分クラス図、採用全体図、課題IDごとの段階コード
...
4. 6段を1周し、各段で「その段が触る変更前の数行 → 決めたこと → 変更後コード → 図の差分」を示す。変更前コードをまとめて再掲する節は置かない。
5. 各段で決まる名前（契約名・具体名・生成役・注入先）を、その段より前に出さない。まだ決まっていない部分は穴として見せ、どの段で決まるかを書く。
...
**設計の醍醐味は、突き詰めると五つの動作に整理できます。**

1. **安定する共通点を見つける** ―― 変えたくない処理の骨格と、境界へ残す最小の契約を見抜く。
2. **共通でない部分を分離する** ―― 条件・計算式・状態別動作・生成判断など、別の理由で変わる部分を切り出す。
3. **分離した責任を配置する** ―― 切り出した部分を、独立した具体クラスとして作れる場所へ置く。
4. **生成して受け渡す** ―― 具体クラスを安定側の外で生成・選択・登録し、契約型として注入する。
5. **実行部は契約だけを呼ぶ** ―― 安定側は、具体が何であるかを知らないまま実行する。

本書の全12章は、題材（割引・状態・通知・決済…）こそ違え、毎回この五つの動作をたどります。デザインパターン名は、この流れを**どう実現したか**を後から共有するための語彙にすぎません。

言い換えれば、全章に共通する核は、**変わる詳細を具体クラスへ分離し、その具体クラスの生成・選択を安定側から外し、安定した接続点を通して渡すことで、変えたくない処理が詳細を知らずに実行できる構造へ変える**ことです。章ごとに違うのは、分離する責任、配置するクラス、生成・所有・登録・選択・注入の方法です。
...
次の流れは共通の骨格を示した例です。分離の単位、配置場所、生成・所有・登録・選択・注入の方法が異なるため、使う番号は章と課題ごとに変わります。
...
flowchart TB
    ...
    E["判断ごとの部分クラス図"]
    F["採用全体クラス図"]
    G1["6段を1周する<br/>前半3段【契約】→【安定骨格】→【具体】は変化軸ごと<br/>後半3段【生成】→【注入】→【利用開始】は共通"]
    G2["6-1<br/>構造ポイントの全貌／代表ケースの実行接続"]
...
この章で持ち帰っていただきたいのは、パターン名の一覧ではありません。変更が届いたとき、現状の仕様とコードへ実際に当て、痛みを観測し、原因から課題を確定して、分離・配置・組み立てを決めるという順序です。
