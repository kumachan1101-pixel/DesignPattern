...
| 1：現状把握 | 1-5：変更要求 | 現状仕様と変更後仕様の差分を固定し、以降の分析対象を決める |
| 2：仮説立案 | 2-1：変わりそうな仕様の見当をつける | フェーズ1の事実から、変化しそうな仕様を仮説として言語化する |
| 2：仮説立案 | 2-2：今回の変更で確実に変わること | 今回対応する変更と、まだ先の話を分ける |
...
| 3：問題特定 | 3-1：変更を試みる | フェーズ1の現状コードへ変更を当てたとき、どこを触ることになるかを見る |
...
| 6：対策検討 | 6-1：採用設計をコードへ段階的に反映する | 確定した構造の責任分離・生成・依存の受け渡し・安定側からの実行を、ステップ順にコードへ反映する |
| 6：対策検討 | 6-2：システム全体の契約とデータ配置を確定する | 課題IDごとに目指した状態・適用コード・結果を追い、システム全体で一度だけ達成を判定する |
| 7：対策実施 | 7-1：解決後のコード | 採用した形を実行可能なコードとして示す |
| 7：対策実施 | 7-2：動作シーケンス図 | 改善後にオブジェクト間で何が起きるかを追えるようにする |
