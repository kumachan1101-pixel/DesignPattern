...
ただし、図は「入力」「加工」「出力」という箱へ分類するためのものではありません。図に出した値・条件・状態・出力名が、後で動作例、`main()`、実行結果、変更要求、接続点のどこで使われるのかまで追える必要があります。
図は装飾ではなく、「いま何を見ているのか」「この要素が後でどこに効くのか」を読者が迷わないための地図です。
...
そのため各章では、仕様の入力・加工・出力を図で見せたうえで、各要素がコード上のどの値・状態・判定条件・出力名に対応し、後続のどの表・コード・変更要求で使われるのかを先に説明します。
...
コードを読む前に、読者が「このシステムは何を受け取り、何をして、何を返すのか」を同じ粒度で見るための図です。ただし、箱を3つ並べるだけでは不十分です。図に入れた各要素は、後でどこに出てくるかまで示します。

| 図の要素 | コードで出る形 | 後で使う場所 |
|---|---|---|
| 入力値・状態 | `main()` の引数、固定データ、状態名、フラグ | 動作例テーブル、実行結果、変更要求 |
| 加工の判定・処理 | `if` / `switch`、計算式、変換処理、呼び出すメソッド | 現状コードの観察、フェーズ2の仮説、フェーズ4の原因分析 |
| 出力・状態変化 | 戻り値、表示文、保存結果、通知、エラー | 動作例テーブル、実行結果、変更シナリオ表 |
...
仕様、動作例、`main()`、実行結果は同じケースを扱います。仕様で説明していない値や状態を、コードで突然使わないようにします。また、仕様図に置いた入力・加工・出力は、動作例、コード、変更要求、接続点のいずれかで再利用されるものに絞ります。
