...
フェーズ1では、現行仕様と今回確定した変更要求へR1、R2、R3の連番を付け、入力と受入条件をコードより先に固定します。フェーズ2では、将来リスクへF1、F2、F3の連番を付け、「変えられるようにする部分」と「当面守る部分」を確定します。F-IDを扱うのはフェーズ2と6です。フェーズ6で採用構造へ再適用し、変化が起きても守る部分へ影響を広げない構造かを評価します。フェーズ3と7は確定要求R-IDだけを扱います。

フェーズ5では、解くべき接続点を洗い出し、接続点ごとに「何が流れるか」「どちら側が変わるか」「どちら側を守るか」をP1、P2、P3の連番で確定します。最後に、現在の波及と目指す状態をシステム全体の課題として文章化します。フェーズ6では、その接続点を変える三つの観点――分離方法、配置場所、組み立て方法――を順に考え、最終構造をコードを書く前に確定します。組み立て方法には、生成・所有・登録・選択・注入・実行までを含めます。その後の「ステップ1」「ステップ2」は、案が揺れた履歴ではなく、確定した構造を理解できる単位で実装する順序です。
...
| フェーズ | 章内の項目 | 何のために読むか |
|---|---|---|
| 1：現状把握 | 1-1：このシステムの仕様 | コードを読む前に、入力・加工・出力・業務上の前提をそろえる |
...
| 1：現状把握 | 1-5：変更要求 | R1から連番を付け、現状との差分・入力・受入条件を固定する |
| 2：仮説立案 | 2-1：変わりそうな仕様の見当をつける | フェーズ1の事実から、変化しそうな仕様を仮説として言語化する |
| 2：仮説立案 | 2-2：今回の変更で確実に変わること | R-IDを確定し、将来リスク・内部契約・対象外と分ける |
| 2：仮説立案 | 2-3：関係者ヒアリング | 仮説を独断で確定せず、変更される機能の種類・理由・頻度を確認する |
| 2：仮説立案 | 2-4：将来リスク | 今すぐ対応する変更と、将来起きそうな変更を分ける |
| 2：仮説立案 | 2-5：変わる見込み/当面安定の前提の確定 | F-IDごとに変えられるようにする部分と当面守る部分を定め、フェーズ6の構造評価条件にする |
| 3：問題特定 | 3-1：変更を試みる | 確定R-IDだけを現状コードへ当て、変えるクラスと変えないクラスを先に示してから差分を見る |
...
| 5：課題定義 | 接続点を特定する | 変更が波及する境界を見つけ、課題ID・流れるデータ・変わる側・守る側を一表で確定し、システム全体で目指す状態を一文にする（クラスや接続方法はまだ書かない） |
| 6：対策検討 | 対策検討のクラス図：1-3の責任と依存をどう変えるか | 分離方法・配置場所・組み立て方法の三観点で最終構造を決め、フェーズ1のクラス図を同じ粒度で更新して着目クラスを色で示す。課題箇所のおさらいも添える |
...
| 7：対策実施 | 7-1：解決後のコード | 完成クラス一覧・採用時と同じクラス図・実行シーケンスを先に示し、クラス単位のコードとR-ID別の実行結果で照合する |
| 7：対策実施 | 7-2：動作シーケンス図の検証 | 7-1でコード前に示した呼び出し順が、完成コードと実行結果に一致することを確認する |
| 7：対策実施 | 7-3：変更影響グラフ | 改善後の主な修正先と確認範囲を見えるようにする |
| 7：対策実施 | 7-4：変更シナリオ表 | 今回の確定要求R-IDごとに、フェーズ1の修正範囲と完成構造の結果を照合する |
...
フェーズ1の最後に、この章で検証する具体的な変更要求を確認します。要求にはR1から欠番なくIDを振り、要求ごとに現状との差分、入力、受入条件を固定します。コードに登場する役割、種類、状態、処理順、操作は、その役割が現状にあるなら1-1で、その要求により追加されるなら1-5で先に説明します。読者がコードを見て初めて仕様を知る状態にはしません。
...
フェーズ1で観察した事実を踏まえ、「何が変わりそうか」の仮説を立て、今回確実に変わることを切り分け、ヒアリングで裏付け、最終的に変わる見込み/当面安定の前提として確定します。各章では、2-1で仮説を立て、2-2で今回の変更をR-IDとして固定し、2-3で関係者に確認し、2-4で将来リスクをF-IDとして固定し、2-5で次のフェーズに渡す見通しをそろえます。ここで、今回の要求を実現するための戻り値や結果型などは「内部契約」、採用しない動作は「対象外」として別に置きます。将来リスク・内部契約・対象外を、確定要求の機能としてフェーズ3や7へ実装しません。
...
ヒアリングで判明したF-IDごとに、「変わる見込み」「変えられるようにする部分」「当面安定として守る部分」を一表で整理します。「変わる見込み＝はい」は、その機能を今回実装するという意味ではありません。フェーズ6で、変える側と守る側をどの境界で分離し、採用構造が影響を局所化できるかを判断するための設計条件です。

このフェーズの出口は、F-IDの変化が起きても守る部分へ影響を広げない設計にするため、何を変えられるようにし、何を当面守るかを根拠付きで説明できるところです。フェーズ3へ渡す実装対象はR-IDだけです。
...
5. フェーズ2のF-IDを採用構造へ再適用し、将来の変更先、守れる範囲、残る設計課題を確認する。
6. 各ステップで変更理由と効果をコードレベルで回収し、フェーズ7で全体コードと変更影響を確認する。
...
採用構造を決めた後、F-IDを次の形で再適用します。

| リスクID・将来リスク | 採用構造での考慮 | 将来の主な変更先 | 今回の判断 |
|---|---|---|---|
| F1：【2-4と同じリスク文】 | 【どの境界で影響を受けるか】 | 【クラス・契約・生成場所】 | 【局所化／一部局所化／未解決。完成コードへ追加しない】 |
...
このフェーズの出口は、「全課題を統合すると、なぜこのシステム構造になるのか」「接続点をどう切り、どこへ置き、誰がどう生成・所有・登録・選択・注入したか」「クラス図のどの差分を、どのコードで実現したか」「各F-IDに対して将来どこが変わり、何を守れ、何が未解決か」を説明できることです。

**全ケース共通・必ず行うこと：**

フェーズ7では、今回確定したR-IDだけを実際のコードへ当てて受入確認を行います。7-4も同じR-IDを使い、フェーズ1の構造で必要だった修正範囲と、完成構造で得られた結果を照合します。
...
最初に、完成コードへ登場するクラス一覧、フェーズ6と同じMermaid定義の完成後クラス図、完成コードに対応する実行シーケンスを示します。読者が全体の責任と呼び出し順を把握してから、値・境界・契約・具体クラス・組み立て・`main()`の順に、クラス単位でコードを示します。最後にR-IDごとの実行結果と、フェーズ1から完成構造までの修正範囲を整理します。

「中心となる処理へ条件分岐を増やさず、変更箇所を実装クラスと組み立て箇所へ絞る代わりに、クラス数や登録処理が増える」——これがこのフェーズで言語化する代表的なトレードオフです。

「変更シナリオ表」は、今回の確定要求R-IDをフェーズ1の現状構造と完成構造へ同じように当て、修正範囲と実装結果を比較するものです。
目的は「今回の要求に対して何を手に入れたか」の可視化です。確定要求、現状構造で必要だった修正、完成構造で確認できた結果を一行で示します。
「何を得て何を諦めたか」という中心の問いに直接答えます。

次の表は、今回の確定要求と、その要求に対して観測した影響を示す形式です。各章では、1-5と同じR-ID・同じ要求文を使います。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：【1-5と同じ確定要求文】 | 【フェーズ3で実際に修正・確認した範囲】 | 【完成コードと実行結果で確認した効果】 |

この表が埋まると、フェーズ3で見た痛みが完成構造でどう減ったかを、確定要求ごとに確認できます。「諦めたもの」は、クラス数や組み立て管理など、採用構造によって増えたコストとして別に明記します。
