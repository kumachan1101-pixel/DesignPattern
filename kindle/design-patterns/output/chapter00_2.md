...
フェーズ6では、確定課題から接続点を変える三つの観点――分離方法、配置場所、組み立て方法――を順に考えます。責任、継承、実装、保持、依存を変える判断ごとに部分クラス図を示し、最後にそれらを統合した最終構造をコードを書く前に確定します。組み立て方法には、生成・所有・登録・選択・注入・実行までを含めます。その後は課題IDごとに節を分け、確定した構造を`【契約】 →【安定骨格】 →【具体】 →【生成】・所有 →【注入】登録・注入 →【利用開始】実行`の順で組み立てます。これは案が揺れた履歴ではなく、確定した構造を理解できる単位で読む順序です。
...
説明する順序は【契約】から【利用開始】ですが、実行時の呼び出し順は違います。多くの章では【生成】 →【注入】 →【利用開始】 →【安定骨格】 →【契約】 →【具体】の順に進みます。各章のフェーズ6には、代表ケース1件をこの順で追う実行接続表を置きます。

番号は用途ごとに形を分けます。フェーズ1の現状コードと仕様の読解順は `(1)`、フェーズ3で変更を試すコードの分割は `[試行1]`、フェーズ6の設計反映順は `【契約】`、フェーズ7の完成コード分割は `【1】` です。本文で途中番号を参照するときも記号を含め、別の段階の番号と取り違えないようにします。
...
flowchart LR
    ...
    C["設計判断ごとの<br/>部分クラス図"]
    D["統合した<br/>採用全体クラス図"]
    E["課題IDごとの実装<br/>【契約】→【安定骨格】→【具体】<br/>→【生成】・所有→【注入】登録・注入→【利用開始】実行"]
    F["フェーズ7<br/>完成コードと検証"]
...
フェーズ6の冒頭には「構造ポイントの全貌 ―― どの責任がどこへ移るか」という表があります。【契約】から【利用開始】までが、**変更前はどのクラス・関数にあり、変更後はどこへ移るか**を1枚で示す表です。断片コードは、この表の1行ずつを順に埋めていく作業だと思って読んでください。

各課題IDの節にはもう1つ「代表ケースの実行接続」という表があります。こちらは**実行時に何がどの順で呼ばれるか**を、実際のコード行で追う表です。説明する順序（【契約】→【安定骨格】→【具体】→【生成】→【注入】→【利用開始】）と、実行時の呼び出し順（【生成】→【注入】→【利用開始】→【安定骨格】→【契約】→【具体】）は違うため、2つの表を分けています。設計を組み立てる順と、プログラムが動く順は別物です。
...
| 6：対策検討 | 課題ID別の節（【契約】〜【利用開始】） | 確定した構造の責任分離・生成・所有・選択・依存の受け渡し・安定側からの実行を、課題IDごとに`【契約】 →【安定骨格】 →【具体】 →【生成】・所有 →【注入】登録・注入 →【利用開始】実行`の順で反映する |
| 6：対策検討 | 6-1：生成・所有・実行順のまとめ | 誰が何を生成・所有し、どの順で安定側から呼ぶかを、課題IDをまたいで一度に確認する |
...
flowchart TB
    ...
    E["判断ごとの部分クラス図"]
    F["採用全体クラス図"]
    G1["課題ID1節<br/>【契約】→【安定骨格】→【具体】<br/>→【生成】・所有→【注入】登録・注入→【利用開始】実行"]
    G2["課題ID2節<br/>同じ読書順で接続点を実装"]
