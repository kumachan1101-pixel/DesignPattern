...
| フェーズ6内：候補案を棚卸しする | フェーズ5の課題から、何もしない・関数化・クラス分離・契約導入・登録/組み立て移動などの案を出す |
...
フェーズ5の出口は、フェーズ6の入口です。課題ごとに次の6点を「課題カード」として言えない場合は、まだ対策検討へ進みません。

| フェーズ6へ渡すもの | 内容 |
|---|---|
| 根拠と着目コード | フェーズ3のどの痛みを解くのか。どのクラス・メソッド・分岐・生成・登録を見るのか |
| 変えたいもの | 判定条件、計算式、処理、生成判断、通知先、状態ごとの振る舞いなど |
| 変えたくないもの | 処理の骨格、利用側コード、業務フロー、共通手順など |
| 接続点に残す約束 | 引数、戻り値、操作名、イベント名、登録方法、生成済みオブジェクトなど |
| 守る制約 | 既存動作、性能、エラー処理、所有関係、実行順序、テスト容易性など |
| 完了条件 | 同じ変更要求を再度当てたとき、不要にしたい修正・再テスト範囲 |
...
設計判断の出発点は、フェーズ3で仕様変更を当てた後の「痛み」です。フェーズ6の冒頭では、フェーズ5の課題カードが指定したクラス・メソッド・分岐・生成・登録だけを、フェーズ3の関連コードからおさらいします。課題に関係しない現行コード全体は再掲しません。読者が覚えておく必要がある範囲を、これから変更する範囲と一致させるためです。

各課題では、この関連コードを変更前として、変更後を段階ごとに示します。省略したDB・Repository・画面・ログなどの不変部分はコードブロックの外で明記します。採用後の全体コードはフェーズ7で掲載し、動作例と実行結果で確認します。

### 課題をコード変更へ変換する

フェーズ5の課題文だけでは、まだコードの最初の一手を決められません。課題ごとに次の6点をそろえると、対策検討の入口が具体化します。

| 課題から取り出すもの | コード上で確認すること |
|---|---|
| 根拠 | フェーズ3のどの修正・再テスト範囲が痛みだったか |
| 着目箇所 | どのクラスの、どのメソッド・分岐・生成・登録を見るか |
| 移すもの | 判定条件、計算式、処理順序、生成判断、通知タイミングなどの何を動かすか |
| 残すもの | 呼び出し側に残す操作・値・型・イベントと、守る業務フロー |
| 制約 | 既存動作、状態更新、副作用、所有関係、性能など、落としてはいけないもの |
| 完了条件 | 同じ変更要求を再度当てたとき、どの修正・再テストが不要になれば解決と言えるか |

この6点が、フェーズ5からフェーズ6へ渡す「課題カード」です。課題が複数ある章ではカードを分け、1枚ずつ検討します。複数課題を一度に完成形へ変換すると、どの変更がどの課題へ効いたのか分からなくなるからです。

フェーズ6冒頭の「見る候補」と課題カードは、別々の課題ではありません。同じ課題IDで接続します。「見る候補」は、原因と課題から**どんな変更を試すか**を示します。課題カードは、その候補をコードへ当てる前に、**どこを変え、何を守り、何をもって完了とするか**を固定します。候補表のP1は課題カードのP1へ、そのまま次の関連コードP1へ進みます。

### 一つの課題を段階的に解く手順

各課題では、次の流れを繰り返します。

1. 課題カードの「着目コード」に該当するフェーズ3の関連部分をおさらいする。
2. 課題の原因へ直接効く、最小のコード変更を示す。
3. 完了条件へ照らし、減った変更と残った問題を確認する。
4. 残った問題が次の変更を必要とする場合だけ、その理由を書いて次へ進む。
5. 問題が十分に小さくなったら、そこで止める案も候補に残す。
...
いきなり完成形（クラス図や採用後の全体コード）へ飛ばず、課題カードを1枚ずつコードへ当てます。各章のフェーズ6は次の順で進みます。

- **課題箇所のおさらい**。課題カードの着目コードに該当するフェーズ3の関連部分だけを示します。候補表・課題カード・関連コードは同じ課題IDで対応させます。
