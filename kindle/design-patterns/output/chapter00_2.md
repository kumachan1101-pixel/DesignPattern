...
> 以下の7フェーズはプログラミング特有の魔法ではありません。ビジネスの現場で行われる基本的な課題解決プロセス★本当にビジネスの現場で使われているのか。ソフトウェアに限らず、別の例で挙げてほしい。使われているかはわからないから、ビジネスの課題もこの流れで解決できると考えているくらいにとどめた方が良いのでは？（**現状把握 ⇒ 仮説立案 ⇒ 問題特定 ⇒ 原因分析 ⇒ 課題定義 ⇒ 対策検討 ⇒ 実施**）と同じ論理で進みます。★ここは具体的に補足したい。まず現状を整理して、変わりそうなところの仮説を立てる、そうすると、問題が明らかになる、その原因何か、原因を解決するために何が課題になっているのか特定して、対策を検討して、最終的にコードを決定する、という流れ。
...
> **産物：** 確定した変動/不変テーブル（根拠付き）。変わりそうな仕様とその理由が仮説として明記されたもの。
