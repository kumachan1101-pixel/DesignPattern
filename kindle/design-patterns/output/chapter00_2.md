...
対策案を作るとき、いきなり「正解のパターン」へ飛んではいけません。手続型のコードがどのような限界にぶつかり、なぜオブジェクト指向やデザインパターンが必要になるのかを、以下の5ステップの段階的進化で確認します。

*   **Step 1：丸ごと関数化**（とりあえず関数に分ける。中身はスパゲティ）
*   **Step 2：処理の関数化**（計算などの処理部分を分ける。条件の複雑さが残る）
*   **Step 3：条件の関数化**（条件も分け、構造の一貫性に気づく。しかしif文は残る）
*   **Step 4：別のクラスへの分離**（具体クラスに分けるが、呼び出し側は全クラスを直接知っている。「具体×直接」の限界）
*   **Step 5：インターフェース化とDI**（インターフェースを導入し、if文ごと呼び出し側に追い出す。「抽象×直接」などへの進化）

この5ステップのどこで止めるかは、接続の形（具体か抽象か、直接か間接か）の到達点を示しています。

### 「接続の形を変える」とはどういうことか

ステップが進むほど、接続点の抽象度と間接度が上がります。2×2マトリクスは、この進化の「到達点（マップ）」として機能します。

```text
Step 1〜4：[A] → [B]                        ← 具体×直接（シンプルな接続）
Step 5：   [A] → «interface» → [B]          ← 抽象×直接（型だけで接続）
...
|:---|:---|:---|
| Step 1〜3（関数化） | 低 | 高 |
| Step 4（具体×直接のクラス化） | 低〜中 | 高〜中 |
| Step 5（抽象化・DIの導入） | 中〜高 | 低 |
