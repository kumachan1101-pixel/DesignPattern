...
フェーズ1では、現状コードが満たす要求へ`要求ID1`、`要求ID2`……を付けてベースライン化します。正常系だけでなく、エラー時に値を変えないこと、保存・通知・内部診断など、変更後も維持する要求を含めます。利用者・運用者から見た「まとまった働き」を束ねる別番号（機能ID）は設けません。追跡は要求ID・変更ID・課題IDで行い、既存動作が落ちていないかは要求IDの回帰で確認します。
...
フェーズ6では、確定課題から接続点を変える三つの観点――分離方法、配置場所、組み立て方法――を順に考えます。責任、継承、実装、保持、依存を変える判断ごとに部分クラス図を示し、最後にそれらを統合した最終構造をコードを書く前に確定します。組み立て方法には、生成・所有・登録・選択・注入・実行までを含めます。その後は課題IDごとに節を分け、確定した構造を`①契約 →②骨格 →③具体 →④生成 →⑤注入 →⑥実行`の順で組み立てます。これは案が揺れた履歴ではなく、確定した構造を理解できる単位で読む順序です。
①〜⑥は、その課題の採用設計をコードへ反映する順番を示します。骨格を持たない差し替え構造では②を置かないなど、章と課題によって使う番号は変わります。
★↑は、文章だけではなく、適切に図を取り入れて説明してほしい。
### 掲載コードを手元で動かす
...
| 6：対策検討 | 課題ID別の節（①〜⑥） | 確定した構造の責任分離・生成・所有・選択・依存の受け渡し・安定側からの実行を、課題IDごとに`①契約 →②骨格 →③具体 →④生成 →⑤注入 →⑥実行`の順で反映する |
...
> - **「実施★対策実施が適切では？」**（フェーズ7で確認）：採用した対策をコードとして組み上げ、動作例・変更影響・得たもの/諦めたものを確認することです。
...
特に「現状コード」という言葉は、読むタイミングによって指す対象がぶれやすいので、本書では必要に応じて次のように呼び分けます。★以下の呼び方は実際に使っている？
...
フェーズ1では、現状コードが満たす要求へ`要求ID1`から欠番なくIDを付けます。利用者または運用者が入力と結果で識別できる単位で書き、メソッドやクラス一つずつを要求として数えません。正常系だけでなく、エラー時に値を変えないこと、保存・通知・内部診断など、変更後も維持する要求を含めます。まとまった働きを束ねる別番号（機能ID）は設けず、追跡は要求ID・変更ID・課題IDで行います。
...
★入力と成果物がない
### 痛みの根本にある設計の問題を言語化する　★問題のきっかけとなる原因の言語化では？
...
**分けると「単体」と「接続点」が生まれる**　★この説明は、課題定義のフェーズで上げる方が良い？
...
★この入力と成果物はフェーズに対して紐づけるのでは？ここでいうと、5-1に限定されている。他とぶれていないか。他も同じかも。フェーズ単位で入出力を定義した方がシンプルではないか。
...
**設計の醍醐味は、突き詰めると四つの動作に尽きます。**

1. **共通点を見つけて分離する** ―― バラバラに見える処理から、変わらない骨格・共通の契約を見抜き、変わる部分をそこから切り出す。★共通でない部分で、どの部品に分離するかが２番に来てもよいのでは？ここでは、共通点を見つけると分離するの２つの事を行っている。
2. **生成する** ―― 切り出した変わる部分を、独立した部品（具体クラス）として作れるようにする。
3. **依存注入する** ―― その部品を、安定側の外から依存として渡す（生成・選択・登録・注入）。
4. **実行部は意識しない** ―― すると安定側の実行部は、具体が何であるかを知らないまま、契約だけを呼んで動く。

本書の全12章は、題材（割引・状態・通知・決済…）こそ違え、毎回この同じ四拍子をたどります。デザインパターン名は、この四拍子を**どう実現したか**を後から共有するための語彙にすぎません。パターン名から入るのではなく、目の前のコードでこの四拍子を回せることが、変更に強い設計の核心です。
...
次の流れは共通の骨格を示した例です。分離の単位、配置場所、生成・所有・登録・選択・注入の方法が異なるため、使う番号と組み立て順は章と課題ごとに変えます。ただし、三つの設計決定に答えず、変えたくない側へ具体的な条件や生成判断を残したままにはしません。★以下も図で表現したい
...
課題ID1・課題ID2…の接続点表とシステム全体の課題
  ...
  設計判断ごとの部分クラス図
      ↓ 部分図を統合して全体を確定
  採用する変更後クラス図
      ↓ 確定した差分を課題IDごとの組み立て順へ変換
  課題ID1節：①契約 →②骨格 →③具体 →④生成 →⑤注入 →⑥実行
  課題ID2節：同じ順で、その課題の接続点を組み立てる
      ↓ フェーズ7
...
この例で見たいのは、端子やケーブルの種類そのものではありません。機器を交換する要求があるなら、「ケーブルを交換できるか」を見ます。利用者に複雑な変換手順を見せたくないなら、「変換をアダプタ側へ移せるか」を見ます。複数の機器を組み合わせたいなら、「ハブが必要か」を見ます。★以下もすでに登場した絵で示す
...
要求の過不足は別の受入エビデンス表で確認します。★以下のフォーマットに各章なっているのか？違うなら、どちらが正しいか確認して統一して
