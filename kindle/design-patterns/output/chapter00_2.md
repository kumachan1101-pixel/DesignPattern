...
> **変更前から抜き出す箇所：** 第11章のフェーズ3「変更を試みる（3-1）」にある`ChangedReportGenerator::execute()`の本文分岐
>
> **ここで確認するコード：** `IReport`の契約と、代表具体`ExecutiveMonthlyReport`の本文生成
...
| 6：対策検討 | 構想を決める | 主要クラスと、契約・具体・生成・選択・受け渡し・実行の全体経路を先に一つへまとめる |
| 6：対策検討 | 構想をコードでつなぐ | 契約、代表具体、生成・選択・受け渡し、実行骨格、公開入口の要点コードを続けて照合する |
| 6：対策検討 | 構想を採用する | 課題IDと将来リスクへ照合し、完成物を重複掲載せず採否を決める |
| 7：対策実施 | 7-1：解決後のコード | 完成クラス一覧・採用構想を実コードの関係として表した完成クラス図・実行シーケンスを先に示し、クラス単位のコードと全有効要求ID別の受入エビデンスで照合する |
...
対策検討では、完成クラス図を先に描かず、主要クラスの構想表と要点コードで、どの具象を生成・所有・登録・選択し、どの利用側へ渡すかを決めます。`main()`は関数なので、フェーズ7の完成クラス図にも架空のクラスとして足しません。完成図に表せない起動時の生成や受け渡しは、完成後の実行シーケンスとコードで確認します。
