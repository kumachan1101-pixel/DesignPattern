...
フェーズ6では、複数案を同じ評価軸で比較し、採用する構造をコードを書く前に確定します。その後の「ステップ1」「ステップ2」は、案が揺れた履歴ではなく、確定した構造を理解できる単位で実装する順序です。
...
| 6：対策検討 | 案を同じ基準で比較して採用構造を確定し、完成形へ至る実装順を示す |
...
| フェーズ6内：責任を見直す | フェーズ1のクラス図を引き継ぎ、変わる詳細をどの契約・具体クラスへ分離するかを変更前後で示す |
| フェーズ6内：課題と採用案を一続きにする | 現在→理想の差、境界・契約、完了条件、比較した案、採用案、実装順を同じ課題IDでまとめる |
| フェーズ6内：採用する形を決める | 変更頻度・将来リスク・初期投資コストを比べ、コードを書く前に採用構造を確定する |
...
フェーズ5で課題が明確になったら、フェーズ6では対策案を比較します。実務では案を行き来することもありますが、本書ではその迷いを時系列で実況しません。読者が結論を見失わないよう、判断を次の順に整理して提示します。

1. フェーズ3・4の変更影響と、フェーズ5の境界・契約・完了条件を固定する。
2. 読者が考えそうな案を同じ評価軸で比較し、採用・不採用の理由を明記する。
3. 採用案を変更後のクラス図として確定し、操作・責任・依存の差分を示す。
4. 確定したクラス図をコードへ反映する実装順を示す。
5. フェーズ7の完成コードへ同じ要求を当て、変更影響が縮んだか確認する。
...
変更後クラス図は候補比較の結論です。コードの途中で結論が揺れているようには見せません。後続のコードでは、クラス図のどの差分を実装しているかを毎段階で対応づけます。改善後の変更影響グラフは、完成コードへ同じ変更要求を当てられるフェーズ7で示します。

### 課題カードへ比較結果と実装順をまとめる

課題IDは表の行番号ではなく、独立した課題の識別子です。P1から欠番なく振り、フェーズ5の引き渡し、候補比較、実装ステップ、完成コード、変更影響の確認まで同じIDで追跡します。

| 項目 | P1 |
|---|---|
| 課題ID | P1 |
| 現在→理想の差 | 【現在から消す波及と、残してよい変更】 |
| 切る境界・守る契約 | 【切る操作・依存と、維持する入力・出力・順序・副作用】 |
| 完了条件 | 【同じ要求で触らなくする場所】 |
| 比較する案 | 【読者が考えそうな案】 |
| 採用案・理由 | 【全条件を満たす案と、他案を採らない理由】 |
| コードへの実装順 | 【責任分離→生成・選択→依存の受け渡し→安定側の実行】 |

候補は、コードを書きながら順番に思いついた形で並べません。「何もしない」「関数化」「クラス分離」「共通契約」「仲介・登録」「生成責任の移動」などを、変更頻度、影響範囲、守る契約、導入コストで一度に比較します。採用案が決まってから、コードへの実装順を定めます。
...
3. **分離先**：変わる詳細を、どの契約・具体クラスへ移すか。
4. **生成と受け渡し**：誰が生成・選択・所有し、どう安定側へ渡すか。
5. **安定側の実行**：変えたくない処理が、どの契約だけを呼ぶ状態になるか。
6. **完了条件との対応**：課題カードのどの条件を実現したか。
...
課題P1
  ...
  採用する変更後クラス図
      ↓ クラス図の差分を実装順へ変換
  ステップ1：変わる詳細と共通契約を分離
  ステップ2：具体クラスの生成・選択場所を実装
  ステップ3：依存を渡し、安定側を契約呼び出しへ変更
      ↓ フェーズ7
  完成コードへ同じ要求を当て、変更影響を確認
```

ここで最も重要なのは、パターン名を先に当てはめないことです。フェーズ1〜5の事実から4つの問いへ答え、候補を比較した結果として採用構造を決めます。一方、本文で示す時点では判断を整理し、読者が「何を採用し、コードで何をすればよいか」を一貫して追える完成した説明にします。
...
### 対策検討の進め方 ―― 採用設計を課題ごとにコードへ落とす（6-1〜6-4）

採用案を決めないままコードを書き始めず、まず前工程の変更影響と完了条件を基準に候補を比較します。採用するクラス図を確定してから、その差分を段階的なコード変更へ変換します。各章のフェーズ6は次の順で進みます。

- **課題カード**。現在→理想の差、境界・契約、完了条件を同じ課題IDで固定します。
- **候補比較と採用判断**。読者が考えそうな案を同じ基準で比較し、コードを書く前に採用案を確定します。
- **変更前・変更後クラス図**。フェーズ1のクラス図を引き継ぎ、着目クラスを色と注記で示し、責任・操作・依存をどう変えるかを示します。
- **課題箇所のおさらい**。カードの着目コードに該当するフェーズ3の関連部分だけを示します。
- **6-1 採用設計を段階的に実装する**。分離、生成・選択、依存の受け渡し、安定側の実行を、クラス図の差分と対応させて示します。
- **6-2 契約とデータ配置を確定する**。誰が生成・所有し、どの方法で注入・登録し、利用側が何を知らなくなるかをまとめます。
- **コード適用結果**。課題IDごとに、採用設計、実装順、守った契約、完了条件の判定をまとめます。
- **6-3 採用設計の統合クラス図**。冒頭で確定したクラス図と段階コードが一致しているかを、全クラスを含む図で再確認します。
- **6-4 完了条件の確認**。採用設計が課題カードの全条件をコード上で満たしたかを確認します。
- **フェーズ7の完成コードと変更影響**。同じ課題IDで実装後の動作を確認し、同じ要求を再適用した変更影響グラフを示します。

関数化、クラス分離、契約、登録、生成責任の移動を、すべての章で同じ順に実行する必要はありません。候補比較では各案が何を解き何を残すかを整理し、採用後はその章で必要と決めた操作だけを実装ステップとして示します。
...
クラスを分離するときは、①誰が具体実装を選ぶか、②誰が生成するか、③誰が破棄まで所有するか、④コンストラクタ・登録・生成契約のどれで注入するか、⑤利用側に具体クラス名が漏れていないか、⑥新しい実装を追加すると組み立てのどこが変わるか、を採用設計の時点で決めます。段階コードでは、この決定をどのコードが実現するかを順に示します。
...
どこまで設計を進めるかはコードを書く前の比較で決めます。各実装ステップでは採用設計のどの部分を実現したかを確認し、同じ変更要求を当てた最終的な修正範囲はフェーズ7で確認します。
