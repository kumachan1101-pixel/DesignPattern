...
フェーズ6では、確定課題から接続点を変える三つの観点――分離方法、配置場所、組み立て方法――を順に考えます。責任、継承、実装、保持、依存を変える判断ごとに部分クラス図を示し、最後にそれらを統合した最終構造をコードを書く前に確定します。組み立て方法には、生成・所有・登録・選択・注入・実行までを含めます。その後は課題IDごとに節を分け、確定した構造を`【生成】・所有 →【注入】登録・注入 →【利用開始】実行 →【安定骨格】 →【契約】 →【具体】`の順で組み立てます。**この並びは、プログラムが実際に動く順です。** 部品を作って渡し、公開操作を1回呼び、骨格が委譲し、契約を経て具体が答える、という流れをそのまま追えます。案が揺れた履歴ではありません。
...
各章のフェーズ6には、代表ケース1件を同じ順で追う実行接続表を置きます。断片コードを読む順と、表で追う順と、プログラムが動く順は、すべて同じです。
...
各課題IDの節にはもう1つ「代表ケースの実行接続」という表があります。こちらは**実行時に何がどの順で呼ばれるか**を、実際のコード行で追う表です。全貌表が「どこからどこへ移るか」を示すのに対し、この表は「どの行がどの行を呼ぶか」を示します。どちらも並び順は【生成】→【注入】→【利用開始】→【安定骨格】→【契約】→【具体】でそろえてあるので、上下に読み比べられます。
