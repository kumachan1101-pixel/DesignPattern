...
①〜⑥は、その課題の採用設計をコードへ反映する全章共通の番号です。②の「安定骨格」はTemplate Methodの基底アルゴリズムだけを指しません。具体実装が増減しても維持する、選択・委譲・反復・配布・履歴移動・生成後の呼び出しなど、利用者から契約を使うまでの制御を表します。どの章でも②を省略せず、何を安定させる骨格かを章固有の名前で示します。
