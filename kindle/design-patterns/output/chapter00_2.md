...
> - **「対策」**（フェーズ6で確定）：課題の完了条件をすべて満たす最終構造のこと。完成形が複数ある場合だけ比較し、関数化やクラス分離は採用構造を実現するコード変更として扱います。
...
| 完了条件 | 【同じ要求で触らなくする場所】 |
| 比較する最終構造 | 【全完了条件を満たす完成形。複数ある場合だけ記載】 |
| 採用する最終構造・理由 | 【採用する完成形と、目的に合う理由】 |
| コードへの実装順 | 【責任分離→生成・選択→依存の受け渡し→安定側の実行】 |

比較対象は、課題カードの完了条件をすべて満たす最終構造だけです。最終構造が複数あるときは、変更影響と導入コストで比較します。一つしかないときは比較表を作りません。関数抽出だけ、Factoryへの分岐移動だけなど、完了条件を残す変更は完成案ではなく、採用構造を実現する途中の操作です。
...
原則はチェックボックスではありません。インターフェースが不要な章もあれば、仲介役だけで十分な章もあります。課題カードの完了条件を満たす最小の最終構造を選びます。

### 対策検討の進め方 ―― 採用構造を課題ごとにコードへ落とす（6-1〜6-4）

フェーズ6で必要なのは、次の5点です。

1. 課題カードで、目的・境界・完了条件を固定する。
2. 全条件を満たす最終構造が複数ある場合だけ比較し、採用構造を決める。
3. フェーズ1のクラス図を変更前・変更後で示し、移す責任と残す責任を色と注記で示す。
4. 課題箇所のコードだけを再掲し、クラス図の差分を必要な実装ステップへ変換する。
5. 課題IDごとに対応コードと完了結果を確認し、フェーズ7で変更影響を再測定する。

6-1では責任分離・生成と選択・依存の受け渡し・安定側の契約呼び出しを実装し、6-2では契約とデータ配置を確定します。6-3は冒頭の変更後クラス図とコードの一致だけを確認し、図を重複掲載しません。6-4は課題カードの完了条件を確認します。

中間コードは課題に関係するクラス・メソッド・生成箇所だけを示します。部分的な操作を比較案にはせず、採用した最終構造のどの差分を実現するかで位置づけます。

| 完成した最終構造 | 完了条件をどう満たすか | 残る変更・受け入れるコスト | 判断 |
|---|---|---|---|
| 【最終構造A】 | 【全課題IDへの答え】 | 【登録・設定・クラス数・運用・性能】 | 【採用／不採用と理由】 |
| 【最終構造B】 | 【全課題IDへの答え】 | 【登録・設定・クラス数・運用・性能】 | 【採用／不採用と理由】 |

この表は最終構造が複数ある場合だけ使います。「何もしない」「関数だけ切り出す」「分岐を別クラスへ移す」など、完了条件を満たさない途中状態は比較しません。

採用後のコード変更は、次の形で課題と結びます。
...
比較するのは実装途中の段階ではなく、完了条件を満たす最終構造同士です。各構造について、現在の導入コストと、想定変更を繰り返したときの将来コストを見積もります。
...
このフェーズの出口は、課題IDごとに「どの最終構造を採用したか」「クラス図のどの差分を、どのコードで実現したか」「完了条件を満たしたか」を説明できることです。
