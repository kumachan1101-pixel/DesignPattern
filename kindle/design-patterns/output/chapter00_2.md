...
> [!TIP] この章の読み方
> この章は全7フェーズの詳細な説明書です。最初から通読してもよいですが、**各フェーズの冒頭にある「目的」「入力」「産物」だけ読んで先の章へ進み、疑問が出たときに戻ってくる**という使い方が馴染みやすいかもしれません。「このフェーズで何を受け取り、何に変えているんだろう？」と感じたら、この章の該当セクションが手がかりになります。
...
各フェーズは、前のフェーズで得た産物を入力として受け取り、次のフェーズで使える形に加工します。
...
flowchart TB
    ...
    O1["産物<br>入力→加工→出力の仕様構造<br>動作例・責任<br>依存/分岐/処理順序<br>変更要求"]

    P2["2 仮説立案<br>変わる理由を確認する"]
    O2["産物<br>変わるもの/変わらないもの<br>今回確定・将来リスク<br>当面安定の前提"]

    P3["3 問題特定<br>変更を当てて痛みを見る"]
    O3["産物<br>修正箇所<br>波及範囲<br>再テスト範囲"]

    P4["4 原因分析<br>痛みの構造を説明する"]
    O4["産物<br>混在している判定/処理/生成判断<br>漏れている依存<br>保ちたい骨格"]

    P5["5 課題定義<br>解くべき接続点を定める"]
    O5["産物<br>接続点<br>流れるデータ<br>分ける単位・制約"]

    P6["6 対策検討<br>案を比べて選ぶ"]
    O6["産物<br>採用案<br>移す判定/処理/生成判断<br>残る変更・受け入れるコスト"]

    P7["7 対策実施<br>コードで実証する"]
    O7["最終産物<br>改善後コード<br>変更シナリオ表<br>得たもの・諦めたもの"]
...
第1章以降では、この7フェーズをさらに小さな項目に分けて進みます。各項目は、読者に情報を渡すためだけではなく、次の項目へ進むための理解を作るために置かれています。
...
| 2：仮説立案 | フェーズ1の事実から変わりそうな仕様を仮説化し、ヒアリングで裏付ける |
| 3：問題特定 | 変更を試みたとき、どこを修正し、どこまで確認が広がるかを見る |
| 4：原因分析 | 痛みを、混在している判定/処理/生成判断・責任・依存として説明する |
| 5：課題定義 | いきなり実装案へ飛ばず、解くべき接続点と制約を定める |
| 6：対策検討 | 複数案を比べ、採用する形と受け入れるコストを決める |
| 7：対策実施 | 最終コードで責任がどこへ移り、変更影響がどう減ったか確認する |

この表はチェックリストではありません。各章を読むときの地図です。「なぜここで動作例テーブルが必要なのか」「なぜクラス図の前にクラス説明があるのか」と迷ったら、この表に戻ってください。

また、本書では文章だけで全体像を説明しきろうとはしません。
入力から出力までの流れ、クラス間の関係、変更影響の広がり、改善前後の差分は、できるだけ図や表で先に見えるようにします。
図は装飾ではなく、「いま何を見ているのか」「前のフェーズの産物が次にどう使われるのか」を読者が迷わないための地図です。
...
> **産物：** 入力→加工→出力の仕様構造、動作例、責任一覧（事実のみ。この段階では仮説を立てない）
...
> **入力：** 前半で把握した仕様・動作例・クラス責任 ＋ 実際の実装コード ＋ 今回の変更要求
> **産物：** 動作例と実行結果の対応、具体クラスへの依存、条件分岐、処理順序、受け渡すデータの観察結果
...
> **産物：** 変わるもの/変わらないものの確定表（根拠付き）。今回確実に変わること、将来リスク、当面安定している前提が分けて整理されたもの。
