...
> **産物：** 課題定義表（接続点・非機能制約・現在の直接呼び出し元の整理）
...
接続点を「今まさに直接呼び出しているクラス」を確認します。これにより、「変化が起きたとき、現在どの範囲まで影響が波及するか」という現在の痛みの広がりが見えてきます。ここで確認するのは**現状**であり、解決後の話ではありません。

```text
例：
- OrderProcessor は DiscountRule.calculate(price, type) を直接知っている
  → DiscountRule が変わるたびに OrderProcessor も変わらざるを得ない（現在の痛み）
- PaymentService は DiscountRule を呼んでいない
  → この接続点から切り離されており、現在は問題ない
```

接続点を直接知っているクラスが多いほど、現在の設計の「痛みの広さ」が大きいことを意味します。
...
|---|---|---|---|
| 接続点A | 変わる理由が異なる | ホットパスではない | OrderProcessor |
| 接続点B | 複雑さを隠したい | 呼び出し頻度は低い | なし |
