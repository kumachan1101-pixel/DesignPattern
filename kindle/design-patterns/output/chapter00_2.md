...
| 🔴 **対策検討** | ステップ5 | 「どの手札を使うか」を決める |
| 🟢 **比較・決定** | ステップ6〜7 | 「どの案を選ぶか」を決めて実装・検証する |
...
責任が混在している構造では、複数の次元で問題が起きていることがほとんどです。「①分割する」と「③規格化する」と「⑤置換する」を同時に適用することで、初めて構造がすっきりします。
...
> [!NOTE] 「構造の発展」は評価の軸でもある
> ⑤置換する・⑥拡張する は、他の手札（①〜④）を適用した結果として得られる**設計の評価基準**でもあります。
> 「この設計で、将来⑤置換できるか？⑥拡張できるか？」という問いが、ステップ6の比較・決定フェーズで使う評価軸になります。
> 対策を選ぶ手札（①〜④）と、設計を評価する軸（⑤⑥）は役割が異なります。
> [!NOTE] ケース分けについて
...
> | 原因特定（ステップ4） | ①分割・②隠蔽の必要性 | ③規格化・④間接化の必要性 | ⑤⑥の安全な実現方法 |
