...
フェーズ6では、確定課題から接続点を変える三つの観点――分離方法、配置場所、組み立て方法――を順に考えます。責任、継承、実装、保持、依存を変える判断ごとに部分クラス図を示し、最後にそれらを統合した最終構造をコードを書く前に確定します。組み立て方法には、生成・所有・登録・選択・注入・実行までを含めます。その後は課題IDごとに節を分け、確定した構造を`①契約 →②安定骨格 →③具体 →④生成・所有 →⑤登録・注入 →⑥実行`の順で組み立てます。これは案が揺れた履歴ではなく、確定した構造を理解できる単位で読む順序です。
①〜⑥は、その課題の採用設計をコードへ反映する全章共通の番号です。②の「安定骨格」はTemplate Methodの基底アルゴリズムだけを指しません。具体実装が増減しても維持する、選択・委譲・反復・配布・履歴移動・生成後の呼び出しなど、公開入口の内側にある制御の順序を表します。どの章でも②を省略せず、何を安定させる骨格かを章固有の名前で示します。

**②と⑥は別の番号です。** ②は呼ばれる側で、公開入口の内側にある制御順です。⑥は呼ぶ側で、利用者や外部イベントが公開入口を起動する行です。「骨格が契約を呼ぶこと」は②の内部にある契約呼び出しであり、⑥ではありません。同じメソッドに見えても、呼ぶ側と呼ばれる側で責任が違うので、各章は②と⑥を別々の見出しとコードで示します。

説明する順序は①から⑥ですが、実行時の呼び出し順は違います。多くの章では④生成 →⑤注入 →⑥利用開始 →②安定骨格 →①契約 →③具体の順に進みます。各章のフェーズ6には、代表ケース1件をこの順で追う実行接続表を置きます。

番号は用途ごとに形を分けます。フェーズ1の現状コードと仕様の読解順は `(1)`、フェーズ3で変更を試すコードの分割は `[試行1]`、フェーズ6の設計反映順は `①`、フェーズ7の完成コード分割は `【1】` です。本文で途中番号を参照するときも記号を含め、別の段階の番号と取り違えないようにします。
...
flowchart LR
    ...
    C["設計判断ごとの<br/>部分クラス図"]
    D["統合した<br/>採用全体クラス図"]
    E["課題IDごとの実装<br/>①契約→②安定骨格→③具体<br/>→④生成・所有→⑤登録・注入→⑥実行"]
    F["フェーズ7<br/>完成コードと検証"]
...
部分クラス図は未採用案ではなく、採用全体図を判断単位に分けたものです。①〜⑥は、その確定構造をコードへ組み立てる読書順です。
### 掲載コードを手元で動かす
...
フェーズ6の冒頭には「構造ポイントの全貌 ―― どの責任がどこへ移るか」という表があります。①契約から⑥利用開始までが、**変更前はどのクラス・関数にあり、変更後はどこへ移るか**を1枚で示す表です。断片コードは、この表の1行ずつを順に埋めていく作業だと思って読んでください。

各課題IDの節にはもう1つ「代表ケースの実行接続」という表があります。こちらは**実行時に何がどの順で呼ばれるか**を、実際のコード行で追う表です。説明する順序（①→②→③→④→⑤→⑥）と、実行時の呼び出し順（④→⑤→⑥→②→①→③）は違うため、2つの表を分けています。設計を組み立てる順と、プログラムが動く順は別物です。
...
| 6：対策検討 | 課題ID別の節（①〜⑥） | 確定した構造の責任分離・生成・所有・選択・依存の受け渡し・安定側からの実行を、課題IDごとに`①契約 →②安定骨格 →③具体 →④生成・所有 →⑤登録・注入 →⑥実行`の順で反映する |
...
flowchart TB
    ...
    E["判断ごとの部分クラス図"]
    F["採用全体クラス図"]
    G1["課題ID1節<br/>①契約→②安定骨格→③具体<br/>→④生成・所有→⑤登録・注入→⑥実行"]
