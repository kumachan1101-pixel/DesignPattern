...
このフレームワークは仕様変更を起点として動きます。**「すでに稼働しているシステムに、新たな仕様変更や機能追加の要望が来たとき」** に発動するプロセスです。特に、以下のような状況で威力を発揮します。

- 既存システムをあまり把握していない人が、安全に変更を加えるアプローチを探るとき
- 熟練の担当者が、複雑化した課題を改めて整理し直し、設計の妥当性を検証するとき

設計書やコードをただ眺めるのではなく、このプロセスに沿って「システムを理解する → 問題が出てくる → 原因を突き止める → 対策を考える → 実装する」と進めることで、安全で確実な変更が可能になります。
...
8ステップは、目的の異なる **6つの局面** に分かれています。

| 局面 | ステップ | 内容 |
|:---|:---|:---|
| 🔵 **システムを理解する** | ステップ0〜2 | コードを読み、仮説を立てる（変更が来る前の準備） |
| ⚡ **仕様変更が来る** | （トリガー） | 変更依頼を受け取る。以降のステップが動き出す起点 |
| 🟡 **問題が出てくる** | ステップ3 | 変更を試みたら痛みが現れる（変更シミュレーション） |
| 🟠 **原因を突き止める** | ステップ4 | なぜ痛いのかを分析する |
| 🔴 **対策を考える** | ステップ5〜6 | 案0〜N・コスト天秤 |
| 🟢 **実装する** | ステップ7 | 実際に直す |

色分けは局面を示しています（青＝システムを理解する、黄＝問題が出てくる、橙＝原因を突き止める、赤＝対策を考える、緑＝実装する）。以下では各局面の役割とステップを順に見ていきます。
...
## 🔵 フェーズ1：システムを理解する（ステップ0〜2）―― 変更が来る前にコードを把握する
...
## 🟡 フェーズ2：問題が出てくる（ステップ3）―― 変更シミュレーションで痛みを発見する
...
## 🟠 フェーズ3：原因を突き止める（ステップ4）―― 「なぜ辛いのか」を構造的に言語化する
...
## 🔴 フェーズ4：対策を考える（ステップ5〜6）―― 案を組み立て、コストで選ぶ
...
## 🟢 フェーズ5：実装する（ステップ7）―― 決断し、変化に強い設計を手に入れる
...
| ステップ3 | 仕様変更を受け取った後・**変更シミュレーション** | 変更が来たとき、どこが痛いか | 哲学1違反の症状確認       |
| ステップ4 | 痛みを観察した後・**原因特定**   | 独立して変わる部分があるか。分けるべきか    | 哲学1の適用判断         |
| ステップ5 | 分ける場所が確定した後・**接続設計** | なぜ分けたかから接続の形を決める   | 哲学1→哲学2→哲学3の順に適用 |
| ステップ6 | 接続の形を決めた後・**コスト天秤** | 対策案のコストを比較して選ぶ         | 哲学1・2が機能しているか検証  |
| ステップ7 | 対策を確定した後・**実装**  | 何を得て何を諦めたか              | 全哲学の確認と言語化       |
...
> |---|---|---|---|
> | システムを理解する（ステップ0〜2） | 「誰の判断で変わるか」を見極める | — | — |
> | 原因を突き止める（ステップ4） | 独立して変わる部分を見つける | 接続の形を「抽象」にするか判断 | 置換・拡張の安全な実現方法 |
> | 対策を考える（ステップ5〜6） | 分ける境界線の引き方 | 接続点のインターフェース設計 | 継承を避けた拡張 |
