...
コードを読む前に、読者が「このシステムは何を受け取り、何をして、何を返すのか」を同じ粒度で見るための図です。ただし、箱を3つ並べるだけでは不十分です。入力が加工のどの段階で使われ、正常系とエラー系のどちらへ分かれるのかまで示します。図に入れた各要素は、後でどこで確認するかまで示します。

| 図の要素 | 仕様上の呼び名 | 後で確認する場所 |
|---|---|---|
| 入力値・状態 | 利用者から渡される情報、現在状態、外部イベント | 動作例テーブル、実行結果、変更要求 |
| 加工の判定・処理 | 業務上の判定、計算、変換、保存、通知 | 現状仕様の確認、フェーズ2の仮説、フェーズ4の原因分析 |
| 出力・状態変化 | 表示、保存結果、通知、エラー、更新後の状態 | 動作例テーブル、実行結果、変更シナリオ表 |
...
仕様、動作例、実行結果は同じケースを扱います。仕様で説明していない値や状態を、コードで突然使わないようにします。また、仕様図に置いた入力・加工・出力は、動作例、変更要求、接続点のいずれかで再利用されるものに絞ります。コードとの対応は、登場クラスと現状コードを読む段階で確認します。
