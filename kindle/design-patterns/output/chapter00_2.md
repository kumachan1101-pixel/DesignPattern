...
フェーズ1では、現行仕様を`要求ID1`、`要求ID2`……という要求IDでベースライン化します。今回届いた変更依頼には`変更ID1`、`変更ID2`……を付け、各変更IDによって、どの要求IDを継続・変更・追加・廃止するかを決めます。変更後の要求ベースラインはコード変更前に確定し、フェーズ7で全有効要求IDをコードと実行結果へ照合します。変更IDは変更のきっかけ、要求IDは最終システムが満たす要求であり、同じものではありません。
...
フェーズ5では、原因から課題IDへ飛びません。まず課題候補を洗い出し、システム全体で必要性、重複、統合／分割、完了状態を検討します。その後、解くべき接続点ごとに「何が流れるか」「どちら側が変わるか」「どちら側を守るか」「何ができれば完了か」を課題ID1、課題ID2、課題ID3の連番で確定します。変更IDと課題IDは一対一とは限りません。一つの変更依頼から複数課題が見つかることも、複数の変更依頼が同じ課題を露呈することも、局所変更で済み課題が生まれないこともあります。
...
フェーズ1では、まず現状コードが満たす要求へ`要求ID1`から欠番なくIDを付けます。正常系だけでなく、エラー時に値を変えないこと、保存・通知・内部診断など、変更後も維持する要求を含めます。

最後に、この章で検証する具体的な変更依頼へ`変更ID1`からIDを付けます。各変更IDが、どの要求IDを継続・変更・追加・廃止するかを要求差分表で確認し、変更後要求ベースラインを確定します。廃止要求も行ごと消さず、廃止理由を残します。コードに登場する役割、種類、状態、処理順、操作は、その役割が現状にあるなら1-1で、変更依頼により追加されるなら1-5で先に説明します。読者がコードを見て初めて仕様を知る状態にはしません。

このフェーズの出口は、「変更前の要求ID一覧は何か」「変更IDを反映した変更後要求ID一覧は何か」「この章のシステムは何をし、どの入力からどの出力を返し、どのクラスが何を担うか」を言えるところです。
...
フェーズ7では、変更後要求ベースラインの全有効要求IDを実際のコードと受入結果へ照合します。今回変わらなかった既存要求も含めることで、要求が実装から抜け落ちていないかを確認します。7-4では変更IDを使い、今回の変更依頼をフェーズ1の構造へ当てた際の修正範囲と、完成構造で得られた結果を照合します。
...
最初に、完成コードへ登場するクラス一覧、フェーズ6と同じMermaid定義の完成後クラス図、完成コードに対応する実行シーケンスを示します。読者が全体の責任と呼び出し順を把握してから、値・境界・契約・具体クラス・組み立て・`main()`の順に、クラス単位でコードを示します。最後に全有効要求IDの受入エビデンス、課題IDごとの構造改善結果、変更IDごとの変更影響を、それぞれ別の追跡表で整理します。
...
この表には、変更のなかった既存要求も含め、変更後要求ベースラインの全有効要求IDを載せます。要求追跡と課題追跡を分離することで、「要求は満たしたが設計課題が残った」「設計課題は解いたが既存要求を落とした」の両方を検出できます。「諦めたもの」は、クラス数や組み立て管理など、採用構造によって増えたコストとして別に明記します。
