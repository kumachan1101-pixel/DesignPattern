...
フェーズ5で課題が明確になりました。フェーズ6では、接続点の契約、データの置き場所、構造（クラス図）、影響範囲を、**接続まで省略しない候補コード**で比較して採用案を選びます。候補をクラスへ分けるなら、そのクラスをどこで生成し、誰が所有し、どのコンストラクタや登録操作で利用側へ渡すか、実行入口からどう呼ぶかまで同時に考えます。フェーズ7では採用案を本番想定の境界と実行ケースへ広げ、実行結果で効果を確かめます。

設計判断の出発点は、フェーズ3で仕様変更を当てた後の「痛み」です。ただし元の構造を読者の記憶に委ねないため、その直前にフェーズ1の現状完全コードを同じ改行で再掲します。現状コードはおさらい用であり、対策を変更前へ巻き戻すものではありません。フェーズ3で追加した要求をすべて保ったまま痛みを**分解して、接続点の"形"（共通のシグネチャ＝契約）を見つける**ところから始めます。
...
### 対策検討の進め方 ―― 分解して設計する（6-1〜6-4）

いきなり完成形（クラス図）へ飛ばず、痛みコードを分解して設計を組み立てます。各章のフェーズ6は次の順で進みます。

- **6-1 痛みコードを分解して、接続点の「形」を探す**（思考の中心）。痛みコードを実際に変換し、詰まる場所から共通の形（同じ引数・戻り値のシグネチャ）を見つけます。候補コードは対象クラスと呼び出し側を省略せず、変更していない行の順序・インデント・改行を起点コードから維持します。
- **6-2 見つけた形を契約にし、データの置き場所を決める**。共通シグネチャを契約（インターフェース）として定義し、どのデータをどのクラス・リポジトリが持つか（データ配置）を決めます。クラス分離を行う案は、具体クラスの生成、所有、依存注入、実行入口まで含む完全な候補コードで示します。
- **6-3 構造の見立て**。契約とデータ配置を決めた**結果**として、クラス図（現状→対策後）を描きます。図は出発点ではなく結論です。
- **6-4 影響範囲**。その構造で変更要求をもう一度当てると、どこを修正し、どこまで再テストするかを設計段階で確認します。
- **採用**。案を変更頻度・影響範囲・導入コストで比べ、どこで止めるかを決めます。

6-1で試す変換（関数へ切り出す→型を束ねる→契約にする）は、一本道の作業手順ではなく、共通の形にたどり着くための思考です。各変換で「まだどこが詰まるか」を示し、それが次の一手を導きます。章の題材によって不要な変換は飛ばして構いません（生成処理が論点でないなら生成分離は扱わない、複数実装の交換要求がなければ契約導入は見送る、など）。複数の変化軸が絡む章（第二部）では、まず**独立して変わる軸がいくつあるか**を数え、片方だけ分けても他方が残ることを分解で確かめ、軸ごとに別の契約を置きます。

候補コードでは `...` や「以下省略」を使いません。クラス定義を1行に潰さず、起点から変えていない行は同じ並びと改行で残します。クラスを分離したのに生成・注入を示さないコードは未完成です。分離先、生成側、利用側を一組として載せ、具体クラスを知る責任がどこへ移ったかまで比較します。

クラスを分離するときは、①誰が具体実装を選ぶか、②誰が生成するか、③誰が破棄まで所有するか、④コンストラクタ・登録・生成契約のどれで注入するか、⑤利用側に具体クラス名が漏れていないか、⑥新しい実装を追加すると組み立てのどこが変わるか、の順で確認します。この六つが決まって初めて、分離したクラスを実行時につないだ設計が完成します。
