...
**フェーズ4の中心的な問い：2つの向きから見る**

本書が扱う多くの設計問題では、「変化をどう管理するか」が重要な視点になります。フェーズ4では、痛みを観察しながら、向きの異なる2つの問いを置きます。この2つは、フェーズ6で使う2つの考え方（共通化・抽象化／変わる部分の外部注入）と、そのまま対応します。

> 問い1（分ける向き）：「この1つの塊の中に、独立して変わる部分が混ざっていないか？」
> 問い2（まとめる向き）：「別々の場所に、同じ判定・手順・生成の構造が重複していないか？」

問い1は、1つのクラス・メソッドに変わる理由の違うものが同居している痛み（変更が波及する）を見つけます。問い2は、複数のクラス・場所に同じ構造が重複している痛み（同じ修正を何箇所にも書く）を見つけます。多くの章は問い1（分ける）から入りますが、第4章のように「同じ手順が複数のクラスにコピーされている」ことが痛みの中心である章は、問い2（共通化）から入ります。どちらの向きでも、次に見るのは同じ「変化の理由」です。

どちらの問いに答えるときも、判断基準は「変化の理由」です。

| 観察した状況 | 判断 |
|---|---|
| 1つの塊の中で、機能種類・変更のきっかけ・頻度が異なり、独立して変わる | → **分ける候補**（問い1） |
| 複数の場所に、同じ判定・手順・生成の構造が重複し、いつも一緒に変わる | → **共通化する候補**（問い2） |
...
これら6つは、突き詰めると2つの素朴な考え方に整理できます。1つは、**共通する部分に着目してまとめ、抽象化する**こと（1〜3が近い）。もう1つは、**変わる部分を外へ出して差し替え・登録・生成の判断を組み立て側へ移し、本体は中身を意識せず決まった流れを回すだけにする**こと（4〜6が近い）。各章のフェーズ6では、まずこの2つの考え方で読者自身が思いつく案（データの表にする、リストに積む、関数を登録する、など）を挙げてコードで試し、どこまで解けて何が残るかを確かめてから、必要な移動だけを採用します。6つの段階は操作の目録、2つの考え方はその目録を束ねる読者の直感、という関係です。
