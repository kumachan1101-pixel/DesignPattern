...
ここからの検討は、次の2つのよくある考え方を軸に進めます。1つ目は、**共通部分に着目して共通化し、抽象化する**ことです。2つ目は、**変わる部分を分離して外から注入し、組み立て側は中身を意識せず決まった流れを回すだけにする**ことです。以下の各案は、この2つの考え方をこの章の課題へ当てたものです。関数切り出しは1つ目の実行です。本文生成の差し替え・装飾のリスト化・生成操作の単位化は、変わる部分を外から与え、骨格は決まった流れを回すだけにするという、2つ目の実行です。
