...
フェーズ1の仕様表を振り返ります。現在のレポート生成には「基本データ出力」「グラフ追加」「ロゴ埋め込み」「透かし追加」という機能があります。このうち変化が予想される仕様があります。

- **レポートの装飾種類**（グラフ・ロゴ・透かしなど）：業務の分析ニーズやブランドガイドラインの変化に合わせて追加・変更されることがあります
- **各装飾の詳細設定**（グラフの種類・ロゴの位置・透かしの文言など）：部署のニーズや方針変更に合わせて変わります

一方、「データを集計して出力する」という基本的なレポート生成の骨格は、レポート業務の本質として安定している部分です。

**仮説：レポートの装飾種類とその設定は、今後も追加・変更が続く可能性がある。**
...
ヒアリングで「HTML出力形式の追加」と「再実行データ選択の見直し」が予告されました。この変更が来たとき、仕様がどう変わるかを整理しておきます。

| 変更軸 | 現在 | 将来（数ヶ月後） |
|---|---|---|
| 本文生成：週次レポート | 対応済み | 変更なし |
| 本文生成：月次レポート | 今回追加 | 変更なし |
| 本文生成：HTMLレポート | —（なし） | 追加予定 |
| 装飾：グラフ追加 | 対応済み | 変更なし |
| 装飾：ロゴ追加 | 今回追加 | 変更なし |
| 操作履歴：再実行データの選択変更 | —（なし） | 変更検討中 |

HTML 形式追加と再実行ロジック変更が同時に来ると、`ReportSkeleton` の 1 クラスに複数の変更軸が積み重なります。この痛みをフェーズ3で確認します。
...
さらに、ヒアリングで予告された「HTML形式の追加」や「再実行データ選択の変更」も加わると、骨格・装飾・履歴という 3 つの変更軸が `ReportSkeleton` にさらに積み重なることが見えてきます。ヒアリング段階ではまだ仕様が固まっていないため全コードを書ける状況ではありませんが、3 軸が同じクラスに混在する構造は変わりません。
...
3 つの変更軸がそれぞれ独立して変わることを確認します。

| 変更軸 | 変わる理由 |
|---|---|
| 本文生成（週次/月次/HTML） | 出力先・形式の要件（ユーザー部門） |
| 装飾機能（グラフ/ロゴ） | 各装飾の仕様（各機能担当） |
| 操作履歴（Undo/再実行） | 操作管理の要件（UX担当） |

レポート形式ごとの本文生成は独立して変わります（HTML追加がグラフ追加に影響しない）。各装飾機能も独立して変わります（グラフ仕様変更がロゴ処理に波及しない）。操作履歴は履歴管理ポリシーとして独立して変わります。

→ **3 軸を独立して変更できるよう、それぞれ別の分離パターンを当てる**
...
| 🟣 フェーズ2：仮説立案 | 仕様表から「レポートの装飾種類とその設定は今後も変更が続く」という仮説を立て、ヒアリングで裏付けた。今回の確定変更と将来リスクを分けて整理した |
...
| **得られること** | **この章のどこで示したか** |
| --- | --- |
| 1. 変動箇所の識別 | フェーズ2の仮説立案とヒアリングで、レポートの装飾種類を変動要因として特定した |
