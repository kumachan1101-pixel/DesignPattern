...
graph LR
    ...
    RG -- "具体×直接\n(if-elseが直書き)" --> RG
    PS -- "具体×直接\n(同じロジックを複製)" --> PS
    style RG fill:#ffe0e0,stroke:#cc4444
    style PS fill:#ffe0e0,stroke:#cc4444
```
...
```mermaid
graph LR
    RGA["ReportGeneratorAuto"]
    PS["PreviewService"]
    HM["HistoryManager"]
    AGC["AddGraphCommand"]
    ALC["AddLogoCommand"]
    EPC["ExportPdfCommand"]
    RGA -- "具体×間接" --> HM
    PS -- "具体×間接" --> HM
    HM -- "具体で生成" --> AGC
    HM -- "具体で生成" --> ALC
    HM -- "具体で生成" --> EPC
    style HM fill:#ffffcc,stroke:#aaaa44
    style AGC fill:#ffeecc,stroke:#cc8800
    style ALC fill:#ffeecc,stroke:#cc8800
    style EPC fill:#ffeecc,stroke:#cc8800
```
...
graph LR
    ...
    PS -- "抽象×間接" --> IHM
    CHM -. "実装" .-> IHM
    style IHM fill:#cce8ff,stroke:#4488cc
    style CHM fill:#ffeecc,stroke:#cc8800
    style main fill:#e8ffe8,stroke:#448844
```
