...
現在の構造では、レポート生成の手順が処理の起点に固定で組み込まれています。
...
「グラフ追加」「ロゴ埋め込み」「透かし追加」という3種類の装飾が用意されているのは、それぞれ異なるチームの要望から生まれたものだからです。グラフの有無は分析チームの判断で変わり、ロゴや透かしは広報チームのブランドガイドラインに従います。つまり「誰が変更を決めるか」が装飾ごとに違います。これが後でコードの設計に響いてくる重要な点です。なお、装飾を「重ねて組み合わせる」というルールは、レポートにロゴだけ付けたい場合もあれば、グラフ・ロゴ・透かしをすべて付けたい場合もある、という現実の多様なニーズに応えるためです。ちょうど、書類にハンコを重ねて押すイメージで理解してもらえると思います。
...
「グラフ機能は分析チーム、ロゴ・透かしは広報チーム」と担当が分かれているのは、それぞれが専門知識を持つ領域だからです。グラフの表示条件はデータ分析の知識がなければ正しく決められず、ブランドロゴの配置は広報が守るガイドラインに従います。この「要件を決める人が機能ごとに違う」という構造は、コードの設計にも影響します。要件を決める人が違えば、変更のタイミングも理由も違うからです。この表の「管轄する知識」の列が、後の章でいう「変わる理由が誰にあるか」を示しています。
