...
また、実運用ではレポート生成は画面をブロックしない `ReportJob` としてバックグラウンドで走り、完了・失敗は `JobResult` で受け取ります。ジョブの起動やスケジューリングそのものは、この章の設計論点ではありません。この章では、生成の骨格・装飾・操作履歴の分け方に集中するため、ジョブ実行の入口を `ReportJob`、その結果を `JobResult` という境界スタブで簡略化し、その先の実行基盤は扱いません。装飾処理が途中で失敗した場合の扱いと、失敗したジョブの再実行は、1-5の変更要求で扱います。
...
| 装飾（グラフ・透かし）の適用が途中で失敗する | 装飾適用時（`ReportRenderingApi` 呼び出し中） | この章の1-1では詳細扱いなし。1-5で扱う | 生成ジョブを失敗として記録し、`JobResult` へ失敗を返す |
...
| **生成の実行形態（新規）** | 呼び出しと同期実行 | **`ReportJob` としてバックグラウンド実行し、結果を `JobResult` で受け取る** |
| **装飾失敗と再実行（新規）** | — （なし） | **装飾が途中失敗したジョブは失敗として記録し、同じ生成操作を再実行できる** |

**複雑度ストレス条件**

| 追加する複雑さ | 具体例 | この章で見ること |
|---|---|---|
| 生成ジョブ・バックグラウンド実行 | 生成を `ReportJob` として起動し `JobResult` を待つ | 同期呼び出し前提のまま骨格へ実行形態が漏れないか |
| 装飾の途中失敗 | グラフ描画APIが失敗し以降の装飾を止める | 装飾失敗を骨格へ混ぜず、装飾側と結果で扱えるか |
| 失敗ジョブの再実行 | 失敗した同じ生成操作をもう一度実行する | 生成操作を記録できる単位として持てているか |
...
|---|---|---|---|
| 入力 | レポート種別、出力形式、装飾オプション | レポート種別、出力形式、装飾オプション、操作の種類（生成・やり直し・取り消し・再実行） | 生成・やり直し・取り消しに加え、失敗ジョブの再実行が増える |
| 判定 | テンプレート有無、形式有効、装飾可否 | 同じ判定に加え、履歴から対象操作を取り出せるか、装飾が成功したか | 履歴対象の判定と、装飾失敗の判定が増える |
| 加工 | 本文生成、装飾、出力 | 生成をジョブとして実行し、成功後に操作を履歴へ記録、失敗時は再実行・取消を扱う | 生成操作を記録可能な単位として追い、ジョブの成否も追う |
| 出力 | 生成ファイル | 生成ファイル、`JobResult`（成功/失敗）、履歴更新、再実行/取消結果 | ファイルだけでなくジョブ結果と履歴結果も追う |

**変更後の入力・加工・出力**

変更後の仕様を、1-1と同じ粒度で確認します。1-1の図との差分は、入力に「操作の種類」が加わること、生成をジョブとして実行して `JobResult` で成否を受けること、成功後に「生成操作を履歴へ記録」という加工が挟まることです。もう1つの要求である「生成ステップの制御」は、入力と出力を変えず加工の内部の組み立て方に関わる変更のため、この図には現れません（フェーズ3以降でコードとして確認します）。

```mermaid
flowchart LR
    A[/操作の種類<br>生成・やり直し<br>取り消し・再実行/]:::input --> B{生成/再実行か<br>やり直し・取り消しか}:::decision
    B -->|生成/再実行| C[生成ジョブとして実行<br>テンプレート確認・形式確認<br>装飾の組み合わせ → 本文生成 → 出力]:::process
...
- レポート生成の判定と加工は1-1のまま変わらず、生成をジョブとして実行して `JobResult` で成否を受け、成功後に「生成操作を履歴へ記録」という加工が加わる。
- やり直し・取り消し・再実行は新しいレポート内容を入力とせず、履歴に記録された生成操作だけを材料にする。装飾失敗のジョブは、同じ生成操作を再実行できる形で残す。
- 「生成ステップの制御」は入出力の形を変えないため、この図ではなくフェーズ3以降のコードで差分を追う。

**変更後のエラー条件**

生成ジョブに関わる失敗は、正常系図へ混ぜずに別で確認します。

| エラー条件 | どこで分かるか | 出力 | 保存・通知などの副作用 |
|---|---|---|---|
| 装飾（グラフ・透かし）の適用が途中で失敗する | 装飾適用時 | `JobResult` に失敗を返す | ジョブを失敗として記録し、同じ生成操作を再実行可能に残す |
| 失敗ジョブを再実行する | 履歴の生成操作を取り出す時 | 再実行結果（成功/失敗） | 記録済みの生成操作を材料に、もう一度ジョブを実行する |

図に加わった「履歴へ記録」「再実行・取り消し」「ジョブの成否」が実際にコードのどこへ書かれるかは、フェーズ3で変更を試すコードと、フェーズ7の最終コード・実行結果で追います。
...
| 生成の実行形態（ジョブ・バックグラウンド） | 1-5の変更要求で追加 | フェーズ1の現状コードでは同期呼び出し | 装飾失敗と再実行に関わる実行形態として見る |
| 装飾失敗と失敗ジョブの再実行 | 1-5の変更要求で追加 | フェーズ1の現状コードにはない | 装飾側の失敗と、生成操作の再実行を分けて見る |
| データ取得元、集計式 | データ取得、集計 | `DataReader.readCSV()` | この章では固定データ取得として扱い、今回は深追いしない |

この表から、今回の検討対象は「本文生成」「出力形式」「装飾」「操作履歴」の4つに絞れます。加えて、生成をジョブとしてバックグラウンド実行し、装飾失敗のジョブを再実行できるようにする要求は、この4つのうち「装飾」と「操作履歴」に直接効きます。一方、CSV取得や基本集計の詳細は、この章の変更要求の中心ではないため、必要な前提としてだけ扱います。
...
| 装飾失敗の扱いが増える可能性（一部だけ再試行など） | 継続的に | 生成をジョブ化すると、装飾ごとの失敗と再実行の要望が出やすい |
...
| 生成の実行形態と装飾失敗 | 同期呼び出し、失敗の記録なし | バックグラウンドのジョブ実行と、装飾失敗ジョブの再実行が求められる |
...
**3つ目：ジョブ化と装飾失敗が骨格へ入り込む。** 生成を `ReportJob` としてバックグラウンドで実行し、装飾が途中で失敗したら `JobResult` に失敗を返して再実行できるようにしたい。しかし今の `generate()` は装飾を `if` フラグで直に呼んでいるため、「どの装飾で失敗したか」の判定と、失敗時に同じ生成操作をもう一度実行する仕組みを、骨格の中に書き足すことになります。実行形態（ジョブ）・装飾の失敗・再実行という別々の関心が、生成ロジックへさらに積み重なります。
...
今回追加した複雑さも、この3つの軸へ割り振れます。装飾の途中失敗は「どの装飾を加えるか」という装飾側の関心に属し、失敗ジョブの再実行は「操作を記録・再実行できるか」という操作履歴側の関心に属します。生成をジョブとして実行するという枠組みは、骨格が装飾失敗や再実行を直接抱えなくても、成否を `JobResult` で受け渡せば足ります。
...
| 装飾の成否と、失敗ジョブの再実行の扱い | 生成をジョブとして起動し、`JobResult` で成否を返すという実行の枠組み |
...
| 骨格 → ジョブ結果 | 装飾失敗の判定と再実行の起点を骨格が抱える | ジョブ化と装飾失敗の扱いが生成手順へ波及する |
...
| 生成履歴や再実行・取消は、本文生成や装飾とは別の理由で変わり、失敗ジョブの再実行も同じ軸に属する | 生成操作そのものを履歴に残し、失敗しても同じ操作を再実行できる単位へ分ける | 生成操作を実行・再実行・取消できる単位にする案を見る |
...
| 生成操作の単位化 | 生成・再実行・取り消し・失敗ジョブの再実行 | 変更要求で追加された履歴操作と再実行を扱える | 本文生成や装飾の分離と組み合わせる必要がある |
...
今回足した「装飾の途中失敗」と「失敗ジョブの再実行」も、この分け方の判断材料になります。装飾失敗は装飾のリスト化の側（装飾を包む部品）で扱い、失敗ジョブの再実行は生成操作の単位化の側（記録できる操作オブジェクト）で扱う——このように、生成骨格・装飾・操作履歴を別軸として分けておくと、装飾失敗を骨格へ持ち込まずに済み、再実行は記録した生成操作を材料に扱えます。組み立てや結果の受け渡しには変更が残りますが、骨格そのものへ手を入れる必要は減ります。
...
行7は、生成ジョブの結果を `JobResult`（結果オブジェクト）として受け取る例です。装飾（グラフ描画）が途中で失敗すると、`GenerateReportAction` はファイルを作らずに `JobResult` へ失敗理由を返し、ログへ「失敗」を記録します。描画基盤が復旧したあと、同じ生成操作をもう一度 `execute()` すると、今度は成功して「成功」が記録されます。失敗した装飾を骨格へ混ぜず、生成操作を「記録・再実行できる単位」として持てているため、同じ操作オブジェクトをそのまま再実行できます。実運用では、この `execute()` をバックグラウンドの `ReportJob` から呼び、`JobResult` を非同期で受け取りますが、ジョブ実行基盤そのものはこの章の論点外です。
...
| **課題** | レポート形式ごとの本文生成処理、追加する装飾機能、操作を記録・取り消す履歴管理を、骨格クラスからそれぞれ独立した部品として外に切り出すこと |
...
|---|---|---|---|
| 生成ジョブ・バックグラウンド実行 | 実行形態が骨格の同期呼び出しへ漏れそうになる | 成否を境界で受け渡し、骨格へ実行形態を持ち込まない | 生成操作を `GenerateReportAction` に閉じ、成否は結果として返す |
| 装飾の途中失敗 | 装飾失敗の判定が骨格の `if` へ入り込む | 装飾失敗を骨格へ混ぜず装飾側で扱う | 装飾を装飾連結構造の部品に閉じ、失敗もその中で扱う |
| 失敗ジョブの再実行 | 再実行の起点が生成手順へ混ざる | 生成操作を記録し、同じ操作を再実行できる単位にする | 操作記録構造の `IReportAction` として記録・再実行する |
