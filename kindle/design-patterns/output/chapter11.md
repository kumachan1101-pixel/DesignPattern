...
|---|---|---|---|
| 入力 | レポート種別、出力形式、装飾オプション | レポート種別、出力形式、装飾オプション、操作の種類 | 生成・やり直し・取り消しの操作が増える |
| 判定 | テンプレート有無、形式有効、装飾可否 | 同じ判定に加え、履歴から対象操作を取り出せるか | 履歴対象の判定が増える |
| 加工 | 本文生成、装飾、出力 | 生成後に操作を履歴へ記録し、再実行・取消を扱う | 生成操作を記録可能な単位として追う |
| 出力 | 生成ファイル | 生成ファイル、履歴更新、再実行/取消結果 | ファイルだけでなく履歴結果も追う |
...
| グラフ・ロゴ・透かしなど装飾の組み合わせが条件分岐になる | 装飾を個別に追加・削除でき、順に適用できる形にする | 装飾をリスト化し、同じ操作で順に実行する案を見る |
| 生成履歴や再実行・取消は、本文生成や装飾とは別の理由で変わる | 生成操作そのものを履歴に残せる単位へ分ける | 生成操作を実行・再実行・取消できる単位にする案を見る |
