...
- **生成・注入・実行による組み立て**：実体を誰が生成・所有し、生成経路へ契約として渡してどう実行するかを一本で決める。

### 課題ID1〜課題ID3を二つの判断で解く
...
**ここから決めること：** 共通順と本文、本文と装飾、受付と生成操作の三つの接続点をそれぞれどの形で表し、生成・所有した部品を一つの生成経路へどう接続するかを決めます。
...
**穴が2つ空きました。** 装飾の `wrapped`（内側をどこから受け取るか）と、履歴が受け取る `IReportAction*`（誰が作るか）です。**どちらも具体の名前をここに書くわけにいきません。** 書けば分けた意味が消えます。**それを決めるのが生成の検討です。**
...
**ここが終わった時点で、実体はすべて存在するようになりました。** 組み立て役、生成サービス、履歴、そして要求ごとに作られる本文と装飾。**まだ起きていないのは1つだけです。骨格の契約と書かれた場所へ、具体が入ることです。次に、注入として確認します。**
...
**操作を1つ作ってから、履歴へ渡しています。** 作った `IReportAction*` は履歴が所有し、履歴のデストラクタで捨てられます。生成の検討で決めた所有のとおりです。
...
**守る範囲との照合：** **要求検証**と**出力**を保っていることを、いま確認しました。テンプレートの登録確認も形式の対応確認も生成の前に行われ、書き出しの形式と出力先も3-1のままです。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、二つの判断で照合済みです。**
...
| 生成・注入・実行による組み立て | 生成本体が全分岐を内包 → `ReportAssembler` が完全要求から連なりを生成 | 本文を選び、装飾で包み、操作を履歴へ渡して実行する | 三つの軸が組み立て役以外で不要に交わらない |

公開入口は完全要求を持つ操作を履歴へ渡します。履歴から生成サービス、組み立て役、本文・装飾の具体まで一つの実行経路になります。

#### 代表ケースの実行接続

生成した本文・装飾・操作が実際に使われるまでを確認します。役員向け月次レポートをグラフ付きで生成する1件を一本で追います。

| 実行順・ポイント | 掲載箇所 | 実際のコード接続 | 次の呼出先 |
|---|---|---|---|
| 1. 生成 | `ReportApplication` | 組み立て役・生成サービス・履歴を作る | 実行開始へ |
| 2. 実行開始＋注入履歴 | `ReportActionHistory::submit(IReportAction*)` | 操作を作り、`history.submit(action);` で履歴へ入る | `IReportAction::execute()` |
...
| 5. 注入本文の引き当て＋装飾 | `ReportAssembler::assemble(const ReportRequest&)` | `templateId` で `ExecutiveMonthlyReport` を作り、装飾の並びを1周して `GraphFeature` で包む | 骨格の `IReport*` へ入る |
...
| 8. 注入継承＋契約→具体 | `ExecutiveMonthlyReport::renderBody(...)` | 実体の型で行き先が決まり、役員向け本文を1行足す | 戻って7がフッターを足し、6がグラフを足す |

**6と7の順序に注目してください。** 外側の装飾が先に呼ばれ、内側の本文が後から作られます。**そして戻りは逆順で、本文の文書へ装飾が足されます。** 「重ねた順が適用順」の実態がこれです。

**注入が3回、3つの違う形で現れました。** 2は渡して保持（履歴）、5は要求による引き当てと渡して保持（本文と装飾）、8は継承です。**同じ章に3形がそろうのは、この章だけです。**

**3つの軸は、注入の確認でだけ交わります。** 組み立て役が本文を作り、それを装飾で包み、その連なりを操作が実行する。**それ以外の場所では、互いを知りません。**
...
| 6：対策検討 | 契約と具体による分離、生成・注入・実行による組み立てを一つの完成構造へした | コードを書く前に最終クラス図と実行経路が決まった |
