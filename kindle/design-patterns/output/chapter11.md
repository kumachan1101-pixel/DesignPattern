...
確定要求・将来リスク・対象外を分離します。

| 項目 | 分類 | フェーズ3で変更試行するか | フェーズ7へ実装するか |
|---|---|---:|---:|
...
1. R1では、本文を一つ増やすたびに、共通の生成順を持つメソッドへID分岐を追加する。
2. R2では、装飾を一つ増やすたびに、同じメソッドへ列挙値とAPI呼び出しの分岐を追加する。
...
今回の変更要求である月次本文、指定順装飾、再実行・取消を、完成コードへ再適用します。

| 要求・課題 | コード適用先と実行結果 | 影響しなくなった箇所 | 残る変更先 |
|---|---|---|---|
