...
**PreviewService クラスと main（案1）：**

```cpp
// 案1：PreviewServiceも同じ構造でプライベートメソッドに整理
class PreviewService {
public:
...
**ReportSkeleton クラス（案2）：**

```cpp
// 案2：ReportSkeletonが具体クラスを知り、処理をそのクラスに委ねる
class ReportSkeleton {
public:
    void generate() {
...
**PreviewService クラスと main（案2）：**

```cpp
// 案2：PreviewServiceも同じ具体クラスを知り処理を委ねる
class PreviewService {
public:
    void previewReport() {
