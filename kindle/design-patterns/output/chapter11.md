...
classDiagram
    ...
    ReportGeneratorAuto --> HistoryManager : 具体×間接
    PreviewService --> HistoryManager : 具体×間接
    HistoryManager --> AddGraphCommand : 具体×直接
    HistoryManager --> AddLogoCommand : 具体×直接
    HistoryManager --> ExportPdfCommand : 具体×直接
```

両クラスは具体コマンドを直接知らず `HistoryManager` 経由でのみ操作するが、`HistoryManager` 自身は具体コマンドクラスを直接知っており、コマンド追加時にマネージャーの修正が必要になる。
...
**得られること1**（3つの変わる理由の識別）：フェーズ2の確定テーブルで、「処理の骨格」（Template Method）・「機能追加の組み合わせ」（Decorator）・「操作履歴の管理」（Command）という3つの変化軸が、それぞれ独立して変わりうることを識別しました。「このクラスが変わる理由は何か、そしてそれは誰の判断によるものか」という問いが、複雑なロジックの変化軸を分ける出発点になります。
...
**得られること3**（段階的な責務分離の手法の説明）：フェーズ7の変更シナリオ表で、骨格が変わるときは `AbstractReportGenerator` のサブクラスのみ、機能が変わるときは `IReportDecorator` の実装クラスのみ、履歴管理が変わるときは `ICommand` の実装のみを変えればよい設計を確認しました。3パターンを組み合わせた結果として、それぞれの変化が独立した場所に閉じる構造を説明できます。

**得られること4**（処理の定型化と機能の動的追加の両立）：フェーズ7の最終コードで、実行順序は骨格が保証しながら、機能の組み合わせは実行時に自由に変えられる状態を確認しました。「定型の中に柔軟性を持たせる」という設計の難しさに対して、パターンの組み合わせで答えを出す経験が、現場の複雑なロジックへの向き合い方を変えます。

レポート生成エンジンという题材を通じて、複数の変化軸を段階的に分離する設計の思考を体験できたのではないかと思います。この章で辿った7つのフェーズは、どんな現場のコードにも同じように使える思考の型です。