...
コードを読む前に、このシステムが「どのような入力に対して、何を出力するか」を具体例で確認します。仕様表が「ルール」の説明なら、動作例テーブルは「ルールが実際に動いた結果」のアンカーです。
...
この5行が、この章で設計するシステムの「正解の動き」です。後続の各案（案1〜案4）は、いずれもこれらの動作を実現します。違いは「変更が来たときにどこを触ることになるか」です。
