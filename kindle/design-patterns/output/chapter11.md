...
★ここで一回mainでの実行処理と実行結果を載せませんか。システムの動かし方がイメージできる。

まずこの一連の動きを押さえ、以降で要求、入力とテンプレート、処理順、外部境界、クラス、コードの順に詳細を確認します。
★１２章のように要求IDとは別に機能IDは一覧にしないの？また、次フェーズ以降の変更要求を受け機能IDがどうなるのかも。
#### 現行要求ベースライン
...
### 2-2：今回の変更で確実に変わること
★この一覧をここで登場させるのは不自然では？フェーズ１で出した方が良いのでは？
...
★問題ID⇒原因ID⇒課題IDのトレースを見れるようにすると良い。
#### 課題候補をコード事実へ戻して確認する
...
| 課題ID | 分ける判断（抽象） | 採用する構造 | 共通契約 | 具体を生成する場所 | 具体を知らず実行する場所 |
|---|---|---|---|---|---|
