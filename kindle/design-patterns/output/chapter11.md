...
* **得られること2：** 処理ステップの固定化と、個別の機能拡張のバランスが崩れている接続点を特定できるようになる。
...
```mermaid
quadrantChart
    title Template Method × Decorator × Command ── ★具体×直接（Lightning直差し）
...
変更頻度が「高」であるため、機能追加のたびに `ReportGenerator` 全体を修正するような構造は避けなければなりません。 パフォーマンスへの制約が低いことから、インターフェースや間接層を積極的に活用し、疎結合な構造を目指すことが合理的です。
...
| 接続点B | 機能追加の頻発 | 高頻度の変更 | `ReportGenerator` の生成ロジック |
...
1. グラフ描画クラス（`GraphFeature`）とロゴ配置クラス（`LogoFeature`）を抽出する
2. `ReportGenerator` の中で各クラスを生成し呼び出す

【コード例】

```cpp
void generate() {
    GraphFeature graph; // ← 具体クラスを知っている
    graph.draw();
}
...
1. `IReportFeature` インターフェースを定義する
2. `ReportGenerator` の骨格を `Template Method` として抽象化する
3. `Decorator` で `IReportFeature` をラップして機能を構成する
...
1. `ICommand` を継承した各生成処理クラスを作成する
2. `HistoryManager` クラスが `ICommand` を保持・実行する
3. 生成者は `HistoryManager` に命令を投げる
...
void execute() {
    history.add(new AddGraphCommand()); // 具体的なコマンドを登録
    history.executeAll();
}
...
1. 各機能要素を抽象インターフェース化する
2. Command や Decorator 構成を Factory で構築する
3. すべてを間接層を介して結合する
...
IFactory* factory = new ReportFactory();
IReportFeature* report = factory->create(target); 
...
| 🟡 フェーズ3：問題特定 | 機能追加のたびに生成フロー全体が不安定になる「痛み」を確認した。
 ...
 |
| 🔴 原因分析 | 処理手順と個別機能の「混在」という構造的問題を特定した。

 |
| 🟣 課題定義 | 骨格・装飾・履歴管理という3つの接続点を課題として定義した。

 |
| 🟢 対策案検討 | Template Method, Decorator, Command を複合適用する案4を採用した。

 |
| 🔵 対策実施 | 責務を疎結合化し、変更影響をクラス単位に閉じ込めた。
