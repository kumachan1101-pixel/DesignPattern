...
|---|---|---|
| P1 | レポート種別追加で `ReportSkeleton` の本文生成と共通手順へ波及する | 読込→本文→装飾→描画という生成骨格と既存形式 |
| P2 | 装飾追加・順序変更で `ReportSkeleton` のフラグ分岐へ波及する | 本文生成、描画API、既存装飾の組み合わせ |
| P3 | 履歴・再実行・取消の追加で生成本体と利用側へ波及する | レポート生成操作そのものと既存検証 |

本文・装飾・履歴は独立して変わるためP1、P2、P3の連番にします。フェーズ6では3つの理想枝から別々の課題カードを導きます。

## 🔴 フェーズ6：対策検討 ―― 案を比べ、採用する形を決める

フェーズ6は、フェーズ5で定めた3つの課題——**本文生成を骨格から切り離すこと／装飾を順に組み合わせられるようにすること／生成操作を記録・再実行・取消できる単位に分けること**——を受けて始めます。まずフェーズ4の現在の変更影響から理想の変更影響グラフを組み立て、その差から課題カードと候補を導きます。その後、カードで指定したフェーズ3の関連コードだけをおさらいし、課題ごとに最小の変更を重ねます。課題は「何を切り離すか」までを決めており、**その接続点をどんな形にするか**は、痛みコードを分解して探します。第二部の要は「いくつの独立した変化軸があるかを数える」ことです。各段階で「今何を変えたか」「何が減ったか」「何が残るか」を関連コードで確認し、統合後の全体コードはフェーズ7で初めて示します。
#### 理想の変更影響グラフを先に組み立てる

フェーズ4では、本文種別、装飾、生成履歴が同じ生成処理へ波及していました。変わる理由が3つあるため、理想グラフもP1〜P3へ分けます。
...
flowchart LR
    ...
    subgraph Ideal["理想の波及"]
        IR1["種別を追加"] --> I1["P1: 本文実装と登録だけを追加"]
        IR2["装飾を追加"] --> I2["P2: Feature実装だけを追加"]
        IR3["履歴操作を変更"] --> I3["P3: Action実装・履歴だけを変更"]
...
|---|---|---|---|
| P1 | 種別追加の影響からheader→body→footerの骨格を外す | generate()の本文分岐を切り、固定順序を守る | レポート種別追加で骨格を複製しない |
| P2 | 装飾追加の影響から組み合わせ分岐と生成骨格を外す | addGraph/addLogo判定を切り、内側の本文と装飾順・失敗結果を守る | 組み合わせクラスやフラグ分岐を増やさない |
| P3 | 再実行・取消の影響から本文生成と装飾判定を外す | 生成入力とexecute/undoの境界を切り、成功操作だけを履歴へ積む | 履歴が具体種別・装飾を判定しない |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 固定順序と本文差分をメソッドへ分ける | 骨格と本文の境界が見える | 固定順序を1か所に置き、本文だけを差し替える |
| P2 | 装飾を同じ操作のリストへする | 組み合わせ分岐が減る | 1装飾が1レポートを包む形へ上げる |
| P3 | 生成処理を入力保持する操作単位へする | 再実行・取消の対象が明確になる | execute/undo契約と履歴管理へ接続する |

ここからP1〜P3に該当する生成骨格・装飾分岐・履歴処理だけをおさらいします。
