...
#### 根本原因A（骨格処理の固定化）(Template Method)

新しいレポート形式を追加するとき、なぜ毎回 `ReportSkeleton` を開かなければならないのでしょうか？ それは、このクラス自身が「CSV読み込み → ヘッダー → グラフ/ロゴ → フッター」という**具体的な処理の骨格をすべて直接知ってしまっている（抱え込んでいる）**からです。



#### 根本原因B（機能の動的重ねがけ）(Decorator)

グラフやロゴの組み合わせを変えたいとき、なぜ骨格コードを触る必要があるのでしょうか？ それは、「どの装飾を加えるか」という機能拡張の判断が、骨格の中に `if` フラグとして直接埋め込まれているからです。



#### 根本原因C（操作の記録化）(Command)

操作履歴の管理がなぜ辛いのでしょうか？ それは、「レポートを生成する」という操作の記録が、生成ロジックそのものの中に混在しているからです。
...
> `ReportSkeleton` から切り離する塊は2つある。1つ目は「どの装飾を加えるか（`if` 文の塊）」という装飾機能の知識で、これを `GraphFeature` や `WatermarkFeature` として骨格の外に独立させること。2つ目は「操作を誰が記録し、どう取り消すか」という履歴管理の知識で、これを `GenerateReportAction` として骨格とは別の層に分離すること。この2つを分離して初めて、骨格・装飾・履歴それぞれへの変更が互いに影響しない構造が実現できる。
...
それぞれの処理が別のファイルに分かれたため、一見すると整理されたように思えます。しかし、クラスを分けたにもかかわらず、新しい装飾が来るたびに `ReportSkeleton` を開いて新しいクラスをインクルードし、新しい記述を追加する必要があるでしょう。これが「具体×間接」の限界です。
...
実は、ステップ6で導き出した3つの仕組みには、それぞれ名前があります。骨格を固定する「第4章で学んだ Template Method」、装飾を動的に重ねる「第6章で学んだ Decorator」、そして操作をオブジェクト化する「第5章で学んだ Command」です。

ステップ6で決断したこの構造を、実行可能な完全なコードとして組み上げます。各役割ごとにコードを分けて見ていきましょう。
