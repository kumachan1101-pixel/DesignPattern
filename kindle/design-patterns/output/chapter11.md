...
この表から、今回の検討対象は「本文生成」「出力形式」「装飾」「操作履歴」の4つに絞れます。加えて、生成結果を `JobResult` で受け取り、装飾に失敗した生成操作を再実行できるようにする複雑度ストレスは、この4つのうち「装飾」と「操作履歴」に直接効きます。一方、CSV取得や基本集計の詳細は、この章の変更要求の中心ではないため、必要な前提としてだけ扱います。

### 2-2：今回の変更で確実に変わること

プロダクトオーナーから確定要求として示された変更は次の2点です。装飾失敗と再実行は、この2点へ現実的な失敗条件を加える複雑度ストレスとして確認します。バックグラウンド実行基盤そのものは対象外です。
