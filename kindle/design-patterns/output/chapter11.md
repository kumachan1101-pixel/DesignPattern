...
| **根本原因** | **観察** | **原因の方向** |
| --- | --- | --- |
| 根本原因A：生成手順の固定化と差し替え不能（→Template Methodが必要） | レポート出力の順序を変えるだけで、全体の生成フローを書き直さなければならない | `ReportSkeleton` が生成の「骨格（ヘッダー→本文→フッター）」をハードコードしており、手順の一部だけを差し替える仕組みがないから。 |
| 根本原因B：装飾の動的な組み合わせができない（→Decoratorが必要） | 新しい装飾機能（グラフ等）を追加するたびに、統括クラスを修正しなければならない | `ReportSkeleton` が、各装飾機能の具体的な「実行タイミング」と「生成ロジック」をすべて直接知っており、実行時に組み合わせを変える仕組みがないから。 |
| 根本原因C：操作の記録・取り消しができない（→Commandが必要） | レポート生成の「やり直し」を実装しようとすると、生成フローと履歴管理が複雑に絡み合う | 「レポートを生成する」という処理の定型フローと、「処理を実行した」という操作の履歴管理が、同じクラス内で混在しているから。 |
...
変更の理由（変化の軸）がそれぞれ異なるこれらを、今のままの構造で維持し続けることは、拡張性を損ない、バグの温床となるため限界です。 対策案を検討する前に、今回のリファクタリングで解決する課題を4つの視点で整理し、確定させます。
...
変更頻度が「高」であるため、機能追加のたびに `ReportSkeleton` 全体を修正するような構造は避けなければなりません。 パフォーマンスへの制約が低いことから、インターフェースや間接層を積極的に活用し、疎結合な構造を目指すことが合理的です。 ただし、大量データを扱うレポートでは処理時間が設計に影響するため、非同期処理やキャッシュの必要性を事前に確認しておく価値があります。
...
現在の `ReportSkeleton` は、処理の骨格の中に装飾機能や履歴管理がべったりと入り込んだ「具体×直接」の状態です。 ここから、各責務を独立したインターフェースやパターンへ切り出し、抽象化と間接層を導入する方向で案を検討します。
...
| 手段B：設定オブジェクトを渡す | `ReportOptions` 構造体をまとめて渡す | ✅（引数爆発を一時的に抑制できる） |
...
* 変更容易性：低（機能追加のたびに `ReportSkeleton` が肥大化する）
...
**この形の考え方：**
グラフ描画やロゴ配置といった各機能を独立したクラスに抽出するが、`ReportSkeleton` はそれらの具体クラスを直接生成・保持する。 責任範囲は整理されるが、機能の差し替えや組み合わせの変更には対応できない。

**手段の比較：**

| 手段 | 内容 | ✅ |
| --- | --- | --- |
| 手段A：機能を個別クラスに分離、直接インスタンス化 | `ReportSkeleton` 内で `GraphFeature graph;` と直接生成する | ✅（クラスは分かれるが依存は具体型のまま） |
...
classDiagram
    ...
    ReportSkeleton --> GraphFeature : 具体×直接
    ReportSkeleton --> LogoFeature : 具体×直接
    PreviewService --> GraphFeature : 具体×直接
    PreviewService --> LogoFeature : 具体×直接
```

クラスは分離されたが、`ReportSkeleton` と `PreviewService` の両方が同じ具体クラスへ直接依存しており、機能クラスへの重複した結合が残る。
...
案3で抽象化を導入したことで、レポートの骨格（どんな手順で生成するか）を差し替え可能にできました。しかし、まだ2つの問題が残っています。

1. **装飾の問題**：グラフと透かしを「どう組み合わせるか」は実行時に決めたいのに、現状では組み合わせのパターンごとにクラスが増えます。「グラフ付きレポート」「透かし付きレポート」「グラフ＋透かし付きレポート」……という爆発が起きます。
2. **操作記録の問題**：レポート生成という操作を「後から取り消せる形」で記録する仕組みがありません。
...
**理由：**
レポート生成の定型フローを固定し、グラフやロゴなどの装飾を動的に追加可能にし、さらに生成操作自体を履歴として管理することで、責務を完全に分離し、変更の波及を局所化できるためです。フェーズ2で確認した3つの変動軸（骨格・装飾・履歴）がそれぞれ独立したクラスに閉じる構造を実現できます。
...
フェーズ6の検討の結果、案4（抽象×間接）を採用しました。実装に入る前に、この構造の「名前」を確認しておきます。

**この構造は、Template Method × Decorator × Command の複合パターンです。**
...
sequenceDiagram
    ...
    participant RG as ReportSkeleton
