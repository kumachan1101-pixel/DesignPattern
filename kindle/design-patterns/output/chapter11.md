...
依頼文を、実装結果を判定できる三つの確定要求へ分けます。

| 要求ID | 確定要求 | 入力 | 受入条件 |
|---|---|---|---|
| R1 | 通常月次を保ち、役員向けだけ専用本文にする | テンプレートID | 通常月次と役員向け月次の本文が異なり、週次・部門別は変わらない |
| R2 | 装飾の種類を順序付きで受け、その順に適用する | 装飾列 | ロゴ→グラフとグラフ→ロゴの結果順が入力どおりになる |
| R3 | 受け付けた完全な生成要求を記録し、再実行・取消する | 完全な生成要求 | 同じテンプレート・形式・装飾順・出力先で再実行でき、取消で成果物を削除できる |
...
ここで確定したのはR1〜R3です。クラス、インターフェース、生成方法はまだ決めません。
...
|---|---|---|
| R1 | 役員向け月次テンプレートと専用本文 | 通常月次・週次・部門別、集計値 |
| R2 | 装飾入力を順序付きにし、その順に適用 | グラフ・ロゴ・透かし自体の意味 |
| R3 | 完全な要求の受付履歴、再実行、取消 | 自動再試行、履歴永続化 |
...
これで、H1〜H3がR1〜R3の確定範囲として裏付けられました。
...
|---|---|---|---|
| F1 | HTML形式の追加 | 時期未定 | プロダクトオーナー「将来は検討」 |
| F2 | 再実行で当時のCSVを使う | 時期未定 | プロダクトオーナー「将来は検討」 |
| F3 | 履歴の永続化・上限管理 | 履歴運用の拡大時 | プロダクトオーナーへ今回は受付履歴だけと確認 |

### 2-5：変わる見込みと当面安定の前提を確定する

2-4のF-IDを、レポート生成で変えられるようにする部分と、受付・生成・結果記録の安定側へ分けます。「はい」は、フェーズ6で**形式・入力の再現方法・履歴運用が変わっても、確定要求の生成経路へ影響を広げない構造か**を判定するための印です。

| F-ID・変化軸 | 変わる見込み | 変えられるようにする部分 | 当面安定として守る部分 |
|---|---|---|---|
| F1：HTML形式の追加 | はい | 出力形式ごとの描画処理と登録 | 完全な生成要求、本文と装飾の組み立て、生成結果 |
| F2：再実行で当時のCSVを使う | はい | 入力データの保存・参照方法 | 同じ生成要求を再実行する入口と結果契約 |
| F3：履歴の永続化・上限管理 | はい | 履歴の保存先、保持件数、削除方針 | 完全な要求を記録し、再実行・取消へ渡す契約 |

したがって2-5の出力は、「出力形式・入力再現・履歴運用は変えられるようにし、生成要求から結果までの契約は守る」という設計条件です。フェーズ3ではR1〜R3だけを現在の構造へ適用し、`DebugLog`は変更対象外の内部基盤として維持します。F-IDはフェーズ6の構造評価に使います。
...
1. R1では、役員向け本文のために、共通の生成順を持つメソッドへID分岐を追加した。
2. R2では、ロゴとグラフの指定順適用のために、同じメソッドへ列挙値とAPI呼び出しの分岐を追加した。
3. R3では、生成処理へ受付履歴の保存時点、再実行、取消の規則が加わる。
4. 変更理由は別なのに、レビューとテストの起点がChangedReportGeneratorへ集中する。

ここで確定したのは、R1〜R3を一つのクラスへ追加すると変更影響が集中する、という事実です。
...
フェーズ1でReportGeneratorへ与えた中心責任は、売上を読み、本文と装飾を一つの文書へまとめ、出力境界へ渡すことでした。R1〜R3を追加した後も「順番に進める」責任は必要です。
...
課題IDは、フェーズ4の接続点を上からP1〜P3として追跡するための番号です。

| 課題ID | 原因となる接続と現在→理想 | 守る契約 | 完了条件 |
|---|---|---|---|
| P1 | 共通順→本文：本文ID判断から本文生成の依頼へ変える | 読込→ヘッダー→本文→フッター | 役員向け本文追加で共通順を修正しない |
| P2 | 文書→装飾：種別ifから順序付き共通部品へ変える | 一つのReportDocumentへ追加 | 装飾追加は新部品と組み立て規則だけで済む |
| P3 | 受付→操作：要求vector直操作から同じ操作単位へ変える | 完全なReportRequestを実行前に記録 | 履歴側が本文種別・装飾種別を判定しない |

接続点を呼び出し元、接続データ、変わる側、守る側で確認します。

<!-- 検証契約: | 課題ID・接続点 | 接続するデータ | 変わる側 | 守る側 | -->

| 課題ID・接続点 | 接続するデータと変わる側 | 守る側 | 結果利用 |
|---|---|---|---|
| P1：共通順→本文 | SalesSummaryとReportDocument。変わる側は本文判断 | 読込・ヘッダー・フッターの順 | 完成文書へ本文を残す |
| P2：文書→装飾 | ReportDocumentと装飾列。変わる側は種類・順序 | 一つの文書を渡す契約 | 装飾済み文書を次へ渡す |
| P3：受付→操作 | ReportRequest。変わる側は履歴規則 | 完全な要求を実行前に記録 | 実行・再実行・取消結果を返す |

P1とP2の現状接続では、呼び出し元が本文IDと装飾種別を読み、同じReportDocumentへ結果を追加しています。
...
P3の現状接続では、submit()が要求を保存した後、同じクラスのexecute()へ戻しています。
...
| 要求ID | 通過する課題ID | 課題が完了すると何が変わるか |
|---|---|---|
| R1 | P1 | 通常月次を保ったまま役員向け本文を追加できる |
| R2 | P2 | 入力された装飾列を同じ規則で順に組み立てられる |
| R3 | P3 | 同じReportRequestを記録・再実行・取消できる |

### 5-3：システム全体の完了条件

P1〜P3を個別に切り出すだけでは完了ではありません。最終システムが次を同時に満たす必要があります。

1. R1のテンプレート選択から、P1の本文差分へ到達する。
...
ここでは完成案を一つずつ試しません。P1〜P3を同時に満たす最終構造を、観察した事実から順に導きます。
...
#### P2：なぜ装飾を同じ部品として連結するのか

グラフ、ロゴ、透かしは内容が異なりますが、いずれも「内側で作ったReportDocumentを受け取り、自分の表示要素を一つ追加して返す」という同じ流れです。

また、R2の入力は順序付きの装飾列です。列の先頭から同じ契約の部品を包めば、種類ごとの組合せクラスを作らず、入力順をそのまま構造へ変換できます。
...
P1〜P3を同時に満たす最終構造は、骨格固定構造・装飾連結構造・操作記録構造を直列に再結合する一つのシステムです。途中の部分分離は完了条件を満たさないため、完成案として比較しません。
...
classDiagram
    ...
    note for GenerateReportAction "【P3・新設】要求を保持\n実行・取消"
...
|---|---|---|---|
| P1 | 共通順と本文差分を別責任にする | create()を固定しrenderBody()へ値を渡す | 1 |
| P2 | 装飾を同じ契約の連結にする | decorations順にunique_ptrをFeatureで包む | 2 |
| P3 | 要求操作を履歴所有にする | submit後execute、同じActionでreplay/undo | 3 |
...
for (DecorationType type : request.decorations) {
    if (type == DecorationType::Graph) {
        report = make_unique<GraphFeature>(
            move(report), renderer);
    } else if (type == DecorationType::Logo) {
        report = make_unique<LogoFeature>(
            move(report), renderer);
    } else {
        report = make_unique<WatermarkFeature>(
            move(report), renderer);
    }
}
```
...
| 本文選択 | ReportAssembler | MonthlyReport等→装飾組み立て | unique_ptrで外側へ移動 |
...
|---|---|---|---|
| P1 | 読込・ヘッダー・フッターは共通、本文だけ差分 | ReportSkeleton::create()、renderBody()、各本文クラス | create()がreadCSV→addHeader→renderBody→addFooterを固定し、renderBodyへsummaryとdocumentを渡す |
| P2 | 全装飾は文書へ一要素追加し、入力は順序付き | IReport、ReportFeature、各Feature、ReportAssembler::assemble() | decorationsを先頭から走査し、現在のunique_ptrを対応Featureで包み直す |
| P3 | 再実行には完全な要求が必要 | GenerateReportAction、ReportActionHistory、ReportApplication | submit時にActionをvectorへ移し、その後execute。replayLastは同じActionのexecute、undoLastはundoを呼ぶ |

「本文判定をヘルパー関数へ移すだけ」では、生成本体がID分岐を持つことは変わりません。「装飾ifを別メソッドへ移すだけ」でも、種類と順序の知識は同じクラスに残ります。P1〜P3の完了条件を同時に満たさないため、これらは別の完成案として比較する対象ではありません。
...
ここは将来機能を実装する場ではありません。フェーズ2のF-IDを採用構造へ再適用し、リスクが現実になった場合の変更先と、今回守れる範囲を確認します。

| リスクID・将来リスク | 採用構造での考慮 | 将来の主な変更先 | 今回の判断 |
|---|---|---|---|
| F1：HTML形式の追加 | 形式検証と出力境界を本文・装飾構造から分けているため、新形式の影響先を限定する | OutputFormat、TemplateRegistry、ReportRenderingApi | 本文・装飾・履歴を守れる。HTMLは完成コードへ追加しない |
| F2：再実行で当時のCSVを使う | Actionが完全な要求を保持する境界は使えるが、現状のReportRequestはデータ版を持たない | ReportRequest、DataReaderの取得契約、GenerateReportAction | 一部局所化。スナップショット仕様は未確定なので完成コードへ追加しない |
| F3：履歴の永続化・上限管理 | 履歴所有をReportActionHistoryへ集め、生成・本文・装飾から保存方針を分ける | ReportActionHistory、Action復元方法、将来の履歴保存境界 | 保存方針の変更先は限定できるが、永続化は未解決。完成コードへ追加しない |

F2・F3は現在の構造だけで対応済みではありません。必要になる契約変更を明記したうえで、R1〜R3に不要なスナップショット保存や永続化クラスを先回りして追加しないと判断します。
...
protected:
    unique_ptr<IReport> wrapped;
    ReportRenderingApi& renderer;
public:
    ReportFeature(unique_ptr<IReport> inner,
                  ReportRenderingApi& renderingApi)
        : wrapped(move(inner)),
          renderer(renderingApi) {}
};
...
public:
    ...
    unique_ptr<IReport> assemble(
        const ReportRequest& request) {
        unique_ptr<IReport> report;

        if (request.templateId ==
            "SALES_MONTHLY_EXECUTIVE") {
            report = make_unique<ExecutiveMonthlyReport>(
                reader, renderer, request.format);
        } else if (request.templateId ==
                   "SALES_MONTHLY") {
            report = make_unique<MonthlyReport>(
                reader, renderer, request.format);
        } else if (request.templateId ==
                   "SALES_WEEKLY") {
            report = make_unique<WeeklyReport>(
                reader, renderer, request.format);
        } else {
            report = make_unique<DepartmentReport>(
                reader, renderer, request.format);
        }
...
```cpp
class ReportActionHistory {
    vector<unique_ptr<IReportAction>> accepted;
public:
    OperationResult submit(
        unique_ptr<IReportAction> action) {
        accepted.push_back(move(action));
...
```mermaid
graph LR
    R1["R1<br/>役員向け本文"] --> B1["ExecutiveMonthlyReport"]
    R1 --> REG["TemplateRegistryの登録"]
    R2["R2<br/>装飾追加・順序"] --> F["対象Feature"]
    R2 --> ASM["ReportAssembler"]
    R3["R3<br/>履歴規則"] --> H["ReportActionHistory"]
    R3 --> C["GenerateReportAction"]
...
今回の確定要求R1〜R3だけを完成コードへ再適用します。

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|---|
| R1：通常月次を保ち、役員向けだけ専用本文にする | `ReportService`の本文分岐へ役員向け条件と本文を追加 | `ExecutiveMonthlyReport`へ専用本文を置き、通常月次・週次・部門別を変えずにA1/A2で違いを確認 |
| R2：装飾の種類を順序付きで受け、その順に適用する | `ReportService`へ装飾種類と順序の分岐を追加 | 各`ReportFeature`を入力順に組み立て、A2/A3でロゴ→グラフとグラフ→ロゴの順序一致を確認 |
| R3：受け付けた完全な生成要求を記録し、再実行・取消する | 生成処理へ要求保存・再実行・取消の条件を追加 | Action・History・Applicationへ分け、A4で同一要求の再実行と成果物削除を確認 |
...
本章では、先にP1〜P3を満たす構造を導きました。既知のデザインパターン名へ対応づけると、次のようになります。
...
共通順、本文差分、順番付き装飾、要求履歴を一つのクラスへ足すと、別々の変更が同じ場所へ波及します。本章では、要求をR1〜R3へ固定し、痛みからP1〜P3を導き、骨格固定構造・装飾連結構造・操作記録構造として分離しました。そして、ReportAssembler、ReportGenerationService、ReportApplicationで生成・注入・実行まで再結合しました。
