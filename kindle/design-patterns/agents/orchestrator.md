...
4. `chapter-agent` で章本文を生成または修正する。
5. `logic-check-agent` で論理の飛躍を確認する。
6. `clarity-agent` で曖昧語を具体化する。
7. `readability-agent` で読者が迷う箇所を直す。
8. `architecture-review-agent` で設計判断を確認する。
9. `review-agent` で総合レビューを行う。
10. `consistency-agent` でテンプレート、第0章、ルール、Agent の整合を確認する。
