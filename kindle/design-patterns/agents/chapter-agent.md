...
3. **`rules/design-decision-guide.md` を読む**（接続点の特定・知識の移動・コスト比較・対策実施。ここを理解してから章を生成する）
...
**【最優先ルール】1ステップの出力量：2000〜3000文字（必須）**

テンプレートの項目を箇条書きで埋めるだけでは絶対に足りない。
各ステップを丁寧に展開し、読者が「理解した」と感じる水準まで書く。
...
- 章の冒頭（この章の核心の直後・S0の前）に `## この章を読むと得られること` を必ず書く
  ...
  - **【重要】構造分析の眼で書く。以下の3視点を基本とする：**
    ...
    2. 接続点でどの知識が呼び出し元へ漏れているかをどう読み取るか
...
- **フェーズ2の責任チェック表は仮説として扱う：** フェーズ1では事実だけを確認する。フェーズ2で「この知識は誰が管理するか」を仮置きし、ヒアリングで裏付ける。フェーズ3で変更を試み、フェーズ4で原因を確定する
...
- **フェーズ3は「変更を試みる」（痛みを前提にしない）：** 「痛みを確認しましょう」ではなく「変更を試みてみましょう」と書く。変更した結果として何が起きるかを中立的に記述する
...
| 案1 | 関数へ切り出す | 知識へ名前を付ける |
| 案2 | 別クラスへ移す | 変わる理由を分ける |
| 案3 | 共通の契約を定める | 複数実装を交換できるようにする |
| 案4 | 窓口・仲介・組み合わせ・生成分離 | 章の問題に必要な役割だけを追加する |
...
- ケーブルを固定的な4種類へ分類しない
- 「何を交換したいか」「端子や変換手順を誰が知るか」「交換時にどこを直すか」を示す
- 比喩を使わなくても説明できる場合は、コード上の値・型・操作を優先する
- S4で、呼び出し元へ漏れているクラス名・生成方法・処理順序・条件を示す
- S6で、各対策がどの知識をどこへ移すかを示す
...
図が必要な章では、S4またはS5に「接続点で何が流れ、どの知識がどちら側へ漏れているか」を示す図を出力する。

## 出力形式

```
[ImagePrompt: A clean flat software dependency diagram for [章の題材].
Place [変更から守りたい側] on the left and [変わる知識を持つ側] on the right.
Between them, clearly label the connection point with the value, type, operation, or event that crosses the boundary.
Show leaked implementation knowledge in red near the caller: class names, creation logic, branching conditions, or call sequence.
Show the intended stable responsibility in blue.
Minimalist flat illustration style, white background, Japanese labels, no matrix and no quadrant classification.]
```

## 章ごとの差し替え

- Strategy：計算の骨格と割引ルールの境界
- Facade：業務処理と外部API手順の境界
- State：コンテキストと状態ごとの振る舞いの境界
- Template Method：固定した骨格と差分処理の境界
- Command：呼び出し元と実行可能な操作の境界
- Decorator：包まれる部品と追加機能の境界
- Observer：通知元と登録された通知先の境界
- Factory Method：利用処理と生成判断の境界

## 禁止事項
- ImagePromptに "Note" という文字を含めないこと（既存ルールと同じ）
- 固定的な分類マトリクスを生成しない
- 1章につきImagePromptは原則1回のみ
...
- 各案について「移す知識・接続点・減る変更・残る変更・導入コスト」を示すこと
...
- 「S〇を出力してください」と指示されたら、そのステップだけを即座に出力する
