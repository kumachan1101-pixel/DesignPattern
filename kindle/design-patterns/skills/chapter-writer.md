...
- `rules/design-decision-guide.md`（S5〜S8の詳細モデル）
...
2. **関係者ヒアリングの具体性**：S2での会話はリアルに、なぜその決断に至ったのかの背景を厚く描く。
...
| :--- | :--- | :--- | :--- |
| 🔵 現状把握 | S0 | フェーズ1：現状把握（ステップ0〜1） | X.0 クラス構成・仕様表・責任一覧（事実のみ） |
| 🔵 現状把握 | S1 | 〃 | X.1 実装コードと責任チェック、X.2 届いた変更要求 |
| 🟣 仮説立案 | S2 | フェーズ2：仮説立案（ステップ2） | X.3 仮説テーブル → ヒアリング → 確定テーブル |
| 🟣 問題特定 | S3 | フェーズ3：問題特定（ステップ3） | 変更シミュレーション・痛みの言語化・依存グラフ |
| 🟠 原因 | S4 | フェーズ4：原因（ステップ4） | 観察→原因表・変わるもの/変わらないもの・ケーブル比喩 |
| 🟡 課題 | S5 | フェーズ5：課題（ステップ5） | 接続点の特定・非機能制約・クライアント影響・課題まとめ表 |
| 🔴 対策案 | S6 | フェーズ6：対策案検討（ステップ6〜7） | 2×2マトリクス・案0〜案N・コスト天秤・耐久テスト・使う/使わない場面 |
| 🔴 対策案 | S7 | 〃 | コスト天秤・採用決定・耐久テスト・使う/使わない場面（過剰コード） |
| 🟢 対策実施 | S8 | フェーズ7：対策実施（ステップ8） | 最終コード全体・変更影響グラフ・変更シナリオ表・最終責任テーブル |
...
- `observations` : 観察リスト（S3 変更シミュレーションの素材）
- `advanced_scenario` : より難しい変化（S7 耐久テストの素材）
- `threshold` : 使う・使わない状況（S7 使い分けの素材）
...
仮説はここでは立てない。観察した事実をS2に持ち込む。

---

#### X.1（S1）：実装コードと責任チェック

**入力：** S0で把握したクラス責任 ＋ 実際の実装コード
...
**入力：** S0のクラス構成・責任一覧 ＋ S1の責任チェック結果・変更要求
**産物：** 確定した変動/不変テーブル（「誰の判断で変わるか」明記）

2段構えで書く：
- **第1段（仮説）**：S0・S1の観察から「変わりそう / 変わらなそう」の仮説テーブルを立てる（根拠列付き）
...
S4で「分ける」と判断した場所に接続点を特定し、以下の4視点で課題を定義する（詳細は `rules/design-decision-guide.md` 参照）：
...
この節の核心：S5の課題定義に対する正当な応答として案を提示する（詳細は `rules/design-decision-guide.md` 参照）。

- **冒頭に2×2マトリクス接続図（mermaid）を必ず出力する**
- 案0〜案Nを並列の選択肢として展開する。各案に優劣はない。
  - 案0：構造を変えない（`if`文追加・定数変更などで対応）
  - 案N：接続点を全て変更（S5で特定した接続の形を全て変える）
...
- コスト天秤表（案0 vs 中間案 vs 案N）で現在コスト・未来コストを比較する
- 採用案を決定し、理由を「現在/未来コストの観点から」1〜2文で説明する
- **耐久テスト**：S2ヒアリングで挙がった将来の変化を実際にシミュレートする（伏線回収）
- **使う場面・使わない場面**：
...
- **9ステップとこの章でやったことの対応表**（S0〜S8 ｜ この章でやったこと）
...
- [ ] S7の耐久テストがS2ヒアリングで出た将来のリスクと対応しているか（伏線回収）
- [ ] S8の変更シナリオ表で「変わるクラス・変わらないクラス」が明示されているか
