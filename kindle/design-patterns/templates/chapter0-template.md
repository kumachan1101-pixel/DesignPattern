...
| **フェーズ6：対策検討** | 接続点表、システム全体の課題 | 入力・実体・処理・結果の全体経路を先に決め、その各地点をコードへ落とし、完成構造が複数残る場合だけ比較する |
...
`課題ID・接続点／接続するデータ／変わる側／守る側／完了条件`の5列で一度だけ整理し、現在の波及と目指す状態をシステム全体の課題として確定する。
...
【入力から結果までの全体経路を最初に決める。起動時と実行時を分け、誰が実体を生成・所有し、何を登録・選択・受け渡しし、どこで実行するかを実際の操作名で示す。その後、**契約と具体による分離**で境界の両側を確かめ、**実体の組み立て**で同じ実体が公開入口から実行まで届くコードを示す。
安定骨格と配置を独立した設計段階にせず、骨格は分離の検算、具体・生成役・所有者・実行役の置き場は全体経路を実現するコードの中で決める。
骨格の置き場は「分岐を外したあとに残る処理」「重複から取り出した共通順序」「一層ぶんの重ね方」の三つを表で一度だけ説明し、各章では一般表を繰り返さず、該当する形と理由だけを書く。
フェーズ6では問題から導いた構造名だけを使い、対応するデザインパターン名は問題を解いた後まで出さない。
