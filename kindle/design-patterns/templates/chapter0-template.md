# 第0章テンプレート：この本の読み方
# 対象：chapter00_1.md／chapter00_2.md（第1〜12章は chapter-template.md を使うこと）

このテンプレートは第0章専用です。第1章以降の7フェーズ章は
`chapter-template.md` を正本とします。第0章は個別の問題を解く章ではなく、
...
【読者が共感できる「構造を学んでも使えなかった状況」を2〜3段落で。
架空の著者体験は書かず、読者が観察できる状況と困りごとを具体的に示す。
...
第0章（3つの原則 + 7フェーズ）→ 各章（変更要求から構造を導く）という構造を可視化する】
...
*第0章が「基礎言語」。各章ではその言語で問題を観察し、必要な構造を導く。*

---

## 掲載コードの読み方と実ファイルの分け方

【各章のコードブロックは読解単位としてトップレベルの型を1つずつ示し、上から連結すると動作確認用の1つの`.cpp`になることを説明する。短い兄弟クラスも同じブロックへ詰めず、隣接する別ブロックとして比較する】

【掲載ブロックと実ファイルは別の判断であることを明記する。実務では`.h`へ公開契約・クラス宣言、`.cpp`へ処理本体、`main.cpp`へ生成・登録・注入を置く基本を、具体的なファイルツリーと対応表で示す。小さな値の型は同じ機能境界なら`OrderTypes.h`のような共有ヘッダーへまとめられることも補足する】

【コード内の空行は、入力・取得→判定→選択・計算→状態変更・保存→通知・返却という処理段階の境界に入れ、同じ段階の連続処理を分断しないという読み方を説明する】
...
【「変わる理由が異なる部分」と「今回守る部分」を同じ変更へ巻き込まないという原則を説明する】
...
3択（型を合意・固定、独自型でくるむ、`void*`で逃がす）のコードと比較表を示す】
...
| **フェーズ5：課題定義** | 原因分析、接続点 | 原因ごとに流れるデータ・変わる側・守る側・完了条件を確定し、部分案を完成候補にしない |
| **フェーズ6：対策検討** | 接続点表、システム全体の課題 | `契約 → 具体を選択して生成 → 受け渡し → 実行`へ組み立て、完成構造が複数残る場合だけ比較する |
...
その後、関係者確認により今回確実に変わること、今回維持すること、将来リスクを分ける、という説明を散文で】
...
| 観点 | 内容 | 根拠 |
|---|---|---|
| 変わりそうな仕様 | 【仮説】 | 【フェーズ1の事実】 |
| 今回確実に変わること | 【確定変更】 | 【確認結果】 |
| 今回維持すること | 【守る仕様・処理と見直し契機】 | 【確認結果】 |
...
各境界がどの原因を解き、どの原因を残すかを確かめ、一部の原因を残す案は途中状態として落とす。`課題ID・接続点／接続するデータ／変わる側／守る側／完了条件`で一度だけ整理し、現在の波及と目指す状態をシステム全体の課題として確定する。
...
【フェーズ5の全課題から、`契約→代表具体→生成・所有・登録または選択→受け渡し→実行骨格→公開入口`を含む一つの構想を先に示す。主要クラス名・メソッド名・生成場所は構想の成立を確認するためにここで示してよい。
契約直後に代表具体で引数・戻り値・操作の成立を確かめ、その後に生成・所有・登録または選択・受け渡し・実行骨格・公開入口の要点コードを続ける。生成・登録・選択・注入を同じ語でまとめず、実際の操作名で書く。変更前コードは境界を判断する関連部分だけを、所属クラスとメソッドの開始・終了が分かる形で抜き出し、全具体と全コードを再掲しない。
責任、継承・実現、保持・注入を決める箇所では、今の判断対象だけの部分クラス図を対応コードとセットで示す。図の前に対象外を省略したことを明記する。
最後は課題IDとの対応と将来リスクの二表だけで採用を確定する。部分図を統合した完成クラス図、完成コード、実行シーケンス、実行結果はフェーズ7だけに置く。
デザインパターン名は対策の理由に使わず、問題を解いた構造を後から共有する語彙として扱う】
...
- この章は構造を「覚えさせる」のではなく「使える問いと判断基準を渡す」章である
- クラス図の読み方は、実際のクラス図を先に置き、線の見た目・意味・コード上の事実・使う場面を図中番号で対応づける。読者向け本文にMermaidソース記法の一覧を置かない
- C++ではインターフェースも抽象クラスの一種であることを説明し、`<<interface>>`（契約だけ）、`<<abstract>>`（純粋仮想＋共通実装または状態）、ステレオタイプなしの通常基底を、基底側のコードと線種で比較する
