...
| **フェーズ6：対策検討** | 接続点表、システム全体の課題 | 契約と具体で分離し、生成から実行まで組み立てる。組み立ては準備と実行に分け、完成構造が複数残る場合だけ比較する |
...
【フェーズ5の全課題から、契約・具体・生成・所有・登録・選択・受け渡し・実行を含む一つの構想を先に示す。主要クラス名・メソッド名・生成場所は構想の成立を確認するためにここで示してよい。
その直後に、契約→代表具体→生成・選択・受け渡し→実行骨格→公開入口の要点コードを続ける。変更前コードは境界を判断する数行だけを抜き出し、全具体と全コードを再掲しない。
生成・登録・選択・注入を同じ語でまとめず、実際の操作名で書く。
最後は課題IDとの対応と将来リスクの二表だけで採用を確定する。完成クラス図、完成コード、実行シーケンス、実行結果はフェーズ7だけに置く。
