...
| **フェーズ6：対策検討** | 接続点表、システム全体の課題 | 契約と具体による分離、生成・注入・実行による組み立てを決め、完成構造が複数残る場合だけ比較する |
...
【「契約と具体による分離」と「生成・注入・実行による組み立て」の二つを決める。契約と具体はセットで往復して境界を確かめ、生成した実体は注入から公開入口・骨格・契約・具体の実行まで一つの経路で追う。
安定骨格と配置を独立した設計段階にせず、骨格は分離の検算、具体・生成役・所有者・実行役の置き場は二つの判断の中で決める。
