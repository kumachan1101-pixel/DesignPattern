...
第0章（3つの原則 + 8ステップ）→ 各章（そのパターンへの適用）という構造を可視化する】

```mermaid
graph TD
    A["第0章<br/>3つの原則 + 8ステップの思考プロセス"]
    B["第1章<br/>Strategy"]
    C["第2章<br/>Facade"]
    D["第3章〜<br/>その他のパターン"]
...
3つの原則の接続：原則1（変わる理由を読み取る出発点）との繋がりを示す】
...
### ステップ4：原因分析 ―― 困難の根本にある設計の問題を言語化する析
...
【第0章の4分類（分ける・まとめる・間に挟む・モノ化する）で手札候補を絞り込む表を冒頭に置く。
rules/design-decision-guide.md に従い、手段①・手段②を選ぶ。
手段①実装→残る課題→手段②実装の流れで示す。
手段①・手段②はどちらも手札表から選ばれた正当な候補として扱う（「直感的な案」「却下した案」と書かない）。
実装コードが終わった後で初めてパターン名を出すという制約を明示する。】
...
4分類（分ける・まとめる・間に挟む・モノ化する）をそれぞれ3行程度で説明する。
