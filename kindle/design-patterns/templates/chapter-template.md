...
- **比較元と起点**：1-4の現行コード全体を、フェーズ6冒頭に振り返りとして一度だけ掲載する。その直後に、フェーズ3で変更要求を試した関連部分を「痛みの差分」として置く。この一組を対策前の基準とし、設計判断の起点をフェーズ1へ巻き戻さない。現行コード全体はコンパイル可能にし、DB・Registry・履歴・検証・副作用も含める。痛みの差分は、変更要求で追加した状態・分岐・入力・出力を落とさない。
- **6-1 課題と着目コードの対応**：フェーズ5の課題IDごとに、対策前全体コードのどのクラス・メソッド・分岐・生成・登録を見るかを示す。「なぜそこが最初の変更箇所になるのか」を、フェーズ3の痛みとフェーズ4の原因から説明する。**クラス図から入らない**。
...
- 現行コード全体は一度だけ掲載し、直後に痛みの差分を置く。途中案と採用後の全体コードを繰り返さない。採用後の全体コードは7-1へ置く。
...
- 起点に1-4の現行コード全体とフェーズ3の痛みの差分を続けて置き、6-1以降がその変更要求を1つも落とさずに進んでいること（設計を簡単に見せるために仕様を落としていないか）
