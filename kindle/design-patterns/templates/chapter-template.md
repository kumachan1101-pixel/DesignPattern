...
| 現行機能 | 機能ID1から連番を付けた変更前システムの観測可能な機能と対応要求ID |
| 現行要求 | 要求ID1から連番を付けた変更前システムの要求と受入条件 |
| 変更依頼 | 変更ID1から連番を付けた今回の変更内容と、対象機能ID・影響する要求ID |
...
| フェーズ | 入力 | 産物 |
|---|---|---|
| 1：現状把握 | 題材システム、現行機能ID、現行要求ID、代表動作、登場クラス、現状コード、変更ID | 入力→加工→出力の仕様構造、現行機能・要求ベースライン、動作例、責任一覧、変更後機能・要求ベースライン |
...
| 7：対策実施 | フェーズ6の採用案、変更後機能・要求ベースライン | 改善後コード、要求ID別受入エビデンス、機能ID別回帰結果、課題ID別構造結果、変更ID別変更影響、得たもの/諦めたもの |
...
- 1-4：フェーズ1の現状コードの `main()` と実行結果を、動作例テーブルと対応づけられる。
 - 1-1：現行機能へ機能ID1から欠番なくIDを付け、各機能に対応する要求IDを説明できる。
 - 1-1：現行要求へ要求ID1から欠番なくIDを付け、受入条件を説明できる。
 - 1-5：今回の変更依頼へ変更ID1から欠番なくIDを付け、対象機能と要求の継続・変更・追加・廃止を説明できる。
 - 1-5：変更後の機能・要求ベースラインに、現行機能・要求の消失や根拠のない追加がない。
...
利用者または運用者が入力と結果で識別できる機能へ、`機能ID1`から欠番のないIDを付ける。クラスやメソッドの数を機能数にしない。一つの機能が複数要求を持つ場合と、共通要求が複数機能へ関係する場合を許容する。

| 機能ID | 現行機能 | 対応する要求ID |
|---|---|---|
| 機能ID1 | 【利用者・運用者から見た機能】 | 要求ID1、要求ID2 |

#### 現行要求ベースライン

現状コードが満たしている要求へ、`要求ID1`から欠番のないIDを付ける。正常系だけでなく、エラー時に値・状態を変えないこと、既存の保存・通知・診断など、この章のシステム境界内で最終版にも残す要求を含める。
...
確認観点:
- 変更依頼は変更ID1から欠番なくIDを付ける。変更IDは機能IDや要求IDではなく、どの機能・要求を継続・変更・追加・廃止したかの根拠である。ここで原因分析やパターン名を先取りしない。
- 機能の目的が同じなら、要求や内部クラスが変わっても機能IDを維持する。新機能は新ID、廃止はIDを残して廃止、分割・統合は新旧IDの系譜を記録する。
...
|---|---|---|---|
| 機能ID1 | 継続／変更／廃止<br/>根拠: 現行／変更ID1 | 【変更後の機能。廃止も履歴を残す】 | 要求ID1 |
| 機能ID2 | 追加<br/>根拠: 変更ID1 | 【新しい機能】 | 要求ID2 |
...
変更後機能ベースラインの全有効機能IDを同じ順序で載せる。継続機能は既存要求の回帰、変更機能は維持部分の回帰と変更部分の受入、追加機能は新規受入、廃止機能は廃止合意と残存動作がないことを確認する。

| 機能ID | 最終状態 | 確認する要求ID | 回帰・受入結果 |
|---|---|---|---|
| 機能ID1 | 継続／変更／追加／廃止 | 要求ID1、要求ID2 | 【上の要求結果を機能単位で回収】<br/>**判定:** 合格／不合格 |

機能ID・要求IDと課題IDを同じ行へ強制的に結びつけない。機能・要求の受入結果はここまでの二表で、課題の構造結果は次の表で別々に確認する。課題IDは`変更ID→変更試行→痛み→原因`からだけ導く。
...
フェーズ7の受入確認では変更後要求ベースラインの全有効要求IDを扱い、7-4の変更影響比較では変更IDだけを扱う。リスクIDの構造評価は6-5で完了しており、7-4へ持ち込まない。
