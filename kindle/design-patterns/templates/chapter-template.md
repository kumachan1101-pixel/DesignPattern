...
レゴブロックにたとえた画像と簡単な説明を入れてください。子供でもわかるように説明してほしい。ここの絵の作成は、トークンを多く使うので、プロンプロトここに埋め込んでもらえたら、別途画像生成AIで作成します。すみません。やっぱりレゴブロックの話にしてほしい。
...
2. 「それだけで**置換・拡張**という評価を満足できるか？」を問う。
    
3. 満足できなければ、さらに**規格化・間接化**（手段②：インターフェース導入）を重ねる。
    
4. 最終的に、**置換・拡張**ができるようになった未来を提示する。
