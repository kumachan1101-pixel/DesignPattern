...
痛みコードを分解し、共通の形（同じ引数・戻り値のシグネチャ＝契約）を見つけて設計を決める。ここでいう「知識」は抽象語のまま使わず、判定条件・計算式・変換式・処理順序・生成判断・具体クラス名・状態名・フラグなど、本文中の具体物として書く。

フェーズ6は**設計案をコードで比較して決める場**である（第0章「対策検討の進め方（6-1〜6-4）」に従う）。接続点の契約、データの置き場所、構造（クラス図）、影響範囲を決め、採用案を選ぶ。候補コードは思考用の断片にせず、比較対象となるクラス、その依存、生成・注入・実行入口まで含む完全な形で載せる。実行結果による実証はフェーズ7で行う。

- **比較元と起点**：読者が構造を思い出せるよう1-4の現状完全コードを同じ改行で再掲し、その直後にフェーズ3で追加した仕様と痛みを明示する。設計判断の論理的な起点はフェーズ3の変更後であり、フェーズ1へ巻き戻して対策を考えない。痛みをコードで再掲する場合も断片ではなく完全コードにする。
- **6-1 痛みコードを分解して、接続点の「形」を探す**（思考の中心）：痛みコードを実際に変換し、詰まる場所から共通シグネチャを見つける。各候補は、対象クラスのメンバー、コンストラクタ、公開操作、呼び出し側を省略しない。元コードから変えていない行は、並び・インデント・改行を変えずに残し、変更点はコード外の説明で示す。共通シグネチャが分割前から見えている場合は、関数化などの小さな整理を飛ばして直接契約の抽出へ進む（第0章「関数へ切り出すを飛ばしてよいとき」に従う）。**クラス図から入らない**。
- **6-2 見つけた形を契約にし、データの置き場所を決める**：共通シグネチャを契約（インターフェース）として定義し、どのデータをどのクラス・リポジトリが持つか（データ配置表）を決める。契約を使う完全な候補コードとして、具体クラスの生成場所、所有者、生存期間、コンストラクタ等での依存注入、実行入口からの呼び出しまで示す。生ポインタで参照を保持する場合は所有・非所有・生存期間を明記する。
- **6-3 構造の見立て**：契約とデータ配置を決めた**結果**として、現状→対策後のクラス図を描く。図は出発点ではなく結論。
- **6-4 影響範囲**：その構造で変更要求をもう一度当てると、どこを修正し、どこまで再テストするかを表で示す。
- **採用する形を決める**：案を比較し、変更頻度・影響範囲・導入コストで決断する。

第二部で複数の構造を組み合わせる場合は、一括で最終形を出さない。6-1で**独立して変わる軸がいくつあるか**を数え、片方だけ分けても他方が残ることを分解で確かめ、軸ごとに別の契約を置く。

#### フェーズ6のコード掲載ルール

- `...`、`/* 省略 */`、宣言だけの疑似コード、1行へ潰したクラス定義を使わない。
- 起点コードと各候補で、変更していない行の順序・インデント・改行を維持する。整形のし直しを設計変更へ混ぜない。
- クラスを分離する候補では、分離先クラスだけを載せて終わらない。生成する場所、所有する場所、依存を渡すコンストラクタまたは登録操作、利用側の呼び出しまで同じ候補コードに含める。
- 説明は「分離した」で止めず、誰が具体実装を選ぶか、誰が生成するか、誰が破棄まで所有するか、どの方法で注入するか、利用側が何を知るか、実装追加時にどの組み立て箇所が変わるかを順に回収する。
- 候補コードは、その節だけでクラス間の接続を追える形にする。外部I/Oをスタブにする場合も、スタブの型と注入箇所は省略しない。
- フェーズ6では候補コードをコンパイル可能な単位で示し、フェーズ7では採用案を本番想定の境界・実行ケース・実行結果まで含めて完成させる。

説明すること:

- フェーズ5のどの課題を解くために、この順序で案を並べるのか
- 起点にフェーズ3の痛みコードを再掲し、6-1の分解がその変更要求を1つも落とさずに進んでいること（設計を簡単に見せるために仕様を落としていないか）
