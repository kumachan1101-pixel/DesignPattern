...
具体クラスへ入る前に、フェーズ4で確認した「別々の理由で変わる判断」を、どの構造へ分けて一本へ再結合するかを先に決める。変わる理由→構造→（対応するパターン名）→課題IDの対応と「守る範囲」をtextブロックで示し、パターン名から設計を選ばない姿勢を明示する。まだクラスの中身は見ない。決めた結論を振り返る表はフェーズ6の末尾（6-3 設計トレース）に置き、ここでは結論表を先に出さない。単一パターンの章は「守りたい骨格 vs 変わる側」に框組みを合わせる。

```text
現在：【安定側クラス】が 【変わる理由1】＋【変わる理由2】＋… を全部持つ
                      ↓
変更後：変わる理由ごとに責任を分け、一本の実行経路へ再結合する
   【変わる理由1】 → 【分離の仕方】（構造名＝パターン名）→ 課題ID1
   【変わる理由2】 → 【分離の仕方】（構造名＝パターン名）→ 課題ID2
守る範囲：【実行順・契約・境界など、変えない範囲】
```
...
フェーズ3のコードから課題ID1〜課題IDnに直接関係する部分だけを再掲する。フェーズ1の全コードは載せない。省略する不変部分はコードブロック外で明記する。このおさらいは、上の「課題IDごとに分離→生成→注入→実行を詳しく展開する」節の前に置き、各課題節で何を差し替えるかの起点にする。
...
要求の過不足と既存要求の消失を確認する。1-5の変更後要求ベースラインにある全有効要求IDを同じ順序で載せる。廃止要求IDはコードがないことではなく、廃止の合意と残存動作がないことを根拠で示す。
