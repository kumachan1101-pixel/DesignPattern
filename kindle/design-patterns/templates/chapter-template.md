...
**必須：** まず第0章の手札選択表の該当行を示す。
`第0章の手札選択表を引くと：「【原因の言葉】」→ **【手札ラベル】**（第0章 手札）。この原因に直接対応します。`

【その上で、この真因に対処できる手札を検討する。

▼ 迷う場合（複数の手札が有効候補になるとき）：
- 有効候補の手札をすべて列挙し、それぞれが真因をどこまで解消できるかを示す
- 評価軸で天秤にかけ、「今回の状況ではどちらが優れるか」を明示して採用を決める

▼ これしかない場合（1つの手札しか真因に対処できないとき）：
- まず「一見よさそうな代替案」がなぜ根本解決にならないかを示す
- 続いて「なぜ採用する手札だけが真因に直接対処できるか」を論理で示す】

---

【迷う場合の記法例（複数の手札を天秤にかける）】
...
→ 【真因への対処度・今回の状況でなぜ優れているか】

【評価軸で比較し、採用する手札を宣言する】

---

【これしかない場合の記法例（1つの手札に論理で誘導する）】

**却下した案：【代替案の名前】**

```cpp
// 【代替案のコード。根本解決にならない理由がわかるように書く】
```

→ 【なぜ真因を解消できないかを1〜2行で。「問題の場所が変わっただけ」「根本原因は残る」など】

**採用する手札：【手札名】**

【なぜこの手札だけが真因に直接対処できるかを論理で示す。1〜3文で明快に】
...
```cpp
// 【採用した手札 or 手札Bのテストコード】
```

**比較のまとめ**

| 基準 | 【却下案または手札A】 | 【採用手札または手札B】 |
|---|---|---|
| 【軸1】 | 【評価】 | 【評価】 |
| 【軸2】 | 【評価】 | 【評価】 |
...
```mermaid
flowchart TD
    【「このパターンを使うべきか」の判断基準をフロー図で示す。変動/不変の判定→インターフェースの要否→コストとのバランスの分岐を含める】
```
