...
| 変更要求 | R1から連番を付けた今回確実に変わる仕様、入力、変更後の受入条件 |
...
| フェーズ | 入力 | 産物 |
|---|---|---|
| 1：現状把握 | 題材システム、代表動作、登場クラス、フェーズ1の現状コード、変更要求 | 入力→加工→出力の仕様構造、動作例、責任一覧、実行結果対応、依存/分岐/処理順序、R-ID付き変更差分と受入条件 |
| 2：仮説立案 | フェーズ1の事実、関係者情報 | 今回確定したR-ID、F-IDを付けた将来リスク、変えられるようにする部分、当面守る部分 |
| 3：問題特定 | 確定要求R-ID、フェーズ1の現状コード | 今回の要求で実際に生じた修正箇所、波及範囲、再テスト範囲 |
| 4：原因分析 | フェーズ3の痛み、変更理由、コード構造 | 混在している判定/処理/生成判断、漏れている依存、保ちたい処理の骨格 |
| 5：課題定義 | フェーズ4の原因、接続点候補、制約 | 課題ID付き接続点表（境界・流れるデータ・変わる側/守る側）、システム全体の課題 |
| 6：対策検討 | フェーズ5の接続点表とシステム全体の課題 | 分離方法・配置場所・組み立て方法、採用クラス図、段階コード、コード適用結果 |
| 7：対策実施 | フェーズ6の採用案、確定要求R-ID | 改善後コード、責任の移動、R-ID別の影響確認、得たもの/諦めたもの |
...
- フェーズ2の出口で、今回確定した要求R-ID、将来リスク、実装上の内部契約、今回の対象外を明確に分ける。フェーズ3・6・7へ実装対象として渡すのは確定R-IDだけとし、未確定の将来機能を完成コードへ先回りして入れない。
...
- フェーズ7の7-1は、完成クラス一覧、フェーズ6と同じMermaid定義の完成後クラス図、完成コードに対応する実行シーケンスをコードより前に置く。その後、値・境界・契約・具体クラス・組み立て・`main()`の順にクラス単位でコードを示し、最後にR-ID別の実行結果を置く。
...
- 1-5：R1から欠番なく要求IDを付け、各要求の入力と受入条件を説明できる。
...
- 1-4と7-1を比較し、仕様変更またはP1〜Pnで説明できない型・保存・検証・履歴・内部デバッグログの追加／削除／移動があれば、共通基盤へ戻すか、変更要求として明示する。
-->
...
2-4の表は全章で次の4列に統一する。F1から欠番のないリスクIDを付け、フェーズ6の設計確認まで同じIDと同じ文言で追跡する。分類・確率などの列や、行頭の🟣🟡🔴などのマーカーは付けない（当面安定の前提は2-5で扱う）。

| リスクID | 将来リスク | 時期の目安 | 根拠 |
|---|---|---|---|
| F1 | （将来変わりうる仕様・機能） | （継続的に／数ヶ月後 等の目安） | （誰との確認か・発言の出所） |

### 2-5：変わる見込みと当面安定の前提を確定する

<!-- 執筆用：この項目の目的・達成基準
目的: F-IDごとに、変えられるようにする部分と当面守る部分を整理し、フェーズ6で採用構造を評価する条件を固定する。
達成基準:
読者が、「変わる見込み＝はい」を先行実装の指示と誤解せず、その変化が起きても守る部分へ影響を広げない構造にするための判断材料だと説明できる。
確認観点:
- 2-4の全F-IDを同じ順序・同じ文言で使う。
- 変えられるようにする部分と、当面安定として守る部分をシステムの仕様語彙で書く。クラス名やパターン名で解決策を先出ししない。
- フェーズ3へ実装対象として渡すのはR-IDだけとし、F-IDはフェーズ6の構造評価へ渡す。
-->
...
|---|---|---|---|
| F1：（2-4と同じリスク文） | はい | （変化を受ける条件・値・手順・種類） | （影響を広げず維持する入力・処理・結果） |
...
フェーズ3では確定要求R-IDだけを扱う。2-4・2-5のF-IDを引用したり、未確定の機能を仮に適用したりしない。3-3の問題文は、今回実際に修正した箇所・波及範囲・再テスト範囲だけから確定する。
...
<!-- 執筆用：このフェーズの目的・達成基準
目的: フェーズ3の変更影響とフェーズ4の原因を、解くべき接続点へ絞る。
内部基準:
- 独立した変化軸へP1から欠番なくIDを振る。
...
5. 変更要求のたびに具体的な判断が漏れ、修正が波及する境界を接続点とする。
...
|---|---|---|---|
| P1：【境界】 | 【値・型・操作・イベント】 | 【変更される具体的な条件・式・状態・手順】 | 【安定させたい処理・利用側・既存機能】 |

複数の変化軸が同じ変更で一緒に変わるか、別々に変わるかは、フェーズ6で分離単位を決める材料として短く説明する。配置先はまだ書かない。

最後に`📌 システム全体の課題（確定）`として、現在なぜ影響が広がるのか、対策後にどの範囲を守りたいかを文章化する。
...
P1〜Pnは別々の採用案ではなく、一つの完成システムをコードまで追うIDである。次の左列は、接続点をどのように変えるかを考える観点である。
...
表の後には、左から右へ読むと各Pがどの契約・クラス・組み立てコードへ変換されたかを1段落で回収する。「この章での答え」のような曖昧な列名だけで済ませず、P1〜Pnのコードへの反映を明示する。

| 接続点を変える観点 | システム全体での考え方 | コードで確定すること | 追跡する課題ID |
|---|---|---|---|
| 分離方法 | 【境界へ残す共通操作・値・イベント】 | 【契約・公開操作】 | P1 |
| 配置場所 | 【一緒に変わる責任の置き場】 | 【既存／新設クラスとメソッド】 | P1 |
| 組み立て方法（生成・所有・登録・注入） | 【生成者、所有者、登録順、選択者、注入先】 | 【main／Application／Contextなどの実コード】 | P1 |
...
|---|---|---|---|
| P1 | 【責任・依存の変更】 | 【クラス・メソッド・生成・注入】 | ステップ1 |

#### 課題箇所のおさらい（フェーズ3の関連コード）

フェーズ3のコードからP1〜Pnに直接関係する部分だけを再掲する。フェーズ1の全コードは載せない。省略する不変部分はコードブロック外で明記する。
...
P1〜Pnは一つの完成システムを別の変化軸から追う。課題別の部分最適として採用判定しない。

| 追跡対象 | 課題定義で目指した状態 | 適用した構造とコード | 適用結果 |
|---|---|---|---|
| P1：【変化軸】 | 【守りたい範囲】 | 【クラス・メソッド・生成場所】 | 【実装後に起きたこと】 |
| P1〜Pnを接続したシステム全体 | 【全体の目標】 | 【生成から実行まで】 | 【全体判定】 |

7-1の完成コード後に、同じ課題IDで適用先・実装結果を最終確認する。

### 6-5：将来リスクに対する設計上の確認

F-IDは完成コードへ実装する機能一覧ではない。2-4のF-IDを採用構造へ再適用し、変更先と守れる範囲を設計時に確認する。

| リスクID・将来リスク | 採用構造での考慮 | 将来の主な変更先 | 今回の判断 |
|---|---|---|---|
| F1：（2-4と同じ文言） | （どの境界で受けるか） | （クラス・契約・生成場所） | （局所化／一部局所化／未解決。完成コードへ追加しない） |

- 2-4と同じF-ID、同じ順序、同じリスク文を使う。
- 採用構造で局所化できない場合も「対応済み」とせず、残る変更範囲を明記する。
- F-IDだけを根拠に、将来用クラス、分岐、データ、実行結果を完成コードへ追加しない。
...
- 7-4：1-5と同じR-ID・同じ要求文で、フェーズ1の影響と完成構造の結果を比較できる。
-->
...
|---|---|---|---|
| R1：【確定した変更要求】／P1 | 【変更前から変更後への責任・依存の差】。コード：【実クラス・メソッド・生成／登録場所】 | 【受入ケースで観測した結果】 | 【次回変更で触る局所】 |
...
<!-- 執筆用：この項目の目的・達成基準
目的: 今回の確定要求ごとに、フェーズ1の現状構造で必要だった修正と、完成構造で得られた結果を比較する。
達成基準:
読者が、1-5と同じR-ID・同じ要求文を使って、フェーズ3で観測した影響と完成コード・実行結果を対応づけられる。未確定の機能を扱わない。
-->

| 確定要求 | フェーズ1の現状構造での影響 | 完成構造での結果 |
|---|---|---|
| R1：【1-5と同じ要求文】 | 【フェーズ3で実際に修正・確認した範囲】 | 【完成コードと実行結果で確認した効果】 |

フェーズ7では確定要求R-IDだけを扱う。F-IDの構造評価は6-5で完了しており、7-4へ持ち込まない。
