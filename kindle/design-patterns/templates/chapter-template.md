...
- 図は分類で終わらせない。入力・加工・出力の各要素について、この章での呼び名と後で確認する場所を対応表で示す。
...
```mermaid
flowchart LR
    A["入力A<br>（利用者操作など）"] --> B["判定1<br>（受付可能か）"]
    C["入力B<br>（現在状態など）"] --> B
    B --> D["加工1<br>（計算・変換）"]
    D --> E["加工2<br>（保存・通知）"]
    E --> F["正常出力<br>（表示・状態変化）"]
...
| 図の要素 | この章での具体名 | 後で確認する場所 |
|---|---|---|
| 入力 | （例：利用者区分、状態、外部イベント） | 1-2動作例、変更要求 |
| 加工 | （例：条件判定、計算、変換、保存） | 1-2動作例、2-1仮説、4-1原因分析 |
| 出力 | （例：表示、通知、エラー、状態変化） | 1-2動作例、実行結果、7-4変更シナリオ |
