...
課題ID1〜課題IDnのうち、静的な責任、継承・実装、保持・依存の変更が発生する判断を順番に示す。各図の直前に「こうなっているから、こういう理由でこの関係に変える」を言語化する。

```mermaid
classDiagram
    class 【この判断で着目する安定側】
    class 【この判断で新設・変更する契約】
    class 【この判断で新設・変更する具象】
    【この判断で新設・変更する契約】 <|.. 【この判断で新設・変更する具象】
    【この判断で着目する安定側】 --> 【この判断で新設・変更する契約】 : 【利用・保持・注入】
```

図の後で、この部分図が課題IDのどの完了条件へ応答し、次の部分図または採用全体図へ何を渡すかを回収する。省略したクラスは変更なしと明記する。
...
|---|---|---|---|
| 課題ID1 | 【責任・依存の変更】 | 【クラス・メソッド・生成・注入】 | ステップ1 |

#### 課題箇所のおさらい（フェーズ3の関連コード）

フェーズ3のコードから課題ID1〜課題IDnに直接関係する部分だけを再掲する。フェーズ1の全コードは載せない。省略する不変部分はコードブロック外で明記する。

### 6-1：採用設計をコードへ段階的に反映する

採用クラス図と責任配置はコードを書く前に確定している。各ステップは試行錯誤ではなく、完成構造を理解できる単位へ分けた実装順である。

- 分離先のクラスだけを載せて終わらない。
- 生成・所有・登録・選択・注入と、利用側が呼ぶ操作をコードで示す。
