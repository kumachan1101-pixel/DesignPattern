...
- 仕様が分かりづらい原因は、情報不足ではなく対応の断絶である。入力・判定・加工・正常出力・異常出力の各要素が、動作例・コード・変更要求・接続点のどこで再利用されるかを追える状態にする。
...
- 読者が最初に試しそうな有力案は、比較表に載せるだけで終わらせず、ステップの中でコードにして試す。その案で解消される痛みと残る課題を確認してから、採用案へ橋を架ける。
- フェーズ6の各案のコードは、全体構造が分かる形で示す。中心クラスだけを断片で見せて構造が読めない状態にしない。その案が成り立つのに必要な登場物（契約・具体クラス・中心クラス）、それらを結ぶ組み立て（`main` または組み立て箇所）、そして実行結果を、読者がその案の骨格を追える範囲でそろえる。前のステップと同じ定義を省く場合は、どのステップの何を再利用しているかを一言添え、組み立てと出力は各案の中で見せる。掲載コードは実際にコンパイル・実行し、出力と照合する。
- フェーズ6の冒頭で、第0章の「2つの素朴な考え方」を軸として示す。1つは共通部分の共通化・抽象化（関数化・クラス移動・契約）、もう1つは変わる部分を外から注入し組み立て側は流れを回すだけにすること（窓口・仲介、コンポジション・登録、生成責任の移動）。読者が思いつく有力案（データ化・リスト化・登録・コールバックなど）は後者の考え方の実行として位置づけ、この2軸と第0章の6つの段階を対応づける。新しい枠組みを章だけで作らない。
...
<!-- 執筆用：このフェーズの目的・達成基準
目的: 問題や解決策を語る前に、読者が同じ現状を見られる土台を作る。仕様、動作例、クラス責任、フェーズ1の現状コード、変更要求を順にそろえる。
内部基準:
...
- 最初にシステムの目的・利用者・扱うデータ・業務上の前提を文章で説明し、その後に入力→加工→出力の仕様構造図を置く。図を先に出して、読者に文脈のない箱を読ませない。
- 1-1の冒頭では、仕様そのものを優先する。担当チーム、責任分担、変更背景、設計上の困りごとは、仕様と図を説明した後に置く。
- 入力→加工→出力の仕様構造図または表を置く。加工は実装詳細ではなく、業務上の変換・判定・保存・通知として書く。
- 図は「入力」「加工」「出力」の3箱で終わらせない。代表的な入力が、どの判定・計算・変換・保存・通知を通り、どの正常出力またはエラーへ分かれるのかまで細分する。
- 図では、入力・判定・加工・正常出力・異常出力を同じ四角で表さない。入力は平行四辺形、判定はひし形、加工は四角、正常出力と異常出力は終端形と色で区別する。
- 図は分類で終わらせない。図の直後には対応表ではなく、「この図から読み取ること」を3点程度で書く。読者が得られるものは、入力がどの加工に使われるか、正常系とエラー系がどこで分かれるか、後でコードを読むときにどの仕様の流れを追えばよいかである。
...
- 条件を満たさない場合は、途中で止まり、（エラー出力）として返る。
...
<!-- 執筆用：この項目の目的・達成基準
目的: フェーズ1の現状コードが仕様をどう実現しているかを、評価ではなく事実として観察させる。
達成基準:
読者が、フェーズ1の現状コードの `main()` と実行結果を動作例テーブルへ対応づけられる。コード上の依存・分岐・処理順序を事実として観察できる。
確認観点:
...
<!-- 執筆用：原因を見る2つの向き（第0章フェーズ4と対応）
フェーズ4は向きの異なる2つの問いで原因を見る。フェーズ6の2軸と対応させる。
- 問い1（分ける向き）：1つの塊に変わる理由の違うものが混ざっていないか。→ 分離＝フェーズ6の「変わる部分の外部注入」へ。
- 問い2（まとめる向き）：複数の場所に同じ判定・手順・生成の構造が重複していないか。→ 共通化・抽象化＝フェーズ6の同名の軸へ。
多くの章は問い1から入る。第4章のように同じ手順の重複（コピー）が痛みの中心の章は問い2から入る。章の痛みがどちらの向きかを本文で明示し、第0章の対応表（分ける候補／共通化する候補／まとめておく候補）と揃える。
-->
