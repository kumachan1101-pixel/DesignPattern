...
| 変更依頼 | 変更ID1から連番を付けた今回の変更内容と、影響する要求ID |
...
| 7：対策実施 | フェーズ6の採用案、変更後要求ベースライン | 改善後コード、要求ID別受入エビデンス、課題ID別構造結果、変更ID別変更影響、得たもの/諦めたもの |
...
- 1-4：フェーズ1の現状コードの `main()` と実行結果を、動作例テーブルと対応づけられる。
 ...
 - 1-5：今回の変更依頼へ変更ID1から欠番なくIDを付け、どの要求IDが継続・変更・追加・廃止になるかを説明できる。
 - 1-5：変更後の最終要求ベースラインに、現行要求の消失や根拠のない追加がない。
...
確認観点:
- 変更依頼は変更ID1から欠番なくIDを付ける。変更IDは要求IDではなく、どの要求IDを継続・変更・追加・廃止したかの根拠である。ここで原因分析やパターン名を先取りしない。
...
要求IDと課題IDを同じ行へ強制的に結びつけない。要求の受入結果はこの表で、課題の構造結果は次の表で別々に確認する。
