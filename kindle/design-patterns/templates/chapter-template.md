...
フェーズ3で中心ロジックだけを抜粋する場合は、コードの前に「中間コードの継続条件」を置く。フェーズ1から維持するDatabase / Repository / Registry / Gateway、入力検証、保存・通知・履歴などの副作用を具体名で示し、抜粋の前後で引き続き実行されることを明記する。省略を削除や別仕様として読ませない。
