...
候補は【1つ／複数】です。【→ 手段を直接適用します。／→ 手段①から試していきます。】

---

<!-- ★ 候補が1つの場合はこのブロックを使う（候補が2つ以上の場合は削除） -->
### X.【節番号】 手段を適用する：【手札ラベル】
...
```
<!-- ★ 候補が1つの場合ここまで。ステップ6は耐久テストと使う/使わない判断のみ -->

---

<!-- ★ 候補が2つ以上の場合はこのブロックを使う（候補が1つの場合は削除） -->
### X.【節番号】 手段①：【最初に思いつくシンプルな案】

最初に検討するのは、直感的に思いつく「【手段①の要約】」という案です。
...
【手段②の解説。なぜこれが①の課題を解決し、真因に直接対処できるのかを述べる】

```cpp
// 【手段②を適用したコード（インターフェース + 実装クラス）】
```
...
<!-- ★ 候補が2つ以上の場合のみ：評価軸宣言 + 比較表 + 採用の決定 -->

### X.【節番号】 評価軸の宣言（候補が2つ以上の場合のみ）
...
### X.【節番号】 手段①vs手段②の比較（候補が2つ以上の場合のみ）

宣言した評価軸で両方を測ります。

【手段①と手段②それぞれのテストコードや検証結果を示す】
