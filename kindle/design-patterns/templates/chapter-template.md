...
- 1-1の仕様が、後続コードに出る値・状態・判定条件・出力と紐づく粒度になっている。入力・加工・出力の各要素は、後でどの動作例・コード・変更要求・接続点で使うかまで示す。
...
内部基準:
システムの目的、代表入力に対する出力、各クラスの責任を説明できる。まだ、この設計がなぜ問題なのかを判断しない。
...
注意: この達成基準は執筆者・レビュー用であり、本文へそのまま書かない。本文では「まずは入力・加工・出力を整理する。ここでは設計の良し悪しを判断しない」のように、読者へ自然な導入文として書く。
-->
...
- 図は分類で終わらせない。入力・加工・出力の各要素について、コードで出る形と後で使う場所を対応表で示す。
...
```mermaid
flowchart LR
    I["入力<br>（コードに出る値・状態・イベント名）"]
    P["加工<br>（後で観察する判定・計算・変換）"]
    O["出力<br>（実行結果で照合する戻り値・表示・エラー）"]
    I --> P --> O
```

| 図の要素 | この章での具体名 | コードで出る形 | 後で使う場所 |
|---|---|---|---|
| 入力 | （例：利用者区分、状態、外部イベント） | （変数名・引数・固定データ） | 1-2動作例、1-4 `main()`、変更要求 |
| 加工 | （例：条件判定、計算、変換、保存） | （条件式・計算式・呼び出すメソッド） | 1-4現状コード、2-1仮説、4-1原因分析 |
| 出力 | （例：戻り値、表示、通知、エラー） | （出力文・戻り値・状態変化） | 1-2動作例、実行結果、7-4変更シナリオ |
<!-- この表で後で使わない要素は、仕様図から外す。後で必要な要素が表に無い場合は、仕様説明へ戻して追加する。 -->
...
内部基準:
変更要求を現状コードへ当てると、どのクラス・メソッドへ修正が広がるかを説明できる。
