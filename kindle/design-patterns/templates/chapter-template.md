...
| 5：課題定義 | フェーズ4の原因、接続点候補、制約 | 接続点定義表（境界・流れるデータ・変わる側/守る側）、受け入れ条件（構造非依存） |
| 6：対策検討 | フェーズ5の接続点定義表と受け入れ条件 | 分離・配置・組み立ての決定、候補評価（単純案含む）、採用クラス図、残る変更、受け入れるコスト |
...
| 5：課題定義 | 解くべき接続点と、境界を流れる値・型・操作、変わる側・守る側、受け入れ条件を示す | graph / 表 |
...
- フェーズ6は、第0章の「問題定義の変更影響から、構造と変更後の影響を追う」ロジックを起点にする。変更前の変更影響を左、影響を切るクラス・契約・依存関係を中央、同じ要求を再適用した変更影響を右へ置く。段階的なコード変更は後続のコード検討で説明する。
...
<!-- 執筆用：このフェーズの目的・達成基準
目的: 原因分析をそのまま実装案に飛ばさず、解くべき接続点を洗い出して定義し、受け入れ条件を定める。
内部基準:
解くべき接続点、そこを流れるデータ、変わる側・守る側、受け入れ条件を説明できる。
- 接続点：どのクラス間・どのメソッド呼び出しが問題の境界か説明できる。
- 定義：境界を越える値・型・操作・イベントと、どちら側が変わりどちら側を守るかを言える。
- 受け入れ条件：変更要求を再適用したとき、修正がどの範囲（役割の言葉）に収まれば解けたと言えるかを、構造に依存しない形で言える。
- 禁止：対策の形（新設クラス名・置き場所・繋ぎ方・「〜へ移す」）をこのフェーズで書かない。
-->

接続点ごとに次を整理する。

- フェーズ3のどの修正・再テスト範囲を解く課題か
- 着目するクラス・メソッド・分岐・生成・登録箇所
- 境界を越えて受け渡している値・型・操作・イベント
- どちら側が変わり（変わる側）、どちら側を安定させたいか（守る側）
- その仕様が属する機能種類と、分かる場合は変更に関わる担当者・関係者
- 非機能制約と影響範囲
- 受け入れ条件：同じ変更要求を再度当てたとき、修正が収まるべき範囲と、修正・再テスト対象に入ってはならない範囲（役割の言葉で書き、構造の形では書かない）

フェーズ5の末尾では、対策の形を決めない。「何をどこへ移すか」はフェーズ6の配置の決定であり、ここでは書かない。フェーズ6で痛みの変更影響をそのまま引き継げるよう、独立した変化軸ごとに次を渡す。

| フェーズ6へ渡すもの | 内容 |
|---|---|
| 課題ID | 独立した接続点（変化軸）へP1から欠番なく振る識別子 |
| 現在の変更影響 | フェーズ3・4で要求が波及したクラス・メソッド・再テスト範囲 |
| 変えたくない範囲 | 同じ要求が再来したとき、修正・再テスト対象から外したい業務フローや既存機能 |
| 受け入れ条件 | 変更要求を再適用したとき、修正がどの範囲に収まれば解けたと言えるか（構造非依存） |

一つの変化軸に一つの課題IDを割り当てる。同じ表の中でIDを重複させず、P1、P2、P3の連番にする。原因が複数あっても、それらが同じ変化軸なら一つの課題としてまとめる。
...
<!-- 執筆用：このフェーズの目的・達成基準
目的: フェーズ5の接続点定義表と受け入れ条件を受け、分離（どう切るか）・配置（どこへ置くか）・組み立て（誰が生成・所有・注入し、安定側は何を呼ぶか）を決める。
内部基準:
- 分離：どの判断・処理・生成・データを、どの境界で切り出すか。境界を越える契約は何かを説明できる。
- 配置：切り出した知識をどの既存・新設クラスへ置き、守る側に何が残るかを説明できる。
- 組み立て：誰が具体クラスを生成・所有し、どの方法（注入・登録・継承・遷移）で安定側へ渡し、安定側がどの契約だけを呼ぶかを説明できる。
- 候補評価：受け入れ条件を満たし得る候補（読者が思いつく単純案を含む）を評価し、採用理由と不採用理由を説明できる。不採用案のコードイメージは章末コラムへ。
- 判定：受け入れ条件はフェーズ5で確定済み。フェーズ6で条件を作り直さない。
-->

フェーズ5から受け取った接続点定義表・現在の変更影響・変えたくない範囲・受け入れ条件を使い、変更前の変更影響→影響を切る構造→同じ要求を再適用した変更影響→課題と候補の一続き表→段階的なコード変更→コード適用結果、の順に設計を決める。ここでいう「知識」は抽象語のまま使わず、判定条件・計算式・変換式・処理順序・生成判断・具体クラス名・状態名・フラグなど、本文中の具体物として書く。

フェーズ6は**各課題に対して、なぜその変更を考え、どの問題が残ったため次へ進むのかをコードで比較して決める場**である（第0章「対策検討の進め方」に従う）。採用後の全体コードを先に提示して、説明を後付けしてはならない。

- **前工程の痛みを図へ引き継ぐ**：フェーズ3・4の変更影響を左、影響を切る関連構造を中央、同じ要求を再適用した変更影響を右へ置く。中央は作業手順ではなく、クラス・契約・委譲・登録の依存関係として描く。
- **課題と候補を一表にする**：現在→理想の差、切る境界・守る契約、受け入れ条件、最初に試す候補、減る影響、次の候補を同じ課題IDの横一行にする。
- **コード結果まで同じIDで回収する**：6-2後に適用コード・段階的なコード変更・結果・守った契約・受け入れ条件の判定を、7-1後に完成コードの適用先・実装結果・最終判定を示す。
- **比較元と起点**：フェーズ6冒頭では、課題カードの着目コードに該当するフェーズ3の関連部分だけをおさらいする。フェーズ1の現行コード全体は再掲しない。省略するDB・Repository・Registry・履歴・検証・副作用は、コードブロックの外で維持条件を明記する。
- **6-1 課題と着目コードの対応**：フェーズ5の課題IDごとに、おさらいした関連コードのどのクラス・メソッド・分岐・生成・登録を見るかを示す。「なぜそこが最初の変更箇所になるのか」を、フェーズ3の痛みとフェーズ4の原因から説明する。**クラス図から入らない**。
- **6-2 課題ごとの段階的なコード変更**：各課題について、最小変更から始める。各段階で、関連部分の変更前コード、変更後コード、減った変更、残った問題、次へ進む理由を示す。残った問題が次の変更を要求しないなら、そこで止める案も候補に残す。
- **6-3 構造の見立て**：課題ごとの検討を統合した**結果**として、現状→対策後のクラス図を描く。対策後の図には、最終コードへ登場する全クラスを記載し、中心課題に関係する依存を強調する。図は出発点ではなく結論。
- **6-4 影響範囲**：フェーズ5の受け入れ条件を使い、変更要求をもう一度当てたときの修正・再テスト範囲を表で示す。
- **採用する形を決める**：途中案を含めて比較し、変更頻度・影響範囲・導入コストで決断する。

第二部で複数の構造を組み合わせる場合は、一括で最終形を出さない。課題カードごとに独立した変化軸を解き、「変更1で解消した課題」「まだ残った課題」を確認してから次の軸へ進む。

#### フェーズ6のコード掲載ルール

- フェーズ6冒頭のおさらいは課題カードで指定した関連コードだけに絞る。フェーズ1の現行コード全体や、採用後の全体コードをフェーズ6へ再掲しない。採用後の全体コードは7-1へ置く。
- 中間コードは課題に関係するクラス・メソッドへ絞る。直前に対象クラス・対象メソッド・省略している不変部分を明記する。
- コードブロック内に `...`、`/* 省略 */`、1行へ潰したクラス定義を使わない。宣言だけで判断できない場合は、判断に必要な実装も示す。
- 変更前と変更後の関連コードで、変更していない行の順序・インデント・改行を維持する。整形のし直しを設計変更へ混ぜない。
- クラスを分離する段階では、分離先クラスだけを載せて終わらない。生成する場所、所有する場所、依存を渡すコンストラクタまたは登録操作、利用側の呼び出しを、判断に必要になった段階で関連コードとして含める。
- 説明は「分離した」で止めず、誰が具体実装を選ぶか、誰が生成するか、誰が破棄まで所有するか、どの方法で注入するか、利用側が何を知るか、実装追加時にどの組み立て箇所が変わるかを順に回収する。
- 関連コードは、その節だけで変更対象の接続を追える形にする。外部I/Oをスタブにする場合も、その段階の判断に関係するスタブの型と注入箇所は省略しない。
- フェーズ6では中間案の構文と接続を確認し、フェーズ7では採用案を本番想定の境界・実行ケース・実行結果まで含めて完成させる。

説明すること:

- フェーズ5のどの変更影響を左へ引き継ぎ、中央のどの構造上の境界で経路を切り、右で変更影響がどう縮んだか
- 起点に課題カードで指定したフェーズ3の関連コードを置き、6-1以降がその課題の入力・出力・副作用を落とさずに進んでいること
- フェーズ6の冒頭を、変更前の変更影響→影響を切る構造→変更後の変更影響のグラフ→課題と候補の一続き表→課題関連コードのおさらい、の順にする。
- 課題IDは独立した課題へ一つだけ割り当て、各表では重複・欠番なくP1、P2、P3と並べる。図では同じIDで変更前の影響、構造上の境界、変更後の影響を結ぶ。
- 課題IDごとに「着目コード→最小変更→減ったこと→残ったこと→次へ進む理由」を本文と関連コードで示す。
...
Mermaid図を `Pain`（問題定義で得た変更前の変更影響）、`TargetStructure`（影響を切るクラス・契約・依存関係）、`Result`（同じ要求を再適用した変更影響）の3領域で描く。中央には変更1→変更2の作業フローを書かない。左の影響が中央のどの境界で止まり、右のどの変更先へ移るかを、同じ課題IDの破線で結ぶ。

#### 構造と変更後の影響から、課題と候補を一続きで導く

| 課題ID | 変更の到達点 | 最初に試すコード変更 | 残る問題に対する次のコード変更 |
|---|---|---|---|
| P1 | **現在→理想の差：**【現在から消す波及】<br>**切る境界・守る契約：**【切るコード境界と維持する契約】<br>**受け入れ条件：**【同じ要求で触らなくする場所（フェーズ5から引用）】 | **候補：**【原因へ直接効く最小単位のコード変更】<br>**減る影響：**【消える影響】 | 【残る影響を根拠に行う次のコード変更。残らなければ「なし」】 |

6-2の後には次を置く。

#### 課題IDごとのコード適用結果

| 課題ID | 候補を適用したコード | 段階的なコード変更と結果 | 守った契約・受け入れ条件の判定 |
|---|---|---|---|
| P1 | 【クラス・メソッド】 | **段階的な変更：**【変更1。必要な理由】→【変更2。前段階に残った問題】<br>**結果：**【減った波及】 | **守った契約：**【維持した入力/出力/順序/副作用】<br>**判定：**【未達/達成と根拠】 |

7-1の後には、同じIDで完成コードの適用先、実装後に起きたこと、受け入れ条件の最終確認を置く。
