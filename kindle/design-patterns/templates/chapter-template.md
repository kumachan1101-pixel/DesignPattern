...
- フェーズ6は、第0章の「痛みの変更影響から構造変更と結果を追う」ロジックを起点にする。前工程の痛みを左、構造の微修正を中央、同じ要求の再適用結果を右へ置き、課題と候補の一続き表からコード適用結果まで同じ課題IDで追う。
...
フェーズ5から受け取った現在の変更影響と変えたくない範囲を使い、痛みの変更影響→構造の微修正→同じ要求の再適用結果→課題と候補の一続き表→コード適用結果、の順に設計を決める。ここでいう「知識」は抽象語のまま使わず、判定条件・計算式・変換式・処理順序・生成判断・具体クラス名・状態名・フラグなど、本文中の具体物として書く。

フェーズ6は**各課題に対して、なぜその変更を考え、どの問題が残ったため次へ進むのかをコードで比較して決める場**である（第0章「対策検討の進め方」に従う）。採用後の全体コードを先に提示して、説明を後付けしてはならない。

- **前工程の痛みを図へ引き継ぐ**：フェーズ3・4の変更影響を左、コード構造の微修正を中央、同じ要求を再適用した結果を右へ置く。完成クラス図を先に置かない。
- **課題と候補を一表にする**：現在→理想の差、切る境界・守る契約、完了条件、最初に試す候補、減る影響、次の候補を同じ課題IDの横一行にする。
- **コード結果まで同じIDで回収する**：6-2後に適用コード・微修正・結果・守った契約・完了判定を、7-1後に完成コードの適用先・実装結果・最終判定を示す。
...
- フェーズ5のどの変更影響を痛みグラフへ引き継ぎ、どの構造の微修正で経路を切ったか
- 起点に課題カードで指定したフェーズ3の関連コードを置き、6-1以降がその課題の入力・出力・副作用を落とさずに進んでいること
- フェーズ6の冒頭を、痛み→構造変更→結果のグラフ→課題と候補の一続き表→課題関連コードのおさらい、の順にする。
- 課題IDは独立した課題へ一つだけ割り当て、各表では重複・欠番なくP1、P2、P3と並べる。図の段階変更はP1-変更1、P1-変更2の枝番で追う。
...
Mermaid図を `Pain`（フェーズ3・4の痛み）、`StructuralChange`（P1-変更1、P1-変更2）、`Result`（同じ要求の再適用結果）の3領域で描く。

#### 構造変更グラフから課題と候補を一続きで導く

| 課題ID | 変更の到達点 | 最初の微修正 | 残れば次の微修正 |
|---|---|---|---|
| P1 | **現在→理想の差：**【現在から消す波及】<br>**切る境界・守る契約：**【切るコード境界と維持する契約】<br>**完了条件：**【同じ要求で触らなくする場所】 | **候補：**【最小の微修正】<br>**減る影響：**【消える影響】 | 【残る影響に効く次の微修正】 |
...
|---|---|---|---|
| P1 | 【クラス・メソッド】 | **微修正：**【変更1→変更2】<br>**結果：**【減った波及】 | **守った契約：**【維持した入力/出力/順序/副作用】<br>**判定：**【未達/達成と根拠】 |
