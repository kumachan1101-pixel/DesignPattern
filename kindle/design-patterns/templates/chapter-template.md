...
- フェーズ6は、第0章の「理想の変更影響グラフから課題カードを作る」ロジックを起点にする。現在の不要な波及と理想で許す変更の差から、切る境界・守る契約・完了条件を導き、その条件に効く候補案を出す。
...
フェーズ5の末尾では、対策の形や課題カードを先に決めない。フェーズ6で理想の変更影響を描けるよう、独立した変化軸ごとに次を渡す。
...
フェーズ5から受け取った現在の変更影響と変えたくない範囲を使い、理想の変更影響、課題カード、候補、コード上の変更手順の順に設計を決める。ここでいう「知識」は抽象語のまま使わず、判定条件・計算式・変換式・処理順序・生成判断・具体クラス名・状態名・フラグなど、本文中の具体物として書く。

フェーズ6は**各課題に対して、なぜその変更を考え、どの問題が残ったため次へ進むのかをコードで比較して決める場**である（第0章「対策検討の進め方」に従う）。採用後の全体コードを先に提示して、説明を後付けしてはならない。

- **理想グラフを起点にする**：フェーズ3・4の現在グラフと同じ変更要求を使い、理想の変更影響グラフを先に描く。理想グラフは完成クラス図ではなく、変更してよい範囲と波及させない範囲を決める図である。
- **グラフ差分から課題カードを導く**：現在にはあり理想では消した経路から、切る境界・守る契約・完了条件を課題カードへ落とす。候補を先に決めてカードを逆算しない。
- **課題カードから候補を出す**：候補はカードの完了条件を満たす手段として、カードの後に示す。グラフ、カード、候補には同じ課題IDを使うが、一つの成果物内では一課題一行とし、P1から欠番なく連番にする。
...
- フェーズ5のどの変更影響と変えたくない範囲から、理想グラフをどう組み立てたか
- 起点に課題カードで指定したフェーズ3の関連コードを置き、6-1以降がその課題の入力・出力・副作用を落とさずに進んでいること
- フェーズ6の冒頭を、理想の変更影響グラフ→現在との差から導く課題カード→カードから出す候補→課題関連コードのおさらい、の順にする。
- 課題IDは独立した課題へ一つだけ割り当て、各表では重複・欠番なくP1、P2、P3と並べる。同じIDの再登場は成果物間の追跡に限る。
...
フェーズ6冒頭では、次の三つをこの順で示す。

#### 理想の変更影響グラフを先に組み立てる

現在と理想を同じMermaid図で対比し、理想側の独立した枝へP1から連番を付ける。

#### 理想グラフとの差から課題カードを導く

| 課題ID | 現在→理想の差 | 切る境界・守る契約 | 完了条件 |
|---|---|---|---|
| P1 | 【現在から消す修正・再テスト経路】 | 【切る分岐・生成・呼び出し】・【維持する入力/出力/順序/副作用】 | 【同じ要求で理想の範囲だけを変えれば済む状態】 |

#### 課題カードから検討候補を出す

| 課題ID | 最初に試す候補 | それで減る影響 | 残れば次に試す候補 |
|---|---|---|---|
| P1 | 【最小の変換】 | 【切れる影響経路】 | 【まだ残る経路へ効く次の変換】 |

候補表の直後には「候補は課題を作る根拠ではなく、課題カードの完了条件を満たす手段」と明記し、課題関連コードのおさらいへ進む。
