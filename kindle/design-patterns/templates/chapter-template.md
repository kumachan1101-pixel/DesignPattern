...
- 最初にシステムの目的・利用者・扱うデータ・業務上の前提・代表的な入力値・正常出力・異常出力を文章または表で一通り説明し、その後に入力→加工→出力の仕様構造図を置く。図を先に出して、読者に文脈のない箱を読ませない。
...
| 【判定】 | 【条件・対象外・エラー条件など】 | 【例: 金額が0以下ならエラー、状態がPendingなら承認可能】 | 【正常系と異常系がどこで分かれるか】 |
| 【加工】 | 【計算・変換・保存・通知など】 | 【例: 税込金額を計算、CSV行を注文データへ変換、在庫を3個から2個へ更新】 | 【入力がどの値・状態へ変わるか】 |
| 【出力】 | 【正常出力・異常出力】 | 【例: 支払金額 4,850円、承認済み、入力エラー】 | 【動作例・実行結果・変更要求で何を照合するか】 |
