...
フェーズ5の末尾では、対策の形を先に決めない。フェーズ6で痛みの変更影響をそのまま引き継げるよう、独立した変化軸ごとに次を渡す。

| フェーズ6へ渡すもの | 内容 |
|---|---|
| 課題ID | 独立した変化軸へP1から欠番なく振る識別子 |
...
フェーズ5から受け取った現在の変更影響と変えたくない範囲を使い、変更前の変更影響→影響を切る構造→同じ要求を再適用した変更影響→課題と候補の一続き表→段階的なコード変更→コード適用結果、の順に設計を決める。ここでいう「知識」は抽象語のまま使わず、判定条件・計算式・変換式・処理順序・生成判断・具体クラス名・状態名・フラグなど、本文中の具体物として書く。
...
- **課題と候補を一表にする**：現在→理想の差、切る境界・守る契約、完了条件、最初に試す候補、減る影響、次の候補を同じ課題IDの横一行にする。
- **コード結果まで同じIDで回収する**：6-2後に適用コード・段階的なコード変更・結果・守った契約・完了判定を、7-1後に完成コードの適用先・実装結果・最終判定を示す。
...
- **6-4 影響範囲**：課題カードの完了条件を使い、変更要求をもう一度当てたときの修正・再テスト範囲を表で示す。
...
|---|---|---|---|
| P1 | **現在→理想の差：**【現在から消す波及】<br>**切る境界・守る契約：**【切るコード境界と維持する契約】<br>**完了条件：**【同じ要求で触らなくする場所】 | **候補：**【原因へ直接効く最小単位のコード変更】<br>**減る影響：**【消える影響】 | 【残る影響を根拠に行う次のコード変更。残らなければ「なし」】 |
...
7-1の後には、同じIDで完成コードの適用先、実装後に起きたこと、完了条件の最終確認を置く。
