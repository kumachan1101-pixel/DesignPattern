...
フェーズ6は**設計を決める場**である（第0章「対策検討の進め方（6-1〜6-4）」に従う）。接続点の契約、データの置き場所、構造（クラス図）、影響範囲を決め、採用案を選ぶ。**動く実装一式（`main`・実行結果）はフェーズ6に置かず、フェーズ7で書く**。各章のフェーズ6は次の構成にする。

- **起点**：フェーズ3で変更要求を当てて痛んだ「変更途中コード」を再掲する。フェーズ1の現状コード（変更要求を容れる前）へ戻さない。
- **6-1 痛みコードを分解して、接続点の「形」を探す**（思考の中心）：痛みコードを実際に変換し、詰まる場所から共通シグネチャを見つける。書くのは動く実装ではなく、「こう変えるとどこで詰まるか」を確かめるスケッチ（`main`・実行結果を付けない）。共通シグネチャが分割前から見えている場合は、関数化などの小さな整理を飛ばして直接契約の抽出へ進む（第0章「関数へ切り出すを飛ばしてよいとき」に従う）。**クラス図から入らない**。
- **6-2 見つけた形を契約にし、データの置き場所を決める**：共通シグネチャを契約（インターフェース）として定義（定義と骨だけ）し、どのデータをどのクラス・リポジトリが持つか（データ配置表）を決める。生ポインタで参照を保持する場合は所有・非所有・生存期間を明記する。
