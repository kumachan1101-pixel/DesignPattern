...
「今回実装しないもの（今回の対象外）」は、フェーズ1ではなく、この2-5末尾で確定する。対象外は、ヒアリングでスコープを確かめて初めて確定できるフェーズ2の出口成果だからである。リスクIDと重複する項目（HTML形式などリスクID化済みの将来機能）は2-4/2-5のリスクID表へ寄せ、対象外リストにはリスクID以外の確定スコープ外（バックグラウンド処理・別用途の監査ログなど）だけを列挙して二重掲載を避ける。フェーズ1では、対象外はフェーズ2で確定する旨を一文で予告するにとどめ、非目標の一覧を先出ししない。
