...
- 仕様図は「システム全体図」と「システム内部図」に分ける。システム全体図は利用者・対象システム・保存データ・外部システム間のデータ／要求／結果、システム内部図は対象システムへの入力・業務処理の順序・出力／保存／通知を示す。二つを同じ内容の言い換えにしない。
...
上の文章と表で仕様を一通り確認したので、まず対象システムを一つの箱として見て、利用者・保存データ・外部システムとの間に何が流れるかを整理します。

```mermaid
flowchart LR
    U["利用者・処理の起点"] -->|"操作・実行要求"| S["対象システム"]
    D1[("入力側の保存データ")] -->|"処理対象データ"| S
    C[("設定・現在状態")] -->|"判定材料"| S
    S -->|"要求データ"| E["外部システム・外部境界"]
    E -->|"結果・イベント"| S
    S -->|"状態・履歴・結果を保存"| D2[("出力側の保存データ")]
    S -->|"表示・通知・戻り値"| O["正常出力"]
```
