...
- 完成コードにだけ見える要素を、直ちに「変更後の新要素」と判定しない。変更要求を根拠に、今回変える要素と、前後で維持する共通基盤（保存方法、結果契約、検証、履歴、既存の外部境界）へ先に分類する。共通基盤は現状仕様・現状クラス・現状コードにも置く。
...
- 差分表には、変更項目だけでなく、誤って作り替えやすい共通基盤を「変更なし」として明示する。変更前と変更後のシステムは、要求で変えると定義した箇所以外を同じにする。
...
- 1-4と7-1を比較し、仕様変更またはP1〜Pnで説明できない型・保存・検証・履歴の追加／削除／移動があれば、共通基盤へ戻すか、変更要求として明示する。
-->
