...
フェーズ5の末尾では、課題を抽象的な一文だけで終わらせず、フェーズ6の最初のコード変更へつながる「課題カード」にする。

| 課題ID | 痛みの根拠と着目コード | 移すもの・残す約束 | 制約・完了条件 |
|---|---|---|---|
| P1 | 【フェーズ3の修正/再テスト範囲】・`【クラス.メソッド】` | 【移す判定/処理】・【残す値/操作】 | 【守る副作用等】・【不要にする修正範囲】 |

課題が複数ある章では、課題カードを分ける。フェーズ6では課題IDごとにコードを変え、どの変更がどの課題へ効いたかを追えるようにする。
...
フェーズ5の課題カードを、コード上の変更手順へ変換して設計を決める。ここでいう「知識」は抽象語のまま使わず、判定条件・計算式・変換式・処理順序・生成判断・具体クラス名・状態名・フラグなど、本文中の具体物として書く。

フェーズ6は**各課題に対して、なぜその変更を考え、どの問題が残ったため次へ進むのかをコードで比較して決める場**である（第0章「対策検討の進め方」に従う）。採用後の全体コードを先に提示して、説明を後付けしてはならない。

- **候補と課題カードの接続**：フェーズ4の原因→フェーズ5の課題→フェーズ6で試す候補を、課題ID付きの表で示す。続く課題カードも同じIDを使い、候補を試す着目コード・最小変更・守る契約・完了条件を固定する。候補表と課題カードを別々の課題に見せない。
...
- フェーズ6の冒頭に、フェーズ4の原因、フェーズ5の課題、そこから出る対策候補を課題ID付きで対応させる表を置く。直後の課題カードと同じIDでつなぎ、候補は「何を試すか」、カードは「どこを変え何を守るか」だと説明する。
...
|---|---|---|---|
| P1 | 【痛みを生んだ構造上の原因】 | 【何を切り離し、接続点に何を残すか】 | 【最初に見る小さな案、次に見る責任移動・契約・登録など】 |

候補表の直後には「候補は変更案、課題カードはその案を検証する条件であり、同じ課題IDで次の関連コードへ進む」と明記する。
