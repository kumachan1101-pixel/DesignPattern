...
フェーズ6は、第0章の段階的進化アプローチを標準フローとして使う。ただし、ステップは一本道の作業手順ではなく、対策案を比較するための候補である。（案を省略・並べ替える場合の理由の書き方は `rules/connection-point-review.md` に従う。本文へ編集方針の宣言そのものを書かない。）

第二部で複数の構造を組み合わせる場合は、一括で最終形を出さない。各ステップで「今回解消した独立原因」「まだ残る独立原因」「次の構造が必要になる理由」を示し、1つ目の構造の限界から2つ目以降を順に導く。

フェーズ6の起点は、フェーズ3で変更要求を当てて痛んだ「変更途中コード」とする。フェーズ1の現状コード（変更要求を容れる前）へ戻さない。痛んだコードには、変更要求を受けて増えた判定・処理・生成・結果が具体物として現れている。そこから、同じ形で扱える共通点（共通の判定・処理・生成・結果の契約）を抽出し、変わる差分を接続点の外へ出す形へ整理する。読者が「痛み → 共通点の発見 → 抽象化」の順で追えるようにする。共通シグネチャが分割前から見えている場合は、関数化などの小さな整理を飛ばして直接インターフェース抽出（対策構造）へ進む（第0章「関数へ切り出すを飛ばしてよいとき」に従う）。まだ見えていない場合のみ、変更途中コードを小さく整理して共通点を発見する段階を置く。

フェーズ6の各ステップが中心責任だけの差分抜粋なら、冒頭に「中間コードの継続条件」を置き、全ステップで維持する外側の境界・検証・保存・通知・履歴を具体名で固定する。メソッドの引数や戻り値が変わるときは、公開操作から委譲先、要求/結果オブジェクト、操作オブジェクト、組み立て箇所のどこへ値・操作が移ったかを説明する。

ステップ1の直前には「ステップ1の比較元：仕様変更後の痛みコードをおさらいする」を置き、フェーズ3の変更途中コードのうちステップ1が実際に書き換える範囲をコードで再掲する。起点を説明文や表だけで済ませない。ステップ1は、その再掲コードに含まれる変更要求を1つも落とさずに整理する。ステップ2以降は直前ステップを比較元とし、何が移り、何が残ったかを説明する。

ステップ2以降で1つの変化軸だけをコード抜粋する場合は、前ステップから維持する仕様・コードを明記する。短い抜粋から要素が消えただけなのか、設計案として削除したのかを読者へ推測させない。

説明すること:

- フェーズ5のどの課題を解くために、この順序で案を並べるのか
- ステップ1の直前に、フェーズ3の仕様変更後コードを比較元として再掲し、ステップ1が同じ要求を保っていること
