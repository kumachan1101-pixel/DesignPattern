...
# 対応フレームワーク：7フェーズ9ステップ（S0〜S8）・接続点2×2マトリクス

---

## ボリューム・文体ルール（最重要・全ステップに適用）

### 1ステップの出力量：2000〜3000文字（必須）
...
S0は「仕様表・クラス図・責任一覧を埋める」のではなく、**システムの物語として読めるように書く**。
...
「ここまでで〇〇が見えてきました。次のS1では〇〇を1行ずつ確認します。」

### 内部見出しで分割する

1ステップに複数の内部見出しを使う。目安は1ステップあたり4〜8個。
...
```
要するに、[S0・S1で得た観察]から[どんな構造の問題]が見えてくる。
```
...
パターン名（Strategy・State等）は S6 より前に出さない。
...
**使用できるステップ：S4・S8のコードのみ。**

S1のコードには入れない。S1の時点では責任外かどうかの分析がまだ完了していないため、このコメントを付ける根拠がない。S4で「この行が責務外だった」と判明した後に初めて意味を持つ。
