...
def check_system_structure_phase6(
    ...
    required = [
        ...
        ("| クラス図をどう変えるか | コードレベルで何をするか | 後続の実装ステップ |",
    ...
    if min(positions) >= 0 and positions != sorted(positions):
        issues.append(Issue(
            path, ln,
            "フェーズ6は課題カード→採用判断→クラス図の責任見直し→思考連鎖→関連コード→"
