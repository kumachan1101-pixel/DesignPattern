...
## 最新：設計の唯一解表現と、パターン名を最後に明かす演出を撤廃

本書の完成構造は唯一の正解ではなく、各章で示した仕様・変更要求・現状コード・制約の
もとで、著者が妥当と考えた一案である。序章の「答えを確定する」という表現を改め、
別の前提では別案や分けない判断も成立する位置づけを明記した。

パターン名は書名・章題で最初から示されているため、「最後に共有名を知る」という工程と、
各章末で名称を初めて明かす移行文を撤廃した。パターン解説は、章で組み立てた具体構造と
章題のパターンの抽象骨格・適用条件・限界を照合する位置づけへ変更した。

本文7ファイルの見出しを横断走査し、です・ます調の見出しを名詞句へ変更した。
テンプレート、ルール、チェックリスト、Agent、Skill、検査も同期した。単体テスト86件、
再発防止の負のテスト79件、Mermaid全84図、掲載コード・実行結果・未使用入力・
分割ソースを含む単冊ゲートに合格している。PDF・EPUBは生成していない。

## 最新：序章・第0章を、文章中心から構造図中心へ再編

著者指摘S79〜S80を受け、序章と第0章で文章を頭の中だけで組み立てなければ
関係が分からなかった箇所を、構造の対比と工程間の受け渡しが見える図へ変更した。

- 序章：三つの問い全体の図を問い1の中から外し、変更判断が集まった構造から、
  分離・共通接続・組み立てへ変わる構造そのものを示した。各問いが図のどこを見るかも明記した
- 第0章：IDの追跡線、模擬システムの入出力、図の色と形、7フェーズの受け渡し、
  各フェーズで見る構造を15個のMermaid図で可視化した
- フェーズ説明は、固定的な「目的・入力・成果物」の繰り返しをやめ、
  「このフェーズで考えること」と確認ポイントを中心にした
- 実践章1〜3は、途中の自動更新で古い構成へ戻ったため、直前の合格版と
  Gitオブジェクト単位で一致する状態へ復旧した
- テンプレート、執筆ルール、チェックリスト、Agent・Skill指示、再発防止検査を同期した
- 本文の★は0件。Mermaidは全84図を実描画し、文字の重なりがないことを目視確認した
- 単体テスト86件、再発防止の負のテスト79件、単冊ゲート、掲載コード6本、
  実行結果、代表実行、未使用入力0、分割ソース3章の生成・ビルド・出力一致に合格した
- PDF・EPUBは生成していない。現在は著者確認待ち
...
著者確認後の追加指摘S69〜S70により、第1〜3章の`この章の核心`を
`この章で解く設計課題`へ変え、定型ラベルではなく題材固有の設計課題を自然な文章で
説明する形へ統一した。`この章での簡略化`は`実システムと掲載コードの違い`へ変更した。
テンプレート、ルール、チェックリスト、自動検査も同期し、単体テスト67件と単冊完了
ゲートに合格している。

単体テスト65件と単冊完了ゲートに合格した。ゲートでは正本7ファイル、掲載コード6本、
掲載実行結果、代表実行、未使用入力、Mermaid 63図、クラス図の線82本、6インチ表示幅、
前方参照、フェーズ接続、コード説明位置、分割ソース3章を確認した。実装コミットは
`2e51373`（`★24件を第1冊へ横断反映`）。
