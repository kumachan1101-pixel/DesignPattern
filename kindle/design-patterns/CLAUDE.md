...
デザインパターンを「覚えるべき答え」として扱うのではなく、「なぜその構造にたどり着いたのか」という思考のプロセスを一緒に体験することで、読者が現場で臨機応変に、自分の頭で設計を考えられるようになること。
...
GoFデザインパターンの「名前を覚える」ではなく、「なぜその構造に辿り着くのか」という思考プロセスを身につけることを目的とした技術書（Kindle本）。
...
- **変更前コード（ステップ1）**：起点コードに `main()` を含め、動作をエンドツーエンドで見せる。`main()` の直後に実行結果を載せる。
    
    - **目的**：「このコードは正しく動く。問題は構造にある」を明示する
        
- **変更後コード（ステップ7）**：`BatchApplication` + `main()` の最終形を示す。
...
- **ヒアリングで挙がったリスクは〇.10（耐久テスト）で必ず実演する**：
    ...
    ヒアリングで「将来〇〇に変わるかもしれない」と言及した変更は、〇.10の深化シナリオとして登場させる。「ステップ2で予告した変化がここで現れた」という伏線回収の構造にする。（例：ヒアリングで「PDF→Excelの可能性」→〇.10で実際にExcel切り替えを実演）
...
1. ステップ3（課題分析）で「変更影響グラフ」を使い、変更が飛び火する箇所を可視化する
    
2. ステップ5（対策案）で「変更前のコードに手を加えると何が壊れるか」を明示してから解決策を示す
    
3. ステップ7（決断と未来）で「変更シナリオ表」を示し、各シナリオで変わるクラス・変わらないクラスを明確にする
...
1. 原因 → 第0章の4分類に当てはめ → 手札候補を絞り込む
    
2. 分岐：
    
    - **ケース①（候補1つ）**：直接適用 → クラス図＋パターン名初登場 → (Step6) 耐久テスト → 使う/使わない判断（天秤なし）
        
    - **ケース②（複数・組み合わせ）**：統合実装 → クラス図＋パターン名初登場 → (Step6) 耐久テスト → 使う/使わない判断（天秤なし）
        
    - **ケース③（複数・別々に試す）**：手段①（手札A） → 残る課題 → 手段②（手札B） → クラス図＋パターン名初登場 → (Step6) 評価軸宣言 → 手段①vs②比較 → 採用決定 → 耐久テスト → 使う/使わない判断
...
現状把握（ステップ1）では「各クラスの責任」を定義し、「責任外の知識が混在していないか」を確認する。
