...
|S0|🔵 現状把握①|クラス構成と責任を読む（事実のみ）|仕様表 → **動作例テーブル**（複数入力パターンの入出力を示す。コードを読む前のアンカー） → クラス図 → 責任一覧を確認。**仮説は立てない。** 実装コードはまだ読まない|
|S1|🟣 現状把握②|実装コードを読む|S0で確認したクラス責任と実装コードを照合し、依存グラフ・責任チェック表を作成する|
|S2|🟠 仮説立案|仮説を立て、関係者に確認する|観察→仮説テーブル → **今回の確定変更テーブル**（変更要求で確実に変わること）→ 関係者ヒアリング問答シーン → **将来リスクテーブル**（ヒアリングで判明した今後の変化リスク。確定変更と混在させない）|
|S3|🟡 問題特定|課題分析|変更影響グラフ（Mermaid LR）・痛みのシナリオ|
|S4|🔴 原因分析|原因分析|なぜなぜ分析（表）。末尾に「ケーブルで考える」比喩 + ImagePrompt|
|S5|🟡 課題定義|課題を定義する|接続点で行き来するデータ（型・値・シグネチャ）を特定し、課題まとめ表（接続点・接続するデータ）を作る|
|S6|🟢 対策案検討|対策案の検討|2×2マトリクス接続図 → 案1（現状のまま）〜案4（抽象×間接）の4案を連番で並列提示。各案は手段A/B/C等の複数実装アプローチを比較し採用理由を明記する。コードはクラスごとに分割して示す|
|S7|🟢 対策案検討|対策選定（コスト天秤）|評価軸宣言 → 案の比較表 → 採用決定 → **パターン名はここで初登場**（「実はこの構造には名前がある」という形で） → 耐久テスト → 過剰コード警告 → 使う/使わない判断|
