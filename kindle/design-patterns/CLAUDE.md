...
この本の各章は、`templates/chapter-template.md`で定義した同じ7フェーズで進む。
順序と標準見出しを崩してはいけない。第1章は実装例であり、章構成の正本ではない。

テンプレートの最新版は `templates/chapter-template.md` を参照。

|**フェーズ**|**標準セクション**|**内容（要点）**|
|---|---|---|
|🔵 1：現状把握|1-1仕様、1-2動作例、登場クラス、1-3クラス図、1-4現状コード、1-5変更要求|事実だけを確認し、問題や対策を先取りしない。「変化の軸」「責任外」「混在している」などの判断はフェーズ4以降にのみ書く|
|🟣 2：仮説立案|2-1混在する知識と担当、2-3確定変更、2-4ヒアリング、2-5将来リスク|仮説と確定事項を分け、関係者の判断で裏付ける|
|🟣 3：問題特定|3-1変更を試みる、3-2変更影響グラフ、3-3痛みの言語化|変更要求を現状へ当て、修正・再テスト範囲を確認する|
|🟠 4：原因分析|4-1観察と原因、4-2変動と不変、4-3接続点へ漏れている知識|変わる理由の混在と、境界を越えて漏れている知識を特定する|
|🟡 5：課題定義|接続点で何が流れているかを見る|値・型・操作・決定者・安定する約束を定義する|
|🔴 6：対策検討|段階的な改善と決断|必要な案だけを比較し、移す知識・残る変更・導入コストを示す|
|🟢 7：対策実施|7-1最終コード、7-2シーケンス図、7-3改善後グラフ、7-4変更シナリオ|実行結果と変更要求で効果・限界を確認する|
|—|整理・振り返り・パターン解説|問題から解決まで、責任の移動、3原則、構造の共有名を確認する|

### 全パターン共通の問い（フェーズ1〜4で使う）
...
|**フェーズ**|**図の種類**|**目的**|
|---|---|---|
|フェーズ1|classDiagram（変更前）|現状の構造を可視化|
|フェーズ3|graph LR 変更影響グラフ（改善前）|変更が波及する様子|
|フェーズ4〜5|接続点の知識表|境界を越えて漏れている知識を可視化|
|フェーズ6|classDiagram（対策案）|各案で責任がどこへ移るかを比較|
|フェーズ7|sequenceDiagram・graph LR（改善後）|実行の流れと変更影響を確認|

変更前後のクラス図は対比して示し、矢印の数の変化で構造改善を説明する。

**クラス図の前にクラス説明を置く（必須）**

1-3クラス構成図の節では、Mermaid図の**前**に必ずクラス一覧表（クラス名・役割・担当する仕様）を置く。
図を見る時点で各クラスが何をするか把握できていないと、矢印の意味が掴めない。
...
フレームワーク・外部ライブラリ・DB固有API・ネットワーク通信ライブラリは使わない。

C++標準ライブラリ（std::map, std::vector, std::fstream など）は積極的に使う：
- データの保持・検索には std::map / std::vector を使い、固定値ハードコードを避ける
- ドメイン上自然な箇所（CSVインポート・家計簿・レポート生成など）では std::fstream でファイル入出力を行う
- 状態の変化は呼び出しをまたいで記憶し、呼び出しのたびにリセットされない形にする
...
`main()` はビジネスロジックの詳細を知ってはいけない。バリデーション（存在確認・残高チェック・状態チェック等）は呼び出し先のクラス内部で行い、`main()` は「誰に何をさせるか」だけを指示する。

```cpp
// ❌ main() がビジネスルールを知っている
if (!db.exists(id)) { std::cout << "エラー"; return 1; }
if (!db.hasCapacity(id)) { std::cout << "エラー"; return 1; }

// ✅ main() は操作を指示するだけ
TicketReservation seat(db, "EVT001");
seat.reserve();  // 内部でexists/hasCapacityを判定
```

**動作テーブルのルール：**

動作テーブルには実際のデータ（口座名・残高・イベント名など）を含め、エラーの根拠が読み手に分かるようにする。抽象的な仮番号（12345678など）は使わない。送金・残高系のエラーは送金元の残高を明記する。

**実行結果のルール：**

実行結果は正常フローだけでなく、エラーケースも含む複数シナリオを示す。各シナリオに `--- 行N: 説明 ---` のラベルを付け、実行結果を読むだけでシステムの動作が把握できるようにする。
...
フェーズ4で「分けるべき」と判断した境界に生まれる接続点を特定し、そこで行き来するデータ（型・値・メソッドシグネチャ）を具体化する。課題まとめ表の列は「接続点 ｜ 接続するデータ（型・値）」の2列のみ。非機能制約・分けた理由・現在の呼び出し元はここに書かない。

**S6（対策案の検討）：**

1. 接続点で受け渡すものと、呼び出し元へ漏れている知識を示す
2. 冒頭に「どの案も動作例テーブル（S0）の動作を実現する。違うのは変更が来たときにどこを触るか」というフレーミング文を置く
3. 案1（現状のまま）〜案4 を連番で並列に提示する（優劣をつけない）
4. 各案の中で手段A/B/Cなど複数の実装アプローチを比較し、採用した手段とその理由を明記する
5. コードはクラスごとに分割して示す（全クラスをまとめて一度に出さない）
6. パターン名はここに出さない（S7で初登場）
...
2. 案1〜案4の比較表を示す
3. 採用を決定する
4. **パターン名をここで初登場させる**：「実は案Xの構造は〇〇パターンと呼ばれています」という形で。S4（原因分析）やS6（対策案）でパターン名を出してはならない
5. 耐久テスト（S2ヒアリングで予告されたリスクを実演）
6. 過剰コード警告（使わない方が良い状況）
7. 使う/使わない判断を示す
...
作業がひと区切りついたら、以下の順序で必ず終わらせること：

1. フィーチャーブランチにコミット・プッシュ（作業中はここまで）
2. `main` にマージ
3. `main` をプッシュ
...
「フィーチャーブランチにプッシュして終わり」は禁止。**main へのプッシュが完了して初めて作業完了。**
