...
ステップ0〜ステップ8の9ステップという思考の型を、実際のコードを題材に繰り返し体験することで、読者が「目の前のコードで何が変わるか・変わらないか・どう分けるか」を自分の頭で考えられるようになること。デザインパターンはそのプロセスの結果として自然に登場する。
...
**9ステップ（ステップ0〜ステップ8）こそが読者への最大の贈り物。パターン名はラベルにすぎない。**
...
|ステップ0|🔵 現状把握①|仕様を把握する（事実のみ）|仕様表・クラス図・責任一覧を確認。**仮説は立てない。** 実装コードはまだ読まない|
|ステップ1|🔵 現状把握②|システムを把握する|ステップ0で確認したクラス責任と実装コードを照合し、依存グラフ・責任チェック表を作成する|
|ステップ2|🟠 仮説立案|仮説を立て、関係者に確認する|観察→仮説テーブル（第1段階）→ 関係者ヒアリング問答シーン → 確定テーブル（第2段階）。ここで初めて仮説を立てる|
|ステップ3|🟡 問題特定|課題分析|変更影響グラフ（Mermaid LR）・痛みのシナリオ|
|ステップ4|🔴 原因分析|原因分析|なぜなぜ分析（表）。末尾に「ケーブルで考える」比喩 + ImagePrompt|
|ステップ5|🔴 原因分析|課題を定義する|接続点の特定（具体/抽象・直接/間接の4視点）・非機能制約・課題まとめ表|
|ステップ6|🟢 対策案検討|対策案の検討|2×2マトリクス接続図 → 案0（現状維持）〜案4（抽象×間接）の5案を連番で並列提示。パターン名はここで初登場|
|ステップ7|🟢 対策案検討|対策選定（コスト天秤）|評価軸宣言 → 案の比較表 → 採用決定 → 耐久テスト → 過剰コード警告 → 使う/使わない判断|
|ステップ8|🔵 対策実施|決断と未来|最終コード全体・変更シナリオ表・変更影響グラフ（改善後）|
|—|—|整理|9ステップとこの章でやったことの対応表・最終責任テーブル|
|—|—|振り返り|3つの原則がコードのどこに現れているかを確認する|
|—|—|パターン解説|抽象GoF構造のクラス図・章固有マッピング図・使いどころと限界|

### 全パターン共通の問い（ステップ0・ステップ1で使う）
...
|ステップ3（X.3）|graph LR 変更影響グラフ（改善前）|変更が飛び火する様子|
|ステップ6（X.6）|2×2マトリクス接続図 + classDiagram（変更後）|接続点の選択肢と解決後の構造を可視化|
|ステップ7（X.7）|flowchart TD 適用判断フロー|パターン適用の判断基準|
|ステップ8（X.8）|graph LR 変更影響グラフ（改善後）|変更が局所化された様子|
...
- **変更後コード（ステップ8）**：`BatchApplication` + `main()` の最終形を示す。
...
2. ステップ6（対策案検討）で「変更前のコードに手を加えると何が壊れるか」を明示してから解決策を示す
    
3. ステップ8（決断と未来）で「変更シナリオ表」を示し、各シナリオで変わるクラス・変わらないクラスを明確にする
...
2. 案の比較表を示す
3. 採用を決定する
4. 耐久テスト（ステップ2のヒアリングで予告されたリスクを実演）
...
- **OK**：「ステップ0でクラスの責任は把握しました。次のステップ1では、その責任通りにコードが書かれているかを1行ずつ確認します。実装と責任が一致しない箇所こそが、のちの問題の発生源になります。」
...
- **NG**：（何もなく次の見出しへ）
- **OK**：「ステップ1で責任チェックは完了しました。この観察をもとに、次のステップ2では変動・不変の仮説を立て、関係者に確認します。」
...
- 目安：各フェーズ（ステップ0〜ステップ8）の切れ目、または複雑な説明が続いた後
...
9ステップの思考プロセス（ステップ0〜ステップ8）・ケーブル比喩・案0〜案4の全体像。
第1章〜第8章を読む前にここを読む。

### 第一部：基本パターンの思考体験（第1章〜第8章）

各章は1パターンを扱い、9ステップ（ステップ0〜ステップ8）の思考プロセスを体験する。
...
- 同じ9ステップ（ステップ0〜ステップ8）で問題を分析するが、解決策として複数のパターンが組み合わさる
...
- ステップ0〜ステップ8の構造は同じ。ただし複数の「変わる理由の混在」が登場する
    
- ステップ4（原因分析）で複数の根本原因が見つかる
    
- ステップ6（対策案検討）で「1つのパターンでは解決しきれない部分」が現れ、2つ目のパターンが登場する
