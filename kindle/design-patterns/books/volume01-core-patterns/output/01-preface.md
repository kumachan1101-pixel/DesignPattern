...
本書では、変更要求と課題から導いた責任・依存・接続点（クラスとクラスが値や操作を受け渡す境目のこと。次の節で詳しく説明します）の形を**デザイン構造（設計構造）**と呼びます（本書ではこの2語を同じ意味で使います）。その設計構造が、繰り返し現れる問題と解決の組として既知の名前を持つとき、その名前が**デザインパターン**です。デザインパターンは、先人たちが泥臭い現場で問題に向き合い、
いくつかの選択肢を天秤にかけ、
