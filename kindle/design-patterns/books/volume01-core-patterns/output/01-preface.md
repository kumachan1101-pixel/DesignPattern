...
**速達の送料を800円→900円に変更したい**

- **混ざった形で触る場所**：`OrderService` を開いて送料計算の箇所を修正
- **分けた形で触る場所**：`ExpressShippingFee` だけを修正
...
**送料ルールを変更しながら同時にメール文面も変えたい**

- **混ざった形で触る場所**：`OrderService` を1つ開いて両方修正——影響範囲が重なって混乱する
- **分けた形で触る場所**：`ExpressShippingFee` と `MarketingEmail` をそれぞれ独立して修正——影響が交わらない
...
**複数の機能を自由に組み合わせたい**（例：リトライ付き＋ログ付きのメール通知）

- **推奨される手段**：**コンポジション（優先）**
- **なぜか**：実行時に部品を付け外すことができ、クラスの爆発を防げるため。

**処理の「骨格（順序）」を固定したい**（例：準備→実行→片付けという流れ）

- **推奨される手段**：**継承**
- **なぜか**：親クラスで流れを強制し、子クラスは「中身」を書くだけに集中させるため。

**完全な「is-a」関係であり、分類だけしたい**（例：管理者ユーザーはユーザーの一種）
