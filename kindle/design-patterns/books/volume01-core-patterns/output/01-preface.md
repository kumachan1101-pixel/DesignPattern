...
次の図では、三つの異なる変更理由が一つの部品へ集まっています。変更の依頼元、変わる時点、参照する情報が違うなら、同じ日に一緒に変わるとは限りません。

```mermaid
flowchart TB
    R1["業務ルールの変更"] --> M["一つの部品<br>業務判断<br>進行の判断<br>外部接続"]
    R2["処理時点・順序の変更"] --> M
    R3["外部サービスの<br>仕様・失敗条件の変更"] --> M
    M --> Q2["問い2<br>両側に必要な約束を残す"]
    Q2 --> Q3["問い3<br>具体を作り、持ち、渡す"]
```

直前の図で問い1が調べるのは、三本の矢印が本当に同じ理由かどうかです。違う理由なら、部品の中に境界候補があります。問い2は境界に残す共通部分を決め、問い3は分けた実体を動くシステムへ戻します。
