...
## この本は、デザインパターンを学ぶ本ではありません

最初にお伝えしておきたいことがあります。

**この本は、デザインパターンを学ぶための本ではありません。**

デザインパターンの一覧を覚えたい、23個の構造を順番に押さえたい——そういう目的であれば、他に良い本がたくさんあります。GoFの原典もありますし、パターンごとに図と用途をまとめた本も出ています。

この本が扱いたいのは、その手前にあるものです。

**どんな構造であっても、目の前のコードから自分なりの最適解へ近づいていくための、考え方**です。

### だから、パターンは3つしか出てきません

本書に出てくるデザインパターンは、Strategy、State、Observer の3つだけです。「たった3つ？」と思われるかもしれません。ですが、これは省略ではなく、**意図した選択**です。

パターンを増やしても、増えるのは知っている構造の数だけです。**増えてほしいのは、知らない構造に出会ったときに自分で考えられる範囲のほう**だと私は考えています。

そこで3つを**代表**として選び、そのかわり一つひとつを、要求を受け取るところから完成コードが動くところまで、省略せずに追いました。3つは、それぞれ「分けて、再び繋ぐ」形が違います。

| 章 | 構造 | 再び繋いだあと、どう動くか |
|---|---|---|
| 第1章 | Strategy | 選んだ**一つ**へ渡す |
| 第2章 | State | **今の状態**によって渡す先が変わる |
| 第3章 | Observer | 登録された**複数**へ伝える |

同じ「分けて再結合する」でも、結合したあとの動き方が三通り違います。この差を見比べる経験は、4つ目、5つ目のパターンを覚えることよりも、**目的から構造を選ぶ**練習になると考えました。

### この本で持ち帰ってほしいもの

覚えて帰ってほしいのは、`Strategy` というクラス構成ではありません。

- このコードは、**何が変わったときに**痛むのか
- 痛む理由は、**変わる理由の違うものが同じ場所に混ざっている**からではないか
- 混ざっているなら、**どこで分ければ**いいのか
- 分けたあと、**どうやって繋ぎ直す**のか

この4つの問いは、Strategy にも State にも Observer にも使えますし、本書に出てこない構造にも、パターン名のない自作の構造にも、そのまま使えます。**問いのほうが、構造より寿命が長い**のです。

ですから、本書のコードをそのまま真似する必要はありません。**考え方のほうを参考にしていただけたら**と思っています。あなたの現場のコードは、本書の例とは違う形をしているはずです。それでも「何が変わるのか」「どこが混ざっているのか」という問いは、同じように立てられます。
...
私自身、同じ壁に何度もぶつかりました。構造の形は覚えたのに、現場では「いつ使えばいいのか」が分からない。自分の判断で構造を選べたことは、一度もなかったように思います。

なぜ、実績のある優れた設計手法が、時にはコードをより複雑にしてしまうのか。理由はシンプルです。**構造を「最初から目指す必要がある答え」として扱っているから**です。

本書では、変更要求と課題から導いた責任・依存・接続点（クラスとクラスが値や操作を受け渡す境目のこと。次の節で詳しく説明します）の形を**デザイン構造（設計構造）**と呼びます（本書ではこの2語を同じ意味で使います）。その設計構造が、繰り返し現れる問題と解決の組として既知の名前を持つとき、その名前が**デザインパターン**です。

デザインパターンは、先人たちが泥臭い現場で問題に向き合い、いくつかの選択肢を天秤にかけ、「この状況ではこれが一番割に合う」と判断した**決断の結果**として生まれたものです。結果だけを真似ても、状況が違えばうまく機能しません。本書が扱うのは、その決断に至るまでの過程のほうです。

実は、私自身も最初はろくにプログラミングができない状態でした。設計なんて、もってのほかです。それでも少しずつ「コードを分ける」ことを意識してみると、変更しやすくなり、構造がきれいになっていく恩恵を感じて、設計の楽しさにすっかりはまってしまいました。この本を手に取ってくださった方にも、そんな設計の楽しみを知ってもらえたらと願っています。
...
それだけです。「変わる理由が混在している」と見抜く目と、「変わる理由の違うコードを違うクラスへ分ける」という判断ができれば、デザイン構造のような構造は自然についてくると私は考えています。

## 設計の基本 ―― 「分ける」と「接続点の形」

この本を通じて、ソフトウェア設計の考え方を「**責任を分けて作る**」という視点で説明します。

何かを作るとき、役割ごとに部品を分けて組み立てると、一部を交換したり改良したりしやすくなります。ソフトウェアも同じで、「どの種類の機能・仕様変更で変わるか」ごとに責任を分けるところが設計の出発点だと、私は考えています。

そして、分けた後には必ず、部品同士が情報や処理を受け渡す**接続点**ができます。本書では、この接続点で「何を渡すか」「どちらが何を知るか」「変更時にどこまで影響するか」を確かめていきます。
...
図を見るときは、次の三点を確かめてみてください。

1. 接続点で何を受け渡すか
2. どちらが端子の詳細を知るか
3. 機器を替えたときにどこを直すか

大切なのは分類名を覚えることではなく、「相手を替えたいのに、専用の端子や変換手順に縛られて替えられない」という変更時の困りごとを見つけることだと考えています。
...
> | 記法 | この本で表すもの | よく出る操作 |
> |---|---|---|
> | `std::vector<T>` | 順番に並んだ複数件 | `push_back` 末尾へ追加／`at(i)` 位置指定の取得／`erase` 削除 |
> | `std::map<K, V>` | キーから値を引く保存領域 | `find` 存在確認／`at` 値の取得 |
> | `std::deque<T>` | 両端から出し入れできる列 | `push_back` 末尾へ追加／`front()` 先頭を見る／`pop_front()` 先頭を除く |
>
> `front()` と `back()` は**見るだけ**で、取り出して消す操作ではありません。消すには `pop_front()` / `pop_back()` を別に呼びます。`std::vector` の先頭を消すときは `erase(begin())` です。
...
「構造を覚えても使えない」理由を考え、判断の土台になる3つの原則を確認します。

**第0章　設計判断の地図**

各章で使う「7つのフェーズ」と、変更の影響を接続点から見直す方法を説明します。

**第1章　ルール差し替え構造（Strategy）**

痛みの種類は、**条件分岐の爆発**。「どのルールを適用するか」が変わるたびに、コード全体が揺らぐ状態を扱います。

**第2章　状態分離構造（State）**

痛みの種類は、**状態による振る舞いの変化**。「今の状態」に応じたif文が散らばり、状態を1つ足すたびに全箇所を直すことになる状態を扱います。

**第3章　通知分離構造（Observer）**

痛みの種類は、**通知先への依存**。「誰に通知するか」が増えるたびに、主処理のコードまで影響が及ぶ状態を扱います。

**おわりに**

三つの構造を振り返り、実務でどう使い分けるかを考えます。

三章に共通するのは、変わる部分を契約の向こう側へ分け、外から再結合するところです。違うのは、冒頭で見た**再結合したあとの動き方**（一つへ渡す／状態で渡す先が変わる／複数へ伝える）です。

### 第1章・第2章・第3章は、どれから読んでも構いません

**三つの実践章は、互いに独立しています。** 第1章を読まないと第2章が分からない、ということはありません。それぞれが別のシステムを題材に、7つのフェーズを最初から最後まで一周します。

ですから、**いま自分のコードで困っていることに近い章から読んでください。**

| こんな状態なら | この章から |
|---|---|
| 条件分岐が増え続けて、1つ足すたびに既存を触る | 第1章（ルール差し替え構造） |
| 「今の状態」を見る `if` があちこちに散っている | 第2章（状態分離構造） |
| 何かが起きたとき、知らせる相手が増え続ける | 第3章（通知分離構造） |

**第0章だけは、先に読んでおくことをおすすめします。** 各章で使う7つのフェーズと、問題・原因・課題・対策をつなぐ言葉づかいを決めている章だからです。10分ほどで読めます。もっとも、第0章を飛ばしても各章は読めるように書いてあります。用語が気になったときに戻ってきてください。

読み終えた後は、残りの二章も読んでみてください。**三つを見比べたときにはじめて、「分けて再結合する」の中に三通りの形があると分かります。**
...
設計と聞くと「A案とB案を並べて比較する」場面を思い浮かべるかもしれません。ですが各章では、ほとんどの場合、案は1つに絞られます。「何が変わるか」「どこが痛むか」「何を分けるか」を順に確定していくと、置き場所と繋ぎ方はだいたい決まってしまうからです。

比較を出すのは、責任の置き方や依存の向きが本当に違う案が2つ以上残ったときだけにしています。迷ったまま両方を並べても、選ぶ根拠がないと決められません。読むときは「なぜこの形になったのか」を、比較ではなく**そこまでの手順**から追ってみてください。
...
各章のコードは、構造の話に集中できるよう、業務の複雑さを意図的に削っています。実際の現場のコードは、例外処理も歴史的な事情も混ざっていて、ここまで整っていないのが普通です。

それでも、持ち帰れるものはあります。**「このメソッドには、どの種類の機能変更が集まっているか？」**という問いは、どれだけ混沌としたコードにも同じように使えます。コードの形をまねるのではなく、この問いを自分のコードへ向けてみてください。
...
> **この本で「原則」と呼ぶもの**
>
> 常に守らなければならないルールという意味ではありません。**特別な理由がない限り、そこから考え始める出発点**という意味で使っています。
>
> ですから、原則から外れること自体は問題ではないと考えています。大切なのは、外れるときに**なぜ外すのかを言葉にできる**ことです。「インターフェースを使わない。実装は1種類しかなく、テストでの差し替えも要らないから」——理由のある逸脱は、たまたまそうなった設計とは違います。

GoFの23のデザインパターンは、一見バラバラに見えます。
でも私は、多くの構造をこれから挙げる3つの原則で読み解けると考えていて、
自分が設計を判断するときの土台にしています。

**ただし、3つですべてを説明できるとは考えていません。**
たとえば Template Method や Factory Method は、継承そのものを骨格に使う構造です。
原則3は「継承より『持つ』を先に考える」ですが、これは継承を禁じるものではなく、
**手が伸びる順番の話**だと捉えています。手順を固定して一部だけ差し替えたい、
生成の判断を派生側へ委ねたい——そうした目的がはっきりしているなら、
継承を選ぶのが素直な場面もあります。原則から外れるときに理由を言葉にできること、
それが原則を持つ意味だと考えています。

原則を先に知っておくと、構造が「暗記する公式の集まり」から
「同じ判断を別の形で表したもの」に見え始める——というのが、私の実感です。

---

### 原則1：変わるものを分けて囲う

> **いつ効くか**：同じクラスの中に、別々の理由で変わるものが2つ以上あるとき。

この原則の核心は、**「どの種類の機能・仕様変更で変わるか」を基準に、コードを分離する**ことです。

なぜ機能・仕様の種類ごとに分けるのか——私はこう考えています。送料ルールの変更、通知文面の変更、保存先の変更は、それぞれ変更される理由もタイミングも異なります。これらが同じクラスにいると、送料だけを変えたいのに通知や保存処理まで影響確認の対象になりがちです。そこで変更の種類ごとに責任を分けておくと、1つの要求が別種類の機能へ飛び火しにくくなる、という見方です。分離それ自体がゴールなのではなく、変化の種類をそろえた結果として構造が決まってくる、と捉えています。

#### なぜこの原則が生まれたのか

コードが変わる理由は、ビジネスや技術前提の変化によって生まれます。
送料ルールの変更、API仕様の変更、出力フォーマットの変更は、それぞれ別の種類の変化です。
...
送料ルールを変えただけなのに、出力フォーマットや保存処理まで確認が必要になる——現場で何度もこの「確認作業」に追われた先人たちがいました。

そこで気づいたのが、「変わる機能・仕様の種類ごとに分離していたら、この不安は生まれにくかった」ということです。それがこの原則の出発点だと私は捉えています。
...
関係者が分かる場合は補助線になりますが、分離の主軸はあくまで機能の種類です。これらの処理が、注文の流れをまとめた1つのクラス（`OrderService`） クラスに混在していると、メール文面を変える際に、データベース保存処理に影響が出ないかテストする必要が生じます。だからこそ、送料計算・通知・保存のように、変更の種類ごとに分離するのです。

次の2つの図で、問題のある構造と解決後の構造を順に比べてみてください。

```mermaid
flowchart TB
    subgraph O["OrderService（問題あり）"]
        P["processOrder()<br>注文処理の流れ"]
        D["calcShippingFee()<br>送料ルール"]
        M["sendEmail()<br>通知文面"]
        S["saveToDb()<br>保存方式"]
    end
    D -.-> R1["価格・販促ルールで変わる"]
...
classDiagram
    ...
    OrderService_after --> IShippingFeeCalc : 契約だけを知る
...
// 注文処理フロー、送料計算、通知送信、DB保存が同じクラスに混在している
...
int main() {
    ...
    service.processOrder(10000, "Express");
    // 上記コードの実行結果：
...
```cpp
// 具体実装①：速達の送料（送料ルール）
class ExpressShippingFee : public IShippingFeeCalc {
public:
    double calc(double itemTotal) override {
        return 800; // 速達は一律800円
    }
};
```
...
| 変更したいこと | 混ざった形 | 分けた形 |
|---|---|---|
| 速達送料を800円から900円へ変える | `OrderService` の送料計算 | `ExpressShippingFee` |
| メール文面を変える | `OrderService` のメール生成 | `MarketingEmail` |
| 保存先を変える | `OrderService` のDB保存 | `DbRepository` または新しい保存実装 |
| 送料とメール文面を同時に変える | `OrderService` で2種類の変更が重なる | `ExpressShippingFee` と `MarketingEmail` を別々に変更 |
...
> その理解で合っています。`IShippingFeeCalc`、`INotifier`、`IOrderRepository`の**契約そのもの**が別々に変われば、それらを呼ぶ`OrderService`にも三種類の変更理由が生まれます。インターフェースを持てば、あらゆる変更から守られるわけではありません。
>
> 具象クラスを直接持つ場合との違いは、契約が変わらない範囲にあります。たとえば`ExpressShippingFee`を別の送料実装へ替えても、`IShippingFeeCalc::calc(double)`が同じなら、`OrderService`は変更せず、組み立て側で差し替えられます。一方、具象クラスを直接持つと、実装の種類や生成方法の変更まで`OrderService`へ届きます。
>
...
基本形「このコードは、どの種類の機能・仕様変更で変わるのか？」を、実際のコードに当てはめるときはこう展開します。
...
この原則を当てると、どんな形になるのか。それは題材によって変わります。本書の三つの章では、それぞれ違う「変わるもの」を分けることになります。

- 第1章では、**適用する割引ルール**を分けます
- 第2章では、**現在の状態ごとの振る舞い**を分けます
- 第3章では、**通知先の種類と通知方法**を分けます

どれも、システムの説明を読んで「何が変わるのか」を確かめてから分けます。**先に構造の形を決めて当てはめるのではありません。** 各章の最後に、その章で何を分けた結果どの構造になったのかを振り返ります。

GoFの構造は、最初から目指すものではないと私は考えています。守りたい骨格から変わる部分を分けた結果、たまたまその形になっていた——という順番です。
...
私はまず原則1から考えますが、**あえて分けない**という判断をする場面もあります。分けることで増える複雑さが、分けて得られるものを上回りそうなときです。
...
> **いつ効くか**：呼び出し先の実装が今後入れ替わる見込みがあるとき、あるいはテストで差し替えたいとき。
...
具体的なクラス名（`EmailSender`）を呼び出し元が知っていると、Email→SMS→Pushに切り替わるたびに呼び出し元も変わります。なぜなら、呼び出し元のコードに「`EmailSender` を使う」という前提が書き込まれているため、`EmailSender` を、SMS送信を担う別のクラス（`SmsSender`）へ替えるときに、呼び出し元のコードも修正が必要になるからです。
...
ここで確認したいのは、**`Notifier` のコードが1行も変わっていない**ことです。`main()` が渡す相手を `EmailSender` から `SmsSender` へ替えても、`Notifier` は `sender_->send(msg)` を呼ぶだけで、相手が何かを知りません。

差し替えられる理由は、`Notifier` が知っているのが `IMessageSender` という**契約だけ**だからです。具体クラスの名前は `main()` にしか出てきません。**「どの実装を使うか」という判断を呼び出し元へ集め、`Notifier` からは追い出した**——これが原則2の言う「具体ではなく契約に向かって書く」の形です。

なお、この差し替えが効くのは接続点の**形が同じまま**のときです。次に、その形をどう決めるかを見ます。
...
### 原則3：組み合わせたいなら、継承より「持つ」を先に考える

> **いつ効くか**：機能を自由に組み合わせたいとき。手順を固定して一部だけ差し替えたい場合は、継承のほうが素直なこともあります。
...
| 実現したいこと | まず検討する構造 | 理由 |
|---|---|---|
| 複数の具体を同じ入口から差し替える | 具体は契約を実装し、利用側は契約を持つ | 実装の種類を利用側から隠して差し替えられる |
| 複数の機能を自由に組み合わせる | コンポジション | 部品を組み替えられ、組み合わせごとの派生クラスを増やさずに済む |
| 処理の骨格と順序を固定する | 継承 | 親が流れを固定し、子は各処理の中身に集中できる |
| 完全な `is-a` 関係を分類として表す | 継承 | 概念上の親子関係をそのまま表せる |
...
機能が2つ以上になったときが、原則3の本題です。

問題①では、リトライ機能1つを継承で足しました。ここへ「ログ出力機能」と「制限機能」も加わったとします。継承で組み合わせようとすると、何が起きるでしょうか。
...
機能が4つになれば組み合わせは指数的に増えます。直前のクラス図はEmailとSMSの2種類を出発点としていますが、追加機能が4種類（リトライ・ログ・制限・暗号化）まで増えると、その組み合わせは 2 × 2^4 = 32クラスが必要になります（図はその一部のみ示しています）。機能を1つ追加するたびに既存の全クラス分だけクラスを追加することになり、現実的に管理しきれなくなっていきます。
...
> **3つを読み終えて**
>
> どの原則にも「あえて外すとき」がありました。冒頭で書いたとおり、原則は出発点であって、
> 守らせるためのものではないと私は考えています。
>
...
ここまで3つを挙げましたが、**これで設計判断のすべてが決まるわけではありません。**

本書の各章で、3つのどれにも当てはまらない判断が繰り返し出てきます。**誰がその実体を持ち、いつ壊すのか**——所有と寿命の話です。

- 第1章：`DiscountRuleSet` が5つのルールを値で持ち、まとめて生きて、まとめて消える
- 第2章：状態オブジェクトは1つずつ共有し、骨格は借りているだけ。**借りているものが先に消えないこと**を確かめる
- 第3章：通知先は `main()` が持ち、通知元は借用ポインタで登録を受ける

C++では、これを決めないとコードが書けません。契約を分けても、**その実体を誰が持つかは別の判断**です。3つの原則は「どこで分けるか」までしか教えてくれません。

なぜ4つ目の原則として立てないのか。私は、所有と寿命が**言語によって答えが変わる**からだと考えています。ガベージコレクタのある言語では意識せずに済む場面が多く、C++やRustでは中心的な判断になります。3つの原則は言語をまたいで通じますが、所有はそうではありません。

ですから本書では、**所有と寿命は各章のフェーズ6で、そのつど決めるもの**として扱います。原則の一覧には入れませんが、判断からは外せません。
...
```mermaid
graph TD
    P1["原則1<br>変わるものを分けて囲う"]
    P2["原則2<br>具体ではなく<br>契約に向かって書く"]
    P3["原則3<br>継承より「持つ」を<br>先に考える"]
...
本書で扱う三つの構造は、この順で判断していった結果として
それぞれの形に落ち着いた、と私は捉えています。
