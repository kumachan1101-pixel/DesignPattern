...
### 1. 章タイトルのパターン名から答えを逆算しない

章タイトルのパターン名は、扱うテーマを探すための索引です。本文では、現状コードへ変更を当て、痛みと原因を確認してから構造を考えます。「この章はStrategyだからインターフェースを作る」とは考えません。

まず、「なぜこのコードが変えにくいのか」「どの事実から、どの境界が必要だと言えるのか」を追ってください。章末のパターン解説は、導いた構造を一般的な名前へつなぐために置いています。

### 2. 比較表がないことを、検討不足だと思わない

責任の置き方や依存の向きが異なる完成案が複数残ったときは、目的に照らして比較します。最終的な構造が一つしか残らないなら、手段を増やすためだけの比較はしません。

大切なのは案の数ではなく、「なぜその案が残り、他の可能性が消えたか」を前工程の事実から説明できることです。

### 3. 章末で、自分のコードへ問いを移す

各章の最後には、同じ考え方を自分のコードへ当てる問いがあります。本書のクラス名を置き換えるのではなく、自分の変更履歴や修正範囲を思い浮かべてください。

実際のコードは、本書の例より複雑です。それでも、「変更でどこを開いたか」「なぜそこまで確認したか」という事実から始めれば、境界を考えることができます。

---

## 判断の土台になる三つの問い

三つの問いは、結論ではありません。**設計案を考える前に、見落としを減らすための問い**です。すべてを毎回使う必要もありません。
