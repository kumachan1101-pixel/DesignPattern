...
flowchart TB
    ...
    NOW --> PAIN --> CAUSE --> DESIGN --> CHECK --> NAME
    ...
    class CAUSE analyze;
    class DESIGN design;
    class CHECK,NAME verify;
```

図の上から下へ進むほど、現状の事実が設計判断へ変わります。パターン名は出発点ではなく、確かめ終えた構造を共有するために最後に使います。
...
三つの問いは、この思考を進めた後の確認に使います。問い1〜3の番号は、本文のどこでも同じ確認を指す固定番号です。実施順や優先順位ではなく、答えに無理があれば前の問いへ戻ります。
...
実践章では、問い1を原因の確定、問い2を契約の決定、問い3を生成・所有・受け渡しの決定で使います。問いを置いただけで通過とはせず、答えをコードと実行結果で示せたときに、その確認を終えます。
...
第0章では、各フェーズで何を考え、何を確かめ、その判断を次へどう渡すかを説明します。その後の実践章で、初めて題材ごとの答えをコードとして組み立てます。
