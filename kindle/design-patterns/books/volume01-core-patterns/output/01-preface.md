...
関係者が分かる場合は補助線になりますが、分離の主軸はあくまで機能の種類です。これらの処理が1つの `OrderService` クラスに混在していると、メール文面を変える際に、データベース保存処理に影響が出ないかテストする必要が生じます。だからこそ、送料計算・通知・保存のように、変更の種類ごとに分離するのです。
