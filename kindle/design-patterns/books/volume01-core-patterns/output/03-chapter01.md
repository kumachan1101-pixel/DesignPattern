...
>
> 顧客情報の取得そのものが失敗する状況は掲載コードでは起きませんが、実運用のDB・API障害を受け止める場所として `try` / `catch` を残しています。
>
...
- **顧客データの取得そのものが失敗した**場合も、エラーを表示して終わります。
...
sequenceDiagram
    ...
    DB-->>OP: true（登録あり）
    Note over OP,DB: エラー系（メモ）: 未登録・空注文・取得失敗なら<br>calculateへ進まず、OPがエラーを表示して終了
...
コードは、クラス構成図で確認した責任の順に、固まりごとに読みます。取得に失敗したときは小計と支払額の計算を続けず、例外を注文単位のエラーへ変換します。先に、あらかじめ登録されている3件の顧客データを把握しておきます。
...
この手順で見ると、「金額を計算する」という大きな処理全体ではなく、その中のどの条件・計算式・判定が変更候補なのかを、読み手が自分で追えます。
...
フェーズ2「仮説立案」で「今変わること（確定）」と「将来変わるかもしれないこと（リスク）」を分けて整理できました。次は、フェーズ3で試す変更と、そのとき維持する動作を確定します。
...
変更ID1（サマーセール追加）・変更ID2（逐次割引）だけを現状の構造へ当て、小計の合算・結果表示・注文処理は動作を維持します。リスクID1（割引ルールが毎月増える）・リスクID2（定額引きへ変わる）は実装せず、フェーズ6の構造評価へ引き継ぎます。
...
>
> このフェーズで見るのは、変更を当てたときにどこまで開くことになったかです。開いた場所を1つずつこの問いにかけ、依頼に直接必要な修正と、そうでない修正を分けます。なぜ関係のない場所まで開くのかは、次のフェーズ4で調べます。`CampaignContext`へサマーセールの入力を足し、新しい割引を実装・実行すること自体は変更に直接必要なので、痛みではありません。新しい施策のために、既存のPremium・キャンペーン分岐の条件と順序まで書き換え、その計算を使う既存経路まで広く確認する部分を痛みとして追います。
...
変更を試した結果、二つの変更IDがどこまで届いたかを図にします。**箱はクラスの中の仕事、矢印のラベルはその仕事が見ている接続情報、淡い黄の面・茶色の枠と `［変更］` は今回書き換えたところ**です。ノード内の`変更前→変更後`で修正内容も示します。
...
graph TD
    ...
    subgraph PC["PaymentCalculator"]
        P2["［変更］ 既存の割引判定<br>Premium・キャンペーンの2分岐 → 4分岐"]:::touched
        P1["商品単価を合算して小計を出す<br>（変更なし）"]:::keep
    end

    subgraph CC["CampaignContext"]
        C1["［変更］ 開催中の施策を持つ<br>フラグ 1個 → 2個"]:::touched
    end
...
**「サマーセールを1つ足す」という依頼の矢印が、新しい箱ではなく、`既存の割引判定` の箱へ届いています。** 足せる場所がないので、既存の条件と順序を書き換えるしかありませんでした。箱の中の「2分岐 → 4分岐」が、その結果です。新しく作った箱は**一つもありません。**

**同じ枠の中には、今回変えない「商品単価を合算して小計を出す」も並んでいます。** 変えていないのに、同じ関数にあるので壊していないかの確認が要ります。点線がそれです。

**「施策の状態」というラベルは、`CampaignContext` の箱へも伸びています。** 同じものを見る場所が2つに分かれている、ということです。この矢印ラベルが、そのままフェーズ4で作る表の行になります。

### 3-3：痛みの言語化

上の三つの読み方のうち、一つめと二つめが痛みです。変更試行で確認した問題は、次の一つにまとめられます。
...
フェーズ3で、問題ID1（既存割引まで修正）と、その問題が起きた変更途中の `PaymentCalculator::calculate()` は特定できました。フェーズ4ではこの結果を入力にし、なぜ新しい施策を1つ足すだけで既存割引まで直すことになったのかを、このクラスが抱えている要素から調べます。

### 4-1：責任の混在を原因として確定する

変更影響グラフで、色の付いた箱と別の箱が同じ枠に並んでいたのは `PaymentCalculator` です。ここを調べます。このクラスが公開しているのは1本だけで、保持するメンバーはありません。
...
グラフの矢印ラベルが、そのまま接続情報です。**その接続情報を見て動く場所**を並べると、こうなります。会員種別と施策の状態は、`calculate()` の一つながりの `if-else` で一緒に使われているので、同じ行にまとめます。

| 接続情報 | それを見て動く場所 | 今回書き換えたか |
|---|---|---|
| 注文の商品リスト | `calculate()` の前半 | いいえ |
| 会員種別と施策の状態 | `calculate()` の `if-else`／`CampaignContext` のフラグ | はい |

1行目は単価を合算して小計を出すところ、2行目は割引の可否と割引率を決めるところと、開催中の施策を持つところです。入力を組み立てる `main()` は、責任を持つ場所ではないので行にしません。

読み方は三つです。

1. **2行目に場所が2つ並んでいます。** 同じものを見ているので、この二つは必ず一緒に変わります。施策を1つ足すと、`CampaignContext` のフラグと `calculate()` の分岐を両方開くことになりました。**この二つは、一つの置き場所へまとめる対象です。**
2. **1行目と2行目の場所が、どちらも `PaymentCalculator` の中にあります。** 見ているものが違うのに同居しています。**これが混在です。** 割引を直すために、同じ関数にある合計の処理まで読み直し、壊していないかを確認することになります。
3. **1行目は「いいえ」です。** 今回は変わりませんが、同居しているので巻き込まれます。

**「今回書き換えたか」は、分ける根拠です。** 同居していても、どれも今回変わっていないなら、まだ痛んでいないだけです。2行目に「はい」があるから、いま分ける理由になります。この「はい」は将来も続きます。リスクID2（パーセント引き以外の計算式）が届けば、同じ `if-else` を開くことになります。
...
**今回触らないクラスもあります。** `OrderProcessor` は、注文を受け、入力を検証し、計算を呼び、結果表示を呼びます。違うものを見ている仕事を複数抱えていますが、今回の変更でも、ヒアリングで確認した将来の理由でも開きません。**抱えていること自体は、直す理由になりません。** 記録して次の依頼を待ちます。
...
目指す形は、**どの置き場所も、一つの要素のためだけに開くようになること**です。原因は確定しました。次は、この原因をなくす責任配置を決めます。
...
**分けると、割引ルールは複数の実体になります。** 施策ごとに1つずつです。そこで避けられない問いが出ます。**複数あるうち、どれを使うのか。** いまはその答えが `if-else` の並び順でした。ルールを外へ出せば、並びの知識も一緒に外へ出ます。置き場所の候補は三つです。

| 置き場所 | 何が起きるか |
|---|---|
| 小計と支払額の計算が持つ | 施策が増えるたびに計算側を開く。**原因ID1が戻る** |
| 個々の割引ルールが持つ | ルールどうしが互いを知る。1つ足すと既存ルールを開く |
| **第三の場所が持つ** | どちらも起きない |

第三の場所しか残りません。**適用割引の優先順・選択**を、計算側にも個々のルールにも置かない、という二つめの課題になります。

そこで、フェーズ4の対策前図を、次の責任配置へ変えます。追加した②は、原因の混在そのものではなく、①を解いた結果として必要になる置き場所です。
...
classDiagram
    ...
    class ConflictPolicy["② 適用割引の優先順・選択"]:::separated {
        <<分ける責任>>
        責任：適用割引の優先順・選択
    }
...
黄色は既存の小計と支払額の計算、青は外へ分ける責任です。①と②は次の課題表に対応します。

この配置になると、フェーズ4で数えた要素は次のように置き直されます。

| 分けたあとの置き場所（責任） | 抱える要素 | 施策が1つ増えたとき |
|---|---|---|
| 割引条件の判定・金額計算 | 割引ルール | 開く（新しいルールを1つ作る） |
| 適用割引の優先順・選択 | どれを使うかの決め方 | 開く（並びへ1つ足す） |
| 小計と支払額の計算 | 小計と支払額 | **開かない** |

フェーズ4では、2つの要素が `PaymentCalculator` の1クラスへ同居していました。分けたあとは、どの置き場所も一つの要素だけを抱えます。施策を1つ足しても、合計の出し方を書いた場所は開きません。ここで決めたのは責任の配置と情報の向きまでで、型名・操作名・生成場所はフェーズ6で決めます。
...
#### 課題ID2（適用割引の優先順・選択の分離）の完了条件

**解く原因：** 原因ID1（小計と支払額の計算と割引条件の判定・金額計算が同じクラスにある）を解いた結果、割引ルールが複数の実体に分かれること

**構造の変更：** 目標図の②のとおり、適用割引の優先順・選択を小計と支払額の計算から分けます。

**接続：** 小計と支払額の計算から会員種別・施策状態を渡し、適用割引の優先順・選択から選ばれた施策を返します。

完成コードで次の三点を満たせば完了です。

- `PaymentCalculator`に具体的な割引の優先順がない
- 選択処理に具体的な施策名と優先順がない
- 優先順を別の一か所で変更できる

二つとも満たせば、小計と支払額の計算は施策の内容も優先順も知らずに計算できます。
...
| 同上 | 同上（課題ID1を解くと割引ルールが複数になる） | 課題ID2：適用割引の優先順・選択を、計算側にも個々のルールにも置かない |

## 🔴 フェーズ6：対策検討 ―― 構想を一つのコード経路へ変える

> **フェーズ6の確認観点：** 「契約と具体」「生成・所有」「選択・登録」「受け渡し」「実行」が、課題ID1（割引条件の判定・金額計算の境界）・課題ID2（適用割引の優先順・選択の分離）を同時に解く一つの経路としてつながるかを確認します。
...
課題ID2（適用割引の優先順・選択の分離）で確定した「選ばれた施策」は、この `IDiscountRule` を指す参照として表します。競合順の決め方は後続の選択器へ置くため、ここで別の契約は増やしません。要求ID5（購入結果の表示）の表示項目もこの境界には不要です。
...
`matches()` と `apply()` は、どちらも課題ID1（割引条件の判定・金額計算の境界）です。同じ施策の条件と計算なので、一つの契約に置きます。課題ID2（適用割引の優先順・選択の分離）は、このあと生成・登録の箇所で確認します。

具体施策はこの契約の裏へ条件と式を一組で置きます。`PremiumDiscount` はPremium会員に一致して20%引きし、逐次割引を受け持つ `SummerSaleAndCampaignDiscount` はRegular会員かつ二施策が有効な場合に10%引き、続けて5%引きします。どちらも小計と支払額の計算・表示・競合順を持ちません。実装本体はフェーズ7で並べます。
...
販促施策を小計と支払額の計算へ残す形や、具体的な施策名と優先順を選択処理へ固定する形では原因が残ります。原因を二つともなくす構造はここまでで一つに定まり、フェーズ7で統合図と完成コードを示します。
...
classDiagram
    ...
    OrderProcessor ..> PaymentResult : 表示へ渡す
    CartPreviewService ..> PaymentResult : 支払金額を使う

    classDef added fill:#eaf2fb,stroke:#527aa3,stroke-width:2px,color:#172033;
    classDef touched fill:#fff7e6,stroke:#9a6b2f,stroke-width:3px,color:#172033;
```

両入口は具体名を知らず、同じ `RuleSelector` と `PaymentCalculator` を使います。顧客取得は現状維持です。結果表示は表示項目を変えずに、受け取る値を `PaymentResult` へまとめ、`OrderProcessor` が直接出していたエラー表示も引き受けます。最後の図で、間を流れる値を確認します。
...
フェーズ4で `PaymentCalculator` に混在していた三つの責任は、完成図では分かれています。`PaymentCalculator` は小計と支払額の計算、各 `IDiscountRule` 実装は割引条件の判定・金額計算、`DiscountRuleSet` は適用割引の優先順・選択を担い、`RuleSelector` は登録順に選択を実行します。`CampaignContext` は施策コード一覧を渡すだけです。対象とした三つの変更理由を、一つのクラスが同時に持つ構造ではなくなりました。
...
| 原因 | `PaymentCalculator` が全ルールの条件・式と、適用する割引の優先順を抱えている |
...
- **得られること2：変更が広がる原因の特定。** `PaymentCalculator` が割引条件の判定・金額計算と適用割引の優先順・選択まで抱えていたため、新しい割引で既存割引と小計と支払額の計算まで開くと説明できました。
...
この四つがそろうことで、条件分岐を機械的にクラスへ移すのではなく、変更理由と影響範囲を根拠にStrategyを採用するか判断できます。
