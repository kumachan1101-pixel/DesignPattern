...
フェーズ3で、問題ID1（既存割引まで修正）と、その問題が起きた変更途中の `PaymentCalculator::calculate()` は特定できました。フェーズ4ではこの結果を入力にし、なぜ新しい施策を1つ足すだけで既存割引まで直すことになったのかを、責任の配置から調べます。
...
| 変更理由 | 第0章の型 | 出どころ |
|---|---|---|
| A：割引施策の種類が増える | 仲間が増える | 変更ID1、リスクID1 |
| B：割引の適用条件・計算式が変わる | 規則が変わる | リスクID2 |
| C：施策どうしの優先・併用が変わる | 選び方・順序が変わる | 変更ID2 |

次に、粒度を一段下げます。変更影響グラフはクラスの粒度でしたが、同居を見るには `PaymentCalculator` の中で何を書き換えたかまで割る必要があります。そこで、`［変更］`が付いたクラスの中身を仕事ごとの行にし、今回書き換えなかった小計の合算も、同じクラスの中にあるので行へ入れます。入力を組み立てる `main()` は行にしません（どの実体をいつ作るかはフェーズ6で扱います）。行が仕事、列がさきほどの変更理由です。`○`は今回開いたところ、`△`は将来その理由が来たときに開くところです。

| 開いた仕事 | いまあるクラスと場所 | A | B | C |
|---|---|---|---|---|
| 開催中の施策を入力として持つ | `CampaignContext`（入力の型） | ○ | | |
| 施策ごとの適用条件と割引率 | `PaymentCalculator::calculate()` の `if-else` | ○ | △ | |
| 施策が重なったときの優先・並び順 | `PaymentCalculator::calculate()` の分岐の並び | ○ | | ○ |
| 商品単価を合算して小計を出す | `PaymentCalculator::calculate()` の前半 | | | |

読み方は四つです。

1. **同じクラスの行に、別々の列の印が付いている**なら、そのクラスに複数の責任が同居しています（混在）。`PaymentCalculator` の3行に、A・B・Cの印が集まっています。分ける対象はここです。
2. **一つの行に印が2つ以上**あるなら、その仕事自体が複数の理由で変わります。条件と割引率の行がAとB、並び順の行がAとCです。
3. **ある列の印が1行に収まっている**なら、その理由には専用の置き場所があります。Aは入力の行と条件の行に分かれていて、専用の場所はまだありません。
4. **どの列にも印がない行**は、今回は変わりません。ただし合算の行は混在したクラスの中にあるため、割引を直すたびに一緒に読み直すことになります。これが巻き込みです。

列は「なぜ変わるか」、責任は「その理由で一緒に変わる仕事が何をするか」です。列に仕事の側から名前を付け直すと、分けたあとの責任になります。AとBは「施策が増えれば、その施策の条件と式も決める」という形で必ず一緒に動くので、一つにまとめます。名前を付けると三つです。

| 責任 | 対応する変更理由 | まとめている仕事 |
|---|---|---|
| 小計と支払額の計算 | 今回の理由では動かない | 商品単価を合算する／割引後の金額を支払金額として返す |
| 割引条件の判定・金額計算 | A・B | 施策ごとに適用できるかを判定する／割引後の金額を求める |
| 適用割引の優先順・選択 | C | 複数の割引が成立したとき、使う割引を決める |

**「どれも支払金額の計算だから、責任は一つでは」と感じるかもしれません。** 責任は、扱う題材ではなく書き換えるきっかけで分けます。金額を出すという題材は同じでも、施策の中身を決める人と、施策どうしの優先を決める人は別で、変わる時期も別々です。

この三つの責任名は、変更試行で観測した処理からここで初めて付けています。変更を試した時点の配置は、次の構造です。
...
今回、割引条件の判定・金額計算と適用割引の優先順・選択は直接変わりました。小計と支払額の計算方法は変わりませんが、三つが同じ `calculate()` にあるため修正・確認へ巻き込まれました。
...
**原因ID2（小計と支払額の計算と適用割引の優先順・選択が同じクラスにある）。** どの割引を優先するかが、同じ `if-else` の並び順で決まります。この配置も問題ID1（既存割引まで修正）を生みました。

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 答えは「ある」です。小計と支払額の計算、割引条件の判定・金額計算、適用割引の優先順・選択は別々の理由で変わりますが、`calculate()` に集まっています。
...
入力は、原因ID1（小計と支払額の計算と割引条件の判定・金額計算が同じクラスにある）と原因ID2（小計と支払額の計算と適用割引の優先順・選択が同じクラスにある）です。

### 5-1：原因をなくす責任配置を決める

**原因ID1への対応。** 割引条件の判定・金額計算を小計と支払額の計算から分けます。適用条件と計算式は同じ割引ルールとして一緒に変わるため、二つには分けません。

**原因ID2への対応。** 適用割引の優先順・選択を小計と支払額の計算から分けます。金額を計算する側は優先順を判断せず、選ばれた割引ルールだけを使います。

割引の条件だけを外へ出しても、小計と支払額の計算に割引式が残ります。具体的な施策名と優先順を計算側に残しても、原因は消えません。そこで、フェーズ4の対策前図を、次の責任配置へ変えます。
...
フェーズ4では、A・B・Cの印が `PaymentCalculator` の1クラスへ集まっていました。分けたあとは、行ごとに自分の責任のきっかけだけが残ります。施策を1つ足すとき（A）は上の2行、優先の決まりを変えるとき（C）は2行目だけを開き、小計の行は開きません。ここで決めたのは責任の配置と情報の向きまでで、型名・操作名・生成場所はフェーズ6で決めます。
...
**解く原因：** 原因ID2（小計と支払額の計算と適用割引の優先順・選択が同じクラスにある）
...
| 問題ID1：新しい割引の追加で既存順序まで修正 | 原因ID2：小計と支払額の計算と適用割引の優先順・選択が同じクラスにある | 課題ID2：適用割引の優先順・選択を小計と支払額の計算から分ける |
