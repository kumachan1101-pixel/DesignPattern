...
この章で扱う現状仕様は、次の範囲です。**「一例」と書いた行は、値を替えてもこの章の話は変わりません**（商品や顧客を替えても、設計の論点は同じです）。**「これで全部」と書いた行は、ほかの選択肢がありません。**

| 仕様項目 | この章で使う値 | 役割 |
|---|---|---|
| 商品リスト | ワイヤレスイヤホン 10,000円（**一例**） | 商品単価を合算して小計を出す |
| 顧客 | C001 は Premium、C002 は Regular（**一例**） | 顧客IDから会員種別を取得し、割引条件に使う |
| キャンペーン | 期間中か、期間外か（**これで全部**） | 期間中だけキャンペーン割引を有効にする |
| 割引の種類 | Premium20%とキャンペーン10%（**これで全部**） | どちらを適用するかを条件で選ぶ |
| 割引適用順序 | Premiumは他割引と併用しない（**この1つだけ**） | 複数条件が成立したときの計算順序を決める |
| 出力 | 購入結果と事前プレビュー（**この2つだけ**） | 同じ入力条件なら同じ支払金額になることを確かめる |
...
変更後の仕様を、現状の仕様と同じ粒度で、正常系の入力・判定・加工・出力として確認します。**新しく足したノードを青のぬりつぶしにしてあります。** この図には、もとからあって中身が変わったノードはありません。差分は3つで、入力に「サマーセールフラグ」が加わること、「Regular会員かつサマーセール中か」という判定が1つ増えること、割引額を計算したあとに「サマーセール5%を逐次適用する」という加工が挟まることです。サマーセールの対象はRegular会員なので、この図だけは代表の1件を C002（佐藤 花子・Regular）の注文へ替えて、新しく増えた経路を通したところを示します。
...
flowchart TB
    ...
    E[/キャンペーンフラグ<br>false/]:::input --> H[割引ルールを選ぶ]:::process
    S[/サマーセールフラグ<br>true/]:::input --> T{Regular会員かつ<br>サマーセール中か}:::decision
    D -->|Yes| H
    D -->|No| H
    G --> I[割引額を計算<br>10,000円]:::process
    H --> I
    I --> T
    T -->|Yes| U[割引後の金額に<br>サマーセール5%を逐次適用<br>9,500円]:::process
...
| 現状コードでの定義 | 今回の変更 | 根拠 |
|---|---|---|
| `CampaignContext` | `isSummerSale` フラグを1つ追加 | 変更ID1（サマーセール追加） |
| `PaymentCalculator` | 割引判定を2分岐から4分岐へ書き換え | 変更ID1（サマーセール追加）・変更ID2（逐次割引） |
| `CheckoutResultRenderer` | 条件表示へサマーセールの有無を1項目追加 | 変更ID1（サマーセール追加）・要求ID5（購入結果の表示） |
...
|---|---|---|---|---|---|
| A 条件だけ | ○ | なし | — | **×** 原因ID2が残る | 落とす |
| B 式だけ | ○ | なし | — | **×** 原因ID1が残る | 落とす |
| C1 施策ごとの適用条件を、選択の流れから外へ | ○ | なし | 新施策の追加で変わる | ○ | **残す** |
| C2 施策ごとの計算式を、小計計算から外へ | ○ | なし | 割引率の改定で変わる | ○ | **残す** |
| D 関数へ切り出す | **×** 条件も式も関数内に残る | — | — | × | 落とす |

**C1とC2は、同じクラスの中に置くことになります。** 施策1つにつき「当てるかどうか」と「いくら引くか」を持つ部品です。それでも課題を2つに分けて追うのは、**完了したと言える条件が別々だから**です。C1は選択側が施策名を1つも持たなくなったとき、C2は計算側が個別の式を1つも持たなくなったときに終わります。片方だけ達成して終わらせないために、別々に数えます。
...
> **悩みどころ：組み合わせのためのクラスが増えないか**
>
> 5つのルールのうち、2つの施策が重なった場合を受け持つ `SummerSaleAndCampaignDiscount` は、施策そのものではなく**2つの施策が重なったときの扱い**を表しています。施策が4つ、5つと増えたら、重なりの数だけクラスが増えるのではないか——実際、私もここで手が止まりました。
>
...
>
> 私は登録順を選び、代わりに**登録している場所の隣にコメントで理由を書く**ことにしました（上のコードの `// Premiumは他施策と併用しない`）。順序が業務方針であることを、読む人がその場で分かるようにするためです。
>
> どちらが正解ということはありません。**順序を変える人が多いチームなら、数値で明示したほうが安全**です。私の判断は「順序を変える機会は年に数回で、変えるのは同じ人」という前提のうえに立っています。
...
> **悩みどころ：結局、全部を知っているクラスが1つ残る**
>
> 分離できたと言いましたが、正直に見ると `DiscountRuleSet` は**5つの具体ルールすべてをメンバーとして持ち、優先順まで知っています。** 施策を1つ足せば、このクラスは必ず変わります。「変更が1か所で済む」とは言えても、**「どこも変えなくてよい」わけではありません。**
>
...
>
> ではその1か所すら変えたくない場合はどうするか。設定ファイルやDBからルールを読み込む形にすれば、コードは変わりません。ただし今度は「設定が正しいか」をコンパイラが見てくれなくなります。**綴りの誤りが実行時まで分かりません。**
>
> どちらを選ぶかは、**ルールを変える人がプログラマかどうか**で決まると私は考えています。運用担当が変えるなら設定へ、開発者が変えるならコードのままで十分です。

#### 完成後のクラス図

フェーズ6で確定したクラス・操作・関係線を完成図にします。ここから3枚続く完成図では、**新しく作ったクラスを青のぬりつぶし、もとからあって中身が変わったクラスを青い枠**にしてあります。色の付いていないクラスは、現状から手を触れていません。

まずルールの差し替え部分です。`IDiscountRule` が差し替え可能なルール契約、各割引クラスがその具象ルールに対応します。`DiscountRuleSet` は具体ルールを所有して競合方針の順に登録し、`RuleSelector` は具体条件を知らずに登録ルールを選びます。**この8つは、すべて新しく作ったクラスです。** 現状クラス図に対応する箱が1つもないので、この図では色を付けていません。
