...
フェーズ3から受け取るのは、問題ID1（既存割引まで修正）と、変更途中の `PaymentCalculator::calculate()` です。ここでは対策を決めません。問題が起きた理由だけを調べます。
...
| 責任 | 変わるきっかけ | 今回の扱い |
|---|---|---|
| 販促施策 | 対象者や割引内容の変更 | **変える** |
| 施策の優先順 | 併用や優先の方針変更 | **変える** |
| 注文金額の計算 | 注文計算の変更 | **守る** |
...
前の表で分かったのは、何を変え、何を守るかまでです。問題が起きた理由は、`calculate()` へ戻って確認します。

**原因ID1（注文計算が販促施策を持つ）。** 割引ごとの条件と計算が、小計を作って返す処理の中にあります。そのため、新しい割引の追加で既存の注文計算まで直しました。

**原因ID2（注文計算が施策の優先順を持つ）。** どの割引を優先するかが、同じ `if-else` の並び順で決まります。そのため、新しい割引の追加で既存条件の順序まで読み直しました。

二つとも、問題ID1（既存割引まで修正）の原因です。

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 答えは「ある」です。注文計算、販促施策、施策の優先順は別々の理由で変わりますが、`calculate()` に集まっています。

原因は確定しました。次は、二つの原因をなくす分け方を決めます。クラス名や操作名はまだ決めません。

---

## 🟡 フェーズ5：課題定義 ―― 原因から課題を検討して確定する

入力は、原因ID1（注文計算が販促施策を持つ）と原因ID2（注文計算が施策の優先順を持つ）です。

### 5-1：原因をなくす分け方を決める

**原因ID1への対応。** 割引ごとの条件と計算を、一つの販促施策として注文計算から分けます。施策を使う側は、適用判断に会員種別・開催状態を渡し、計算時に適用前金額を渡します。施策から受け取るのは、適用可否と適用後金額です。

**原因ID2への対応。** どの施策をどの順で試すかを、注文計算から分けます。注文計算は優先順を判断せず、選ばれた施策だけを使います。

条件と計算は同じ施策理由で変わるため、別々の責任にはしません。ここでは責任と受け渡す情報だけを決め、型名や操作名はフェーズ6で決めます。

### 5-2：原因が残らないか確認する

条件だけを分けても、注文計算に割引式が残ります。条件と式を分けても、注文計算が具体的な施策名と順序を知れば、二つ目の原因が残ります。

したがって、条件と式を施策単位で分け、施策の優先順も注文計算から分けます。
...
|---|---|---|
| 課題ID1（販促施策の境界）<br>解く原因：原因ID1 | 施策の条件と計算を注文計算から分ける | 入力：会員種別・施策状態・適用前金額<br>結果：適用可否・適用後金額 |
| 課題ID2（優先順の分離）<br>解く原因：原因ID2 | 施策の優先順を注文計算から分ける | 入力：会員種別・施策状態<br>結果：選ばれた施策 |

#### 課題ID1（販促施策の境界）の完了条件

- `PaymentCalculator::calculate()` に、具体的な施策名・条件・割引式がない
- 同じ施策の条件と計算を、一つの施策単位で変更できる
- 注文計算は、すべての施策を同じ方法で利用する

#### 課題ID2（優先順の分離）の完了条件

- `PaymentCalculator::calculate()` に、施策の優先順がない
- 選択処理に、具体的な施策名と優先順がない
- 優先順は、注文計算・選択処理とは別の一か所で変更できる
...
|---|---|---|
| 問題ID1：新しい割引の追加で既存割引まで修正 | 原因ID1：注文計算が販促施策を持つ | 課題ID1：販促施策を注文計算から分ける |
| 問題ID1：新しい割引の追加で既存順序まで修正 | 原因ID2：注文計算が優先順を持つ | 課題ID2：優先順を注文計算から分ける |

## 🔴 フェーズ6：対策検討 ―― 構想を一つのコード経路へ変える

> **フェーズ6の確認観点：** 「契約と具体」「生成・所有」「選択・登録」「受け渡し」「実行」が、課題ID1（販促施策の境界）・課題ID2（優先順の分離）を同時に解く一つの経路としてつながるかを確認します。

### 構想を決める

課題ID1（販促施策の境界）と課題ID2（優先順の分離）を、**具体ルールの追加と登録だけで施策を増やせる一つの経路**へまとめます。
...
増えるのは施策の条件と計算です。小計計算と結果返却は変わりません。課題ID1（販促施策の境界）では、会員種別と施策状態を渡して適用可否を受け取ります。
...
同じ課題ID1（販促施策の境界）で、適用前金額を渡して適用後金額を受け取ります。条件と計算は、一つの施策の中に置きます。
...
`matches()` と `apply()` は、どちらも課題ID1（販促施策の境界）です。同じ施策の条件と計算なので、一つの契約に置きます。課題ID2（優先順の分離）は、このあと生成・登録の箇所で確認します。
...
完成構造は一つです。販促施策を注文計算へ残す形や、具体的な施策名と優先順を選択処理へ固定する形では原因が残ります。そのため完成案として比較しません。課題とリスクだけを確認し、統合図はフェーズ7で示します。

#### 課題から採用構想までを照合する

| 対象 | 採用した構造とコード | 確認する効果 |
|---|---|---|
| 課題ID1（販促施策の境界） | 各ルールの `matches()` と `apply()`。`PaymentCalculator` は契約だけを呼ぶ | 施策の条件と計算を、対象ルールへ閉じる |
| 課題ID2（優先順の分離） | `DiscountRuleSet` が優先順に登録し、`RuleSelector` は登録順に選ぶ | 優先順を組み立て箇所だけで変更する |
...
|---|---|---|
| 課題ID1（販促施策の境界） | 条件と計算を各施策の `matches()` と `apply()` へ移した | 施策内容の変更は対象ルールに閉じる |
| 課題ID2（優先順の分離） | 優先順を `DiscountRuleSet` の登録順へ移した | 優先順の変更は組み立て箇所に閉じる |
