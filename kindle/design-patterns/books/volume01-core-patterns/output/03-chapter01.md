...
問題はフェーズ3で確定済みです。責任一覧と `calculate()` の変更箇所へ戻り、問題を生んだ責任の組だけを取り出します。

| 問題ID | 関係する責任（変更の中心 → 影響した責任） | コード上の目印 |
|---|---|---|
| 問題ID1（修正範囲） | 新しい施策の条件・式 → 小計合算と結果返却 | `if-else`・`return total` |
| 問題ID2（解読範囲） | 施策の重複順序 → 既存施策の条件 | `isSummerSale && isCampaignActive` |
| 問題ID3（壊しやすさ） | 新規施策の差し込み → 既存の優先・排他 | `memberType == "Premium"` |

割引責任自体が問題なのではありません。変更理由の異なる責任まで、同じ `calculate()` にあるため影響した関係を問題として見ます。
...
| 責任 | 変わるきっかけ | 今回 |
|---|---|---|
| 販促施策の適用条件と割引式 | 施策の対象・割引内容の変更 | **変える** |
| 施策どうしの順序と排他 | 施策の競合方針の変更 | **変える** |
| 小計の合算から結果返却まで | 注文計算の入出力契約の変更 | **守る** |
