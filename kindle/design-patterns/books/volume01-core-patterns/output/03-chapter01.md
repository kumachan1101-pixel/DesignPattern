...
| 仕様項目 | この章で使う具体値 | 役割 |
|---|---|---|
| 商品リスト | ワイヤレスイヤホン 10,000円 | 商品単価を合算して小計を出す |
| 顧客 | C001 は Premium、C002 は Regular | 顧客IDから会員種別を取得し、割引条件に使う |
| キャンペーン | 期間中なら true、期間外なら false | 期間中だけキャンペーン割引を有効にする |
| 割引適用順序 | Premiumは他割引と併用しない | 複数条件が成立したときの計算順序を決める |
| 出力 | 購入結果と事前プレビュー | 同じ入力条件なら同じ支払金額になることを確かめる |
