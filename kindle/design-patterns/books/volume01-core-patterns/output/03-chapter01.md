...
**2本の矢印が集まっているのは `PaymentCalculator` です。** このクラスが何を抱えているのかは、次のフェーズ4で調べます。
...
変更影響グラフで矢印が集まったのは `PaymentCalculator` です。ここを調べます。まず、このクラスの接続情報を見ます。公開しているのは `calculate(注文, 会員種別, 施策の状態)` の1本で、支払金額を返します。保持するメンバーはありません。

接続情報は4つです。一つずつ、**これを通じて中の何が変わるか**を問います。

| 接続情報 | これを通じて中の何が変わるか | 要素か |
|---|---|---|
| 注文（商品リスト） | 単価を合計するだけ。分岐しない | 経路 |
| 会員種別 | 値を見て割引率が変わる | **要素** |
| 施策の状態 | 値を見て割引率が変わる | **要素** |
| 戻り値の支払金額 | 求めた金額を返すだけ | 経路 |

会員種別と施策の状態は、`calculate()` の中で**一つの `if-else` の連なり**になっていて、どちらも同じ合計額を出しています。同じ結果を出すために連なっている分岐なので、**一つの要素**として数えます。ここでは「割引ルール」と呼びます。

商品リストから戻り値までは分岐がありませんが、単価を合算して支払金額を返すという仕事がそこにあります。これも一つの要素です。「小計と支払額」と呼びます。

| 要素 | いまコードのどこにあるか | 今回 | 将来 |
|---|---|---|---|
| 割引ルール（適用条件と割引率、その並び） | `CampaignContext` のフラグ／`PaymentCalculator::calculate()` の `if-else` | ○ | △ |
| 小計と支払額 | `PaymentCalculator::calculate()` の前半と戻り値 | | |

`○`は今回の変更で実際に書き換えたところ、`△`はリスクID2（パーセント引き以外の計算式）が届いたときに書き換えるところです。

読み方は二つです。

1. **`PaymentCalculator` が要素を2つ抱えていて、そのうち1つに印が付いています。** これが混在です。割引ルールを直すために、同じ `calculate()` にある合計の処理まで読み直し、壊していないかを確認することになります。
2. **割引ルールという1つの要素が、`CampaignContext` と `calculate()` の2か所に散っています。** 施策を1つ足すと、入力のフラグと判定の分岐を両方開くことになります。この要素には、まだ専用の置き場所がありません。

要素から責任名を付けます。その要素について何を判断・計算するのか、という形で書きます。

| 責任 | 対応する要素 | まとめている仕事 |
|---|---|---|
| 割引条件の判定・金額計算 | 割引ルール | 施策ごとに適用できるかを判定する／割引後の金額を求める |
| 小計と支払額の計算 | 小計と支払額 | 商品単価を合算する／割引後の金額を支払金額として返す |

**「どれも支払金額の計算だから、責任は一つでは」と感じるかもしれません。** 責任は、扱う題材ではなく変わる要素で分けます。金額を出すという題材は同じでも、施策の中身を決める人と、合計の出し方を決める人は別で、変わる時期も別々です。

**今回触らないクラスもあります。** `OrderProcessor` は、注文を受け、入力を検証し、計算を呼び、結果表示を呼びます。要素を複数抱えていますが、今回の変更でも、ヒアリングで確認した将来の理由でも開きません。**抱えていること自体は、直す理由になりません。** 記録して次の依頼を待ちます。
