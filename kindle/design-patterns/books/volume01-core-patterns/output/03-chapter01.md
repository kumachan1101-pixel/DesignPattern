...
|---|---|---|
| 小計を作るループ | 商品の金額を合算する | 問題ID1 |
| 割引の `if-else` | 各施策を適用できるか判断する | 問題ID1・問題ID2 |
| 割引の `if-else` | 一致した施策の割引額を計算する | 問題ID1 |
| 分岐の並びと排他条件 | 複数施策が競合したときの順序を決める | 問題ID2・問題ID3 |
| `return total` | 最終金額を呼び出し側へ返す | 問題ID1 |
