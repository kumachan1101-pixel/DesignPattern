...
|---|---|---|
| 原因ID1（選択が施策条件を抱える） | 施策追加のたびに選択処理へ分岐が増える | 施策ごとの適用条件を選択の流れから分離する |
| 原因ID2（計算骨格が計算式を抱える） | 式を変えるだけでも小計計算まで再確認する | 施策ごとの計算式を小計計算から分離する |
...
| 課題候補 | 確認できた必要性 | 整理結果 |
|---|---|---|
| 適用条件の分離 | 変更ID1で選択処理への分岐追加が起きた | 計算式とは変わる理由が違うため、独立課題にする |
| 計算式の分離 | 変更ID2で計算骨格と各式を同時に直した | 選択後の共通実行契約として、独立課題にする |
...
整理した候補に課題ID1（選択条件の境界）から欠番なくIDを付けます。
...
- **課題ID1** ← 原因ID1（選択の流れが、施策条件と競合時の順序を抱えている）← 問題ID2（差し込み位置の解読）・問題ID3（選択条件と適用順序が読み解けず、触ると壊れる）
