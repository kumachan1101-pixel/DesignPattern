...
>
> **実務でファイルを分けるなら：** 1ファイルにまとめてあるのは、手元で動かしやすくするためです。実務では次のように分けます。**ファイルの分け方も設計判断の一つ**なので、この章の構造がそのまま形になります。
>
> | ファイル | 入れるもの | なぜそこか |
> |---|---|---|
...
>
> 5つのルールのうち `SummerSaleAndCampaignDiscount` は、施策そのものではなく**2つの施策が重なったときの扱い**を表しています。施策が4つ、5つと増えたら、重なりの数だけクラスが増えるのではないか——実際、私もここで手が止まりました。
>
