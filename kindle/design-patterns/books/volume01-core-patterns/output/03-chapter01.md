...
責任名は、変更前後で共通する仕事のまとまりです。1行が一つの責任で、左から変更前、変更後の順に規則を比べます。左右が同じ行は、今回維持する責任です。

| 責任 | 変更前 | 変更後 |
|---|---|---|
| 販促方針 | Premiumは20%引き<br>Regularはキャンペーン中10%引き | Premiumは20%引き<br>Regularはキャンペーン10%、サマー5%、重複時は順に適用 |
| 競合方針 | Premium→キャンペーン→割引なし | Premium→重複→サマー→キャンペーン→割引なし |
| 注文計算 | 商品小計へ選ばれた販促方針を適用 | 商品小計へ選ばれた販促方針を適用 |

販促方針と競合方針の規則が変わり、注文計算は変わりません。それでも直前に確認したコードでは、三つが同じ `calculate()` にあるため、注文計算まで読み直しました。

### 4-3：原因を確定する

前の表で分かったのは、どの責任の規則が変わり、どの責任が維持されるかまでです。問題が起きた理由は、`calculate()` へ戻って確認します。

**原因ID1（注文計算が販促施策を持つ）。** 割引ごとの条件と計算が、小計を作って返す処理の中にあります。そのため、Regular会員のサマー5%割引への対応で、既存の注文計算まで直しました。
...
|---|---|---|
| ① 課題ID1（販促施策の境界）<br>解く原因：原因ID1 | 販促方針を注文計算から分ける | 入力：会員種別・施策状態・適用前金額<br>結果：適用可否・適用後金額 |
| ② 課題ID2（優先順の分離）<br>解く原因：原因ID2 | 競合方針を注文計算から分ける | 入力：会員種別・施策状態<br>結果：選ばれた施策 |
...
| 課題 | 決定した構造 | コード上の実現 |
|---|---|---|
| 課題ID1（販促施策の境界） | 販促方針を共通契約の後ろへ置く | `IDiscountRule`と各施策クラス |
| 課題ID2（優先順の分離） | 競合順を注文計算・選択処理の外で保持する | `DiscountRuleSet`の登録順と`RuleSelector` |
...
| フェーズ5の完了条件 | コード上の証拠 | 判定 |
|---|---|---|
| `PaymentCalculator`に施策の優先順がない | `calculate()`は受け取った施策を適用するだけ | 合格 |
