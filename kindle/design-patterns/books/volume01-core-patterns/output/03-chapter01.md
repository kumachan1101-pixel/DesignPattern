...
この入力と出力から、(1)顧客・商品・会員条件を受け取り、(2)会員種別とキャンペーンで割引を判定して金額を計算し、(3)小計と支払金額が表示される、という一連の動きが読み取れます。同じ入力を含む完全なコードと実行結果は現状コードとして後で載せます。
...
仕様で見た入力を、受け取り口から利用箇所、確認できる結果まで追います。構造体に値があるだけでなく、各値が判定または計算に実際に使われることを確認します。

| 入力 | コードでたどる経路 | 結果にどう現れるか |
|---|---|---|
| 商品リスト | `Order::items` → `PaymentCalculator::calculate()` | 小計と支払金額になる |
| 顧客ID | `Order::customerId` → `CustomerDatabase::exists()` / `get()` | 会員別の割引か、未登録エラーに分かれる |
| キャンペーン状態 | `CampaignContext::isCampaignActive` → `PaymentCalculator::calculate()` | 有効なRegular会員だけ10%引きになる |
...
要求ID1（小計から支払金額）〜要求ID4（未登録IDを拒否）と要求ID6（プレビュー）は変更前と変更後が同じなので、下の対照表には出てきません。変更後ベースラインで内容を確認できます。要求ID5（購入結果の表示）を変更に含めたのは、サマーセールという新しい条件と、逐次適用したときの割引名を購入結果へ出す必要があり、`CheckoutResultRenderer` へ渡す値が増えるためです。
...
> **この抜粋の外は、現状のままです。** `Item`、`Order`、`CustomerInfo`、`CustomerDatabase`、`CartPreviewService`、`OrderProcessor` は現状コードの定義をそのまま使います。以下は現状コードで読んだ順に、変更が入った定義だけを並べたものです。

変更した定義は3つです。現状コードと同じ並び順で、上から見ていきます。
