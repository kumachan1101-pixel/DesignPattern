...
graph TD
    ...
    subgraph PC["PaymentCalculator"]
        P2["［変更］ 割引の可否と割引率を決める<br>分岐 2本 → 4本"]:::touched
        P1["商品単価を合算して小計を出す"]:::keep
    end
...
二つの依頼はどちらも `PaymentCalculator` の「割引の可否と割引率を決める」へ届いています。**同じ箱に2本届いていること自体は、同居ではありません。** 見るのは枠の中です。`PaymentCalculator` の枠には、今回書き換えない「商品単価を合算して小計を出す」も並んでいて、この二つは違うものを見ています。また、「施策の状態」というラベルは `CampaignContext` の箱へも伸びています。**この矢印ラベルが、そのままフェーズ4で作る表の行になります。**
