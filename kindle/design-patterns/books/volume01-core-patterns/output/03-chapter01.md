...
要求ID1（小計から支払金額）〜要求ID4（会員種別の取得）と要求ID6（プレビュー）は継続（変更前＝変更後）のため対照表には載せません。変更後ベースラインで内容を確認できます。要求ID5（購入結果の表示）を変更に含めたのは、サマーセールという新しい条件と、逐次適用したときの割引名を購入結果へ出す必要があり、`CheckoutResultRenderer` へ渡す値が増えるためです。
