...
| 要求ID2（Premium20%） | Premium会員へ20%割引を適用し、Regular向けキャンペーン割引とは併用しない | Premiumの10,000円が8,000円になる |
| 要求ID3（Regularキャンペーン10%） | Regular会員はキャンペーン中だけ10%割引する | キャンペーン中は9,000円、期間外は10,000円になる |
...
|---|---|---|
| ① | 購入者 → 購入機能 | 選んだ商品と購入操作 |
| ② | 購入機能 → 支払金額計算 | 商品リスト・顧客ID・キャンペーン状態 |
| ③ | 顧客情報 → 支払金額計算 | 顧客IDに対応する会員種別 |
| ④ | 支払金額計算 → 購入機能 | 支払金額または入力エラー |
| ⑤ | 購入機能 → 購入者 | 注文確定額またはプレビュー額 |

支払金額計算を一つのシステムとして見て、②と③を入力に④を作り、⑤として購入者へ返す流れです。
...
| 要求ID2（Premium20%） | Premiumは20%引きとし、キャンペーンもサマーセールも適用しない | 変更なし | `小計×80%`。10,000円なら8,000円 |
...
| 要求ID7（サマーセール5%） | Regularへサマーセール5%を追加し、キャンペーン適用後の金額へ続けて適用する | 追加 | 単独は95%、キャンペーン併用は90%後に95%。Premiumは対象外 |
...
この章では、商品ごとの単価を合算して割引前の小計を作る流れが**幹候補**、会員制度や販促施策によって条件・計算方法が変わる各割引が**変化点候補**に見えます。これはフェーズ1の要求と現状コードを重ねて得た見立てであり、まだ分離方針ではありません。
...
>
> 確認結果は「ある」です。小計と支払額の計算と、割引条件の判定・金額計算は別々の理由で変わりますが、`calculate()` に集まっています。
...
**分けると、割引ルールは施策ごとに1つずつ、複数の実体になります。** そのうち**どれを使うかを誰が決めるのか**は、契約の形と実体の作り方を踏まえて判断します。フェーズ6で、契約・生成・所有と一緒に決めます。
...
フェーズ5で残した問いを、ここで判断します。**分けた割引ルールは施策ごとに1つずつあります。そのうちどれを使うかを、誰が決めるのか。** 現状では、その判断が `if-else` の並び順に埋まっていました。ルールを外へ出せば、並びの知識も一緒に外へ出ます。置き場所の候補は三つです。
...
本章で組み立てた「ルール差し替え構造」を、章題に掲げた **Strategyパターン**の抽象的な骨格と照合します。完成コードを型へ当てはめて採用を正当化するのではなく、どの関係が対応し、どの条件では別の設計が適するかを確認します。
