...
二つの依頼はどちらも `PaymentCalculator` の「割引の可否と割引率を決める」へ届いています。**同じ箱に2本届いていること自体は、同居ではありません。** 見るのは枠の中です。`PaymentCalculator` の枠には、今回書き換えない「商品単価を合算して小計を出す」も並んでいて、この二つは違うものを見ています。**この図の箱と矢印ラベルが、そのままフェーズ4で作る表になります。**
...
接続情報は、受け取る3つと返す1つです。本文を読むと、**前半と後半で見ているものが違います。** 前半は `order.items` を回して単価を合計し、後半は `memberType` と `context.isCampaignActive` を見て割引率を決めています。同じ関数の中に、違うものを見ている仕事が2つあるということです。

| そのクラスがしている仕事 | コードのどこにあるか | 見ている接続情報 | 今回書き換えたか |
|---|---|---|---|
| 商品単価を合算して小計を出す | `calculate()` の前半 | 注文の商品リスト | いいえ |
| 割引の可否と割引率を決める | `calculate()` の `if-else` | 会員種別、施策の状態 | はい |

「はい」の仕事は、将来も書き換わります。リスクID2（パーセント引き以外の計算式）が届けば、同じ `if-else` を開くことになります。

読み方は二つです。

1. **2つの仕事が違う接続情報を見ていて、片方に「はい」が付いています。** これが混在です。割引を直すために、同じ `calculate()` にある合計の処理まで読み直し、壊していないかを確認することになります。合計の仕事は今回変わりませんが、同居しているので巻き込まれます。
2. **割引を決める仕事が、`CampaignContext` と `calculate()` の2か所に散っています。** 施策を1つ足すと、入力のフラグと判定の分岐を両方開くことになります。この仕事には、まだ専用の置き場所がありません。

**表の左の列が、そのまま責任です。** 別の表へ変換しません。この章では、**小計と支払額の計算**と、**割引条件の判定・金額計算**の二つになります。
