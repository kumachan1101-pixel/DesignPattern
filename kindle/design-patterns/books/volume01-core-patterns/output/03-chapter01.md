...
> [!NOTE] この動作例は、第0章で説明した模擬環境で動きます。`CustomerDatabase db;` はクラス定義ではなく、オブジェクトの生成です。このときコンストラクタが顧客データをメモリへ登録します。`main()` は注文と開催中の施策を渡し、検索・計算・表示を直接呼びます。
>
> **実システムと掲載コードの違い**
>
> 実際のECサイトでは、顧客情報はデータベースに保存され、計算結果は画面に表示され、注文は購入フォームから届きます。掲載コードでは、そこを次の3つが代わりに務めます。**この先に出てくる `db`・`renderer`・`Order` は、どれもその代役です。**
>
> | 実システムでの動き | 掲載コードでの表現 |
> |---|---|
> | 顧客情報をDBから取得 | `CustomerDatabase` が固定の登録データを返す |
> | 金額を画面・帳票へ表示 | `CheckoutResultRenderer` が標準出力へ出す |
> | 注文を購入フォームで受け取る | `main()` が `Order` を組み立てる |
>
> 代役にしたぶん、DB接続と障害時の復旧、UI・帳票の生成、Web/APIの入力処理は掲載コードにありません。顧客情報の取得そのものが失敗する状況も起きません。**省いた処理が存在しないという意味ではありません。** 割引条件と計算式の変更影響だけを追うために落としています。
...
**出力の「佐藤 花子」と「Regular」は、`main()` が渡した値ではありません。** `main()` が渡したのは顧客ID `"C002"` だけです。氏名と会員種別は、準備の1行目で作った顧客台帳（`CustomerDatabase`）から、このIDをキーに引いた値です。今回引いたのは、次の1行です。
...
この表は、いまの並びを一覧にしただけです。どの責任が本当に一つにまとまっているかは、変更を当てるフェーズ3以降で確かめます。まずは、次の図でクラス同士の依存関係を見ます。注文確定を担う`OrderProcessor`と事前表示を担う`CartPreviewService`は、どちらも`CustomerDatabase`から会員種別を取得し、同じ`PaymentCalculator`へ渡します。これにより、プレビューの呼び出し元が会員種別を顧客IDとは別に指定して食い違うことを防ぎます。
...
以下の受入条件にある金額は、単価10,000円のワイヤレスイヤホンを1点購入した場合の例です。

| 要求ID | 変更後の要求 | 現行からの変化 | 受入条件 |
|---|---|---|---|
| 要求ID1（小計から支払金額） | 商品リストの小計から最終支払金額を計算する | 変更なし | 3,000円と1,000円の2商品を合算し、小計4,000円を算出する |
| 要求ID2（Premium20%） | Premium会員へ20%割引を適用し、他割引と併用しない | 変更なし | セール中でも8,000円になる |
| 要求ID3（Regularキャンペーン10%） | Regular会員はキャンペーン中だけ10%割引する | 変更なし | キャンペーンだけなら9,000円になる |
| 要求ID4（未登録IDを拒否） | 未登録の顧客IDでは注文を受け付けず、その旨を表示する | 変更なし | 登録・未登録の既存動作を維持する |
| 要求ID5（購入結果の表示） | 計算結果を注文確定時の購入結果として表示する | 変更なし（表示する金額の値だけ変わる） | 顧客・会員種別・キャンペーン状態・小計・支払金額を表示する |
| 要求ID6（プレビュー） | 注文確定前に同じ条件の支払金額をプレビューする | 変更なし | 購入結果と同じ金額を返す |
| 要求ID7（サマーセール5%） | Regularへサマーセール5%を追加し、既存割引後へ逐次適用する | 追加 | セールのみ9,500円、併用8,550円、Premiumは対象外になる |
...
graph TD
    ...
    V["CartPreviewService<br>（利用側）"]:::keep
...
**この差が問題です。** 足すだけのはずが、動いていた既存の判定へ手が入り、触っていない処理まで確認対象になりました。上の三つの読み方のうち、一つめと二つめがそれにあたります（三つめは、フェーズ4でまとめる根拠に使います）。
...
|---|---|---|
| `PaymentCalculator` | 既存の割引判定（色つき） | 施策の状態／会員種別と施策の状態 |
| `PaymentCalculator` | 商品単価を合算して小計を出す | なし |
| `CampaignContext` | 開催中の施策の持ち方（色つき） | 施策の状態 |
| （枠の外） | `main()` | 入力の組み立て |

手順は5つです。

1. **矢印のラベルを集めます。** 右の列にあるのは「施策の状態」「会員種別と施策の状態」「入力の組み立て」の3種類です。フェーズ3では**依頼が動かすもの**として書きました。ここからは、**その場所が見ているもの**として読み直します。これを接続情報と呼びます。
2. **`main()` の行を外します。** `main()` は入力を組み立てるだけで、接続情報を見て動く場所ではありません。残るのは2種類です。
3. **複合のラベルを分けます。** 「会員種別と施策の状態」は2つなので、**会員種別**と**施策の状態**に分かれます。重複をまとめると、接続情報は2つです。
4. **ラベルごとに、同じ行の箱を書きます。** 施策の状態は「既存の割引判定」と「開催中の施策の持ち方」の2つへ、会員種別は「既存の割引判定」へ届いています。コードで言えば、既存の割引判定は `calculate()` の `if-else`、開催中の施策の持ち方は `CampaignContext` のフラグです。
5. **色の付いていない箱も行にします。** 「商品単価を合算して小計を出す」です。矢印は届いていませんが、同じ枠にあります。これが見ているのは引数の注文なので、**注文の商品リスト**の行になります。

3行になります。

| 接続情報 | それを見て動く場所 |
|---|---|
| 注文の商品リスト | `calculate()` の前半（単価の合算） |
| 会員種別 | `calculate()` の `if-else`（プレミアムの判定） |
| 施策の状態 | `calculate()` の `if-else`（施策の判定）／`CampaignContext` のフラグ |

右の列がコードのどこなのかを、変更を試したあとの `calculate()` で見ます。
...
引数の3つが、そのまま表の3行です。`for` の中が**注文の商品リスト**、`if-else` の並びが**会員種別**と**施策の状態**にあたります。`CampaignContext` のフラグは、`context.isSummerSale` と `context.isCampaignActive` として読まれている側です。
...
**原因ID1（小計と支払額の計算と割引条件の判定・金額計算が同じクラスにある）。** 割引ごとの条件と計算が、小計を作って返す処理の中にあります。新しい割引を足せる場所が既存割引と同じ `if-else` の中しかないため、既存の枝まで開いて直すことになりました。この配置が問題ID1（既存割引まで修正）を生みました。
...
黄色は既存の小計と支払額の計算、青は外へ分ける責任です。①が次の課題表に対応します。
...
**解く原因：** 原因ID1（小計と支払額の計算と割引条件の判定・金額計算が同じクラスにある）

**構造の変更：** 目標図の①のとおり、割引条件の判定・金額計算を小計と支払額の計算から分けます。

**接続：** 小計と支払額の計算から会員種別・施策状態・適用前金額を渡し、割引条件の判定・金額計算から適用可否・適用後金額を返します。
...
フェーズ5では、割引条件の判定・金額計算と適用割引の優先順・選択を小計と支払額の計算から分けるところまで決めました。ここからは、割引条件の判定・金額計算の契約、施策ごとの具体、競合順を持つ場所、実体の生成・所有、二つの利用側への受け渡し、実行の順にC++の構造へ変えます。
...
classDiagram
    ...
    OrderCalculation --> ConflictPolicy : 選択を依頼して割引ルールを受け取る
...
図の「小計と支払額の計算」「割引条件の判定・金額計算」「適用割引の優先順・選択」はフェーズ5と同じ責任名です。「小計と支払額の計算」の箱には二つのクラスを並べています。注文確定の流れを進める `OrderProcessor` が選択を依頼し、`PaymentCalculator` が小計と支払額を計算します。どちらも割引の具体を知りません。ここから、各責任を表す契約・操作・所有・受け渡しを部分クラス図と対応コードで確定します。すべてを接続した完成クラス図はフェーズ7で確認します。
...
リスクID1（割引ルールが毎月増える）・リスクID2（割引の中身が変わる）は、採用構造を評価する根拠の一部です。ただし今回実装する機能は変更後要求ベースラインの要求IDで確定した範囲だけであり、リスクIDは構造が将来の変化を局所化できるかの評価に使います。
