...
**1つ目：既存ルールまで修正・確認範囲になること。** 入力と新しい割引を追加するだけでなく、`PaymentCalculator`にある既存のPremium・キャンペーン分岐の条件と順序を開いて書き換え、その計算を使う二つの利用経路も回帰確認しました。
...
---
> **📌 問題（確定）**
> 変更ID1（サマーセール追加）・変更ID2（逐次割引）を適用すると、既存のPremium・キャンペーン分岐がある`PaymentCalculator`を修正し、その計算を使う注文確定と`CartPreviewService`を回帰確認する必要があった。入力と新規ルールの追加だけに収まらず、既存ルールの条件・計算順序まで変更対象になった。
---
...
|---|---|---|
| 問題ID1（修正範囲） | 新規ルールの追加だけでなく、既存割引がある計算本体を修正し、既存経路も再確認した | 変更ID1（サマーセール追加） |
...
フェーズ3から受け取るのは、問題ID1（修正範囲）・問題ID2（解読範囲）・問題ID3（壊しやすさ）と、変更途中の `PaymentCalculator::calculate()` です。ここでは対策を決めず、三つの問題が生まれた理由だけを、責任と変更理由から確定します。

### 4-1：痛んだ場所の処理・判断を並べる

**1つ目：痛んだ場所を1つ選び、そこにある処理を並べて書き出します。** 三つの問題で共通して開いた場所は `PaymentCalculator::calculate()` です。メソッド名や行数ではなく、コードが行っている処理・判断の単位で分けます。

| 分析対象を選ぶ根拠 | 原因分析で開くコード箇所 |
|---|---|
| 問題ID1（修正範囲）・問題ID2（解読範囲）・問題ID3（壊しやすさ） | `PaymentCalculator::calculate()` |

問題IDは、この場所を原因分析の対象に選ぶ根拠です。以下の処理がすべて悪いというラベルではありません。変更要求のために直接変えた処理と、同じメソッドにあるため巻き込まれて確認した処理を分けて見ます。

| コード上の目印 | そこで行っている処理・判断 | 変更試行で起きたこと |
|---|---|---|
| `for (const auto& item : order.items)` | 商品の金額を合算する | 変更なし。同じ `calculate()` を修正したため巻き込まれて回帰確認した |
| `memberType == "Premium"` | Premiumの優先と他施策の適用外を判断する | 分岐自体は維持し、新規条件との前後関係を直接確認した |
| `context.isSummerSale` | サマーセールの適用可否を判断する | 変更要求のため新規追加した |
| `total = total * 95 / 100` | サマーセールの割引額を計算する | 変更要求のため新規追加した |
| `isSummerSale && isCampaignActive` を単独条件より前に置く | 複数施策が重なるときの順序を決める | 既存分岐を読み直し、変更要求のため分岐の並びを変えた |
| `return total` | 最終金額を呼び出し側へ返す | 変更なし。同じ `calculate()` を修正したため巻き込まれて回帰確認した |

### 4-2：各責任が変わるきっかけを分ける

**2つ目：並べた処理・判断ごとに、「これが変わるのはどんなときか」を書きます。** 「どこを直したか」ではなく、業務上の変更のきっかけを確認します。ここでの変える／守るは今回の変更に対する区別であり、将来ずっと変わる／変わらないという意味ではありません。

| 処理・判断が担う責任 | 変わるきっかけ | 今回の位置づけ |
|---|---|---|
| 商品の金額を合算する | 注文明細や価格の扱いが変わる | **守る** |
| 各施策を適用できるか判断する | 販促施策の対象条件が変わる | **変える** |
| 一致した施策の割引額を計算する | 販促施策の割引内容が変わる | **変える** |
| 複数施策の順序と排他を決める | 施策どうしの競合方針が変わる | **変える** |
| 最終金額を返す | 注文計算の利用契約が変わる | **守る** |

適用条件と計算式は、同じ販促施策の変更で一緒に変わるため、後で同じ単位へまとめるべき責任です。一方、競合時の順序は複数施策を横断する方針であり、個別施策とは変更理由が異なります。小計計算と結果返却は、今回の販促施策追加では変えていません。
...
コードへ戻ると、小計計算の間に個別施策の条件・式・競合順が直接置かれています。そのため、販促施策を変えたいだけでも、注文金額を合算して返す流れを開いて確認しなければなりません。

原因IDは、条件と式を別の施策だと考えて分けるものではありません。**同じ施策の責任が、選択の流れと金額計算の流れという二つの守りたい場所へ入り込んでいる**ため、残存確認を二つに分けます。

| 原因ID | 異なる理由で変わる責任の同居 | 対応する問題ID |
|---|---|---|
| 原因ID1（選択が施策条件を抱える） | 選択の流れが、個別施策の条件と競合時の順序を直接持つ | 問題ID1・問題ID2・問題ID3 |
| 原因ID2（計算骨格が計算式を抱える） | 小計を作って返す流れが、個別施策の計算式を直接持つ | 問題ID1 |

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 答えは「ある」です。注文計算、個別の販促施策、施策どうしの競合方針は別々の理由で変わりますが、`calculate()` の分岐へ集まっています。

フェーズ4で確定したのは、原因ID1（選択が施策条件を抱える）・原因ID2（計算骨格が計算式を抱える）と、それぞれで同居している責任です。まだクラス、インターフェース、操作名は決めていません。次のフェーズでは、この二つの原因から境界候補を導きます。

---

## 🟡 フェーズ5：課題定義 ―― 原因から課題を検討して確定する

入力は、フェーズ4で確定した原因ID1（選択が施策条件を抱える）と原因ID2（計算骨格が計算式を抱える）です。まず原因を「どちらを変え、どちらを開かずに済ませたいか」へ言い換えます。

### 5-1：原因から境界候補を導く

**1つ目：変える側と守る側の間に線を引きます。** 原因ID1（選択が施策条件を抱える）・原因ID2（計算骨格が計算式を抱える）にある個別施策の判断と、選択・小計計算の流れを分けます。

**2つ目：線を越えて本当に必要な共通部分を探します。** 個別施策を横に並べ、選択と計算に必要な依頼・入力・結果だけを次の表へ残します。

| 元の原因 | 変える側 | 守る側 | 境界を越える必要がある業務情報 |
|---|---|---|---|
| 原因ID1（選択が施策条件を抱える） | 個別施策の対象条件、施策どうしの競合順 | 施策を順に調べる選択の流れ | 会員種別と施策の開催状態を渡し、適用できるかを受け取る |
| 原因ID2（計算骨格が計算式を抱える） | 個別施策の割引計算式 | 小計を作って最終金額を返す流れ | 適用前金額を渡し、適用後金額を受け取る |

同じ施策理由で変わる対象条件と計算式は、分断せず一つの施策単位にまとめます。複数施策をどう並べるかは横断的な競合方針なので、その単位の外に残します。ここで確定したのは必要な業務情報までであり、型名や操作名はまだ決めません。

### 5-2：部分対応で原因が残らないか確認する

**3つ目：部分的な切り出しで原因が残らないかを、システム全体で確かめます。**

条件だけを外へ出すと原因ID2（計算骨格が計算式を抱える）が残り、式だけを外へ出すと原因ID1（選択が施策条件を抱える）が残ります。`if-else` を補助関数へ移すだけでも、選択の流れが具体的な施策名・条件・順序を知る状態は変わりません。いずれも完成候補ではありません。

必要なのは、同じ施策単位に条件と式を置き、選択と計算の流れが全施策を同じ形で扱える一つの完成構造です。責任配置の異なる別の完成構造は成立していないため、手段の比較は行いません。課題を二つに分けるのは、一つの完成構造から原因ID1（選択が施策条件を抱える）と原因ID2（計算骨格が計算式を抱える）が両方消えたことを別々に確認するためです。

### 5-3：課題ID・接続点・完了条件を確定する

| 課題ID | 元の原因ID | 変える側 | 守る側 |
|---|---|---|---|
| 課題ID1（選択条件の境界） | 原因ID1（選択が施策条件を抱える） | 個別施策の条件と競合順 | 共通の選択の流れ |
| 課題ID2（計算式の境界） | 原因ID2（計算骨格が計算式を抱える） | 個別施策の計算式 | 小計計算と結果返却 |

| 課題ID | 接続点で流す業務情報 |
|---|---|
| 課題ID1（選択条件の境界） | 会員種別・施策状態を渡し、適用可否を受け取る |
| 課題ID2（計算式の境界） | 適用前金額を渡し、適用後金額を受け取る |

#### 課題ID1（選択条件の境界）の完了条件

- 選択処理に、具体的な施策名と個別条件式がない
- 選択処理が、すべての施策へ同じ依頼を行う
- 競合順を変えるとき、個別施策の条件式を修正しない

#### 課題ID2（計算式の境界）の完了条件

- `PaymentCalculator::calculate()` に、個別施策の割引率と計算式がない
- 選択された施策を、同じ金額変換として適用する

**二つとも満たした完成状態**では、価格計算は個別施策の条件も式も知りません。同じ施策の条件と式は一緒に変更でき、競合方針は個別施策の条件式を変えずに変更できます。

| 問題ID（フェーズ3の痛み） | 原因ID（フェーズ4の構造原因） | 課題ID（フェーズ5の達成目標） |
|---|---|---|
| 問題ID1・問題ID2・問題ID3：施策追加で選択条件と順序を解読・修正し、壊しやすい | 原因ID1（選択が施策条件を抱える） | 課題ID1（選択条件の境界） |
| 問題ID1：施策追加で計算式も同時修正する | 原因ID2（計算骨格が計算式を抱える） | 課題ID2（計算式の境界） |

## 🔴 フェーズ6：対策検討 ―― 構想を一つのコード経路へ変える

> **フェーズ6の確認観点：** 「契約と具体」「生成・所有」「選択・登録」「受け渡し」「実行」が、課題ID1（選択条件）・課題ID2（計算式）を同時に解く一つの経路としてつながるかを確認します。

### 構想を決める

課題ID1（選択条件）と課題ID2（計算式）を、**具体ルールの追加と登録だけで施策を増やせる一つの経路**へまとめます。
...
増えるのは条件と式で、小計計算と結果返却は変わりません。課題ID1（選択条件の境界）の条件には会員種別と施策状態を渡し、競合順は個別ルールではなく組み立て側に残します。
...
課題ID2（計算式の境界）では、小計を重複計算させず、表示にも依存させないため、適用前金額を渡して適用後金額だけを受け取ります。
...
`matches()` が課題ID1（選択条件の境界）、`apply()` が課題ID2（計算式の境界）です。分離単位はいずれも施策なので、一つの契約に置けます。実装で成立を確認します。
...
完成構造は一つです。選択と計算を再び一クラスへまとめる形は二つの変更理由を分けられない途中状態なので、完成案として比較しません。課題とリスクだけを確認し、統合図はフェーズ7で示します。
...
変更前は、`PaymentCalculator`の条件分岐が「どの割引を使うか」と「どう計算するか」を同時に抱えていました。条件を`matches()`、式を`apply()`として施策クラスへ移し、競合順を組み立てへ分けたことで、選択側と計算側は具体ルールを知らずに実行できます。
