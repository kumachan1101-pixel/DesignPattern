...
| 図中の文字 | 見え方 | 意味 |
|---|---|---|
| `［新規］`／`<<new>>` | **青のぬりつぶし** | 今回新しく作るもの |
| `［変更］`／`<<changed>>` | **青い枠**（中は白） | もとからあって、開いて直すもの |
| 差分ラベルなし | 色なし | 触らないもの |

たとえば、変更前の構造では `［変更］` と枠ばかり、完成後の構造では `［新規］` とぬりつぶし中心、という形で図が変わります。**枠が減っていれば、開くところが減ったということです。** 端末や印刷で色が分かりにくくても、文字ラベルだけで差分を読めます。
...
>
> **成果物：** 主要クラスと接続順を示す構想、契約・具体の選択と生成・受け渡し・実行の要点コード、課題と構想の対応、将来リスクの評価
...
境界の両側が守る契約を決めたら、その契約を実現する具体を用意します。次に、構成を知ってよい場所が具体を選んで生成し、利用側へ渡します。利用側は受け取った契約だけを呼び、具体の種類や生成方法を知らずに実行します。本書が各章で追う一列は、**契約 → 具体を選択して生成 → 受け渡し → 実行**です。具体クラスの定義は「選択して生成」できるようにする準備として、契約の直後に代表一つを確認します。
...
| 選択・生成・所有 | どの具体を誰が選んで生成し、いつまで保持するか |
| 登録 | 複数を使う場合、どの生成済み実体をどの順で加えるか |
...
構想を書いたら、そのまま順にコードでつなぎます。契約 → 代表的な具体一つ → 具体を選択して生成 → 受け渡し → 実行骨格 → 公開入口、の順です。**全部のクラスを並べる必要はありません。** ここで確かめたいのは「この構想で生成から実行までつながるか」だけなので、代表が一つ通れば構想は成立します。残りの具体と全体コードは、フェーズ7で見ます。
...
この章で持ち帰っていただきたいのは、パターン名の一覧ではありません。変更が届いたとき、現状の仕様とコードへ実際に当て、痛みを観測し、原因から課題を確定して、契約から具体の選択・生成、受け渡し、実行までを一つの構想へまとめ、要点コードで成立を確かめるという流れです。
