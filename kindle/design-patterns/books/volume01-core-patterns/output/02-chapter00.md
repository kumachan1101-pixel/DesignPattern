...
ここで問題IDを使うのは、**どのコードを原因分析で開くかを選ぶため**です。その中の処理がすべて問題という意味ではありません。変更要求のため直接追加・変更した処理と、同じ場所にあるため巻き込まれて読み直したり回帰確認したりした処理を分けます。表にするときは、`if (status == ...)` や `return total` のような短いコード上の目印を添えると、どの処理を指すか迷いません。
...
**分けて考えるべき責任の組が見つかると、次に見るものが決まります。** このフェーズで確定するのは「何と何が、別の理由で変わるか」までです。どこに線を引き、両側をどう接続するかは、フェーズ5で決めます。
