...
| フェーズ4の原因ID・責任ごとの変更前後・同居の証拠 | フェーズ5で、原因をなくす責任配置と接続点を課題へ変える |
...
本書でいう**責任**は、同じ業務上の理由で一緒に変わる仕事のまとまりです。フィールドやメソッドごとには分けません。たとえば、ある業務ルールの適用条件と結果の計算が同じ方針変更で一緒に変わるなら、まとめて「業務ルール」という一つの責任として扱います。
...
2. **責任ごとに変更前後を比べる。** 責任名は「販促方針」「注文計算」のように、実務で通じ、実現方法が変わっても使える名前にします。その責任が担うルールを変更前と変更後で並べ、今回どこが変化したかを見ます。
3. **問題が起きた理由を確定する。** 変化した責任と変化していない責任が同じ場所にあるか、一方が他方の具体を直接知っているかをコードで確かめ、原因IDを付けます。

2つ目で分かるのは、責任ごとの変化までです。3つ目でコードへ戻って初めて、「変更理由の異なる責任が同じ場所にあるため、変化していない側まで巻き込まれた」と原因を確定できます。
