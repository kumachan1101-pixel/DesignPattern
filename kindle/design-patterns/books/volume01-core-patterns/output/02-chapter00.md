...
記録するときは、**理想と現状の差**の形で書きます。

| 書くこと | 内容 |
|---|---|
| 理想 | この依頼なら、本来ここだけを書けば済むはず |
| 現状 | 実際に書き換えたところと、確認が要ったところ |
| その差 | 依頼が求めていないのに手が入った部分＝問題 |

理想を先に書いておくと、**どこまでが当然の修正で、どこからが痛みか**を、誰が見ても同じ基準で判断できます。
...
>
> **成果物：** 接続情報と、それを見て動く場所の一覧、責任の一覧、問題が起きた理由を示す原因ID一覧
...
>
> 掃除のやり方は掃除の手順表を、知らせる先は連絡先の一覧を見ています。**見ているものが違うのに、一人が抱えている。** これがコードで起きているのが、この後に出てくる**責任の混在**です。
...
問題が起きたコードと変更影響は、フェーズ3ですでに特定しています。フェーズ4では、その結果へ接続情報の軸を足し、次の順で分析します。

1. **調べるクラスを決めます。** 変更影響グラフで色の付いた箱を含むクラスです。届いた矢印が1本でも構いません。実体を組み立てる `main()` は、責任を持つ場所ではないので調べません（フェーズ6で扱います）。
2. **接続情報を行にします。** 変更影響グラフの矢印ラベルが、そのまま行になります。そのクラスの公開操作・引数・戻り値・保持するメンバーのうち、今回の依頼で動いたものです。今回動かなかったものも、同じクラスの中にあるなら行に入れます。巻き込みがそこで読めます。
3. **その接続情報を見て動く場所を、行の中に並べます。** クラスをまたいでも構いません。`クラス名::メソッド名` のどこで何をしているか、まで書きます。
4. **行ごとに、今回書き換えたかを書きます。** 今回の変更で実際に書き換えた、または壊れていないか確認したなら「はい」。今回は触っていないが、ヒアリングで確認した理由が届いたら書き換えるなら「将来」。どちらでもなければ「いいえ」。**観測した事実だけで書きます。** 量は問いません。
5. **読みます。** ①**同じ行に場所が2つ以上並んでいる**なら、それらは同じものを見ているので必ず一緒に変わります。**一つへまとめる対象**です。②**違う行の場所が同じクラスの中にあり、そのうち1つでも「はい」がある**なら、**混在**です。分ける対象はここです。③**「いいえ」だけの行**は今回変わりませんが、混在したクラスの中にあるため、そのクラスへ手を入れるたびに読み直すことになります（巻き込み）。
6. **責任名を付け、原因IDと対策前図を置きます。** **行がそのまま責任です。** 「その接続情報を見て、何を判断・計算・保持・選択・更新するのか」で名前を付けます。責任を箱にして、いまどれが同じクラスへ同居しているかを1枚にし、直接変わった責任と、同居のせいで巻き込まれた責任を図の直後で説明します。ここで決めるのは「どの同居が、どの問題IDを生んだか」までです。**どこへ置き直すかはフェーズ5**で決めます。
7. **今回触らないクラスを1行残します。** 違うものを見ている場所を複数抱えていても、今回の変更でも、ヒアリングで確認した理由でも手が入らないクラスは、直す対象にしません。**抱えていること自体は、直す理由になりません。** 記録して次の依頼を待ちます。

**「今回書き換えたか」は、分ける根拠です。** 違うものを見ている場所が同居していても、どれも今回変わっていないなら、それは**まだ痛んでいない**だけです。本書は、実際に痛んだことを観測してから分けます。「はい」が一つもない同居は、記録して次の依頼を待ちます。

#### 誰が作っても同じ表になるようにする

表がぶれるのは、行の切り方と印の付け方が人によって変わるからです。次の三つを規則にすると、同じコードと同じヒアリング結果からは同じ表が出ます。

**行の切り方。** **接続情報が変わるところで切ります。** 一つの関数でも、前半と後半で違う引数を見ているなら別の行です。1メソッド＝1行にはしません。二つの接続情報を同じ行にまとめてよいかは、**一つながりの分岐で一緒に使われているか**で決めます。目の前のコードだけを見て決め、**将来こういう依頼が来るかもしれない、という想像は使いません。**

**接続情報の書き方。** その仕事の中に名前が出てくるものだけを書きます。引数、保持するメンバー、呼ぶ相手の名前です。言語や標準ライブラリは、差し替えを考える相手ではないので数えません。

**今回書き換えたかの書き方。** 「はい」は観測した事実、「将来」はヒアリングで確認した理由に限ります。「たぶんここも直すことになりそう」は入れません。

二人が別々に書いて、行の呼び名が違っても構いません。**数と分け目が一致すれば、同じ表です。** 呼び名の一致まで条件にすると、かえって使われなくなります。

#### 変更が来る前に読めること

この表は、変更が来る前でも作れます。コードを読むだけだからです。違う接続情報を見ている場所を同じクラスに抱えているものを並べれば、**変更依頼を待たずに棚卸しができます。**

ただし、それだけでは直す根拠になりません。**いま何を抱えているかは読めますが、それが今後変わるかは読めません。** 変わらなければ痛みは起きません。直す根拠になるのは、変更を当てて観測した「はい」か、ヒアリングで確認した「将来」だけです。

表に行が複数並ぶだけでも、直ちに問題とは言えません。フェーズ3の変更影響と突き合わせ、「違うものを見ている場所が同じクラスにあるため、別の場所まで修正・確認対象になった」と言えたとき、初めて原因として確定できます。手順6の図（以下、対策前図）が、フェーズ5で責任配置を変える前の比較元になります。
