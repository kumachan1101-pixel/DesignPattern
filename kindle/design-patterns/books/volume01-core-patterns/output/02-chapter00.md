...
そこでこのフェーズでは、変更IDを**一つずつ追跡します。** 複数の変更IDが同じ実行経路で一緒に成立する場合も、どの変更IDがどの修正を生んだかは分けて残します。
...
原因を突き止めていれば、完了条件の**候補**はそこから導けます。ただし、原因の文を反転しただけでは確定しません。ほかの原因を残さないか、既存要求を失わないか、導入コストに見合うかをシステム全体で確かめてから課題として確定します。完了条件がうまく書けないときは、原因の言語化か境界候補の検討が足りていない合図です。
