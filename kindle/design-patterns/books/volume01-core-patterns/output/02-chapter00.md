...
| フェーズ4の原因ID・変える責任と守る責任が同じ場所にある証拠 | フェーズ5で、どこに境界を引くかを検討して課題へ変える |
...
2. **責任を「変える」と「守る」に分ける。** 責任名は「業務ルール」「結果の計算」のように、実務で通じる単位にします。
3. **問題が起きた理由を確定する。** 変える責任と守る責任が同じ場所にあるか、守る側が変える側の具体を直接知っているかをコードで確かめ、原因IDを付けます。

2つ目で分かるのは分類までです。3つ目でコードへ戻って初めて、「同じ場所にあるから巻き込まれた」と原因を確定できます。
...
1. **原因をなくす分け方を決める。** 変える責任を、守る責任から分けます。同じ理由で変わる仕事は細かく分けません。
2. **原因が残らないか確認する。** 一部を関数へ移しただけで具体名や判断が残るなら、まだ途中です。分けた処理が実行時につながるかも確認します。
3. **課題と完了条件を確定する。** 元の原因、分ける責任、接続点で渡す情報、コードで確認できる完了条件を対応づけます。
