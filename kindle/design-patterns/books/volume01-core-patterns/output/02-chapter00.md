...
**一段ごとに前の段で確定した事実を使うため、調査や説明の重複を減らし、判断根拠を置き去りにせず、要求から実装確認までを体系的に追えます。**

7つが何を受け取って何を次へ渡すのかは、この章の「7つのフェーズ」で図にしています。各章の見出しには、フェーズごとに色を変えた丸印が付くので、いま自分がどのフェーズにいるかも見て分かります。

### 7フェーズで、構造の見え方がどう変わるか
★これ必要か？後述で、フェーズ別の説明があります。

7フェーズは、決められた書類を作る手順ではありません。現状の事実から、変更に耐える構造と、その実装結果までを一段ずつ具体化する考え方です。

```mermaid
flowchart TB
    CURRENT["現状の構造<br>幹候補と変化点候補が見える"]
    IMPACT["変更を当てた構造<br>直接修正と巻き込みが見える"]
    CAUSE["原因の構造<br>混在する責任と依存が見える"]
    TARGET["目標の責任配置<br>分ける責任と接続点が見える"]
    RUNTIME["動くコード構造<br>契約・具体・生成・所有・受け渡し"]
    VERIFIED["確認済みの完成構造<br>要求・完了条件・影響縮小の証拠"]

    CURRENT --> IMPACT --> CAUSE --> TARGET --> RUNTIME --> VERIFIED

    classDef current fill:#dbeafe,stroke:#1565c0,color:#172033;
    classDef analysis fill:#ffe6cc,stroke:#ef6c00,color:#172033;
    classDef target fill:#fbf0c4,stroke:#c9a227,color:#172033;
    classDef runtime fill:#ffd8d8,stroke:#c62828,color:#172033;
    classDef verified fill:#d6f0d8,stroke:#2e7d32,color:#172033;
    class CURRENT current;
    class IMPACT,CAUSE analysis;
    class TARGET target;
    class RUNTIME runtime;
    class VERIFIED verified;
```

現状を把握せずに原因を決めると、問題ではない箇所まで分離しかねません。原因を確定せずに完成形を選ぶと、その構造が効く理由を説明できません。図の各段階を飛ばさず、前段で見えた構造を次段で確かめ直します。
...
| 番号    | 付ける場所 | この番号で追うもの               |
| ----- | ----- | ----------------------- |
| 要求ID  | フェーズ1 | システムが満たす要求と、最終的な受入・回帰結果 |
| 変更ID  | フェーズ1 | 今回の変更依頼、変更後要求、変更影響      |
| リスクID | フェーズ2 | 将来変わる可能性と、設計での備え        |
| 問題ID  | フェーズ3 | 変更を試して観測した修正・再テストの広がり   |
| 原因ID  | フェーズ4 | 痛みを生んだ責任の集まり方           |
| 課題ID  | フェーズ5 | 解くべき構造上の課題から、コードと改善結果まで |
...
設計課題を導く中心線は、**変更ID → 問題ID → 原因ID → 課題ID**です。要求IDは変更後も守る機能、リスクIDは変化への備えとして、この中心線とは別の目的で使います。
...
したがって、本書の動作例は次の流れで読んでください。

1. `SampleDataStore store;` のような生成行で、サンプルデータを持つオブジェクトを作る。
2. `main()` からID・数量・操作などを渡し、利用者や外部イベントの入力を模擬する。
3. 処理クラスがメモリ上のデータを検索・更新し、業務ルールを実行する。
4. 画面表示や外部送信の代わりに、実行内容と結果を標準出力で確認する。
...
**掲載コードを1つの `.cpp` にまとめているのは、手元ですぐ動かせるようにするためです。** 実務でこう書くという意味ではありません。実際にはクラスごとに `.h` と `.cpp` へ分けます。**どう分けるかも設計判断の一つ**なので、各章の完成コードのあとに、その章の構造ならどのファイル構成になるかを表で示します。読んでいる途中で「これを1ファイルに書くのか」と気になった場合は、その表で答え合わせができます。
...
各章は、次の7つのフェーズを上から下へ進みます。箱の下段は、そのフェーズで確定して次へ渡す判断です。
...
矢印は単なる章順ではありません。前のフェーズで確定した言葉を、次の判断で実際に使います。問題IDは観測した影響を原因へつなぎ、原因IDは責任の目標配置へつなぎ、課題IDと完了条件は完成コードの合否判定へつなぎます。
...
**7つのフェーズは、同じ説明を7回繰り返すための章立てではありません。** 現状の全体コードはフェーズ1、完成した全体コードはフェーズ7だけに置きます。フェーズ3は変更で手が入る箇所、フェーズ4は原因が見える箇所、フェーズ6は構想を成立させる箇所だけを抜粋します。すでに確定した仕様・表・図・コードは、後のフェーズでは必要な行を参照し、同じ内容を丸ごと再掲しません。
...
現状把握では、一つの資料だけを正解にしません。四つの入口を、同じ動作へ重ねます。
...
flowchart TB
    ...
    class REQ,RUN,CLASS,CODE,CHANGE source;
    class MATCH check;
    class BASE,DELTA result;
```

食い違いがあれば、そこで止めて確認します。`if` 文だけを読んでも区別の理由は分からず、仕様だけを読んでも実際にどこが動くかは分からないからです。小さな実行例は動作像をつかむ入口であり、そこから成功・失敗、保存される値、関係するクラスへ範囲を広げます。
...
要求、動作、構造を重ねて見ると、複数のケースに共通する処理順や役割と、特定の条件・状態・外部接続によって変わる判断が見えてきます。前者を**幹候補**、後者を**変化点候補**としてメモしておくと、後の調査で見る場所を絞れます。ただし、この段階では現状を理解した結果から得た候補にすぎません。「幹だから残す」「変化点だから分離する」とは、まだ決めません。

さらに、幹が変化点を「いつ、いくつ使うか」を見ると、構造の大まかな型も候補として浮かびます。ただし、ここで既知のパターン名を選び、その完成形から逆算してはいけません。現状から得た型の候補が本当に必要かどうかは、変更を当てた結果と原因を見て判断します。
...
> [!NOTE] フェーズ2では「責任の混在」を断定しない
> ここで決めるのは「どの仕様に変更の見込みがあり、今回どの動作を維持するか」の見立てです。どの責任を分離し、どんな境界を作るかは、変更要求を当てたときの痛みと原因を確認した後、フェーズ5で課題として定めます。

フェーズ1で見つけた幹候補と変化点候補を、ここで変更履歴や関係者の見通しに照らします。

```mermaid
flowchart LR
    FACT["フェーズ1で確認した事実<br>要求・動作・構造・コード"]
    TRUNK["幹候補<br>複数のケースに共通する流れ・役割"]
    VAR["変化点候補<br>条件・状態・外部接続で変わる判断"]
    EVIDENCE["変更履歴・業務判断・運用上の事実"]
    KEEP["今回維持する動作"]
    OUTLOOK["変更が見込まれる仕様"]
    ...
    class FACT,EVIDENCE input;
    class TRUNK,VAR candidate;
    class KEEP,OUTLOOK confirmed;
```

幹候補は今回も維持する流れ・結果か、変化点候補は何をきっかけに誰の判断で変わるかを確かめます。

ここでいう幹は永久に不変な部分ではありません。**今回の変更と、確認できた見通しに対して安定している部分**です。見通しが変われば、幹と変化点の境界も見直します。フェーズ3で変化点へ変更を当て、幹候補まで修正・再確認が広がったら、フェーズ4でその原因を調べます。フェーズ5では、その原因をなくす目標構造として幹と変化点の配置を確定します。
...
**クラス図ではなく、この形にしています。** 理由は二つです。クラス図の箱は1クラスで1つなので、**1つのメソッドの中に別々のものが同居している**という、本書がいちばん見せたい状態が消えてしまいます。もう一つは、クラス図には変更依頼を置く場所がないことです。どの依頼がどの仕事へ届いたかは、この本では矢印そのものが答えになります。
...
図にすると、原因として確かめるのは次の形です。

```mermaid
flowchart TB
    RULE_A["変更理由A<br>条件・状態・外部仕様など"]
    RULE_B["変更理由B<br>Aとは別の時点・依頼元・情報"]

    subgraph ONE["同じクラス"]
        KEEP["複数ケースを進める<br>共通の仕事"]
        CHANGE["条件ごとに変わる<br>判断・反映"]
    end

    RULE_A --> KEEP
    RULE_B --> CHANGE
    KEEP -. 同居のため再確認 .-> CHANGE
    ...
    class RULE_A,RULE_B reason;
    class KEEP stable;
    class CHANGE changing;
```

別の情報を見て、別の理由で変わる仕事が同じクラスにあるため、片方の変更で片方まで読み直す。この因果が、責任の混在を原因として確定する根拠です。
...
問題が起きたコードと変更影響は、フェーズ3ですでに特定しています。フェーズ4では、変更影響グラフで番号を付けた場所について、**何を見て動くのか、どの実装なのか、どの責任として読めるのか**を一つの表で対応させます。

見るのは、同じクラスの中に、違う情報を見て動く責任が並んでいるかです。変更された責任と、同じクラスにあるため読み直しや再確認へ巻き込まれた責任が分かれたら、その混在を原因IDとして確定します。変更されていないクラスに責任が複数あるという理由だけでは、今回の原因にしません。

最後に、確定した責任を同じクラスの枠へ置いた対策前図を作ります。この図が、フェーズ5で責任を分けた目標図との比較元になります。具体的な表の作り方とコードの読み方は、各実践章で実際の変更影響グラフを使って示します。
...
フェーズ4の対策前図を、同じ責任名のまま次の配置へ変えます。

```mermaid
flowchart LR
    subgraph BEFORE["対策前：同じクラスに同居"]
        B1["複数ケースを進める<br>共通の仕事"]
        B2["条件ごとに変わる<br>判断・反映"]
    end

    subgraph AFTER["目標：変更理由ごとに分離"]
        A1["複数ケースを進める<br>共通の仕事"]
        LINK["接続点<br>依頼・入力・結果"]
        A2["条件ごとに変わる<br>判断・反映"]
        A1 --> LINK --> A2
    end
...
この図で先に決めるのはクラス名ではなく、責任の置き場所と、分けた両側が受け渡す情報です。フェーズ6で同じ図の責任名をC++の型と操作へ置き換えます。
...
この一列を途切れさせないため、次の五点を**一つの構造として**決めます。これを本書では「構想」と呼びます。

| 構想の要点 | 決めること |
|---|---|
| 契約と具体 | フェーズ5の入力・結果を表す型と操作、その操作を実現する具体 |
...
実装して動いたら終わり、とはしません。要求と構造を、別々の根拠で確かめます。

```mermaid
flowchart TB
    DONE["完成コードと実行結果"]
    REQ["要求の確認<br>新しい要求を満たしたか<br>既存要求を壊していないか"]
    STRUCT["構造の確認<br>フェーズ5の完了条件を<br>すべて満たしたか"]
    IMPACT["影響の確認<br>同じ変更を当てたとき<br>修正・再確認がどこまで届くか"]
    ACCEPT["受入・回帰エビデンス"]
    COMPLETE["設計課題の完了証拠"]
    EFFECT["得た効果と増えたコスト"]
...
最後に、**今回の変更依頼をもう一度当ててみます。** フェーズ3では現状の構造へ当てて痛みを観測しました。完成構造では、今回追加した機能だけがまだ無い状態を基準に置き、同じ変更IDを加えるならどの既存部品へ手を入れ、どの新規部品を足すかを数えます。別の将来変更へ置き換えず、今回の変更に必要な部品を省きません。同じ要求・同じ粒度で比べて初めて、修正範囲がどう変わったかを説明できます。「何を手に入れたのか」に、これがいちばん直接答えます。
...
ここまで説明した7つのフェーズは、**すでに動いているシステムへ、具体的な仕様変更や機能追加の依頼が届いた場面**を主な対象にしています。現行仕様と変更前コードがあるからこそ、変更前の動作を基準にして、修正・再確認の広がりを観測できます。

| 見る点 | 既存システムの変更――本書の主対象 | 新規設計への転用 |
|---|---|---|
| 出発点 | 現行仕様、動作中のコード、具体的な変更依頼 | 初期要求、構造案や小さな試作 |
| 影響の確かめ方 | 実際の変更経路を追い、修正・再確認範囲を記録 | 起こりそうな変更を構造案へ当て、影響を予測 |
| 判断の根拠 | 現行コードと動作で観測した事実 | 業務知識、関係者の見通し、類似変更の履歴 |
| 分離する条件 | 観測した痛みの原因を取り除く必要がある | 変化の見込みと影響が、分離コストを上回る |

何も作られていない新規開発へ、7つをそのまま当てはめるわけではありません。現状コードがなければ、フェーズ3で実際の変更の痛みを観測できないからです。ただし、**何が変わるかを考え、その変化が関係のない責任まで届かないかを確かめる思考**は使えます。

新規設計では、初期要求から作った構造案へ、根拠のある変更仮説を設計上のテストとして当てます。異なる理由で変わるものが一つに集まり、その変更が繰り返される見込みも大きいなら、実装前に境界を作る判断ができます。一方、考えられる変更をすべて先回りして分離すれば、使われない柔軟性のために構造だけが複雑になります。観測できないものは仮説として扱い、備える根拠とコストを比較します。
...
→ 変更を試して痛みを観測する
→ 痛みを生んだ原因を特定する
→ 責任の目標配置と、解くべき接続点・完了条件を決める
→ 目標配置を、契約・具体・生成から実行までのコード構造へ変える
