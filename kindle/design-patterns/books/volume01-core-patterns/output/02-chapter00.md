...
番号には、括弧で短い名前が付きます。末尾の数字は種類ごとに独立した連番です。たとえば変更依頼が三つあれば、`変更ID1`、`変更ID2`、`変更ID3` と増えます。要求IDや問題IDも、それぞれ別に1から始まります。
...
| IDの種類 | この番号で追うもの |
|---|---|
| 要求ID | システムが満たす要求と、完成後の受入・回帰結果 |
| 変更ID | 今回の変更依頼、変更後要求、変更影響 |
| リスクID | 変わる可能性と、採用する設計での備え |
| 問題ID | 変更を試して観測した修正・再テストの広がり |
| 原因ID | 痛みを生んだ責任の集まり方 |
| 課題ID | 原因をなくす構造上の課題と、完成コードでの改善結果 |

追跡に使う番号は、この6種類です。各IDをどこで確定し、次へどう渡すかは、このあとの7フェーズと一緒に説明します。ここでは、`ID1` が全体で一つだけあるのではなく、**種類ごとに必要な件数だけ連番になる**ことを押さえてください。
...
洗い出した箇所は、最後に1枚の**変更影響グラフ**へまとめます。作り方は、変更試行の差分から、追加・書き換えたコード箇所をクラス名、関数名、変更内容とともに一箱ずつ書くことです。同じコード箇所を複数の変更IDで直した場合は箱を分けず、箱内に該当IDを併記します。同じクラスでも仕事が違う箇所は別の箱にします。

**箱はクラスの中の仕事、枠はクラス**です。変更した箱は黄色、追加した箱は青、書き換えていないが回帰確認する箱は灰色にします。回帰確認だけ、変更箇所から点線でつなぎます。変更IDから同じ箱へ何本も矢印を引く必要はありません。

**クラス図ではなく、この形にしています。** クラス図の箱は1クラスで1つなので、**1つのメソッドの中に別々の仕事が同居している**状態が消えるためです。変更影響グラフはコード差分を仕事の単位で分け、クラスの枠内へ置きます。

この図は、次の三つの見方で読みます。

1. **一つの枠の中に、違うものを見ている箱が2つ以上並んでいないか。** 片方だけを書き換えても、もう片方まで読み直すことになります。
2. **同じ変更IDが、複数の枠へ付いていないか。** その変更に専用の置き場所がまだありません。
...
> - 同じクラスに、参照情報と変更理由が独立した責任が同居していないか
...
やることは一つです。**変更影響グラフの箱ごとに、そこが見て動く情報と該当実装を対応させ、同じクラス内の仕事を責任として簡潔に表します。**

見る情報は、公開する操作と引数、戻り値、保持するメンバーや呼ぶ相手に現れます。
...
一つの関数の中でも、**前半と後半が違う情報を見て動く**ことがあります。そこが境界の候補です。依頼された日時や担当者が同じでも、片方の規則だけを変えたときに、もう片方の実装まで触るなら、変更理由の異なる責任が同居しています。
...
ここで新しい判断軸を増やすわけではありません。「はじめに」の三つの問いを、次のように使います。

| 三つの問い | このフェーズで確定すること |
|---|---|
| 問い1：同じ場所に、別々の理由で変わるものがないか | フェーズ5で確定済み。分ける責任を、このフェーズで選び直さない |
| 問い2：境界では、何を約束すれば足りるか | フェーズ5の業務上の依頼・入力・結果を、C++の型と操作へ翻訳する |
| 問い3：実体を、誰が作り、持ち、渡すのか | 実体を組み立てる場所と、所有・受け渡しを決める |

三つの問いは、設計判断を確かめる**観点**です。フェーズ6でいう「構想」は、その観点を使って確定した責任、契約、具体、生成・所有、受け渡しが、実行できる一つの構造としてつながった結果です。別の構造図をもう一度作るのではなく、各章ではフェーズ5の目標責任図をクラス名へ置き換え、部分クラス図を足しながら完成図へ近づけます。
