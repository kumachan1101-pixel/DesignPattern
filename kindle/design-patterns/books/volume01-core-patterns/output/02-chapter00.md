...
**箱はクラスの中の仕事、枠はクラス、矢印のラベルはその仕事が見ている接続情報**です。今回書き換えた箱には色を付け、書き換えなかった箱は白のまま同じ枠の中へ並べます。
...
フェーズ3では、変更で実際に開いた範囲を問題として確定しました。ここでは、**なぜその範囲まで開いたのか**を調べます。調べる対象は、変更影響グラフで**色の付いた箱を含むクラス**です。
...
5. **読みます。** ①**同じ行に場所が2つ以上並んでいる**なら、それらは同じものを見ているので必ず一緒に変わります。**一つへまとめる対象**です。②**違う行の場所が同じクラスの中にあり、そのうち1つでも「はい」がある**なら、**混在**です。分ける対象はここです。③**「いいえ」だけの行**は今回変わりませんが、混在したクラスの中にあるため、開くたびに読み直すことになります（巻き込み）。
...
7. **今回触らないクラスを1行残します。** 違うものを見ている場所を複数抱えていても、今回の変更でもヒアリングで確認した理由でも開かないクラスは、直す対象にしません。**抱えていること自体は、直す理由になりません。** 記録して次の依頼を待ちます。
...
2. **分けた実体が複数になるなら、どれを使うかを決める。** 仕事を外へ出すと、その仕事は複数の実体になります。「どれを使うか」を使う側が持つと、実体が増えるたびに使う側を開くことになり、原因が戻ります。個々の実体が持つと、実体どうしが互いを知ることになり、1つ足すたびに既存を開きます。どちらも避けるには、選ぶ仕事を**第三の場所**へ置きます。全部を使う場合は選ぶ必要がないので、配る仕組みだけを決めます。分けた両側で何を受け渡すかも、ここで決めます。
