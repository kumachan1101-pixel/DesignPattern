...
問題はフェーズ3で確定済みです。すべての処理をもう一度分解せず、責任一覧と変更途中コードへ戻ります。各問題IDについて、**変更したかった責任**と、同じ場所にあるため修正・解読・回帰確認へ巻き込まれた**影響した責任**の組だけを拾います。`if (status == ...)` のような短いコード上の目印も添えます。

責任そのものが問題なのではありません。一つの変更理由が、別の責任まで巻き込んだ関係を分析します。

**2つ目：関係した責任を、今回「変える」ものと「守る」ものに分けます。**

責任ごとに、**コードの場所ではなく、変わる業務上のきっかけ**を書きます。今回の変更理由に直接対応する責任を「変える」、維持したい責任を「守る」とします。
...
**痛みと原因は1対1とは限りません。** 問題IDごとの責任の組を突き合わせ、同じ構造原因は一つの原因IDへまとめます。このフェーズで確定するのは「何と何が、別の理由で変わるか」までです。境界はフェーズ5、コード上の対策はフェーズ6で決めます。
...
最後に、確定した境界へ課題IDを付けます。まず「元の原因ID」「変える側」「守る側」を対応づけ、続く表で「接続点を流す業務上の依頼・入力・結果」「どうなれば完了か」を対応づけます。一つの横長な表へ詰め込まず、原因から境界を導く表と、境界から完了条件を導く表に分けても構いません。課題の数を変更依頼の数へ合わせません。一つの変更が複数の構造原因を表すことも、複数の変更が一つの原因から生じることもあるからです。
...
> **問い1から課題へ：** フェーズ4で答えた「別々の理由で変わるもの」を、実際に線を引ける境界候補へ言い直します。ここで決めるのは、境界を越える必要がある業務上の依頼・入力・結果までです。型名、操作名、シグネチャというコード上の契約や、実体を誰が作るかは決めません。それは問い2・問い3を使うフェーズ6の仕事です。
