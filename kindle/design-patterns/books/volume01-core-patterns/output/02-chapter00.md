...
現状把握では、一つの資料だけを正解にしません。要求、実行結果、クラス図、コードを突き合わせ、食い違いがあればそこで止めて確認します。`if` 文だけを読んでも「なぜこの区別があるのか」は分からず、仕様だけを読んでも「実際にどこが動くのか」は分からないからです。

最初に小さな実行例を見ると、何を渡すと何が返るシステムなのかを短時間でつかめます。ただし、実行例は現状を知る一つの入口です。そこで見えた入口から、成功・失敗、保存される値、関係するクラスへ範囲を広げ、要求とコードが同じ動きを指しているかを確かめます。
...
| 仮の見方 | このフェーズで確かめること |
|---|---|
| 幹候補 | 今回の変更でも維持する流れ・結果か。どんな変更が来たら見直すか |
| 変化点候補 | 何をきっかけに、誰の判断で、どの程度の頻度で変わるか |
