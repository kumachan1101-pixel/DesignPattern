...
1. **問題が起きたコードを見る。** 何を変えたかったのか、そのために既存のどこまで直したのかを、短いコード上の目印とともに確認します。
2. **一つのクラスにある責任を特定する。** 責任名は「販促方針」「注文計算」のように、実務で通じ、実現方法が変わっても使える名前にします。各責任が問題の起きたコードで何を担い、その責任内のルールや処理が今回直接変わったのか、同じクラスにあるため巻き込まれたのかを分けます。ここで直接変わるのは、責任の名前ではなく、その責任が担うルールや処理です。
3. **問題が起きた理由を確定する。** 変更理由の違う責任が同じ場所にあるか、一方が他方の具体を直接知っているかをコードで確かめます。その配置がフェーズ3の問題を実際に生んだ関係だけに原因IDを付けます。

2つ目で分かるのは、一つのクラスが何の責任を持つかまでです。複数の責任があるだけでは、直ちに問題とは言えません。3つ目でコードへ戻り、「変更理由の異なる責任が同じ場所にあるため、別の責任まで修正・確認対象になった」と追跡できたとき、初めて原因として確定できます。
...
2. **課題と完了条件を確定する。** 目標図で分ける箇所に番号を付け、元の原因、分ける責任、接続点で渡す情報、コードで確認できる完了条件を対応づけます。

フェーズ5の図には、「販促方針」「状態方針」のような**責任名**を使います。フェーズ6では、その責任配置をインターフェース名、具体クラス名、操作名、生成場所へ順に変換します。

接続点では、分けた両側の動作をつなぐために必要な情報だけを決めます。型名、操作名、生成場所はフェーズ6で決めます。
