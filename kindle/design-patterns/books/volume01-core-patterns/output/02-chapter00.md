...
>
> **成果物：** 仕事と見ている接続情報の一覧、責任の一覧、問題が起きた理由を示す原因ID一覧

### このフェーズの考え方

フェーズ3では、変更で実際に開いた範囲を問題として確定しました。ここでは、**なぜその範囲まで開いたのか**を調べます。調べる対象は、変更影響グラフで**色の付いた箱を含むクラス**です。

やることは一つです。**そのクラスがしている仕事を並べ、それぞれが何を見て変わるのかを書きます。**

見ているものを、本書では**接続情報**と呼びます。コードの3か所に書いてあります。
...
本書でいう**責任**は、**同じ接続情報を見て一緒に変わる仕事の束**です。フィールドやメソッド、今回増えた作業の数には合わせません。**表に書いた仕事の名前が、そのまま責任名になります。** 「方針」「管理」「処理」だけで終わる名前にはしません。何を対象に、何を判断・計算・保持・選択・更新するのかが読める名前にします。
...
2. **そのクラスがしている仕事を並べます。** 今回書き換えた仕事だけでなく、書き換えなかった仕事も並べます。巻き込みがそこで読めます。表には、その仕事が**コードのどこにあるか**も書きます。
3. **仕事ごとに、見ている接続情報を書きます。** 公開する操作・引数・戻り値・保持するメンバーのうち、その仕事の中に名前が出てくるものです。コードの行を読めば決まります。
4. **今回書き換えたかを書きます。** 今回の変更で実際に書き換えた、または壊れていないか確認したなら「はい」。今回は触っていないが、ヒアリングで確認した理由が届いたら書き換えるなら「将来」。どちらでもなければ「いいえ」。**観測した事実だけで書きます。** 量は問いません。1行直しても100行直しても「はい」は一つです。
5. **読みます。** **同じクラスの仕事が違う接続情報を見ていて、そのうち1つでも「はい」があれば混在**です。分ける対象はここです。「いいえ」の仕事は今回変わりませんが、同じクラスにあるため、開くたびに読み直すことになります（巻き込み）。**一つの仕事が2か所以上に散っている**なら、その仕事に専用の置き場所がまだありません。
6. **原因IDを付け、対策前図を置きます。** 責任を箱にして、いまどれが同じクラスへ同居しているかを1枚にし、直接変わった責任と、同居のせいで巻き込まれた責任を図の直後で説明します。ここで決めるのは「どの同居が、どの問題IDを生んだか」までです。**どこへ置き直すかはフェーズ5**で決めます。
7. **今回触らないクラスを1行残します。** 違うものを見ている仕事を複数抱えていても、今回の変更でもヒアリングで確認した理由でも開かないクラスは、直す対象にしません。**抱えていること自体は、直す理由になりません。** 記録して次の依頼を待ちます。

#### 誰が作っても同じ表になるようにする

表がぶれるのは、仕事の切り方と印の付け方が人によって変わるからです。次の三つを規則にすると、同じコードと同じヒアリング結果からは同じ表が出ます。

**仕事の切り方。** **見ている接続情報が変わるところで切ります。** 一つの関数でも、前半と後半で違う引数を見ているなら2行です。1メソッド＝1行にはしません。目の前のコードだけを見て決め、**将来こういう依頼が来るかもしれない、という想像は使いません。**
...
二人が別々に書いて、仕事の呼び名が違っても構いません。**数と分け目が一致すれば、同じ表です。** 呼び名の一致まで条件にすると、かえって使われなくなります。

#### 変更が来る前に読めること

この表は、変更が来る前でも作れます。コードを読むだけだからです。違う接続情報を見ている仕事を同じクラスに抱えているものを並べれば、**変更依頼を待たずに棚卸しができます。**

ただし、それだけでは直す根拠になりません。**いま何を抱えているかは読めますが、それが今後変わるかは読めません。** 変わらなければ痛みは起きません。直す根拠になるのは、変更を当てて観測した「はい」か、ヒアリングで確認した「将来」だけです。

表に仕事が複数並ぶだけでも、直ちに問題とは言えません。フェーズ3の変更影響と突き合わせ、「違うものを見ている仕事が同じ場所にあるため、別の仕事まで修正・確認対象になった」と言えたとき、初めて原因として確定できます。手順6の図（以下、対策前図）が、フェーズ5で責任配置を変える前の比較元になります。
