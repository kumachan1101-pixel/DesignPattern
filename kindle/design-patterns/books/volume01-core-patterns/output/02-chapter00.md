...
六つのIDのうち、設計課題を導く因果関係は、次の四つを一本につなぎます。

1. **変更ID** ―― 今回届いた変更依頼
2. **問題ID** ―― 変更を現状コードへ当てて観測した痛み
3. **原因ID** ―― その痛みを生んでいる構造上の理由
4. **課題ID** ―― 原因をなくすために解くべき境界

変更を試すのは、変更IDから問題IDを観測するための作業です。要求IDは変更後も守る機能、リスクIDは将来変化への備えとして、この因果関係を外側から確認します。
...
| 受け渡し | 次のフェーズで行うこと |
|---|---|
| フェーズ1の現状・変更ID | フェーズ2で、変更の見込みと今回維持する動作を確定する |
| フェーズ2の確定変更・今回維持する動作 | フェーズ3で、維持する動作まで修正・確認が広がる変更を「痛み」として観測する |
| フェーズ3の問題ID・変更影響グラフ | フェーズ4で、痛みを生んだ責任の集まり方を原因へ変える |
| フェーズ4の原因ID・一つのクラスに混在する責任・同居が問題を生んだ証拠 | フェーズ5で、原因をなくす責任配置と接続点を課題へ変える |
| フェーズ5の責任の目標配置・課題ID・接続点・完了条件 | フェーズ6で、契約から実行までつながるコード構造へ変える |
| フェーズ6で確定した構想・生成／所有／注入規則 | フェーズ7で、全体コードへ統合し、要求と完了条件を別々に検証する |
...
ここから示す各フェーズの「入力」と「成果物」は、題材に依存しない共通の定義です。第1章以降では、同じ定義を各題材の具体的な情報へ置き換えて進めます。
...
各実践章の入口では、まず最小限の `main()` と実行結果を一組だけ見せます。これは、読者が「何を渡すと、どんな動きをするシステムか」を先にイメージするための**動作の予告**です。クラスの責任や構造は、この動作を把握したあとで順に調べます。
...
**目的：仕様とシステムの現状から「何が変わりそうで、今回どの動作を維持するか」の仮説を立て、ヒアリングで裏付けること**

> **入力：** フェーズ1の仕様・クラス・コードの事実、変更ID、関係者情報
>
> **成果物：** フェーズ3で試す確定変更、今回維持する動作、フェーズ6でも使うリスクID付き将来リスク
...
> **入力：** フェーズ2で確定した変更ID・今回維持する動作と、フェーズ1の変更前コード
>
...
**目的：問題が起きた理由を、コードの責任配置から特定すること**

> **入力：** フェーズ3の問題ID、変更途中コード、変更影響グラフ
>
> **成果物：** 変更理由の一覧、変更理由と修正場所の対応表、責任の一覧、問題が起きた理由を示す原因ID一覧

### このフェーズの考え方

フェーズ3では、変更で実際に開いた範囲を問題として確定しました。ここでは、**なぜその範囲まで開いたのか**を調べます。手がかりは、変更影響グラフが示す「場所」へ、もう一本**「変更理由」**の軸を足すことです。

**変更理由とは、誰がどんなきっかけで書き換えを決めるか**です。題材が違っても、次の型のどれかに収まります。

| 変更理由の型 | 何が変わるのか |
|---|---|
| 仲間が増える | 同じ役割のものの種類が1つ増える |
| 規則が変わる | 条件・計算式・可否の中身が変わる |
| 選び方・順序が変わる | 複数が成り立つとき、どれを使うか・どの順に並べるか |
| 入口が増える | 受け付ける操作や外部イベントの種類が増える |
| 相手が変わる | つなぐ外部の手段・API・保存先が変わる |
| 時点が変わる | 同期から非同期へ、即時から後追いへ |
| 見せ方が変わる | 表示・文言・出力形式が変わる |

型の名前を覚える必要はありません。大事なのは、**変更理由ごとに決める人と決まる時期が違う**ことです。理由が違えば、変更は別々の日に別々の依頼として届きます。

本書でいう**責任**は、**一つの変更理由に結びつく仕事の束**です。フィールドやメソッド、今回増えた作業の数には合わせません。同じデータを使うかどうかでもありません。同じ理由で一緒に書き換わるなら一つ、別の理由で別々に書き換わるなら別、と数えます。

**変更理由と責任は、同じ束を別の側から見たものです。** 変更理由は「なぜ変わるか」、責任は「その理由で一緒に変わる仕事が何をしているか」です。あとで作る表の列に、仕事の側から名前を付け直したものが責任になります。ただし一対一とは限りません。必ず一緒に動く列は一つの責任へまとめ、今回どの列でも動かない仕事も、別の場所に分かれているならそのまま一つの責任として残します。

**修正箇所が2つ以上あること自体は、問題ではありません。** 一つの理由のためだけに用意された場所が並んでいるだけなら、それは分かれている形です。問題は、**一つの場所が複数の理由に引っ張られているとき**です。別の理由の変更でも同じ場所を開くことになり、関係のない仕事まで再確認することになります。

責任名は「方針」「管理」「処理」だけで終わらせません。「料金条件の判定・金額計算」「工程別の操作可否・次工程選択」のように、**何を対象に、何を判断・計算・保持・選択・更新するのか**が読める名前にします。クラス名が決まる前でも、変更理由と仕事の内容を具体的に追えるためです。
...
問題が起きたコードと変更影響は、フェーズ3ですでに特定しています。フェーズ4では、その結果へ変更理由の軸を足し、次の順で分析します。

1. **変更理由を並べます。** 今回の変更IDから来た理由と、フェーズ2で聞いた将来の理由（リスクID）を、上の型で書き出します。
2. **理由と場所を交差させます。** ここで粒度を一段下げます。変更影響グラフは、クラスと組み立て箇所の粒度で「どこまで開いたか」を示しました。同居を見るには、`［変更］`が付いたクラスの**中で何を書き換えたか**まで割る必要があります。そこで、そのクラスの中身を仕事ごとの行に開き、変更理由を列にした表を作ります。今回開いたところへ`○`、将来その理由が来たときに開くところへ`△`を付け、行には**その仕事がいまどのクラスのどこにあるか**も書きます。クラス名を省くと、どこが同居しているのかが読めません。
3. **四つの読み方で見ます。** ①**同じクラスの行に、別々の列の印が付いている**なら、そのクラスに複数の責任が同居しています（混在）。分ける対象はここです。②**一つの行に印が2つ以上**あるなら、その仕事自体が複数の理由で変わります。必ず一緒に動く理由なら一つの束として扱い、そうでなければ行をさらに分けます。③**ある列の印が1行に収まっている**なら、その理由には専用の置き場所があります。④**どの列にも印がない行**は今回変わりませんが、混在したクラスの中にあるなら、開くたびに巻き込まれます。
4. **混在したクラスへ原因IDを付け、列に責任名を付けます。** 列がそのまま、分けたあとの仕事の束になります。必ず一緒に動く列どうしは、一つの責任へまとめます。ここで決めるのは「どの同居が、どの問題IDを生んだか」までです。**どこへ置き直すかはフェーズ5**で決めます。
5. **対策前図を置きます。** 責任を箱にして、いまどれが同じクラスへ同居しているかを1枚にし、直接変わった責任と、同居のせいで巻き込まれた責任を図の直後で説明します。

#### 誰が作っても同じ表になるようにする

表がぶれるのは、行の切り方・列の数え方・印の付け方が人によって変わるからです。次の三つを規則にすると、同じコードと同じヒアリング結果からは同じ表が出ます。

**行の切り方。** 行にするのは、変更影響グラフで`［変更］`が付いたクラスの中身です。「実際に書き換えた、または壊れていないか確認したひとかたまり」で割り、1メソッド＝1行にはしません。今回書き換えなかった仕事も、同じクラスの中にあるなら行に入れます（巻き込みがそこで読めます）。実体を組み立てる `main()` は、責任を持つ場所ではなく、どの実体をいつ作るかを決める場所なので行にしません（フェーズ6で扱います）。同じ理由で動きそうに見えて実は別のクラスにある仕事は、巻き込みとの違いを示すために1行だけ足して構いません。切るか迷ったら、**「このかたまりだけが変わる依頼を一つ挙げられるか」**を問います。挙げられるなら別の行、挙げられないなら同じ行へまとめます。行には必ず、クラス名から始まる場所を書きます（`クラス名::メソッド名`、`クラス名` のメンバーなど）。クラス名を省くと、どこが同居しているのかが読めません。

**列の数え方。** 列は変更理由です。変更IDとリスクIDを一つずつ「なぜ書き換えるのか」へ言い換え、上の型へ当てます。二つの理由を分けるかどうかは、**「片方だけが届く依頼がありうるか」**で決めます。ありうるなら別の列、必ず一緒に届くなら同じ列です。列は、今回の依頼から来た理由を先に、将来の理由を後に並べます。

**印の付け方。** 今回の変更試行で実際に書き換えた、または壊れていないか確認したところへ`○`。今回は触っていないが、その列の理由が届いたら書き換えるところへ`△`。どちらでもないところは空欄です。**印は観測した事実だけで付けます。** 「たぶんここも直すことになりそう」は入れません。

この三つで、表は変更試行の事実とヒアリングで確認した理由だけから決まります。好みの入る余地はありません。

表に責任が複数並ぶだけでは、直ちに問題とは言えません。フェーズ3の変更影響と突き合わせ、「変更理由の異なる責任が同じ場所にあるため、別の責任まで修正・確認対象になった」と言えたとき、初めて原因として確定できます。手順5の図（以下、対策前図）が、フェーズ5で責任配置を変える前の比較元になります。

同じ理由で変わる仕事は一つの責任にまとめます。別の理由で変わる仕事だけを分けます。行数やメソッド数だけでは判断しません。
...
**目的：原因をなくすために、何を分け、どうなれば完了かを決めること**

> **入力：** フェーズ4で確定した原因IDと責任
>
> **成果物：** 課題ID、分ける責任、接続点で受け渡す情報、完了条件
...
1. **原因をなくす責任配置を決める。** フェーズ4の対策前図と同じ責任名を使い、変更理由の異なる責任を分けた目標図を作ります。分けた両側を何でつなぐかも決めます。一部を関数へ移しただけで具体名や判断が残る配置は、原因が残るため途中案として退けます。
2. **課題と完了条件を確定する。** 目標図で分ける箇所に番号を付けます。各番号を、解く原因、構造の変更、接続点で渡す情報、コードで確認できる完了条件の順に短い課題カードへまとめます。目標構造は図で確定済みなので、別の文章や表で定義し直しません。
...
> **入力：** フェーズ5の課題カードにある接続、完了条件、システム全体の到達状態
>
...
**2つ目は、構造の痛みが消えたか**です。フェーズ5で書いた完了条件を省略せず一つずつ並べ、条件ごとにコードまたは実行結果の証拠と合否を示します。課題名だけを「対応済み」と要約しても、何を根拠に完了したのかは確認できません。
