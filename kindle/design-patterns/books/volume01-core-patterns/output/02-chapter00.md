...
洗い出した箇所は、最後に1枚の図へまとめます。本書ではこれを**変更影響グラフ**と呼びます。変更要求を起点に、触ることになったクラスと、そこからさらに触ることになった先を矢印でつないだものです。矢印には、**そのクラスの中で何を書き換えることになったか**をラベルとして書きます。このラベルが、フェーズ4で仕分ける単位になります。

この図は、次の三つの見方で読みます。

1. **一つのクラスへ、矢印が2本以上集まっていないか。** 別々の依頼が同じ場所へ届いています。依頼が1本しかない場合でも、その1本が**その箱の中の複数箇所**を開いているなら同じことです。
2. **一つの矢印が、複数のクラスへ枝分かれしていないか。** その依頼に専用の置き場所がありません。
3. **点線（回帰確認）が伸びていないか。** 変えていないのに確認が要るところです。

**ただし、三つとも「候補」までです。** この図はクラスの粒度なので、その箱の中で本当に別々のものが同居しているかは、中を割らないと分かりません。確定はフェーズ4で行います。
...
>
> **成果物：** 要素の一覧、責任の一覧、問題が起きた理由を示す原因ID一覧

### このフェーズの考え方

フェーズ3では、変更で実際に開いた範囲を問題として確定しました。ここでは、**なぜその範囲まで開いたのか**を調べます。調べる対象は、変更影響グラフで**矢印が集まったクラス**です。

やることは一つです。**そのクラスが抱えている「要素」を数えます。**

**要素とは、外とのやり取りを通じて変化するもの**です。ある操作を受け取ると手続きの進み具合が変わる。ある区分を受け取ると計算の結果が変わる。この「変わるもの」が要素です。受け取る操作や引数そのものは、要素へ届く**経路**であって、要素ではありません。
...
> **これを通じて、このクラスの中の何が変わるか。**

答えが出るものが要素です。「何も変わらない。素通りする」なら、それは経路であって要素ではありません。

> [!NOTE] 学校のクラスで言うと
> 一人の子が、掃除のやり方も、知らせる先も、当番の順番も全部抱えているとします。掃除のやり方が変わっても、知らせる先が増えても、呼ばれるのはその子です。**掃除のやり方を変えただけなのに、知らせ方の話まで一緒に確認することになります。**
>
> この「掃除のやり方」「知らせる先」「当番の順番」が要素です。**一人が2つ以上抱えていて、そのうち1つでも変わるなら、分ける合図**になります。

**修正箇所が2つ以上あること自体は、問題ではありません。** 一つの要素のためだけに用意された場所が並んでいるだけなら、それは分かれている形です。問題は、**一つのクラスが複数の要素を抱えているとき**です。片方を直すために、もう片方まで読み直すことになります。

本書でいう**責任**は、**一つの要素について行う仕事の束**です。フィールドやメソッド、今回増えた作業の数には合わせません。同じデータを使うかどうかでもありません。

責任名は、要素から決まります。**〈要素〉について、何を判断・計算・保持・選択・更新するのか**を書きます。「方針」「管理」「処理」だけで終わる名前にはしません。クラス名が決まる前でも、何を対象に何をするのかを追えるようにするためです。
...
問題が起きたコードと変更影響は、フェーズ3ですでに特定しています。フェーズ4では、その結果へ要素の軸を足し、次の順で分析します。

1. **調べるクラスを決めます。** 変更影響グラフで矢印が集まったクラスです。矢印が1本でも、その箱の中の複数箇所を開いているなら同じです。実体を組み立てる `main()` は、責任を持つ場所ではないので調べません（フェーズ6で扱います）。
2. **要素を並べます。** そのクラスの公開操作・引数・戻り値・保持するメンバーを見て、「これを通じて中の何が変わるか」を問います。変わるものが要素、素通りするものは経路です。表には、その要素が**いまコードのどこにあるか**も書きます。
3. **要素ごとに印を付けます。** 今回の変更で実際に書き換えた、または壊れていないか確認した要素へ`○`。今回は触っていないが、ヒアリングで確認した理由が届いたら書き換える要素へ`△`。どちらでもなければ空欄です。**印は観測した事実だけで付けます。** 量は問いません。1行直しても100行直しても`○`は一つです。
4. **読みます。** **一つのクラスに要素が2つ以上あり、そのうち1つでも印が付いていれば混在**です。分ける対象はここです。印のない要素は今回変わりませんが、同じクラスにあるため、開くたびに読み直すことになります（巻き込み）。**一つの要素が2か所以上に散っている**なら、その要素に専用の置き場所がまだありません。
5. **原因IDを付け、対策前図を置きます。** 責任を箱にして、いまどれが同じクラスへ同居しているかを1枚にし、直接変わった責任と、同居のせいで巻き込まれた責任を図の直後で説明します。ここで決めるのは「どの同居が、どの問題IDを生んだか」までです。**どこへ置き直すかはフェーズ5**で決めます。
6. **今回触らないクラスを1行残します。** 要素を2つ以上抱えていても、今回の変更でもヒアリングで確認した理由でも開かないクラスは、直す対象にしません。**抱えていること自体は、直す理由になりません。** 記録して次の依頼を待ちます。

#### 誰が作っても同じ表になるようにする

表がぶれるのは、要素の切り方と印の付け方が人によって変わるからです。次の二つを規則にすると、同じコードと同じヒアリング結果からは同じ表が出ます。

**要素の切り方。** 接続情報を一つずつ見て、「これを通じて中の何が変わるか」に答えます。目の前の差分だけを見て答え、**将来こういう依頼が来るかもしれない、という想像は使いません。** 二つの変化を一つの要素にまとめてよいかは、**同じ値を見ている分岐か、同じ結果を出すために連なっている分岐か**で決めます。どちらかに当てはまれば一つ、当てはまらなければ別です。

**印の付け方。** `○`は観測した事実、`△`はヒアリングで確認した理由に限ります。「たぶんここも直すことになりそう」は入れません。

二人が別々に書いて、要素の呼び名が違っても構いません。**数と分け目が一致すれば、同じ表です。** 呼び名の一致まで条件にすると、かえって使われなくなります。

#### 変更が来る前に読めること

要素の一覧は、変更が来る前でも作れます。接続情報を読むだけだからです。要素を2つ以上抱えているクラスを並べれば、**変更依頼を待たずに棚卸しができます。**

ただし、それだけでは直す根拠になりません。**いま何を抱えているかは読めますが、それが今後変わるかは読めません。** 変わらなければ痛みは起きません。直す根拠になるのは、変更を当てて観測した`○`か、ヒアリングで確認した`△`だけです。

表に要素が複数並ぶだけでも、直ちに問題とは言えません。フェーズ3の変更影響と突き合わせ、「変わる要素と別の要素が同じ場所にあるため、別の要素まで修正・確認対象になった」と言えたとき、初めて原因として確定できます。手順5の図（以下、対策前図）が、フェーズ5で責任配置を変える前の比較元になります。
...
1. **原因をなくす責任配置を決める。** フェーズ4の対策前図と同じ責任名を使い、混在していた要素を分けた目標図を作ります。一部を関数へ移しただけで具体名や判断が残る配置は、原因が残るため途中案として退けます。打ち手は、その要素がどう変わるかで決まります。
...
2. **分けた実体が複数になるなら、どれを使うかを決める。** 要素を外へ出すと、その要素は複数の実体になります。「どれを使うか」を使う側が持つと、実体が増えるたびに使う側を開くことになり、原因が戻ります。個々の実体が持つと、実体どうしが互いを知ることになり、1つ足すたびに既存を開きます。どちらも避けるには、選ぶ仕事を**第三の場所**へ置きます。全部を使う場合は選ぶ必要がないので、配る仕組みだけを決めます。分けた両側で何を受け渡すかも、ここで決めます。
