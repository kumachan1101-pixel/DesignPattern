...
`［変更］` は変更箇所を示す目印であって、それだけでは変更内容を表しません。フロー図と変更影響グラフでは、同じ箱に「条件2本→4本」「手段2種→3種」のような変更前後も書きます。たとえば、変更前の構造では既存の複数箱に変更が広がり、完成後の構造では新規の箱へ差分が閉じる、という形で図が変わります。**変更の箱が減っていれば、既存コードを開くところが減ったということです。** 端末や印刷で色が分かりにくくても、文字と枠で差分を読めます。
...
> **三つの問いへの準備：** ここで確定する「変わる側／守る側」は、問い1「同じ場所に、別々の理由で変わるものがないか」を考える材料です。分離する場所は変更の痛みと原因を確定したあと、方法は責任配置を決めたあとに導きます。
...
問題が起きたコードと変更影響は、フェーズ3ですでに特定しています。フェーズ4では、その結果を次の順で分析します。

1. **問題箇所にある責任を特定する。** 責任名は「販促方針」「注文計算」のように、実務で通じ、実現方法が変わっても使える名前にします。各責任が問題箇所で何を担い、その責任内のルールや処理が今回直接変わったのか、同じクラスにあるため巻き込まれたのかを分けます。ここで直接変わるのは、責任の名前ではなく、その責任が担うルールや処理です。
2. **責任の配置を原因として確定する。** 変更理由の違う責任が同じ場所にあるか、一方が他方の具体を直接知っているかを確かめます。その配置によって、フェーズ3で特定した別の責任まで修正・確認対象になったと追跡できた関係だけに原因IDを付けます。

1つ目で分かるのは、一つのクラスが何の責任を持つかまでです。複数の責任があるだけでは、直ちに問題とは言えません。2つ目でフェーズ3の変更影響と突き合わせ、「変更理由の異なる責任が同じ場所にあるため、別の責任まで修正・確認対象になった」と言えたとき、初めて原因として確定できます。
...
1. **原因をなくす責任配置を決める。** 変更理由の異なる責任を分け、分けた両側を何でつなぐかを決めます。対策前と目標の部分クラス図を同じ責任名で並べると、責任がどこからどこへ移るかを確認できます。一部を関数へ移しただけで具体名や判断が残る配置は、原因が残るため途中案として退けます。
...
実践章では、最初にこの表を完成答案として埋めません。フェーズ5の責任配置を出発点に、契約、具体、生成・所有、受け渡し、実行の順で判断理由と部分コードを示します。必要な部分クラス図も、その判断を説明する位置へ置きます。すべてがつながった後で、課題ごとに「決定した構造」と「コード上の実現」をまとめ、構想を確定します。
...
最後に、フェーズ5で書いた完了条件へ戻ります。課題ごとに「構想のどの責任・どの接続で解くか」を確認します。完了したかどうかは、フェーズ7で条件を一つずつコードや実行結果へ照合します。あわせて、フェーズ2で挙げた将来リスクについて、**この構想なら何を守れて、何が守れないか**を書きます。守れない部分があること自体は問題ではありません。分かっていて選んだのか、気づかずに選んだのかが違います。
