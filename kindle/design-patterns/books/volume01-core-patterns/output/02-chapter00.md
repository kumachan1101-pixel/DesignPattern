...
この本では変更前と変更後を何度も並べます。そのたびに読者が2つを見比べて差分を探すのでは、読むのが仕事になってしまいます。そこで、**変わったところは青、変わっていないところは元の色**という決まりで統一しました。青いところだけ拾えば、その章で何が動いたかが分かります。

| 見ているもの | 変わったところの見え方 |
|---|---|
| コード | 行に**青い帯**が敷かれる（帯の無い行は変更前のまま） |
| 表 | セルが**青い太字**になる（黒いセルは変更前と同じ値） |
| 図 | ノードが**青地に白文字**になる（色の薄いノードは現状のまま） |

コードの行末にある `// ← 追加` `// ← 変更` は、その帯が何なのかを言葉で補うものです。

図では、四角・ひし形・平行四辺形といった**形**がそのノードの役割（処理・判定・入力など）を表し、**色**が「今回変わったかどうか」を表します。形と色で別のことを言っているので、青いノードでも役割は形から読み取れます。
