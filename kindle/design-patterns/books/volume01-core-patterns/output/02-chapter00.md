...
線ごとに、それがどんなコードから生まれるかを添えます。**図の線は、コードのこの形を1本で表したもの**です。

**①　実線＋白抜き三角 ―― 継承**

派生クラスが、実装を持つ基底クラスを `public` 継承しています。共通実装を受け継ぎ、一部を上書きするときの線です。

```cpp
class 派生クラス : public 基底クラス {   // ← この「: public」が①
    void 操作() override { ... }
};
```
...
①と同じ `: public` ですが、基底が**純粋仮想関数だけ**を持つ契約であるときは点線になります。呼び出し側を共通契約だけに依存させたいときの線です。

```cpp
class 契約 {
public:
    virtual void 操作() = 0;   // ← 中身が無い（= 0）ので契約
};

class 実装クラス : public 契約 {   // ← 契約が相手なので②
    void 操作() override { ... }
};
```
...
ひし形の側が部品を**値で**持っています。所有者が消えれば部品も一緒に消えます。
...
ひし形の側が「全体」、反対側が「部分」です。**部分は全体と別に存在できます。** カタログと登録項目のような関係です。

```cpp
class カタログ {
    std::vector<登録項目*> 一覧;   // ← 実体は外にあり、並べているだけなので④
};
```

**⑤　実線＋矢印 ―― 借りて使い続けている**

相手をメンバーとして持ちますが、**実体は他の誰かが所有しています。** 参照やポインタで借り続けている関係です。

```cpp
class 利用側 {
    参照する相手& 相手;   // ← 借りているだけなので⑤
};
```
...
```cpp
class 呼び出し側 {
    void 処理(const 引数の型& 値) {   // ← 引数で受けるだけなので⑥
        std::cout << 値.名前;
    }
    // 引数の型 をメンバーには持っていない
};
...
ここまで、パターン名を1つも使わずに7つのフェーズを説明してきました。**この本では、パターン名は最後に出てきます。**

「はじめに」で書いたとおり、課題から導いた責任・依存・接続点の形が設計構造で、その構造に既に名前が付いているとき、その名前がデザインパターンです。ですから名前は、フェーズ7まで進んで構造が出来上がったあとに、**その構造を人に説明するための語彙**として登場します。各章の最後に「パターン解説」を置いているのは、そのためです。

第1章から第3章の構造には、Strategy・State・Observerという名前が付いています。ただし本書はGoFの23パターンを網羅しません。名前を覚えることより、変更要求を接続点へ当てて「どの知識を持ちすぎているか」「何を移せば変更の影響が小さくなるか」を説明できることを目的にしています。
