...
4. `main()` は自由に組み替えて構いません。各章の境界クラス（`〜Database` / `〜Repository` など）は、実行終了まで状態を覚えるインメモリの保存です。`save()` で新しいデータ（顧客・口座・イベント・メニュー・承認者など）を足し、その値で処理を呼べば、追加したデータの結果がその場の出力に表れます。
...
| `［変更］`／`<<changed>>` | **淡い黄の面と少し太い茶色の枠** | もとからあって、開いて直すもの。フロー図では箱の中の`変更前→変更後`で内容も示す |
| 差分ラベルなし | 色なし | 触らないもの |

`［変更］` は変更箇所を示す目印であって、それだけでは変更内容を表しません。フロー図と変更影響グラフでは、同じ箱に「条件2本→4本」「手段2種→3種」のような変更前後も書きます。たとえば、変更前の構造では既存の複数箱に変更が広がり、完成後の構造では新規の箱へ差分が閉じる、という形で図が変わります。**変更の箱が減っていれば、既存コードの修正箇所が減ったということです。** 端末や印刷で色が分かりにくくても、文字と枠で差分を読めます。
...
>
> **成果物：** 問題が起きた理由を示す原因ID一覧

### このフェーズの考え方

フェーズ3では、変更で実際に直した範囲を問題として確定しました。ここでは、**なぜその範囲まで直したのか**を調べます。

本書でいう**責任**は、同じ業務上の理由で一緒に変わる仕事のまとまりです。フィールドやメソッド、今回増えた作業の数には合わせません。たとえば、ある業務ルールの適用条件と結果の計算が同じ方針変更で一緒に変わるなら、まとめて「業務ルール」という一つの責任として扱います。処理が増えただけでは、新しい責任が増えたとは判断しません。別の業務上の理由で変わる仕事が生まれたときだけ、別の責任として捉えます。
...
問題が起きたコードと変更影響は、フェーズ3ですでに特定しています。フェーズ4では、その結果を一つの流れで分析します。

1. 問題が起きたクラスを、同じ業務上の理由で変わる責任に分けて見ます。
2. その責任を一つの部分クラス図へ置き、直接変わった責任と巻き込まれた責任を説明します。
3. その配置が問題IDの変更影響を生んだと追跡できた関係だけに、原因IDを付けます。

複数の責任があるだけでは、直ちに問題とは言えません。部分クラス図をフェーズ3の変更影響と突き合わせ、「変更理由の異なる責任が同じ場所にあるため、別の責任まで修正・確認対象になった」と言えたとき、初めて原因として確定できます。この図が、フェーズ5で責任配置を変える前の比較元になります。
...
フェーズ5の図には、「販促方針」「状態方針」のような**責任名**を使います。フェーズ6では、その責任配置をクラス／インターフェース名、具体クラス名、操作名、生成場所へ順に変換します。
