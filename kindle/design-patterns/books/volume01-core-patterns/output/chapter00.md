...
たとえば、請求書発行システムなら「税率や端数処理が変わる」「PDFとCSVの出力形式が増える」「承認済みだけ発行できる」といった変化軸を見るかもしれません。倉庫管理システムなら「在庫引当ルール」「通知先」「外部配送API」が変化軸になるかもしれません。題材が違っても、見る順番は同じです。
...
> - **「変化軸」**：「この種類の変更が来る」という、変更が起きる方向のこと。たとえば「割引ルールが増える」「状態が増える」「通知先が増える」は、それぞれ別の変化軸です。フェーズ2でリスクIDごとに見立て、フェーズ6でどの軸に沿って分離するかを決めます。1つのコードに別の軸が2本通っていれば、それが混在です。
