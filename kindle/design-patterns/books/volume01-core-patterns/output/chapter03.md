...
- **現行要求**：出庫後在庫が閾値以下なら通知する
- **受入条件**：閾値到達時だけ不足通知が発生する

**要求ID3**

- **現行要求**：出庫後在庫が閾値以下になった時点で、システム構成として保持する全通知先へ同じ在庫イベントを送る
- **受入条件**：メール・ダッシュボード・チャットが各1件受け取る
...
- **変更種別・根拠となる変更ID**：継続<br/>根拠: —
- **変更後要求**：出庫後在庫が閾値以下なら通知する
- **受入条件**：閾値到達時だけ不足通知が発生する

**要求ID3**

- **変更種別・根拠となる変更ID**：変更<br/>根拠: 変更ID1
- **変更後要求**：登録済み通知先へ在庫イベントを送り、非同期SMSの受付結果と後日確定する最終配信結果を記録する
- **受入条件**：全4手段の受付を集計し、SMS受付IDが保留から配信完了または配信失敗へ更新される
...
- **変更前の要求（現行）**：出庫後在庫が閾値以下になった時点で、システム構成として保持する全通知先へ同じ在庫イベントを送る
- **変更後の有効要求**：登録済み通知先へ在庫イベントを送り、非同期SMSの受付結果と後日確定する最終配信結果を記録する
- **根拠変更ID**：変更ID1
...
- **変わる見込み**：はい
- **変えられるようにする部分**：通知手段ごとの送信処理
- **今回維持する部分**：在庫更新、閾値判定、通知データ

**リスクID2：通知先の増減（動的な登録・解除）**

- **変わる見込み**：はい
- **変えられるようにする部分**：通知先の構成と登録
- **今回維持する部分**：出庫処理の入口、登録済み通知先を順に呼ぶ流れ

**リスクID3：通知先ごとの受付と最終配信結果、失敗の扱い**

- **変わる見込み**：はい
- **変えられるようにする部分**：通知先ごとの受付結果と非同期完了履歴
- **今回維持する部分**：通知失敗で在庫更新と他通知を止めない規則
...
- **接続するもの・変わる側**：**接続:** 商品ID・名称・更新後在庫・閾値と個別配送結果<br/>**変わる側:** 通知種類・文面・同期／非同期・受付成否
...
- **実行シナリオ・観測結果・判定**：商品IDと変更前→変更後の数量を出力<br/>**判定:** 合格

**要求ID2**

- **最終要求**：出庫後在庫が閾値以下なら通知する
- **適用コード**：`InventoryManager::reduceStock()`
- **実行シナリオ・観測結果・判定**：閾値到達時だけ不足通知<br/>**判定:** 合格

**要求ID3**

- **最終要求**：登録済み通知先へ在庫イベントを送り、非同期SMSの受付結果と後日確定する最終配信結果を記録する
- **適用コード**：`INotification`一覧、`SMSNotifier`、`DeliveryStatusLog`、`SMSDeliveryCallback`
- **実行シナリオ・観測結果・判定**：行2で全4手段を集計し `SMS-1` を保留→配信完了、行7で `SMS-2` を保留→配信失敗へ更新<br/>**判定:** 合格

**要求ID4**

- **最終要求**：未登録商品・不正数量・在庫不足を従来どおり拒否し、個別通知失敗を在庫更新・他通知から局所化する
- **適用コード**：在庫検証、`notifyAll()`、`SMSDeliveryCallback`
- **実行シナリオ・観測結果・判定**：不正操作では在庫と通知を変えず、行6の受付失敗・行7の最終配信失敗でも在庫更新と他通知が完了<br/>**判定:** 合格

**要求ID5**

- **最終要求**：在庫の内部数値変化を実行ログへ残す
- **適用コード**：`StockEvent`、`StockEventLog`
- **実行シナリオ・観測結果・判定**：在庫変動ログ8行に商品ID・変更前→変更後・単位が並ぶ<br/>**判定:** 合格
