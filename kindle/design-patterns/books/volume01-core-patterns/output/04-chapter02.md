...
> [!NOTE] この動作例は、第0章で説明した模擬環境で動きます。`EventDatabase db;` はクラス定義ではなく、オブジェクトの生成です。このときコンストラクタがイベント名・定員・予約数をメモリへ登録します。`main()` はイベントIDと操作を渡し、検索・状態更新・表示を直接呼びます。
>
> **実システムと掲載コードの違い**
>
> 実際の予約システムでは、イベントと予約はデータベースに保存され、支払いは決済サービスを通り、操作は予約画面から届きます。掲載コードでは、そこを次の4つが代わりに務めます。**この先に出てくる `db`・`seat1`・`pay()` は、どれもその代役です。**
>
> | 実システムでの動き | 掲載コードでの表現 |
> |---|---|
> | イベント情報をDBから取得 | `EventDatabase` がメモリ上の表で持つ |
> | 予約1件をDBの行として持つ | `TicketReservation` のオブジェクト1個で表す |
> | 決済サービスで支払う | 状態上許可された `pay()` を決済成功として扱う |
> | 操作と結果を予約画面でやり取りする | `main()` が操作を渡し、結果は標準出力へ出す |
>
> 代役にしたぶん、DB接続、同時予約制御、決済API、発券処理、画面は掲載コードにありません。**ただし、状態遷移と予約数の増減は実際に行い、数値で確認します。** この章で追うのは、予約状態に応じて振る舞いが切り替わる部分だからです。
...
**出力のイベント名「春の音楽祭」と「20/100」（予約数/定員）は、`main()` が渡した値ではありません。** `main()` が渡したのはイベントID `"EVT001"` だけです。イベント名・予約数・定員は、準備の行で作ったイベント台帳（`EventDatabase`）から、このIDをキーに引いた値です。今回参照したのは、次のデータです。
...
> 最初に置き換えた境界はそのままです。ここでは、**今回の変更で新しく増える境界**だけを足します。
>
