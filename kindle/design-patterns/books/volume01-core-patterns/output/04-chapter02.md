...
| 種別 | 要求ID | 今回確定した内容 |
|---|---|---|
| 変更 | 要求ID1（予約対象と空席の確認） | 空席数の表示と未登録拒否は保ち、満席時だけ予約数を増やさず待機登録へ切り替える |
| 変更 | 要求ID3（購入の確定） | 通常予約だけでなく、一時保留した席も支払いを完了できる |
| 変更 | 要求ID4（空席の引き継ぎ） | 席を空け、待機者がいれば同じ処理で先頭へ引き継ぐ |
| 追加 | 要求ID6（順番待ち） | 満席時も通常の予約操作から順番待ちへ参加できる |
| 追加 | 要求ID7（支払期限と一時保留） | 標準15分または選択した24時間だけ席を確保し、期限切れなら席を戻す |

要求ID2（席の確保）・要求ID5（誤操作からの保護）は変更前と変更後が同じなので、この表には出てきません。
...
変更後の仕様を、現状の仕様と同じ粒度で、正常系の入力・判定・加工・出力として確認します。**もとからあって中身が変わったノードは、青い枠とノード内の `［変更］` で示します。** この図には新しい箱はありません。増えるのは箱の中身です。差分は、内部に保持する「現在状態」が3種類から5種類へ、「操作・イベント」が3種類から7種類へ増えることです。満席のときのキャンセル待ち登録は新しい操作ではなく、「予約する」を満席のときに呼んだときの行き先が増えたものです。判定・加工・出力の流れ自体は変わりません。図中のイベントIDも、現状データに存在する3件を省略せず示します。
...
変更を試した結果、2本の変更要求がどのクラスのどの部分まで届いたかを図にします。**開いて修正するノードは、青い枠と `［変更］` の両方で示します。**
...
フェーズ3「問題特定」で確認した「状態追加のたびに条件分岐が増え、修正漏れの可能性が高まる」という痛み。このフェーズでは、状態ごとの決まりを**誰が持っているか**を確かめます。
...
ここでいう責任は「操作メソッドを持つこと」ではなく、**どの業務判断を行い、そのためにどの状態を持ち、何を結果として約束するか**というまとまりです。現在の`TicketReservation`は、予約1件の現在状態を保持するだけでなく、状態名を判定し、操作の可否、次状態、席数の増減まで決めています。

変更を当てると、このうち状態別の可否・遷移・副作用へ手が入り、利用者が同じ予約へ操作を依頼する入口と、予約1件が現在状態を一つ持つという考え方には手が入りませんでした。責任を変える方向を決めるため、**どの判断が同じ変更理由で動いたか**を次の表で分けます。

| このコードが持っているもの | 変更を当てたとき | どちら側か |
|---|---|---|
| 予約・支払い・取消という利用者の入口 | 触っていない | **守る** |
| 「今の状態がある」という考え方そのもの | 触っていない | **守る** |
| イベント台帳の席数を読み書きする | 触っていない | **守る** |
| 状態の種類 | 3つから5つへ増やした | **変える** |
| 状態ごとに何ができるかの判断 | 4つの公開操作へ `else if` を足した | **変える** |
| 状態を動かす入力の種類 | 期限切れ・昇格を足した | **変える** |
...
つまり守りたいのは「予約1件が現在状態を持ち、同じ公開操作から処理を依頼できること」です。切り離す候補は、状態名を読んで可否・次状態・席数更新を決める責任と、イベントごとの待機順を管理する責任です。責任を移すと、予約本体・状態別処理・待ち行列は再び情報を交換する必要があります。次の課題定義では、その接続点で渡す操作と共有データを検討します。

### 4-3：公開操作が、状態ごとの決まりをどこまで知っているか

ここでは原因から対策へ飛ばず、`TicketReservation` と状態ごとの判断の間で何を受け渡せばよいかを確認します。利用側が必要とするのは、状態名や分岐条件ではなく、現在の状態に対して操作を依頼し、結果を受け取ることです。

`TicketReservation` の各操作メソッドは、`status` の文字列と状態ごとの遷移ルールを知っています。`TicketReservation` がすべきことは「今の状態へ操作を渡すこと」だけのはずですが、**状態の名前と、状態ごとの可否判断まで自分で持っています。**
...
>
> 答えは「ある」です。状態の可否・遷移と待機順・昇格は、別々に変わるのに`TicketReservation`へ集まっています。

## 🟡 フェーズ5：課題定義 ―― 原因から課題を検討して確定する

### 5-1：どこに線を引けるかを挙げる

原因は2つ確定しました。`TicketReservation` が**状態ごとの可否判断**（`status == "Reserved"`）を4つの公開操作に散らして持っていることと、**待機順の管理**が状態処理に絡んでいることです。

**線の引き方は1通りではありません。** どこで切るかで、公開操作に何が残るかが変わります。

| 線の引き方 | `TicketReservation` に残るもの | 状態を1つ足すとき |
|---|---|---|
| A 状態の名前を列挙型にする | 判断そのものは4操作に残る | 4操作の `switch` へ枝を足す |
| B 判断を1つのメソッドへ集める | 集めたメソッドに全状態×全操作 | その1つを開いて全組み合わせを直す |
| C 状態ごとに1つの部品へまとめる | 「今の状態へ渡す」だけ | 部品を1つ作る |
| D 何もせず `else if` を足し続ける | 全部 | 4操作を横断して直す |

**AとBは、判断の置き場所を変えるだけです。** Aは文字列比較が型比較になるだけで、状態が増えれば枝も増えます。Bは4か所が1か所になりますが、その1か所が**状態数×操作数**の表になり、フェーズ3で数えた組み合わせの点検はそのまま残ります。

**Cだけが、状態を足すときに既存を開かずに済みます。** 新しい状態の部品を1つ作れば、公開操作も他の状態も触りません。

ただし、Cにしても**待機順の話は解けません。** 席が空いたときに誰を昇格させるかは、どの状態にも属さない別の関心事です。ここは別の線として追います。

### 5-2：候補を絞る

A・B・Dは、完成構造の候補として比較しません。列挙型への変更は型を安全にするだけで分岐を残し、一メソッドへの集約は確認場所を一か所へ寄せるだけで状態数×操作数の組み合わせを残します。何もしなければ、フェーズ3の痛みがそのままです。いずれも原因を解き切らない途中状態です。

原因を解く線はCですが、Cの内側には二つの変更理由があります。状態が増えたときに変わる「状態ごとの可否と遷移」と、運用方針が変わったときに変わる「待機順と自動昇格」です。同じ理由で変わる可否と遷移は状態単位にまとめ、別の理由で変わる待機順は分けます。状態だけ分けて待機順を予約本体に残すと、利用側が昇格を手で呼ぶ痛みが残るため、二つの境界をどちらも課題として確定します。

### 5-3：課題IDと接続点を確定する

| 課題ID | 変わる側と守る側をどう接続するか | 完了条件 |
|---|---|---|
| 課題ID1（状態固有動作の境界） | 状態別の可否・遷移へ現在状態とイベントを渡し、結果を公開入口・席数へ返す | 状態追加が新状態と遷移登録に閉じる |
| 課題ID2（待ち行列の境界） | 待ち順の管理へ追加・先頭取出しを任せ、席解放直後の昇格へつなぐ | 取消・期限切れから自動昇格までシステム内で完了する |

**2つとも解けたとき、システムはこうなっています。** 公開操作は現在状態へ処理を渡すだけになり、取消や期限切れで空席が出たときは、保存された待機順に従って自動で昇格します。利用側が昇格を呼ぶ必要はありません。
...
課題ID1（状態固有動作の境界）と課題ID2（待ち行列の境界）を、1本のコード経路にします。
...
| 生成・所有・受け渡し | 状態取得関数が状態を静的所有し、`BatchApplication`がDB・待ち行列を所有する | 具体状態の寿命と共有データ |
| 実行 | 利用側は公開操作だけを呼び、状態委譲と席解放後の昇格を内部で続ける | 現在状態と待機者選択 |

**構想上のコード経路**

1. 状態取得関数 ―― 状態実体を一つずつ生成・静的所有する
2. `BatchApplication` ―― DB・待ち行列を所有し、初期状態と共有資源を予約へ渡す
3. `TicketReservation::cancel()` ―― 公開入口から現在状態へ委譲する
4. 予約済み状態の `cancel()` ―― 席を解放する
5. `ReservationWaitlist` ―― 先頭の待機予約を返し、自動昇格へつなぐ

この経路が成立するには、契約だけでなく、具体を選ぶ方法、実体の生成・所有、利用側へ渡す行、公開入口からの呼び出しが同時に必要です。以下では、この構想を分断せず、コードの依存順に続けて確認します。

### 構想をコードでつなぐ

> **コードの読み方：** 「契約を決める → 具体状態が契約を満たす → 実体と共有資源を組み立てて渡す → 公開入口が現在状態から実行する」の順に読みます。これは設計を説明する順序です。実行時は、状態取得関数が初期状態または遷移先を返し、`TicketReservation` がその実体へ操作を委譲します。変更前の抜粋は境界を引く根拠、変更後の断片は構想を成立させるコードです。
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 状態ごとの可否は `reserve`・`hold`・`pay`・`cancel`・`expire` の5つの関数それぞれに `if (status == ...)` として現れます。そしてヒアリングで挙げたリスクID1（遷移ルール）、リスクID2（状態追加）、リスクID3（イベント追加）が、3方向とも「変わる・増える」と言っています。可否と遷移先は運用見直しで変わり、状態は新機能として増え、状態を動かす外部イベントはすでに複数あります。**現に変更要求で、保留中とキャンセル待ちの2状態が増えました。** そのため、各操作が同じ問い方で状態固有の判断を呼べるクラス契約まで分けます。
...
直前の図の白三角は**実線**です。`IReservationState` は不可操作の既定処理を持つため、契約だけのインターフェース実現ではなく、共通実装を含む継承です。次のコードで、基底の既定処理と `ReservedState` の上書きがこの関係に一致するかを確かめます。
...
**6つ並びます。** 利用者の操作が4つ（予約・支払い・取消・保留）、システム側の入力が2つ（空席発生による昇格・期限切れ）です。利用者操作とシステム入力のどちらも現在状態で振る舞いが変わるため、同じ状態契約に並べます。
...
>
> 本章は前者を選びます。解きたいのが「状態追加のたびに全メソッドを触る」痛みだからです。遷移元の近くへ次状態を置くため、変更範囲は狭まります。一方、遷移の全体像はクラスをまたいで確認する必要がある、というコストが残ります。
...
- **「操作対象を引数で渡す」の実態。** `reservation` を受け取り、次状態・席数・昇格をこのクラスが動かしています。**変更前の `cancel()` の `if` の中身が、そっくりここへ来ました**
...
直前の図では、待機順を持つのは `ReservationWaitlist`、空席ができたときに先頭を1件取り出して状態イベントを起こすのは `TicketReservation` と分けました。次のコードに状態名が出てこないことが、この分離の確認点です。
...
> **導入コスト：状態ごとにクラスが増える**
>
> このあと状態クラスが5つできます。`if` 文なら1つのファイルに収まっていたものが、5つに分かれます。状態名ごとの動作は追いやすくなる一方、全遷移を読むには複数クラスを開く必要があります。
>
...
**課題ID1（状態固有動作の境界）。** 直前の契約コードで見た `ReservedState` の疑問が、ここで解けます。**`if (status == ...)` を書かないのは、クラスであること自体が状態を表しているからです。** 契約の裏に置くのは「この状態で何が許されるか」「許されたとき次はどこで、何が起きるか」の2つだけです。
...
**`PaidState` には何も書きません。** 支払済みからは何もできないので、6操作すべてが既定のままです。**クラスは空でも要ります。** 「支払済みという状態が存在する」ことを型で表しているからです。

**課題ID2（待ち行列の境界）。こちらは直前の契約コードで見た `ReservationWaitlist` がそのまま完成形です。** 待機順を扱う以外に書くことがないので、増える段がありません。**実装も1つだけです。** 待機順の方針は今1つで、抽選や優先度つきに替える予定はヒアリングで挙がっていません。**替えられる形にしておく必要が無いので、契約を作らず、クラス1つで止めます。**
...
**守る範囲との照合：** **座席数更新**に触りました。呼ぶ場所が公開操作の中から状態クラスの中へ移っただけで、予約で1つ増やし取消と期限切れで1つ戻すことは変わっていません。**席解放直後に1件を昇格する契約**にも触りましたが、`ReservedState::cancel()`・`ReservedState::expire()`・`HeldState::cancel()`・`HeldState::expire()` の4か所すべてで `promoteNextWaitlisted()` を呼んでいます。変更を試したときは、取消の2分岐と、期限切れの `Reserved || Held` の1本で、同じ4つの遷移が昇格していました。席が戻る遷移と昇格する遷移は、変更前後で一致しています。
...
>
> 状態取得関数がメンバーを持たない五つの状態実体を生成・静的所有し、`TicketReservation`が現在状態へのポインタを持ちます。`BatchApplication`は座席数・待ち行列を所有して予約へ渡します。作った状態が公開操作から呼ばれ、遷移後も同じ共有資源へ戻れる経路を確かめます。
...
構想で定めた接続点は2つです。骨格の `state` に入る実体と、状態が呼び返す先（席数・待ち行列）です。ここでは、その所有と生存期間をコードで確かめます。
...
**次に、状態が呼び返す先を確認します。** 具体コードで `reservation->cancelSeat()`・`promoteNextWaitlisted()` と書きました。これらを実行するには、骨格が在庫・待ち行列を持っている必要があります。
...
**3つとも借用ポインタです。** 所有しません。**とくに `waitlist` は借りる必要があります。** 同じイベントの予約が全員で1つの待ち行列を見なければ、待機順が成り立たないからです。値で持つと予約ごとに別の行列ができます。

**では、待ち行列と在庫の実体は誰が持つのか。** 全予約より外側にいる者です。この章では組み立て役の `BatchApplication` がそれにあたります。
...
`db` と `waitlist` は `BatchApplication` の生成時にメンバーとして生成され、`run()` の各予約より先に存在します。状態実体はここにはなく、直前の状態取得関数が静的に所有します。

ここまでで、5つの状態、待ち行列、在庫の生成と所有を確認できました。次に `IReservationState*` を受け取る行を見て、同じ状態実体が予約から参照されることを確かめます。
...
前節で、`TicketReservation`へ初期状態と、座席・待ち行列の共有実体を渡しました。公開操作は現在の`IReservationState`へ委譲し、状態は必要な更新だけを予約へ返します。
...
**組み立ての行は増えました。** 予約を1件作るのに、状態・在庫・待ち行列の3つを渡します。**この対策が払ったコストは、依存を受け渡す引数が3つ増えたことです。** 増えたぶんは、状態を1つ足すときに触る範囲が新しいクラス1つと遷移元に収まることと引き換えです。
...
ここまでの部分クラス図で、現在状態への委譲・継承と、待ち行列の分離を別々に確定しました。ここでは課題との対応と将来リスクへの備えを表で確定し、部分図をシステム全体に統合した完成クラス図はフェーズ7で示します。
...
リスクID1（遷移ルール）〜リスクID3（イベント追加）により、状態分離が現在の要求だけの分岐整理ではなく、状態とイベントの将来変更をどこへ閉じるかという判断になっていることを確認できます。
...
この設計変更により、今回追加したキャンセル待ちと一時保留の判断は各状態クラスへ、待機順は `ReservationWaitlist` へ収まりました。既存の予約入口は具体状態を判定せず、同じ公開操作から実行できます。
...
変更要求で加わった期限切れは、**利用者が操作したわけではないのに状態が変わる**入力です。そこで期限を検知する実行役を置き、予約の `expire()` を呼びます。
...
フェーズ6の部分クラス図で確定したクラス・操作・関係線を、ここでシステム全体の完成図へ統合します。ここから2枚続く完成図では、**新規クラスを青のぬりつぶしと `<<new>>`、変更クラスを青い枠と `<<changed>>`**で示します。差分表示のないクラスは、現状から手を触れていません。

まず状態の差し替え部分です。`TicketReservation` が骨格、`IReservationState` が状態ごとの振る舞いの契約、各状態クラスがその具象に対応します。**枠は `TicketReservation` の1つだけで、残る6つはぬりつぶしです。** 現状クラス図にあったのは `TicketReservation` だけで、中身を入れ替えました。契約と5つの状態クラスは、いずれも新しく作ったものです。
...
色が付かないのは `EventDatabase` と `EventInfo` の2つです。**席数を持つイベント台帳は、状態の作り替えに巻き込まれませんでした。** `[予約数]` の数値ログも変更前から同じ形で出しているため、保存方法や在庫処理を設計変更に見せません。ぬりつぶしは3つ（待ち行列・期限監視・組み立て）で、いずれも新しく作ったクラスです。枠は `TicketReservation` の1つだけで、依頼先が状態契約へ変わりました。

この完成図では、`TicketReservation` が現在状態へ操作を委譲し、`IReservationState` が状態ごとの共通契約、5つの状態クラスがその具体動作を担います。掲載コードに登場する在庫・待ち行列・期限監視・組み立てクラスも省略せず記載しています。予約数の増減と待機者の自動昇格は状態処理から予約本体を経由して実行され、`BatchApplication` はシナリオを起動するだけです。
...
シナリオ本体は `BatchApplication::run()` にあるため、`run()` のコードと実行結果をシナリオごとに並べます。最後に、`main()` がこの入口を1回呼ぶところを示します。
...
次の1行は**前方宣言**です。この位置では `TicketReservation` の本体はまだ定義されていませんが、待ち行列はそのポインタを保持するだけなので、型名だけを先に知らせれば宣言できます。メソッドを呼ぶ処理は、`TicketReservation` の定義後に置きます。
...
ここには双方向の**参照**があります。`TicketReservation` は自分を登録し、空席発生時に次の待機者を取り出すため `ReservationWaitlist` を参照します。待ち行列は昇格対象を指すため `TicketReservation*` を保持します。ただし、どちらも相手を所有しません。`BatchApplication` が待ち行列を所有し、各予約より長く生かします。さらに予約のデストラクタが `remove(this)` を呼ぶため、待機中の予約が先に破棄されてもキューに無効なポインタは残りません。相互参照を隠すのではなく、必要な協調と寿命契約を明示した構造です。

---

**IReservationState**

新しい設計の基盤となる状態インターフェースを定義します。このインターフェースが「すべての状態クラスが守るべき契約」を定めます。C++では、基底クラスの仮想関数にデフォルト挙動（エラーメッセージを出力する、または何もしない）を実装しておくことで、各状態の具体クラスは自分に関係するメソッドだけをオーバーライドすればよくなります。引数の `TicketReservation*` は、直前に前方宣言した同じ型です。
...
状態オブジェクトは振る舞いだけを持つため、関数ローカルの静的オブジェクトを共有し、遷移のたびに `new` しない形にします。`state` はそれらへの非所有ポインタなので、`setState()` は参照先を切り替えるだけです。古い状態オブジェクトはプログラム終了まで共有され、削除対象になりません。待ち行列も予約を所有せず、登録中だけアドレスを借ります。そのため予約を移動・複製せず、破棄時に登録を外すところまでを契約に含めました。席数の増減（`reserveSeat`/`cancelSeat`）は、呼び出し側が手動で行うのではなく、状態遷移の副作用として各状態のメソッド内から予約コンテキスト経由で実行します。さらに席を解放した状態処理が `promoteNextWaitlisted()` まで呼ぶため、キャンセルと自動昇格が1つのシステム処理として完結します。
...
昇格した `waiting` をそのまま再取消できました。待機中に受け取った予約実体が昇格後も有効であり、同じイベントDBと待ち行列を参照して次の席解放まで完了することを、この数値変化で確認できます。

---

**シナリオ5b：保留の期限切れを起点にした自動昇格**

同じ `BatchApplication::run()` の中の続きです。シナリオ4は待ち行列が空だったため昇格が発動せず、要求ID7（支払期限と一時保留）の受入条件「期限切れ時は空席へ戻して待機者がいれば自動昇格する」の後半が未確認でした。ここでは待機者がいる状態で期限を切らせます。
...
| 要求ID6（順番待ち） | `AvailableState`、`WaitlistedState`、`ReservationWaitlist` | `reserve()` だけで待機登録し、先頭から昇格する |
...
> > `sources/chapter02/`
>
> **状態クラスどうしが互いを参照します**（`ReservedState` が `AvailableState` を名指しする）。状態ごとにファイルを分けると、前方宣言や相互インクルードの扱いが要ります。本章は5状態を一望できることを優先し、`States.h` と `States.cpp` にまとめています。
...
フェーズ3で当てた**同じ二つの変更ID**を、完成構造からキャンセル待ちと一時保留だけを除いた基準へ当て直します。**新規は青のぬりつぶしと `［新規］`、変更は青い枠と `［変更］` で示します。差分表示のないノードは触りません。**
...
|---|---|---|
| 変更ID1（キャンセル待ち） | 予約・取消・`main()` へ待ち行列処理を追加 | 取消直後に先頭を自動昇格し、共有資源は組み立て側が渡す |
| 変更ID2（一時保留） | 複数操作と `main()` へ期限処理を追加 | 各状態が期限切れを扱い、スケジューラから自動実行する |
...
| 問い | この章での答え |
|---|---|
| 問い1：別々の理由で変わるものはないか | 状態ごとの可否・遷移と、待機順・自動昇格が予約本体へ集まっていた |
| 問い2：何を約束すれば足りるか | 状態には六つの共通操作を置き、実装が一つの待ち行列には契約用の型を増やさなかった |
| 問い3：誰が作り、持ち、渡すのか | 状態取得関数が共有状態を持ち、`BatchApplication`が資源を予約へ渡した |
...
次のクラス図では、直前の対応表を本章のクラス名で描き直し、骨格の形が同じであることを見ます。待ち行列との関係は双方向の借用参照です。`TicketReservation` が登録・先頭取得を依頼し、`ReservationWaitlist` が待機中の予約を指します。所有者はどちらでもなく `BatchApplication` です。
