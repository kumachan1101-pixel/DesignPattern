...
| 要求ID | 現行要求 | 受入条件 |
|---|---|---|
| 要求ID1（予約対象と空席の確認） | 入力したイベントIDの登録有無と空席を予約前に確認する | 登録済みは予約数を表示。未登録・満席では状態と予約数を維持する |
...
| 要求ID5（誤操作からの保護） | 現在状態で許可されない予約・支払い・取消によって、確保済みの席を失わない | エラー前後の状態・予約数が一致する |
...
| ① | 利用者 → 予約処理 | 操作と対象の予約 |
| ② | 予約情報 → 予約処理 | 操作前の現在状態 |
| ③ | イベント情報 → 予約処理 | イベント名・定員・予約数 |
| ④ | 予約処理 → イベント情報 | 増減後の予約数 |
| ⑤ | 予約処理 → 予約情報 | 操作後の状態 |
| ⑥ | 予約処理 → 利用者 | 完了またはエラーの表示 |
...
|---|---|---|---|
| 要求ID1（予約対象と空席の確認） | 予約前にイベントIDの登録有無と空席数を確認し、満席なら待機登録する | 変更：満席は拒否から待機登録へ | 現在の予約数を表示。未登録は不変、満席は50/50のまま `Waitlisted` になる |
...
| 要求ID5（誤操作からの保護） | 許可されない操作（予約・支払・取消・保留・期限切れ）で席を失わない | 変更なし | エラー前後の状態・予約数が一致する |
| 要求ID6（順番待ち） | 満席時の予約依頼を待機列へ登録し、空席時に先頭から予約済みへ昇格する | 追加 | 利用者が予約を1回依頼すれば、待機登録と空席発生後の自動昇格が行われる |
| 要求ID7（支払期限と一時保留） | 決済期限内に支払い、期限切れなら席を空席へ戻して待機列の先頭へ引き継ぐ | 追加 | Reservedは15分、Heldは24時間。期限切れ後は先頭が自動昇格する |
...
既存の線は前の表と同じです。変更・追加した線だけを取り出すと、次のようになります。

| 番号 | どこからどこへ | 変わった受け渡し |
|---|---|---|
| ① | 利用者 → 予約処理 | 一時保留の操作が加わる |
| ⑤ | 予約処理 → 予約情報 | 一時保留・キャンセル待ちの状態が加わる |
| ⑦ | 時間経過 → 予約処理 | 支払期限を過ぎた予約を知らせる |
| ⑧ | 予約処理 → 待機順 | 待機登録または先頭の取り出しを行う |

増えた入口は⑦、保存データは⑧の待機順です。イベント情報と②〜④・⑥の受け渡しは変わりません。
...
この章では、予約を受け付け、記録し、席数を増減して結果を返す流れが**幹候補**、現在状態に応じた操作可否と遷移先が**変化点候補**に見えます。これはフェーズ1の要求と現状コードを重ねて得た見立てであり、状態ごとのクラスへ分けると決めたわけではありません。
...
変更を試したコードの差分から、追加・書き換えた場所をクラス名・関数名・変更内容つきで一箱ずつ書きます。同じ場所を複数の変更IDで直した場合は一箱へまとめ、回帰確認だけの場所は灰色にして点線でつなぎます。①の公開操作追加と②の状態判定は同じクラスでも別の仕事なので分け、変更ID1（キャンセル待ち）と変更ID2（一時保留）の両方で直した②は一箱にします。

```mermaid
graph TD
    subgraph TR["TicketReservation"]
        R1["① ［変更］ 変更ID2（一時保留）<br>公開操作へ hold()・expire() を追加"]:::touched
        R2["② ［変更］ 変更ID1（キャンセル待ち）・変更ID2（一時保留）<br>5つの公開操作へ Held・Waitlisted の判定を追加"]:::touched
        R3["③ ［変更］ 変更ID1（キャンセル待ち）<br>待機登録・先頭選択・昇格開始を追加"]:::touched
    end

    subgraph EDB["EventDatabase"]
        D1["④ ［回帰確認］<br>予約数の増減と空席判定<br>処理は変更なし"]:::keep
    end

    M["⑤ ［変更］ 変更ID1（キャンセル待ち）・変更ID2（一時保留）<br>main() へ待機・保留・期限切れの入力を追加"]:::touched
...
①〜③はすべて `TicketReservation` の中です。公開操作、状態ごとの可否と遷移、イベント別の待機順という異なる情報を見る処理が同居し、状態追加で `reserve()`・`pay()`・`cancel()`・`hold()`・`expire()`・昇格処理まで横断して変わりました。④の `EventDatabase` は呼ばれますが、予約数の保存方法は変わっていません。

### 3-3：痛みの言語化

痛みは、**理想と現状の差**として書きます。

変更前の `TicketReservation` が担うべき中心は、利用側から予約・支払い・取消を受け付け、その処理を実行することです。依頼は「キャンセル待ちと一時保留という状態を2つ足すこと」です。**理想は、この受付方法を変えず、新しい2つの状態の振る舞いと待機順の仕組みだけを追加できること**です。既存の予約可能・予約済み・支払い済みの振る舞いには触りません。

**ところが実際には、** 既存の予約・支払い・取消の判定を横断して書き換えました。待機順を足すためには、予約本体が持つデータと席解放の処理まで書き換えることになりました。

**この差が問題です。** 変更影響グラフの①と②では、新しい状態を足すために既存の操作入口と状態判定を横断して修正しています。③では待機順と昇格も同じ `TicketReservation` に加わりました。状態を二つ足す依頼が、既存操作の書き換えと待機処理の同居を必要としました。
...
変更影響グラフの番号を使い、各場所が何を見て動くかと、該当実装を対応させます。原因を調べる中心は、同じ `TicketReservation` に入っている①〜③です。④と⑤は、維持する別クラスと入力の入口を区別するために残します。

| 図の番号 | その場所が見ている情報 | 該当実装 | 責任としての読み方 |
|---|---|---|---|
| ① | 受け付ける操作の種類 | `reserve()`・`pay()`・`cancel()`・`hold()`・`expire()` | 公開操作の受付・実行調整 |
| ② | 予約の現在状態 | 各公開操作にある状態名の `if` | 状態別の操作可否・遷移 |
| ③ | イベント別の待ち行列 | 待機順の保持と先頭の昇格 | イベント別の待機順・先頭昇格 |
| ④ | イベントの予約数と定員 | `EventDatabase` | 予約数の増減・空席判定。別クラスで維持 |
| ⑤ | 操作と期限イベント | `main()` | 変更要求を渡す入口。混在の判定対象外 |

①〜④がコードのどこに現れるかを、変更を試したあとの `reserve()` で見ます。
...
void TicketReservation::reserve() {
    ...
    if (status == "Available") {        // ② 状態別の操作可否・遷移
        if (db.hasCapacity(eventId)) {  // ④ 予約数・空席判定
            db.reserveSeat(eventId);    // ④ 予約数の更新
            status = "Reserved";
        } else {
            status = "Waitlisted";
            waitlist.push_back(this);   // ③ 待機順・先頭昇格
        }
    } else {
        handleReserveError();           // ② 状態別の拒否
    }
}
```

`pay()`・`cancel()`・`hold()`・`expire()` も同じ形です。どれも先頭で `status` を見て、許される操作かどうかを自分で判定しています。表の②が、五つの公開操作へ散っている状態です。
...
| 図の番号 | 責任 | 見ている接続情報 |
|---|---|---|
| ① | 公開操作の受付・実行調整 | 受け付ける操作と、状態・待ち行列・席数をつなぐ順序 |
| ② | 状態別の操作可否・遷移 | 予約の状態 |
| ③ | イベント別の待機順・先頭昇格 | 待ち行列 |
| ④ | 予約数の増減・空席判定 | `EventDatabase` の予約数 |

**「どれも予約の話だから、責任は一つではないか」と感じるかもしれません。** 責任は、依頼された時期や担当者ではなく、変更理由と参照する情報で分けます。公開する操作だけを変える場合、状態ごとの可否は変わらないことがあります。状態遷移だけを変える場合、待機順は変わらないことがあります。ところが現在は、片方だけを変えても同じ `TicketReservation` の骨格を横断して読み直し、修正します。だから、公開操作の受付・実行調整、状態別の操作可否・遷移、イベント別の待機順・先頭昇格を別の責任として扱います。
...
今回、状態別の操作可否・遷移とイベント別の待機順・先頭昇格は直接変わりました。公開操作の受付には一時保留と期限切れの入口が増えましたが、既存の予約・支払い・取消の入口の中にも状態の判定が書かれているため、入口ごとに修正・確認へ巻き込まれました。予約数の増減・空席判定は `EventDatabase` にあり、この混在には含まれません。

**原因ID1は、公開操作の受付・実行調整と状態別の操作可否・遷移が同じクラスにあることです。** 受付役が各状態の可否・遷移・副作用まで直接持つため、問題ID1（状態追加の横断修正）が起きました。

**原因ID2は、公開操作の受付・実行調整とイベント別の待機順・先頭昇格が同じクラスにあることです。** 受付役が満席時の接続だけでなく、待機順の保持、先頭選択、昇格開始まで持つため、問題ID2（待機処理で予約本体を修正）が起きました。

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 確認結果は「ある」です。公開操作の受付・実行調整、状態別の操作可否・遷移、イベント別の待機順・先頭昇格は別々の理由で変わりますが、`TicketReservation` の操作メソッドへ集まっています。
...
>
> - 原因ID1（状態判断の同居）：公開操作の受付・実行調整と、状態別の操作可否・遷移が同じクラスにある
> - 原因ID2（待機処理の同居）：公開操作の受付・実行調整と、イベント別の待機順・先頭昇格が同じクラスにある

### 5-1：原因をなくす責任配置を決める

**原因ID1への対応。** 状態別の操作可否・遷移を、公開操作の受付・実行調整から分けます。予約本体は操作を現在状態へ渡し、状態クラスが可否・次状態・席数への作用を決めます。

**原因ID2への対応。** イベント別の待機順・先頭昇格を、公開操作の受付・実行調整から分けます。待ち行列は満席時の昇格処理を受け取り、空席時に先頭の処理を実行します。

二つの原因を同時になくす配置を、そのまま図にします。`TicketReservation` には公開操作の受付と、状態・待ち行列・席数をつなぐ実行調整を残します。状態別判断と待機順は別の置き場所へ移し、予約数の増減・空席判定は現在の `EventDatabase` 側に残します。席が空いたときの昇格は利用側へ任せず、内部の接続で完了させます。
...
classDiagram
    %% provisional-role-diagram
    direction TB
    class ReservationEntry["公開操作の受付・実行調整"]:::current {
        <<維持する責任>>
        責任：公開操作の受付・実行調整
    }
    class StatePolicy["課題ID1の対象<br>状態別の操作可否・遷移"]:::separated {
        <<分ける責任>>
        責任：状態別の操作可否・遷移
    }
    class WaitPolicy["課題ID2の対象<br>イベント別の待機順・先頭昇格"]:::separated {
        <<分ける責任>>
        責任：イベント別の待機順・先頭昇格
    }
    class SeatCount["予約数の増減・空席判定"]:::stable {
    ...
    ReservationEntry --> StatePolicy : 操作を委譲し結果を反映する
    ReservationEntry --> SeatCount : 予約数を確認・更新する
    ReservationEntry --> WaitPolicy : 昇格処理を登録し空席時に実行する
...
黄色は `TicketReservation` に残す公開操作の受付・実行調整、青は外へ分ける責任、白は現在の置き場所を維持する予約数の増減・空席判定です。フェーズ3の①〜⑤は変更試行のコード箇所を追う番号でした。この目標図では別の連番を振らず、分離対象を課題ID1（状態固有動作の境界）と課題ID2（待ち行列の境界）で直接示します。

フェーズ2で見立てた幹候補は「公開操作の受付・実行調整」と「予約数の増減・空席判定」として残し、変化点候補は「状態別の操作可否・遷移」と「イベント別の待機順・先頭昇格」として外へ分けます。ここで初めて、候補だった見方が目標の責任配置になります。
...
直前の目標図で示した課題ID1（状態固有動作の境界）と課題ID2（待ち行列の境界）について、原因・構造変更・接続・完了の見分け方を一つの表にまとめます。
...
|---|---|
| 解く原因 | 原因ID1（状態判断の同居） |
| 構造の変更 | 状態別の操作可否・遷移を、公開操作の受付・実行調整から分ける |
| 守る責任 | 公開操作の受付・実行調整 |
| 接続 | 予約本体から現在状態へ操作を渡す |
| 反映する結果 | 状態側から次状態と席数・待機列操作を反映する |
| 完了条件1 | 公開操作に状態名の条件分岐がない |
| 完了条件2 | 可否・次状態・副作用を現在状態へ委ねる |
| 完了条件3 | 状態追加で全公開操作を修正しない |
...
| ---------- | -------------------------------------- |
| 解く原因  | 原因ID2（待機処理の同居） |
| 構造の変更 | イベント別の待機順・先頭昇格を、公開操作の受付・実行調整から分ける |
| 守る責任       | 公開操作の受付・実行調整 |
| 接続    | イベントIDと昇格処理を渡して待機登録する |
| 反映する結果     | 空席時に先頭を実行し、予約状態と予約数へ反映する               |
| 完了条件1 | 待機順の保持と先頭選択が状態分岐にない |
| 完了条件2 | 席が空くとシステム内から先頭が昇格する |
| 完了条件3 | 利用側に待機者を昇格させる操作がない |
...
|---|---|---|
| 問題ID1（状態追加の横断修正） | 原因ID1（状態判断の同居） | 課題ID1（状態固有動作の境界） |
| 問題ID2（待機処理で予約本体を修正） | 原因ID2（待機処理の同居） | 課題ID2（待ち行列の境界） |
...
フェーズ5では、状態別の操作可否・遷移とイベント別の待機順・先頭昇格を公開操作の受付・実行調整から分け、席解放から自動昇格までを内部でつなぐところまで決めました。ここからは、状態契約、具体状態、生成・共有、待ち行列、公開操作からの委譲、自動昇格の順にC++の構造へ変えます。

フェーズ5で決めた「公開操作の受付・実行調整」「状態別の操作可否・遷移」「イベント別の待機順・先頭昇格」「予約数の増減・空席判定」に、C++で実装する置き場所を割り当てます。この段階では新しい責任を増やさず、責任名とクラスの対応だけを確定します。
...
classDiagram
    ...
    ReservationEntry --> SeatCount : 予約数を確認・更新する
    ReservationEntry --> WaitPolicy : イベントIDと昇格処理を渡す
...
図の責任名はフェーズ5と同じです。`TicketReservation` は公開操作を受け、状態・待ち行列・席数の処理をつなぎます。`IReservationState` と各具体状態は状態別判断、`ReservationWaitlist` は待機順を担当します。状態から予約本体へ反映される状態切替や席数操作は、実行時の呼び戻しであって新しい静的依存ではないため、逆向きの矢印は描きません。待ち行列には予約オブジェクトではなく、昇格時に実行する処理を渡します。ここから、契約・操作・所有・受け渡しを部分クラス図とコードで確定します。完成クラス図はフェーズ7で確認します。
...
>
> フェーズ5で確定した「予約本体から現在状態へ操作を渡し、状態変更と席数・待機列操作を予約へ反映する接続」と「予約本体から待ち行列へイベントIDと昇格処理を渡し、空席時に先頭処理を実行する接続」を、C++の型と操作へ変えます。前者には共通の状態基底クラスを置き、後者には予約の型ではなく、昇格時に実行する関数オブジェクトを渡します。

状態ごとの可否と遷移は、`reserve()`、`pay()`、`cancel()`、`hold()`、`expire()` の五つの公開操作に分散しています。状態・遷移・イベントが増える見込みもあるため、関数の切り出しではなく、すべての状態依存操作を同じ形で呼べる状態契約まで分けます。
...
**`IReservationState`：状態で振る舞いが変わる操作の契約（代表3操作）**
...
完成する基底クラスには、利用者が起こす `reserve()`・`pay()`・`cancel()`・`hold()` と、システムが起こす `expire()`・`promoteBySystem()` を置きます。どれも現在状態によって振る舞いが変わるからです。上の抜粋では、関係を確認する代表として `reserve()`・`cancel()`・`promoteBySystem()` を示しました。許可されない操作は基底の既定処理へ任せ、呼ぶ側の状態判定をなくします。`ReservedState::cancel()` は席を戻し、受付可能状態へ切り替え、待機者を一件昇格させます。この具体処理の完成コードはフェーズ7で示します。
...
課題ID2（待ち行列の境界）で確定した接続は、「イベントIDと昇格処理を登録し、空席時に先頭の処理を実行する」です。C++では昇格処理を `std::function<void()>`、登録を `enqueue(eventId, promote)`、先頭実行を `promoteNext(eventId)` と表します。`ReservationWaitlist` が保持するのは実行可能な処理なので、その処理を作ったクラスも予約の内部状態も知りません。

次の部分クラス図は、**予約本体から待ち行列へイベントIDと昇格処理を渡す関係**だけを示します。状態クラスと生成者は省いています。
...
classDiagram
    ...
    class ReservationWaitlist:::added {
        <<new>>
        責任：イベント別の待機順・先頭昇格
        +enqueue(eventId, promote)
        +promoteNext(eventId)
    }
    TicketReservation --> ReservationWaitlist : イベントIDと昇格処理を渡す

    classDef added fill:#eaf2fb,stroke:#527aa3,stroke-width:2px,color:#172033;
    classDef touched fill:#fff7e6,stroke:#9a6b2f,stroke-width:3px,color:#172033;
```

`joinWaitlist()` と `promoteNextWaitlisted()` は利用者向けの予約操作ではありません。`TicketReservation` が状態クラス、待ち行列、席数台帳をつなぐための内部協調です。前者は自分の現在状態へ `promoteBySystem()` を渡す処理を作り、後者は空席発生時に待ち行列へ先頭実行を依頼します。待ち順の保持と先頭選択そのものは `ReservationWaitlist` に閉じているため、予約本体へ待ち行列の責任は戻りません。別イベントの待機者を昇格させないよう、行列はイベントIDごとに持ちます。完成実装はフェーズ7で確認します。
...
**状態が一つでも収まらなければ、その形は契約にしません。** 待ち行列の境界は継承関係ではありません。予約が待機列要素の一種なのではなく、予約が「昇格時に実行する処理」を作り、待ち行列がそれを持つ has-a の関係です。
...
次は生成と所有だけを決めます。図には `ReservationAssembly` が直接作って持つ三つの実体だけを載せ、状態契約や実行役は省きます。
...
`ReservationAssembly` が予約より長く生きる席数台帳と待ち行列を所有し、予約を作るときに二つの参照を渡します。状態実体だけはメンバーを持たない共有物なので、無名名前空間の状態取得関数が静的所有します。

##### 生成・所有：実体と所有者をコードで示す

五つの状態はデータを持たず、渡された予約だけを動かすため、各一実体を全予約で共有します。

**無名名前空間の `reservedState()`：共有状態の生成と所有**
...
`reservedState()` はクラスのメンバーではなく、翻訳単位内だけで使う自由関数です。五つの状態取得関数を同じ形で用意します。関数内 `static` は終了まで生存し、`state` は共有実体を借り、`setState()` で参照先だけを替えます。

**`TicketReservation`：3つの借用と状態切替**
...
**`ReservationAssembly::startReservation()`：共有実体の受け渡し**
...
**`BatchApplication::run()`：組み立て済みの共有実体から予約を1件作る行**
...
ここまでに決めたクラスを、満席の `EVT003` で既存予約を取り消し、待機中の先頭予約を自動昇格する経路へつなぎます。参加者を読みやすい数に抑えるため、取消と昇格を2枚に分けます。1枚目は、取消で席を戻し、同じイベントの待ち行列へ処理を渡すまでです。
...
sequenceDiagram
    ...
    S->>R: cancelSeat()
    R->>DB: cancelSeat(EVT003)
    DB-->>R: 50/50 → 49/50
    S->>R: setState(Available)
    S->>R: promoteNextWaitlisted()
    R->>WQ: promoteNext(EVT003)
```

2枚目は、待ち行列の判定と、登録してあった昇格処理による状態・席数の更新です。

```mermaid
sequenceDiagram
    participant WQ as ReservationWaitlist
    participant P as 先頭の昇格処理
    participant W as 待機TicketReservation
    participant WS as WaitlistedState
    participant DB as EventDatabase

    alt EVT003の待機列に登録あり
        WQ->>WQ: 先頭処理を取り出す（待機数 2 → 1）
        WQ->>P: 引数なしで実行
        P->>W: 現在状態へpromoteBySystem(this)
        W->>WS: promoteBySystem(reservation)
        WS->>W: reserveSeat()
        W->>DB: reserveSeat(EVT003)
        DB-->>W: 49/50 → 50/50
...
利用側が呼ぶのは `cancel()` だけです。取消状態が席を1つ戻したあと、待ち行列は同じイベントの先頭処理を実行します。その処理が待機中の予約へ届き、席数は49から50、状態はWaitlistedからReservedへ変わります。待ち行列は予約の型も状態も知らず、利用側による手動昇格もありません。フェーズ7では、この経路を完成コードとログで検証します。
...
部分図を完成図へ統合します。図は、状態、待機列との接続、境界データの3枚です。
...
共有する席数台帳・待ち行列・期限入力と予約は、`ReservationAssembly` が1か所で作って持ちます。待機中の昇格処理は `ReservationWaitlist` が関数オブジェクトとして所有します。予約は待機列要素を継承しません。
...
**待ち行列へ渡るのは、イベントIDと引数なしの昇格処理です。** 状態の名前も、何席空いているかも、この線には乗りません。`ReservationWaitlist` は先頭の処理を実行するだけで、予約の型や現在状態を問いません。だから状態が1つ増えても、待ち行列側の線は動きません。

`EventDatabase` と `EventInfo` は現状維持です。新しい待ち行列・期限監視・組み立て・実行役を含め、掲載コードの全クラスを3枚のどれかに載せています。フェーズ4で `TicketReservation` に混在していた三つの責任は、公開操作を受けて状態・待ち行列・席数処理をつなぐ `TicketReservation`、状態別に操作可否と遷移を決める各状態クラス、イベント別に待機順を保持して先頭処理を実行する `ReservationWaitlist` に分かれました。**違うものを見ている責任が分かれ、元の `TicketReservation` には公開操作の受付・実行調整だけが残ります。** 共有実体の生成・所有・受け渡しは `ReservationAssembly`、入力例の実行と表示は `BatchApplication` へ分かれています。
...
sequenceDiagram
    participant C as 既存TicketReservation
    participant WQ as ReservationWaitlist
    participant P as 待機列が所有する昇格処理
    participant W as 待機TicketReservation
    participant WS as WaitlistedState
    participant DB as EventDatabase

    C->>WQ: promoteNext(EVT003)
    Note right of WQ: EVT003の先頭処理を取り出す<br>待機数 2 → 1
    WQ->>P: 引数なしで実行
    P->>W: 現在状態へpromoteBySystem(this)
    W->>WS: promoteBySystem(reservation)
...
イベントごとの先着順キューです。予約オブジェクトではなく、先頭になったときに実行する昇格処理を値として所有します。
...
`ReservationWaitlist` は `TicketReservation` を参照しません。先頭がどのクラスかを判定せず、登録された `std::function<void()>` を実行します。予約は待機列要素を継承せず、待ち行列が昇格処理を持つ has-a の関係です。

昇格処理は `TicketReservation` の `this` を取り込むため、待機中の予約が先に破棄されてはいけません。本章では `ReservationAssembly` がすべての予約を `std::list` でアプリケーション終了まで所有し、この前提を保証します。実システムで待機列を永続化する場合は、関数オブジェクトではなく予約IDを保存し、保存先から予約を取得する境界が必要です。
...
`state` は共有状態への非所有ポインタです。待機登録時の関数オブジェクトは `this` を取り込むため、コピー・移動を禁止し、組み立て役が要素の場所を動かさない `std::list` で予約を所有します。席数更新と自動昇格は状態処理から連続して呼ばれます。
...
本章で組み立てた「状態分離構造」を、章題に掲げた **Stateパターン**の抽象的な骨格と照合します。完成コードを型へ当てはめて採用を正当化するのではなく、どの関係が対応し、どの条件では別の設計が適するかを確認します。
...
- **使うと良い：** 状態が複数あり、同じ操作でも状態ごとに可否・遷移先・副作用が変わる場合。状態やイベントの追加で複数の公開操作を横断して直しているなら、判断を状態クラスへ分ける効果があります。
- **使わない方が良い：** 状態が少なく、遷移も増えず、短い分岐を一か所で読める場合。クラスを分けると、単純な規則を複数ファイルへ追いかけるコストの方が大きくなります。
- **別の構造も検討する：** 遷移規則を運用データとして一覧編集したい場合。状態クラスへ遷移を分散させるより、遷移表やルールエンジンとして中央管理する方が、全体像と変更手順をそろえやすくなります。
- **Stateだけでは解けない：** 本章の待機順のように、複数の予約をまたいで管理する規則です。個々の予約状態とは別の責任なので、`ReservationWaitlist` のような専用の置き場所が必要です。

### 過剰適用になる例

「未処理」と「完了」の二状態しかなく、許可される操作も `complete()` 一つだけなら、状態クラスを二つ作る必要はありません。列挙値と一つの条件分岐の方が、状態と処理を同じ場所で読めます。

本章でStateを採用した根拠は、五つの状態に対して六つの操作があり、状態追加で既存の公開操作を横断して修正していたことです。判断基準は状態数だけではなく、**状態ごとの振る舞いが独立して変わり、現在の分岐へ変更が広がっているか**です。
