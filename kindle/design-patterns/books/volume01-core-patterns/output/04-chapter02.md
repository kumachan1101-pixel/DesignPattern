...
フェーズ3から受け取るのは、二つの問題IDと変更途中の `TicketReservation` です。ここでは対策を決めません。問題が起きた理由だけを調べます。
...
| 責任 | 変更前 | 変更後 |
|---|---|---|
| 状態方針 | Available・Reserved・Paidの可否と遷移 | Waitlisted・Heldを含む5状態の可否と遷移 |
| 待機方針 | 満席時は拒否し、待機者を保持しない | 満席申込を先着順で保持し、空席時に先頭を昇格 |
| 席数管理 | 定員と予約数を保存し、1席単位で増減 | 定員と予約数を保存し、1席単位で増減 |

状態方針と待機方針の規則が変わり、席数管理の規則は変わりません。ところが直前に確認したコードでは、三つをつなぐ処理が `TicketReservation` に集まり、状態方針と待機方針の変更で予約本体を横断しました。

### 4-3：原因を確定する

前の表で分かったのは、どの責任の規則が変わり、どの責任が維持されるかまでです。問題が起きた理由は、`TicketReservation` へ戻って確認します。

**原因ID1（公開操作が状態ごとの処理を持つ）。** 公開操作が、各状態の可否・遷移・副作用を直接持っています。この配置が問題ID1（状態追加の横断修正）を生みました。

**原因ID2（予約本体が待機順を持つ）。** 予約本体が、待機者の保持・待機順・先頭選択まで持っています。この配置が問題ID2（待機処理で予約本体を修正）を生みました。

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 答えは「ある」です。公開操作、状態固有のルール、待機順の方針は別々の理由で変わりますが、`TicketReservation` の操作メソッドへ集まっています。

原因は確定しました。次は、二つの原因をなくす分け方を決めます。コード上の手段はまだ決めません。

## 🟡 フェーズ5：課題定義 ―― 原因から課題を検討して確定する

入力は、原因ID1（公開操作が状態ごとの処理を持つ）と原因ID2（予約本体が待機順を持つ）です。

### 5-1：原因をなくす責任配置を決める

**原因ID1への対応。** 状態方針を予約の入口から分けます。予約の入口は操作を現在状態へ渡し、状態方針が可否・次状態・席数への作用を決めます。

**原因ID2への対応。** 待機方針を予約本体から分けます。満席申込を受け取り、空席時に次の予約を返します。

状態値を列挙型へ変えるだけでは公開操作の分岐が残り、状態方針だけを分けても待機順を予約本体へ残せば原因は消えません。また、利用側が昇格を別に呼ぶ形では自動運用になりません。そこで、変更試行後の同居構造を次の責任配置へ変え、席解放から昇格までは内部でつなぎます。
...
**目標：状態方針と待機方針を分けて自動昇格までつなぐ**

```mermaid
classDiagram
    %% provisional-role-diagram
    direction TB
    class ReservationEntry["予約の入口"]:::current {
        <<維持する責任>>
    }
    class StatePolicy["① 状態方針"]:::separated {
        <<分ける責任>>
    }
    class WaitPolicy["② 待機方針"]:::separated {
        <<分ける責任>>
    }
    class SeatCount["席数管理"]:::stable {
        <<維持する責任>>
...
黄色は既存の予約入口、青は外へ分ける責任、白は規則を維持する席数管理です。①と②は次の課題表に対応します。型名・状態クラス・生成場所はフェーズ6で決めます。
...
|---|---|---|
| ① 課題ID1（状態固有動作の境界）<br>解く原因：原因ID1 | 状態方針を予約の入口から分ける | 入力：操作の依頼<br>反映：状態変更・席数更新 |
| ② 課題ID2（待ち行列の境界）<br>解く原因：原因ID2 | 待機方針を予約本体から分ける | 入力：待機登録・先頭取出しの依頼<br>結果：次に昇格する予約 |
...
|---|---|---|
| 問題ID1：状態追加で複数操作を修正 | 原因ID1：公開操作が状態ごとの処理を持つ | 課題ID1：状態ごとの処理を公開操作から分ける |
| 問題ID2：待機処理の追加で予約本体を修正 | 原因ID2：予約本体が待機順まで持つ | 課題ID2：待機順を予約本体から分ける |
...
フェーズ5では、状態方針と待機方針を予約の入口から分け、席解放から自動昇格までを内部でつなぐところまで決めました。ここからは、状態契約、具体状態、生成・共有、待ち行列、公開操作からの委譲、自動昇格の順にC++の構造へ変えます。
...
| 課題 | 決定した構造 | コード上の実現 |
|---|---|---|
| 課題ID1（状態固有動作の境界） | 予約の入口が現在状態へ委譲する | `IReservationState`と各状態クラス |
| 課題ID2（待ち行列の境界） | 待機順を別に持ち、席解放から内部接続する | `ReservationWaitlist`と`onSeatReleased()` |
