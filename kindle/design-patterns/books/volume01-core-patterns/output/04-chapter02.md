...
呼び出される `EventDatabase::reserveSeat()` は、`TicketReservation::reserve()` がイベントの存在と空席を確認済みであることを前提に、予約数の加算だけを行います。未登録IDを直接渡すと `records.at(id)` が `std::out_of_range` を投げますが、公開操作の `reserve()` は存在確認で先に終了します。
...
|---|---|---|
| キャンセル待ち | `ReservationWaitlist` のメモリ上の先着順キュー | 永続化、同時操作、再起動後の復元 |
...
変更後の仕様を、現状の仕様と同じ粒度で、正常系の入力・判定・加工・出力として確認します。平行四辺形は入力、円柱形は保持値、四角は処理、ひし形は判定、両端が丸い箱は出力です。色に加えて `［変更］` と `変更前→変更後` の文字でも差分を判別できます。

もとからあって中身が変わったノードは、淡い黄の面・茶色の枠とノード内の `［変更］` で示します。この図には新しい箱はありません。増えるのは箱の中身です。差分は、内部に保持する「現在状態」が3種類から5種類へ、「操作・イベント」が3種類から6種類へ増えることです。満席のときのキャンセル待ち登録は新しい操作ではなく、「予約する」を満席のときに呼んだときの行き先が増えたものです。判定・加工・出力の流れ自体は変わりません。図中のイベントIDも、現状データに存在する3件を省略せず示します。

```mermaid
flowchart TB
    A[(［変更］ 検証済み予約<br>3状態 → 5状態<br>Waitlisted・Heldを追加)]:::data --> B[イベント存在・空席・操作可否を確認]:::process
    C[/［変更］ 操作・イベント<br>3種類 → 6種類<br>予約昇格・一時保留・期限切れを追加/]:::input --> B
...
フェーズ3で試す変更と、そのとき維持する動作を確定します。状態に関する責任の分け方は、痛みと原因を確認した後のフェーズ5で確定します。ここでそろえるのは、変更を現状構造へ当てたとき、維持する動作を担うコードまで修正・確認が広がるかを観測する条件です。
...
| イベントIDと予約IDを受け付け、状態を保存する | 今回の変更でも必要な既存動作 | 動作を維持する |
| 席数とキャンセル待ちの順番を正しく管理する | 今回の要求を成立させる一貫した業務動作 | 動作を維持する |

フェーズ3では変更ID1（キャンセル待ち）・変更ID2（一時保留）だけを現状構造へ適用し、予約の受付・保存・席数・待ち順の動作を維持したまま、修正と確認がどこまで広がるかを観測します。リスクID1（遷移ルール）・リスクID2（状態追加）・リスクID3（イベント追加）は実装せず、フェーズ6へ構造評価の材料として引き継ぎます。
...
第0章で定めたとおり、この仮実装は対策案ではありません。変更IDを現状構造へ当て、修正箇所と連動する確認範囲を観測したら捨てます。ここで得た事実だけをフェーズ4へ渡します。
...
現状コードから増えたのは、待ち行列への参照、状態値2つ（`Held`・`Waitlisted`）、内部遷移2つ、エラー出力2つ、公開操作2つ（`hold()`・`expire()`）です。**公開操作は3つから5つになりました。** 試行コードでは、待ち行列へ登録する予約を同じ実行スコープの末尾まで生存させるため、格納したポインタはこの動作例の途中では無効になりません。
...
|---|---|---|
| 問題ID1（状態追加の横断修正） | Held・Waitlisted追加で4操作を修正した | 変更ID1・変更ID2 |
...
責任は、同じ業務上の理由で一緒に変わる仕事のまとまりです。問題が起きた `TicketReservation` をこの粒度で見ると、変更を試した時点では次の構造になっています。
...
予約操作の受付は公開操作を受け、状態方針は操作可否・遷移・副作用を決め、席割当方針は満席時の待機と空席時の昇格を決めます。今回、状態方針と席割当方針のルールは直接変わりました。受付に追加した保留・期限切れは要求上必要ですが、既存の予約・支払い・取消まで同じクラスにあるため横断修正・確認へ巻き込まれました。席数管理は `EventDatabase` にあり、この混在には含まれません。

**原因ID1（予約操作の受付と状態方針が同じクラスにある）。** 予約操作の受付が、各状態の可否・遷移・副作用を直接持つ配置が、問題ID1（状態追加の横断修正）を生みました。

**原因ID2（予約操作の受付と席割当方針が同じクラスにある）。** 予約操作の受付が、満席時の扱い、待機順、空席時の先頭選択まで持つ配置が、問題ID2（待機処理で予約本体を修正）を生みました。

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 答えは「ある」です。予約操作の受付、状態方針、席割当方針は別々の理由で変わりますが、`TicketReservation` の操作メソッドへ集まっています。

原因は確定しました。次は、二つの原因をなくす責任配置を決めます。

## 🟡 フェーズ5：課題定義 ―― 原因から課題を検討して確定する

入力は、原因ID1（予約操作の受付と状態方針が同じクラスにある）と原因ID2（予約操作の受付と席割当方針が同じクラスにある）です。

### 5-1：原因をなくす責任配置を決める

**原因ID1への対応。** 状態方針を予約操作の受付から分けます。予約操作の受付は操作を現在状態へ渡し、状態方針が可否・次状態・席数への作用を決めます。

**原因ID2への対応。** 席割当方針を予約操作の受付から分けます。満席申込を受け取り、空席時に次の予約を返します。

状態値を列挙型へ変えるだけでは公開操作の分岐が残り、状態方針だけを分けても席割当方針を予約操作の受付へ残せば原因は消えません。また、利用側が昇格を別に呼ぶ形では自動運用になりません。そこで、フェーズ4の対策前図を次の責任配置へ変え、席解放から昇格までは内部でつなぎます。

**目標：状態方針と席割当方針を分けて自動昇格までつなぐ**
...
classDiagram
    ...
    class StatePolicy["① 状態方針"]:::separated {
        <<分ける責任>>
        責任：状態方針
    }
    class WaitPolicy["② 席割当方針"]:::separated {
        <<分ける責任>>
        責任：席割当方針
    }
    class SeatCount["席数管理"]:::stable {
    ...
    ReservationEntry --> StatePolicy : 操作を渡す
    StatePolicy --> SeatCount : 席数を更新する
    StatePolicy --> WaitPolicy : 席解放を知らせる
    WaitPolicy --> StatePolicy : 次の予約を昇格する
...
黄色は既存の予約操作の受付、青は外へ分ける責任、白は規則を維持する席数管理です。①と②は次の課題表に対応します。型名・状態クラス・生成場所はフェーズ6で決めます。
...
二つとも満たせば、公開操作は現在状態へ処理を渡し、待機順は別の責任が管理します。席が空くと、利用側の追加操作なしで先頭が昇格します。

最後に、問題から課題までのつながりを確認します。1行が一つの原因なので、それぞれの問題がどの原因を経て、どの課題へ変わったかを横に追えます。

| 観測した問題 | 確定した原因 | 課題で目指す状態 |
|---|---|---|
| 問題ID1：状態追加で複数操作を修正 | 原因ID1：予約操作の受付と状態方針が同じクラスにある | 課題ID1：状態方針を予約操作の受付から分ける |
| 問題ID2：待機処理の追加で予約本体を修正 | 原因ID2：予約操作の受付と席割当方針が同じクラスにある | 課題ID2：席割当方針を予約操作の受付から分ける |
...
フェーズ5では、状態方針と席割当方針を予約操作の受付から分け、席解放から自動昇格までを内部でつなぐところまで決めました。ここからは、状態契約、具体状態、生成・共有、待ち行列、公開操作からの委譲、自動昇格の順にC++の構造へ変えます。
...
図の「予約操作の受付」「状態方針」「席割当方針」「席数管理」はフェーズ5と同じ責任名です。ここから、各責任を表す契約・操作・所有・受け渡しを部分クラス図と対応コードで確定します。完成クラス図はフェーズ7で確認します。

### 構想をコードでつなぐ

> **現在位置：** 契約 → 具体状態 → 生成・共有 → 公開操作からの委譲、の順で確認します。

#### 契約：状態方針と席割当方針の入出力をC++の型と操作にする
...
待機順は `ReservationWaitlist`、空席後の昇格開始は `TicketReservation` に置きます。`enqueue(eventId, reservation)` が待機登録を受け、`popNext(eventId)` が先頭の予約だけを返します。状態を知らないため、待機順の変更は `popNext()` 内へ閉じます。また、別イベントの待機者を昇格させないよう、行列はイベントIDごとに持ちます。完成実装はフェーズ7で確認します。
...
最後に、公開入口からは具体状態を指定せず、予約操作だけを呼ぶことを確認します。

**ここで確認するコード：`BatchApplication::run()`** ―― 予約と取消
...
公開入口は `TicketReservation`、席数管理は `EventDatabase` の既存契約を維持します。
...
`state` は共有状態への非所有ポインタです。待ち行列は予約のアドレスを借りるため、コピー・移動を禁止して登録中のアドレスが変わらないようにし、デストラクタで `remove(this)` を呼んで破棄済みの予約を残しません。組み立て役も、要素を追加しても既存要素の場所が動かない `std::list` で予約を所有します。席数更新と自動昇格は状態処理から連続して呼ばれます。
...
| 利用側に待機者を昇格させる操作がない | `main()`は取消・期限切れだけを実行する | 合格 |
