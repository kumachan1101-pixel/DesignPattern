...
問題はフェーズ3で確定済みです。責任一覧と `TicketReservation` の変更箇所へ戻り、問題を生んだ責任の組だけを取り出します。

| 問題ID | 関係する責任（変更の中心 → 影響した責任） | コード上の目印 |
|---|---|---|
| 問題ID1（修正漏れ） | 状態の追加・遷移 → 複数の公開操作 | `if (status == ...)` |
| 問題ID2（横断確認） | 一つの状態の振る舞い → 全操作の状態分岐 | `reserve()`・`pay()`・`cancel()` |
| 問題ID3（待機責任の散在） | 待機順と自動昇格 → 予約状態の遷移処理 | `waitlist`・`promoteNextWaitlisted()` |

状態や待機処理の責任自体が問題なのではありません。変更理由の異なる責任まで、同じクラスにあるため影響した関係を問題として見ます。
...
| 責任 | 変わるきっかけ | 今回 |
|---|---|---|
| 状態ごとの可否・次状態と追加操作 | 状態ルールと操作の変更 | **変える** |
| 待機順と自動昇格 | キャンセル待ちの運用変更 | **変える** |
| 既存の予約・支払い・取消の入口 | 予約サービスの利用契約の変更 | **守る** |
| イベント存在確認と席数保存 | イベント台帳の保存方法の変更 | **守る** |
