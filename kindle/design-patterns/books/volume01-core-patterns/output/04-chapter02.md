...
静的な関係に続けて、正常系（予約→支払い）で各操作がどの順に呼ばれ、状態がどう遷移するかを時系列で示します。状態ごとの具体の分岐は `TicketReservation` の注記に、エラー系はメモとして添えます。現状コードを読む前の地図です。
...
予約成功で件数を増やし、キャンセルで減らすという在庫の契約を次のコードで確認します。
...
次の図では、`［変更］` は開いて直す箱そのものの印、`（追加）` はその箱の中で今回加わる項目の印です。箱に付いた印で「どこを開くか」、箱の中の印で「何が増えるか」を読みます。
...
次のフェーズで試す変更IDと、そのとき動作を維持するものを分けておきます。

| 仕様・動作 | 判断根拠 | フェーズ3での扱い |
|---|---|---|
| 状態ごとの許可操作と遷移先 | 変更ID1・2。リスクID1から今後も見直しが見込まれる | 変更を試す |
| 状態の種類と固有の振る舞い | 変更ID1・2。リスクID2から今後も追加が見込まれる | 変更を試す |
| 期限切れ・空席発生と状態遷移の関係 | 変更ID1・2。リスクID3から外部イベントの追加も見込まれる | 変更を試す |
| イベントIDと操作を受け付け、予約ごとの状態を保持する | 今回の変更でも必要な既存動作 | 動作を維持する |
| 予約数を正しく増減し、キャンセル待ちを先着順に保つ | 今回の要求を成立させる一貫した業務動作 | 動作を維持する |

変更ID1（キャンセル待ち）・変更ID2（一時保留）だけを現状の構造へ当て、予約の受付・保存・席数・待ち順は動作を維持します。リスクID1（遷移ルール）・リスクID2（状態追加）・リスクID3（イベント追加）は実装せず、フェーズ6の構造評価へ引き継ぎます。
...
>
> このフェーズで見るのは、変更を当てたときに**どこまで開くことになったか**です。開いた場所を1つずつこの問いにかけ、依頼に直接必要な修正と、そうでない修正を分けます。新しい状態、待ち行列、期限切れ入力を足すこと自体は依頼に直接必要なので、痛みではありません。一つの状態を追加するために既存の複数操作を横断して判定を直すことと、待機順だけを変える場合にも状態分岐を開くことを痛みとして記録します。**なぜ関係のない場所まで開くのか**（責任の集まり方）は、次のフェーズ4で調べます。
...
- **変更ID1（キャンセル待ち）**では、`TicketReservation`の保持データと`reserve()`・`cancel()`を開き、満席時の待機登録と、席を解放した直後の自動昇格を加えます。
- **変更ID2（一時保留）**では、状態値と`pay()`・`cancel()`を開き、`hold()`・`expire()`・期限切れ入力を加えます。
...
変更を試した結果、2本の変更要求がどのクラスのどの部分まで届いたかを図にします。**開いて修正するノードは、淡い黄の面・茶色の枠と `［変更］` で示し、ノード内の`変更前→変更後`で修正内容も示します。**

```mermaid
graph TD
    T1["変更ID1：キャンセル待ち"]:::req -->|"待機登録・席解放・自動昇格"| A["［変更］ TicketReservation<br>3状態×3操作 → 5状態×6操作"]:::touched
    T1 -->|"共有待ち行列を生成・受け渡し"| E["［変更］ main()<br>予約操作の入力 → 待ち列・期限入力も追加"]:::touched
...
|---|---|---|
| 問題ID1（状態追加の横断修正） | Held・Waitlisted追加で、既存3操作の修正と新規2操作の状態判定が要った | 変更ID1・変更ID2 |
| 問題ID2（待機処理で予約本体を修正） | 待機順と自動昇格を足すために、予約本体の保持データと席解放処理を修正した | 変更ID1 |
...
責任にいきなり名前を付けるのではなく、**この題材で何がきっかけで書き換えが起きるか**を並べるところから始めます。今回の依頼から来た理由と、フェーズ2のヒアリングで分かった将来の理由を、第0章の変更理由の型で書き出します。

| 変更理由 | 第0章の型 | 出どころ |
|---|---|---|
| A：状態の種類が増える | 仲間が増える | 変更ID1・2、リスクID2 |
| B：状態ごとの操作可否・遷移先が変わる | 規則が変わる | 変更ID1・2、リスクID1 |
| C：待機者の並べ方・選び方が変わる | 選び方・順序が変わる | 変更ID1 |
| D：受け付ける操作・イベントが増える | 入口が増える | 変更ID2、リスクID3 |

次に、粒度を一段下げます。直前では「クラスと組み立て箇所の粒度で2か所」と数えましたが、同居を見るには `TicketReservation` の中で何を書き換えたかまで割る必要があります。そこで、`［変更］`が付いたクラスの中身を仕事ごとの行にします。入力を組み立てる `main()` は行にしません（どの実体をいつ作るかはフェーズ6で扱います）。今回影響のなかった `EventDatabase` の予約数更新は、同じクラスの中にある仕事との違いを見るために1行だけ並べます。行が仕事、列がさきほどの変更理由です。`○`は今回開いたところ、`△`は将来その理由が来たときに開くところです。

| 開いた仕事 | いまあるクラスと場所 | A | B | C | D |
|---|---|---|---|---|---|
| 操作とイベントの入口を公開する | `TicketReservation` の公開メソッド | | | | ○ |
| 現在状態で可否を判定し、次状態へ移す | `TicketReservation` 各公開メソッドの `if` | ○ | ○ | | △ |
| 満席のときの扱いを決める | `TicketReservation::reserve()` の中 | ○ | ○ | ○ | |
| 待機者を並べ、先頭を選んで昇格させる | `TicketReservation` の待ち行列と昇格処理 | | | ○ | |
| 予約数を増減する | `EventDatabase` | | | | |

読み方は四つです。

1. **同じクラスの行に、別々の列の印が付いている**なら、そのクラスに複数の責任が同居しています（混在）。上の4行はどれも `TicketReservation` の中にあり、A・B・C・Dの印が1つのクラスへ集まっています。分ける対象はここです。
2. **一つの行に印が2つ以上**あるなら、その仕事自体が複数の理由で変わります。可否と遷移の行がAとB、満席の扱いの行がA・B・Cです。
3. **ある列の印が1行に収まっている**なら、その理由には専用の置き場所があります。今回そう言えるものはありません。
4. **どの列にも印がない行**は、今回は変わりません。`EventDatabase` の予約数の増減がこれにあたります。別のクラスにあるため、上の4行を直すときに巻き込まれることもありません。

列は「なぜ変わるか」、責任は「その理由で一緒に変わる仕事が何をするか」です。列に仕事の側から名前を付け直すと、分けたあとの責任になります。AとBは「状態が増えれば、その状態の規則も決める」という形で必ず一緒に動くので、一つにまとめます。名前を付けると三つです。

| 責任 | 対応する変更理由 | まとめている仕事 |
|---|---|---|
| 予約操作の受付 | D | 操作と期限切れの入口を用意する／予約1件の状態を持つ |
| 状態別の操作可否・遷移 | A・B | 可否を判定する／次状態を決める／席数と待機列への作用を起こす |
| イベント別の待機順・先頭昇格 | C | 満席の予約を先着順で並べる／先頭を選んで昇格を依頼する |

**「どれも予約の話だから、責任は一つではないか」と感じるかもしれません。** 責任は、扱う題材ではなく書き換えるきっかけで分けます。題材は同じ予約でも、操作の種類・状態の規則・待たせ方は、別々の人が別々の時期に決めます。今回も、一時保留と、キャンセル待ちは別々の依頼として届きました。

**業務の判断と仕組みの骨格で二つに分ける見方もあります。** その線引きだと、BとCが同じ側に入ります。しかし今回は、Cだけを足したときにも状態分岐（B）を開くことになりました。別々に動く列を一つにまとめると、片方の変更でもう片方を開くことになります。ここでは、実際に別々に動いた三つを、そのまま三つの責任として扱います。
...
目指す形は、**どの置き場所も、一つの責任のきっかけだけで開くようになること**です。同じ表で言えば、行ごとに印が一つの責任へ閉じ、別の責任の理由では開かない状態です。原因は確定しました。次は、二つの原因をなくす責任配置を決め、その形になるかを確かめます。
...
この配置になると、フェーズ4の表の印は次のように変わります。列はフェーズ4と同じ変更理由です。

| 分けたあとの置き場所（責任） | A | B | C | D |
|---|---|---|---|---|
| 予約操作の受付 | | | | ○ |
| 状態別の操作可否・遷移 | ○ | ○ | | |
| イベント別の待機順・先頭昇格 | | | ○ | |
...
フェーズ4では、A・B・C・Dの印が `TicketReservation` の1クラスへ集まっていました。分けたあとは、行ごとに自分の責任のきっかけだけが残ります。状態を1つ足すとき（A・B）に開くのは2行目、待たせ方を変えるとき（C）は3行目、操作を増やすとき（D）は1行目だけです。型名・状態クラス・生成場所はフェーズ6で決めます。
...
`PaidState` は全操作を拒否する終端状態なので空のクラスです。`IWaitlistEntry` は待ち行列の差し替え用ではなく、`ReservationWaitlist` から `TicketReservation` の具体型を隠し、先頭昇格だけを許す境界です。
...
`EventDatabase` と `EventInfo` は現状維持です。新しい待ち行列・期限監視・組み立て・実行役を含め、掲載コードの全クラスを図に載せています。フェーズ4で `TicketReservation` に混在していた三つの責任は、完成図では、予約操作を受けて現在状態へ渡す `TicketReservation`、状態別に操作可否と遷移を決める各状態クラス、イベント別に待機順を保持して先頭を昇格させる `ReservationWaitlist` に分かれました。待ち行列は `IWaitlistEntry` だけを呼ぶため、予約本体の具体型を知りません。共有実体の生成・所有・受け渡しは`ReservationAssembly`、入力例の実行と表示は`BatchApplication`へ分かれています。対象とした変更理由と組み立て以外の処理を、一つのクラスが同時に持つ構造ではなくなりました。
...
要求の受入とは分けて、フェーズ5の完了条件を完成コードと実行結果へ照合します。1行が一つの完了条件で、左から条件、証拠、判定の順に読みます。
