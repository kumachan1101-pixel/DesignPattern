...
>
> この章では、遷移元の状態クラスが次の状態を名指しします。状態ごとにクラスを作り、「予約済み」を受け持つクラス（`ReservedState`）の中に「次は「予約可能」を受け持つクラス（`AvailableState`）」と書く形です。素直ですが、**遷移の全体像がコードのあちこちに散ります。** 「この状態からどこへ行けるのか」を知りたいとき、全クラスを開くことになります。
>
...
- **課題ID1** ← 原因ID1（現在状態の判断と、処理フローが同居している）← 問題ID1・問題ID2（状態を1つ足すと、複数の操作を横断して直し、確認することになる）
...
- **実行シナリオ**：50/50→49/50→50/50、手動昇格なし
- **判定**：合格
...
- **実行シナリオ**：シナリオ5で `reserve()` だけを呼び自動登録、先頭から昇格
- **判定**：合格
...
- **実行シナリオ**：シナリオ4aで通常期限、シナリオ4・5bで保留期限と自動昇格
- **判定**：合格
...
> **実務でファイルを分けるなら**
>
> 7-1の完成コードは1つの `.cpp` にまとめてあります。手元でそのまま動かせるようにするためで、実務ではここまでに決めた責任のとおりにファイルを分けます。
>
> | ファイル | 入れるもの | なぜそこか |
> |---|---|---|
