...
前の表で分かったのは、何を変え、何を守るかまでです。問題が起きた理由は、`TicketReservation` へ戻って確認します。
...
**原因ID1への対応。** 状態ごとの処理を、公開操作から分けます。公開操作は依頼を現在状態へ渡し、状態変更と席数更新を予約へ反映します。

**原因ID2への対応。** 待機順の管理を、予約本体から分けます。イベントと待機対象を渡し、次に昇格する予約を受け取ります。

席解放と自動昇格は分断せず、システム内でつなぎます。型・操作・生成場所はフェーズ6で決めます。

### 5-2：原因が残らないか確認する

`status` を列挙型へ変えるだけでは、原因ID1（公開操作が状態ごとの処理を持つ）は消えません。状態ごとの処理だけを分けても、待機順を予約本体に残せば原因ID2（予約本体が待機順を持つ）が残ります。また、昇格を利用側から別に呼ぶと、席の解放と自動昇格がシステム内でつながりません。

したがって、状態固有の判断と待機順の両方を予約本体から分け、席解放から先頭予約の昇格までは内部で接続する必要があります。これで二つの原因を同時に解き、手動運用も残しません。
...
|---|---|---|
| 課題ID1（状態固有動作の境界）<br>解く原因：原因ID1 | 状態ごとの処理を公開操作から分ける | 入力：操作の依頼<br>反映：状態変更・席数更新 |
| 課題ID2（待ち行列の境界）<br>解く原因：原因ID2 | 待機順の管理を予約本体から分ける | 入力：待機対象・追加／取出しの依頼<br>結果：次に昇格する予約 |
