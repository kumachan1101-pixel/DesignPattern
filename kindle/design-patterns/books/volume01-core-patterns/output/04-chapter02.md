...
| 要求ID | 現行要求 | 受入条件 |
|---|---|---|
| 要求ID1（存在と空席の確認） | イベントの存在と空席数を確認する | 未登録イベント・満席では予約状態を変えない |
...
要求ID1（存在と空席の確認）・要求ID2（予約で予約数+1）・要求ID5（不許可操作で変えない）は変更前と変更後が同じなので、下の対照表には出てきません。変更後ベースラインで内容を確認できます。
...
- **起点の変更ID**：変更ID1（キャンセル待ち）・変更ID2（一時保留）
...
- **対応する問題ID**：問題ID1（複数メソッドへ分岐追加）・問題ID2（全操作を横断確認）

**原因ID2**

- **構造上の原因（何が同じ責任へ集まっているか）**：席解放後の自動昇格と待機順の管理が状態処理へ絡み、待ち行列の責任が予約本体へ散在している
- **対応する問題ID**：問題ID3（昇格と待機順が散らばる）
...
- **システム全体での設計判断**：課題ID1（状態固有動作の境界）の状態振る舞いを`IReservationState`の各具体へ、課題ID2（待ち行列の境界）の待ち行列方針を`ReservationWaitlist`へ置く
...
- **そう決めた理由**：昇格は状態遷移なので、課題ID1（状態固有動作の境界）の契約の仕事。待ち行列が状態を動かすと、2つの軸が絡む
...
|---|---|---|---|
| 状態（課題ID1（状態固有動作の境界）） | 現在の状態クラスと、受けた操作の組 | 直前の遷移で設定された `state` | `TicketReservation::setState()` |
| 待ち行列（課題ID2（待ち行列の境界）） | **選ばない。実装は1つ** | ―― | `BatchApplication` が渡す1回だけ |
...
ここまでの決定を、課題ID1（状態固有動作の境界）→課題ID2（待ち行列の境界）の順に一望へまとめます。この表は設計課題だけを追います。変更要求の受入はフェーズ7の要求ID表、変更影響は7-4（変更シナリオ表）の変更ID表で別に確認します。
...
変更IDの受入証拠も省略しません。要求ID表の後に、7-4（変更シナリオ表）で変更ID1（キャンセル待ち）・変更ID2（一時保留）を「どの最終要求へ反映し、どの実行結果で確認したか」の単位で再照合します。つまり、要求ID表は最終仕様全体の受入・回帰、変更ID表は今回の依頼差分の受入という二つの役割です。
...
上の表は継続（要求ID1（存在と空席の確認）・要求ID2（予約で予約数+1）・要求ID5（不許可操作で変えない））・変更（要求ID3（支払済みへ遷移）・要求ID4（取消と自動昇格））・追加（要求ID6（キャンセル待ち登録）・要求ID7（15分と24時間の期限））を同じ順序で並べ、変わらなかった既存要求も回帰対象に含めています。継続要求が合格していることで、既存動作が落ちていないことを確認できます。要求の受入・回帰はここで完了します。課題IDへ直接対応付けず、以下では変更試行の痛みから導いた構造課題だけを別に確認します。
...
|---|---|---|---|
| 課題ID1（状態固有動作の境界） | 状態別動作を`IReservationState`の各実装へ分離 | 状態追加が新しい状態クラスと遷移接続に閉じた | 新状態と遷移登録 |
| 課題ID2（待ち行列の境界） | `ReservationWaitlist`を席解放経路へ接続 | 取消・期限切れから自動昇格まで閉じた | 待機順序方針 |
#### 変更前→変更後の不変条件照合
...
フェーズ3の変更影響グラフと同じ要求・同じ粒度で比べると、課題ID1（状態固有動作の境界）の状態追加は共通契約 `IReservationState` の実装1クラスへ、課題ID2（待ち行列の境界）の待ち行列方針変更は `ReservationWaitlist` の中だけへ限定されました。`TicketReservation` の公開操作と、キャンセル直後の自動昇格の接続はどちらの変更でも触りません。
...
- **🔴 フェーズ6：対策検討**：課題ID1（状態固有動作の境界）の状態判断と課題ID2（待ち行列の境界）の待ち行列を同時に分離し、共有状態と待ち行列の生成・受け渡しから、取消後の自動昇格までが一つの実行としてつながる構造を確定した。
...
- **4. 状態追加の判断**：フェーズ6で、課題ID1（状態固有動作の境界）・課題ID2（待ち行列の境界）を残さない最終構造を先に確定し、状態追加と待ち行列変更の影響先を分けたこと。
