...
|---|---|
| 問題ID1（修正漏れ） | `hold()`・`pay()`・`cancel()`・`expire()` |
| 問題ID2（横断確認） | `reserve()`・`hold()`・`pay()`・`cancel()`・`expire()` |
| 問題ID3（待機責任の散在） | `waitlist`・`reserve()`・`promoteNextWaitlisted()` |
| 問題ID3（待機責任の散在） | `cancel()`・`expire()`（席解放後の昇格呼び出し） |

問題IDは原因分析で開く場所を選ぶ根拠であり、以下の処理がすべて問題という意味ではありません。変更要求で直接追加・変更した処理と、現在の構造のため巻き込まれて確認した既存処理を分けます。

| コード上の目印 | そこで行っている処理・判断 | 変更試行で起きたこと |
|---|---|---|
| `void hold()`・`void expire()` | 利用者の保留とタイマーの期限切れを受け取る | 変更要求のため公開操作を新規追加した |
| `string status = "Available"` | 予約1件の現在状態を保持する | 保持方法は変更なし。各操作の状態分岐を横断確認した |
| `if (status == ...)` | 現在状態で各操作を許可するか判断する | 既存3操作の分岐を変更し、保留・期限切れの分岐を追加した |
| `status = ...`・`db.reserveSeat()`・`db.cancelSeat()` | 次状態と席数の増減を決める | 新しい遷移のため追加し、既存遷移の席数更新も回帰確認した |
| `db.exists()`・`db.hasCapacity()` | イベントの存在と空席を確認する | 変更なし。`reserve()` の変更に巻き込まれて回帰確認した |
| `waitlist.push_back(this)` | 満席の予約を待機列へ加える | 変更要求のため新規追加した |
| `waitlist.front()`・`pop_front()` | 待機列の先頭を1件選び、列から外す | 変更要求のため新規追加した |
| `promoteNextWaitlisted()` | 席解放後に先頭を予約へ自動昇格させる | `cancel()` の2分岐と `expire()` へ新規追加した |
...
|---|---|---|
| 公開操作から依頼を受ける | システムが受け付ける操作そのものが変わる | **変える** |
