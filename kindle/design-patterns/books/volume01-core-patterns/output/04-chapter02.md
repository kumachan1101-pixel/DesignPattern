...
**この形を採ると、状態どうしが互いの名前を知ります。** `ReservedState::cancel()` は `availableState()` を呼びます。**状態を1つ足すと、そこへ遷移する既存の状態も触ることになります。** これは5-3の完了条件が「状態追加が新しい状態動作と**遷移登録**に閉じる」と書いているとおりで、遷移元を触ることは条件に含まれています。
