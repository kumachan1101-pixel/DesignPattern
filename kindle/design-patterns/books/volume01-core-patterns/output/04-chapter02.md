...
フェーズ4で分ける責任は確定しています。ここで列挙型・関数・クラスという手段を先に比べるのではなく、二つの原因から必要な接続を導きます。

原因ID1（状態判断と処理が同居）からは、公開操作が現在状態へ同じ形で処理を依頼し、状態側が可否・遷移・副作用を返せる境界が必要です。原因ID2（待ち行列が状態処理へ絡む）からは、席解放後に待機列の先頭を取り出し、利用側の追加操作なしで昇格へつなぐ境界が必要です。

状態名を列挙型にしても判断は各操作に残り、判断を補助関数へ集めても状態数×操作数の組み合わせは残ります。これは原因を解く完成案ではありません。**状態ごとの可否と遷移は状態単位にまとめ、別の理由で変わる待機順は別責任にする**ことを課題候補にします。

### 5-2：全原因を解ける境界だけを残す

必要な境界には二つの変更理由があります。状態が増えたときに変わる「状態ごとの可否と遷移」と、運用方針が変わったときに変わる「待機順と自動昇格」です。同じ理由で変わる可否と遷移は状態単位にまとめ、別の理由で変わる待機順は分けます。状態だけ分けて待機順を予約本体に残すと、利用側が昇格を手で呼ぶ痛みが残るため、二つの境界をどちらも課題として確定します。責任配置の異なる完成方向はないため、手段の比較は行いません。
