...
★色が派手すぎるのか、何か見づらさを感じる。一度どんな色合いが良いか全修正する前に提案してほしい。
...
三つは借用ポインタです。とくに待ち行列は同一イベントの全予約で共有するため、予約ごとに値で持てません。実体は外側の `BatchApplication` が所有します。
...
状態と振る舞いを同じクラスに置くと、状態追加のたびに関連する分岐を横断します。公開操作を保ち、状態ごとの許可・遷移・副作用を具体状態へ移すのが本章のStateパターンです。分岐がどの業務理由で変わるかを問うことが、適用判断の入口になります。
