...
| **第0章** 設計判断の地図 | 各章で使う「7つのフェーズ」と、変更影響を接続点から見直す方法 |
| **第1章** ルール差し替え構造（Strategy） | **【痛みの種類】条件分岐の爆発**<br>「どのルールを適用するか」が変わるたびにコード全体が揺らぐ痛みを解消する。 |
| **第2章** 状態分離構造（State） | **【痛みの種類】状態による振る舞いの変化**<br>「今の状態」に応じたif文が散らばり、状態追加のたびに全箇所を修正する痛みを解消する。 |
| **第3章** 通知分離構造（Observer） | **【痛みの種類】通知先への依存**<br>「誰に通知するか」が増えるたび、主処理のコードに影響が及ぶ痛みを解消する。 |
