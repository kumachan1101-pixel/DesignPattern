...
- **NGコードで触る場所**：`OrderService` を開いて割引計算の箇所を修正
- **OKコードで触る場所**：`PremiumDiscount` だけを修正

**メール文面を変更したい**

- **NGコードで触る場所**：`OrderService` を開いてメール生成の箇所を修正
- **OKコードで触る場所**：`MarketingEmail` だけを修正

**注文データの保存先を変更したい**

- **NGコードで触る場所**：`OrderService` を開いてDB保存の箇所を修正
- **OKコードで触る場所**：`DbRepository` だけを修正（または新しいリポジトリを追加）

**計算ルールを変更しながら同時にメール文面も変えたい**

- **NGコードで触る場所**：`OrderService` を1つ開いて両方修正——影響範囲が重なって混乱する
- **OKコードで触る場所**：`PremiumDiscount` と `MarketingEmail` をそれぞれ独立して修正——影響が交わらない
...
>
> この本の各章でも、`IDiscountRule`（割引ルールの契約、第1章）・`IReservationState`（予約状態ごとの振る舞いの契約、第2章）・`INotification`（通知先の契約、第3章）のように、ビジネス責任で命名したインターフェースが登場します。名前を見たときに「この境界線は何のためにあるか」がわかることが、良いインターフェース名の基準です。
