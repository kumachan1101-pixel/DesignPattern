...
この入力と出力から、(1)イベントを指定して予約後に支払いまたは取消を行い、(2)各操作が順に受け付けられ、(3)操作の成否が表示される、という一連の動きが読み取れます。各操作に対応する内部状態は、後続の状態表とコードで対応づけます。同じ入力を含む完全なコードと実行結果は1-4に掲載します。
...
1-1（このシステムの仕様）の「イベント」にあたるデータと在庫です。イベントIDから定員・予約数を引き、エラー条件「存在しないID」「満席」もここで判定します。
...
- **コード上の受け取り口**：`TicketReservation(db, eventId)`
- **実際に使う箇所**：`EventDatabase::exists()` / `hasCapacity()` / `reserveSeat()` / `cancelSeat()`
- **結果への現れ方**：対象イベントの予約数が増減し、未登録IDは拒否される

**予約・支払・キャンセル操作**

- **コード上の受け取り口**：`reserve()` / `pay()` / `cancel()`
- **実際に使う箇所**：各メソッド内の現在状態による分岐
- **結果への現れ方**：状態遷移または操作不可メッセージになる

**現在状態**

- **コード上の受け取り口**：`TicketReservation::status`
- **実際に使う箇所**：3つの操作メソッドの条件判定
- **結果への現れ方**：同じ操作でも状態に応じて成功・拒否が変わる
...
- **確定した変更内容**：満席時の予約要求を自動でキャンセル待ちへ登録し、空席発生時に先頭を自動昇格する
- **入力**：イベントID、満席状態、予約要求、予約済み取消
- **受入条件**：予約要求だけで待機登録され、取消で49/50になった直後、先頭待機者が予約済みとなり50/50へ戻る

**変更ID2**

- **確定した変更内容**：通常の15分決済期限を、明示的な一時保留で24時間へ延長し、期限切れで席を解放する
- **入力**：予約済み状態、一時保留操作、期限切れイベント
- **受入条件**：Reserved/Heldの期限中は席を確保し、期限切れ時は空席へ戻して待機者がいれば自動昇格する
...
- **変更種別・根拠となる変更ID**：変更<br/>根拠: 変更ID2
- **変更後要求**：予約済みまたは一時保留から支払済みへ遷移する
- **受入条件**：支払成功時だけPaidになる

**要求ID4**

- **変更種別・根拠となる変更ID**：変更<br/>根拠: 変更ID1
- **変更後要求**：予約取消で席を解放し、待機者がいれば先頭を同じ処理内で自動昇格する
- **受入条件**：50/50→49/50→50/50と件数変化し、手動昇格を要しない
...
- **変更種別・根拠となる変更ID**：追加<br/>根拠: 変更ID1
- **変更後要求**：満席時の予約要求をシステムがキャンセル待ちへ登録する
- **受入条件**：利用者は予約操作だけを行い、待機順を保存して空席発生時に先頭から昇格する

**要求ID7**

- **変更種別・根拠となる変更ID**：追加<br/>根拠: 変更ID2
- **変更後要求**：Reservedは15分、一時保留したHeldは24時間の期限を持ち、期限切れで席を解放する
- **受入条件**：期限中は席を確保し、期限切れ後は待機者を自動昇格する
...
- **変更前の要求（現行）**：予約済みを支払済みへ遷移する
- **変更後の有効要求**：予約済みまたは一時保留から支払済みへ遷移する
- **根拠変更ID**：変更ID2

**要求ID4**

- **変更前の要求（現行）**：予約済みをキャンセルして予約可能へ戻し、予約数を1減らす
- **変更後の有効要求**：予約取消で席を解放し、待機者がいれば先頭を同じ処理内で自動昇格する
- **根拠変更ID**：変更ID1

**要求ID6**

- **変更前の要求（現行）**：（新規・現行なし）
- **変更後の有効要求**：満席時の予約要求をシステムがキャンセル待ちへ登録する
- **根拠変更ID**：変更ID1

**要求ID7**

- **変更前の要求（現行）**：（新規・現行なし）
- **変更後の有効要求**：Reservedは15分、一時保留したHeldは24時間の期限を持ち、期限切れで席を解放する
- **根拠変更ID**：変更ID2
...
- **操作**：reserve（満席時）
- **遷移先**：Waitlisted
- **処理内容**：通常の予約要求をキャンセル待ちへ自動登録する

**Waitlisted**

- **操作**：promoteBySystem
- **遷移先**：Reserved
- **処理内容**：空席発生時にシステムが予約へ昇格する

**Reserved**

- **操作**：hold
- **遷移先**：Held
- **処理内容**：通常の15分期限を24時間へ延長する

**Reserved**

- **操作**：expire
- **遷移先**：Available
- **処理内容**：通常の15分決済期限が切れ、予約枠を空きに戻す
...
- **操作**：cancel
- **遷移先**：Available
- **処理内容**：保留中の予約を取り消し、予約枠を空きに戻す

**Held**

- **操作**：expire
- **遷移先**：Available
- **処理内容**：保留期限（24時間）が切れ、予約枠を空きに戻す

**Reserved / Held**

- **操作**：paymentFailed
- **遷移先**：Reserved / Held
- **処理内容**：決済失敗を記録し、予約済みまたは保留中のまま再試行できる状態に戻す
...
- **変更前（1-1の現状仕様）**：イベントID、操作、内部に保持している現在の予約状態
- **変更後（今回の要求）**：イベントID、操作/イベント、内部に保持している現在の予約状態、キャンセル待ち/一時保留/決済失敗に関わる入力
- **差分として追うもの**：操作・イベントと状態の種類が増える

**判定**

- **変更前（1-1の現状仕様）**：現在状態で操作可能か
- **変更後（今回の要求）**：新状態を含めて操作/イベントを受け付けるか、保留期限内か、決済失敗時に再試行できるか
- **差分として追うもの**：状態ごとの判定が増える

**加工**

- **変更前（1-1の現状仕様）**：予約・支払・キャンセルに応じて状態を更新
- **変更後（今回の要求）**：キャンセル待ち登録、予約昇格、一時保留、期限切れ、決済失敗時の状態維持を処理
- **差分として追うもの**：状態遷移ルールと状態維持ルールが増える

**出力**

- **変更前（1-1の現状仕様）**：操作結果と更新後状態
- **変更後（今回の要求）**：操作結果、Waitlisted/Heldを含む更新後状態
- **差分として追うもの**：新状態の出力を追う
...
変わりそうな仕様は、1-3（登場クラスとクラス構成図）のクラス図の中心 `TicketReservation` にあります。`reserve()` / `pay()` / `cancel()` の中で判定している「どの状態なら操作できるか」と「操作後にどの状態へ遷移するか」を、どんな仕様が、誰の何をきっかけに変わりそうかと一緒に整理します。
...
ここで見るのは所在だけです。責任の多さや構造の善し悪しは、変更を当てて痛みを観測してから判断します。上の表は、変わりそうな仕様が `TicketReservation` のどのメソッドのどの行にあるかという所在の見立てです。状態遷移の仕様を1つのクラスにまとめている今の配置が、変更要求を当てたときに**実際に痛みになるか**はフェーズ3「問題特定」で確かめ、なぜ辛いのかはフェーズ4「原因分析」で原因として言語化します。2-1（変わりそうな仕様の見当をつける）は、その検証対象に当たりをつける段階です。
...
この二点を2-3（関係者ヒアリング）で確認し、「状態名が増えるだけ」なのか「振る舞いも変わる」のかを分けます。
...
- **将来リスク**：状態遷移のルール（アクションの可否）の変更
- **時期の目安**：キャンペーンや運用の見直し時
- **根拠**：企画担当 鈴木氏との確認

**リスクID2**

- **将来リスク**：状態の種類（特別優待予約などの追加）
- **時期の目安**：新機能導入時
- **根拠**：企画担当 鈴木氏との確認
...
- **将来リスク**：状態を動かすイベントの追加
- **時期の目安**：外部連携や運用自動化の追加時
- **根拠**：決済失敗、期限切れ、空席発生など、利用者操作以外の入力が増えるため
...
したがって2-5（変わる見込みと今回維持する範囲を確定する）の出力は、「状態・遷移・イベント接続は変えられるようにし、予約の受付・保存・席数管理は守る」という設計条件です。フェーズ3「問題特定」では変更ID1・変更ID2だけを現在の構造へ適用し、リスクIDはフェーズ6の構造評価に使います。
...
変更した定義は6つです。1-4（実装コード）と同じ並び順で、上から見ていきます。座席在庫の存在確認・空席判定・座席の増減は1-4の `EventDatabase` がそのまま担うため、この抜粋では在庫まわりを省き、状態遷移に絡む部分へ集中します。
...
graph TD
    ...
    T2["待ち行列方針の変更"] -->|"影響が飛び火"| A
    T2 -->|"影響が飛び火"| C
```

このグラフが示す通り、「状態追加」や「状態を動かすイベント追加」という変更要求が、クラス内の複数のロジックに飛び火しています。もう1本の `T2` は、状態そのものではなく待機順の決め方を変える要求です。待機列を予約本体が持っているため、状態分岐と同じ `reserve()`・`cancel()` を開くことになります。7-3（改善後の変更影響）では、この2本を同じ粒度で当て直して比べます。
...
3つ目は、変更ID1で足したキャンセル待ちが、状態判定とは別の知識を予約本体へ持ち込んだことです。誰が待っているかという待機列そのものを予約本体が持ち、席が空いたときに先頭を昇格させる判断も、待機列へ積む判断も、状態分岐が並ぶ同じメソッドの中にあります。待機順の決め方を変えたいだけでも、状態分岐と同じ場所を開くことになります。
...
- **観測した痛み（変更途中コード）**：保留・キャンセル待ちを足すだけで `reserve/pay/cancel/expire` の複数メソッドへ分岐を追加し、修正漏れが受入不成立へ直結する
- **起点の変更ID**：変更ID1・変更ID2

**問題ID2**

- **観測した痛み（変更途中コード）**：「今この状態でこの操作は何をするか」が一箇所で読めず、今回修正した全操作を横断して確認する
- **起点の変更ID**：変更ID2

**問題ID3**

- **観測した痛み（変更途中コード）**：キャンセル待ちの追加で、席解放後の自動昇格と待機順の管理が予約本体の各所へ散らばる
- **起点の変更ID**：変更ID1
...
- **構造上の原因（何が同じ責任へ集まっているか）**：`TicketReservation` が「現在状態の判断」と「操作・イベントごとの処理フロー」を同居させている
- **対応する問題ID**：問題ID1・問題ID2

**原因ID2**

- **構造上の原因（何が同じ責任へ集まっているか）**：席解放後の自動昇格と待機順の管理が状態処理へ絡み、待ち行列の責任が予約本体へ散在している
- **対応する問題ID**：問題ID3
...
**原因ID1：一つの予約管理が状態名ごとの全操作・イベントを分岐する**

- **そのままだと残る痛み**：状態追加で無関係な操作とイベント入口まで修正する
- **課題候補**：状態固有の許可操作と遷移判断を状態ごとに分離する
- **候補を導いた理由**：状態別動作は独立して増減する

**原因ID2：席解放と待機者選択・昇格が別の手動操作になり得る**

- **そのままだと残る痛み**：取消・期限切れ後に運用者が昇格を呼ぶと業務が閉じない
- **課題候補**：席解放から待機先頭の選択・自動昇格までを接続する
- **候補を導いた理由**：空席発生は自動昇格の業務イベントだから
...
**課題ID1：状態固有動作と公開操作・イベント入口の境界**

- **接続するもの・変わる側**：**接続:** 現在状態、操作・タイマー・空席イベント、次状態、結果<br/>**変わる側:** 状態ごとの操作可否・遷移先
- **守る側**：公開入口、座席数更新、履歴
- **完了条件**：状態追加が新しい状態動作と遷移登録に閉じる

**課題ID2：待ち行列と席解放処理の境界**

- **接続するもの・変わる側**：**接続:** 待機者の追加・先頭取出し・昇格イベント<br/>**変わる側:** 待ち順序・探索方針
- **守る側**：席解放直後に1件を昇格する契約
- **完了条件**：取消・期限切れから自動昇格まで一つのユースケースで完了する
...
- **システム全体での設計判断**：課題ID1の状態振る舞いを`IReservationState`の各具体へ、課題ID2の待ち行列方針を`ReservationWaitlist`へ置く
- **変えたくない側が知らなくなる詳細**：状態の種類・遷移、待ち行列の方針

**生成・所有・受け渡し**

- **システム全体での設計判断**：共有状態は状態取得関数が所有し、`BatchApplication`は在庫・履歴・待ち行列を値で所有する。予約はそれらを借り、遷移元の状態が次状態を渡す
- **変えたくない側が知らなくなる詳細**：具体状態と共有データの実体

**公開入口からの実行**

- **システム全体での設計判断**：利用側は`reserve()`／`cancel()`だけを呼び、現在状態への委譲と席解放後の自動昇格は内部で続く
- **変えたくない側が知らなくなる詳細**：現在状態と待機者選択の詳細
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 状態ごとの可否は `reserve`・`hold`・`pay`・`cancel`・`expire` の5つの関数それぞれに `if (status == ...)` として現れます。そして2-4（ヒアリングで判明した将来リスク）のリスクID1（可否の変更）、リスクID2（状態の種類の追加）、リスクID3（入力イベントの追加）が、3方向とも「増える」と言っています。**現に変更要求で、保留中とキャンセル待ちの2状態が増えました。** そのため、各操作が同じ問い方で状態固有の判断を呼べるクラス契約まで分けます。

分けると決めたので、コードに線を引きます。

**変更前から抜き出す箇所：`TicketReservation::cancel()`** ―― 3-1（変更を試みる）の分岐から、変更要求で保留対応を足した部分（対策前）
...
**5-3（課題IDと接続点を確定する）が挙げた候補は4つです。** 現在状態、操作・タイマー・空席イベント、次状態、結果について、境界を流す形を1つずつ確定します。

**現在状態**

- **決めた形**：引数で渡さない。状態ごとに別のクラスを用意し、**オブジェクトそのものが現在状態を体現する**
- **そう決めた理由**：引数で渡すと、受け取った側がまた状態で分岐する。分岐が移動するだけになる

**操作・タイマー・空席イベント**

- **決めた形**：引数で渡さない。**操作ごとに別のメソッド**にする
- **そう決めた理由**：変更前は関数ごとに `if` を書いていた。文字列や種別で渡せば、その `if` が移動するだけになる

**次状態**

- **決めた形**：**状態オブジェクトそのもの**を、呼び出し元へ設定してもらう
- **そう決めた理由**：下で理由を詳しく書く

**結果**

- **決めた形**：**戻り値では返さない。** 不可のときはその場でメッセージを出す
- **そう決めた理由**：変更前も `handleCancelError()` がその場で出していた。呼び出し側は成否で分岐していない
...
- **決めた形**：**予約とイベントIDを引数で渡す**
- **そう決めた理由**：どのイベントの待ち行列かは予約が知っている。待ち行列の側がイベントを探しに行くと、予約の中身を知ることになる

**先頭取出し**

- **決めた形**：**イベントIDを渡し、予約を1件返す**（居なければ何も返さない）
- **そう決めた理由**：「誰を上げるか」は待機順の方針そのもの。返すのは選ばれた1件だけでよい

**昇格イベント**

- **決めた形**：**境界を流れない。** 返ってきた予約へ、呼んだ側が昇格を伝える
- **そう決めた理由**：昇格は状態遷移なので、課題ID1の契約の仕事。待ち行列が状態を動かすと、2つの軸が絡む
...
**課題ID2（待ち行列軸）。こちらは直前の契約コードで見た `ReservationWaitlist` がそのまま完成形です。** 待機順を扱う以外に書くことがないので、増える段がありません。**実装も1つだけです。** 待機順の方針は今1つで、抽選や優先度つきに替える予定は2-4に挙がっていません。**替えられる形にしておく必要が無いので、契約を作らず、クラス1つで止めます。**
...
**変更前から抜き出す箇所：`main()`** ―― 3-1の予約と取消（対策前）
...
**守る範囲との照合：** **公開入口**を保っていることを、いま確認しました。`reserve()` の引数が1つ減りましたが、これは呼び出し側が持つべきでない在庫の知識を返しただけで、操作の形は変わっていません。分離と組み立てを通して、守る範囲の4つはいずれも定義に反していません。最終確認はフェーズ7の受入・回帰エビデンスで行いますが、**そこで初めて確かめるのではなく、分離から実行までのコードで照合済みです。**
...
- **採用構造と生成・接続場所**：状態分離。`TicketReservation`が現在状態へ委譲し、各状態が遷移先を持つ
- **完成コードの主な場所**：`IReservationState`、`AvailableState`／`ReservedState`／`PaidState`／`WaitlistedState`／`HeldState`
- **確認**：状態追加が新状態と遷移登録に閉じる

**課題ID2（待ち行列）**

- **採用構造と生成・接続場所**：待ち行列分離。席を解放する各状態処理が`promoteNextWaitlisted()`を呼び自動昇格
- **完成コードの主な場所**：`ReservationWaitlist`、`ReservedState::cancel()`／`HeldState::cancel()`／`HeldState::expire()`
- **確認**：席解放直後にシステムが先頭を自動昇格する

**変更対象外**

- **採用構造と生成・接続場所**：公開入口・座席数更新・履歴。委譲先だけが振る舞いを変える
- **完成コードの主な場所**：`TicketReservation`の公開操作、`EventDatabase`
- **確認**：1-4、50/50→49/50→50/50

#### 将来リスクに対して構想を確認する

フェーズ2のリスクIDを採用構造へ再適用し、現在状態へ委譲する入口をどこまで守れ、何が弱点として残るかを評価します。

**リスクID1：状態遷移のルール（アクションの可否）の変更**

- **現在の構造による備え**：可否と次状態を現在状態の`IReservationState`実装へ置く
- **リスク発生時の変更先**：影響する状態クラスの対象操作
- **守れる範囲・残る弱点**：他状態と受付入口を守り、変更を遷移元の状態クラスへ閉じられる。複数状態共通の規則は重複しやすい

**リスクID2：状態の種類（特別優待予約などの追加）**

- **現在の構造による備え**：状態ごとの振る舞いを共通契約で分ける
- **リスク発生時の変更先**：新しい状態クラスと、その状態へ入る遷移
- **守れる範囲・残る弱点**：既存状態と公開操作を守れる。進入元が多い状態では複数の状態クラスへの遷移追加が残る

**リスクID3：状態を動かすイベントの追加**

- **現在の構造による備え**：イベントの起点と状態判断を分け、システム起点の処理も`TicketReservation`へ接続する
- **リスク発生時の変更先**：新イベント入口、`IReservationState`の操作、必要なスケジューラ
- **守れる範囲・残る弱点**：状態判断を状態クラス側へ保てる。時刻監視や再試行の実行基盤は現在の同期入口の外に必要になる
...
この実行結果は、フェーズ1の動作例テーブルと、フェーズ1-5（変更要求）で追加した仕様遷移の代表ケースに対応しています。EVT003は、まず `50/50` の満席が見え、通常の予約要求が自動待機登録になり、既存予約のキャンセルで `50→49`、直後の自動昇格で `49→50` へ戻ります。利用側が待機登録の専用メソッドを呼ぶ行はなく、利用側が昇格メソッドを呼ぶ行はありません。
...
変更IDの受入証拠も省略しません。要求ID表の後に、7-4（変更シナリオ表）で変更ID1・変更ID2を「どの最終要求へ反映し、どの実行結果で確認したか」の単位で再照合します。つまり、要求ID表は最終仕様全体の受入・回帰、変更ID表は今回の依頼差分の受入という二つの役割です。

**要求ID1**

- **最終要求**：イベントの存在と空席数を確認する
- **適用コード**：`EventDatabase`、`AvailableState`
- **実行シナリオ・観測結果・判定**：未登録・満席の既存判定を維持<br/>**判定:** 合格

**要求ID2**

- **最終要求**：予約可能から予約済みへ遷移し、予約数を1増やす
- **適用コード**：`AvailableState::reserve()`
- **実行シナリオ・観測結果・判定**：残り1席から満席へ数値変化<br/>**判定:** 合格

**要求ID3**

- **最終要求**：予約済みまたは一時保留から支払済みへ遷移する
- **適用コード**：`ReservedState`、`HeldState`
- **実行シナリオ・観測結果・判定**：支払成功時だけPaid<br/>**判定:** 合格

**要求ID4**

- **最終要求**：予約取消で席を解放し、待機者がいれば先頭を同じ処理内で自動昇格する
- **適用コード**：`ReservedState::cancel()`、`ReservationWaitlist`
- **実行シナリオ・観測結果・判定**：50/50→49/50→50/50、手動昇格なし<br/>**判定:** 合格

**要求ID5**

- **最終要求**：許可されない操作や決済失敗では状態と予約数を変えない
- **適用コード**：各`IReservationState`実装
- **実行シナリオ・観測結果・判定**：エラー前後の状態・件数が一致<br/>**判定:** 合格

**要求ID6**

- **最終要求**：満席時の予約要求をシステムがキャンセル待ちへ登録する
- **適用コード**：`AvailableState`、`WaitlistedState`、`ReservationWaitlist`
- **実行シナリオ・観測結果・判定**：シナリオ5で `reserve()` だけを呼び自動登録、先頭から昇格<br/>**判定:** 合格

**要求ID7**

- **最終要求**：Reservedは15分、一時保留したHeldは24時間の期限を持ち、期限切れで席を解放する
- **適用コード**：`ReservedState`、`HeldState`、`ReservationExpiryScheduler`
- **実行シナリオ・観測結果・判定**：シナリオ4aで通常期限、シナリオ4・5bで保留期限と自動昇格<br/>**判定:** 合格
...
- **フェーズ1の現状構造での影響**：`TicketReservation`の予約・取消へ待ち行列と昇格条件を追加
- **完成構造での結果**：通常の`reserve()`だけで待機登録し、取消直後に49/50から先頭を自動昇格して50/50へ戻ることを確認

**変更ID2：通常の15分決済期限を、明示的な一時保留で24時間へ延長し、期限切れで席を解放する**

- **フェーズ1の現状構造での影響**：`TicketReservation`の複数操作へ通常・保留の期限分岐を追加
- **完成構造での結果**：`ReservedState`と`HeldState`へ期限切れを置き、どちらも空席へ戻した後の自動昇格まで確認
...
**状態が1つ増えるたびに全操作メソッドの条件分岐を書き直さなければならず、ヒアリングで確定した追加頻度ではコストが合わない**
...
- **🔴 フェーズ6：対策検討**：課題ID1の状態判断と課題ID2の待ち行列を同時に分離し、共有状態と待ち行列の生成・受け渡しから、取消後の自動昇格までが一つの実行としてつながる構造を確定した。
...
- **見えた原因**：利用者操作でなくても、状態ごとの受け付け可否を中心クラスが知っている
- **定めた課題**：操作とイベントの発生源を問わず、現在状態へ委譲する
- **採用した扱い**：`WaitlistedState` に昇格時の振る舞いを置く

**保留期限切れタイマー**

- **見えた原因**：時間起点の入力も `Held` だけが意味を持つのに、中心クラスへ条件が増える
- **定めた課題**：特定状態だけが扱うイベントを、その状態の責任に寄せる
- **採用した扱い**：`HeldState` に期限切れ時の遷移を置く

**決済失敗後の戻り**

- **見えた原因**：成功時と失敗時で、維持すべき状態が状態ごとに変わる
- **定めた課題**：外部API結果を受けても、呼び出し元へ状態分岐を漏らさない
- **採用した扱い**：`ReservedState` / `HeldState` が失敗時の状態維持と再試行可否を決める
