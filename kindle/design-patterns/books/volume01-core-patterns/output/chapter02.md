...
|---|---|---|
| 契約と具体 | 課題ID1の状態振る舞いを`IReservationState`の各具体へ、課題ID2の待ち行列方針を`ReservationWaitlist`へ置く | 状態の種類・遷移、待ち行列の方針 |
| 生成・所有・受け渡し | 共有状態は状態取得関数が所有し、`BatchApplication`は在庫・履歴・待ち行列を値で所有する。予約はそれらを借り、遷移元の状態が次状態を渡す | 具体状態と共有データの実体 |
| 公開入口からの実行 | 利用側は`reserve()`／`cancel()`だけを呼び、現在状態への委譲と席解放後の自動昇格は内部で続く | 現在状態と待機者選択の詳細 |
