...
**この本のコードでは、`〜Database` という名前のクラスが「あらかじめ登録されているデータ」を持ちます。** 在庫0の商品を含む全3件は、後の「コードを読む前に：開始データ」で並べます。
...
**代役でも、通知が何件出たかは本物です。** `(1件)` は各クラスが自分で数えた件数で、要求ID2（閾値以下で3手段へ通知）の「3つがそれぞれ1件ずつ受け取る」を、この出力で確かめられます。
...
この入力と出力から、(1)商品と増減数を受け取り、(2)在庫更新→閾値判定→通知の順に進み、(3)更新後の在庫が表示される（閾値を下回れば通知が飛ぶ）、という一連の動きが読み取れます。同じ入力を含む完全なコードと実行結果は現状コードとして後で載せます。
...
| 要求ID | 現行要求 | 受入条件 |
|---|---|---|
| 要求ID1（入出庫で在庫更新） | 登録商品の入出庫で在庫数を更新する | 出庫で在庫が減り、補充で増える |
| 要求ID2（閾値以下で3手段へ通知） | 在庫が閾値以下になったら、メール・ダッシュボード・チャットへ同じ警告を送る | 閾値に達した出庫のときだけ、3つが1件ずつ受け取る |
| 要求ID3（不正入力を拒否） | 未登録商品・不正数量・在庫不足を拒否する | 在庫数と通知件数を変えない |
| 要求ID4（在庫変動をログへ） | 在庫がいくつからいくつになったかを、あとで追えるよう記録する | 商品ID・変更前→変更後・単位を確認できる |

在庫の増減を記録し、出庫後の在庫が一定数以下になると、登録されているすべての通知先へメッセージを送ります。

この章で扱う現状仕様は、次の範囲です。**「一例」と書いた行は、値を替えてもこの章の話は変わりません**（商品を替えても、設計の論点は同じです）。**「これで全部」と書いた行は、ほかの選択肢がありません。**

| 仕様項目 | この章で使う値 | 役割 |
|---|---|---|
| 商品 | USBハブ（PRD002）（**一例**） | 現在在庫と閾値を取得する |
| 在庫変動 | 3個から2個へ減る（**一例**） | 現在在庫を更新する |
| 通知条件 | 更新後の在庫が閾値以下（**この1つだけ**） | 通知を出すかどうかを決める |
| 通知手段 | メール・ダッシュボード・チャット（**これで全部**） | 同じ在庫警告を3つへ届ける |
| 出力 | 在庫の変更前後と、3手段への通知（**これで全部**） | 在庫更新と通知発生を照合する |
...
| 実システムの対象 | 掲載コードでの代替 | この章で確認すること |
|---|---|---|
| 商品マスター | `ProductDatabase`内の`std::map` | 商品検索、在庫更新、閾値判定 |
| メール・ダッシュボード・チャット | 3つの通知クラスの受信箱と`std::cout` | 通知先、件数、呼び出し順 |
| 在庫操作画面 | `main()`からの商品ID・数量指定 | 入力検証から結果表示まで |
| 運用ログ | 標準出力 | 変更前後の在庫と通知発生 |

したがって、在庫更新、閾値判定、受信箱への蓄積は実際に行います。永続DB、GUI、外部通信・監視基盤そのものの実装品質は扱いません。現状コードの通知メソッドは同期的な`void`であり、通信失敗や非同期完了は本章の現行要求に含めません。
...
次の3つが通知先です。**関数名も、引数も、戻り値も、3つともそろっていません。** 実際のメール・ダッシュボード・チャットへの送信は、標準出力で代替します。各クラスが数える件数は、要求ID2（閾値以下で3手段へ通知）どおり同じ通知を各手段が1件ずつ受け取ったことを確認するテスト用の観測点です。
...
入庫のときに呼ぶ、`reduceStock()` の対になる操作です。仕入れが届いたり、キャンセルで戻ってきたりした分を在庫へ足します。要求ID1（入出庫で在庫更新）の「入庫」がこれにあたります。
...
ここまでに確認した識別子を使い、仕様で見た仕様入力が受け取り口から判定・更新・結果まで途切れず使われているかを検算します。コードを読んだ後に、未使用入力や固定値への置き換えがないことを確認する表です。

| 入力・構成 | コードでたどる経路 | 結果にどう現れるか |
|---|---|---|
| 商品ID | 入出庫操作 → `exists()` / `get()` / `save()` | 対象商品の在庫更新か未登録エラーになる |
| 数量 | 入出庫操作 → 正数・在庫不足の検証 → 加減算 | 更新前後の在庫数へ現れる |
| 商品ごとの閾値 | `ProductInfo::alertThreshold` → `isBelowThreshold()` | 閾値以下のときだけ通知される |
| 3つの通知手段 | `InventoryManager` のメンバー → `notifyAll()` | 各手段の受信件数が増える |
...
| 要求ID2（変更） | 閾値以下なら全通知先へ送り、SMSの受付と最終結果を記録する | 4手段を集計し、受付IDを保留から完了・失敗へ更新する |
| 要求ID3（変更） | 不正入力を拒否し、個別通知失敗を在庫更新・他通知から切り離す | 不正操作では何も変えず、1通知の失敗では他通知を止めない |
| 要求ID4（継続） | 在庫の内部数値変化をログへ残す | 商品ID・変更前→変更後・単位を確認できる |

要求ID2（閾値以下で3手段へ通知）と要求ID3（不正入力を拒否）は、変更ID1（非同期SMS追加）による変更です。
...
| 要求ID | 変更前 | 変更後 |
|---|---|---|
| 要求ID2（閾値以下で3手段へ通知） | 閾値以下なら3手段へ送る | SMSを加え、受付IDと最終配信結果も記録する |
| 要求ID3（不正入力を拒否） | 不正入力を拒否する | 従来の拒否に加え、1通知の失敗を在庫更新と他通知から切り離す |

要求ID1（入出庫で在庫更新）・要求ID4（在庫変動をログへ）は変更前と変更後が同じなので、下の対照表には出てきません。要求ID3（不正入力を拒否）の変更は、変更ID1（非同期SMS追加）に明記された「SMSの受付・最終配信の失敗でも在庫更新と他通知を止めない」という受入条件を、従来の入力拒否と両立させるためのものです。変更依頼にない在庫ログ項目の追加は行いません。
...
変更後の仕様も、現状の仕様と同じく入力検証済みの正常系として確認します。登録済み通知手段へSMSが加わる即時経路と、後日届く最終配信結果の入力経路が差分です。変更したノードは黄色で、ラベルの頭に `【追加】` または `【変更】` が付きます。
...
|---|---|---|
| 変更ID1（非同期SMS追加） | 非同期SMSを追加し、受付・最終配信の失敗でも在庫更新と他通知を止めない | 要求ID2（閾値以下で3手段へ通知）、要求ID3（不正入力を拒否） |
...
フェーズ3「問題特定」では、変更ID1（非同期SMS追加）によって通知元クラスを書き換え、通知先ごとの成否・待ち方まで通知元へ入った痛みを確認しました。次のフェーズ4「原因分析」では、通知元と通知先の接続点に漏れている知識を確認します。
...
| 今回変える責任 | ほかの変更から守る責任 |
|---|---|
| 通知先の種類・増減・固有の送信方法・受付と最終配信状態 | 在庫更新、閾値判定、在庫警告の発生、1通知失敗でも在庫を止めない規則 |
...
|---|---|---|
| 原因ID1（通知先の呼び方を抱える） | 在庫管理の骨格と、クラス名・引数・戻り値・同期方式の翻訳 | 問題ID1〜3 |
...
|---|---|---|
| 原因ID1（通知先の呼び方を抱える） | 手段追加で在庫更新を修正し、個別失敗が他通知へ波及する | 通知手段の種類と実行を在庫更新から分離する |

### 5-2：課題候補の重複と依存を整理する

| 課題候補 | 確認できた必要性 | 整理結果 |
|---|---|---|
| 通知手段の分離 | SMSの種類・非同期・失敗処理が在庫更新へ集中した | 登録・反復・個別結果を1つの通知境界として扱う |
...
|---|---|---|
| 課題ID1（在庫更新と通知手段の境界） | 商品・在庫・閾値を警告として渡し、手段別の受付結果を在庫更新へ返す | 新通知を登録で加え、1件失敗しても在庫更新と他通知を完了する |
...
| 問題 | 原因 | 課題 |
|---|---|---|
| 問題ID1〜3：通知手段と成否が在庫管理へ混在 | 原因ID1：通知先のクラス名・送信方法・成否を在庫管理が抱える | 課題ID1：通知手段の種類と実行を在庫更新から分離する |

要求ID4（在庫変動をログへ）の在庫変動記録は、通知手段の混在から生じた設計課題ではありません。フェーズ6でシステム全体の生成・所有を決めるときに実装場所も確定しますが、課題ID1（在庫更新と通知手段の境界）の追跡には混ぜず、要求IDとしてフェーズ7で検証します。
...
**この構想が何を解こうとしているのか**を、先に確認します。課題は1つですが、痛みから原因までの道筋は次のとおりです。

- **課題ID1** ← 原因ID1（通知先のクラス名・関数名・引数の形・戻り値の意味・同期か非同期かまで、在庫管理が抱えている）← 問題ID1（メンバと通知が連動）・問題ID2（在庫管理と通知が同居）・問題ID3（通知手段とその成否が、在庫管理へ混ざっている）

これを分離対象にします。

**この課題（何を解きたいか）：** SMSを1つ足すだけで `InventoryManager` のメンバ・受付IDの状態表・コンストラクタ・公開操作・`notifyAll()` まで連動する。**在庫変化の事実を配るだけにし、通知先を共通の契約で登録・差し替えできるようにする**のが課題ID1（在庫更新と通知手段の境界）です。
...
1. `main()` ―― 通知先を生成し、登録する
2. `InventoryManager::reduceStock()` ―― 公開入口
3. `notifyAll()` ―― 登録順に反復する
4. `INotification::send()` ―― 契約
5. 各通知先 ―― 具体処理
...
**関数を一つ切り出すだけでは、変更を閉じ込められません。** 手段ごとの知識は `notifyAll()` の中だけでなく、`InventoryManager` のメンバ宣言・コンストラクタ・受付IDの状態表・`receiveSMSCompletion()` にも現れます。変更を試したときにSMSを足すと、書き換えた場所は7か所でした。そしてヒアリングで挙げたリスクID1（通知先の種類と実装）、リスクID2（通知先の増減）、リスクID3（受付と最終配信結果の扱い）が、どれも「増える・変わる」と言っています。**現に変更要求で、SMSという通知先が1つ増えました。** だから各通知手段を同じ問い方で呼べる契約まで進みます。
...
**接続点を確定したところで課題ID1（在庫更新と通知手段の境界）と分けておいた要求ID4（在庫変動をログへ）の記録先も、ここで配置を確定します。** 現状コードの `cout` だけでは実行の最後に全在庫変動を取り出せないため、在庫変動を1件ずつ積む `StockEventLog` を置き、`main()` が最後に `printAll()` で一覧します。これは通知分離の対策ではなく、要求ID4（在庫変動をログへ）の実装です。
...
**構想の確認で `ProductDatabase db;` と書いた値メンバが、参照になりました。** 在庫の正本と在庫変動ログは、同じ1件の在庫更新を記録する対です。片方を `main()` が持ち、もう片方を通知元が内側に隠すと、対応が読み取れなくなります。**課題ID1の分離とは別の判断なので、そのことを書いておきます。** 現状コードのまま値メンバにしても、課題ID1（在庫更新と通知手段の境界）は成立します。
...
ここまでの構想表と要点コードを合わせると、責任配置と実行経路が一つにつながります。**この章の完成構造は一つに定まります。** 通知手段ごとの専用メンバを残したまま契約だけを足す形も置けますが、それは課題ID1（在庫更新と通知手段の境界）の波及を解消しない途中状態なので比較しません。責任配置や依存が異なる完成構造が複数残ったわけではないので、当て馬を並べた比較は行いません。採用するのは、共通契約を実装した通知クラスを組み立て側が生成・登録し、通知元が登録リストへ一律配布する**通知分離構造**です。
...
|---|---|---|
| 課題ID1（通知手段・非同期完了） | `send()`、各Notifier、`attach()`、状態ログ、SMSコールバック | 手段追加を具体と登録へ閉じ、他処理を止めない |
| 在庫更新・閾値判定・ログ | `InventoryManager::reduceStock()`、`ProductDatabase`、`StockEventLog` を保つ | 通知手段が変わっても在庫50→45の更新を変えない |
...
リスクID1（通知先の種類が増える）〜リスクID3（受付と最終結果）は、通知契約と所有方法を評価する材料です。今回合意したSMS最終結果の記録までは実装し、自動再送や専用運用UIは将来機能として区別します。
...
各行に商品ID・種別・数量と単位・変更前→変更後がそろっています。要求ID4（在庫変動をログへ）の受入条件はこの8行で確認できます。閾値は要求ID2（閾値以下で3手段へ通知）の判定と通知文面にだけ使い、変更依頼にないログ項目は追加していません。
...
|---|---|---|
| 要求ID1（入出庫で在庫更新） | `InventoryManager`、`ProductDatabase` | 商品IDと変更前→変更後の数量を出力 |
| 要求ID2（閾値以下で3手段へ通知） | `InventoryManager::reduceStock()`、各Notifier、配信状態ログ | 4手段を集計し、SMS-1は完了、SMS-2は最終失敗へ更新 |
| 要求ID3（不正入力を拒否） | 在庫検証、`notifyAll()`、`SMSDeliveryCallback` | 不正操作では何も変えず、SMS失敗でも他通知と在庫更新を完了 |
| 要求ID4（在庫変動をログへ） | `StockEvent`、`StockEventLog` | 8行すべてに商品ID・変更前→変更後・単位を出力 |

全4件が合格です。

上の表は継続（要求ID1（入出庫で在庫更新）・要求ID4（在庫変動をログへ））・変更（要求ID2（閾値以下で3手段へ通知）・要求ID3（不正入力を拒否））を同じ順序で並べ、変わらなかった既存要求も回帰対象に含めています。継続要求が合格していることで、既存動作が落ちていないことを確認できます。要求の受入・回帰はここで完了します。課題IDへ直接対応付けず、以下では変更試行の痛みから導いた構造課題だけを別に確認します。
...
| 在庫イベントログ | 通知の成否と独立して更新結果を記録する | 通知失敗後も残るログ |
...
graph TD
    ...
    T1 -->|組み立てで2行増やす| N2["attach() と<br>SMSDeliveryCallback の生成"]
    T1 -. "影響なし" .-> A["INotification<br>InventoryManager ✅"]
    T1 -. "影響なし" .-> B["EmailNotifier など<br>既存通知先 ✅"]
```

同じ要求・同じ粒度で比べると、共通契約 `INotification` と通知元 `InventoryManager`、既存通知先は変更先から消えました。変更影響グラフで `InventoryManager` へ入り込んでいた3つ――呼び分け、受付IDの保管、後日の完了コールバック――が、どこへ移ったのかを1つずつ見ます。

| 3-2で影響した場所 | 完成構造での修正 | 移した先 |
|---|---|---|
...
| 原因 | 通知元が各手段のクラス名・呼び方・結果・完了時期を知っている |
| 課題 | 通知手段と後日の最終結果を在庫更新から分離する |
...
| 接続点の診断 | フェーズ4で、通知方法と受付結果の知識が在庫管理へ漏れていることを確認した |
...
| 原則 | この章での適用 |
|---|---|
| 変わるものを分けて囲う | 送信方法と受付結果を通知先ごとのクラスへ置いた |
| 具体ではなく契約に向かって書く | 通知元は具体クラスを知らず、`INotification::send()`だけを呼ぶ |
| 継承より「持つ」を先に考える | 在庫管理が通知先一覧を保持する。継承は共通入口のために使う |
