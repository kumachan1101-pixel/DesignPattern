...
在庫が閾値以下になったときは現在の3通知先すべてへ送り、閾値を超えているときは通知しないことが核心です。
...
静的な関係に続けて、正常系（出庫でしきい値以下になり通知が走る）で各クラスがどの順に呼ばれ何を受け渡すかを時系列で示します。通知の具体の分岐は `InventoryManager` の注記に、エラー系はメモとして添えます。現状コードを読む前の地図です。
...
> | 配信履歴の保存と担当者による確認 | `DeliveryStatusLog` がメモリへ記録し、標準出力へ表示する | 永続DB、検索画面 |
...
|---|---|---|
| SMSの送信、受付結果、別経路の完了履歴 | 変更ID1。リスクID1・3から手段と完了時期の追加も見込まれる | 変更を試す |
...
変更ID1（非同期SMS追加）だけを現状の構造へ当て、在庫更新・閾値判定・他通知の継続は動作を維持します。リスクID1（通知先の種類）・リスクID2（通知先の構成）・リスクID3（受付と最終結果）は実装せず、フェーズ6の構造評価へ引き継ぎます。
...
>
> このフェーズで見るのは、変更を当てたときにどこまで開くことになったかです。開いた場所を1つずつこの問いにかけ、依頼に直接必要な修正と、そうでない修正を分けます。なぜ関係のない場所まで開くのかは、次のフェーズ4で調べます。新しいSMS通知役を作り、SMSを使うケースを実行すること自体は変更に直接必要なので、痛みではありません。通知先を1つ足すために、在庫更新を担う既存の`InventoryManager`のメンバー、生成、通知処理へ修正が連鎖し、受付後の状態表と別時点の完了入口まで入る部分を痛みとして追います。
...
graph TD
    T1["変更ID1：非同期SMSと最終結果の記録"]:::req
        -->|"送信・状態保管・別経路の結果を追加"| A["［変更］ InventoryManager<br>在庫＋3同期通知 → SMS受付・状態・完了も保持"]:::touched
...
**クラス・組み立て箇所の粒度では、変更が2つ、新規が1つです。** 新しく作れるのは `SMSNotifier` だけで、既存の `InventoryManager` と入力・動作確認を担う `main()` を開きます。`InventoryManager` という1箱の内側では、手段ごとの送信と結果の読み替え、受付IDごとの配信状態の保持と更新を横断して直しています。6か所の修正は、直前のコードで確認したとおりです。なぜ在庫管理まで開くことになったのかは、フェーズ4で調べます。
...
| 変更理由 | 第0章の型 | 出どころ |
|---|---|---|
| A：通知先の種類が増える | 仲間が増える | 変更ID1、リスクID1・2 |
| B：手段ごとに呼び方と結果の表し方が違う | 相手が変わる | 変更ID1、リスクID1 |
| C：受付と最終結果の時点が分かれる | 時点が変わる | 変更ID1、リスクID3 |
| D：在庫の更新と警告の基準が変わる | 規則が変わる | 今回は対象外（ヒアリングで固定と確認） |

次に、粒度を一段下げます。変更影響グラフはクラスと組み立て箇所の粒度でしたが、同居を見るには `InventoryManager` の中で何を書き換えたかまで割る必要があります。そこで、`［変更］`が付いたクラスの中身を仕事ごとの行にし、今回書き換えなかった在庫更新も、同じクラスの中にあるので行へ入れます。新しく作る `SMSNotifier` と、入力を組み立てる `main()` は行にしません（どの実体をいつ作るかはフェーズ6で扱います）。行が仕事、列がさきほどの変更理由です。`○`は今回開いたところ、`△`は将来その理由が来たときに開くところです。

| 開いた仕事 | いまあるクラスと場所 | A | B | C | D |
|---|---|---|---|---|---|
| 在庫を更新し、閾値以下かを判定する | `InventoryManager::reduceStock()` | | | | △ |
| 手段ごとに引数を組み立て、戻り値を読み替える | `InventoryManager::notifyAll()` | ○ | ○ | | |
| 受付結果を数え、集計を表示する | `InventoryManager::notifyAll()` の末尾 | ○ | | ○ | |
| 受付IDと配信状態を持ち、最終結果で更新する | `InventoryManager` のメンバーと完了入口 | | | ○ | |

読み方は四つです。

1. **同じクラスの行に、別々の列の印が付いている**なら、そのクラスに複数の責任が同居しています（混在）。4行すべてが `InventoryManager` の中にあり、A・B・Cの印が1つのクラスへ集まっています。分ける対象はここです。
2. **一つの行に印が2つ以上**あるなら、その仕事自体が複数の理由で変わります。引数の組み立ての行がAとB、集計の行がAとCです。
3. **ある列の印が1行に収まっている**なら、その理由には専用の置き場所があります。今回そう言えるものはありません。
4. **どの列にも今回の印がない行**は、今回は変わりません。ただし在庫更新の行は混在したクラスの中にあるため、通知を直すたびに一緒に読み直すことになります。これが巻き込みです。

列は「なぜ変わるか」、責任は「その理由で一緒に変わる仕事が何をするか」です。列に仕事の側から名前を付け直すと、分けたあとの責任になります。AとBは「通知先が増えれば、その手段の呼び方も決める」という形で必ず一緒に動くので、一つにまとめます。名前を付けると三つです。

| 責任 | 対応する変更理由 | まとめている仕事 |
|---|---|---|
| 在庫更新・警告要否の判定 | D | 入力を検証する／在庫を増減して保存する／閾値以下かを判定する |
| 通知先別の送信・結果変換 | A・B | 手段ごとに文面と引数を作る／APIを呼ぶ／戻り値を成否へ読み替える |
| 受付ID別の配信状態更新 | C | 受付IDと配信状態を記録する／最終結果で状態を更新する |

**「どれも在庫管理の話だから、責任は一つでは」と感じるかもしれません。** 責任は、扱う題材ではなく書き換えるきっかけで分けます。通知手段を決めるのは運営の施策、受付後の結果をどう追うかは外部基盤の都合で、在庫の数え方とは別々に変わります。
...
**原因ID1（在庫更新・警告要否の判定と通知先別の送信・結果変換が同じクラスにある）。** 在庫管理が、通知先固有のクラス名・引数・結果の扱いを直接持つ配置が、問題ID1（SMS追加で在庫管理を修正）を生みました。

**原因ID2（在庫更新・警告要否の判定と受付ID別の配信状態更新が同じクラスにある）。** 在庫管理が、受付IDの保持と別経路の最終結果を受ける入口まで持つ配置が、問題ID2（非同期完了を在庫管理へ追加）を生みました。
...
黄色は在庫管理に残す責任、青は外へ分ける責任です。①と②は次の課題表に対応します。
...
フェーズ4では、A・B・Cの印が `InventoryManager` の1クラスへ集まっていました。分けたあとは、行ごとに自分の責任のきっかけだけが残ります。通知先を1つ足すとき（A・B）に開くのは1行目、受付後の結果の扱いが変わるとき（C）は2行目で、在庫更新の行は開きません。型名・具体通知先・生成場所はフェーズ6で決めます。
...
**解く原因：** 原因ID1（在庫更新・警告要否の判定と通知先別の送信・結果変換が同じクラスにある）

**構造の変更：** 目標図の①のとおり、通知先別の送信・結果変換を在庫更新・警告要否の判定から分けます。

**接続：** 在庫更新・警告要否の判定から商品ID・商品名・更新後在庫を渡し、通知側から受付結果を返します。
...
**解く原因：** 原因ID2（在庫更新・警告要否の判定と受付ID別の配信状態更新が同じクラスにある）

**構造の変更：** 目標図の②のとおり、受付ID別の配信状態更新を在庫更新・警告要否の判定から分けます。

**接続：** 通知受付時に受付IDを記録し、別経路の最終結果から受付IDと配信成否を渡して、該当する配信状態へ反映します。
...
| `SMSNotifier` | 受付ID付きの `PENDING` または `FAILED` | 別経路の最終結果の受信・確定 |
...
通知先を `InventoryManager` より先に宣言し、借用中の寿命を保証します。受付IDと配信状態だけを持つ `DeliveryStatusLog` もここで所有し、SMS送信時と後で定義する `SMSDeliveryCallback` から共有します。在庫操作とは入口が異なるため、最終結果を `InventoryManager` へ戻しません。
...
**ここで確認するコード：`SMSDeliveryCallback::receive()`** ―― 別経路の最終結果から台帳への受け渡し
...
classDiagram
    ...
    SMSDeliveryCallback --> DeliveryStatusLog : 別経路の最終結果を更新
...
在庫台帳は現状維持です。`InventoryApplication` が具体通知・状態台帳・受信入口を所有し、借用先の寿命を保証します。フェーズ4で `InventoryManager` に混在していた三つの責任は、完成図では、在庫更新・警告要否の判定を持つ `InventoryManager`、通知先別の送信・結果変換を持つ各 `INotification` 実装、受付ID別の配信状態更新を持つ `DeliveryStatusLog` と `SMSDeliveryCallback` に分かれました。対象とした三つの変更理由を、一つのクラスが同時に持つ構造ではなくなりました。
...
全条件が合格です。通知先別の送信・結果変換は各Notifierと登録へ、受付ID別の配信状態更新は `DeliveryStatusLog` と `SMSDeliveryCallback` へ閉じました。
...
> | `DeliveryStatusLog.h` / `.cpp` | 配信結果の記録と、別経路の最終結果の受け口 | SMS送信側と最終結果の受信側が共有し、通知元を太らせない |
...
| 原因 | 通知元が固有APIと即時結果を知り、在庫操作が別経路の最終結果まで持つ |
| 境界 | 通知契約と、受付IDを追う状態台帳・受信入口を分ける |
| 再結合 | 組み立て役が具体通知を登録し、通知元は同じ事実を全件へ配る |
| 結果 | 即時受付は集約し、別経路の成否は該当する受付IDだけを更新する |
