...
フェーズ3から受け取るのは、問題ID1（連動修正）・問題ID2（変更連鎖）と問題ID3（非同期処理の混入）、変更途中の `InventoryManager` です。ここでは具体的な接続方法を決めず、痛んだ場所の責任と変更理由を調べます。
...
|---|---|
| 問題ID1（連動修正） | `InventoryManager` のメンバー・コンストラクター・`notifyAll()` |
| 問題ID2（変更連鎖） | `InventoryManager::reduceStock()` から `notifyAll()` へ至る流れ |
| 問題ID3（非同期処理の混入） | `smsStatuses`・`receiveSMSCompletion()` |

問題IDは原因分析の対象を選ぶ根拠です。在庫検証や保存まで問題という意味ではありません。SMS追加で直接変えた処理と、同じ `InventoryManager` にあるため巻き込まれて確認した処理を分けます。

| コード上の目印 | そこで行っている処理・判断 | 変更試行で起きたこと |
|---|---|---|
| `db.exists()`・`db.get()`・`db.save()` | 在庫数を検証し、減らして保存する | 変更なし。同じ `InventoryManager` を変更したため巻き込まれて回帰確認した |
| `db.isBelowThreshold(...)` | 更新後在庫が閾値以下か判断する | 変更なし。`notifyAll()` への呼び出し条件を回帰確認した |
| `string message = ...` | 在庫警告の内容を作る | 変更なし。同じ `notifyAll()` へSMS処理を追加したため巻き込まれて確認した |
| `sendMail()`・`refreshStockWidget()`・`postMessage()` | 通知先ごとの関数名・引数へ翻訳する | 既存3手段の呼び方は変更せず、SMS追加のため同じ関数内を読み直した |
| `accepted++`・`postId.empty()`・`result.status` | 手段ごとに異なる戻り値を受付結果へ翻訳する | 4手段を集計するため既存呼び出しの周囲を変更し、SMSの判定を追加した |
| `sms.requestAsync(alert)` | SMSに警告を渡し、受付結果を受け取る | 変更要求のため新規追加した |
| `smsStatuses[result.requestId]` | 受付IDごとの配信状態を保持する | 変更要求のため新規追加した |
| `receiveSMSCompletion()` の `find()` と状態更新 | 後日届く最終結果で該当する受付状態を更新する | 変更要求のため新規追加した |
