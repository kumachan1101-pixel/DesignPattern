...
フェーズ3から受け取るのは、二つの問題IDと変更途中の `InventoryManager` です。このコードから、二つの問題が起きた理由を調べます。
...
|---|---|---|
| ① 課題ID1（通知手段の境界）<br>解く原因：原因ID1 | 外部通知との接続を在庫方針から分ける | 入力：商品ID・商品名・更新後在庫<br>結果：受付結果 |
| ② 課題ID2（非同期完了の境界）<br>解く原因：原因ID2 | 配信結果の管理を在庫方針から分ける | 入力：受付ID・配信成否<br>反映：該当する配信状態 |
