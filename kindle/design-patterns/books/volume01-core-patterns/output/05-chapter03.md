...
★この章には仕様変更後のシステムがあるが、他の章にはないのでは？
...
入力を `StockAlert`、即時結果を `DeliveryResult` とします。閾値は通知要否の判断なので渡しません。共通入力を肥大化させないため、全通知が共有する事実だけを契約に置きます。手段固有知識は `notifyAll()` 以外にメンバー、生成、受付状態にも分散しているため、関数抽出ではなくクラス契約まで分けます。
