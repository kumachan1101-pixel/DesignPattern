...
運用者が商品IDと入庫・出庫数量を指定すると、システムは在庫を更新します。出庫後の在庫が閾値以下なら、商品ID・商品名・更新後在庫をメール・在庫ダッシュボード・警告チャットへ渡します。
...
**代役でも、通知が何件出たかは本物です。** `(1件)` は各クラスが数えた件数です。後の要求表で定める、メール・在庫ダッシュボード・警告チャットへの通知を、この出力で確かめられます。
...
| 要求ID2（在庫不足の通知） | 出庫後に閾値以下ならメール・在庫ダッシュボード・警告チャットへ通知する | 各1回呼び、失敗後も在庫更新と残りを続ける |
...
| 通知手段 | メール、在庫ダッシュボード、在庫警告チャット | 現行の通知先を特定する |
| 出力 | 在庫の変更前後、通知先ごとの受信内容と件数 | 在庫更新と通知発生を照合する |

メール・在庫ダッシュボード・警告チャットは、同じ在庫不足を独立して受け取る通知手段です。掲載コードは順に呼びますが、登録順に業務上の優先順位はありません。
...
flowchart TB
    ...
    M -->|"④ 閾値以下なら通知する"| G["通知手段<br>メール・ダッシュボード・チャット"]
    G -->|"⑤ 通知受付結果"| M
    R -->|"⑦ 処理結果"| U
...
| 番号 | どこからどこへ | 受け渡すもの |
|---|---|---|
| ① | 担当者 → 在庫更新・閾値判定 | 商品IDと出庫数 |
| ② | 在庫更新・閾値判定 → 商品マスター | 商品IDによる参照と更新後在庫 |
| ③ | 商品マスター → 在庫更新・閾値判定 | 商品名・現在在庫・閾値 |
| ④ | 在庫更新・閾値判定 → 通知手段 | 閾値以下なら在庫警告 |
| ⑤ | 通知手段 → 在庫更新・閾値判定 | メール・画面・チャットの受付結果 |
| ⑥ | 在庫更新・閾値判定 → 実行結果 | 更新後在庫と通知受付結果 |
| ⑦ | 実行結果 → 担当者 | 処理結果の表示 |
...
| 状況 | 動作 |
|---|---|
| 在庫が閾値以下に減少した | メール・在庫ダッシュボード・在庫警告チャットへ通知を送る |
| 在庫が閾値を超えている | 通知しない |

三つの通知は、発注漏れを防ぐために同じ在庫警告から独立して実行します。どれか一つが失敗しても、残りの通知は続けます。

**この仕様を決める業務機能**

同じ在庫警告を扱いますが、在庫を管理する仕様と、各通知手段を扱う仕様では変更理由が異なります。システムが担う業務の種類で分けると、次の四分類になります。

| 業務分類 | 決める仕様 |
|---|---|
| 在庫管理 | 在庫の増減、警告する閾値、通知失敗後も残りを続ける規則 |
| メール通知 | 宛先、件名・本文、送信方法 |
| ダッシュボード表示 | 表示先、表示項目、更新方法 |
| チャット通知 | 投稿先、投稿内容、送信方法 |
...
直前の現状クラス図が示す通り、`ProductDatabase` が商品と在庫を保持し、`InventoryManager` が在庫操作に加えて、メール、在庫ダッシュボード、在庫警告チャットを直接保持しています。
...
この章の中心です。仕様で見た「在庫の増減」と「閾値以下でのメール・在庫ダッシュボード・在庫警告チャットへの通知」に対応します。
...
確定した変更は、SMSの追加、受付と最終配信の記録、SMS失敗と在庫更新・既存通知の切り離しです。操作UIは変えません。
...
>
> 掲載コードに待ち時間・通信はなく、`main()`から受信入口を別に呼びます。同じ受付IDによる対応づけ、保留から最終状態への更新、SMS失敗後も在庫更新と既存通知を続ける処理は実際に動かします。再送の実行は範囲外です。

依頼とSMS境界の制約を、一つの変更IDと、その変更で更新・追加する要求として固定します。

| 変更ID | 変更内容 | 確認する具体例 |
|---|---|---|
| 変更ID1（非同期SMS追加） | 在庫警告へSMSを加え、受付・最終配信の失敗後も他の処理を続ける | 各通知を1回呼び、SMS受付IDを保留から完了または失敗へ更新する |
...
| 要求ID2（在庫不足の通知） | 出庫後に閾値以下ならメール・在庫ダッシュボード・警告チャット・SMSへ通知する | 変更：SMSを追加 | 各1回呼び、SMS失敗後も在庫更新と残りを続ける |
...
要求ID2（在庫不足の通知）にはSMSが加わり、要求ID5（SMS配信結果の確認）が追加されます。失敗後も残りを続ける規則は変更前から有効です。受付IDと保留状態は、この追加要求を実現するSMS境界の情報です。

#### 変更後に有効な業務ルール

次の表では、コードの通知処理に必要なルールを、継続する通知と追加する通知に分けます。

| 区分 | 通知先 | 変更内容 |
|---|---|---|
| 継続 | EmailNotifier、DashboardUpdater、ChatNotifier | メール・在庫ダッシュボード・在庫警告チャットをそのまま使う |
| 新規 | **SMSNotifier** | 受付IDを返す非同期SMSを追加する |

`ProductDatabase`の取得・更新契約と通知条件は変わりません。SMSだけは受付IDを返した後、別経路で最終配信結果が届きます。
...
flowchart TB
    ...
    M -->|"④ 閾値以下なら通知する"| G["［変更］通知手段<br>メール・ダッシュボード・チャット<br>SMS（追加）"]:::touched
    G -->|"⑤ 通知受付結果"| M
    R -->|"⑦ 処理結果"| U
...
| 番号 | どこからどこへ | 変わった受け渡し |
|---|---|---|
| ④ | 在庫更新・閾値判定 → 通知手段 | 通知先にSMSが加わる |
| ⑤ | 通知手段 → 在庫更新・閾値判定 | SMSの受付結果が加わる |
| ⑧ | SMS通知 → 受付ID別の配信状態 | 受付IDと、あとから届く最終配信結果を渡す |

①〜③・⑥・⑦は変わりません。⑧だけは一回の出庫が終わったあとに届くため、受付ID別の配信状態を保存します。
...
この章では、出庫を受けて在庫数を更新し、しきい値を判定して必要なら通知する流れが、今回の変更に対する**幹候補**です。通知手段と、同期・非同期で異なる受付結果の扱いが**変化点候補**に見えます。しきい値も別の要求では変わりうるため、幹候補は「永久に変わらない部分」という意味ではありません。
...
**メール・画面・チャット・SMSの呼び方がそれぞれ違うまま、1つの関数に並んでいます。**
...
在庫が3から2へ減り、メール・画面・チャットの出力は現状コードのままです。そこへSMSの1行と `[受付結果]` の集計が加わりました。SMSの行に出ている `SMS-1` が受付IDです。最後の `[SMS最終結果]` は、その `SMS-1` が保留から配信完了へ変わったことを示しています。
...
SMSは断られましたが、在庫更新もメール・画面・チャットも止まっていません。**動作は正しくなっています。** 変更ID1（非同期SMS追加）で確認する具体例――各通知の受付結果を確認し、受付IDを最終結果へ更新する――を満たせました。
...
>
> 確認結果は「ある」です。在庫更新・警告要否の判定、通知先別の送信・結果変換、受付ID別の配信状態更新は別々の理由で変わりますが、三つが `InventoryManager` へ集まっています。
...
| 完了条件3 | メール・在庫ダッシュボード・在庫警告チャット・SMSへ同じ在庫警告を渡す |
...
メール・画面・チャット・SMSを起動時に登録し、一件でも登録に失敗すれば構成不良として起動を止めます。通知元は借用した `INotification*` を反復し、具体型や同期方式に依存しません。通知構成は起動時に確定する範囲とします。
...
> **実行条件：** メール・画面・チャット・SMSを同一プロセスで順に呼び、一件の失敗後も続けます。外部送信は標準出力のスタブで表します。
...
sequenceDiagram
    ...
    participant IM as InventoryManager
    participant N as Email・Dashboard・Chat・SMS
...
| 要求ID2（在庫不足の通知） | `InventoryManager::reduceStock()`、Email・Dashboard・Chat・SMSの各Notifier | 閾値以下のときメール・画面・チャット・SMSが1件ずつ受け取る |
...
| メール・在庫ダッシュボード・在庫警告チャット・SMSへ同じ在庫警告を渡す | `notifyAll()`が同じ`StockAlert`を各Notifierへ渡す | 合格 |
...
4. 対象となる通知先の入力と出力を並べると、共通の事実と受付結果をどの型で約束できるか。
5. 各通知先は独立しているか。順序依存や前段結果の受け渡しがあるなら、同報ではなく誰が手順を制御するか。
6. 具体通知先を誰が生成・所有・登録し、発生元が具体名を知らずに全員を実行できるか。非同期の最終結果はどの別境界で追跡するか。

本章で組み立てた「通知分離構造」を、章題に掲げた **Observerパターン**の抽象的な骨格と照合します。完成コードを型へ当てはめて採用を正当化するのではなく、どの関係が対応し、どの条件では別の設計が適するかを確認します。
