...
問題はフェーズ3で確定済みです。責任一覧と `InventoryManager` の変更箇所へ戻り、問題を生んだ責任の組だけを取り出します。

| 問題ID | 関係する責任（変更の中心 → 影響した責任） | コード上の目印 |
|---|---|---|
| 問題ID1（連動修正） | SMSの呼出し・結果解釈 → 通知全体の集計 | `requestAsync()`・`accepted` |
| 問題ID2（変更連鎖） | 通知先の追加 → 在庫更新と閾値判定 | `reduceStock()` → `notifyAll()` |
| 問題ID3（非同期処理の混入） | 非同期完了管理 → 在庫管理の状態と入口 | `smsStatuses`・`receiveSMSCompletion()` |

通知や在庫の責任自体が問題なのではありません。変更理由の異なる責任まで、同じクラスにあるため影響した関係を問題として見ます。
...
| 責任 | 変わるきっかけ | 今回 |
|---|---|---|
| 在庫の検証・更新・閾値判定 | 在庫業務や保存方法の変更 | **守る** |
| 警告の発行と通知全体の集計 | 通知全体の運用変更 | **守る** |
| 通知先固有APIと結果の翻訳 | 外部通知手段の仕様変更 | **変える** |
| 受付IDの保持と後日結果の更新 | 非同期配信の仕様変更 | **変える** |
