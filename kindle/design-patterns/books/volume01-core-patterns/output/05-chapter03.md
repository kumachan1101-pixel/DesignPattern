...
フェーズ3から受け取るのは、二つの問題IDと変更途中の `InventoryManager` です。ここでは対策を決めません。問題が起きた理由だけを調べます。

### 4-1：問題が起きたコードを確認する

**問題ID1（SMS追加で在庫管理を修正）。** 変えたかったのは、SMSの呼び出しと受付結果の扱いです。しかし `notifyAll()` が具体的な通知先の呼び分けと集計を行うため、既存通知と在庫管理まで直しました。コード上の目印は `requestAsync()` と `accepted` です。

**問題ID2（非同期完了を在庫管理へ追加）。** 変えたかったのは、受付IDの保持と後日の結果更新です。しかし `smsStatuses` と `receiveSMSCompletion()` を `InventoryManager` へ置いたため、在庫管理へ別時点の処理が増えました。

### 4-2：変える責任と守る責任を分ける

1行が一つの責任です。横に読むと、その責任が変わる理由と、今回の変更で変えるのか守るのかが分かります。

| 責任 | 変わるきっかけ | 今回の扱い |
|---|---|---|
| 在庫の更新 | 在庫ルールの変更 | **守る** |
| 通知全体の開始 | 通知対象や実行方針の変更 | **守る** |
| 通知先ごとの送信 | 外部通知手段の変更 | **変える** |
| 非同期通知の状態管理 | 非同期配信の変更 | **変える** |

「変える／守る」は今回の変更に対する区別です。将来ずっと変わる／変わらないという意味ではありません。

### 4-3：原因を確定する

前の表で分かったのは、何を変え、何を守るかまでです。問題が起きた理由は、`InventoryManager` へ戻って確認します。

**原因ID1（在庫管理が通知先ごとの送信を持つ）。** 在庫管理が、通知先固有のクラス名・引数・結果の扱いを直接持っています。この配置が問題ID1（SMS追加で在庫管理を修正）を生みました。

**原因ID2（在庫管理が配信状態を持つ）。** 在庫管理が、受付IDの保持と後日結果を受ける入口まで持っています。この配置が問題ID2（非同期完了を在庫管理へ追加）を生みました。

> **問い1：同じ場所に、別々の理由で変わるものがないか**
>
> 答えは「ある」です。在庫更新は在庫業務、通知先の翻訳は外部API、受付後の状態は非同期配信のライフサイクルで変わりますが、三つが `InventoryManager` へ集まっています。

原因は確定しました。次は、二つの原因をなくす分け方を決めます。型や生成場所はまだ決めません。

---

## 🟡 フェーズ5：課題定義 ―― 原因から課題を検討して確定する

入力は、原因ID1（在庫管理が通知先ごとの送信を持つ）と原因ID2（在庫管理が配信状態を持つ）です。

### 5-1：原因をなくす分け方を決める

**原因ID1への対応。** 通知先ごとの送信を、在庫管理から分けます。在庫側は商品ID・商品名・更新後在庫を渡し、受付結果を受け取ります。

**原因ID2への対応。** 非同期通知の状態管理を、在庫管理から分けます。後日の入口は受付IDと配信成否を受け取り、該当する状態だけを更新します。

送信受付と後日の配信結果は、同じ受付IDでつなぎます。型・操作・生成場所はフェーズ6で決めます。

### 5-2：原因が残らないか確認する

通知先ごとに補助関数を作るだけでは、`InventoryManager` が具体クラスを名指しするため、原因ID1（在庫管理が通知先ごとの送信を持つ）は消えません。送信時の呼び方だけをそろえても、受付IDの状態表と後日の受信処理を在庫管理へ残せば原因ID2（在庫管理が配信状態を持つ）が残ります。

したがって、通知先固有の翻訳と非同期の状態管理を両方とも在庫操作から分け、受付結果と後日の結果は受付IDでつなぐ必要があります。これで二つの原因を同時に解き、SMSだけが失敗しても在庫更新と他通知を維持できます。

### 5-3：課題と完了条件を確定する

課題は「原因をなくせた」とコードで判断できる状態です。1行が一つの課題で、左から「どの原因を解くか」「何を分けるか」「分けた両側を何でつなぐか」を読みます。3列目の「入力」は元の処理から分けた側へ渡すもの、「結果／反映」は分けた側から返すもの、または配信状態へ反映するものです。

| 課題（解く原因） | 分ける責任 | 分けた後のつなぎ方 |
|---|---|---|
| 課題ID1（通知手段の境界）<br>解く原因：原因ID1 | 通知先ごとの送信を在庫管理から分ける | 入力：商品ID・商品名・更新後在庫<br>結果：受付結果 |
| 課題ID2（非同期完了の境界）<br>解く原因：原因ID2 | 非同期通知の状態管理を在庫管理から分ける | 入力：受付ID・配信成否<br>反映：該当する配信状態 |
...
|---|---|---|
| 問題ID1：SMS追加で在庫管理を修正 | 原因ID1：在庫管理が通知先ごとの送信を持つ | 課題ID1：通知先ごとの送信を在庫管理から分ける |
| 問題ID2：非同期完了を在庫管理へ追加 | 原因ID2：在庫管理が配信状態を持つ | 課題ID2：配信状態の管理を在庫管理から分ける |
...
|---|---|---|
| 課題ID1（通知手段の境界） | `send()`、各Notifier、`attach()` | 手段追加を具体と登録へ閉じ、在庫更新を変えない |
| 課題ID2（非同期完了の境界） | `DeliveryStatusLog`、`SMSDeliveryCallback` | 受付IDと最終結果を在庫操作から分け、該当状態だけ更新する |
...
|---|---|---|
| 課題ID1（通知手段の境界） | 具体通知を `INotification` 実装と登録一覧へ分離 | SMS追加・失敗が在庫更新へ波及しない。新手段ではNotifierと登録を追加する |
| 課題ID2（非同期完了の境界） | 状態を台帳へ、受信入口をコールバックへ分離 | 在庫操作を変えず、該当IDの状態だけを更新する |
