...
責任名は、変更前後で共通する仕事のまとまりです。1行が一つの責任で、左から変更前、変更後の順に規則を比べます。

| 責任 | 変更前 | 変更後 |
|---|---|---|
| 在庫方針 | 入出庫で在庫数を更新し、閾値以下で通知 | 入出庫で在庫数を更新し、閾値以下で通知 |
| 通知方針 | 登録済み3手段へ通知し、失敗しても残りを続ける | SMSを含む4手段へ通知し、失敗しても残りを続ける |
| 外部通知との接続 | Emailはbool、画面はvoid、Chatはintを返す | 左記に加え、SMSは受付成否と受付IDを返す |
| 配信結果の追跡 | 呼出し時点の結果だけを扱う | SMSの受付IDを保持し、後日の配信結果で更新する |

在庫方針は変わりません。通知対象、外部APIとの接続、配信結果を扱う期間が変わりますが、直前に確認したコードでは、それらが `InventoryManager` に集まり、在庫管理まで修正しました。

### 4-3：原因を確定する

前の表で分かったのは、各責任の規則がどのように変わるかまでです。問題が起きた理由は、`InventoryManager` へ戻って確認します。
...
**原因ID1への対応。** 外部通知との接続を在庫管理から分けます。在庫側は商品ID・商品名・更新後在庫を渡し、手段ごとの差を共通化した受付結果を受け取ります。

**原因ID2への対応。** 配信結果の追跡を在庫管理から分けます。受付IDと配信成否を受け取り、該当する状態だけを更新します。
...
classDiagram
    ...
    class DeliveryTracking["② 配信結果の追跡"]:::separated {
        <<分ける責任>>
    }
...
|---|---|---|
| ① 課題ID1（通知手段の境界）<br>解く原因：原因ID1 | 外部通知との接続を在庫管理から分ける | 入力：商品ID・商品名・更新後在庫<br>結果：受付結果 |
| ② 課題ID2（非同期完了の境界）<br>解く原因：原因ID2 | 配信結果の追跡を在庫管理から分ける | 入力：受付ID・配信成否<br>反映：該当する配信状態 |
...
フェーズ5では、外部通知との接続と配信結果の追跡を在庫管理から分け、受付IDでつなぐところまで決めました。ここからは、通知契約、性質の異なる具体、生成・登録、配信状態の台帳と完了入口、在庫操作からの同報の順にC++の構造へ変えます。
...
全条件が合格です。外部通知との接続は各Notifierと登録へ、配信結果の追跡は `DeliveryStatusLog` と `SMSDeliveryCallback` へ閉じました。
