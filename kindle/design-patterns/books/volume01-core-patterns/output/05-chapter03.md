...
* **得られること1：** 変更要求を現状コードへ当て、通知先追加で開くメンバー・呼び出し・結果解釈を変更影響として特定できる
* **得られること2：** 在庫更新と通知手段が同居するコードから、変化の発生元・通知先固有の翻訳・後日の完了管理という異なる責任を切り分けられる
* **得られること3：** 各通知先に共通して渡せる事実と、手段ごとに残す設定を区別し、共通契約の入力・結果を決められる
* **得られること4：** 具体通知を組み立て役で生成・所有・登録し、通知元は具体型を知らずに全件へ同報する構造へ変更できる
...
| 通知構成の分離 | フェーズ6と7で、組み立て役による初期登録から一律配布までを追った |
