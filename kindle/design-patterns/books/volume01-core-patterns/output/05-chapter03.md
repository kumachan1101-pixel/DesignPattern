...
**この図は、代表となる1件の出庫を最後まで通したものです。** 他の商品や数量の組み合わせは1-2の動作例でまとめて確認します。ここで見てほしいのは**箱の並びと矢印の向き**です。
...
|---|---|---|---|
| リスクID1（通知先の種類が増える） | 通知先となるクラスの種類とその実装（音声通知システムなど） | 業務要件の変更があるたび | 田中部長との合意 |
| リスクID2（通知先の増減） | 通知先の増減（動的な登録・解除） | 随時 | 設計側の予見（ヒアリング外） |
| リスクID3（受付と最終結果） | 通知先ごとの受付と最終配信結果、失敗の扱い | 送信基盤や外部連携が増えるたび | 田中部長との合意（在庫は止めず、最終結果を履歴化する） |
...
- **対応する問題ID**：問題ID1（メンバと通知が連動）・問題ID2（在庫管理と通知が同居）・問題ID3（同期と非同期が混ざる）
...
|---|---|---|---|
| 通知手段の分離 | 必須。変更ID1（非同期SMS追加）で種類・非同期・失敗処理が在庫更新へ集中した | 登録・反復・個別結果を一つの通知接続点へ統合 | 課題として残す |
...
**問題ID1・問題ID2（在庫管理と通知が同居）・問題ID3（同期と非同期が混ざる）：通知手段と成否が在庫管理へ混在**

- **原因ID（フェーズ4の構造原因）**：原因ID1（通知先の呼び方を抱える）：通知先のクラス名・送信方法・成否まで在庫管理が抱える
- **課題ID（達成目標）**：課題ID1（在庫更新と通知手段の境界）：通知手段の種類と実行を在庫更新から分離
...
- **システム全体での設計判断**：課題ID1（在庫更新と通知手段の境界）の文面・送信・受付結果を`INotification`の各具体へ、最終結果を`DeliveryStatusLog`と`SMSDeliveryCallback`へ置く
...
**この2つを満たす場所は、通知元でも通知先でもありません。新しい箱が要ります。** 受付IDごとの配信状態だけを持つので `DeliveryStatusLog` とします。そして、後日届く最終結果を受け取る入口も要ります。3-1では `InventoryManager` の `receiveSMSCompletion()` が公開操作として通知元にありました。**在庫操作の入口とは別の起点なので、別のクラスへ置きます。** SMS基盤からのコールバックを受けるので `SMSDeliveryCallback` とします。
...
- **実行シナリオ**：閾値到達時だけ全4手段を集計する。行2は `SMS-1` が保留から配信完了へ、行7は `SMS-2` が保留から配信失敗へ更新される
- **判定**：合格
...
|---|---|---|---|
| 課題ID1（在庫更新と通知手段の境界） | 具体通知を`INotification`実装と登録一覧へ分離 | SMS追加・失敗が在庫更新と他通知へ波及しない | 新Notifierと起動時登録 |
#### 変更前→変更後の不変条件照合
...
- **🟡 フェーズ5：課題定義**：`StockAlert` と `DeliveryResult` を課題ID1（在庫更新と通知手段の境界）の接続データとし、通知手段の種類・文面・同期非同期を在庫処理から外す課題を定めた。
...
inventory.reduceStock("P001", 5);   // 中で mailer.send(...) を直接呼ぶだけ
```
