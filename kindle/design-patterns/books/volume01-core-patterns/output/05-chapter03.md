...
| 要求ID | 現行要求 | 受入条件 |
|---|---|---|
| 要求ID1（入出庫で在庫更新） | 登録商品の入出庫で在庫数を更新する | 商品IDと変更前→変更後の数量を記録する |
| 要求ID2（閾値以下で全通知先へ） | 出庫後の在庫が閾値以下なら、全通知先へ同じイベントを送る | 閾値到達時だけ3手段が各1件受け取る |
| 要求ID3（不正入力を拒否） | 未登録商品・不正数量・在庫不足を拒否する | 在庫数と通知件数を変えない |
| 要求ID4（在庫変動をログへ） | 在庫の内部数値変化をログへ残す | 商品ID・変更前→変更後・単位を確認できる |
...
| 仕様項目 | この章で使う具体値 | 役割 |
|---|---|---|
| 商品 | PRD002: USBハブ | 現在在庫と閾値を取得する |
| 在庫変動 | 3個から2個へ減る | 現在在庫を更新する |
| 通知条件 | 閾値5個に対して現在2個 | 閾値以下かを判定する |
| 出力 | 在庫2個、メール通知送信 | 在庫更新と通知発生を照合する |
...
次の3つが1-1の通知先です。**導入時期も提供元も違うため、呼び方がそろっていません。** 関数名、引数、戻り値のどれもが手段ごとに違います。この違いは作り話ではなく、別々に導入した外部基盤を1つのシステムから使うときに実際に起きることです。実際のメール・ダッシュボード・チャットへの送信は標準出力で代替します。各クラスが数える件数は、要求ID2（閾値以下で全通知先へ）どおり同じ通知を各手段が1件ずつ受け取ったことを確認するテスト用の観測点です。
...
要求ID2（閾値以下で全通知先へ）と要求ID3（不正入力を拒否）は、変更ID1（非同期SMS追加）による変更です。
...
| 要求ID | 変更前 | 変更後 |
|---|---|---|
| 要求ID2（閾値以下で全通知先へ） | 閾値以下なら3手段へ送る | SMSを加え、受付IDと最終配信結果も記録する |
...
|---|---|---|
| 変更ID1（非同期SMS追加） | 非同期SMSを追加し、受付・最終配信の失敗でも在庫更新と他通知を止めない | 要求ID2（閾値以下で全通知先へ）、要求ID3（不正入力を拒否） |
...
各行に商品ID・種別・数量と単位・変更前→変更後がそろっています。要求ID4（在庫変動をログへ）の受入条件はこの8行で確認できます。閾値は要求ID2（閾値以下で全通知先へ）の判定と通知文面にだけ使い、変更依頼にないログ項目は追加していません。
...
| 要求ID2（閾値以下で全通知先へ） | `InventoryManager::reduceStock()`、各Notifier、配信状態ログ | 4手段を集計し、SMS-1は完了、SMS-2は最終失敗へ更新 |
