...
この入力と出力から、(1)商品と増減数を受け取り、(2)在庫更新→閾値判定→通知の順に進み、(3)更新後の在庫が表示される（閾値を下回れば通知が飛ぶ）、という一連の動きが読み取れます。
...
| 要求ID2（在庫不足の通知） | 閾値以下になった商品を、メール・ダッシュボード・チャットで担当者へ知らせる | 閾値に達した出庫のときだけ、3つが1件ずつ受け取る |
...
| 仕様項目 | この章で使う値 | 役割 |
|---|---|---|
| 商品マスター | PRD001：マウス、在庫50、閾値10 | 開始データ |
...
ここで「3手段を三つの補助関数へ分ければ十分では」と考えるのは自然です。関数分割をすれば読みやすさは上がります。しかし、`InventoryManager` が三つの具体クラスをメンバーに持ち、各補助関数を名指しし、結果の違いを解釈する依存は残ります。SMSを足すと、メンバー、生成・初期化、呼び出し、結果解釈を再び `InventoryManager` へ加えるため、**関数分割は可読性の改善であって、通知先追加の変更影響を止める境界にはなりません。**

反対に、通知同士へ「メール成功後にだけチャットを送る」のような依存を足すと、単純な一斉通知ではなく順序付きの業務手順になります。その場合、登録先を独立に呼ぶObserverだけでは要件を表せません。この章は、現状仕様どおり**同じ事実を、互いに依存しない複数手段へ届ける場面**を扱います。順序や前段結果が必要な場面との違いは、対策検討で改めて判断します。
...
動作例テーブルの各行について、在庫が閾値以下になった減少では3通知先へ送信され、補充では通知されず、在庫0・未登録IDはエラーになることを確認できました。
...
|---|---|---|
| 要求ID1（継続） | 登録商品の入出庫で在庫数を更新する | 商品IDと変更前→変更後の数量を記録する |
| 要求ID2（変更） | 閾値以下なら、メール・ダッシュボード・チャット・SMSへ在庫不足を知らせる | 同じ警告機会に4手段が1件ずつ受け取る |
| 要求ID3（継続） | 未登録商品・不正数量・在庫不足による誤った在庫更新を防ぐ | 不正操作では在庫数と通知件数を変えない |
| 要求ID4（継続） | 在庫の内部数値変化をログへ残す | 商品ID・変更前→変更後・単位を確認できる |
| 要求ID5（追加） | 一つの通知手段が失敗しても、確定した在庫更新とほかの通知を失わない | SMSの受付・最終配信が失敗しても在庫を戻さず、残りの手段を実行する |
| 要求ID6（追加） | SMSを受け付けた後も、担当者が最終配信結果を追跡できる | 受付IDを保留から配信完了または配信失敗へ更新できる |

要求ID2（在庫不足の通知）の通知先追加と、要求ID5（通知失敗から在庫を保護）・要求ID6（SMS配信結果の追跡）は、変更ID1（非同期SMS追加）による変更です。要求ID3（誤更新の防止）は変更せず、通知失敗という別の関心事を混ぜません。
...
| 要求ID5（通知失敗から在庫を保護） | なし | **1通知の失敗を在庫更新と他通知から切り離す** |
| 要求ID6（SMS配信結果の追跡） | なし | **受付IDを使って最終配信結果を記録する** |

要求ID1（正確な在庫の維持）・要求ID3（誤更新の防止）・要求ID4（在庫変動の追跡）は変更前と変更後が同じので、この表には出てきません。これにより、既存の入力拒否と、新しく必要になった通知失敗の扱いを別々に検証できます。変更依頼にない在庫ログ項目の追加は行いません。
...
変更後の仕様も、現状の仕様と同じく入力検証済みの正常系として確認します。箱の色は入力・処理・判定・出力という**役割**を、箱の先頭に置いた `［新規］`・`［変更］` は現状からの**差分**を表します。色だけで差分を判別させません。
...
|---|---|---|
| 変更ID1（非同期SMS追加） | SMSの失敗でも在庫更新と他通知を続ける | 要求ID2（通知）、要求ID5（失敗分離）、要求ID6（配信追跡） |
...
| リスクID2（通知先の増減） | 実行中に登録・解除する | ヒアリングでは未確認の設計上の見込み |
...
ヒアリングで挙げたリスクIDを、通知先ごとに変わる側と、在庫更新の安定側へ分けます。「はい」は、フェーズ6で**通知先や受付方法が変わっても、在庫更新と通知条件へ影響を広げない構造か**を判定するための印です。
...
田中部長からの「倉庫担当者のスマホへSMSで通知を送りたい」という要求を、**構造は今のまま**当ててみます。
...
graph TD
    T1["変更要求：非同期SMSと最終結果の記録"]:::req
        -->|"メンバ・呼び分け・結果解釈を修正"| A["［変更］ InventoryManager<br>（呼び分けと結果解釈）"]:::touched
    A -->|"固有APIを呼ぶ"| B["［新規］ SMSNotifier<br>（非同期の受付API）"]:::added
    A -->|"受付IDを保留で保持"| C["［変更］ InventoryManager<br>（受付IDの保管）"]:::touched
    A -->|"コールバックを受けて更新"| D["［変更］ InventoryManager<br>（完了コールバック）"]:::touched
    D -->|"同じ受付IDを完了/失敗へ"| C
...
---
> **📌 問題（確定）**
> 変更ID1（非同期SMS追加）のSMS追加で、通知元の`InventoryManager`のメンバ変数・コンストラクタ・`notifyAll()`が連動して変わった。通知手段と受付結果が在庫管理ロジックに混在し、一つの確定要求が通知元まで修正対象にした。
---
...
3行に共通しているのは、`InventoryManager` が出したいのは「在庫が閾値を下回った」という事実1つなのに、**それを誰がどう受け取るかまで持っている**ことです。受け取り方が1つ増えるたびに、事実を出す側が変わります。

### 4-2：今回変える責任/ほかの変更から守る責任

> **「今回守る」と「変わらない」は異なります。** 左列は今回の変更試行とヒアリングで変更理由を確認した責任、右列はその変更に巻き込まず守りたい責任です。将来ずっと変わる／変わらないという分類ではありません。

変更を当ててみて、`InventoryManager` の中で**手が入った処理と入らなかった処理**が分かりました。それを1つずつ並べます。

| このコードが持っているもの | 変更を当てたとき | どちら側か |
|---|---|---|
| 在庫を減らして保存する | 触っていない | **守る** |
| 閾値を下回ったかを判定する | 触っていない | **守る** |
...
原因を解くには、この5つを契約の向こうへ移す必要があります。途中まで移したときに何が残るかを確認します。

| 線の引き方 | `InventoryManager` に残るもの | 手段を1つ足すとき |
|---|---|---|
| A 呼び方だけそろえる | 通知先の一覧と、結果の読み分け | メンバと登録と結果解釈を足す |
| B 結果の読み方だけそろえる | 通知先の一覧と、呼び分け | メンバと呼び分けを足す |
| C 通知先の一覧を外から渡す | 呼び分けと、結果の読み分け | 呼び分けと結果解釈を足す |
| D 5つすべてを契約の向こうへ置く | 「配る」ことだけ | 何も足さない |

**A・B・Cは、どれも半分です。** 通知元を開く回数は減りますが、ゼロにはなりません。フェーズ3でSMSを足したとき、`InventoryManager` を開いたのは**呼び分け・結果の読み分け・一覧への追加の3つが揃っていたから**で、1つ残せば1つ分は開き続けます。

**Dだけが、通知元を開かずに済みます。** 通知先が同じ形で呼ばれ、同じ形で結果を返すなら、通知元は「登録されている相手へ順に配る」以外に何も知りません。

**ただし、SMSの「後から届く結果」はDでも残ります。** 呼んだ時点で成否が決まらないので、受付IDを覚えて後で更新する仕事がどこかに要ります。これを通知元に置くと、また `InventoryManager` を開くことになります。**別の置き場所が要る**という点は、課題を確定してから構想で決めます。

### 5-2：全原因を解ける境界だけを残す

A・B・Cは、Dへ至る途中の状態です。呼び方だけをそろえても通知先一覧と結果解釈が残り、結果だけをそろえても具体クラスの所有と呼び分けが残ります。一覧だけを外から渡しても、型ごとの分岐が必要です。**どれも通知先追加で `InventoryManager` を開く原因を残すため、完成案として比較しません。**

Dでは、すべての通知先が共通の警告を受け取り、共通の受付結果を返し、通知元は契約の一覧だけを反復します。この章で全原因を解く完成方向は一つなので、手段の比較表は不要です。

ここまででDを残せば、原因ID1（通知先の呼び方を抱える）は解けます。しかし、SMSの最終配信結果は呼び出し後に別の入口から到着するため、同報の契約だけでは原因ID2（非同期の完了管理を抱える）を解けません。受付IDごとの状態を持つ場所と、最終結果を受けてその状態だけを更新する入口が必要です。これはDと競う完成案ではなく、**一つの完成システムへ同時に必要な二つ目の境界**です。
...
|---|---|---|
| 課題ID1（在庫更新と通知手段の境界） | `StockAlert`を渡し、`DeliveryResult`を受け取る | 具体APIを知らず、1件失敗しても他を続ける |
| 課題ID2（在庫操作と非同期完了の境界） | 受付IDを台帳へ渡し、受信入口が最終状態を更新する | 在庫操作が状態表と受信入口を持たない |

3つの痛みが、1つの構造原因を経て、1つの課題へどうつながったかを一列で見ます。
...
ここで最初に確認するのは、Observerという名前ではなく**通知間の関係**です。メール、ダッシュボード、チャット、SMSは同じ在庫不足を受け取りますが、ある通知の出力を次の通知へ渡しません。失敗時も残りを続け、登録順は業務上の契約ではありません。したがって、順序付きの工程ではなく、**互いに依存しない複数先への同報**として構造化できます。

もし「メールが成功した後だけSMSを送る」「前段の結果を次段の入力にする」「途中失敗なら送信済み通知を取り消す」が要求なら、この構想は採用しません。その場合は、順序と失敗時の補償を明示するオーケストレーターやパイプラインが必要です。題材をその仕様へ変えるとObserverの説明ではなくなるため、本章では独立通知という現状の業務規則を保ちます。
...
| 生成・所有・受け渡し | `main()` がDB・ログ・通知先・後日入口を所有し、通知元へ登録する | 具体通知先とコールバックの実体・生存期間 |
| 実行 | 在庫更新は全登録先へ `send(alert)` を呼び、保留IDを後日の結果へつなぐ | 各手段の送信処理と同期方式 |

**構想上のコード経路**

1. `INotification` ―― 共通の入力と出力を約束する
2. Email・Dashboard・Chat・SMS ―― 契約を満たす具体を定義する
3. `main()` ―― 具体を選択・生成・所有して登録する
...
> **コードの読み方：** 「契約 → 具体を選択して生成 → 受け渡し → 実行」が中心経路です。その準備として、契約直後に代表的な具体クラスが本当に契約を満たせるかを確認します。変更前の抜粋は境界を引く根拠、変更後の断片は構想を成立させるコードです。断片は所属クラスとメソッドの開始・終了を残し、フェーズ7「解決後のコード」でクラス単位の全文へ統合します。
...
この契約は一つの通知だけを見て決めません。現行のメール・ダッシュボード・チャットと、追加するSMSの入力と出力をすべて並べ、共通して必要な情報を残します。

| 通知先 | 固有APIへ必要な入力 | 固有APIの結果 | 共通契約で運ぶ形 |
|---|---|---|---|
| メール | 件名、商品名、商品ID、在庫、閾値 | `bool` | `StockAlert` → `DeliveryResult(ACCEPTED/FAILED, Email)` |
| ダッシュボード | 商品ID、在庫 | 戻り値なし | `StockAlert` → `DeliveryResult(ACCEPTED, Dashboard)` |
| チャット | 投稿先、商品名、商品ID、在庫 | 投稿ID | `StockAlert` → `DeliveryResult(ACCEPTED/FAILED, Chat)` |
| SMS | 商品ID、在庫 | 受付成否、受付ID | `StockAlert` → `DeliveryResult(PENDING/FAILED, SMS, requestId)` |

商品ID・商品名・更新後在庫・閾値を `StockAlert` にそろえれば、各具体が自分に必要な項目を選び、固有API向けの文面や引数を作れます。状態・手段名・受付IDを `DeliveryResult` にそろえれば、通知元は固有の戻り値を解釈せず、成功・保留・失敗を集計できます。**共通点だけでなく、全通知先の入力と出力に不足がないことを確認してから契約を決めています。**
...
**接続点を確定したところで挙げた候補は5つです。** 商品ID、商品名、更新後在庫、閾値、個別配送結果について、境界を流す形を1つずつ確定します。

| 接続候補 | 決めた形 | 理由 |
|---|---|---|
| 商品ID・商品名・在庫・閾値 | `StockAlert` の4項目として渡す | 各手段が必要な値を使って固有の文面・表示を作れる |
...
- **「商品ID・商品名・更新後在庫・閾値を1つの値で渡す」の実態。** `a.productName`・`a.productId`・`a.stock`・`a.threshold` の4項目が、この1クラスの中で1本の文字列へ組み立てられています。現状コードではこの組み立てが通知元にありました
...
**文面を作っていません。** ダッシュボードは商品コードと在庫数という生の値しか受け取らないので、`StockAlert` の4項目のうち2つだけを使います。**そして失敗を返す道がありません。** 現状コードでは「成否が返らないので成功として数えるしかない」という割り切りを通知元が持っていました。**判断そのものは消えていません。それが妥当かどうかを判断できる場所へ移しただけです。**
...
| `SMSNotifier` | 受付ID付きの `PENDING` または `FAILED` | 最終配信の待受けと状態管理 |
...
>
> `main()`が通知先とログを生成・所有し、通知先への参照を`InventoryManager::attach()`で登録します。`InventoryManager`は具体型を知らず、登録された同じ実体へ`INotification`として通知します。非同期SMSの完了先まで含め、生成から実行までをつなぎます。
...
**この2つを満たす場所は、通知元でも通知先でもありません。新しい箱が要ります。** これから、受付IDごとの配信状態だけを持つ台帳を `DeliveryStatusLog` として定義します。そして、後日届く最終結果を受け取る入口も要ります。変更を試したときは `InventoryManager` の `receiveSMSCompletion()` が公開操作として通知元にありました。**在庫操作の入口とは別の起点なので、別のクラスへ置きます。** SMS基盤からのコールバックを受ける入口を `SMSDeliveryCallback` として定義します。
...
>
> **受付だけを結果として扱うか、最終的な配信結果まで追うか**を、要求ID6（SMS配信結果の追跡）から決めます。受付だけでは「送ったつもりで届いていない」を検知できません。最終結果を追うには、後から届く通知を別の場所で受け取る必要があります。
>
...
次の部分クラス図は、**非同期完了の状態をどこに持ち、送信時と後日受信時の両方からどう使うか**だけを示します。通知契約と各Notifierは省いた部分図です。
...
classDiagram
    ...
    InventoryManager --> DeliveryStatusLog : 受付IDを記録
...
直前の図の2本の線は、送信受付と後日結果が同じ `DeliveryStatusLog` を使うことを表します。ただし、後日受信の入口は `InventoryManager` ではなく `SMSDeliveryCallback` です。この共有と分離が、次の生成コードとクラス宣言に対応します。

**ここで確認するコード：`main()`** ―― 配信状態と後日入口の生成、そして通知元の生成
...
**ここで確認するコード：`InventoryManager::notifyAll(const StockAlert&)`** ―― 引数の到着点と、構想で決めた記録先
...
class InventoryManager {
    ...
    void notifyAll(const StockAlert& alert) {
        ...
        for (auto* observer : observers) {
            ...
            deliveryStatusLog.record(result);
...
`alert` は外から突然渡される値ではありません。次の `reduceStock()` が、商品台帳から取得した4項目を使って作り、閾値以下のときだけ `notifyAll()` へ渡します。上の抜粋では、所属クラス、メソッドの入口、注目行、終了位置を残しました。
...
public:
    void reduceStock(string productId, int quantity) {
        ...
        if (db.isBelowThreshold(productId, info.stock)) {
            StockAlert alert{productId, info.name,
...
在庫の正本を保存し、現状と同じ標準出力へ商品IDと変更前後を記録したあと、更新後在庫で閾値を判定します。通知に必要な4項目はここで `StockAlert` へまとめられ、以降の通知元は同じ値を登録先へ渡すだけです。
...
**ここで確認するコード：`InventoryManager` の `attach()`** ―― 契約だけを受け取る登録操作（引数は `INotification*`）
...
登録は組み立て時に行い、以後は同じ実体を使います。`detach()` と `attach()` で登録を変えれば次の通知から反映されます。コンストラクタで受け取る商品マスタと2つのログは別の依存であり、課題ID1（在庫更新と通知手段の境界）で登録する通知先とは混同しません。
...
| リスクID2（通知先の増減） | 登録・解除操作と所有方法 | 在庫計算を守れる。実行中の解除には生存期間規則が要る |
...
`ProductDatabase` と `ProductInfo` には差分ラベルがありません。**在庫そのものを持つ商品台帳は、通知の作り替えに巻き込まれませんでした。** `<<new>>` は5つ、`<<changed>>` は `InventoryManager` の1つです。
...
sequenceDiagram
    ...
    deactivate IM
    Note over M,CB: 時間経過を模擬<br>本番では外部受信口がこの呼び出しを行う
    M->>CB: receive("SMS-1", true)
...
通知元となる中心クラスです。`INotification*` のリストを持ち、閾値以下になると登録済みの全通知先へ同じ `send` を呼びます。SMSの `send` が返す `PENDING` と受付IDを `DeliveryStatusLog` へ渡しますが、後日の最終結果を受ける入口は持ちません。`SMSDeliveryCallback` が同じ受付IDを更新するため、在庫操作と非同期完了を切り離したまま、ヒアリングで合意した最終配信履歴まで実装できます。
...
`InventoryManager` は注入された `ProductDatabase` を唯一の在庫データとして更新し、商品IDと変更前後の数値を現状と同じログへ出します。通知時は `StockAlert` を作り、各手段へ同じ `send` を呼び、保留IDだけを状態ログへ渡します。SMSの受付失敗でも在庫更新と他通知は止まらず、最終配信失敗は後日のコールバックで状態ログだけを更新します。この例の通知ポインタは非所有なので、組み立て側が通知中の生存期間を保証します。
...
**確認したいのは、外部から見える結果を保ちながら、変更理由ごとの責任が分離されていることです。** シナリオ（行1〜行7）ごとに、実行するコードとその実行結果を並べます。要求ID4（在庫変動の追跡）は、行1・2・3・6・7の「在庫: 変更前 -> 変更後」で確認します。
...
**メールとチャットの文面が、現状コードから変わっています。** 現状コードではどちらも「商品 PRD002（USBハブ） の在庫が閾値以下です。」でした。通知元が1つの文面を作り、それを両方へ配っていたからです。対策後は通知元が文面を作りません。渡すのは `StockAlert` の4項目だけで、**それをどう文にするかは各通知先が決めます。** メールは件名と本文に分け、チャットは行動を促す一文にしています。**文面が変わったのは副作用ではなく、「手段ごとの表現を手段の側へ移す」という決定がそのまま出たものです。**

SMS基盤はこの呼び出し中に最終結果を返しません。掲載コードではプログラムを再実行せず、同じ `main()` の後続行から `SMSDeliveryCallback::receive()` を呼び、**外部結果が後の時点で届く状況を模擬**します。
...
行7の処理後に `return 0;` で `main()` を閉じています。在庫の保存方式は変更要求の対象ではないため、完成版だけに別の在庫ログクラスを追加しません。要求ID4（在庫変動の追跡）は、現状と同じ入出庫ログにある商品ID・操作数・変更前→変更後の数値で確認できます。
...
|---|---|---|
| 要求ID1（正確な在庫の維持） | `InventoryManager`、`ProductDatabase` | 商品IDと変更前→変更後の数量を出力 |
| 要求ID2（在庫不足の通知） | `InventoryManager::reduceStock()`、各Notifier | 閾値以下のとき4手段が1件ずつ受け取る |
| 要求ID3（誤更新の防止） | `InventoryManager` の入力検証 | 不正操作では在庫と通知件数を変えない |
| 要求ID4（在庫変動の追跡） | `InventoryManager::reduceStock()`、`replenishStock()` | 各成功操作に商品ID・操作数・変更前→変更後を出力 |
| 要求ID5（通知失敗から在庫を保護） | `notifyAll()`、各Notifier | SMS失敗でも他通知と在庫更新を完了する |
| 要求ID6（SMS配信結果の追跡） | `DeliveryStatusLog`、`SMSDeliveryCallback` | SMS-1は完了、SMS-2は最終失敗へ更新する |

最終要求6件がすべて合格です。

上の表は継続（要求ID1（正確な在庫の維持）・要求ID3（誤更新の防止）・要求ID4（在庫変動の追跡））、変更（要求ID2（在庫不足の通知））、追加（要求ID5（通知失敗から在庫を保護）・要求ID6（SMS配信結果の追跡））を同じ順序で並べ、変わらなかった既存要求も回帰対象に含めています。継続要求が合格していることで、既存動作が落ちていないことを確認できます。要求の受入・回帰はここで完了します。課題IDへ直接対応付けず、以下では変更試行の痛みから導いた構造課題だけを別に確認します。
...
> | `DeliveryStatusLog.h` / `.cpp` | 配信結果の記録と、後から届く結果の受け口 | 非同期の結果を扱う唯一の場所。通知元を太らせないために分けた |
...
graph TD
    ...
    T1 -->|組み立てで2行増やす| N2["［変更］ attach() と<br>SMSDeliveryCallback の生成"]:::touched
...
|---|---|---|
| 呼び分けと結果の解釈 | **修正しない** | 各通知クラスが `send()` の中で行う |
| 受付IDの保管 | **修正しない** | `DeliveryStatusLog` が受付IDごとに持つ |
| 後日の完了コールバック | 組み立てで1行増やす | `SMSDeliveryCallback` が受けて台帳を更新する |
| （新設） | `SMSNotifier` を1クラス追加する | 手段そのものは新しく作るしかない |
| 通知先の登録 | 組み立てで `attach` を1行増やす | 登録だけを増やす |

**組み立てが2行増えるのは、SMSが非同期だからです。** 同期の手段（たとえば音声通知を同期で呼ぶ場合）なら、増えるのは `attach` の1行だけです。**後日の結果を受ける入口が要るかどうかが、その差になります。**
...
|---|---|---|
| 変更ID1（非同期SMS追加） | `InventoryManager`へSMS固有処理と状態管理を追加 | SMSの実装・登録・状態ログへ分離し、失敗でも他処理を完了 |
...
| 得られる視点 | この章で確認した場所 |
|---|---|
| 変動箇所の識別 | フェーズ2で、通知先と通知方法の変わる見込みを確認した |
| 受け渡し口の診断 | フェーズ4で、通知方法と受付結果の知識を在庫管理が持っていることを確認した |
| 構造改善の説明 | フェーズ7で、通知先追加の中心が新しい通知クラスと登録へ移ったことを確認した |
| 動的な通知先管理 | フェーズ6と7で、`attach()`による登録から一律配布までを追った |

### 「はじめに」の三つの問いにどう答えたか

| 問い | この章での答え |
|---|---|
| 問い1：別々の理由で変わるものはないか | 在庫更新と、通知先ごとの呼び方・結果の翻訳が`InventoryManager`へ集まっていた |
| 問い2：何を約束すれば足りるか | `StockAlert`を渡し、`DeliveryResult`を返す`INotification::send()`へそろえた |
| 問い3：誰が作り、持ち、渡すのか | `main()`が通知先とログを生成・所有し、通知先を`attach()`で在庫管理へ登録した |
